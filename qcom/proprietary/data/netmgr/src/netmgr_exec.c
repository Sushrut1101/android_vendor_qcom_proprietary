                                   ...
                                   netmgr_ctl_port_config_type links[])
{
  ...
  {
    netmgr_log_err("%s(): Invalid link num: %d", __func__, link_num);
    NETMGR_LOG_FUNC_EXIT;
    return NETMGR_FAILURE;
  }

  /*-------------------------------------------------------------------------
    Initialize the state machine instance for this particular network link.
  -------------------------------------------------------------------------*/
  /* Skip already enabled links - return failure so we do not post INITED_EV again */
  if (links[link_num].enabled)
  {
    netmgr_log_err("%s(): Link is already enabled: %d", __func__, link_num);
    NETMGR_LOG_FUNC_EXIT;
    return NETMGR_FAILURE;
  }

  if (STM_SUCCESS !=
      stm_instance_activate(NETMGR_SM, link_num, (void*)(uintptr_t)link_num))
  {
    netmgr_log_err("%s(): Failed to initialize state machine: %d",
                   __func__, link_num);
    return NETMGR_FAILURE;
  }

  netmgr_exec_state_info.links[link_num].sm = &NETMGR_SM[link_num];
  netmgr_exec_state_info.nlink += 1;
  links[link_num].enabled = TRUE;
  links[link_num].on_demand = TRUE;

  netmgr_log_high("%s(): state machine inited for newly enabled link: %d",
                  __func__, link_num);
...
netmgr_exec_process_is_complete_msg ()
{
  ...
  if( NETMGR_GET_PROCESS_READY_REQ() > 0){
    ...
    netmgr_cb_runall(NETMGR_MAIN_CB_TABLE, NETMGR_MAIN_CB_INITDONE, NULL);
  }
