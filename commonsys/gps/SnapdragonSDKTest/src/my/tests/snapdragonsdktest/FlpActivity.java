...
public class FlpActivity extends Activity {
    ...
    protected void onDestroy() {
        if (VERBOSE) {
            Log.v(TAG, "onDestroy sessions to stop: " + mFlpSessionNames.size());
        }
        for (String name: mFlpSessionNames) {
            IZatFlpService.IZatFlpSessionHandle handle =  mSessionNameToHandleMap.get(name);
            mFlpService.stopFlpSession(handle);
        }
        super.onDestroy();
    }
    ...
    private void showAddDialog() {
        ...
        editSessionName.setText("Session " + (mFlpSessionNames.size() + 1));
        editTimeInterval.setText("1");
        editDistanceInterval.setText("0");
        editTripDistance.setText("0");
