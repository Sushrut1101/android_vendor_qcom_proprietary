...
#define SWIM_DEFAULT_T_BLACKLIST_CQE        900 //secs
...
#define SWIM_DEFAULT_RMP_UBCNT_THR            100 //Acceptable range [0, 65k]
#define SWIM_DEFAULT_TMR_UBCNT_THR            300 //Acceptable range [0, 65k]
#define SWIM_DEFAULT_PERAM_THR                0.25 //Acceptable range [0, 1]
#define SWIM_DEFAULT_PERAM_CNT_THR            10 //Acceptable range [0, 65k]
#define SWIM_DEFAULT_PERAM_UBCNT_THR          80 //Acceptable range [0, 65k]
#define SWIM_DEFAULT_CQE_TPUT_THR             1000000 //Acceptable range [0.1, 100Mbps
...
#define SWIM_RAT_SUBTYPE_MAX                19
...
typedef enum WlanChipsetType {
    WLAN_CHIPSET_TYPE_PRONTO = 0,
    WLAN_CHIPSET_TYPE_ROME,
    WLAN_CHIPSET_TYPE_HELIUM,
    WLAN_CHIPSET_TYPE_HASTINGS,
}WlanChipsetEnum;
...
struct SwimInterfaceMgrConfigType
{
    SwimInterfaceMgrConfigType()
    {
        ...
        cqeTBlacklist = SWIM_DEFAULT_T_BLACKLIST_CQE;
    }
    ...
    // Duration of CQE Blacklisting in seconds
    uint16_t cqeTBlacklist;
    ...
    bool operator== (SwimInterfaceMgrConfigType &cfg1)
    {
        return ( ( *this ).bqePassiveTestDuration == cfg1.bqePassiveTestDuration &&
            ...
            ( *this ).cqeTBlacklist == cfg1.cqeTBlacklist);
    }
...
SwimIcdMgrConfigType& operator=(const SwimIcdMgrConfigType &rhs){
    if (this != &rhs){
        this->icdDisabled = rhs.icdDisabled;
        this->icdPastSize = rhs.icdPastSize;
        this->icdValidityTimeout = rhs.icdValidityTimeout;
        this->icdTransactionTimeout = rhs.icdTransactionTimeout;
        this->icdHighProbability = rhs.icdHighProbability;
        strlcpy(this->icdUri, rhs.icdUri, SWIM_ICD_URI_STRLEN);
        strlcpy(this->icdHttpUri, rhs.icdHttpUri, SWIM_ICD_URI_STRLEN);
        this->icdMaxAuthTime = rhs.icdMaxAuthTime;
        this->icdRetestTime = rhs.icdRetestTime;
        this->icdApIds = rhs.icdApIds; // operator= for map
    }
    return *this;
}
};
...
struct SwimCQERomeConfigType
{
    ...
    unsigned int Rmp_Ubcnt_Thr;
    unsigned int Tmr_Ubcnt_Thr;
    float Peram_Thr;
    unsigned int Peram_Cnt_Thr;
    unsigned int Peram_Ubcnt_Thr;
    float Cqe_Tput_Thr;
    ...
    bool operator== (SwimCQERomeConfigType &cfg1)
    {
            ...
            (*this).Rmp_Ubcnt_Thr == cfg1.Rmp_Ubcnt_Thr &&
            (*this).Tmr_Ubcnt_Thr == cfg1.Tmr_Ubcnt_Thr &&
            (*this).Peram_Thr == cfg1.Peram_Thr &&
            (*this).Peram_Cnt_Thr == cfg1.Peram_Cnt_Thr &&
            (*this).Peram_Ubcnt_Thr == cfg1.Peram_Ubcnt_Thr &&
            (*this).Cqe_Tput_Thr == cfg1.Cqe_Tput_Thr );
    }
    bool operator!= ( SwimCQERomeConfigType &cfg1)
    {
        return !(*this == cfg1);
...
struct SwimCQEConfigType
{
    SwimCQEConfigType()
    {
        std::fill(RSSIAddThreshold, RSSIAddThreshold+FREQ_BAND_SIZE,
                SWIM_DEFAULT_RSSI_ADD_THRESHOLD );
        std::fill(RSSIDropThreshold, RSSIDropThreshold+FREQ_BAND_SIZE,
                SWIM_DEFAULT_RSSI_DROP_THRESHOLD );
        ...
        wlanChipsetType = WLAN_CHIPSET_TYPE_PRONTO;
    }
    int RSSIAddThreshold[FREQ_BAND_SIZE];
    int RSSIDropThreshold[FREQ_BAND_SIZE];
    ...
    WlanChipsetEnum wlanChipsetType;
    union
    {
    ...
    {
        bool retVal = false;
        if((*this).wlanChipsetType == WLAN_CHIPSET_TYPE_ROME ||
           (*this).wlanChipsetType == WLAN_CHIPSET_TYPE_HELIUM)
        {
        ...
        for( int i = 0; i< FREQ_BAND_SIZE; i++){
            if( ((*this).RSSIAddThreshold[i] != cfg1.RSSIAddThreshold[i]) ||
                ((*this).RSSIDropThreshold[i] != cfg1.RSSIDropThreshold[i]) ){
                retVal = false;
            }
        }
