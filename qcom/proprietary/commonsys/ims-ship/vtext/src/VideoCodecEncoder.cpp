...
{
  return AMediaCodecnativeWindow.get();
}
...
QPE_VIDEO_ERROR VideoCodecEncoder::Configure(QP_VIDEO_CONFIG tCodecConfig)
{
  ...
  do {
    ...
    if (m_codecConfig.eCodec != VIDEO_CODEC_H263) {
      ...
      // we are using vendor extension now for setting bitrate mode
      ...
      int32_t iVbvdelay = 0;
      property_get("persist.vendor.ims.vt.vbvdelay", vbvdelay, "500");
      ...
      if (BITRATE_MODE_CBR_PLUS_VFR == m_codecConfig.eBitRateMode) {
        ...
        if (((iWidth * iHeight) == VIDEO_FRAME_RES_VGA)
            ...
            || ((m_codecConfig.iOut_Width * m_codecConfig.iOut_Height) ==
                VIDEO_FRAME_RES_VGA)) {
          CRITICAL2("Configure:set CBR [%d] value for", iVbvdelay);
...
QPE_VIDEO_ERROR VideoCodecEncoder::GetEncodedFrame()
{
  ...
  if (eVideoStateActive == iRecorderState && mCodec) {
    ...
    if (status >= 0) {
      ...
      DEBUG4("GetEncodedFrame: dequeueoutputbuffer buffer size %d index %d offset %d",
      ...
      CRITICAL5_STR("GetEncodedFrame: FTB_DONE : data[4] : 0x%x Frame num %d, Time: %llu, size %d",
                    buf[4], m_ulFrameCount,
                    (unsigned long long)info.presentationTimeUs, info.size);
  ...
  switch (m_codecConfig.eCodec) {
    ...
    case VIDEO_CODEC_H264:
      ...
      if (m_codecConfig.eH264ProfileType == VIDEO_AVC_PROFILE_BASELINE) {
        ...
        CRITICAL3("Configure: H264 profile set to 0x%x, eH264ProfileType %d",
