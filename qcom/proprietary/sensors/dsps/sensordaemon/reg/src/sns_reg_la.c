 ...
 */
static char sns_reg_filename[255] = "";
...
sns_reg_storage_init( void )
{
  ...
  char *sns_reg_filenames[] = {SNS_REG_DATA_FILENAME,
                               SNS_REG_DATA_FILENAME_LEGACY};

  for (int index = 0; index < 2; index++) {
    if (SNS_SUCCESS == sns_fsa_realpath(sns_reg_filenames[index])) {
      strlcpy(sns_reg_filename, sns_reg_filenames[index], strlen(sns_reg_filenames[index]) + 1);
      break;
    }
  }
#ifdef SNS_LA_SIM
