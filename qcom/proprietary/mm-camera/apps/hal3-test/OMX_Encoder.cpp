    ...
    m_IsInit(0)
{
    ...
    while (omxresult == OMX_ErrorNone) {
        if (mComponentNameEnum != NULL) {
            omxresult = mComponentNameEnum(m_SupportComponents[idx], sizeof(m_SupportComponents[idx]), idx);
        }
        else {
...
OMX_ERRORTYPE QCameraOMX_Encoder::setConfig(omx_config_t *config, QCameraBufferHolder *holder)
{
    ...
    if (mGetHandle != NULL)
    {
        omxresult = mGetHandle(&m_OmxHandle, m_Config.componentName, this, &cb);
    }
    else
    {
        omxresult = OMX_ErrorUndefined;
