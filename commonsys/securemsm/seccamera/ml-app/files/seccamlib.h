...
// Source camera definition as provided by HLOS side.
typedef enum {
    SOURCE_CAPTURE = 1,
    SOURCE_PREVIEW = 0,
...
// Messages IDs used with the ML app
typedef enum {
...
// Seccam command id (expected to be the first int32 of any
// command request structure passed to the seccamlib)
typedef struct {
    uint32_t cmd_id;
} mlvmapp_generic_req_t;

// Response error code (will be first int32 of any response
// structure returned to HLOS caller)
typedef struct {
    mlvmapp_error_t ret;
...
// Note: since common data structures are used for different Apps,
// setting it to 2 (for MLApp only one camera is supported).
...
typedef struct {
    uint64_t buffer_handle; //physical address of the buffer
 ...
 * hyp again
 */
mlvmapp_error_t start_cmd_handler(void *rcv_buf, uint32_t rcv_size,
 ...
 * hyp again
 */
mlvmapp_error_t stop_cmd_handler(void *rcv_buf, uint32_t rcv_size,
                                 void *rsp_buf, uint32_t *rsp_size);
...
/**
 * De-initializes the socket communication for receiving commands from
 * seccamservice in HLOS. Here ML app behaves as a server and HLOS
 * seccamservice as client.
 *
 * @return
 * MLVM_APP_E_SUCCESS                   -- Successful. \n
 * Other mlvmapp_error_t error value   -- Error occurred.
 *
 * @dependencies
 *
 */
mlvmapp_error_t sock_deinit(void);
 ...
 *
 */
mlvmapp_error_t init_cmd_handler(void *rcv_buf, uint32_t rcv_size,
                                 void *rsp_buf, uint32_t *rsp_size);
 ...
 *
 */
mlvmapp_error_t deinit_cmd_handler(void *rcv_buf, uint32_t rcv_size,
                                   void *rsp_buf, uint32_t *rsp_size);
