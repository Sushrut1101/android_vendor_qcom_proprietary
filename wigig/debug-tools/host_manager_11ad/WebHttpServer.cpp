...
{
    string contentType;
    std::transform(fileName.begin(), fileName.end(), fileName.begin(), [](char c) { return static_cast<char>(::tolower(c)); });
...
string WebHttpServer::GetHostDataJson()
{
    ...
    host["HostAlias"] = data.m_hostName;
...
{
    (void)command;
    (void)pCommandsExecutor;

    // Host alias is no more configured on host. Real hostname is used instead.
    throw invalid_argument("Custom host alias is not support. Only real host name may be used");
}
