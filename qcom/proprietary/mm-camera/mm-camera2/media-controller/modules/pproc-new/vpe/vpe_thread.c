        ...
        if(pollfds[i].revents & (POLLIN|POLLPRI)) {
          ...
          case PIPE_FD_IDX: {
            ...
            if(num_read < 0) {
              ...
              CDBG_ERROR("%s:%d, error in read(), num_read=%d, msg_size=%zu",
  ...
  vpe_module_event_t* vpe_event)
{
  ...
  return 0;
#endif
  key = (vpe_module_ack_key_t *) CAM_MALLOC(sizeof(vpe_module_ack_t));
  if(!key) {
  ...
  if (vpe_event->u.process_buf_data.isp_buf_divert.native_buf) {
    in_frame_fd = (unsigned long)vpe_event->u.process_buf_data.isp_buf_divert.fd;
  } else {
    in_frame_fd =
