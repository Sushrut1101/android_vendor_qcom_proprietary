...
VehicleNetworkProvider::VehicleNetworkProvider()
    : MultiplexingProvider(SLIM_PROVIDER_VNW, true, 0)
...
{
  static bool sCANBufferIsReleased = false;
  SLIM_LOGD("eService: %d uEnable: %d",eService, uEnable);
  MutexLock _l(m_zMutex);
  if (uEnable)
  {
    SLIM_MASK_SET(m_qEnabledServices, eService);
    if (false == sCANBufferIsReleased) {
