...
extern void RadioConfigProcessSlotStatusInd(const qmi_uim_slots_status_type *status_rsp_ptr);
...
)
{
  ...
  if ((res < 0) || (pin_rsp_data.qmi_err_code != QMI_ERR_NONE))
  {
    ...
    return QCRIL_UIM_RIL_E_SIM_ERR;
  }
...
)
{
     ...
     (pin_data_ptr->data_len <= QMI_UIM_MAX_ENCRYPTED_PIN_LEN))
  {
    ...
    if(qcril_uim.card_iccid_info.iccid_len > 0 &&
       qcril_uim.card_iccid_info.iccid_len <= QCRIL_UIM_ICCID_LEN)
    {
      memcpy(qcril_uim.pin1_info[pin_index].iccid_data,
             qcril_uim.card_iccid_info.iccid_data,
             qcril_uim.card_iccid_info.iccid_len);
      qcril_uim.pin1_info[pin_index].iccid_len = qcril_uim.card_iccid_info.iccid_len;
    }
#else
...
)
{
...
#ifdef QMI_RIL_UTF
  RIL_Errno                              read_status   = QCRIL_UIM_RIL_E_INTERNAL_ERR;
  uint8                                  iccid_len     = QCRIL_UIM_ICCID_LEN;
  uint8                                  iccid_data[QCRIL_UIM_ICCID_LEN];
  uint8                                  err_count     = 0;
#endif /* QMI_RIL_UTF */
...
#ifndef QMI_RIL_UTF
  if((qcril_uim.pin1_info[0].iccid_len == qcril_uim.card_iccid_info.iccid_len) &&
  ...
  else if((qcril_uim.pin1_info[1].iccid_len == qcril_uim.card_iccid_info.iccid_len) &&
...
)
{
  ...
  qcril_uim_update_gstk_card_event(params_ptr->instance_id,
                                   params_ptr->modem_id,
                                   slot);
  ...
  /* Release QCRIL UIM REMOTE service handles */
  qcril_uim_remote_release();

  /* Release QCRIL UIM HTTP service handles */
  ...
  /* send card present false to SecureElement */
  SecureElementhandleCardStateChange(false);
...
)
{
  ...
  /* Re-init QCRIL UIM Remote */
  qcril_uim_remote_init();

  /* Re-init QCRIL UIM HTTP */
  qcril_uim_http_init();
