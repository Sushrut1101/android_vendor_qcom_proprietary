...
#define MSG_SSID_QCNEA_LO                    10379
...
#define CNE_MSG_SUBTYPE_QCNEA_LO            MSG_SSID_QCNEA_LO            // Latency Optimization


#define CNE_MSG_SUBTYPE_NO_TAG_DEFINED  0                                // default sub-type (should not be used)

// Define a flag to set runtime debug logging
#define CNE_LOG_QXDM_PERSIST_PROPERTY "persist.vendor.cne.logging.qxdm"
...
class CneLog
{
  public:
     ...
     * SIDE EFFECTS  None
     *--------------------------------------------------------------------------*/
    virtual void printLog( int level, int ssid, const char *filename, int linenum, const char *fmt, ... );
     ...
     * SIDE EFFECTS  None
     *--------------------------------------------------------------------------*/
    virtual void printReleaseLog( int level, int ssid, const char *filename, int linenum, const char *fmt, ... );
     ...
     * DEPENDENCIES  None
     *
     * RETURN VALUE  persist.vendor.cne.logging property value
     *
     * SIDE EFFECTS  None
     *--------------------------------------------------------------------------*/
  ...
  protected:
     ...
     * SIDE EFFECTS  None
     *--------------------------------------------------------------------------*/
    void printToQXDM( int level, int ssid, const char *filename, int linenum, const char *fmt, va_list ap );
    ...
    const char * subtypeToStr( const int ssid ){
      switch( ssid ){
        ...
        case CNE_MSG_SUBTYPE_QCNEA_LO:             return "LO";
...
class CneLogDiagAdditional : public CneLog
{
  public:
     ...
     * SIDE EFFECTS  None
     *--------------------------------------------------------------------------*/
    void printLog( int level, int ssid, const char *filename, int linenum, const char *fmt, ... );
};
...
class CneLogAdb : public CneLog
{
  public:
     ...
     * SIDE EFFECTS  None
     *--------------------------------------------------------------------------*/
    void printReleaseLog( int level, int ssid, const char *filename, int linenum, const char *fmt, ... );
};
#endif /* CNE_LOG_H */
