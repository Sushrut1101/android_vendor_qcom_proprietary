...
#define ABF48_VERSION "48"

#define ABF_MODULE_NAME(n) \
  "abf"n

static boolean module_abf48_set_stream_config(mct_module_t *module,
  isp_sub_module_t *isp_sub_module, mct_event_t *event);

static boolean module_abf48_module_cfg(mct_module_t *module,
  isp_sub_module_t *isp_sub_module, mct_event_t *event);

static isp_sub_module_private_func_t abf48_private_func = {
  .isp_sub_module_init_data              = abf48_init,
  .isp_sub_module_destroy                = abf48_destroy,

  .control_event_handler = {
    [ISP_CONTROL_EVENT_STREAMON]         = isp_sub_module_port_streamon,
    [ISP_CONTROL_EVENT_STREAMOFF]        = abf48_streamoff,
  ...
  .module_event_handler = {
    ...
    [ISP_MODULE_EVENT_SET_STREAM_CONFIG] =
      module_abf48_set_stream_config,
  ...
  .isp_private_event_handler = {
    ...
    [ISP_PRIVATE_SET_TRIGGER_UPDATE]     = abf48_trigger_update,
    [ISP_PRIVATE_EVENT_GET_MOD_CFG_MASK] = module_abf48_module_cfg,
  },
 ...
 **/
static mct_module_t *module_abf48_init(const char *name)
{
  boolean                ret = TRUE;
  ...
  if (strncmp(name, ABF_MODULE_NAME(ABF48_VERSION),
      strlen(name))) {
    ISP_ERR("failed: invalid name %s expected %s", name,
      ABF_MODULE_NAME(ABF48_VERSION));
    return NULL;
  }

  module = isp_sub_module_init(name, NUM_SINK_PORTS, NUM_SOURCE_PORTS,
    &abf48_private_func, ISP_MOD_ABF, "abf", ISP_LOG_ABF);
...
/** module_abf48_module_cfg:
 *
 *  @module: abf48 module pointer
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  ret = abf48_decide_hysterisis(isp_sub_module);
  ...
  enable_bit_info->enableLPM = !(abf->filterEn);
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  return abf48_set_stream_config(isp_sub_module,
 ...
 *  Returns: void
 **/
static void module_abf48_deinit(mct_module_t *module)
{
  if (!module) {
...
static isp_submod_init_table_t submod_init_table = {
  .module_init = module_abf48_init,
  .module_deinit = module_abf48_deinit,
};
