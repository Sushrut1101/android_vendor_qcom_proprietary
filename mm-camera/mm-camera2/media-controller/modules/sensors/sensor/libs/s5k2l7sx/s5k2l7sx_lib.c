...
#include <string.h>
#include "debug_lib.h"
#include "s5k2l7sx_lib.h"

 /**
 * FUNCTION: s5k2l7sx_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static unsigned int s5k2l7sx_real_to_register_gain(float real_gain)
{
    unsigned int reg_gain = 0;

    if (real_gain < S5K2L7SX_MIN_AGAIN)
    {
        real_gain = S5K2L7SX_MIN_AGAIN;
    }
    else if (real_gain > S5K2L7SX_MAX_AGAIN)
    {
        real_gain = S5K2L7SX_MAX_AGAIN;
    }
...
/**
 * FUNCTION: s5k2l7sx_register_to_real_gain
 *
 * DESCRIPTION: Calcuate the sensor exposure
 **/
static float s5k2l7sx_register_to_real_gain(unsigned int reg_gain)
{
    float real_gain;

    if (reg_gain > S5K2L7SX_MAX_AGAIN_REG_VAL)
    {
        reg_gain = S5K2L7SX_MAX_AGAIN_REG_VAL;
    }
...
/**
 * FUNCTION: s5k2l7sx_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static unsigned int s5k2l7sx_digital_gain_calc(float real_gain, float sensor_real_gain)
{
    unsigned int reg_dig_gain = S5K2L7SX_MIN_DGAIN_REG_VAL;
    float real_dig_gain = S5K2L7SX_MIN_DGAIN;
    ...
    {
        real_dig_gain = S5K2L7SX_MIN_DGAIN;
    }

    if(real_dig_gain > S5K2L7SX_MAX_DGAIN)
    {
        real_dig_gain = S5K2L7SX_MAX_DGAIN;
    }
...
/**
 * FUNCTION: s5k2l7sx_calculate_exposure
 *
 * DESCRIPTION: Calcuate the sensor exposure
 **/
  ...
  __attribute__((unused)) float s_real_gain)
{
    ...
    exp_info->reg_gain = s5k2l7sx_real_to_register_gain(real_gain);
    exp_info->sensor_real_gain = s5k2l7sx_register_to_real_gain(exp_info->reg_gain);
    exp_info->sensor_digital_gain =
      s5k2l7sx_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
    exp_info->sensor_real_dig_gain = (float)exp_info->sensor_digital_gain / 256.0;
...
/**
 * FUNCTION: s5k2l7sx_fill_exposure_array
 *
 * DESCRIPTION: Fill the sensor exposure array
 **/
int32_t sensor_fill_exposure_array(uint32_t gain,
    ...
    __attribute__((unused)) int is_hdr_enabled)
{
    ...
    reg_setting->reg_setting[reg_count].reg_addr = S5K2L7SX_DIG_GAIN_GR_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = S5K2L7SX_DIG_GAIN_GR_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = S5K2L7SX_DIG_GAIN_R_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = S5K2L7SX_DIG_GAIN_R_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = S5K2L7SX_DIG_GAIN_B_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = S5K2L7SX_DIG_GAIN_B_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = S5K2L7SX_DIG_GAIN_GB_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = S5K2L7SX_DIG_GAIN_GB_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;
