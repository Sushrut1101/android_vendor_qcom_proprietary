...
int BufferManager::GetBuffer(ISource *pNode,
        OMX_U32 ePortIndex, OMX_BUFFERHEADERTYPE *pHeader,
        BufferInfo **pBuffer, BufferPool **ppBufferPool) {

    OMX_U32 i = -1;
    BufferPool *pBufferPool;

    pBufferPool = GetBufferPool(pNode, ePortIndex);
    if (pBufferPool == NULL) {
    ...
    for (i = 0; i < pBufferPool->nBufferCount; i++) {
...
OMX_ERRORTYPE BufferManager::FreeBuffers(ISource *pNode, OMX_U32 ePortIndex) {
    ...
    if (pBufferPool->pBuffers != NULL) {
        ...
        pBufferPool->pUseNode->pSource->FreeUsedBuffers(
                &pBufferPool->pBuffers, &(pBufferPool->pExtraDataBuffer[0]),
                pBufferPool->nBufferCount, pBufferPool->nExtraDataBufferCount,
...
OMX_ERRORTYPE BufferManager::SetupBufferPool(ISource *pNodeSrc, ISource *pNodeSink) {
    ...
    OMX_U32 nExtraDataBufferCount = 0;
    OMX_U32 nExtraDataBufferSize = 0;
    ...
    nExtraDataBufferCount = MAX(sPortCapSrc.nExtraDataBufferCount, sPortCapSink.nExtraDataBufferCount);
    nExtraDataBufferSize = MAX(sPortCapSrc.nExtraDataBufferSize, sPortCapSink.nExtraDataBufferSize);
    ...
    pBufferPool->nExtraDataBufferCount = nExtraDataBufferCount;
    pBufferPool->nExtraDataBufferSize = nExtraDataBufferSize;
    ...
    if (pBuffers == NULL) {
        result = pBufferPool->pAllocateNode->pSource->AllocateBuffers(
                    &pBufferPool->pBuffers,
                    &(pBufferPool->pExtraDataBuffer[0]),
                    pBufferPool->nWidth,
                    pBufferPool->nHeight,
                    pBufferPool->nBufferCount,
                    pBufferPool->nBufferSize,
                    pBufferPool->nExtraDataBufferCount,
                    pBufferPool->nExtraDataBufferSize,
                    pBufferPool->pAllocateNode->ePortIndex,
    ...
    result = pBufferPool->pUseNode->pSource->UseBuffers(
                &pBufferPool->pBuffers,
                &(pBufferPool->pExtraDataBuffer[0]),
                pBufferPool->nWidth,
                pBufferPool->nHeight,
