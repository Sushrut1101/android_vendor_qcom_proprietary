...
#define CORE_CTL_MIN "core_ctl_min_cluster_%d"
#define CORE_CTL_MAX "core_ctl_max_cluster_%d"
    ...
    if (ftell(defval) == 0) {
        ...
        if (d.core_ctl_present > 0) {
            for (int i =0; i < MAX_CLUSTER; i++) {
                ...
                if (rc > 0) {
                    snprintf(tmp, NODE_MAX, CORE_CTL_MAX, i);
                    QLOGI("Updating %s with %s", d.core_ctl_max_cpu_node, tmp);
                ...
                if (rc > 0) {
                    snprintf(tmp, NODE_MAX, CORE_CTL_MIN, i);
                    QLOGI("Updating %s with %s", d.core_ctl_min_cpu_node, tmp);
