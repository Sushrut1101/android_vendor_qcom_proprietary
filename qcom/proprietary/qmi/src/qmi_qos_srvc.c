/******************************************************************************
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2010-2012,2014,2018 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
#define QMI_QOS_EVENT_BEARER_ID_TLV_ID                    0x16
...
/*===========================================================================
  FUNCTION  qmi_qos_srvc_extract_return_code
===========================================================================*/
/*!
@brief
...
)
{
  ...
  while (msg_size > 0)
  {
    ...
    if (QMI_RESULT_CODE_TYPE_ID == type)
    {
      if(length != SIZE_32_BIT_VAL)
      {
        goto bail;
      }
      ...
      case QMI_QOS_FILTER_PRECEDENCE_REQ_RSP_TLV_ID:
        {
          ...
          READ_16_BIT_VAL (value_ptr, filter_ind_data->qos_filter.filter_desc.precedence);
        }
        break;
      case QMI_QOS_FILTER_FILTER_ID_REQ_RSP_TLV_ID:
        {
          ...
          READ_16_BIT_VAL (value_ptr, filter_ind_data->qos_filter.filter_desc.filter_id);
        }
        break;
...
)
{
  ...
  while (msg_buf_len > 0)
  {
    ...
    if(filter_index >= QMI_QOS_MAX_FLOW_FILTER)
    {
      return QMI_INTERNAL_ERR;
    }
...
)
{
  ...
  while (rx_buf_len > 0)
  {
    ...
    switch (type)
    {
      ...
      case QMI_QOS_EVENT_BEARER_ID_TLV_ID:
        {
          if(length != SIZE_8_BIT_VAL)
          {
            return QMI_INTERNAL_ERR;
          }
          READ_8_BIT_VAL( value_ptr, ind_data->flow_info[index].bearer_id );
          ind_data->flow_info[index].param_mask |= QMI_QOS_EVENT_REPORT_FLOW_INFO_BEARER_ID_PARAM;
        }
        break;
...
/*===========================================================================
  FUNCTION  qmi_qos_srvc_process_nested_event_report_ind
===========================================================================*/
/*!
@brief
...
{
  short int num_flows_counter = 0;
  unsigned long type = 0;
  unsigned long length = 0;
  unsigned char *value_ptr = NULL;

  ind_data->num_flows = 0;
  while (rx_msg_len > 0)
  {
    ...
    if (type == QMI_QOS_EVENT_FLOW_INFO_TLV_ID)
    {
      ...
      /*
        ...
        filled in.
      */
      ind_data->param_mask |= QMI_QOS_EVENT_REPORT_GLOBAL_FLOW_INFO_PARAM;
      num_flows_counter++;
      ind_data->num_flows = num_flows_counter;
    }
    else if (type == QMI_QOS_NW_SUPPORTED_QOS_PROFILES_TLV_ID)
    {
      int num_instances = 0;
      int index;
      ind_data->param_mask |= QMI_QOS_EVENT_REPORT_NW_SUPPORTED_QOS_PROFILES_PARAM;
      READ_16_BIT_VAL (value_ptr, ind_data->nw_supported_qos_profiles.iface_type);
      READ_8_BIT_VAL (value_ptr,ind_data->nw_supported_qos_profiles.num_profiles);
      num_instances = ind_data->nw_supported_qos_profiles.num_profiles;

      if(num_instances > QMI_QOS_MAX_PROFILES)
      {
        QMI_ERR_MSG_1( "qmi_qos_srvc_indication_cb: num_profiles [%d] > QMI_QOS_MAX_PROFILES\n",
                       num_instances );
        continue;
      }
      ...
      {
        READ_16_BIT_VAL(value_ptr,ind_data->nw_supported_qos_profiles.profile[index]);
      }
    }
    else
    {
    ...
    case QMI_QOS_EVENT_REPORT_IND_MSG_ID:
      {
        ...
        qmi_qos_srvc_process_nested_event_report_ind(rx_msg_buf,
                                                     rx_msg_len,
    ...
    case QMI_QOS_PRIMARY_QOS_EVENT_IND_MSG_ID:
      {
        ...
        if (type != QMI_QOS_STATUS_PRIMARY_QOS_MODIFY_RESULT_TLV_ID
            || length != SIZE_16_BIT_VAL)
        {
          QMI_ERR_MSG_2( "qmi_qos_srvc_indication_cb:: Unknown TLV ID=%x or mismatched length=%d",
                         (unsigned int)type,(int)length);
          return;
        }
...
)
{
  ...
  {
    tmp_buf_ptr = tmp_buf;
    WRITE_16_BIT_VAL (tmp_buf_ptr, filter_data->filter_desc.precedence);
  ...
  {
    tmp_buf_ptr = tmp_buf;
    WRITE_16_BIT_VAL (tmp_buf_ptr, filter_data->filter_desc.filter_id);
...
)
{
  ...
  for (spec_index=0; spec_index < num_qos_specs; spec_index++)
  {
    ...
    if ((rc = qmi_qos_format_spec_req_tlvs (msg,
                                            msg_size,
                                            spec_index,
                                            qos_spec_array++,
...
{
  int spec_index, rc = QMI_INTERNAL_ERR;
...
{
  int i, rc = QMI_INTERNAL_ERR;
  unsigned char tmp_buf[50], *tmp_buf_ptr;
  ...
  /* Prepare the TLV */
  if ((rc = qmi_util_write_std_tlv (msg,
                                    msg_size,
...
)
{
  ...
  if (type != QMI_QOS_STATUS_INFO_TLV_ID
      || length == SIZE_8_BIT_VAL)
  {
    QMI_ERR_MSG_2( "qmi_qos_process_get_status_resp: Invalid TLV %lx or size %d",
                   type, length );
    return QMI_INTERNAL_ERR;
  }
...
)
{
  ...
  {
    tmp_msg_ptr = msg;
    if(QMI_NO_ERR != qmi_qos_process_get_status_resp(&tmp_msg_ptr,
                                                           &msg_size,
                                                           status_resp))
    {
      return QMI_INTERNAL_ERR;
...
/*===========================================================================
  FUNCTION  qmi_qos_process_get_filter_params_resp
===========================================================================*/
/*!
@brief
  Process the response for QMI_QOS_GET_FILTER_PARAMS_MSG_ID
      ...
      case QMI_QOS_GET_FILTER_PARAMS_TX_FILTER_DATA_TLV_ID:
        {
          if (qmi_qos_srvc_event_prcss_nstd_filter_tlvs(value_ptr,
                                                        ...
                                                        (unsigned int *)&granted_info->tx_filter_count) < 0)
          {
            return QMI_INTERNAL_ERR;
          }
        }
      ...
      case QMI_QOS_GET_FILTER_PARAMS_RX_FILTER_DATA_TLV_ID:
        {
          if (qmi_qos_srvc_event_prcss_nstd_filter_tlvs(value_ptr,
                                                        ...
                                                        (unsigned int *)&granted_info->rx_filter_count) < 0)
          {
            return QMI_INTERNAL_ERR;
          }
        }
...
/*===========================================================================
  FUNCTION  qmi_qos_process_get_primary_granted_qos_req
===========================================================================*/
/*!
@brief
  This function updates the given given message buffer with the required TLVs
...
)
{
  ...
  if (QMI_NO_ERR == rc)
  {
    rc = qmi_qos_process_get_filter_params_resp(msg, msg_size, granted_info);
  }
...
/*===========================================================================
  FUNCTION  qmi_qos_process_get_granted_qos_resp
===========================================================================*/
/*!
@brief
  Process the response for QMI_QOS_GET_GRNTD_QOS_INFO_MSG_ID
...
)
{
  ...
  if (QMI_NO_ERR == rc)
  {
    rc = qmi_qos_process_get_granted_qos_resp(msg, msg_size, granted_info); 
...
/*===========================================================================
  FUNCTION  qmi_qos_process_get_secondary_granted_qos_req
===========================================================================*/
/*!
@brief
  This function updates the given given message buffer with the required TLVs
...
{
  int index, tlv_length, rc = QMI_INTERNAL_ERR;
  unsigned char tmp_buf[50], *tmp_buf_ptr;
  ...
  {
    WRITE_32_BIT_VAL(tmp_buf_ptr, qos_id_array[index]);
  }
        ...
        case QMI_QOS_FLOW_OP_INACTIVITY_TIMER_RESP_TLV_ID:
          {
            ...
            READ_32_BIT_VAL(value_ptr,resp_data->inactivity_timer);
          }
          break;
        default:
          {
