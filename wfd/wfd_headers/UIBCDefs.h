...
typedef enum uibcDeviceType : int {
    UIBC_SOURCE,
    UIBC_PRIMARY_SINK,
    UIBC_SECONDARY_SINK,
    UIBC_UNKNOWN
...
#define   UIBC_CAMERA    BIT5
#define   GESTURE        BIT6
#define   REMOTECONTROL  BIT7
#define   STYLUS         BIT8
#define   GYROSCOPE      BIT9
#define   ACCELEROMETER  BIT10
#define   VENDORSPECIFIC BIT31
...
typedef struct WFD_uibc_capability_config_t
{
  uint16    category;              /* 16 possible values types defined,
                                    * 2-(14)15 reserved  depending on profile*/
...
typedef struct WFD_uibc_capability_t
{
  ...
  int                           profile;    /* WFD Spec Profile */
} WFD_uibc_capability;

typedef enum ProfileType: int{
  R1,
  R2,
  AllWfdProfiles,
}ProfileType;
...
typedef struct WFD_uibc_touch_event_parms_t
{
...
typedef struct WFD_uibc_event_t
{
