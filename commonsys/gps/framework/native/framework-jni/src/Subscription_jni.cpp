...
using ILocHidlIzatSubscriptionV1_1 = ::vendor::qti::gnss::V1_1::ILocHidlIzatSubscription;
...
static void classInit(JNIEnv* env, jclass clazz)
{
    ...
    registerDeathRecipient("IzatSubscription", []() {
            Mutex::Autolock autoLock(sLock);
            gnssSubscriptionIface = nullptr;
            ALOGE("ILocHidlIzatSubscription died");
            instanceInit(nullptr, nullptr);
    });
}
        ...
        JNIEnv* /*env*/, jobject /*obj*/, jint batterypct)
{
    ...
    do {
        Mutex::Autolock autoLock(sLock);
        sp<ILocHidlIzatSubscriptionV1_1> iface =
            ILocHidlIzatSubscriptionV1_1::castFrom(getSubscriptionIface());
        BREAK_IF_ZERO(1, iface != nullptr,
                "gnssSubscriptionIface null !");

        ILocHidlIzatSubscriptionV1_1::BatteryLevelDataItem dataItem;
