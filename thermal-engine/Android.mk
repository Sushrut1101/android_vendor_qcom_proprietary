...
BOARD_PLATFORM_LIST += sdm710
BOARD_PLATFORM_LIST += qcs605
BOARD_PLATFORM_LIST += msmnile
BOARD_PLATFORM_LIST += sdmshrike
BOARD_PLATFORM_LIST += $(MSMSTEPPE)
BOARD_PLATFORM_LIST += $(TRINKET)
BOARD_PLATFORM_LIST += kona
BOARD_PLATFORM_LIST += lito
BOARD_PLATFORM_LIST += atoll
BOARD_PLATFORM_LIST += bengal
...
IS_SDM710:=$(strip $(call is-board-platform,sdm710))
IS_QCS605:=$(strip $(call is-board-platform,qcs605))
IS_MSMNILE:=$(strip $(call is-board-platform,msmnile))
IS_SDMSHRIKE:=$(strip $(call is-board-platform,sdmshrike))
IS_MSMSTEPPE:=$(strip $(call is-board-platform,$(MSMSTEPPE)))
IS_TRINKET:=$(strip $(call is-board-platform,$(TRINKET)))
IS_KONA:=$(strip $(call is-board-platform,kona))
IS_LITO:=$(strip $(call is-board-platform,lito))
IS_ATOLL:=$(strip $(call is-board-platform,atoll))
IS_BENGAL:=$(strip $(call is-board-platform,bengal))
...
ifneq (,$(IS_MSM8994)$(IS_MSM8992)$(IS_MSM8996)$(IS_MSM8996GVMQ)$(IS_MSM8998)$(IS_APQ8098LATV))
...
ifneq (,$(IS_MSM8909)$(IS_MSM8952)$(IS_MSM8953)$(IS_MSM8937))
...
ifneq (,$(IS_MSM8937))
ENABLE_BW_SENSOR:=1
LOCAL_CFLAGS += -DENABLE_CAMERA_REG_BW_CALLBACK
endif

ifneq (,$(IS_SDM845)$(IS_QCS605)$(IS_SDM710)$(IS_MSMNILE)$(IS_SDMSHRIKE)$(IS_MSMSTEPPE)$(IS_TRINKET)$(IS_KONA)$(IS_LITO)$(IS_ATOLL)$(IS_BENGAL))
LOCAL_SRC_FILES += ./thermal_monitor-data.c ./sensors/sensors-bcl.c \
                   ./sensors/sensors-tsens.c ./sensors/sensors-adc.c \
                   ./sensors/sensors_manager.c ./sensors/sensors.c \
                   ./devices/devices_manager.c ./devices/devices.c \
...
ifneq (,$(IS_KONA)$(IS_LITO)$(IS_ATOLL)$(IS_BENGAL))
LOCAL_INIT_RC := init_thermal-engine.rc
endif

ifneq (,$(IS_KONA)$(IS_LITO))
ENABLE_LIMITS_PARTITION:=1
endif

ifneq ($(ENABLE_LIMITS_PARTITION),)
LOCAL_CFLAGS += -DENABLE_LIMITS_PARTITION
LOCAL_SRC_FILES += ./limits_partition_write.c
endif

ifneq (,$(IS_SDM845)$(IS_SDM710))
ENABLE_MODEM_TS_QMI:=1
LOCAL_CFLAGS += -DENABLE_MODEM_QFE_SENSOR
endif

ifneq (,$(IS_QCS605)$(IS_MSMSTEPPE)$(IS_TRINKET))
ENABLE_HVX_FASTRPC_MITIGATION:=1
endif

ifneq (,$(IS_TRINKET)$(IS_LITO)$(IS_BENGAL))
ENABLE_BW_SENSOR:=1
endif
...
LOCAL_SRC_FILES += ./thermal_kernel_config.c ./thermal_kernel_monitor.c
LOCAL_SRC_FILES += ./devices/cooling_devices.c
else
INCLUDE_IOCTL_LIB:=1
LOCAL_SHARED_LIBRARIES += libcutils libthermalioctl liblog libdl
endif

ifneq ($(ENABLE_BW_SENSOR),)
LOCAL_CFLAGS += -DENABLE_BW_SENSOR
LOCAL_SRC_FILES += ./sensors/sensors-bandwidth.c
endif
...
ifneq ($(ENABLE_HVX_FASTRPC_MITIGATION),)
LOCAL_CFLAGS += -DENABLE_HVX_FASTRPC_MITIGATION
