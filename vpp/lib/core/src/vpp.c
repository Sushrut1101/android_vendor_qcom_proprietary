...
#define VPP_LOG_DEFAULT (VPP_LOG(PIPELINE) | VPP_LOG(GPU) | VPP_LOG(HVX) | \
                         ...
                         VPP_LOG(FRC_MC) | VPP_LOG(FRC_CORE))
...
static void vVpp_InputBufferDone(void *pv, t_StVppBuf *pstBuf)
{
    ...
    u32 = u32VppBuf_InternalPut(pstCtx, pstBuf, &pVppExtBuf);
    if (u32 != VPP_OK)
        return;
                                 ...
                                 t_StVppBuf *pstIntBuf)
{
    ...
    // Tag buffer with calculated frame rate
    pstIntBuf->u32FrameRate = u32AvgFrameRate;
...
static void vVpp_Event(void *pv, t_StVppEvt stEvt)
{
    ...
    else if (stEvt.eType == VPP_EVT_RECONFIG_DONE)
    {
        ...
        u32VppBuf_FlushBufferCache(pstCtx);
#endif
    }
...
void vpp_term(void *ctx)
{
    ...
    if (VPP_STATE_EQUAL(pstCb, VPP_STATE_ACTIVE))
    {
        ...
        if (u32Ret != VPP_OK)
        {
            LOGE("error closing pipeline, u32Ret=%u", u32Ret);
        }
    }
    ...
    if (u32Ret != VPP_OK)
    {
        LOGE("error term utils, u32Ret=%u", u32Ret);
    }

    u32Ret = u32VppPipeline_Term(VPP_CTX_GET(ctx));
    if (u32Ret != VPP_OK)
    {
        LOGE("error term pipeline, u32Ret=%u", u32Ret);
    }

    u32Ret = u32VppBuf_Term(VPP_CTX_GET(ctx));
    if (u32Ret != VPP_OK)
    {
        LOGE("error term buffer pool, u32Ret=%u", u32Ret);
    }

    u32Ret = u32VppIon_Term(VPP_CTX_GET(ctx));
    if (u32Ret != VPP_OK)
    {
        LOGE("error ion term, u32Ret=%u", u32Ret);
    }
    ...
    if (u32Ret != VPP_OK)
    {
        LOGE("error stats term, u32Ret=%u", u32Ret);
    }
            ...
            switch (ctrl.ctrl_type)
            {
                ...
                case HQV_CONTROL_AIE:
                    if(ctrl.mode == HQV_MODE_MANUAL)
                    {
                        ...
                        {
                            LOGE("AIE mode %d or HUE mode %d out of range",
                                 ctrl.aie.mode, ctrl.aie.hue_mode);
                            LOG_EXIT_RET(VPP_ERR_PARAM);
                        }
                        ctrl.aie.cade_level = RANGE_CHECK(ctrl.aie.cade_level, AIE_CADE_LEVEL_MIN,
                                                          AIE_CADE_LEVEL_MAX);
                        ctrl.aie.ltm_level = RANGE_CHECK(ctrl.aie.ltm_level, AIE_LTM_LEVEL_MIN,
                                                         AIE_LTM_LEVEL_MAX);
...
uint32_t vpp_queue_buf(void *ctx, enum vpp_port port, struct vpp_buffer *buf)
{
    ...
    if (u32Ret != VPP_OK)
    {
        ...
        u32VppBuf_InternalPut(VPP_CTX_GET(ctx), pstIntBuf, &pstExtBuf);
