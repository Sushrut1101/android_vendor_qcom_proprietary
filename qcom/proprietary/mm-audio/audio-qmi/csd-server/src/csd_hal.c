...
static int csd_hal_start_voice(void)
{
  ...
  // set vsid
  rc = qahw_set_parameters(csd_hal_common_data.primary_hal_handle, session_id);
  if (rc) {
  ...
  //set in call mode
  rc = qahw_set_mode(csd_hal_common_data.primary_hal_handle, AUDIO_MODE_IN_CALL );
   if (rc) {
  ...
  rc = qahw_out_set_parameters(csd_hal_common_data.out_stream_handle, device_route);
  if (rc) {
...
static int csd_hal_end_voice(void)
{
  ...
  // set vsid
  rc = qahw_set_parameters(csd_hal_common_data.primary_hal_handle, "call_state=1");
  if (rc) {
  ...
  //set in call mode
  rc = qahw_set_mode(csd_hal_common_data.primary_hal_handle, AUDIO_MODE_NORMAL );
  if (rc) {
    LOGE("set call mode failed %d \n",rc);
    goto EXIT;
  }
...
static int csd_hal_volume_control()
{
  ...
  // set volume
  rc =  qahw_set_voice_volume(csd_hal_common_data.primary_hal_handle, volume);
  if (rc) {
...
static int csd_hal_voice_mute()
{
  ...
  // set device mute
  if (device_mute != NULL) {
    rc = qahw_set_parameters(csd_hal_common_data.primary_hal_handle, device_mute);
  } else {
...
int csd_hal_init(void)
{
  ...
  csd_hal_common_data.primary_hal_handle = NULL;
  csd_hal_common_data.primary_hal_handle = qahw_load_module(QAHW_MODULE_ID_PRIMARY);
  if (csd_hal_common_data.primary_hal_handle == NULL) {
    LOGE("Could not open hw device. rc %d \n",rc);
    return -1;
  }

  /* Open audio output stream. */
  csd_hal_common_data.out_stream_handle = NULL;
  LOGD("calling open_out_put_stream:\n");
  rc = qahw_open_output_stream(csd_hal_common_data.primary_hal_handle,
                               handle,
                               output_device,
                               flags,
                               &config,
                               &(csd_hal_common_data.out_stream_handle),
                               stream_name);
  if (rc) {
    LOGE("Could not open output stream. rc = %d \n",rc);
    return -1;
  }
...
int csd_hal_deinit(void)
{
  ...
  qahw_close_output_stream(csd_hal_common_data.out_stream_handle);

  rc = qahw_unload_module(csd_hal_common_data.primary_hal_handle);
