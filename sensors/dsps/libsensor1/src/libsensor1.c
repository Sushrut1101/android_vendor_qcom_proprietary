...
static const uint32_t SMGR_TIMEOUT_MS = 8000;
...
bool check_sensors_enable(){
  char enabled;
  int settings_fd = -1;
  char sns_settings_file_name[255] = "";
  ...
  if( -1 != settings_fd ) {
    ...
    if( 1 == read( settings_fd, &enabled, 1 ) ) {
      if( enabled == '1' ) {
        close( settings_fd );
        LOG_DEBUG("%s: Sensors enabled = true", __func__);
        return true;
      } else {
        close( settings_fd );
        LOG_ERROR("%s: Sensors not enable in settings file", __func__);
        return false;
      }
    } else {
        close( settings_fd );
        LOG_ERROR("%s: read sensors settings file failed", __func__);
        return false;
      }
  }
  LOG_ERROR("%s: sensors settings file not available", __func__);
  return false;
}
...
wait_for_service( uint32_t sns_svc_num, uint32_t timeout )
{
  ...
  //check for sensors enabled
  if(false == check_sensors_enable()){
    LOG_ERROR("%s: sensors setting disabled sensors", __func__);
    return false;
  }
...
/**
 * Checks the path shared in the the argument
 * exist in the device or not
 * return SNS_SUCCESS if exists SNS_ERR_FAILED otherwise
 */
sns_err_code_e
sensor1_has_acessto_directory(const char*  pathname)
{
  char *resolved_dirname_path;
  char *dir;
  char *dir_name;
  int err;

  resolved_dirname_path = malloc( PATH_MAX );
  if( NULL == resolved_dirname_path )
  {
  ...
  {
    LOG_ERROR( "realpath failed for directory name%i", errno );
    free( resolved_dirname_path );
    free( dir );
    return SNS_ERR_FAILED;
  }
  ...
  {
    LOG_ERROR( "invalid directory path %i", err );
    free( resolved_dirname_path );
    free( dir );
    return SNS_ERR_FAILED;
  }
