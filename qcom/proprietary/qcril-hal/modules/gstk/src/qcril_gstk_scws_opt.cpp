...
#define TAG "QCRIL_GSTK_SCWS_OPT"
  ...
  const qcril_scws_opt_traffic_analyzer_type * analyzer_ptr)
{
  ...
  {
    QCRIL_LOG_ERROR("Invalid value, cannot process request");
    return;
  }
  ...
  size_t                               * line_len)
{
  ...
  {
    eol_ptr = (uint8_t *)memmem(*begin_ptr,
                           data_len,
                           QCRIL_SCWS_CRLF,
                           QCRIL_SCWS_CRLF_LEN);
  ...
  if (eol_ptr == NULL)
  {
    /* If CRLF not found, obtain pointer to LF,
       if present */
    eol_ptr = (uint8_t *)memmem(*begin_ptr, data_len, QCRIL_SCWS_LF, QCRIL_SCWS_LF_LEN);
  ...
  *line_len = (eol_ptr - *begin_ptr)+eol_len;
  line_ptr = (char *) qcril_malloc((*line_len + 1)*sizeof(uint8_t));
...
void qcril_scws_opt_process_rx(
  ...
  size_t                                 data_len)
{
  uint8_t * merged_data_buffer      = NULL;
  size_t    merged_data_buffer_len  = 0;
  uint8_t * begin_ptr               = NULL;
  char    * line_ptr                = NULL;
  size_t    line_len                = 0;
  ...
  if (analyzer_ptr->request_buffer != NULL &&
      analyzer_ptr->request_buffer_len > 0)
  {
    /* Ensure that data_len can accommodate the length of the
       merged buffer. */
    if (data_len <= SIZE_MAX - analyzer_ptr->request_buffer_len)
    {
      merged_data_buffer_len =
        (analyzer_ptr->request_buffer_len+data_len)*sizeof(uint8_t);
      merged_data_buffer =
        (uint8_t *)qcril_malloc(merged_data_buffer_len);
    }
    ...
    data_len = analyzer_ptr->request_buffer_len + data_len;
  ...
  while(begin_ptr < data_ptr + data_len)
  {
    ...
    line_ptr =
      qcril_scws_opt_get_line(&begin_ptr,
                              ((size_t)data_ptr+data_len)-(size_t)begin_ptr,
                              &line_len);
    ...
    if (line_ptr == NULL)
    {
      ...
      analyzer_ptr->request_buffer =
          (uint8_t *)qcril_malloc(analyzer_ptr->request_buffer_len * sizeof(uint8_t));
    ...
    if (http_ptr != NULL)
    {
      /* search for request method. Check to make sure
         ...
         section 3.1. */
      if (((size_t)analyzer_ptr->request_count) < sizeof(analyzer_ptr->requests_arr) / sizeof(analyzer_ptr->requests_arr[0]))
      {
    ...
    else if (strncmp(line_ptr, QCRIL_SCWS_CRLF, QCRIL_SCWS_CRLF_LEN) != 0)
    {
      if (strncmp(line_ptr, QCRIL_SCWS_LF, QCRIL_SCWS_LF_LEN) == 0)
      {
        ...
        if (analyzer_ptr->request_processing_post_request &&
            analyzer_ptr->request_content_length_found)
        {
          data_len = (data_ptr+data_len)-begin_ptr;
        }
      }
    }
    else
  ...
  size_t                                 data_len)
{
      ...
      analyzer_ptr->response_buffer != NULL)
  {
    /* Ensure that data_len can accommodate the length of the
       merged buffer. */
    if (data_len <= SIZE_MAX - analyzer_ptr->response_buffer_len)
    {
      merged_data_buffer_len =
        (analyzer_ptr->response_buffer_len+data_len)*sizeof(uint8_t);
      merged_data_buffer =
        (uint8_t *)qcril_malloc(merged_data_buffer_len);
    }
    ...
    data_ptr = merged_data_buffer;
    data_len = analyzer_ptr->response_buffer_len + data_len;
  ...
  while(begin_ptr <= data_ptr + data_len)
  {
    ...
    if (!analyzer_ptr->response_header_end_found)
    {
      ...
      else
      {
        ...
        analyzer_ptr->response_buffer =
          (uint8_t *)qcril_malloc(analyzer_ptr->response_buffer_len * sizeof(uint8_t));
    ...
    if(analyzer_ptr->response_header_end_found)
    {
      ...
      else
      {
        ...
        else if (analyzer_ptr->response_transfer_encoding_chunked_found)
        {
          ...
          uint8_t* final_ptr = NULL;
          final_ptr = (uint8_t*) memmem(begin_ptr,
                                           (data_ptr+data_len)-begin_ptr,
