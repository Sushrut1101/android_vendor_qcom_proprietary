/* fovcrop46.c
 *
 * Copyright (c) 2012-2014, 2016-2017,2019 Qualcomm Technologies, Inc. All Rights Reserved.
...
static boolean fovcrop46_get_module_enable_mask(fovcrop46_t *fovcrop,
 ...
 * Return: nothing
 **/
static void fovcrop46_cmd_debug(ISP_FOV_CropConfigCmdType cmd ,int index)
{
  ISP_HIGH(" FOV[%d](%d-Enc/%d-View/%d-Video/%d-Ideal)", index,
    ISP_HW_STREAM_ENCODER, ISP_HW_STREAM_VIEWFINDER,
    ISP_HW_STREAM_VIDEO, ISP_HW_STREAM_IDEAL_RAW);

  ISP_HIGH(" FOV_CROP[%d] Y firstPixel %d",
     index, cmd.y_crop_cfg.firstPixel );
  ISP_HIGH(" FOV_CROP[%d] Y lastPixel %d",
     index, cmd.y_crop_cfg.lastPixel );
  ISP_HIGH(" FOV_CROP[%d] Y firstLine %d",
     index, cmd.y_crop_cfg.firstLine );
  ISP_HIGH(" FOV_CROP[%d] Y lastLine %d",
 ...
 **/
static boolean fovcrop46_validate_config(fovcrop46_entry_t *entry)
{
  ...
  if(entry->reg_cmd.y_crop_cfg.firstPixel >
    ...
    ISP_ERR("failed: invalid Y width, first pix %d, last pix %d",
  ...
  if(entry->reg_cmd.y_crop_cfg.firstLine >
    ...
    ISP_ERR("failed: invalid Y Height, first line %d, last line %d",
  ...
  if(entry->reg_cmd.cbcr_crop_cfg.firstPixel >
    ...
    ISP_ERR("failed: invalid CbCr width, first pix %d, last pix %d",
  ...
  if(entry->reg_cmd.cbcr_crop_cfg.firstLine >
    ...
    ISP_ERR("failed: invalid CbCr Height, first line %d, last line %d",
 ...
 * Return: TRUE on success and FALSE on failure
 **/
static boolean fovcrop46_split_config(fovcrop46_t *fovcrop,
  uint32_t cbcr_scale)
{
  fovcrop46_entry_t *entry = NULL;
  ...
  ISP_HIGH("======= SPLIT CONFIG: stripe id %d(0- LEFT, 1- RIGHT)==========",
    fovcrop->isp_out_info.stripe_id);
  ISP_HIGH("split_input: hw_stream_info.width %d idx %d",
    fovcrop->hw_stream_info.width, fovcrop->entry_idx );
  ISP_HIGH("split_input: SCALER: total out %d right_out_Y %d, right_out_CBCR %d",
    fovcrop->scaler_output.width,
    fovcrop->scaler_output.right_split_y_width,
    fovcrop->scaler_output.right_split_cbcr_width);
  ISP_HIGH("split_input: FOVCROP: left out %d",
    fovcrop->isp_out_info.left_output_width);

  /*output for split config*/
  ISP_HIGH(" split_output: Y: total_crop %d, lef_cropt %d righ_cropt %d",
    y_total_crop, y_left_crop, y_right_crop);
  ISP_HIGH(" split_output: CbCr: total_crop %d, left_crop %d right_crop %d",
    cbcr_total_crop, cbcr_left_crop, cbcr_right_crop);

  return TRUE;
}

/** fovcrop46_compute_min_split_width_left:
 *
 ...
 **/
static uint32_t fovcrop46_compute_min_split_width_left(
  uint32_t input_width, uint32_t output_width, boolean cds)
{
  ...
  /* the output to AXI need to be allign
     ...
     Normally the padding is 32, with extra chroma subsampling requirement,
     it becomes 64 */
  if (cds) {
    split_pt = PAD_TO_SIZE(output_width / 4, 32) * 2 *input_width / output_width;
  } else {
    split_pt = PAD_TO_SIZE(output_width / 2, 32) * input_width / output_width;
  }
  return split_pt;
}

/** fovcrop46_update_stripe_limit
 ...
 **/
static int fovcrop46_update_stripe_limit(fovcrop46_t *fovcrop,
  isp_stripe_limit_info_t *stripe_limit_info)
{
  ...
  fovcrop->left_split_input = EVEN_CEIL(fovcrop46_compute_min_split_width_left(
    sensor_output_width, fovcrop->hw_stream_info.width, fovcrop->cds_enabled));

  if (fovcrop->left_split_input > stripe_limit_info->min_left_split) {
    stripe_limit_info->min_left_split = fovcrop->left_split_input;
  }
  ISP_DBG("split_dbg stream %x sensor %d dim %d x %d split_pt %d CDS %d",
    fovcrop->hw_stream_info.identity, sensor_output_width,
    fovcrop->hw_stream_info.width, fovcrop->hw_stream_info.height,
    fovcrop->left_split_input, fovcrop->cds_enabled);
  if (!stripe_limit_info->smallest_split_point ||
    (fovcrop->left_split_input < stripe_limit_info->smallest_split_point)) {
    stripe_limit_info->smallest_split_point = fovcrop->left_split_input;
  }

  return 0;
}

/** fovcrop46_handle_set_chromatix_ptr
 *
 ...
 **/
boolean fovcrop46_handle_set_chromatix_ptr(fovcrop46_t *fovcrop,
  mct_event_t *event)
{
...
/** fovcrop46_calculate_scaler_output
 *    @M:       output
 *    @N:       input
 *    @offset:  offset
 ...
 **/
uint32_t fovcrop46_calculate_scaler_output(uint32_t  M, uint32_t  N,
  uint32_t  offset, uint32_t  actual_input_width, boolean   y_scaler)
{
  ...
  if (y_scaler && ((phase_init >> INTERP_SHIFT) != 0)) {
...
{
  boolean   ret           = TRUE;
  uint32_t  left_crop     = 0;
  uint32_t  right_crop    = 0;
  uint32_t  total_crop    = 0;
  float     scale_w       = 0;
  float     scale_h       = 0;
  uint32_t  camif_w       = 0;
  uint32_t  camif_h       = 0;
  ...
  fovcrop46_entry_t *entry = &fovcrop->fov_entry;
  ...
  if (fovcrop->ispif_out_info.is_split == TRUE) {
    ret = fovcrop46_split_config(fovcrop, scale_w);
    if (ret == FALSE) {
      ISP_ERR("fovcrop46_split_config failed!");
      return FALSE;
    }
  }

  ret = fovcrop46_validate_config(entry);
  if (ret == FALSE) {
...
boolean fovcrop46_config(fovcrop46_t *fovcrop)
{
  ...
  camif_h = fovcrop->sensor_out_info.request_crop.last_line -
      fovcrop->sensor_out_info.request_crop.first_line + 1;
  if (camif_w < (uint32_t)fovcrop->hw_stream_info.width)
    fovcrop->hw_stream_info.width = camif_w;
  if (camif_h < (uint32_t)fovcrop->hw_stream_info.height)
    fovcrop->hw_stream_info.height = camif_h;

  ISP_DBG(" fov_entry hw stream info  width %d, height %d",
     fovcrop->hw_stream_info.width,
       fovcrop->hw_stream_info.height);
  ISP_DBG(" fov_entry scaler_output: width %d, height %d",
     fovcrop->scaler_output.width,
       fovcrop->scaler_output.height);
  ISP_DBG(" camif : width %d, height %d",
     camif_w, camif_h);
  ...
  crop_width = fovcrop->hw_stream_info.width;
  crop_height = fovcrop->hw_stream_info.height;
  h_factor = fovcrop->scaler_output.width * Q12;
  v_factor = fovcrop->scaler_output.height * Q12;
  ...
  entry->crop_window.crop_out_x = crop_width * scaler_factor /
    fovcrop->crop_factor.crop_applied;
  ...
  /* Differentiate applied crop vs calculated
   * this is to support per stream scaler */
  entry->crop_window_calculated.crop_out_x = crop_width * scaler_factor /
    fovcrop->crop_factor.crop_to_calculate;
  if (entry->crop_window_calculated.crop_out_x > crop_width)
    entry->crop_window_calculated.crop_out_x = crop_width;

  entry->crop_window_calculated.crop_out_y = crop_height * scaler_factor /
    fovcrop->crop_factor.crop_to_calculate;
  if (entry->crop_window_calculated.crop_out_y > crop_height)
    entry->crop_window_calculated.crop_out_y = crop_height;

  entry->crop_window_calculated.x =
    (crop_width - entry->crop_window_calculated.crop_out_x) >> 1;
  entry->crop_window_calculated.y =
    (crop_height - entry->crop_window_calculated.crop_out_y) >> 1;
  ...
  return TRUE;
} /* fovcrop46_config_entry */

/** fovcrop46_trigger_update
 ...
 * Return: 0 - success and negative value - failure
 **/
static int fovcrop46_trigger_update(fovcrop46_t *fovcrop)
{
  int rc = 0;
  ...
  //rc = fovcrop46_config(fovcrop, &trigger_input->cfg, sizeof(trigger_input->cfg));

  return rc;
} /* fovcrop46_trigger_update */

/** fovcrop46_set_zoom_ratio
 ...
 * Return: 0 - success and negative value - failure
 **/
static int fovcrop46_set_zoom_ratio(fovcrop46_t *fovcrop,
  uint32_t in_param_size)
{
  int rc = 0;
  //rc = fovcrop46_config(fovcrop, pix_setting, in_param_size);
 ...
 * Return: nothing
 **/
void fovcrop46_reset(fovcrop46_t *fovcrop)
{
  ISP_HIGH("E");
 ...
 * Return: 0 - success and negative value - failure
 **/
static int fovcrop46_enable(fovcrop46_t *fovcrop, isp_mod_set_enable_t *enable,
  uint32_t in_param_size)
{
  int i;
 ...
 * Return: 0 - success and negative value - failure
 **/
static int fovcrop46_trigger_enable(fovcrop46_t *fovcrop,
 ...
 * Return: 0 - success and negative value - failure
 **/
static int fovcrop46_get_fov_crop(fovcrop46_t *fovcrop,
  isp_hw_zoom_param_t *hw_zoom_param)
{
  int i;
  fovcrop46_entry_t *entry;
 ...
 * Return: 0 always
 **/
static int fovcrop46_destroy (void *mod_ctrl)
{
  fovcrop46_t *fovcrop = mod_ctrl;

  memset(fovcrop,  0,  sizeof(fovcrop46_t));
  free(fovcrop);
  return 0;
}

/** fovcrop46_update_zoom_params
 ...
 **/
static int fovcrop46_update_zoom_params(fovcrop46_t *fov_mod,
  ...
  if (!fov_mod->session_based_identity) {
    for (i = 0; i < ISP_MAX_STREAMS; i++) {
      ...
      if (zoom_params->identity == identity) {
        ...
        zoom_params->crop_window_calculated = fov_mod->fov_entry.crop_window_calculated;
 ...
 * Return: nothing
 **/
static int fovcrop46_do_hw_update(fovcrop46_t *fov_mod,
  isp_sub_module_output_t *output, uint32_t identity)
{
  ...
  if (fov_mod->fov_entry.hw_update_pending == TRUE) {
    reg_cmd = (ISP_FOV_CropConfigCmdType *)
      CAM_MALLOC(sizeof(ISP_FOV_CropConfigCmdType));
    if (!reg_cmd) {
    ...
    *reg_cmd = fov_mod->fov_entry.reg_cmd;
    fovcrop46_cmd_debug(fov_mod->fov_entry.reg_cmd, fov_mod->entry_idx);

    if (fov_mod->entry_idx == ISP_HW_STREAM_ENCODER) {
      rc = fovcrop46_util_single_HW_write(
        (void *)reg_cmd, sizeof(*reg_cmd), ISP_FOV46_ENC_OFF,
        ISP_FOV46_ENC_LEN, VFE_WRITE, &output->hw_update_list);
    }else if (fov_mod->entry_idx == ISP_HW_STREAM_VIEWFINDER){
      rc = fovcrop46_util_single_HW_write(
        (void *)reg_cmd, sizeof(*reg_cmd), ISP_FOV46_VIEW_OFF,
        ISP_FOV46_VIEW_LEN, VFE_WRITE, &output->hw_update_list);
    } else if(fov_mod->entry_idx == ISP_HW_STREAM_VIDEO){
      rc = fovcrop46_util_single_HW_write(
        (void *)reg_cmd, sizeof(*reg_cmd), ISP_FOV46_VIDEO_OFF,
        ISP_FOV46_VIDEO_LEN, VFE_WRITE, &output->hw_update_list);
    }
    fov_mod->fov_entry.hw_update_pending = FALSE;
  }

  rc = fovcrop46_update_zoom_params(fov_mod, output->stats_params->zoom_params,
    identity);

  return rc;
}

/** fovcrop46_get_module_enable_mask:
 *
...
static boolean fovcrop46_loop_saved_events(mct_module_t *module,
  fovcrop46_t *fovcrop, isp_saved_events_t *saved_events, uint32_t identity)
{
  ...
  if (saved_events->set_params[ISP_SET_PARM_ZOOM] &&
    ...
    hal_param =
      ...
      control_event_data;
    port_fovcrop46_handle_hal_set_parm(module, fovcrop, identity, hal_param);
  }
  ...
  if (saved_events->module_events[ISP_MODULE_EVENT_STATS_FOVC_MAGNIFICATION_FACTOR] &&
    ...
    port_fovcrop46_handle_lens_position_update(module,fovcrop, identity,
  ...
  if (saved_events->module_events[ISP_MODULE_EVENT_CDS_REQUEST] &&
    ...
    port_fovcrop46_handle_cds_request(module, fovcrop,
      saved_events->module_events[ISP_MODULE_EVENT_CDS_REQUEST]);
  }
  return TRUE;
}

/** fovcrop46_handle_isp_private_event:
 *
  ...
  fovcrop46_t *fovcrop, void *data, uint32_t identity)
{
  ...
  if (!module || !fovcrop || !data) {
    ISP_ERR("failed: module %p fovcrop46 %p data %p", module, fovcrop, data);
    return FALSE;
  }

  switch (private_event->type) {
  case ISP_PRIVATE_REQUEST_STRIPE_LIMITATION:
    rc = fovcrop46_update_stripe_limit(fovcrop, private_event->data);
    if (rc < 0) {
  ...
  case ISP_PRIVATE_SET_MOD_ENABLE:
    rc = fovcrop46_enable(fovcrop,
      (isp_mod_set_enable_t *)private_event->data, private_event->data_size);
    if (rc < 0) {
      ISP_ERR("failed: fovcrop46_enable rc %d", rc);
      ret = FALSE;
    }
    break;

  case ISP_PRIVATE_SET_TRIGGER_ENABLE:
    rc = fovcrop46_trigger_enable(fovcrop, private_event->data,
      private_event->data_size);
    if (rc < 0) {
      ISP_ERR("failed: fovcrop46_config rc %d", rc);
      ret = FALSE;
    }
    break;
  ...
  case ISP_PRIVATE_SET_TRIGGER_UPDATE: {
    ...
    if (output->saved_events) {
      fovcrop46_loop_saved_events(module, fovcrop, output->saved_events,
        identity);
    }
    ISP_HIGH("ISP_PRIVATE_SET_TRIGGER_UPDATE");
    PTHREAD_MUTEX_LOCK(&fovcrop->mutex);
    rc = fovcrop46_do_hw_update(fovcrop,
      (isp_sub_module_output_t *)private_event->data, identity);
    if (rc < 0) {
      ISP_ERR("failed: fovcrop46_do_hw_update rc %d", rc);
      ret = FALSE;
    }
    PTHREAD_MUTEX_UNLOCK(&fovcrop->mutex);
  }
  ...
  case ISP_PRIVATE_ACTION_RESET:
    fovcrop46_reset(fovcrop);
    break;

  case ISP_PRIVATE_REQUEST_ZOOM_PARAMS:
    rc = fovcrop46_update_zoom_params(fovcrop,
        (isp_zoom_params_t *)private_event->data, identity);
    if (rc < 0) {
      ISP_ERR("failed: fovcrop46_update_zoom_params");
      ret = FALSE;
    }
    break;
 ...
 *    @version: hw version
 *
 *  fov 46 module open and create func table
 *
 * Return: fov module ops struct pointer
 **/
boolean fovcrop46_open(fovcrop46_t *fovcrop)
{
  uint8_t i = 0;
  ISP_HIGH(" E");
  if (!fovcrop) {
 ...
 * Return: 0 always
 **/
boolean fovcrop46_init (void *mod_ctrl, void *in_params)
{
  fovcrop46_t *fov = mod_ctrl;
