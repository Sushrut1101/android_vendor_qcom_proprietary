...
/*===========================================================================
  FUNCTION  lqe_get_qmi_ott_subscription_id
===========================================================================*/
/*!
@brief
  Returns the subscription ID corresponding to the current global subs id

  @return
  ott_bind_subscription_enum_v01
*/
/*=========================================================================*/
static ott_bind_subscription_enum_v01 lqe_get_qmi_ott_subscription_id()
{
  ott_bind_subscription_enum_v01 sub_num;
  switch (global_subs_id) {
    case WDS_DEFAULT_SUBS_V01:
      sub_num = OTT_DEFAULT_SUBS_V01;
      break;
    case WDS_PRIMARY_SUBS_V01:
      sub_num = OTT_PRIMARY_SUBS_V01;
      break;
    case WDS_SECONDARY_SUBS_V01:
      sub_num = OTT_SECONDARY_SUBS_V01;
      break;
    case WDS_TERTIARY_SUBS_V01:
      sub_num = OTT_TERTIARY_SUBS_V01;
      break;
    case WDS_DONT_CARE_SUBS_V01:
    default:
      sub_num = OTT_DONT_CARE_SUBS_V01;
      break;
  }
  return sub_num;
}
...
)
{
  ...
  //OTT client needs to bind to subscription
  ott_bind_subscription_req_msg_v01 req;
  ott_bind_subscription_resp_msg_v01 resp;
  memset(&req, 0, sizeof(req));
  memset(&resp, 0, sizeof(resp));
  req.subscription = lqe_get_qmi_ott_subscription_id();
  rc = qmi_client_send_msg_sync (ott_client->client_handle,
                                 QMI_OTT_BIND_SUBSCRIPTION_REQ_V01,
                                 (void *)&req,
                                 sizeof(req),
                                 (void*)&resp,
                                 sizeof(resp),
  ...
  if (QMI_NO_ERR != rc || resp.resp.result != QMI_RESULT_SUCCESS_V01) {
    LQE_LOG_ERROR("%s(): failed on qmi_client_ott_bind_subscription with rc=%d, qmi_err=%d!\n",
                  __func__, rc, resp.resp.result);
    //Bind subscription is a must for ott client
    ott_client = NULL;
    goto bail;
  } else {
      LQE_LOG_DEBUG("%s(): qmi ott client bind to subscription %d successfully\n",
                    __func__, lqe_get_qmi_ott_subscription_id());
  }
...
)
{
  ...
  LQE_LOG_INFO("%s(): Registering for QMI OTT indications\n" ,__func__);
  rc = qmi_client_send_msg_sync(ott_client->client_handle,
...
/*===========================================================================
  FUNCTION  lqe_ott_configure_throughput_report_criteria
===========================================================================*/
int lqe_ott_configure_throughput_report_criteria
(
  int                   lqe_client_handle,
  lqe_data_direction_e  direction,
...
)
{
  ...
  if(!criteria)
  {
    LQE_LOG_ERROR("%s(): invalid criteria\n", __func__);
    return ret;
  }

  if (direction == LQE_DATA_DOWNLINK_DIRECTION)
  {
    uint32_t num_threshold_levels =
      MIN(QMI_OTT_DL_THRESHOLD_LIST_LEN_V01, criteria->thresholdsDownlinkKbps_size);
    if (criteria->thresholdsDownlinkKbps_size > QMI_OTT_DL_THRESHOLD_LIST_LEN_V01) {
      LQE_LOG_ERROR("Downlink thresholds size exceeded max allowed...truncated!");
    }
    ott_configure_downlink_throughput_settings_req_msg_v01 req;
    ott_configure_downlink_throughput_settings_resp_msg_v01 resp;
    memset((void *)&req, 0, sizeof(req));
    ...
    req.rat_type = criteria->ran;

    /* configure DL hysteresis time (ms) */
    req.dl_throughput_hyst_time_valid = 1;
    req.dl_throughput_hyst_time = criteria->hysteresisMs;

    /* configure DL hysteresis magnitude change (kbps) */
    req.dl_throughput_hyst_mag_chng_size_valid = 1;
    req.dl_throughput_hyst_mag_chng_size = criteria->hysteresisDlKbps;

    /* configure DL threshold levels (num) and values (kbps) */
    req.dl_throughput_threshold_list_valid = 1;
    req.dl_throughput_threshold_list_len = num_threshold_levels;

    if( num_threshold_levels > 0 ) {
      for (uint32_t i = 0; i < num_threshold_levels; i++) {
        req.dl_throughput_threshold_list[i] = criteria->thresholdsDownlinkKbps[i];
      }
    } /* DL levels available */
    LQE_LOG_INFO("%s(): Configured downlink throughput params \n",
               __func__);
    rc = qmi_client_send_msg_sync(ott_client->client_handle,
                                  QMI_OTT_CONFIGURE_DOWNLINK_THROUGHPUT_SETTINGS_REQ_V01,
                                  &req,
  ...
  else if (direction == LQE_DATA_UPLINK_DIRECTION)
  {
    ...
    if (criteria->thresholdsUplinkKbps_size > QMI_OTT_UL_THRESHOLD_LIST_LEN_V01) {
      LQE_LOG_ERROR("Uplink thresholds size exceeded max allowed...truncated!");
    }
    ott_configure_uplink_throughput_settings_req_msg_v01 req;
    ott_configure_uplink_throughput_settings_resp_msg_v01 resp;
    memset((void *)&req, 0, sizeof(req));
    ...
    /* configure UL hysteresis time (ms) */
    req.ul_throughput_hyst_time_valid = 1;
    req.ul_throughput_hyst_time = criteria->hysteresisMs;

    /* configure UL hysteresis magnitude change (kbps) */
    req.ul_throughput_hyst_mag_chng_size_valid = 1;
    req.ul_throughput_hyst_mag_chng_size = criteria->hysteresisUlKbps;

    /* configure UL threshold levels (num) and values (kbps) */
    req.ul_throughput_threshold_list_valid = 1;
    req.ul_throughput_threshold_list_len = num_threshold_levels;

    if( num_threshold_levels > 0 ) {
      for (uint32_t i = 0; i < num_threshold_levels; i++) {
        req.ul_throughput_threshold_list[i] = criteria->thresholdsUplinkKbps[i];
      }
    } /* UL levels available */
    LQE_LOG_INFO("%s(): Configured uplink throughput params \n",
               __func__);
    rc = qmi_client_send_msg_sync(ott_client->client_handle,
                                  QMI_OTT_CONFIGURE_UPLINK_THROUGHPUT_SETTINGS_REQ_V01,
                                  &req,
...
/*===========================================================================
  FUNCTION  lqe_ott_toggle_reporting
===========================================================================*/
/*!
@brief
  Toggle reporting of throughput based estimation
*/
/*=========================================================================*/
int lqe_ott_toggle_reporting
(
...
)
{
  ...
  ott_client = lqe_qmi_get_ott_client(qmi_client_handle);
  ...
  if (direction == LQE_DATA_DOWNLINK_DIRECTION)
  {
    ...
    if (LQE_TURN_ON_INDICATIONS == ind_switch)
    {
      ...
      LQE_LOG_INFO("%s(): Turning on downlink throughput indications: %d \n",
                 __func__, req.dl_threshold_action);
    }
    else
    {
      req.dl_threshold_action_valid = 1;
  ...
  else if (direction == LQE_DATA_UPLINK_DIRECTION)
  {
    ...
    if (LQE_TURN_ON_INDICATIONS == ind_switch)
    {
      ...
      LQE_LOG_INFO("%s(): Turning on uplink throughput indications: %d \n",
                 __func__, req.ul_threshold_action);
    }
    else
    {
      req.ul_threshold_action_valid = 1;
...
/*===========================================================================
  FUNCTION  lqe_ott_get_uplink_capability
===========================================================================*/
/*!
@brief
  Check if uplink throughput capability is supported
*/
/*=========================================================================*/
int lqe_ott_get_uplink_capability
(
  int                         qmi_client_handle
)
{
  ...
  ott_get_capabilities_req_msg_v01 req;
  ott_get_capabilities_resp_msg_v01 resp;
  memset((void *)&req, 0, sizeof(req));
  memset((void *)&resp, 0, sizeof(resp));
  req.req_capability_mask = OTT_MASK_UL_THROUGHPUT_REPORT_V01;
  int rc = qmi_client_send_msg_sync(ott_client->client_handle,
                                QMI_OTT_GET_CAPABILITIES_REQ_V01,
                                &req,
                                sizeof(req),
                                &resp,
                                sizeof(resp),
  ...
  LQE_LOG_INFO("%s(): UL cap valid %d, UL cap %d\n", __func__,
      resp.ul_capability_valid, resp.ul_capability);
  if (resp.ul_capability_valid &&
    (resp.ul_capability & OTT_UL_THROUGHPUT_REPORT_BY_THRESHOLD_V01))
  {
    ret = LQE_SUCCESS;
  }
...
/*===========================================================================
  FUNCTION  lqe_ott_get_downlink_capability
===========================================================================*/
/*!
@brief
  Check if downlink throughput capability is supported
*/
/*=========================================================================*/
int lqe_ott_get_downlink_capability
(
  int                         qmi_client_handle
)
{
  ...
  memset((void *)&req, 0, sizeof(req));
  memset((void *)&resp, 0, sizeof(resp));
  req.req_capability_mask = OTT_MASK_DL_THROUGHPUT_REPORT_V01;
  int rc = qmi_client_send_msg_sync(ott_client->client_handle,
                                QMI_OTT_GET_CAPABILITIES_REQ_V01,
                                &req,
                                sizeof(req),
  ...
  LQE_LOG_INFO("%s(): DL cap valid %d, DL cap %d\n", __func__,
      resp.dl_capability_valid, resp.dl_capability);
  if (resp.dl_capability_valid &&
    (resp.dl_capability & OTT_DL_THROUGHPUT_REPORT_BY_THRESHOLD_V01))
  {
    ret = LQE_SUCCESS;
  }
