...
NewDeviceDiscoveredEvent::NewDeviceDiscoveredEvent(const std::string& deviceName, const FwVersion& fwVersion,
...
void NewDeviceDiscoveredEvent::ToJsonInternal(Json::Value& root) const
{
    ...
    m_uCodeTimestamp.ToJson(root, "UcodeTimestamp");
}
...
FwUcodeStatusChangedEvent::FwUcodeStatusChangedEvent(const std::string& deviceName, uint32_t fwError, uint32_t uCodeError) :
    HostManagerEventBase(deviceName),
    m_fwError(fwError),
...
{
    root["FwError"] = m_fwError;
    root["UcodeError"] = m_uCodeError;
}
...
//********************************** FwStateChangedEvent **************************************//
FwHealthStateChangedEvent::FwHealthStateChangedEvent(const std::string& deviceName, int fwHealthState, int prevFwHealthState) :
    HostManagerEventBase(deviceName),
    m_fwHealthState(fwHealthState),
    m_prevFwHealthState(prevFwHealthState)
{}

void FwHealthStateChangedEvent::ToJsonInternal(Json::Value& root) const
{
    root["FwHealthState"] = m_fwHealthState;
    root["PrevFwHealthState"] = m_prevFwHealthState;
}
