...
int get_bt_soc_type()
{
    ...
    if (ret != 0) {
        printf( "vendor.qcom.bluetooth.soc set to %s\n", bt_soc_type);
        if (!strncasecmp(bt_soc_type, "rome", sizeof("rome"))) {
            ...
            printf( "vendor.qcom.bluetooth.soc not set, so using default.\n");
...
boolean extract_program_service()
{
  uint8 buf[64] = {0};
  int ret;
  ret = read_data_from_v4l2(fd_radio,buf,IRIS_BUF_PS_RDS);
  if(ret < 0) {
     return TRUE;
  }
...
boolean extract_radio_text()
{
  ...
  int bytesread = read_data_from_v4l2(fd_radio,buf,IRIS_BUF_PS_RDS);
  if(bytesread < 0) {
     return TRUE;
  }
...
{
  int retval = FM_CMD_SUCCESS;
  char versionStr[40] = {'\0'};
  struct v4l2_capability cap;
  struct fm_recv_conf_req recv_conf;
  struct fm_rds_grp_req rds_grp;
  struct fm_stereo_mode_req stereo_mode;
  int rdsmask = 0x7F;
  ...
  if (soc_type != BT_SOC_CHEROKEE) {
      ...
      retval = ioctl(fd_radio, VIDIOC_QUERYCAP, &cap);
      ...
      retval = fm_enable_receiver();
      if (retval != FM_CMD_SUCCESS) {
          printf("Enable receiver failed\n");
          goto end;
      }
      recv_conf.emphasis = radiocfgptr->emphasis;
      recv_conf.ch_spacing = radiocfgptr->spacing;
      recv_conf.rds_std = radiocfgptr->rds_system;
      recv_conf.hlsi = 0;
      recv_conf.band_low_limit = radiocfgptr->bandlimits.lower_limit;
      recv_conf.band_high_limit = radiocfgptr->bandlimits.upper_limit;

      retval= fm_set_config_receiver(&recv_conf);
      if (retval != FM_CMD_SUCCESS) {
          printf("Set configure failed\n");
          goto end;
      }

      stereo_mode.stereo_mode = 0x00;
      stereo_mode.sig_blend = 0x01;
      stereo_mode.intf_blend = 0x01;
      stereo_mode.most_switch = 0x01;

      retval = fm_set_stereo_mode_req(&stereo_mode);

      if (retval != FM_CMD_SUCCESS) {
          printf(" set stereo mode failed\n");
          goto end;
      }
      retval = fm_set_rds_grp_process_req(rdsmask);
      if (retval != FM_CMD_SUCCESS) {
          printf("RDS group processing failed\n");
          goto end;
      }
      rds_grp.rds_grp_enable_mask = 0xFFFF;
      rds_grp.rds_buf_size = 0x01;
      rds_grp.en_rds_change_filter = 0x01;

      retval = fm_rds_grp_mask_req(&rds_grp);
      if (retval != FM_CMD_SUCCESS) {
          printf("RDS group req failed\n");
          goto end;
      }
      retval = fm_set_antenna_req(1);
      if (retval != FM_CMD_SUCCESS) {
          printf("set antenna failed\n");
          goto end;
      }
      poweron = COMPLETE;
end:
...
fm_cmd_status_type (ftm_on_long_thread)(void *ptr)
{
  ...
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_STATE,FM_RX);
  if(ret == FALSE)
  {
...
#endif
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_EMPHASIS,
        radiocfgptr->emphasis);
  if(ret == FALSE)
  {
  ...
  print2("spacing   :%d\n",radiocfgptr->spacing);
#endif
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_SPACING,
        radiocfgptr->spacing);
  if(ret == FALSE)
  {
...
#endif
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_RDS_STD,
        radiocfgptr->rds_system);
  if(ret == FALSE)
  {
  ...
  print2("Band      :%d\n",radiocfgptr->band);
#endif
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_REGION,
        radiocfgptr->band);
  if(ret == FALSE)
  {
  ...
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_RDSON,1);
  if(ret == FALSE)
  {
  ...
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_RDSGROUP_PROC,
        rds_group_mask);
  if(ret == FALSE)
  {
    print2("EnableReceiver Failed to set RDS on = %d\n",ret);
  ...
  {
      ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_RDSGROUP_MASK, 1);
      if (ret == FALSE)
      {
          print("Failed to set RDS GRP MASK!!!\n");
          return FM_CMD_FAILURE;
      }
      ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_RDSD_BUF, 1);
      if (ret == FALSE)
      {
      ...
      ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_PSALL,
                    (psAllVal >> 4));
      if (ret == FALSE)
      {
          print2("EnableReceiver Failed to set RDS on = %d\n",ret);
  ...
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_ANTENNA,0);
  if(ret == FALSE)
  {
    print2("EnableReceiver Failed to set RDS on = %d\n",ret);
    return FM_CMD_FAILURE;
...
)
{
  ...
  if (soc_type != BT_SOC_CHEROKEE) {
      ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_STATE,0);
      if(ret == FALSE)
      {
...
)
{
  ...
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_REGION,radiocfgptr->band);
  if(ret == FALSE)
  {
...
)
{
  ...
  if (soc_type != BT_SOC_CHEROKEE) {
      ...
      err = fm_tune_station_req(ulfreq);
      if (err != FM_CMD_SUCCESS) {
          printf("tune frequency failed\n");
          return err;
      }
  }
#ifdef FM_DEBUG
...
)
{
  ...
  if (soc_type != BT_SOC_CHEROKEE) {
    ...
    ret = fm_get_station_param_req();
    if (ret != FM_CMD_SUCCESS) {
        printf("Get station param failed\n");
        goto end;
    }
  }
end:
...
)
{
  ...
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_RDSGROUP_MASK,
  ...
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_RDSD_BUF,
...
)
{
  ...
  control.id = V4L2_CID_PRIVATE_IRIS_LP_MODE;
...
)
{
  ...
  control.id = V4L2_CID_PRIVATE_IRIS_SIGNAL_TH;
...
)
{
  ...
  if (soc_type != BT_SOC_CHEROKEE) {
      ...
      ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_SRCHMODE,
      ...
      ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_SCANDWELL,
      ...
      ret = fm_srch_stations(&searchstationsoptions);
      if (ret != FM_CMD_SUCCESS) {
          printf("search station failed\n");
          return ret;
     }
  }
...
)
{
  ...
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_SRCH_PTY,
  ...
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_SRCH_PI,
...
)
{
  ...
  if (soc_type != BT_SOC_CHEROKEE) {
      ...
      ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_SRCH_CNT,
    ...
    ret = fm_search_list(&searchliststationsoptions);
    if (ret != FM_CMD_SUCCESS) {
        printf("Search list station failed\n");
        return ret;
    }
  }
  print("SearchStationListReceiver<\n");
...
)
{
  ...
  if(fd_radio < 0)
    return FM_CMD_NO_RESOURCES;
  ret = set_v4l2_ctrl(fd_radio,V4L2_CID_PRIVATE_IRIS_SRCHON,0);
  if(ret == FALSE)
  {
    return FM_CMD_FAILURE;
  }
