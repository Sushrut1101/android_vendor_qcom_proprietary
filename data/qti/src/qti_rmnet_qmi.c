...
--------------------------------------------------------------------------*/
  ...
  count = (uint16_t)qmux_msg->sdu.qmux.msg.msg_length;
  ...
  {
    tlv_type = (uint8_t)get_value(data, bytes_processed,sizeof(uint8_t));
    LOG_MSG_INFO1("TLV type tx %d", tlv_type, 0, 0);
    bytes_processed += sizeof(uint8_t);

    tlv_length = (uint16_t)get_value(data, bytes_processed,sizeof(uint16_t));
    LOG_MSG_INFO1("TLV length tx %d", tlv_length, 0, 0);
    bytes_processed += sizeof(uint16_t);

    if(tlv_type == ep_info_tlv_t)
    {
      usb_ep_type = (uint32_t)get_value(data, bytes_processed, sizeof(uint32_t));
      LOG_MSG_INFO1("USB EP type %d", usb_ep_type, 0, 0);
      LOG_MSG_INFO1("Modem EP type %d", modem_ep_type, 0, 0);
      set_value(data, bytes_processed, sizeof(uint32_t), modem_ep_type);
      bytes_processed += sizeof(uint32_t);

      usb_ep_id = (uint32_t)get_value(data, bytes_processed, sizeof(uint32_t));
      LOG_MSG_INFO1("USB EP ID %d", usb_ep_id, 0, 0);
      LOG_MSG_INFO1("Modem EP ID %d", modem_ep_id, 0, 0);
      set_value(data, bytes_processed, sizeof(uint32_t), modem_ep_id);
      bytes_processed += sizeof(uint32_t);
    }
    else
    {
...
{
  int                                   ret_val;
  uint32_t                              dl_max_size = 0;
  uint32_t                              ul_max_size = 0;
  uint32_t                              ul_aggr_prot = 0;
  uint32_t                              dl_aggr_prot = 0;
  uint32_t                              dl_max_datagrams = 0;
  uint32_t                              ul_max_datagrams = 0;
...
---------------------------------------------------------------------------*/
  ...
  if (ul_aggr_prot > 0 &&
      ul_max_datagrams == 1)
  {
    ret_val = qti_rmnet_ph_set_mru(DEFAULT_MTU_MRU_VALUE, USB_DATA_INTERFACE);
    if (ret_val)
    {
    ...
    ret_val = qti_rmnet_modem_set_mtu(DEFAULT_MTU_MRU_VALUE, MHI_DATA_INTERFACE);
    if (ret_val)
    {
  ...
  {
    ret_val = qti_rmnet_ph_set_mru(ul_max_size, USB_DATA_INTERFACE);
    if (ret_val)
    {
      LOG_MSG_ERROR("Failed to set peripheral interface MRU to %d",
                    ul_max_size, 0, 0);
    ...
    ret_val = qti_rmnet_modem_set_mtu(ul_max_size, MHI_DATA_INTERFACE);
    if (ret_val)
    {
      LOG_MSG_ERROR("Failed to set modem interface MTU to %d",
                     ul_max_size, 0, 0);
...
---------------------------------------------------------------------------*/
  ...
  if (dl_aggr_prot > 0 &&
      dl_max_datagrams == 1)
  {
    ret_val = qti_rmnet_ph_set_mtu(DEFAULT_MTU_MRU_VALUE, USB_DATA_INTERFACE);
    if (ret_val)
    {
    ...
    ret_val = qti_rmnet_modem_set_mru(DEFAULT_MTU_MRU_VALUE, MHI_DATA_INTERFACE);
    if (ret_val)
    {
  ...
  {
    ret_val = qti_rmnet_ph_set_mtu(dl_max_size, USB_DATA_INTERFACE);
    if (ret_val)
    {
      LOG_MSG_ERROR("Failed to set peripheral interface MTU to %d",
                     dl_max_size, 0, 0);
    ...
    ret_val = qti_rmnet_modem_set_mru(dl_max_size, MHI_DATA_INTERFACE);
    if (ret_val)
    {
      LOG_MSG_ERROR("Failed to set modem interface MRU to %d",
                    dl_max_size, 0, 0);
  ...
  ret_val = qti_is_iface_up(MHI_DATA_INTERFACE);
  if(ret_val)
  {
  ...
  ret_val = qti_is_iface_up(USB_DATA_INTERFACE);
  if(ret_val)
  {
...
/*-------------------------------------------------------------------------*/
  ...
  svc_type = (uint8_t)qmux_msg->qmux_hdr.svc_type;
  message_id = (uint16_t)qmux_msg->sdu.qmux.msg.msg_id;
     ...
     case WDA_SERVICE_ID:
       if (message_id == WDA_SET_DATA_FORMAT_MESSAGE_ID)
       {
             ...
             is_fusion_target)
         {
           ...
           {
             /*For new fusions NOT supporting SW path this is Not needed*/
             ret_val = qti_rmnet_wda_set_data_format_process_resp(data,
                                                                  qmux_msg_len,
                                                                  message_id);
             if (ret_val != QTI_SUCCESS)
             {
               LOG_MSG_ERROR("Failed to process WDA service msg %d",
                             message_id, 0, 0);
               ret = QTI_FAILURE;
             }
           }
    ...
    case WDA_SET_DATA_FORMAT_MESSAGE_ID:
      {
        ...
        if( DS_TARGET_FUSION_SDM845_SDX24 == rmnet_state_config->target ||
            ...
            is_fusion_target)
        {
          ret_val = qti_rmnet_modem_get_ep_id ((uint32_t*)&mhi_ep_id, MHI_DATA_INTERFACE);
          if (ret_val != QTI_SUCCESS)
          {
...

{
  ...
  ret_val = qmi_idl_message_decode(
              ...
              req_resp_ind,
              message_id,
              (char*)msg + QTI_QMUX_SDU_MESSAGE_HDR_LEN_BYTES,
...
------------------------------------------------------------------------------*/
  ...
  *ep_type_ptr = (data_ep_type_enum_v01) rmnet_state_config->modem_iface_ep.ep_type;
...
------------------------------------------------------------------------------*/
  ...
  *ep_type_ptr = (data_ep_type_enum_v01) rmnet_state_config->ph_iface_ep.ep_type;
...
)
{
  ...
  qmi_client_error_type                                    qmi_error, qmi_err_code = QMI_NO_ERR;
  qmi_client_os_params                                     qti_wda_os_params;
  int                                                      ret, rc = QTI_SUCCESS;
  qmi_client_type                                          qti_wda_handle;
  wda_set_data_format_req_msg_v01                          request;
  wda_set_data_format_resp_msg_v01                         response;
  qmi_service_info info;
/*---------------------------------------------------------------------------*/

  LOG_MSG_INFO1("qti_rmnet_qmi_wda_set_data_format()", 0, 0, 0);
...
------------------------------------------------------------------------------*/
  ...
  if (qti_wda_service_object == NULL)
  {
  ...
  memset(&qti_wda_os_params, 0, sizeof(qmi_client_os_params));
/*-----------------------------------------------------------------------------
  Client init
------------------------------------------------------------------------------*/

  qmi_error = qmi_client_init_instance(qti_wda_service_object,
                                       QMI_CLIENT_INSTANCE_ANY,
                                       NULL,
                                       NULL,
                                       &qti_wda_os_params,
  ...
  ret = qmi_client_send_msg_sync(qti_wda_handle,
...
bail:
  ret = qmi_client_release(qti_wda_handle);
  if (ret != QMI_NO_ERR)
  {
