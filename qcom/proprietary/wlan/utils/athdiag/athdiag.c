...
#define AR6320V1_REG_PART1_START_ADDR 0x0
/*WLAN_MBOX_BASE_ADDRESS - RTC_SOC_BASE_ADDRESS*/
#define AR6320V1_REG_PART1_LEN        (0x800 - 0x0)
/*STEREO_BASE_ADDRESS*/
#define AR6320V1_REG_PART2_START_ADDR 0x27000
/*USB_BASE_ADDRESS - STEREO_BASE_ADDRESS*/
#define AR6320V1_REG_PART2_LEN        (0x60000 - 0x27000)
...
struct ath_target_reg_info {
 ...
 static const struct ath_target_reg_info reg_ar9888_v2[] = {
	{DUMP_DRAM_START_ADDR,
		DUMP_DRAM_LEN,
		"DRAM",
		"fwdump_prgr_v2_dram"},
	{PEREGRINE_REG_PART1_START_ADDR,
		PEREGRINE_REG_PART1_LEN,
		"REG_PART1",
		"fwdump_prgr_v2_reg1"},
	{PEREGRINE_REG_PART2_START_ADDR,
		PEREGRINE_REG_PART2_LEN,
		"REG_PART2",
		"fwdump_prgr_v2_reg2"},
	{0, 0, 0, 0}
};
 ...
 static const struct ath_target_reg_info reg_ar6320_v1[] = {
	{DUMP_DRAM_START_ADDR,
		...
		"DRAM",
		"fwdump_rome_v1_dram"},
	{AR6320V1_REG_PART1_START_ADDR,
		AR6320V1_REG_PART1_LEN,
		"REG_PART1",
		"fwdump_rome_v1_reg1"},
	{AR6320V1_REG_PART2_START_ADDR,
		AR6320V1_REG_PART2_LEN,
		"REG_PART2",
		"fwdump_rome_v1_reg2"},
	{0, 0, 0, 0}
};
 ...
 static const struct ath_target_reg_info reg_ar6320_v2[] = {
	{AR6320V2_DRAM_START_ADDR,
		AR6320V2_DUMP_DRAM_LEN,
		"DRAM",
		"fwdump_rome_v2_dram"},
	{AR6320V2_IRAM_START_ADDR,
		AR6320V2_IRAM_LEN,
		"IRAM",
		"fwdump_rome_v2_iram"},
	{AR6320V2_AXI_START_ADDR,
		AR6320V2_AXI_LEN,
		"AXI",
		"fwdump_rome_v2_axi" },
	{0, 0, 0, 0}
 ...
 static const struct ath_target_reg_info reg_ar6320_v3[] = {
	{AR6320V3_DRAM_START_ADDR,
		AR6320V3_DUMP_DRAM_LEN,
		"DRAM",
		"fwdump_rome_v3_dram"},
	{AR6320V3_IRAM_START_ADDR,
		AR6320V3_IRAM_LEN,
		"IRAM",
		"fwdump_rome_v3_iram"},
	{AR6320V3_AXI_START_ADDR,
		AR6320V3_AXI_LEN,
		"AXI" ,
		"fwdump_rome_v3_axi" },
...
static inline void *MALLOC(int nbytes) {
...
void list_supported_target_names()
{
...
void ReadTargetOTP(int dev, A_UINT32 offset,
		   A_UINT8 *buffer, A_UINT32 length)
{
 ...
 */
int ValidWriteOTP(int dev, A_UINT32 offset,
		  A_UINT8 *buffer, A_UINT32 length)
{
	A_UINT32 i;
	...
	for (i=0; i<length; i++) {
		if (BAD_OTP_WRITE(otp_contents[i], buffer[i])) {
			fprintf(stderr, "Abort. Cannot change"
				" offset %d from 0x%02x"
				" to 0x%02x\n",
				offset+i,
				otp_contents[i],
				buffer[i]);
...
WriteTargetOTP(int dev, A_UINT32 offset, A_UINT8 *buffer, A_UINT32 length)
{
	...
	WriteTargetWord(dev, EFUSE_BASE_ADDRESS+VDDQ_SETTLE_TIME_REG_OFFSET,
			2200);
...
void DumpTargetMem(int dev, unsigned int target_idx, char *pathname)
{
	...
	while ((reg_info->reg_start != 0) || (reg_info->reg_len != 0)) {
		...
		if ((dump_fd = fopen(filename, "wb+")) == NULL) {
			fprintf(stderr, "err %s cannot create/open output file"
					" (%s)\n",
		...
		if(flag & HEX_FLAG) {
			fprintf(dump_fd,"target mem dump area[0x%08x - 0x%08x]",
				address,
				address+length);
		}

		nqprintf("DIAG Read Target (address: 0x%x, length: 0x%x,"
			 " filename: %s)\n",
...
unsigned int parse_target_index(char *optarg)
{
	unsigned int i, index = INVALID_TARGET_INDEX;

	for (i = 0; i < sizeof(target_info)/sizeof(target_info[0]); i++) {
		if (strncmp(optarg, target_info[i].name,
			    strlen(target_info[i].name)) == 0) {
			/* found */
			index = i;
			break;
		}
...
int main (int argc, char **argv)
{
	...
	while (1) {
		...
		case 'P':
			...
			if (strlen(pathname) && (pathname[strlen(pathname)-1] != '/')) {
				snprintf(tempfn, sizeof(tempfn),
				    "%s", pathname);
	...
	for (;;) {
		...
		if (!(flag & DEVICE_FLAG)) {
			...
			/* Convenience: if no device was specified on the
			...
			if (find_dev) {
				nbytes=fread(devicename, 1,
				    sizeof(devicename), find_dev);
				pclose(find_dev);

				if (nbytes > 15) {
					/* auto-detect possibly
					   successful */
					/* replace \n with 0 */
		...
		if ((flag & (ADDRESS_FLAG | LENGTH_FLAG | FILE_FLAG)) ==
			    ...
			    " filename: %s)\n", address, length, filename);
			{
				...
				if(flag & HEX_FLAG) {
					if (flag & OTP_FLAG) {
						fprintf(dump_fd,
						    "target otp dump area"
						    " [0x%08x - 0x%08x]",
						    address,address+length);
					} else {
						fprintf(dump_fd,
						    "target mem dump area"
						    " [0x%08x - 0x%08x]",
						    address,address+length);
					}
				}
	...
	case DIAG_WRITE_TARGET:
		 ...
		 */
		{
			...
			if (flag & OTP_FLAG) {
				/* Validate OTP write before committing
				   anything */
				remaining = length;
				while (remaining) {
					int nbyte;
	...
	case DIAG_WRITE_WORD:
		if ((flag & (ADDRESS_FLAG | PARAM_FLAG)) ==
			...
			if (flag & BITWISE_OP_FLAG) {
				if (quiet()) {
					...
					printf("DIAG Bit-Wise (%s)"
					       " modify Word (address: 0x%x,"
					       " orig:0x%x, new: 0x%x,"
					       " mask:0x%X)\n",
					       (flag & AND_OP_FLAG)?"AND":"OR",
					       address,
					       origvalue,
					       param,
					       bitwise_mask );
