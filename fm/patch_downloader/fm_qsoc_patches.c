/*============================================================================
  Copyright (c) 2009-2013, 2015 Qualcomm Technologies, Inc.  All Rights Reserved.
...
#define CAL_FILE_PATH  "/data/vendor/fm/Riva_fm_cal"
...
#define CAL_DAT_0 0x20
#define CAL_DAT_1 0x21
...
int get_cal_data_bahama( int i2c_fd )
{
    ...
    cal_file = open("/data/vendor/fm/fm_cal", O_RDWR | O_CREAT, S_IRUSR | S_IRGRP);
    if (cal_file < 0)
    {
                    return cal_file;
    }
...
int set_desired_notch_filter(int fd, int wcm_mode)	{
	...
	if(wcm_mode == 2)	{
		...
		if ( transport == BT_SOC_SMD ) {
			property_set("vendor.notch.value","NONE");
		}
		else
		...
		if(band_may_affect_fm_rf(resp_msg.rf_band_info_list[resp_msg.rf_band_info_list_len-1].active_band))	{
			...
			if( transport == BT_SOC_SMD ) {
				property_set("vendor.notch.value", "HIGH");
			}
			else
			...
			if( transport == BT_SOC_SMD) {
				property_set("vendor.notch.value", "LOW");
			}
			else
...
int  do_riva_calibration(){
	...
	fd = open(RADIO_DEVICE_PATH, O_RDONLY | O_CREAT, O_NONBLOCK);
	if(fd < 0)
	{
                free(str);
		return -1;
...
int get_bt_soc_type()
{
    ...
    printf("fm_qsoc_patches: get_bt_soc_type\n");
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
...
#ifdef ANDROID
	debug_file = fopen("/data/vendor/fm/fm_wa_mode", "w+");
#else
	debug_file = fopen("/var/log/fm_wa_mode", "w+");
		...
		transport = get_bt_soc_type();
		fprintf(debug_file,"vendor.qcom.bluetooth.soc = %d\n", transport);
		if( transport == BT_SOC_SMD ){
	...
	if(atoi(argv[2]) == 3) {
#ifdef ANDROID
		debug_file = fopen("/data/vendor/fm/fm_dacconfig_mode", "w+");
#else
	...
	transport = get_bt_soc_type();
	if( transport  == BT_SOC_SMD ){
		cal_fp = fopen("/data/vendor/fm/Riva_fm_cal","r" );
		if (!cal_fp ||
...
#ifdef ANDROID
	debug_file = fopen("/data/vendor/fm/fm_dld_enable", "w+");
#else
	...
	is_marimba = chip_is_Marimba(chipVersion);
        ...
        cal_file = open("/data/vendor/fm/fm_cal", O_RDONLY);
        if (cal_file < 0) {
