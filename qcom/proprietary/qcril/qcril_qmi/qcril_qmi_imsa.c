...
typedef struct
{
   ...
   boolean handover_occured;
   imsa_service_rat_enum_v01 handover_target_rat;
   boolean ims_wlan_registration_status;
...
boolean qcril_qmi_imsa_update_wlan_registration_status();
//===========================================================================
...
void qcril_qmi_imsa_init()
{
   ...
   qcril_qmi_imsa_info.ims_wlan_registration_status = FALSE;
   qcril_qmi_imsa_info_unlock();
...
void qcril_qmi_imsa_reg_status_ind_hdlr(void *ind_data_ptr)
{
   ...
   boolean nw_status_reported = FALSE;
   ...
   if (NULL != reg_ind_msg_ptr)
   {
      ...
      if(qcril_qmi_imsa_update_wlan_registration_status())
      {
      ...
      if(qmi_ril_is_feature_supported(QMI_RIL_FEATURE_RIL_DATA_REG_STATE_CONDITIONAL_REPORT))
      {
        ...
        if(unsol_nw_status && !nw_status_reported)
        {
...
void qcril_qmi_imsa_service_status_ind_hdlr(void *ind_data_ptr)
{
    ...
    if (NULL != service_ind_msg_ptr)
    {
        ...
        //clean up handover cache as it is no more needed to determine the data rat
        qcril_qmi_imsa_info.handover_occured = FALSE;
    ...
    if (NULL != rat_ho_ind_msg_ptr)
    {
        if (rat_ho_ind_msg_ptr->rat_ho_status_info_valid)
        {
            ...
            if((rat_ho_ind_msg_ptr->rat_ho_status_info.rat_ho_status == IMSA_STATUS_RAT_HO_SUCCESS_V01) &&
               (rat_ho_ind_msg_ptr->rat_ho_status_info.source_rat == IMSA_WLAN_V01 ||
                rat_ho_ind_msg_ptr->rat_ho_status_info.target_rat == IMSA_WLAN_V01))
            {
                //only when either of source or target rats is IWLAN and when handover is successful
                //we will store it in the cache
                qcril_qmi_imsa_info_lock();
                qcril_qmi_imsa_info.handover_occured = TRUE;
                qcril_qmi_imsa_info.handover_target_rat = rat_ho_ind_msg_ptr->rat_ho_status_info.target_rat;
                qcril_qmi_imsa_info_unlock();
...
/*!
    @brief
    Checking whether IMS registered or registering over WLAN RAT through

    @return
    If registered over WLAN, return TRUE
    If call handed over to IWLAN, return TRUE
    Else return FALSE
*/
/*=========================================================================*/
uint8_t qcril_qmi_imsa_ims_registered_wlan_status()
{
    ...
    qcril_ims_setting_wfc_status_type wifi_call;
    ...
    else if(qcril_qmi_imsa_info.handover_occured == TRUE && qcril_qmi_imsa_info.handover_target_rat == IMSA_WLAN_V01)
    {
        status = TRUE;
    }
    qcril_qmi_imsa_info_unlock();
...
uint8_t qcril_qmi_imsa_handover_occured()
{
    QCRIL_LOG_FUNC_ENTRY();
    uint8_t ret = FALSE;
    qcril_qmi_imsa_info_lock();
    ret = qcril_qmi_imsa_info.handover_occured;
    QCRIL_LOG_DEBUG("handover_occured: %d", qcril_qmi_imsa_info.handover_occured);
    if(ret)
    {
       QCRIL_LOG_DEBUG("target rat: %d", qcril_qmi_imsa_info.handover_target_rat);
    }
    qcril_qmi_imsa_info_unlock();
    QCRIL_LOG_FUNC_RETURN();
    return ret;
...
boolean qcril_qmi_imsa_update_wlan_registration_status()
{
  ...
  QCRIL_LOG_FUNC_ENTRY();
  qcril_qmi_imsa_info_lock();
  boolean wlan_status = qcril_qmi_imsa_ims_registered_wlan_status();
  if(qcril_qmi_imsa_info.ims_wlan_registration_status != wlan_status)
  {
     QCRIL_LOG_DEBUG("updating wlan reg status from %d to %d", qcril_qmi_imsa_info.ims_wlan_registration_status, wlan_status);
     qcril_qmi_imsa_info.ims_wlan_registration_status = wlan_status;
     changed = TRUE;
  }
  qcril_qmi_imsa_info_unlock();
...
uint8_t qcril_qmi_imsa_compare_ims_registered_report_unsol()
{
    ...
    else
    {
        ...
        else if(qcril_qmi_imsa_handover_occured())
        {
            QCRIL_LOG_INFO("Handover has occured which involves IWLAN rat");
            ret = TRUE;
        }
    }
...
)
{
    ...
    uint64_t* ims_sub_config_arr[QMI_DMS_MAX_SUBSCRIPTION_LIST_LEN_V01 + 1];
    int i;
    QCRIL_NOTUSED(ret_ptr); // Output parameter
