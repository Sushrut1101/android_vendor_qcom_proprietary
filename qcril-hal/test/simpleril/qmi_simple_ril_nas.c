...
int qmi_simple_ril_show_cdma_ntw_info_callback (qmi_util_service_message_info* callback_msg_info,qmi_simple_ril_cmd_completion_info* uplink_message)
{
    if (uplink_message->error_code == QMI_NO_ERR || uplink_message->error_code == QMI_IDL_LIB_MISSING_TLV)
        ...
        if(NULL != callback_msg_info->message_specific_payload)
            ...
            {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "\nCallback has no message payload\n");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);            
            }                                        
        }
    else
        {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] ERROR in sending the Get_Serving_Info Request, ERROR CODE:%d\n",uplink_message->error_code);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    return 0;
}
...
static void nas_cdma_print_serving_info_resp(nas_get_serving_system_resp_msg_v01* nas_arg_resp_msg,qmi_simple_ril_cmd_completion_info* uplink_message)
{
    ...
    if(QMI_RESULT_SUCCESS_V01 == nas_arg_resp_msg->resp.result)
    {
        ...
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Registration State : ");
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        switch(nas_arg_resp_msg->serving_system.registration_state)
        {
            case 0: //QMI_NAS_NOT_REGISTERED
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Not registered-mobile is not currently searching for a new network to provide service\n");
                break;
            case 1: //QMI_NAS_REGISTERED
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Registered with a network\n");
                break;
            case 2: //QMI_NAS_NOT_REGISTERED_SEARCHING
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Registration denied by the visible network \n");
                break;
            case 3: //QMI_NAS_REGISTRATION_DENIED
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Registered with a network\n");
                break;
            case 4: //QMI_NAS_REGISTRATION_UNKNOWN
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Registration state is unknown\n");
                break;
            default:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "QMI ERROR : Invalid Value\n");
        }
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);

        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Circuit switch domain attach state : ");
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        switch(nas_arg_resp_msg->serving_system.cs_attach_state)
        {
            case 0:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Unknown\n");
                break;
            case 1:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Attached\n");
                break;
            case 2:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Detached\n");
                break;
            default:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "QMI ERROR : Invalid Value\n");
        }
        ...
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Packet switch domain attach state : ");
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        ...
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Registered Network : ");
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        switch(nas_arg_resp_msg->serving_system.selected_network)
        {
            case 0:
                ...
                break;
            case 1:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "3GPP2 network\n");
                break;
            case 2:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "3GPP network\n");
                break;
            default:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "QMI ERROR : Invalid Value\n");
        }
        ...
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Radio Interface(s) currently in use : ");
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        if(nas_arg_resp_msg->serving_system.radio_if_len <= NAS_RADIO_IF_LIST_MAX_V01)
        {
            for(iter1 = 0; iter1 < nas_arg_resp_msg->serving_system.radio_if_len; iter1++)
            {
                ...
                {
                    case 0:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "No Service, ");
                        break;
                    case 1:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "cdma2000 1X, ");
                        break;
                    case 2:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "cdma2000 HRPD (1xEV-DO), ");
                        break;
                    case 3:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "AMPS, ");
                        break;
                    case 4:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "GSM, ");
                        break;
                    case 5:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "UMTS, ");
                        break;
                    default:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "QMI ERROR : Invalid Value");
                }            
        ...
        {
           snprintf(temp_char_array, TEMP_ARRAY_SIZE, "QMI ERROR : Invalid Length\n");
           qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }

        if(TRUE == nas_arg_resp_msg->detailed_service_info_valid)
        {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] Detailed Service Info : ");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] Service status : %d\n", nas_arg_resp_msg->detailed_service_info.srv_status);
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] Service capability : %d\n", nas_arg_resp_msg->detailed_service_info.srv_capability);
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] HDR Service status : %d\n", nas_arg_resp_msg->detailed_service_info.hdr_srv_status);
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] HDR hybrid : %d\n", nas_arg_resp_msg->detailed_service_info.hdr_hybrid);
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] Is system forbidden : %d\n", nas_arg_resp_msg->detailed_service_info.is_sys_forbidden);
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }

        if(TRUE == nas_arg_resp_msg->cdma_system_id_ext_valid) 
        {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] CDMA System Info : ");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] MCC : %d\n", nas_arg_resp_msg->cdma_system_id_ext.mcc);
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] IMSI 11 12 : %d\n", nas_arg_resp_msg->cdma_system_id_ext.imsi_11_12);
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
        if(TRUE == nas_arg_resp_msg->nas_3gpp_time_zone_valid)
        {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] 3GPP Time Zone Info: ");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] Leap seconds since start of CDMA time : %d\n", nas_arg_resp_msg->nas_3gpp_time_zone.lp_sec);
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] Offset (2's complement) in 30 mins increment : %d\n", nas_arg_resp_msg->nas_3gpp_time_zone.ltm_offset);
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, " [NAS_TEST] Daylight savings (1 = ON, 0 = OFF) : %d\n", nas_arg_resp_msg->nas_3gpp_time_zone.daylt_savings);
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    }
    else
    {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] ERROR in Get_Serving_Info Response, ERROR CODE:%d",nas_arg_resp_msg->resp.error);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    }
}
...
static void nas_wcdma_print_network_scan_resp(nas_perform_network_scan_resp_msg_v01* nas_arg_resp_msg,qmi_simple_ril_cmd_completion_info* uplink_message)
{
    ...
    if(QMI_RESULT_SUCCESS_V01 == nas_arg_resp_msg->resp.result)
    {
        if(TRUE == nas_arg_resp_msg->nas_3gpp_network_info_valid)
        {
            ...
            if(nas_arg_resp_msg->nas_3gpp_network_info_len <= NAS_3GPP_NETWORK_INFO_LIST_MAX_V01)
            {
                ...
                for(iter1 = 0; iter1 < nas_wcdma_networks_found_number; iter1++)
                {
                    ...
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Network ID - %d :\n",iter1+1);
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Description : %s\n",nas_arg_resp_msg->nas_3gpp_network_info[iter1].network_description);
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Country Code : %d\n",nas_arg_resp_msg->nas_3gpp_network_info[iter1].mobile_country_code);
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Network Code : %d\n",nas_arg_resp_msg->nas_3gpp_network_info[iter1].mobile_network_code);
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    
                    network_status = (nas_arg_resp_msg->nas_3gpp_network_info[iter1].network_status) & 3;                    
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] In-use status : ");
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    switch(network_status)
                    {
                        case 0:
                            ...
                            break;
                        case 1:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Current Serving\n");
                            break;
                        case 2:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Available\n");
                            break;
                        default:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "QMI ERROR : Invalid Value\n");
                    }            
                    ...
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Roaming status : ");
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    switch(network_status)
                    {
                        case 0:
                            ...
                            break;
                        case 1:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Home\n");
                            break;
                        case 2:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Roam\n");
                            break;
                        default:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "QMI ERROR : Invalid Value\n");
                    }            
                    ...
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Forbidden status : ");
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    switch(network_status)
                    {
                        case 0:
                            ...
                            break;
                        case 1:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Forbidden\n");
                            break;
                        case 2:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Not Forbidden\n");
                            break;
                        default:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "QMI ERROR : Invalid Value\n");
                    }            
                    ...
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Preferred status : ");
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    switch(network_status)
                    {
                        case 0:
                            ...
                            break;
                        case 1:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Preferred\n");
                            break;
                        case 2:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Not Preferred\n");
                            break;
                        default:
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "QMI ERROR : Invalid Value\n");
                    }            
        ...
        {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] No Info\n");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    }
    else
    {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] ERROR in Perform_Network_Scan Response, ERROR CODE:%d",nas_arg_resp_msg->resp.error);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    }
    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "\n");
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    ...
    if (qmi_msg_info->error_code == QMI_NO_ERR)
        ...
        if(NULL != qmi_msg_info->message_specific_payload)
            ...
            if (set_3gpp2_subs_resp->resp.result == QMI_RESULT_SUCCESS_V01)
                {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_Test] Set Service Status returned success\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                }
            else
                {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_Test] Set Service Status returned error code: %d\n", set_3gpp2_subs_resp->resp.error);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                }
            }
        else
            {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_Test] Set Service status callback has no message payload\n");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);            
            }                                                
        }
    else
        {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] ERROR in sending Set Service Status request, error code :%d\n",qmi_msg_info->error_code);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    return 0;
}
...
static void nas_print_serving_system_ind(nas_serving_system_ind_msg_v01* nas_arg_ind_msg,qmi_simple_ril_cmd_completion_info* uplink_message)
{
    ...
    {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Data Capabilities supported : ");
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        if(nas_arg_ind_msg->data_capabilities_len <= NAS_DATA_CAPABILITIES_LIST_MAX_V01)
        {
            for(iter1 = 0; iter1 < nas_arg_ind_msg->data_capabilities_len; iter1++)
            {
                ...
                {
                    case 1:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "GPRS, ");
                        break;
                    case 2:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "EDGE, ");
                        break;
                    case 3:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "HSDPA, ");
                        break;
                    case 4:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "HSUPA, ");
                        break;
                    case 5:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "WCDMA, ");
                        break;
                    case 6:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "CDMA, ");
                        break;
                    case 7:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "EV-DO REV 0, ");
                        break;
                    case 8:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "EV-DO REV A, ");
                        break;
                    case 9:
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "GSM, ");
                        break;
    ...
    {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Current PLMN : ");
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    ...
    {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[NAS_TEST] Roaming Indicator Information : ");
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        switch(nas_arg_ind_msg->roaming_indicator)
        {
            case 0:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Roaming");
                break;
            case 1:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Home");
                break;
            case 2:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Flashing");
                break;
            default:
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "Operator Defined value");
        }
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "\n");
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
...
int qmi_simple_ril_qmi_sar_power_handler(qmi_simple_ril_cmd_input_info* cmd_params, qmi_simple_ril_cmd_ack_info* ack_info)
{
    ...
    if (cmd_params->info_set->nof_entries != 1)
        ...
        strlcpy(str_param, cmd_params->info_set->entries[0], sizeof(str_param));
        ...
        for ( idx = RTE_FIRST; idx < RTE_MAX; idx++ ) 
        {
            switch ( reg_info[idx] ) 
            {
                case NAS_SYS_SRV_STATUS_SRV_V01:
                    if (*full_srv_tech) 
                    {
                        if((strlen(full_srv_tech) +1) < 128)
                        {
                        strlcat( full_srv_tech, ",", sizeof(full_srv_tech));  
                        }
                    }
                    if((strlen(full_srv_tech) + strlen(nas_tech_names[ idx ])) < 128)
                    {
                    strlcat( full_srv_tech, nas_tech_names[ idx ], sizeof(full_srv_tech) );  
                    }
                    break;
                ...
                case NAS_SYS_SRV_STATUS_LIMITED_REGIONAL_V01:
                    if (*ltd_srv_tech) 
                    {
                        if((strlen(ltd_srv_tech) +1) < 128)
                        {
                        strlcat( ltd_srv_tech, ",", sizeof(ltd_srv_tech));  
                        }
                    }
                    if((strlen(ltd_srv_tech) + strlen(nas_tech_names[ idx ])) < 128)
                    {
                    strlcat( ltd_srv_tech, nas_tech_names[ idx ], sizeof(ltd_srv_tech) );
                    }
                    break;
