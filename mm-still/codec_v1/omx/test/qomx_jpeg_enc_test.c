...
volatile uint32_t g_mmstillloglevel;
 ...
 **/
static const omx_test_color_format_t col_formats[MAX_COLOR_FMTS] =
{
...
OMX_ERRORTYPE omx_test_enc_configure_buffer_ext(omx_enc_test_t *p_client)
{
  ...
  QIDBG_HIGH("%s:%d] yOffset = %u, cbcrOffset = %u, totalSize = %u,"
 ...
 *
 **/
OMX_ERRORTYPE omx_test_encoding_mode(omx_enc_test_t *p_client)
{
  OMX_ERRORTYPE rc = 0;
  ...
  QIDBG_ERROR("%s:%d] encoding mode = %d ", __func__, __LINE__,
    (int)encoding_mode);
 ...
 *
 **/
static OMX_ERRORTYPE omx_test_enc_set_workbuf(
  omx_enc_test_t *p_client)
{
  OMX_ERRORTYPE ret = OMX_ErrorNone;
  ...
  if (!p_client->work_buf.addr || !p_client->work_buf.size) {
    QIDBG_ERROR("%s:%d] Error invalid input %d",
  ...
  ret = OMX_GetExtensionIndex(p_client->p_handle,
    ...
    &work_buffer_index);
  if (ret) {
    QIDBG_ERROR("%s:%d] Error getting work buffer index %d",
      __func__, __LINE__, ret);
    return ret;
  }
  ...
  QIDBG_ERROR("%s:%d] Work buffer %d %p WorkBufSize: %d", __func__, __LINE__,
 ...
 *  Arguments:
 *     @p_client: encoder test client
 *     @p_buffer: buffer pointer
 *     @p_image: image pointer
 *
 *  Return:
 *     Openmax error types
 *
...
OMX_ERRORTYPE omx_test_enc_read_file(const char *filename,
  ...
  omx_test_image_t *p_image __unused)
{
  OMX_ERRORTYPE ret = OMX_ErrorNone;
  FILE *fp = NULL;
  size_t file_size = 0;
  ...
  fseek(fp, 0, SEEK_SET);
  QIDBG_ERROR("%s:%d] input file size is %zu buf_size %zu",
    __func__, __LINE__, file_size, p_buffer->size);

  if (p_buffer->size < file_size) {
    QIDBG_ERROR("%s:%d] error %d %d", __func__, __LINE__,
      p_buffer->size,
      file_size);
    fclose(fp);
    return ret;
 ...
 *
 **/
OMX_ERRORTYPE omx_test_enc_ebd(OMX_OUT OMX_HANDLETYPE hComponent __unused,
 ...
 *
 **/
OMX_ERRORTYPE omx_test_enc_fbd(OMX_OUT OMX_HANDLETYPE hComponent __unused,
  OMX_OUT OMX_PTR pAppData, OMX_OUT OMX_BUFFERHEADERTYPE* pBuffer)
{
  ...
  DUMP_TO_FILE(p_client->output_file[p_client->fbd_count], pBuffer->pBuffer,
    (size_t)pBuffer->nFilledLen);

  QIDBG_MED("%s:%d] fbd count %d buf_count %d ebd %d %d",
    __func__, __LINE__,
    p_client->fbd_count,
    p_client->buf_count,
    p_client->ebd_count,
 ...
 *
 **/
OMX_ERRORTYPE omx_test_enc_event_handler(
...
error:
  ...
  ret = omx_test_enc_deallocate_buffer(&p_client->work_buf,
    TRUE);
  if (ret) {
    QIDBG_ERROR("%s:%d] Error %d", __func__, __LINE__, ret);
  }
...
void *omx_test_enc_encode(void *data)
{
  ...
  static char* lomx_name =
#ifdef OMX_JPEG_PIPELINE
  "OMX.qcom.image.jpeg.encoder_pipeline";
#else
  ...
  ret = OMX_GetHandle(&p_client->p_handle,
    lomx_name, p_client, &p_client->callbacks);
  ...
  /* set encoding mode */
  omx_test_encoding_mode(p_client);

  /* set work buffer */
  if (p_client->use_workbuf) {
    p_client->work_buf.size = p_client->total_size;
    ret = omx_test_enc_allocate_buffer(&p_client->work_buf, TRUE);
    if (ret) {
      QIDBG_ERROR("%s:%d] Error", __func__, __LINE__);
      goto error;
    }
    omx_test_enc_set_workbuf(p_client);
  }
...
#ifdef DUMP_INPUT
  DUMP_TO_FILE(QCAMERA_DUMP_FRM_LOCATION"test.yuv",
  ...
  DUMP_TO_FILE(QCAMERA_DUMP_FRM_LOCATION"testThumbnail.yuv",
...
void omx_test_enc_init_test_args(omx_enc_test_args_t *p_test)
{
  ...
  memset(p_test, 0, sizeof(omx_enc_test_args_t));
  ...
  omx_enc_test_args_t *p_test)
{
  ...
  while ((c = getopt(argc, argv, "I:O:W:H:F:Q:r:ti:w:h:f:p:um:n:x:y:M:d:N:"
    "Uj:k:X:Y:J:K:a:c:b:BPZ")) != -1) {
    switch (c) {
    case 'O':
    ...
    case 'i':
      ...
      break;
    case 'Q':
      p_test->main.quality = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "Main image quality", p_test->main.quality);
      break;
    case 'W':
      p_test->main.width = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "Input width", p_test->main.width);
      break;
    case 'w':
      p_test->thumbnail.width = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "Thumbnail width", p_test->thumbnail.width);
      break;
    case 'H':
      p_test->main.height = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "Input height", p_test->main.height);
      break;
    case 'h':
      p_test->thumbnail.height = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "Thumbnail height", p_test->thumbnail.height);
      break;
    case 'r':
      p_test->rotation = (int16_t)atoi(optarg);
    ...
    case 'F': {
      uint32_t format = 0;
      format = atoi(optarg);
      CLAMP(format, 0, MAX_COLOR_FMTS - 1);
    ...
    case 'u':
      ...
      break;
    case 'm':
      p_test->main_scale_cfg.input_width = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "main image scale input width",
        p_test->main_scale_cfg.input_width);
      break;
    case 'n':
      p_test->main_scale_cfg.input_height = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "main image scale input height",
        p_test->main_scale_cfg.input_height);
      break;
    case 'M':
      p_test->main_scale_cfg.output_width = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "main image scale output width",
        p_test->main_scale_cfg.output_width);
      break;
    case 'N':
      p_test->main_scale_cfg.output_height = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "main image scale output height",
        p_test->main_scale_cfg.output_height);
      break;
    case 'x':
      p_test->main_scale_cfg.h_offset = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "main image scale h offset",
        p_test->main_scale_cfg.h_offset);
      break;
    case 'y':
      p_test->main_scale_cfg.v_offset = (uint32_t)atoi(optarg);
    ...
    case 'U':
      ...
      break;
    case 'j':
      p_test->tn_scale_cfg.input_width = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "thumbnail scale input width",
        p_test->tn_scale_cfg.input_width);
      break;
    case 'k':
      p_test->tn_scale_cfg.input_height = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "thumbnail scale input height",
        p_test->tn_scale_cfg.input_height);
      break;
    case 'J':
      p_test->tn_scale_cfg.output_width = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "thumbnail scale output width",
        p_test->tn_scale_cfg.output_width);
      break;
    case 'K':
      p_test->tn_scale_cfg.output_height = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "thumbnail scale output height",
        p_test->tn_scale_cfg.output_height);
      break;
    case 'X':
      p_test->tn_scale_cfg.h_offset = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "thumbnail scale h offset",
        p_test->tn_scale_cfg.h_offset);
      break;
    case 'Y':
      p_test->tn_scale_cfg.v_offset = (uint32_t)atoi(optarg);
...
int main(int argc, char* argv[])
{
...
#ifdef _ANDROID_
  property_set("persist.vendor.camera.mmstill.logs", "3");
#endif
  ...
  OMX_Deinit();
  for (i = 0; i < test_args.instance_cnt; i++)
    fprintf(stderr, "Encoding %d completed %s in %llu milliseconds\n", i,
