...
//=========================================================================
extern "C" jlong Java_com_qualcomm_qti_seccamservice_SecCamServiceVendorHandler_startSecCamUsecase(
    JNIEnv* env, jobject thiz) {

    jlong ret = 0;
    ALOGD("startSecCamUsecase - Enter");

    lock();

    int retval = secCamStartUsecase();

    if (retval) {
        ALOGE("secCamStartUsecase - failed (%d)", retval);
    }

    ALOGI("secCamStartUsecase - ret_data is (%d)", retval);
    unlock();
...
//=========================================================================
extern "C" jlong Java_com_qualcomm_qti_seccamservice_SecCamServiceVendorHandler_stopSecCamUsecase(
    JNIEnv* env, jobject thiz) {

    jlong ret = 0;
    ALOGD("stopSecCamUsecase - Enter");

    lock();

    int retval = secCamStopUsecase();

    if (retval) {
        ALOGE("secCamStopUsecase - failed (%d)", retval);
    }

    ALOGI("secCamStopUsecase - ret_data is (%d)", retval);
    unlock();
