...
Copyright (c) 2008-2009, 2018 Qualcomm Technologies, Inc.
All Rights Reserved.
...
)
{
  ...
  if ( pFreeSpace != NULL && pPath != NULL)
  {
    ...
    {
      *pFreeSpace =  statvfs.f_bsize * (unsigned long long)statvfs.f_bavail;
      nResult = 0;
    }
  }
  return nResult;
...
)
{
  ...
  if ( pFreeSpace != NULL && pPath != NULL)
  {
    struct statfs64 statvfs;
    if (statfs64 ( pPath, &statvfs) == 0 )
    {
