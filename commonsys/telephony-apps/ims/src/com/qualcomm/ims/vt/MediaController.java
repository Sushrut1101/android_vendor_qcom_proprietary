...
public class MediaController implements ImsMedia.CameraListener,
    ...
    @Override
    public void onRecordingSurfaceChanged(Surface recordingSurface, int width, int height) {
    ...
    public void onCallTypeChanging(ImsCallSessionImpl session, int newCallType) {
        if (!ImsCallUtils.isVideoCall(session) && ImsCallUtils.isVideoCall(newCallType)) {
            maybeInitializeMedia();
        }
    }
