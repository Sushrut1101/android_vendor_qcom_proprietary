...
#else
  #define FILESOURCE_DLL __attribute__((visibility("default")))
#endif
...
typedef enum
{
  ...
  //! Below status are used if Callback function is not registered
  FILE_SOURCE_S_OPEN_COMPLETE = 100,
  FILE_SOURCE_S_OPEN_FAIL,
  FILE_SOURCE_S_OPEN_DATA_UNDERRUN,
  FILE_SOURCE_S_SEEK_COMPLETE,
  FILE_SOURCE_S_SEEK_UNDERRUN,
  FILE_SOURCE_S_SEEK_FAIL,
  FILE_SOURCE_S_ERROR_ABORT,
  FILE_SOURCE_S_SEEK_UNDERRUN_IN_FRAGMENT
} FileSourceStatus;
...
typedef enum
{
  ...
  FILE_SOURCE_AIFF,     //.aif                      <22>
  FILE_SOURCE_FLV,      //.flv                      <23>
  FILE_SOURCE_APE,      //.ape                      <24>
  FILE_SOURCE_DSF,      //.dsf                      <25>
  FILE_SOURCE_DSDIFF,   //.dif                      <26>
  FILE_SOURCE_WEBM,     //.webm                     <27>
  FILE_SOURCE_WFD_MP2TS, // .ts for WFD             <28>
  FILE_SOURCE_DASH_MP2TS,// .ts for DASH            <29>
  FILE_SOURCE_REAL,      //.rm                      <30>
  FILE_SOURCE_MOV,       // .mov, .3gp, .mp4        <31>
  FILE_SOURCE_MHAS,      // .mhas                   <32>
  FILE_SOURCE_RAW,       // .raw pcm/aac etc.       <33>
...
typedef enum
{
  ...
  FILE_SOURCE_MJ_TYPE_IMAGE,
  FILE_SOURCE_MJ_TYPE_META
...
typedef enum
{
  ...
  FILE_SOURCE_MN_TYPE_ALAC,
  FILE_SOURCE_MN_TYPE_SOWT,
  ...
  FILE_SOURCE_MN_TYPE_APE,
  FILE_SOURCE_MN_TYPE_DSD,
  FILE_SOURCE_MN_TYPE_META_TEXT,
  FILE_SOURCE_MN_TYPE_HEIF,
  FILE_SOURCE_MN_TYPE_MHAS,
  FILE_SOURCE_MN_TYPE_AV1,
  FILE_SOURCE_MN_TYPE_AC4,
...
typedef enum
{
  ...
  FILE_SOURCE_MD_MANUFACTURER,    //! Manufacture of the clip (in mp4)
...
typedef enum FS_MEDIA_INDEXTYPE
{
  ...
  FS_IndexParamAudioOPUS,       /*!< reference: FS_AUDIO_PARAM_OPUSTYPE */
  ...
  FS_IndexParamVideoSampleOfffset,  /**< ref: FS_CURRENT_SEEK_POINT_INFOTYPE */
  FS_IndexParamVideoColorInfo,          /**< reference: FS_VIDEO_PARAM_COLOR_INFOTYPE */
  FS_IndexParamVideoMasterMetaDataInfo, /**< reference: FS_VIDEO_PARAM_MASTER_METADATA_INFOTYPE */
  ...
  /* extra-data */
  FS_IndexParamExtraData        = 0x04000000,
  FS_IndexParamExtraDataAudioPreselection,
  FS_IndexParamExtraDataHDR10PlusInfo,
}FS_MEDIA_INDEXTYPE;

typedef enum
{
  FS_FORMAT_DATA_DEFAULT,
  FS_FORMAT_DATA_ESDS,
  FS_FORMAT_DATA_HEIF_HVCC,
  FS_FORMAT_DATA_HEIF_THUMBNAIL_HVCC,
  FS_FORMAT_DATA_HEIF_ICC,
  FS_FORMAT_DATA_MHAS_CONFIG,
  FS_FORMAT_DATA_MHAS_SCENEINFO,
  FS_FORMAT_DATA_VORBIS_BOOKS,
  FS_FORMAT_DATA_VORBIS_DATA,
  FS_FORMAT_DATA_VORBIS_INFO,
}FS_FORMAT_DATA_TYPE;
...
/*
 *@brief Enumeration to indicate seek mode.
*/
   typedef enum FS_SEEK_MODE{
     //! Depending on seek and playback times either do fwd or backward seek
     FS_SEEK_DEFAULT,
     //! Seek to next sync frame
     FS_SEEK_NEXT,
     //! Seek to Previous sync frame
     FS_SEEK_PREV,
     //! Seek to closest sync frame
     FS_SEEK_CLOSEST,
     //! Seek to closest frame not necessarily a sync frame
     FS_SEEK_CLOSEST_NOSYNC,
     //! Seek to frame at given index
     FS_SEEK_FRAME_INDEX
   }FS_SEEK_MODE;
/*!
 *@brief Structure to store subtitle track sub sample information.
 */
   typedef struct FS_SEEK_PARAMS{
     FS_SEEK_MODE eSeekMode;
     uint32       ulPlaybackRate;
   } FS_SEEK_PARAMS;
 ...
 *@brief Structure to store subtitle track sub sample information.
 */
typedef struct FS_SEEK_INFOTYPE{
  //! Target time in case of closest seek
  int64 targetTimeUs;
}FS_SEEK_INFOTYPE;
...
typedef struct
{
  ...
  //! Indicate if extra-data available
  bool bIsExtraDataAvailable;
...
typedef struct
{
  ...
  //!Max Frame size buffer to use
  uint32    ulMaxBufSize;
  //! track language
  uint8     ucLanguageCode[4];
  //! display width
  uint32     ulDisplayWidth;
  //! display height
  uint32     ulDisplayHeight;
  //! frame count
  uint32     ulFrameCount;
}FileSourceVideoInfo;
...
typedef struct
{
  //! Track Id
  uint32    id;
  //! Image codec ( eg: JPEG, PNG etc)
  FileSourceMnMediaType eImageCodec;
  //! time scale of the track
  uint32      ulTimeScale;
  //! total duration for the track in milliseconds
  uint64      ullDuration;
  ...
  uint8       ucImageBitWidth;
  //! Image Rotation
  uint16      ucRotation;
  //! Primary Image
  bool        bIsPrimary;
  //! Thumbnail width in pixels
  uint32      ulThumbnailWidth;
  //! Thumbnail height in pixels
  uint32      ulThumbnailHeight;
  //! Grid params
  bool        bIsGrid;
  //! Grid rows
  uint8       ucGridRows;
  ...
  uint32      ulTileHeight;
  //! Exif offset
  uint64      ullExifOffset;
  //! Exif size
  uint32      ulExifSize;
}FileSourceImageInfo;
/*!
*@brief Structure to store meta track information.
*/
typedef struct
{
  //! Track Id
  uint32    id;
  //! Meta minor type
  FileSourceMnMediaType metaCodec;
  //! time scale of the track
  uint32      ulTimeScale;
  //! total duration for the track in milliseconds
  uint64      ullDuration;
...
/*!
*@brief Structure to get Current Video frame details after seek opration.
        Video frame entry details are used for audo seek.
        For a proper interleaved clips, this reduces seek processing time for
        audio tracks.
*/
typedef struct FS_CURRENT_SEEK_POINT_INFOTYPE
{
  FileSourceMjMediaType eMajorType;          //! Track Type
  uint64                ullSyncSampleOffset; //! Last sample offset value
  uint64                ullSeekTime;         //! Last sample time
} FS_CURRENT_SEEK_POINT_INFOTYPE;
/*!
...
typedef struct FS_AUDIO_PARAM_ALACTYPE
{
  uint32 nFrameLength;
  uint8 nCompatibleVersion;
  uint8 nBitsPerSample;
  uint8 nPb;
  uint8 nMb;
  uint8 nKb;
  uint8 nChannels;
  uint16 nMaxRun;
  uint32 nMaxFrameBytes;
  uint32 nBitRate;
  uint32 nSampleRate;
  uint32 ulChannelLayoutTag;
} FS_AUDIO_PARAM_ALACTYPE;
...
typedef union
{
  ...
  FileSourceImageInfo sImageTrackInfo;
  FileSourceMetaInfo  sMetaTrackInfo;
} MediaTrackInfo;
...
typedef struct
{
  ...
  //! extra-data length
  uint32  ulDataLength;
  //! Buffer to fill extra-data
  uint8*  pucData;
} FS_PARAM_EXTRA_DATATYPE;
...
typedef enum FS_AUDIO_DTS_SUBTYPE
{
  ...
  FILE_SOURCE_SUB_MN_TYPE_DTS_BASE_LAYER,
  FILE_SOURCE_SUB_MN_TYPE_DTS_ENH_LAYER,
...
/*!
*@brief Defines OPUS stream specific parameter.
*/
typedef struct FS_AUDIO_PARAM_OPUSTYPE
{
  //! Opus codec delay value in nanoseconds
  //! This will be descarded from the start of the stream
  uint64 ullCodecDelay;
  //! Opus seek pre rool delay value in nanoseconds.
  //! This must be discarded after as seek for that stream
  uint64 ullSeekPreRoll;
}FS_AUDIO_PARAM_OPUSTYPE;

/*!
*@brief Defines video color parameter.
*/
typedef struct FS_VIDEO_PARAM_COLOR_INFOTYPE
{
  //matrix coefficients
  int64 llMatrixCoefficients;
  //bits per channel
  int64 llBitsPerChannel;
  //horizonal chroma subsample
  int64 llChromaSubsamplingHorz;
  //vertical chroma subsample
  int64 llChromaSubsamplingVert;
  //horizonal cb subsample
  int64 llCbSubsamplingHorz;
  //vertical cb subsample
  int64 llCbSubsamplingVert;
  //horizonal chroma siting
  int64 llChromaSitingHorz;
  //vertical chroma siting
  int64 llChromaSitingVert;
  //range
  int64 llRange;
  //transfer characteristics
  int64 llTransferCharacteristics;
  //primaries
  int64 llPrimaries;
  //maxCLL
  int64 llMaxCLL;
  //maxFALL
  int64 llMaxFALL;
}FS_VIDEO_PARAM_COLOR_INFOTYPE;

/*!
*@brief Defines video METADATA parameter.
*/
typedef struct FS_VIDEO_PARAM_MASTER_METADATA_INFOTYPE
{
  //chroma coordinate for R
  double fPrimChromaticityR_XY[2];
  //chroma coordinate for G
  double fPrimChromaticityG_XY[2];
  //chroma coordinate for B
  double fPrimChromaticityB_XY[2];
  //chroma coordinate for while
  double fPrimChromaticityWhile_XY[2];
  //luma max
  double fLuminance_Max;
  //luma min
  double fLuminance_Min;
}FS_VIDEO_PARAM_MASTER_METADATA_INFOTYPE;
...
/*!
*@brief Defines structure to be used to store file information
*/
typedef struct _FILE_INFO_TYPE_ {
  uint64 ullFileSize;
...
{
  uint8*  pDataBuff;    //! Buffer pointer to store data
  uint32  nDataBufSize; //! Actual buffer allocated size
  uint32  nDataSize;    //! Data filled into the buffer
  uint32  nReadIndex;   //! Amount of data consumed
  double  nBaseTime;    //! Timestamp after seek operation
  double  nFrameTime;   //! Timestamp for the current sample/frame
  float   nDelta;       //! Frame delta or duration
  uint8   nFrameHeaderLen;//! Frame header size to strip (if user requests)
}audio_data_buffer;
#endif
