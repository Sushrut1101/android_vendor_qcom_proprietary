...
void LocHidlIzatSubscription::IzatSubscriptionCallback::updateSubscribe(
    ...
    for (std::list<DataItemId>::const_iterator it = l.begin();
        ...
        DataItemId id = *it;
        LocHidlSubscriptionDataItemId sid =
                LocHidlUtils::translateToSubscriptionDataItemId(id);
        dataItemVec[idx] = sid;
    }
...
void LocHidlIzatSubscription::IzatSubscriptionCallback::turnOnModule(
    ...
    LocHidlSubscriptionDataItemId sdi =
            LocHidlUtils::translateToSubscriptionDataItemId(dit);

    TO_HIDL_CLIENT();
    auto r = mHidlCbIface->turnOnModule(sdi, timeOut);
    if (!r.isOk()) {
        LOC_LOGE("Error invoking HIDL CB [%s]", r.description().c_str());
    }
}
...
void LocHidlIzatSubscription::IzatSubscriptionCallback::turnOffModule(
    ...
    TO_HIDL_CLIENT();
    auto r = mHidlCbIface->turnOffModule(sdi);
    if (!r.isOk()) {
        LOC_LOGE("Error invoking HIDL CB [%s]", r.description().c_str());
    }
}
