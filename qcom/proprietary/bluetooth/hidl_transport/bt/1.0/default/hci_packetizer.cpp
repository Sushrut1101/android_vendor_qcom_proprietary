...
size_t GetPreambleSizeForType(HciPacketType type)
{
...
size_t GetPacketLenOffsetForType(HciPacketType type)
{
  size_t offset = 0;
...
void HciPacketizer::OnDataReady(int fd, HciPacketType packet_type)
{
  switch (state_) {
    case HCI_PREAMBLE: {
      ...
      if(bytes_read < 0) {
        ALOGE("preamble read returned %ld error %s", bytes_read, strerror(errno));
        ...
        if(++retry_read_ < 5) {
          ...
          ALOGE("read returned %ld retry_read_ %d", bytes_read, retry_read_);
    ...
    case HCI_PAYLOAD: {
      ...
      if(bytes_read < 0) {
        ALOGE("payload read returned %ld error %s", bytes_read, strerror(errno));
        if((-1 == bytes_read ) && (( EAGAIN == errno)||(EWOULDBLOCK == errno)||( EIO == errno)))
          break;
      } else if(bytes_read == 0) {
        if(++retry_read_ < 5) {
