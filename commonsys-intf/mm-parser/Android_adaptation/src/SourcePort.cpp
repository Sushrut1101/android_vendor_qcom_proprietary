...
SourcePort::SourcePort(DataSourceHelper *source)
    : mDataSource(source) {
    mOffset = 0;
    mRefCnt = 0;
}
...
SourcePort::~SourcePort() {
  ...
  */
  iStreamPort::DataSourceReturnCode SourcePort::ReadAt(
...
iStreamPort::DataSourceReturnCode SourcePort::Read(/*rout*/ unsigned char* pBuf,
                                                   ...
                                                   /*rout*/ ssize_t* pnRead){
    if (!pBuf) {
        LOGE("SourcePort::Read Error :: Buffer is NULL");
        return DS_FAILURE;
    }
    ssize_t nRet  = mDataSource->readAt(mOffset, (void*)pBuf, (size_t)nBufSize);
    if (nRet == -EAGAIN) {
        *pnRead = 0;
        LOGV("SourcePort::Read under-run @ %lld",(long long)mOffset);
        return DS_WAIT;
    }
    else if (nRet <= 0) {
        *pnRead = 0;
        LOGV("SourcePort::Read failed @ %lld with error code %zu",(long long)mOffset, nRet);
        return DS_FAILURE;
    }
    *pnRead = (ssize_t)nRet;
    return DS_SUCCESS;
...
iStreamPort::DataSourceReturnCode SourcePort::GetContentLength(/*rout*/ int64* pContentLength){
   ...
   if (eRet == -1010) return DS_UNSUPPORTED;
...
iStreamPort::DataSourceReturnCode SourcePort::GetSourceType(/*rout*/ DataSourceType* pSourceType){
    if (mDataSource->flags() &
        (DataSourceBase::kWantsPrefetching | DataSourceBase::kIsCachingDataSource)) {
...
iStreamPort::DataSourceReturnCode SourcePort::GetAvailableOffset(/*rout*/ int64* pAvailableOffset,
    ...
    *pbEOS = true;
    ...
    LOGV("SourcePort::GetAvailableOffset pAvailableOffset = %lld", *pAvailableOffset);
