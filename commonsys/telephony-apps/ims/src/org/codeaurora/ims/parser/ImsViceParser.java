...
public class ImsViceParser extends ConfInfo {
    ...
    public void updateViceXmlBytes(byte[] vicexml) {
        ...
        try {
            ...
            //Start the xml dump from a new line
    ...
    public List<ImsExternalCallState> getCallPullInfo(
        ...
        for (int i = 0; i < mDialogIds.size(); i++) {
            ...
            final DialogInfo info = mDialogIds.get(i);
            final String callId = info.dialogId;
            final Uri remoteAddress = Uri.parse(info.remoteIdentity);
            final Uri localAddress = Uri.parse(info.localIdentity);
            final boolean isPullable = isCallPullable(info,
                    isVideoSupported, isVolteSupported);
            final int callState = getCallState(info);
            final int callType = mapViceCallTypeToImsCallProfileCallType(
                    getCallType(info));
            final boolean isHeld = isCallHeld(getCallType(info));

            extCallState = new ImsExternalCallState(callId, remoteAddress,
                    localAddress, isPullable, callState, callType, isHeld);
