...
public class UimRemoteClientService extends Service {
    ...
    private class RadioStateListener extends PhoneStateListener {
        public RadioStateListener(int subId) {
            mSubId = subId;
        }
        @Override
        public void onRadioPowerStateChanged(int state) {
            Log.i(LOG_TAG, "state: " + state + "slot: " + mSubId);
            Message msg = mRadioStateHdlr.obtainMessage(state, mSubId, 0);
            msg.sendToTarget();
        }
    }
    ...
    public void onCreate() {
        ...
        mUimRemoteClientHals = new UimRemoteServiceClient[simSlots];
        tm = new TelephonyManager[simSlots];
        rl = new RadioStateListener[simSlots];

        for(int i = 0; i < simSlots; i++) {
            rl[i] = new RadioStateListener(i);
            tm[i] = manager.createForSubscriptionId(i);
            tm[i].listen(rl[i], PhoneStateListener.LISTEN_RADIO_POWER_STATE_CHANGED);
    ...
    public void onDestroy() {
        ...
        for(int i = 0; i < simSlots; i++) {
            tm[i].listen(rl[i], PhoneStateListener.LISTEN_NONE);
        ...
        private final class ServiceNotification extends IServiceNotification.Stub {
            ...
            public void onRegistration(String fqName, String name, boolean preexisting) {
                ...
                if (mUimremoteClient != null &&
                    mCb != null &&
                    mInstance < simSlots && tm[mInstance] != null) {
                    try {
                        if(tm[mInstance].getRadioPowerState() == TelephonyManager.RADIO_POWER_ON) {
                            mCb.uimRemoteRadioStateIndication(mInstance, 2);
                        }
                        else if(tm[mInstance].getRadioPowerState() == TelephonyManager.RADIO_POWER_OFF) {
