...
Return<void> LocHidlDebugReportService::getReport(int32_t maxReports, getReport_cb _hidl_cb) {
    LocHidlSystemStatusReports hidlReports = {};
    hidlReports.base_1_0.mSuccess = false;

    if (maxReports <= 0) {
        LOC_LOGE("Invalid maxReports %d", maxReports);
        _hidl_cb(hidlReports.base_1_0);
        return Void();
    }
    if (mSystemStatus == NULL) {
...
void LocHidlDebugReportService::populateRfParamsReport(
    ...
    hidlReports.base_1_0.mRfAndParamsVec.resize(count);

    for (auto p = make_pair(0, systemReports.mRfAndParams.begin());
         (p.first < maxReports) && (p.second != systemReports.mRfAndParams.end());
         p.first++, ++p.second) {

        vendor::qti::gnss::V1_0::ILocHidlDebugReportService::LocHidlSystemStatusRfAndParams&
                status_1_0 = hidlReports.base_1_0.mRfAndParamsVec[p.first];
        status_1_0.mUtcTime.tv_sec = (*p.second).mUtcTime.tv_sec;
        status_1_0.mUtcTime.tv_nsec = (*p.second).mUtcTime.tv_nsec;

        status_1_0.mUtcReported.tv_sec = (*p.second).mUtcReported.tv_sec;
        status_1_0.mUtcReported.tv_nsec = (*p.second).mUtcReported.tv_nsec;

        status_1_0.mPgaGain = (*p.second).mPgaGain;
        status_1_0.mAdcI = (*p.second).mAdcI;
        status_1_0.mAdcQ = (*p.second).mAdcQ;
        status_1_0.mJammerGps =(*p.second).mJammerGps;
        status_1_0.mJammerGlo = (*p.second).mJammerGlo;
        status_1_0.mJammerBds = (*p.second).mJammerBds;
        status_1_0.mJammerGal = (*p.second).mJammerGal;
        status_1_0.mGpsBpAmpI = (*p.second).mGpsBpAmpI;
        status_1_0.mGpsBpAmpQ = (*p.second).mGpsBpAmpQ;
