...
{
    if(getAttribOK() == true) {
        svc_num = SNS_SAM_TAP_SVC_ID_V01;
...
int Tap::enable(int en)
{
    ...
    if (enabled == en) {
        HAL_LOG_INFO("TAP is already enabled/disabled %d", enabled);
        return 0;
    }
...
===========================================================================*/
void Tap::processResp(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    if(true != error ) {
        ...
        case SNS_SAM_TAP_ENABLE_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_TAP_ENABLE_RESP_V01", __FUNCTION__);
            instance_id = ((sns_sam_tap_enable_resp_msg_v01 *)msg_ptr)->instance_id;
            break;
        case SNS_SAM_TAP_CANCEL_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_TAP_CANCEL_RESP_V01", __FUNCTION__);
            /* Reset instance ID */
            instance_id = 0xFF;
            break;
        case SNS_SAM_TAP_GET_ATTRIBUTES_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_TAP_GET_ATTRIBUTES_RESP_V01", __FUNCTION__);
...
/*===========================================================================
  FUNCTION:  processInd
===========================================================================*/
void Tap::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    switch( msg_hdr->msg_id )
    {
        case SNS_SAM_TAP_REPORT_IND_V01:
            HAL_LOG_DEBUG("%s: SNS_SAM_TAP_REPORT_IND_V01", __FUNCTION__);
        ...
        case SNS_SAM_TAP_ERROR_IND_V01:
            HAL_LOG_ERROR("%s: SNS_SAM_TAP_ERROR_IND_V01", __FUNCTION__);
            error = true;
            break;
        default:
    ...
    if (error == false) {
        ...
        HAL_LOG_VERBOSE( "%s: TAP: %f SAM TS: %u HAL TS:%" PRId64 " elapsedRealtimeNano:%" PRId64 "",
