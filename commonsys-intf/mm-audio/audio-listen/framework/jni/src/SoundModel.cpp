                                      ...
                                      listen_sound_model_info * pModelInfo)
{
   ...
   status = querySoundModel(pKeywordModel, pModelInfo);
                             ...
                             const char *        userStr )
{
   ...
   status = getSoundModelHeader(pModel, &modelHeader);
   if (kSucess != status) {
   ...
   if (keywordStr != NULL) {
      ...
      status = getKeywordPhrases(pModel, &numKWs, keywordPhrases);
      if (kSucess != status) {
   ...
   if (userStr != NULL) {
      ...
      status = getUserNames(pModel, &numUsers, userNames);
      if (kSucess != status) {
                 ...
                 jobject                   soundModelInfo)  // ListenTypes$SVASoundModelInfo
{
   ...
   status = getKeywordPhrases(pModel, &numKeyphrases, keywordPhrases);
   if (kSucess != status) {
   ...
   for (int kw=0; kw < numKeyphrases; kw++) {
      ...
      if (NULL != modelHeader.numUsersSetPerKw)
          numUserForThisKW = modelHeader.numUsersSetPerKw[kw];
      ...
      if (numUserForThisKW > 0) {
         ...
         for (int u=0, iActiveUserForKW = 0; u < numUsers; u++) {
            if (NULL != modelHeader.userKeywordPairFlags) {
                pUserActiveFlag = modelHeader.userKeywordPairFlags[u];
            } else if (pUserActiveFlag != NULL) {
                for (int i = 0; i <= kw; i++) {
                    pUserActiveFlag[i] = 0;
                }
            } else {
