...
OMX_ERRORTYPE XmlParser::ProcessMasterConfig(VideoStaticProperties* pGlobalVideoProp)
{
    ...
    pGlobalVideoProp->sRoiQpRoot[0] = '\0';
    ...
    while (pElem)
    {
        ...
        } else if (!STRCMP((OMX_STRING)pElem->name, "ROIQpFileRootPath")) {
            STRCPY(pGlobalVideoProp->sRoiQpRoot, (OMX_STRING)pElem->children->content);
            VTEST_MSG_LOW("MasterXml: RoiQpRoot: %s", pGlobalVideoProp->sRoiQpRoot);
