...
using ILocHidlDebugReportServiceV1_1 = ::vendor::qti::gnss::V1_1::ILocHidlDebugReportService;
...
static void DebugReportService_class_init(JNIEnv* env, jclass clazz)
{
    ...
    registerDeathRecipient("DebugReportService", []() {
            debugReportServiceIface = nullptr;
            ALOGE("ILocHidlDebugReportService died");
            DebugReportService_init(nullptr, nullptr);
    });
}
...
static void populateRfStateReportListObjV1_1(
        JNIEnv* env, jint maxReports, jobject& rfStateReportListObj,
        hidl_vec<ILocHidlDebugReportServiceV1_1::LocHidlSystemStatusRfAndParams>& rfParamsReportVec) {
    ...
    for (int i=0; i < count && i < maxReports; i++) {
        ...
        ILocHidlDebugReportServiceV1_1::LocHidlSystemStatusRfAndParams& status =
                rfParamsReportVec[i];
        ...
        jobject xtraReportListObj)
{
    ...
    TO_HIDL_SERVICE();
    sp<ILocHidlDebugReportServiceV1_1> ifaceV1_1 = ILocHidlDebugReportServiceV1_1::castFrom(iface);
    if (ifaceV1_1 != nullptr) {
        ILocHidlDebugReportServiceV1_1::LocHidlSystemStatusReports allReports = {};
        auto r = ifaceV1_1->getReport_1_1(
                maxReports,
                [&allReports]
                (const ILocHidlDebugReportServiceV1_1::LocHidlSystemStatusReports& reports) {
                    allReports = reports;
                });
        if (!r.isOk()) {
            ALOGE("Error invoking HIDL API [%s]", r.description().c_str());
        ...
        populateRfStateReportListObjV1_1(
        ...
        auto r = iface->getReport(
                maxReports,
                [&allReports]
                (const ILocHidlDebugReportService::LocHidlSystemStatusReports& reports) {
                    allReports = reports;
                });
        if (!r.isOk()) {
            ALOGE("Error invoking HIDL API [%s]", r.description().c_str());
