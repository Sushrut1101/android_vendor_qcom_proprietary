...
static int wifi_chan_rf_characterization_info_handler(struct nl_msg *msg,
        void *arg)
{
    struct nlattr *tb[NL80211_ATTR_MAX + 1];
    struct genlmsghdr *gnlh = NULL;
    struct nlattr *gnld = NULL;
    struct nl80211_chan_rf_characterization_info_list *val = NULL;
    void *temp_msg_ptr = NULL;
    uint32_t msg_len = 0;
    uint8_t num_entries_to_copy = 0;
    uint8_t num_entries_in_msg = 0;

    ICM_ASSERT(msg != NULL);
    ICM_ASSERT(arg != NULL);

    val = arg;
    val->num_prichannels = 0;

    gnlh = nlmsg_data(nlmsg_hdr(msg));
    if (gnlh == NULL) {
        icm_printf("nl80211: nlmsg_data() returned NULL. Investigate.\n");
        return NL_SKIP;
    }

    gnld = genlmsg_attrdata(gnlh, 0);
    if (gnld == NULL) {
        icm_printf("nl80211: genlmsg_attrdata() returned NULL. Investigate.\n");
        return NL_SKIP;
    }

    if (nla_parse(tb, NL80211_ATTR_MAX, gnld, genlmsg_attrlen(gnlh, 0), NULL)) {
        icm_printf("nl80211: nla_parse() failed\n");
        return NL_SKIP;
    }
    ...
    if (tb[NL80211_ATTR_VENDOR_DATA]) {
        ...
        nld = nla_data(nl_vendor);
        if (nld == NULL) {
            icm_printf("nl80211: nla_data() returned NULL. Investigate.\n");
            return NL_SKIP;
        }

        if (nla_parse(tb_vendor, NL80211_ATTR_MAX_INTERNAL, nld,
                    nla_len(nl_vendor), NULL)) {
            icm_printf("nl80211: nla_parse() failed\n");
            return NL_SKIP;
        }
        ...
        /*
         * The vendor specific data expected here is an array of zero or more
         * nl80211_chan_rf_characterization_info entries.
         */
        if (tb_vendor[QCA_WLAN_VENDOR_ATTR_PARAM_LENGTH]) {
            msg_len =
                nla_get_u32(tb_vendor[QCA_WLAN_VENDOR_ATTR_PARAM_LENGTH]);
        ...
        if (msg_len > 0) {
            num_entries_in_msg = msg_len/
                sizeof(struct nl80211_chan_rf_characterization_info);

            if (num_entries_in_msg == 0) {
                icm_printf("nl80211: Number of entries in message determined "
                           "to be 0. Message length=%u, size of RF "
                           "characterization info structure=%zu. Investigate."
                           "\n",
                           msg_len,
                           sizeof(struct \
                               nl80211_chan_rf_characterization_info));
                return NL_SKIP;
            }

            if ((num_entries_in_msg *
                        sizeof(struct nl80211_chan_rf_characterization_info)) !=
                    msg_len) {
                icm_printf("nl80211: Message length=%u is not an exact "
                           "multiple of size of RF characterization info "
                           "structure=%zu. Investigate.\n",
                           msg_len,
                           sizeof(struct \
                               nl80211_chan_rf_characterization_info));
                return NL_SKIP;
            ...
            if (tb_vendor[QCA_WLAN_VENDOR_ATTR_PARAM_DATA]) {
                ...
                if (temp_msg_ptr == NULL) {
                    icm_printf("nl80211: Error while getting param data. "
                               "nla_data() returned NULL. Investigate.\n");
                    return NL_SKIP;
                }

                num_entries_to_copy =
                    MIN_INT(num_entries_in_msg, ARRAY_LEN(val->prichaninfo));
                memcpy((void*)val->prichaninfo, temp_msg_ptr,
                       num_entries_to_copy *
                          sizeof(struct nl80211_chan_rf_characterization_info));

                val->num_prichannels = num_entries_to_copy;
            }
        }
    }
...
/*
 * driver_get_wifi_chan_rf_characterization_info - Get radio level channel
 * RF characterization information.
 * @ctx: nl80211 context
 * @ifname: Name of radio interface
 * @val: pointer to nl80211_chan_rf_characterization_info_list structure to be
 * populated
 * Return: 0 on success, -1 on error
 */
int driver_get_wifi_chan_rf_characterization_info(void *ctx,
                         const char *ifname,
                         struct nl80211_chan_rf_characterization_info_list *val)
{
    int ret = -1;
    int status = -1;
    struct nl_msg *msg = NULL;
    ...
    if (nla_put_u32(msg,
                QCA_WLAN_VENDOR_ATTR_CONFIG_GENERIC_COMMAND,
                QCA_NL80211_VENDOR_SUBCMD_GET_CHAN_RF_CHARACTERIZATION_INFO)) {
        icm_printf("nl80211: nla_put_u32() failed\n");
        goto fail;
    }
    ...
    status = send_and_recv_msgs(drv->global, msg,
                    wifi_chan_rf_characterization_info_handler,
                    val);
    msg = NULL;

    if (status) {
        icm_printf("nl80211: get command for radio channel RF characterization "
                   "info failed status=%d (%s)\n", status, strerror(-status));
        goto fail;
    } else {
        return 0;
    }
