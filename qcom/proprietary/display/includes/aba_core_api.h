...
/*=================================================================================================
  ...
  Algorithm (ABA) feature for display enhancements on Qualcomm Technologies,
  Inc. MSM display solutions. It reduces display power consumption by adjusting
  pixel luminance and, in some cases, backlight. There are multiple algorithms
  implemented by ABA: CABL, FOSS...
...
Function: AbaDynamicConfigValidate

This function validates the dynamically configurable parameters. Can be called anytime after
AbaInit is successful. Use the feature flag to indicate the feature for which the
validation is requested.


Parameters:
pHandle           - [in]:  Handle to the ABA context.
pAbaDynParameters - [in]:  Pointer to dynamic parameters from the client.
uParamSize        - [in]:  Size of the structure pointed to by pAbaDynParameters.
uFeature          - [in]:  Feature to apply the setting to.

Return:
AbaStatusType
-------------------------------------------------------------------------------------------------*/
AbaStatusType AbaDynamicConfigValidate(
  void                *pHandle,
  void                *pAbaDynParameters,
  uint32_t             uParamSize,
  uint32_t             uFeature);
...
Function: AbaDeactivate2
  ...
  Call AbaDeactivate2 on a feature if converging is desired before feature reaches inactive state.
...
-------------------------------------------------------------------------------------------------*/
AbaStatusType AbaDeactivate2(void *pHandle, uint32_t uFeature);
...
Function: AbaSuspend
  ...
  Changes the ABA state to suspend. When in the suspend state, ABA will not perform any operation on the histograms.

Parameters:
  pHandle     - [in]:   Handle to Aba module.
  pLUT        - [out]:  Caller allocated pointer used by ABA to return the LUT.
  pBacklight  - [out]:  Optional: caller pointer to return the calculated backlight value.

Return:
  AbaStatusType

-------------------------------------------------------------------------------------------------*/
AbaStatusType AbaSuspend(void *pHandle, uint32_t *pLUT, uint32_t *pBacklight);
