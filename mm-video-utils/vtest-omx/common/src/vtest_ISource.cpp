...
ISource::ISource()
    ...
    snprintf(m_pName, MAX_NAME_LENGTH, "%s", "ISource");
}
...
OMX_ERRORTYPE ISource::Start() {
    ...
    OMX_ERRORTYPE result = OMX_ErrorNone;
    {
        ...
        // Do not start if already stopped, can turn into
        // an infinite loop on error.
        if (m_pThread->Started() || m_bThreadStop) {
            return result;
        }
...
OMX_ERRORTYPE ISource::AllocateBuffers(BufferInfo **pBuffers,
    ...
    (void)pBuffers, (void)pExtraDataBuffers, (void)nWidth, (void)nHeight;
    (void)nBufferCount, (void)nBufferSize, (void)nExtraDataBufferCount;
    (void)nExtraDataBufferSize, (void)ePortIndex, (void)nBufferUsage;
    Mutex::Autolock autoLock(m_pLock);
...
OMX_ERRORTYPE ISource::UseBuffers(BufferInfo **pBuffers,
    ...
    (void)ppExtraDataBuffers, (void)nExtraDataBufferCount;
...
OMX_ERRORTYPE ISource::FreeUsedBuffers(
        BufferInfo **pBuffers, OMX_BUFFERHEADERTYPE **ppExtraDataBuffers,
        OMX_U32 nBufferCount, OMX_U32 nExtraDataBufferCount, OMX_U32 ePortIndex) {

    OMX_ERRORTYPE result = OMX_ErrorNone;
    OMX_BUFFERHEADERTYPE **ppTempExtraDataBuffers = ppExtraDataBuffers;
    ...
    if (ePortIndex == PORT_INDEX_OUT)
        for (OMX_U32 i = 0; i < nExtraDataBufferCount; i++) {
            ppTempExtraDataBuffers = ppExtraDataBuffers;
            VTEST_MSG_LOW("%s, free-use 0x%p FreeExtraDataBuffer",Name(),
                          ppExtraDataBuffers);
            result = FreeExtraDataBuffer(ppExtraDataBuffers, ePortIndex+2);
            ppExtraDataBuffers++;
        }
    return result;
}
...
OMX_ERRORTYPE ISource::FreeExtraDataBuffer(
        OMX_BUFFERHEADERTYPE **pExtraDataBuffer, OMX_U32 ePortIndex) {
    (void)pExtraDataBuffer, (void)ePortIndex;
    VTEST_MSG_MEDIUM("%s: unimplemented FreeBuffer", Name());
    return OMX_ErrorNone;
}
