...
vfe_status_t vfe_test_vector_init(vfe_test_vector_t *mod, void* vfe_obj)
{
  ...
  property_get("persist.vendor.camera.vfe.test.enable", value, "0");
  ...
  property_get("persist.vendor.camera.vfe.test.index", value, "0");
...
vfe_status_t vfe_test_vector_validate(vfe_test_vector_t *mod)
{
  ...
  CDBG("%s: enter", __func__);
  status = vfe_wb_tv_validate(&mod->mod_input, &mod->mod_output);
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_wb_tv_validate failed", __func__);
  }

  status = vfe_gamma_tv_validate(&mod->mod_input, &mod->mod_output);
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_gamma_tv_validate failed", __func__);
  }

  status = vfe_color_correct_tv_validate(&mod->mod_input, &mod->mod_output);
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_color_correct_tv_validate failed", __func__);
  }

  status = vfe_color_conversion_tv_validate(&mod->mod_input, &mod->mod_output);
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_color_conversion_tv_validate failed", __func__);
  }

  status = vfe_abf_tv_validate(&mod->mod_input, &mod->mod_output);
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_abf_tv_validate failed", __func__);
  }

  status = vfe_clf_tv_validate(&mod->mod_input, &mod->mod_output);
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_clf_tv_validate failed", __func__);
  }

  status = vfe_linear_test_vector_validate(&(mod->mod_input),
    &(mod->mod_output));
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_linear_test_vector_validate failed", __func__);
  }

  status = vfe_mce_test_vector_validate(&(mod->mod_input),
    &(mod->mod_output));
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_mce_test_vector_validate failed", __func__);
  }

  status = vfe_sce_test_vector_validate(&(mod->mod_input),
    &(mod->mod_output));
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_sce_test_vector_validate failed", __func__);
  }

  status = vfe_chroma_suppression_test_vector_validate(&(mod->mod_input),
    &(mod->mod_output));
  if (VFE_SUCCESS != status) {
  ...
  status = vfe_rolloff_tv_validate(&mod->mod_input,
    &mod->mod_output);
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_pca_tv_validate failed", __func__);
  }

  status = vfe_demosaic_test_vector_validation(&(mod->mod_input),
    &(mod->mod_output));
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_demosaic_test_vector_validation failed", __func__);
  }
  status = vfe_bcc_test_vector_validation(&(mod->mod_input),
    &(mod->mod_output));
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_bcc_test_vector_validation failed", __func__);
  }
  status = vfe_bpc_test_vector_validation(&(mod->mod_input),
    &(mod->mod_output));
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_bpc_test_vector_validation failed", __func__);
  }
  status = vfe_asf_test_vector_validation(&mod->mod_input, &mod->mod_output);
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_asf_tv_validate failed", __func__);
  }
  status = vfe_la_test_vector_validate(&(mod->mod_input), &(mod->mod_output));
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: vfe_la_test_vector_validate failed", __func__);
  }
