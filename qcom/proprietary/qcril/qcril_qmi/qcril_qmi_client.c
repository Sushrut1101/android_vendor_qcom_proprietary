...
#define IMS_CLIENT_CACHE_LOCK()       { pthread_mutex_lock(&qcril_ims_client_cache_lock_mutex); }
#define IMS_CLIENT_CACHE_UNLOCK()     { pthread_mutex_unlock(&qcril_ims_client_cache_lock_mutex); }
...
static pthread_mutex_t       qcril_ims_client_cache_lock_mutex;
static pthread_mutexattr_t   qcril_ims_client_cache_lock_mutex_atr;
...
static RIL_Errno qcril_qmi_init_core_ims_client_handles( void );
...
static void qcril_qmi_ssr_excluded_service_down_event
(
  qmi_client_type clnt,
  qmi_client_error_type error,
  void *error_cb_data
...
static void qcril_qmi_ssr_excluded_service_up_event
(
  qmi_client_type clnt,
  qmi_idl_service_object_type svc_obj,
  qmi_client_notify_event_type service_event,
...
//===========================================================================
void qcril_qmi_client_bootup_init(void)
{
    pthread_mutexattr_init(&qcril_ims_client_cache_lock_mutex_atr);
    pthread_mutex_init(&qcril_ims_client_cache_lock_mutex,
                       &qcril_ims_client_cache_lock_mutex_atr);
}
...
RIL_Errno qcril_qmi_client_init( void )
{
  ...
  do
  {
      ...
      //Handle IMSS and IMSA clients init seperately as
      //these services are not part of SSR handling in RIL
      qcril_qmi_init_core_ims_client_handles();
...
//===========================================================================
//qmi_ril_client_get_port_for_legacy_targets
//===========================================================================
qmi_client_qmux_instance_type qmi_ril_client_get_port_for_legacy_targets(void)
{
...
{
    int qmi_default_port;
    qmi_default_port = QMI_CLIENT_INSTANCE_ANY;

    if ( !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_8998 ) &&
         !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_8996 ) &&
         !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_8937 ) &&
         !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_8953 ) &&
         !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_SDM ) )
    {
        qmi_default_port = qmi_ril_client_get_port_for_legacy_targets();
    }
...
//===========================================================================
// qcril_qmi_init_ssr_excluded_client_handles
//===========================================================================
int qcril_qmi_init_ssr_excluded_client_handles
(
    int                           client_idx,
    qmi_client_ind_cb             client_ind_cb,
    qmi_client_recv_msg_async_cb  client_cmd_cb
)
{
  ...
  IMS_CLIENT_CACHE_LOCK();
  if(client_info.client_state[client_idx] == QCRIL_QMI_SERVICE_NOT_CONNECTED)
  {
    client_err = qmi_client_init_instance(client_info.service_objects[client_idx],
                                        QMI_CLIENT_INSTANCE_ANY,
                                        client_ind_cb,
                                        NULL,
                                        &client_info.os_params[client_idx],
                                        time_out,
                                        &client_info.qmi_svc_clients[client_idx]);

    QCRIL_LOG_INFO("qmi_client_init_instance returned status(%s) for %d",
                    qmi_errstr(client_err), client_idx);

    if (client_err != QMI_NO_ERR)
    {
      // do not return failure if QMI initialization fails
      client_info.qmi_svc_clients[ client_idx ] = NULL;

      //Register for UP event in case of client init failure
      if (client_info.notifier[client_idx])
      {
        QCRIL_LOG_INFO("Release notifier for client: %d", client_idx);
        qmi_client_release(client_info.notifier[client_idx]);
        client_info.notifier[client_idx] = NULL;
      }

      rc = qmi_client_notifier_init(
                        client_info.service_objects[client_idx],
                        &client_info.os_params[client_idx],
                        &client_info.notifier[client_idx]);
      QCRIL_LOG_INFO("qmi_client_notifier_init return %d", (int) rc);

      if (!rc)
      {
        client_info.client_state[client_idx] = QCRIL_QMI_SERVICE_NOT_CONNECTED;
        rc = qmi_client_register_notify_cb(
                         client_info.notifier[client_idx],
                         qcril_qmi_ssr_excluded_service_up_event,
                         (void*)(intptr_t)client_idx);
        QCRIL_LOG_ERROR("qmi_client_register_notify_cb %d", (int) rc);
      }
    }
    else
    {
      QCRIL_LOG_ERROR("Register for service_down event");
      client_info.client_cbs[client_idx] = client_cmd_cb;
      client_err = qmi_client_register_error_cb(
                               client_info.qmi_svc_clients[client_idx],
                               qcril_qmi_ssr_excluded_service_down_event,
                               (void *)(intptr_t)client_idx);
      client_info.client_state[client_idx] = QCRIL_QMI_SERVICE_CONNECTED;
      res = 1;
    }
  }
  IMS_CLIENT_CACHE_UNLOCK();
...
RIL_Errno qcril_qmi_init_core_client_handles( void )
{
  ...
  do
  {
        ...
        res = qcril_qmi_lte_direct_disc_init();
        ...
        {
            qcril_qmi_nas_get_device_capability(TRUE, TRUE, TRUE, FALSE);
        }
...
//qcril_qmi_init_core_ims_client_handles
//===========================================================================
RIL_Errno qcril_qmi_init_core_ims_client_handles( void )
{
  ...
  // IMSA
  if(qcril_qmi_init_ssr_excluded_client_handles(
                                QCRIL_QMI_CLIENT_IMSA,
                                qcril_qmi_imsa_unsol_ind_cb,
                                qcril_qmi_imsa_command_cb))
  {
    QCRIL_LOG_INFO("Initing IMSA client...");
    qcril_qmi_imsa_init();
  }
...
RIL_Errno qcril_qmi_client_dsds_bind_to_subscription( RIL_SubscriptionType sub_num )
{
  ...
  // nas
  res =  qcril_qmi_bind_subscription_helper( QCRIL_QMI_CLIENT_NAS,
  ...
  if ( RIL_E_SUCCESS == res )
  {
    res =  qcril_qmi_bind_subscription_helper( QCRIL_QMI_CLIENT_VOICE,
  ...
  if ( RIL_E_SUCCESS == res )
  {
    res =  qcril_qmi_bind_subscription_helper( QCRIL_QMI_CLIENT_PBM,
  ...
  if ( RIL_E_SUCCESS == res )
  {
    res =  qcril_qmi_bind_subscription_helper( QCRIL_QMI_CLIENT_WMS,
  ...
  if ( RIL_E_SUCCESS == res )
  {
    res1 =  qcril_qmi_bind_subscription_helper( QCRIL_QMI_CLIENT_DMS,
  ...
  if ( RIL_E_SUCCESS == res )
  {
    ...
    {
      lte_res =  qcril_qmi_client_send_msg_sync_ex( QCRIL_QMI_CLIENT_LTE,
...
void qmi_ril_send_modem_restart_notification(char* reason) {
    ...
    if (reason) {
        ...
        unsol_resp_params.resp_pkt = MODEM_RESTART_REASON_DEFAULT;
...
void qmi_ril_ssr_perform_final_post_ssr_init(void * param)
{
    ...
    if ( !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_8960 ) &&
         ...
         !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_8996 ) &&
         !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_8952 ) &&
         !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_8937 ) &&
         !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_8953 ) &&
         !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_8998 ) &&
         !qmi_ril_is_feature_supported( QMI_RIL_FEATURE_SDM )
        )
...
/*===========================================================================
  ...
  FUNCTION: qcril_qmi_check_if_ssr_excluded_service_is_up
...
)
{
  ...
  if(qcril_service_up_data)
  {
    int client_idx = (intptr_t)qcril_service_up_data->cb_data;
    QCRIL_LOG_INFO("Handle service_up for %d", client_idx);
    if(client_info.client_state[client_idx] == QCRIL_QMI_SERVICE_NOT_CONNECTED)
    {
      if (QMI_NO_ERR == (rc = qmi_client_get_service_instance(qcril_service_up_data->svc_obj,
                                                        QMI_CLIENT_INSTANCE_ANY,
                                                        &info)))
      {
        QCRIL_LOG_INFO("Release client notifier for %d", client_idx);
        qmi_client_release(client_info.notifier[client_idx]);
        client_info.notifier[client_idx] = NULL;
      }
      else
    ...
    switch (client_idx)
    {
      case QCRIL_QMI_CLIENT_IMS_SETTING:
        if(qcril_qmi_init_ssr_excluded_client_handles(
                                 QCRIL_QMI_CLIENT_IMS_SETTING,
                                 qcril_qmi_imss_unsol_ind_cb,
                                 qcril_qmi_imss_command_cb))
      ...
      default:
        QCRIL_LOG_INFO("Unexpected client_idx %d", client_idx);
        break;
    }
  }
...
/*===========================================================================
  ...
  FUNCTION: qcril_qmi_ssr_excluded_service_up_event

===========================================================================*/
/*!
    @brief
    processes service up event for SSR excluded service

    @return
    None.
*/
/*=========================================================================*/
void qcril_qmi_ssr_excluded_service_up_event
(
  qmi_client_type  clnt,
  qmi_idl_service_object_type   svc_obj,
  qmi_client_notify_event_type  service_event,
...
)
{
  ...
  if ( service_event == QMI_CLIENT_SERVICE_COUNT_INC ||
       client_info.client_state[(intptr_t)cb_data] == QCRIL_QMI_SERVICE_NOT_CONNECTED )
  {
    qcril_service_up_data.svc_obj = svc_obj;
    qcril_service_up_data.cb_data = cb_data;
...
/*===========================================================================
  ...
  FUNCTION: qcril_qmi_ssr_excluded_service_down_event

===========================================================================*/
/*!
    @brief
    processes SSR excluded service down event

    @return
    None.
*/
/*=========================================================================*/
void qcril_qmi_ssr_excluded_service_down_event
(
  qmi_client_type       clnt,
  qmi_client_error_type error,
  void                 *error_cb_data
...
/*===========================================================================
  ...
  FUNCTION: qcril_qmi_handle_event_ssr_excluded_service_down

===========================================================================*/
/*!
    @brief
    processes service down event for SSR excluded service
...
)
{
  ...
  if(params_ptr)
  {
    ...
    if (client_info.client_state[srv_index] == QCRIL_QMI_SERVICE_CONNECTED)
    {
      QCRIL_LOG_INFO("Client unintialized: %d", srv_index);
      client_info.client_state[srv_index] = QCRIL_QMI_SERVICE_NOT_CONNECTED;

      if(client_info.qmi_svc_clients[srv_index])
      {
        rc = qmi_client_release(client_info.qmi_svc_clients[srv_index]);
        if(rc) {
          QCRIL_LOG_INFO("Failed to release client: %d, %p", srv_index, client_info.qmi_svc_clients[srv_index]);
        }
        else {
          QCRIL_LOG_INFO("Release client: %d, %p", srv_index, client_info.qmi_svc_clients[srv_index]);
          client_info.qmi_svc_clients[srv_index] = NULL;
        }
      }
      else {
          QCRIL_LOG_INFO("Unexpected for client: %d, %p", srv_index, client_info.qmi_svc_clients[srv_index]);
      }

      if (!client_info.notifier[srv_index])
      {
        rc = qmi_client_notifier_init(client_info.service_objects[srv_index],
                                      &client_info.os_params[srv_index],
                                      &client_info.notifier[srv_index]);
        QCRIL_LOG_INFO("qmi_client_notifier_init return %d", (int) rc);
      }

      if (!rc)
      {
        rc = qmi_client_register_notify_cb(client_info.notifier[srv_index],
                                      qcril_qmi_ssr_excluded_service_up_event,
                                      (void*)(intptr_t)srv_index);
        QCRIL_LOG_ERROR("qmi_client_register_notify_cb %d", (int) rc);
      }
    }
  }
} // qcril_qmi_handle_event_ssr_excluded_service_down
...
)
{
    ...
    if(!qmi_wds_clat_query_is_supported)
    {
        qcril_data_qmi_dpm_init();
    }
