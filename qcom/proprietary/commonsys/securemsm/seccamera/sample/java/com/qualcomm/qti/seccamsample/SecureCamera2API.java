...
public class SecureCamera2API {
    ...
    protected SecureCamera2Surface secureCamera2SecondarySurface_ = null;
    ...
    private Integer format_NonLinear_ = ImageFormat.PRIVATE;
    private Integer formatLinear_ = ImageFormat.YUV_420_888;
    ...
    protected void createCameraSessionCommon() {
        boolean dualstreamsupported = false;
        try {
            List<Surface> outputSurfaces = new ArrayList<Surface>();;
            secureCamera2Surface_ = null;
            secureCamera2Surface_ = new SecureCamera2Surface(cameraId_, width_, height_, formatLinear_, numOfBuffers_);
            outputSurfaces.add(secureCamera2Surface_.getCaptureSurface());
            SecureCamera2Surface secureCamPrevSurf_t = secureCamera2Surface_;

            if ((previewSurfaceView_ != null) && (!is_2dfa)) {
                dualstreamsupported = secureCamera2Surface_.isDualStreamSupported();
                if (dualstreamsupported) {
                    secureCamera2SecondarySurface_ = null;
                    secureCamera2SecondarySurface_ = new SecureCamera2Surface(cameraId_, width_, height_, format_NonLinear_, numOfBuffers_);
                    outputSurfaces.add(secureCamera2SecondarySurface_.getCaptureSurface());
                    secureCamPrevSurf_t = secureCamera2SecondarySurface_;
                }
                ...
                secureCamPrevSurf_t.assignPreviewSurface(previewSurfaceView_.getHolder(),
            ...
            Iterator<Surface> outputSurfacesIterator = outputSurfaces.iterator();
            for(; outputSurfacesIterator.hasNext();) {
                captureBuilder_.addTarget(outputSurfacesIterator.next());
            }

            cameraDevice_.createCaptureSession(outputSurfaces,
                ...
                if (!cameraOpenCloseSemaphore_.tryAcquire(2500, TimeUnit.MILLISECONDS)) {
                    ...
                    if(!cameraStartedLatch_.await(5000, TimeUnit.MILLISECONDS)) {
                        ...
                        if(null == cameraDevice_) {
                            ...
                            SecCamServiceClient.getInstance().dispatchVendorCommand(SecCamServiceVendorClient.MSG_VENDOR_START_SECCAM_USECASE, null);
                            cameraStarted_ = true;
                            result = Boolean.TRUE;
                        }
                    }
    ...
    public Boolean stop() {
        ...
        SecCamServiceClient.getInstance().dispatchVendorCommand(SecCamServiceVendorClient.MSG_VENDOR_STOP_SECCAM_USECASE, null);
        ...
        if (null != secureCamera2SecondarySurface_) {
            Log.d(cameraActivity_.getLogTag(), "SecureCamera2API::stop - Camera #" + cameraId_ + ": Release Secure Camera Surface");
            /** If preview enabled, capture surface release happens post preview surface release
             *  If preview not enabled below release must be called */
            if (previewSurfaceView_ == null) {
                secureCamera2SecondarySurface_.release();
            }
            secureCamera2SecondarySurface_ = null;
        }
