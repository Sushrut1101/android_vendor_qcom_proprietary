/*============================================================================
  @file RelativeMotionDetector.cpp

  @brief
  RelativeMotionDetector class implementation.
...
/*============================================================================
  RelativeMotionDetector Constructor
============================================================================*/
RelativeMotionDetector::RelativeMotionDetector(int handle)
    :SAMSensor(handle)
{
    if (getAttribOK() == true) {
        svc_num = SNS_SAM_RMD_SVC_ID_V01;
        trigger_mode = SENSOR_MODE_EVENT;
        HAL_LOG_INFO("%s: handle:%d", __FUNCTION__, handle);
        setName("RMD");
        setVendor("QTI");
        setType(SENSOR_TYPE_MOTION_RELATIVE);
...
/*============================================================================
  RelativeMotionDetector Destructor
============================================================================*/
RelativeMotionDetector::~RelativeMotionDetector()
{
...
/*============================================================================
  enable
============================================================================*/
int RelativeMotionDetector::enable(int en)
{
    int err;
    ...
    if (enabled == en) {
        HAL_LOG_INFO("RMD is already enabled/disabled %d", enabled);
        return 0;
    }
...
===========================================================================*/
void RelativeMotionDetector::processResp(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    if (crsp_ptr->sns_result_t != 0 &&
        msg_hdr->msg_id != SNS_SAM_RMD_CANCEL_RESP_V01) {
    ...
    if(true != error ) {
        ...
        case SNS_SAM_RMD_ENABLE_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_RMD_ENABLE_RESP_V01", __FUNCTION__);
            instance_id = ((sns_sam_qmd_enable_resp_msg_v01 *)msg_ptr)->instance_id;
            break;
        case SNS_SAM_RMD_CANCEL_RESP_V01:
        case SNS_SAM_RMD_DISABLE_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_RMD_CANCEL/DISABLE_RESP_V01", __FUNCTION__);
            /* Reset instance ID */
            instance_id = 0xFF;
            break;
        case SNS_SAM_RMD_GET_ATTRIBUTES_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_RMD_GET_ATTRIBUTES_RESP_V01", __FUNCTION__);
...
/*===========================================================================
  FUNCTION:  processInd
===========================================================================*/
void RelativeMotionDetector::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    sns_sam_qmd_report_ind_msg_v01*  sam_rmd_rpt_ptr =
    ...
    switch( msg_hdr->msg_id )
    {
        case SNS_SAM_RMD_REPORT_IND_V01:
            HAL_LOG_DEBUG("%s: SNS_SAM_RMD_REPORT_IND_V01", __FUNCTION__);
            sensor_data.type = SENSOR_TYPE_MOTION_RELATIVE;
            sensor_data.sensor = HANDLE_MOTION_RELATIVE;
            sensor_data.data[0] = sam_rmd_rpt_ptr->state;
            timestamp = sam_rmd_rpt_ptr->timestamp;
            break;
        case SNS_SAM_RMD_ERROR_IND_V01:
            HAL_LOG_ERROR("%s: SNS_SAM_RMD_ERROR_IND_V01", __FUNCTION__);
            error = true;
            break;
        default:
    ...
    if (error == false) {
        ...
        HAL_LOG_VERBOSE("%s: RMD: motion %f SAM TS: %u HAL TS:%" PRId64 " elapsedRealtimeNano:%" PRId64 "",
