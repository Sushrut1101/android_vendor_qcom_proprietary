...
public class CaseServiceManager {
    ...
    public int runService(Context context, TestCase testCase) {
        ...
        String groupName = testCase.getGroup();
        ServiceHandler serviceHandler = getServiceHandler(groupName);

        if (serviceHandler == null) {
            startCaseService(context, testCase);
            serviceHandler = getServiceHandler(groupName);
        }
    ...
    public void stopAllService(Context context) {
        for (String groupName : mServiceHandlerMap.keySet()) {
            Class<?> clazz = Utils.getServiceClass(groupName);
            if (clazz == null) {
                LogUtils.loge("can't found class:" + groupName);
                return;
            }
            stopServiceThread(groupName);
    ...
    public void stopServiceThread(String groupName) {
        ServiceHandler handler = getServiceHandler(groupName);
        if (handler == null) {
