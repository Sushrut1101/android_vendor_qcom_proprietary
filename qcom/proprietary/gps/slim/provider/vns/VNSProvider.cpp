...
void slim::VNSProvider::FormatData(VNS_Event_Struct *pzNewEvent, int64_t time)
{
  ...
  switch (pzNewEvent->propertyType)
  {
    case VEHICLE_PROPERTY_PERF_VEHICLE_SPEED:
    {
      ...
      zData.samples_len = 1;
      zData.samples[0].speed.data = pzNewEvent->speed;
      zData.samples[0].speed.sampleTimeOffset = 0;
      routeIndication(eSLIM_SERVICE_VEHICLE_SPEED, zData);
    }
    break;
    case VEHICLE_PROPERTY_CURRENT_GEAR:
    {
      ...
      zData.samples_len   = 1;
      zData.samples[0].gear.data = pzNewEvent->gear;
      zData.samples[0].gear.sampleTimeOffset = 0;
      routeIndication(eSLIM_SERVICE_VEHICLE_GEAR, zData);
    }
    break;
    default:
    {
...
bool uEnable, slimServiceEnumT eService)
{
  ...
  SLIM_LOGD("doUpdateVehicleDataStatus Mask:0x%x, service: %d, Enable: %d",
            m_qEnabledServices, eService, uEnable);
