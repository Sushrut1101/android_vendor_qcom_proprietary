...
public class Settings extends Activity {
    final String TAG = "Presence_UI2: Settings";
    ...
    public void updateDataSetting() {
        ...
        {
            Log.d(TAG, "Inside updateDataSetting isMobile_data_enabled =  "
    ...
    Log.d(TAG, "isVt_calling_enabled:"+isVt_calling_enabled+":isMobile_data_enabled:"+isMobile_data_enabled);
        ...
        || (AppGlobalState.getOperatorMode() == AppGlobalState.ATT_MODE))
        {
           if(AppGlobalState.getOperatorMode() == AppGlobalState.ATT_MODE) {
            ...
            if (isVt_calling_enabled && !isMobile_data_enabled) {
                Log.d(TAG, "Data is disabled ... video was checked, video = "
                        + isVt_calling_enabled);
                videoSetting.setChecked(true);
                videoSetting.setEnabled(false);
            }
            else if (isVt_calling_enabled && isMobile_data_enabled) {
                Log.d(TAG, "Data is enabled ... video was checked , video = "
                        + isVt_calling_enabled);
                videoSetting.setChecked(true);
            ...
            {
                Log.d(TAG, " Data is disabled ...video was unchecked, video = "
                        + isVt_calling_enabled);
            ...
            {
                Log.d(TAG, "Data is enabled ... video was unchecked, video = "
                        + isVt_calling_enabled);
                videoSetting.setChecked(false);
                videoSetting.setEnabled(true);
            }
...
//For AT, VZW and TMO
 ...
 private void handleVtCheckBox(CheckBox vtCheckBox) {
     ...
     vtCheckBox.setOnClickListener(new OnClickListener() {
         ...
         public void onClick(View v) {
             ...
             {
                 isVtCheckBoxStateChange = false;
                 Log.d(TAG, "Setting : handleVtCheckBox : "+isVtCheckBoxStateChange);
             }
             else
             {
 ...
 private void handleFtCheckBox(CheckBox ftCheckBox) {
  ...
  private ImsConfigListener.Stub imsConfigListener = new ImsConfigListener.Stub() {
      ...
      Log.d(TAG, "Settings| inside getfeatureresp imsconfiglistener " + String.valueOf(feature) );
    }
      ...
      boolean isVTBoxChecked = videoCheckBox.isChecked();
            Log.d(TAG, "if setting vt call config response | status " + String.valueOf(status) );
      if(status == ImsConfig.OperationStatusConstants.SUCCESS){
                                isVt_calling_enabled = isVTBoxChecked;
                Log.d(TAG, "if setting vt call config response value" + String.valueOf(value));
                                Log.d(TAG, "if setting vt call config success | isVt_calling_enabled" + String.valueOf(isVt_calling_enabled));
                  }
                  else{
                                videoCheckBox.setChecked((!isVTBoxChecked));
                                Log.d(TAG, "if QrcsImsSettings_SetQipcallConfig failed | phoneId 0");
    ...
    private void handleVtImFtCheckBox()
    {
        ...
        Log.d(TAG, "handleVtImFtCheckBox()");
        Log.d(TAG, "isFtCheckBoxStateChange:"+isFtCheckBoxStateChange);
        Log.d(TAG, "isChatCheckBoxStateChange:"+isChatCheckBoxStateChange);
        Log.d(TAG, "isVtCheckBoxStateChange:"+isVtCheckBoxStateChange);
        Log.d(TAG, "isPresenceCheckBoxStateChange:"+isPresenceCheckBoxStateChange);
        ...
        if(isFtCheckBoxStateChange || isChatCheckBoxStateChange)
        {
            ...
            if(isPresenceCheckBoxStateChange)
            {
    Log.d(TAG, "presenceCheckBox.isChecked():"+presenceCheckBox.isChecked());

                if (presenceCheckBox.isChecked())
                {
                    Log.d(TAG, "Presence is Checked");
                    editor.putBoolean("PRESENCE_KEY", true);
                } else {
                    Log.d(TAG, "Presence is UnChecked");
            ...
            Log.d(TAG, "Trigerring publish for FT or IM change ");
        ...
        if(isVtCheckBoxStateChange)
        {
            ...
            boolean isVTBoxChecked = videoCheckBox.isChecked();
            Log.d(TAG, "Before setting vt calling prop | isVTBoxChecked " + String.valueOf(isVTBoxChecked));
                try
                {
                    Log.d(TAG, "Before setting vt calling prop");
                    ImsManager imsManager = AppGlobalState.getImsManager();
            ...
            // calling on background thread
              ...
              try{
                ...
                imsManager.setVtSetting(isVTBoxChecked);
                } catch(Exception e){
                        e.printStackTrace();
          ...
          if(isVtCheckBoxStateChange && (AppGlobalState.getOperatorMode() == AppGlobalState.ATT_MODE)) {
           ...
           Log.d(TAG, "Update VT Prefs val :isVt_calling_enabled:"+isVt_calling_enabled);
     ...
     if(isVtCheckBoxStateChange)
       isVt_calling_enabled = videoCheckBox.isChecked();
        ...
        if ((Utility.getNotifyFmt(mContext) == AppGlobalState.NOTIFY_FMT_STRUCT))
                {
                    Log.d(TAG, "Before Call PublishTask from Settings ");
                    sendLogMesg(mContext, "Publish fmt=STRUCT based.");
                    mPublishTask = new PublishTask().execute(false);
                    Log.d(TAG, "After  Call PublishTask from Settings ");
                }

        }
        else
        {
            Log.d(TAG, "No Capability change for Video, im and ft ");
        ...
        if((isVtCheckBoxStateChange || isFtCheckBoxStateChange || isChatCheckBoxStateChange || isPresenceCheckBoxStateChange))  //CD by OPTION & Setting changes
        {
            try
            {
                Log.d(TAG, "Before QCDService_GetMyCdInfo");
        ...
        if(AppGlobalState.getCdService() != null){
                ...
                Log.d(TAG, "After QCDService_SetMyCdInfo");
            }   catch (Exception e) {
                   e.printStackTrace();
            }
        }
    ...
    try{
      ...
      Log.d(TAG, "before SetPresenceConfig ");
      final boolean presenceBoxChecked = presenceCheckBox.isChecked();
        new Thread(new Runnable() {
              ...
              if(presenceBoxChecked)
                    {
                        ...
                        if(res1 == 0)
                        {
                            ...
                            Log.d(TAG, "After SetPresenceConfig | volteuseroptinstatus" + " " + String.valueOf(volteUserOptInStatus));
                        }
                    }
                    else
                    {
          ...
          Log.d(TAG, "After SetPresenceConfig | PhoneId 0" + " " + String.valueOf(volteUserOptInStatus));
                }catch(Exception e){
                            e.printStackTrace();
                }
            }
    ...
    private void handleOkButtonClick(Button cancelButton) {
        cancelButton.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                Log.d(TAG,"On OK button clicked");
    ...
    public void updatePresencePrefs(String key, boolean value) {
        ...
        Log.d(TAG, "Update prefs: key:" + key+":value:"+value);
    ...
    public void updateSettingValues() {
        ...
        {
            Log.d(TAG, "isAvailability_cache_expiration_valid View  "
        ...
        if (!isCapabilites_cache_expiration_valid) {
            Log.d(TAG, "isCapabilites_cache_expiration_valid ");
        ...
        if (!isCapability_poll_interval_valid) {
            Log.d(TAG, "isCapability_poll_interval_valid ");
        ...
        if (!isCapability_poll_list_subscription_expiry_timer_valid) {
            Log.d(TAG, "isCapability_poll_list_subscription_expiry_timer_valid ");
        ...
        if (!isMinimum_publish_interval_valid) {
            Log.d(TAG, "isMinimum_publish_interval_valid ");
        ...
        if (!isMax_subcription_list_entries_valid) {
            Log.d(TAG, "isMax_subcription_list_entries_valid ");
    ...
    private void populateInitialValues() {
        ...
        Log.d(TAG, "ftSupported =" + ftSupported + "chatSupported =" + chatSupported);
