...
/**QIImageType: Image Type
*  QI_JPEG - Jpeg Image
*  QI_MPO  - MPO Image
*  */
typedef enum QIImageType{
  QI_JPEG,
  QI_MPO
}QIImageType;
...
/** QIARRAY_SIZE - returns the array size
*    @a - array
**/
#define QIARRAY_SIZE(a) (sizeof(a)/sizeof(a[0]))
...
/** QI_ALIGN - Align up value x to a aligment.
*    @x - value
*    @a - alignment
**/
#define QI_ALIGN(x, a)  (((x) + ((a) - 1)) & ~((a) - 1))
...
/** QI_BROADCAST - abstraction for condition broadcast.
*    @c - pointer to condition variable
**/
#define QI_BROADCAST(c) pthread_cond_broadcast(c)
...
/** QI_UNUSED - Supress build warning for unused parameter
*    @x: parameter to be supressed
**/
#define QI_UNUSED(x) (void)(x)
...
/** SetDynamicLogging()
*   @aLogLevel: Log level "1" to "4"
*
*   Set dynamic logging level
**/
inline void SetDynamicLogging(const char* aLogLevel)
{
#ifdef _ANDROID_
  char prop[PROPERTY_VALUE_MAX];
  property_get("persist.vendor.camera.mmstill.logs", prop, aLogLevel);
  g_mmstillloglevel = (uint32_t)atoi(prop);
  QIDBG_HIGH("###mmstill_Loglevel %d", g_mmstillloglevel);
#endif
}
