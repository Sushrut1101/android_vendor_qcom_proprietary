...
int OptsData::Init() {
    ...
    /*Init paths for new Opcode for llccbw and cpubw*/
    init_llcbw_hwmon_path_newOpcode();
    init_cpubw_hwmon_path_newOpcode();
...
void OptsData::init_node_paths() {
    ...
    for (idx = 0; idx < MAX_MINOR_RESOURCES; idx++) {
        ...
        if (strncasecmp("SPECIAL_NODE", sysfsnode_path[idx], strlen("SPECIAL_NODE")) == 0) {
            ...
            assigned as special_node and their ApplyOpts and ResetOpts needs to be defined*/
        ...
        case MEMLAT_START_INDEX + LLCC_DDR_LAT_MIN_FREQ_OPCODE:
        case MEMLAT_START_INDEX + LLCC_MEMLAT_MIN_FREQ_OPCODE:
        node_type[idx] = MEM_LAT_NODE;
        break;

        case SCHED_START_INDEX + SCHED_UPMIGRATE_OPCODE:
        case SCHED_START_INDEX + SCHED_DOWNMIGRATE_OPCODE:
...
int OptsData::init_cpubw_hwmon_path() {
    ...
    char folder_path[NODE_MAX] = "";
    char *parent = NULL;
    const char *delim = ":";
    char* pFolder_path = NULL;

    get_nodes_folder_path(sysfsnode_path[CPUBW_HWMON_START_INDEX + CPUBW_HWMON_MINFREQ_OPCODE], folder_path);

    if (strlen(folder_path) > 0) {
        parent = strtok_r(folder_path, delim, &pFolder_path);
        parent = strtok_r(NULL, delim, &pFolder_path);
    }

    ret = init_devfreq_child_path(parent, d_name);
...
int OptsData::init_llcbw_hwmon_path() {
    ...
    get_nodes_folder_path(sysfsnode_path[LLCBW_HWMON_START_INDEX + LLCBW_HWMON_MINFREQ_OPCODE], folder_path);
    ...
    if (ret == SUCCESS) {
        ...
        for (int i = 0; i < llcbw_avail_freqs_n; i++) {
...
/*New init function supporting LLCC_DDR_BW_MIN_FREQ*/
 int OptsData::init_llcbw_hwmon_path_newOpcode() {
     char d_name[NODE_MAX] = "";
     int ret = FAILED;
     char folder_path[NODE_MAX] = "";
     char *parent = NULL;
     ...
     get_nodes_folder_path(sysfsnode_path[LLCBW_HWMON_START_INDEX + LLCC_DDR_BW_MIN_FREQ], folder_path);
...
/*New Init function supporting CPU_LLCC_BW_MIN_FREQ*/
 int OptsData::init_cpubw_hwmon_path_newOpcode() {
     int ret = FAILED;
     char d_name[NODE_MAX] = "";
     char folder_path[NODE_MAX] = "";
     char *parent = NULL;
     ...
     get_nodes_folder_path(sysfsnode_path[CPUBW_HWMON_START_INDEX + CPU_LLCC_BW_MIN_FREQ], folder_path);
     if (strlen(folder_path) > 0) {
         parent = strtok_r(folder_path, delim, &pFolder_path);
         parent = strtok_r(NULL, delim, &pFolder_path);
     }
...
int OptsData::check_core_ctl_presence() {
    ...
    snprintf(core_ctl_min_cpu_node, NODE_MAX, CORE_CTL_MIN_CPU, 0);
    FREAD_STR(core_ctl_min_cpu_node, core_ctl_min_s[0], NODE_MAX, rc);
    if (rc > 0) {
        core_ctl_present = 1;
    } else {
        core_ctl_present = 0;
...
int OptsData::core_ctl_init() {
    ...
    if(core_ctl_present > 0) {
       for (int i =0; i < MAX_CLUSTER; i++) {
           snprintf(core_ctl_min_cpu_node, NODE_MAX, CORE_CTL_MIN_CPU, t.getFirstCoreIndex(i));
           FREAD_STR(core_ctl_min_cpu_node,  val, NODE_MAX, rc);
           if (rc > 0)
                min_cores[i] = atoi(val);
           snprintf(core_ctl_max_cpu_node, NODE_MAX, CORE_CTL_MAX_CPU, t.getFirstCoreIndex(i));
           FREAD_STR(core_ctl_max_cpu_node, val, NODE_MAX, rc);
           if (rc > 0)
                max_cores[i] = atoi(val);
       }
    }
    return rc;
}
...
unsigned int OptsData::find_next_llccbw_available_freq(unsigned int freq) {
     int setval = -1, i = 0;

     for (i = 0; i < llcbw_avail_freqs_n; i++) {
         if (llcbw_avail_freqs[i] >= freq) {
             setval = llcbw_avail_freqs[i];
             break;
         }
     }
     if (i == llcbw_avail_freqs_n) {
         setval = llcbw_avail_freqs[i - 1];
     }
     return setval;
 }
