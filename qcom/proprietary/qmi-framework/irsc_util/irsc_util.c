...
/**
 * is_comment_or_empty() - Check if a string is a comment or blank
 * @input: The input string to be checked.
 *
 * @Return: True if the string is a comment or blank, otherwise False.
 */
int is_comment_or_empty(char *input)
{
...
static  enum irsc_tool_err irsc_parser(void *irsc_obj)
{
   ...
   if(irsc_d->file_name_len > FILE_NAME - 1) {
      IRSC_ERR("File name too long %zu !< %d\n",
...
int main(int argc, char *argv[])
{
   ...
   if(argc <= 1) {
      IRSC_ERR("Usage: IRSC Tool Mandatory File Name required\n");
      return -1;
   }

   d = create_irsc_tool(argv[1],':');
