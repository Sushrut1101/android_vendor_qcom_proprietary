...
OMXImageEncoder::OMXImageEncoder()
{
  ...
  m_currentInTmbBuffHdr = NULL;
  m_encoding_mode = OMX_Serial_Encoding;
  m_thumbEncodingComplete = OMX_FALSE;
  m_MetadataEncKey.metaKey = (OMX_U8 *) DEFAULT_MN_ENC_KEY;
  m_MetadataEncKey.keyLen = (uint32_t)strlen(DEFAULT_MN_ENC_KEY);
  memset(&m_imageBufferOffset, 0x00, sizeof(m_imageBufferOffset));
  memset(&m_thumbnailInfo, 0x00, sizeof(m_thumbnailInfo));
...
{
  a_outPort->nSize = (uint32_t)sizeof(OMX_PARAM_PORTDEFINITIONTYPE);
...
{
  a_inPort->nSize = (uint32_t)sizeof(OMX_PARAM_PORTDEFINITIONTYPE);
...
==============================================================================*/
OMX_ERRORTYPE OMXImageEncoder::omx_component_init(
  OMX_IN OMX_HANDLETYPE hComp __unused)
{
  ...
  m_inputFormatTypes->nSize = (uint32_t)sizeof(OMX_IMAGE_PARAM_PORTFORMATTYPE);
  ...
  m_inputTmbFormatTypes->nSize =
    (uint32_t)sizeof(OMX_IMAGE_PARAM_PORTFORMATTYPE);
  ...
  m_messageThread.StartThread(this);
  ...
  //Qtable init
  for (int i = 0; i < QIQuantTable::QTABLE_MAX; i++) {
    m_quantTable[i].eQuantizationTable = OMX_IMAGE_QuantizationTableMax;
  }
  ...
  OMX_INOUT OMX_PTR paramData)
{
  ...
  case OMX_IndexParamQuantizationTable: {
    ...
    if (lquantTable->eQuantizationTable == OMX_IMAGE_QuantizationTableLuma) {
        memcpy(lquantTable, &m_quantTable[0],
          (sizeof(OMX_IMAGE_PARAM_QUANTIZATIONTABLETYPE)));
      }
    else if(lquantTable->eQuantizationTable ==
      OMX_IMAGE_QuantizationTableChroma) {
        memcpy(lquantTable, &m_quantTable[1],
          (sizeof(OMX_IMAGE_PARAM_QUANTIZATIONTABLETYPE)));
      }
    else {
      QIDBG_ERROR("%s: %d]: Error bad qtable params ", __func__,__LINE__);
  ...
  case QOMX_IMAGE_EXT_MULTI_IMAGE: {
    QOMX_JPEG_MULTI_IMAGE_INFO *lmulti_image =
      reinterpret_cast<QOMX_JPEG_MULTI_IMAGE_INFO *> (paramData);
...
int OMXImageEncoder::setExifData(QOMX_EXIF_INFO *a_exifInfo)
{
  ...
  if (!mExifObjInitialized) {
    // set 3a version info during exif info obj initialization
    memcpy(m_sw3aVersion, a_exifInfo->debug_data.sw_3a_version, sizeof(a_exifInfo->debug_data));
  ...
  OMX_IN OMX_PTR paramData)
{
  ...
  case OMX_IndexParamQuantizationTable: {
    OMX_IMAGE_PARAM_QUANTIZATIONTABLETYPE* lquantTable =
      reinterpret_cast <OMX_IMAGE_PARAM_QUANTIZATIONTABLETYPE *> (paramData);
    if (lquantTable->eQuantizationTable == OMX_IMAGE_QuantizationTableLuma) {
        memcpy(&m_quantTable[0], lquantTable,
          (sizeof(OMX_IMAGE_PARAM_QUANTIZATIONTABLETYPE)));
      }
    else if(lquantTable->eQuantizationTable ==
      OMX_IMAGE_QuantizationTableChroma) {
        memcpy(&m_quantTable[1], lquantTable,
          (sizeof(OMX_IMAGE_PARAM_QUANTIZATIONTABLETYPE)));
      }
    else {
      QIDBG_ERROR("%s: %d]: Error bad qtable params ", __func__,__LINE__);
  ...
  OMX_IN OMX_PTR configData)
{
  ...
  case OMX_IndexParamQuantizationTable: {
    QIDBG_MED("%s %d] set OMX_IndexParamQuantizationTable",
      __func__, __LINE__);
    OMX_IMAGE_PARAM_QUANTIZATIONTABLETYPE* lquantTable =
      reinterpret_cast <OMX_IMAGE_PARAM_QUANTIZATIONTABLETYPE *> (configData);
    if (lquantTable->eQuantizationTable == OMX_IMAGE_QuantizationTableLuma) {
...
OMX_PARAM_PORTDEFINITIONTYPE *OMXImageEncoder::getPortDef(OMX_U32 eIdx)
{
  ...
  switch (ePort) {
    ...
    case OMX_INPUT_THUMBNAIL_PORT_INDEX:
      ...
      break;
    default:
      QIDBG_ERROR("%s: Unknown port index %u", __func__, (uint32_t)eIdx);
      return NULL;
  }
}
  ...
  OMX_IN OMX_U32 aport)
{
  ...
  (*lbufferData)[i].mHeader.nSize = (uint32_t)sizeof(OMX_BUFFERHEADERTYPE);
  ...
  OMX_U32 aport)
{
  ...
  mOutOMXBufferData[i].mHeader.nSize = (uint32_t)sizeof(OMX_BUFFERHEADERTYPE);
  ...
  mOutOMXBufferData[i].mHeader.pAppPrivate =
    &(mOutOMXBufferData[i].mInfo);
  mOutOMXBufferData[i].mHeader.pPlatformPrivate = &mOutOMXBufferData[i].mInfo;
  mOutOMXBufferData[i].mHeader.pOutputPortPrivate = &mOutOMXBufferData[i].mInfo;
  ...
  OMX_IN OMX_U8* buffer)
{
  ...
  if (bufferHdr == NULL || buffer == NULL || bytes == 0 ) {
    QIDBG_ERROR("%s %d]: bad param 0x%p %u 0x%p", __func__, __LINE__,
...
void OMXImageEncoder::portEnable(OMX_U32 a_portIndex)
{
  ...
  if ((a_portIndex == OMX_ALL) || (a_portIndex == m_inPort->nPortIndex)) {
    ...
    QIDBG_ERROR("%s: ERROR Invalid Port Index", __func__);
    m_callbacks->EventHandler(m_compHandle, m_appData, OMX_EventError,
...
OMX_ERRORTYPE OMXImageEncoder::flushBufferQueues(OMX_U32 a_portIndex)
{
  ...
  if ((a_portIndex == OMX_ALL) || (a_portIndex == m_inPort->nPortIndex)) {
    for (OMX_U32 i = 0; i < (OMX_U32)m_etbQueue.Count(); i++) {
      ...
      (a_portIndex == m_inTmbPort->nPortIndex)) {
    for (OMX_U32 i = 0; i < (OMX_U32)m_etbTmbQueue.Count(); i++) {
    ...
    (a_portIndex == m_outPort->nPortIndex)) {
    for (OMX_U32 i = 0; i < (OMX_U32)m_ftbQueue.Count(); i++) {
  ...
  OMX_BUFFERHEADERTYPE *aBuffer, QOMX_Buffer_Data_t *pBufferData)
{
  ...
  for (i = 0; i < (OMX_U32)aPort->nBufferCountActual; i++) {
  ...
  OMX_IN OMX_BUFFERHEADERTYPE *buffer)
{
  ...
  if (m_state == OMX_StateIdle) {
    if (m_compTransState != OMX_StateLoaded_Pending) {
      QIDBG_MED("%s:%d: Component needs to be in transition to Loaded state "
  ...
  if (port == m_inPort->nPortIndex) {
    ...
    if (CanFreeBuffers(m_outPort, buffer, mOutOMXBufferData)) {
      ...
      if (m_outBuffAllocCount > 0) {
        ...
        m_outPort->bPopulated = OMX_FALSE;
      }
    }
  } else {
  ...
  if ((m_inBuffAllocCount == 0) && (m_outBuffAllocCount == 0) &&
    ...
    if (lMessage == NULL) {
      ...
      lMessage->m_qMessage = OMX_MESSAGE_CHANGE_STATE_DONE;
      lret = postMessage(lMessage);
      QIDBG_MED("%s:%d: Free Buffer complete: State change from"
      ...
      QI_LOCK(&m_abortlock);
      QIDBG_MED("%s:%d] Reset abort flag to false", __func__, __LINE__);
      m_abort_flag = OMX_FALSE;
...
==============================================================================*/
OMX_ERRORTYPE OMXImageEncoder::omx_component_role_enum(
  OMX_IN OMX_HANDLETYPE hComp __unused,
  OMX_OUT OMX_U8 *role __unused,
  OMX_IN OMX_U32 index __unused)
{
  return OMX_ErrorNotImplemented;
}
...
/*==============================================================================
* Function : setNumExifMarkers
* Parameters: None
* Return Value : Number of exif markers
* Description: Calculates number of EXIF markers based on if
* mobicat/3adebug/mpo is enabled
==============================================================================*/
void OMXImageEncoder::setNumExifMarkers()
{
  mNumExifMarkers = NUM_EXIF_MARKERS;
  if (m_Metadata.mobicat_mask & MOBICAT_MASK) {
    mNumExifMarkers += NUM_EXIF_MARKERS_MOBICAT;
  }

  if (m_Metadata.mobicat_mask & STATS_DEBUG_MASK) {
    mNumExifMarkers += NUM_EXIF_MARKERS_STATS_DEBUG;
  }

  if (m_MultiJpegSequence.image_type == QOMX_JPEG_IMAGE_TYPE_MPO) {
    mNumExifMarkers += NUM_EXIF_MARKERS_MPO;
  }

  QIDBG_MED("%s %d]: numExifMarkers = %d", __func__, __LINE__, mNumExifMarkers);
}
