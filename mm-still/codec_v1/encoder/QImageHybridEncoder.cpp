 ...
 *
 * Description: Static function to register the buffer
 *
 * Input parameters:
 *   buf - base address of buffer
 *   size - size of buffer
...
static void register_buf(void* buf, int size, int fd) {
  ...
  if(remote_register_buf) {
    QIDBG_HIGH("%s:%d] remote_register_buf buf_addr 0x%x size %d fd %d",
...
bool QImageHybridEncoder::IsAvailable(QIEncodeParams& aEncodeParams)
{
  ...
  if (lScaleEnabled&& !mUseFastCv) {
    ...
    QIDBG_HIGH("%s:%d] scale input %dx%d crop (%d, %d) output %dx%d rotation %d",
      __func__, __LINE__,
      mHybridConfig.main_scale_cfg.input_width,
      mHybridConfig.main_scale_cfg.input_height ,
      mHybridConfig.main_scale_cfg.h_offset,
...
#endif
  QIDBG_LOW("%s:%d] mUseFastCv %d mUseMultiFastCV %d", __func__, __LINE__,
    mUseFastCv,mUseMultiFastCV);
  mRszOutImage = NULL;
  mRszOutBuffer = NULL;
  mfastCVbuffer = NULL;
#ifdef USE_PERFORMANCE_LOCK
 ...
 * Notes: none
 *==========================================================================*/
int QImageHybridEncoder::Start(QIThread *a_pUseEncThread)
{
...
int QImageHybridEncoder::setEncodeParams(QIEncodeParams &aParams)
{
  ...
  QIDBG_HIGH("%s:%d] Rotation %d Quality %d", __func__, __LINE__,
...
void QImageHybridEncoder::ConfigureDimensions()
{
  ...
  mHybridConfig.preference = JPEG_ENCODER_PREF_SOFTWARE_ONLY;
  ...
  if (lCropEnabled || lScaleEnabled) {
    QIDBG_HIGH("%s:%d] Crop or Scaling enabled so disabling multi threaded FastCV", __func__, __LINE__);
    mUseMultiFastCV = false;
  }
...
int QImageHybridEncoder::addInputImage(QImage &aImage)
{
  ...
  if (mInputImage->SubSampling() != QI_H2V2) {
    QIDBG_ERROR("%s:%d] Invalid format \n", __func__, __LINE__ );
    QI_UNLOCK(&(mMutex));
  ...
  //size calculated assuming h2v2 subsampling
  buffer_size = mHybridConfig.main_scale_cfg.output_width *
    mHybridConfig.main_scale_cfg.output_height * 1.5;
  if (mUseMultiFastCV) {
    mfastCVbuffer = (uint8_t *)QIHeapBuffer::New(buffer_size);
  ...
  QIDBG_HIGH("%s:%d] Y addr %p len %d fd %d", __func__, __LINE__,
  ...
  QIDBG_HIGH("%s:%d] CbCr addr %p len %d fd %d", __func__, __LINE__,
    lPlane->ActualAddr(), lPlane->Length(), lPlane->Fd());
  QI_ERROR_RET_UNLOCK(jpeg_buffer_use_external_buffer(
    mSource.p_fragments[0].color.yuv.chroma_buf,
    lPlane->ActualAddr(),
 ...
 * Notes: none
 *==========================================================================*/
void QIHybridOutputThread::run(QThreadObject *aData __unused)
{
}
 ...
 * Notes: none
 *==========================================================================*/
void QIHybridEncoderThread::run(QThreadObject *aData __unused)
{
  mEncoder.Encode();
}
...
int QImageHybridEncoder::Resize()
{
  QIDBG_ERROR("%s:%d] FastCV Disabled !!!", __func__, __LINE__);
  return QI_ERR_GENERAL;
}
#endif
...
void QImageHybridEncoder::Encode()
{
  ...
  if (mHybridConfig.main_scale_cfg.enable) {
    ...
    QIDBG_MED("%s:%d] width %d height %d ", __func__, __LINE__,
      mHybridConfig.main.width, mHybridConfig.main.height);
  }
  ...
  if (mUseMultiFastCV && mHybridConfig.main_scale_cfg.enable) {
    ...
    if (mfastCVbuffer) {
      ...
      if (QI_ERROR(lrc)) {
        QIDBG_ERROR("%s:%d] Failed to start hybrid_jpege_start\n", __func__, __LINE__);
        for (uint32_t k = 0; k < mObserverCnt; k++) {
          mObserver[k]->EncodeError(QImageEncoderObserver::ERROR_GENERAL);
        }
        QI_UNLOCK(&mMutex);
      ...
      QIDBG_ERROR("%s:%d] Allocation failed: falling back to no FastCV\n",
        __func__, __LINE__);
      p_args->fastCV_flag = 0;
    }
  }
  ...
  jpege_cfg_t *pConfig, uint8_t *fastCV_buffer)
{
  ...
  QIDBG_LOW("%s:%d] Entered... %p\n", __func__, __LINE__, fastCV_buffer);
  // Set default configuration
...
void encoder_event_handler(void *p_user_data,
  jpeg_event_t event, void *p_arg __unused)
{
