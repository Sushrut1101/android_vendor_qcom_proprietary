...
)
{
  ...
  if (qmi_ind.bearer_id_valid)
  {
    flow_info->bearer_id = qmi_ind.bearer_id;
    flow_info->param_mask |= NETMGR_QOS_EVENT_REPORT_FLOW_INFO_BEARER_ID_PARAM;
  }
  ...
  if(qmi_ind.tx_qos_filters_len > 0 && qmi_ind.tx_qos_filters_valid)
  {
    ...
    for(index=0;index < qmi_ind.tx_qos_filters_len ;index++)
    {
      ...
      match_all = FALSE;
      if (qmi_ind.tx_filter_match_all_valid)
      {
        for (i = 0; i < qmi_ind.tx_filter_match_all_len; i++)
        {
          if (qmi_ind.tx_filter_match_all[i].filter_id == qmi_ind.tx_qos_filters[index].filter_id)
          {
            match_all = TRUE;
            netmgr_log_low("filter %u is a match all filter\n", index);
            break;
          }
        }
      }
      ...
      if (match_all)
      {
        filter_data->ip_version = qmi_ind.tx_qos_filters[index].fltr_info.ip_info.ip_version;
        filter_data->filter_desc.precedence = qmi_ind.tx_qos_filters[index].filter_precedence;
      }
      else
      {
...
)
{
  ...
  netmgr_qos_ip_version_type ip_ver = NETMGR_QOS_IP_VERSION_4;
  int i;
  ...
  if (user_handle == netmgr_qmi_cfg.links[link].qos_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV6])
  {
...
/#endif*/
  ...
  switch (msg_id)
  {
    ...
    case QMI_QOS_GLOBAL_QOS_FLOW_IND_V01:
    {
      ...
      {
        netmgr_log_err("failed qmi_client_message_decode error=%d\n", qmi_err);
        goto bail;
      }
      else
      {
        ...
        netmgr_qmi_qos_process_event_report_ind(netmgr_ind_data,qmi_ind,ip_ver);
      }
    }
    break;
  ...
  cmd->data.info.qmi_msg.data.qos_ind.info.ip_ver = ip_ver;
  ...
  *clnt_hndl = NETMGR_QMI_CLIENT_INVALID;
  ...
  else
  {
    ...
    if (netmgr_main_get_qos_via_idl_enabled())
    {
      ...
      if (netmgr_main_cfg.ibfc || netmgr_main_cfg.kfc_mode)
      {
        /* Kernel is doing FC so no need to get msg on userspace */
        request.suppress_report_flow_control_valid = TRUE;
        request.suppress_report_flow_control = TRUE;
      }
      ...
      if (rc != QMI_NO_ERR || QMI_RESULT_SUCCESS_V01 != response.resp.result)
      {
        netmgr_log_err("%s(): failed to set ip_family=%d with rc=%d result=%d err=%d!\n",
                     __func__, ip_family, rc, response.resp.result, response.resp.error);
        *err = resp.resp;
        goto bail;
      }
    }
...
/*===========================================================================
  FUNCTION  netmgr_qmi_qos_cache_flow_info
===========================================================================*/
/*!
@brief
  Cache the QMI QOS flow info.
...
*/
/*=========================================================================*/
static int netmgr_qmi_qos_cache_flow_info
(
  int                          link,
  netmgr_qmi_qos_flow_info_t * flow_info
)
{
  netmgr_qmi_qos_flow_info_t * cache_flow_info = NULL;
  int i;
  ...
  {
    if (netmgr_qmi_cfg.links[link].qos_info.qos_flows[i] == NULL)
    {
      cache_flow_info = (netmgr_qmi_qos_flow_info_t *)
      ...
      memcpy(cache_flow_info, flow_info, sizeof(netmgr_qmi_qos_flow_info_t));
      netmgr_qmi_cfg.links[link].qos_info.qos_flows[i] = cache_flow_info;

      return NETMGR_SUCCESS;
    }
  }

  netmgr_log_err("Cache limit reached, flow %d will not be activated\n",
                    flow_info->flow_id);
...
/*===========================================================================
  FUNCTION  netmgr_qmi_qos_cache_default_flow
===========================================================================*/
/*!
@brief
  Cache default flow info.
...
*/
/*=========================================================================*/
int netmgr_qmi_qos_cache_default_flow
(
  int                         link,
  uint8                       bearer_id,
  netmgr_qos_ip_version_type  ip_ver
)
{
  ...
  qos_flow.bearer_id = bearer_id;
  qos_flow.ip_ver = ip_ver;
  qos_flow.is_new = TRUE;
  qos_flow.priority = NETMGR_TC_DEFAULT_PRIORITY;
  qos_flow.datarate = NETMGR_TC_DEFAULT_DATARATE;

  if (netmgr_qmi_qos_get_flow_info(link, qos_flow.flow_id,
             &qos_flow.priority, &qos_flow.datarate) != NETMGR_SUCCESS)
  {
    netmgr_log_err("failed to query default qos flow info\n");
    qos_flow.priority = NETMGR_TC_DEFAULT_PRIORITY;
    qos_flow.datarate = NETMGR_TC_DEFAULT_DATARATE;
  }

  return netmgr_qmi_qos_cache_flow_info(link, &qos_flow);
}
