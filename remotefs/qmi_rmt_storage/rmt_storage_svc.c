...
#define LOG_TAG "vendor.rmt_storage"
...
/* Global counter to track all write/sync requests issued by client */
...
static void * rmt_storage_client_thread(void *data)
{
	...
	while(1) {
		...
		for(i=0; i<client->xfer_cnt; i++) {
			...
			KMSGI("Calling %s [offset=%lu, size=%u, req_h 0x%x, wr_count %" PRIu64 "] for %s!",
			      (client->xfer_dir == RMTFS_DIRECTION_WRITE_V01) ?
			      "Write" : "Read",
			      xfer->sector_addr * (off_t)SECTOR_SIZE,
			      xfer->num_sector * SECTOR_SIZE, (uint32_t)req_handle,
			      write_counter, client->path);
			...
			if (ret >= 0 && (unsigned int)ret == xfer->num_sector * SECTOR_SIZE) {
				KMSGI("Done %s (bytes = %zd, req_h 0x%x, wr_count %" PRIu64 ") for %s!",
				      (client->xfer_dir == RMTFS_DIRECTION_WRITE_V01) ?
				      "Write" : "Read",  ret, (uint32_t)req_handle, write_counter,
				      client->path);
			} else {
				KMSGI("%s failed (ret = %zd) [err= %d (%s) req_h 0x%x wr_count %" PRIu64 "] for %s!",
				      (client->xfer_dir == RMTFS_DIRECTION_WRITE_V01) ?
				      "Write" : "Read",  ret, errno,
				      strerror(errno), (uint32_t)req_handle,
				      write_counter, client->path);
			}
			...
			RMTS_LOG_REQ("Bytes %s = %d, wr_count %" PRIu64,
				      (client->xfer_dir == RMTFS_DIRECTION_WRITE_V01) ?
				      "written" : "read", (int) ret, write_counter);
		}
		...
		if (client->xfer_cnt) {
			...
			RMTS_LOG_REQ("Send response: res=%d err=%d, wr_count %" PRIu64,
				     rw_resp.resp.result, rw_resp.resp.error, write_counter);
			resp_err = qmi_csi_send_resp(client->req_handle, client->msg_id, &rw_resp, sizeof(rw_resp));
			if(resp_err != QMI_CSI_NO_ERR)
				RMTS_LOG_REQ("qmi_csi_send_resp returned error %d, wr_count %" PRIu64,
					     resp_err, write_counter);
			client->xfer_cnt = 0;
				...
				void *service_cookie)
{
	...
	for (i = 0;  i < sizeof(part_lookup_table)/sizeof(struct partition_lookup_entry);  i++) {
		if (!strncmp(part_lookup_table[i].path, data->path, MAX_PATH_NAME)) {
			...
			KMSGI("Processing: Open Request (req_h 0x%x) for %s!",
					(uint32_t)req_handle, data->path);
			break;
		}
	}
			...
			void *service_cookie)
{
	...
	RMTS_LOG_REQ("R/W request received (wr_count %" PRIu64 ")", write_counter);
	KMSGI("%s iovec request (req_h 0x%x wr_count %" PRIu64 ") received for %s",
	      (data->direction == RMTFS_DIRECTION_WRITE_V01) ?
	      "Write" : "Read", (uint32_t)req_handle, write_counter,
	      client->path);

	pthread_mutex_lock(&client->th_mutex);

	client->xfer_dir = data->direction;
	if (data->iovec_struct_len > RMT_STORAGE_MAX_IOVEC_XFR_CNT) {
		KMSGI("Received max io_vec requests.");
		LOGI("Received max io_vec requests.");
		data->iovec_struct_len = RMT_STORAGE_MAX_IOVEC_XFR_CNT;
	}
 ...
 */
static int check_support_using_libmdm()
{
    int i, ret = 0;
	...
	if (!strncmp(devinfo.mdm_list[i].mdm_name, INTERNAL_MDM_DEV_NAME,
	    sizeof(INTERNAL_MDM_DEV_NAME))) {
		ret = 1;
		KMSGI("Modem subsystem found on target!");
		LOGI("Modem subsystem found on target!");
		break;
	}
    }
    return ret;
 ...
 * return 0 for fail case.
 */
static int check_support_using_property()
{
    int ret = 0;
...
#if !defined(RMTS_LE_BUILD)

	ret = check_support_using_libmdm();
	if (ret == 0) {
	    /* failed to get modem subsystem using libmdmdetect.*/
	    ret = check_support_using_property();
	    if (ret == 0)
		    exit(1);
	}
