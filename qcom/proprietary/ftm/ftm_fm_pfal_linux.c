...
# Copyright (c) 2010-2015, 2018 by Qualcomm Technologies, Inc.  All Rights Reserved.
...
void *(ftm_on_long_thread)(void *ptr)
{
  ...
  if ((strcmp(value, "rome") != 0) && (strcmp(value, "hastings") != 0))
  {
    ...
    property_get("ro.vendor.qualcomm.bt.hci_transport", transport, NULL);
    print2("ro.vendor.qualcomm.bt.hci_transport = %s\n", transport);
...
fm_cmd_status_type ftm_fm_audio(uint8 source, uint8 volume) {
   ...
   ftm_audio_fd = open("/data/vendor/audio/ftm_commands", O_RDWR);
   if (ftm_audio_fd < 0) {
   ...
   prop_ret = property_get("vendor.qcom.bluetooth.soc", bt_soc_type, NULL);
   if (prop_ret != 0) {
       printf("vendor.qcom.bluetooth.soc set to %s\n", bt_soc_type);
       if ((!strncasecmp(bt_soc_type, "rome", sizeof("rome"))) &&
       (!strncasecmp(bt_soc_type, "hastings", sizeof("hastings")))) {
           buffer = audio_config;
           len = config_len;
           goto skip;
       }
