  ...
  mct_event_t *event)
{
  ...
  if (scaler->is_fast_aec_mode) {
     scaler->orig_width = scaler->hw_stream_info.width;
     scaler->orig_height = scaler->hw_stream_info.height;
     ISP_DBG("scaler: orig wxh = %d x %d\n", scaler->orig_width, scaler->orig_height);
  }
...
ERROR:
  if (scaler->is_fast_aec_mode) {
    ISP_DBG("scaler: orig wxh = %d x %d hw_strem wxh = %d x %d\n",
      scaler->orig_width, scaler->orig_height, scaler->hw_stream_info.width,
      scaler->hw_stream_info.height);
    scaler->hw_stream_info.width = scaler->orig_width;
    scaler->hw_stream_info.height = scaler->orig_height;
  }
  ...
  scaler46_t *scaler, mct_event_control_parm_t *hal_param, uint32_t identity)
{
  ...
  case CAM_INTF_PARM_USERZOOM: {
    scaler->zoom_value = *(cam_zoom_info_t *)hal_param->parm_data;
    ret = scaler46_parm_zoom(module, scaler,
      (cam_zoom_info_t *)hal_param->parm_data,
  ...
  mct_event_t *event)
{
  ...
  case MCT_EVENT_MODULE_SET_FAST_AEC_CONVERGE_MODE: {
    mct_fast_aec_mode_t *fast_aec =  (mct_fast_aec_mode_t*)
        module_event->module_event_data;
    ISP_DBG("%s: FASTAEC identity = 0x%x enable= %d\n",
        __func__, event->identity, fast_aec->enable);
    scaler->is_fast_aec_mode = fast_aec->enable;
    /* Forward event to FRAME port */
    ret = port_scaler46_forward_event(port, event);
    if (ret == FALSE) {
      ISP_ERR("failed: mct_port_send_event_to_peer event %d",
  ...
  void *peer_caps, void *stream_info)
{
  ...
  /* This value will get overwritten if two user streams are mapped to
   * same HW stream */
  ...
  mct_port_direction_t direction, mct_port_caps_type_t caps_type)
{
  ...
  port_scaler46_data =
    (port_scaler46_data_t *)CAM_MALLOC(sizeof(port_scaler46_data_t));
  if (!port_scaler46_data) {
