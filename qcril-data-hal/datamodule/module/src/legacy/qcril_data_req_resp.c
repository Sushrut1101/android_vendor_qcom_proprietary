...
#define REMOVE_ENTRY_FROM_REQLIST( instance_id, token )              \
                                                                     \
  IxErrnoType reqlistErrno = E_FAILURE;                              \
  reqlistErrno = qcril_reqlist_free( instance_id, token );           \
  if( reqlistErrno == E_SUCCESS)                                     \
  {                                                                  \
    QCRIL_LOG_DEBUG("Reqlist Free SUCCESS");                         \
  }                                                                  \
  else                                                               \
  {                                                                  \
    QCRIL_LOG_DEBUG("ERROR!! Reqlist Free failed with error code = %lu"       \
                    ,reqlistErrno);   \
  }
...
/*===========================================================================
  ...
  FUNCTION: qcril_data_retrieve_and_compare_modem_attached_apn
=============================================================================*/
/*!
    @brief
    Before triggering a detach & reattach , we need to check modem is already
    attached to the RIL requested apn & ip type . This helps us in avoiding
    unecessary detach & reattach

    @return
    RIL_E_SKIP_LTE_REATTACH :- If Telephony & modem attach params match
    RIL_E_SUCCESS           :- If Telephony & modem attach params do not match
    RIL_E_GENERIC_FAILURE   :- If QMI WDS returns rc = negative
*/
/*===========================================================================*/
static RIL_Errno qcril_data_retrieve_and_compare_modem_attached_apn(
  const char *ril_apn,
  const char *ril_ipfamily
);
...
/*===========================================================================
  ...
  FUNCTION: qcril_data_match_modem_attached_profile_params
=============================================================================*/
/*!
   @brief
   Compare Telephony APN and IP type with that of APN,IP type to which modem
   is already attached

   @return
   FALSE if any of the parameters doesn't match
   TRUE if all the parameters match
*/
/*===========================================================================*/
static boolean qcril_data_match_modem_attached_profile_params
(
...
)
{
  ...
  REMOVE_ENTRY_FROM_REQLIST( instance_id , t );
...
)
{
  ...
  do
  {
                                                                   ...
                                                                   &max_attach_pdns))
    {
      ...
      else
      {
        ...
        {
          if (QDP_SUCCESS == qdp_match_lte_attach_profile_params(
                             prev_pdn_list.attach_pdn_list[i],
                             qdp_params))
          {
    ...
    else
    {
      ...
      if (QDP_SUCCESS == qdp_match_lte_attach_profile_params(lte_profile_id,
                                                         qdp_params))
      {
  ...
  {
    if( qcril_data_retrieve_and_compare_modem_attached_apn( ril_apn, ril_ipfamily) )
    {
      return RIL_E_SKIP_LTE_REATTACH;
    }
...
/*!
    @brief
    Before triggering a detach & reattach , we need to check if modem is already
    attached to telephony requested apn & ip type . This helps us in avoiding
    unecessary detach & reattach
...
static RIL_Errno qcril_data_retrieve_and_compare_modem_attached_apn
(
  const char *ril_apn, const char *ril_ipfamily
)
{
  wds_get_lte_attach_params_resp_msg_v01 curr_attach_params;
  memset(&curr_attach_params, 0x0, sizeof(curr_attach_params));
  int rc = qcril_data_qmi_wds_get_lte_attach_params(global_qcril_qmi_wds_hndl,
                                                    &curr_attach_params);
  if (QMI_NO_ERR != rc)
  {
    QCRIL_LOG_ERROR("qcril_data_qmi_wds_get_lte_attach_params,"
                    "failed with error [%d][%d]", rc,
                    curr_attach_params.resp.error);
    return RIL_E_GENERIC_FAILURE;
  }
  else
  {
    if(qcril_data_match_modem_attached_profile_params(&curr_attach_params,ril_apn,ril_ipfamily))
    {
      return RIL_E_SKIP_LTE_REATTACH;
    }
    else
...
/*!
   @brief
   Compare Telephony APN and IP type with that of APN,IP type accepted by N.W
...
)
{
      ...
      (modem_attach_params->ip_type_valid) )
  {
                   ...
                   (strlen(ril_apn) + 1)) == 0 )
    {
      switch (modem_attach_params->ip_type)
      {
        ...
        case WDS_IP_SUPPORT_TYPE_IPV4V6_V01:
          ...
          break;
        default:
          QCRIL_LOG_DEBUG("%s", "Modem IP is unsupported and ril_ipfamily is [%s]",
                          ril_ipfamily);
          break;
      }
    }
    ...
    {
      QCRIL_LOG_DEBUG("Modem Attach APN [%s] Attach APN [%s] do not match",
                      modem_attach_params->apn_string, ril_apn);
      return false;
    }
  }
  return false;
...
/*===========================================================================
  ...
  FUNCTION: qcril_data_get_access_network_from_radio_tech

===========================================================================*/
/*!
  @brief
  Get access network type for radio technology

  @return
  Access network type RIL_RadioAccessNetworks
*/
/*===========================================================================*/
RIL_RadioAccessNetworks qcril_data_get_access_network_from_radio_tech
(
  qcril_data_ril_radio_tech_t tech
)
{
  RIL_RadioAccessNetworks ret = UNKNOWN;

  switch(tech) {
    case QCRIL_DATA_RIL_RADIO_TECH_GPRS:
    case QCRIL_DATA_RIL_RADIO_TECH_EDGE:
    case QCRIL_DATA_RIL_RADIO_TECH_GSM:
      ret = GERAN;
      break;

    case QCRIL_DATA_RIL_RADIO_TECH_UMTS:
    case QCRIL_DATA_RIL_RADIO_TECH_HSDPA:
    case QCRIL_DATA_RIL_RADIO_TECH_HSPAP:
    case QCRIL_DATA_RIL_RADIO_TECH_HSUPA:
    case QCRIL_DATA_RIL_RADIO_TECH_HSPA:
    case QCRIL_DATA_RIL_RADIO_TECH_TDSCDMA:
      ret = UTRAN;
      break;

    case QCRIL_DATA_RIL_RADIO_TECH_IS95A:
    case QCRIL_DATA_RIL_RADIO_TECH_IS95B:
    case QCRIL_DATA_RIL_RADIO_TECH_1xRTT:
    case QCRIL_DATA_RIL_RADIO_TECH_EVDO_0:
    case QCRIL_DATA_RIL_RADIO_TECH_EVDO_A:
    case QCRIL_DATA_RIL_RADIO_TECH_EVDO_B:
    case QCRIL_DATA_RIL_RADIO_TECH_EHRPD:
      ret = CDMA;
      break;

    case QCRIL_DATA_RIL_RADIO_TECH_LTE:
    case QCRIL_DATA_RIL_RADIO_TECH_LTE_CA:
      ret = EUTRAN;
      break;

    case QCRIL_DATA_RIL_RADIO_TECH_IWLAN:
      ret = IWLAN;
      break;

    case QCRIL_DATA_RIL_RADIO_TECH_UNKNOWN:
    default:
      ret = UNKNOWN;
      break;
  }
...
boolean isEmergencyCall(const qcril_request_params_type *const rilParams)
{
  const char *apnTypes = NULL;
  if (NULL == rilParams) {
    return FALSE;
  }
#if (QCRIL_RIL_VERSION >= 15)
  apnTypes = ((const char **)rilParams->data)[8];
#endif
  if (NULL != apnTypes) {
    QCRIL_LOG_DEBUG("checking for emergency APN");
    return qcrili_data_is_apn_type_found(RIL_APN_TYPE_EMERGENCY, atoi(apnTypes));
  }
  return FALSE;
}

void prepareEmergencyLookupParams(
  const qcril_request_params_type *const rilParams,
  const char  ** qdpParams
)
{
  const char               *ril_tech = NULL;
  const char               *ril_profile = NULL;
  const char               *ril_apn = NULL;
  const char               *ril_user = NULL;
  const char               *ril_pass = NULL;
  ...
  ril_tech = ((const char **)rilParams->data)[QDP_RIL_TECH];
  ril_profile = ((const char **)rilParams->data)[QDP_RIL_PROFILE_ID];
  ril_apn  = ((const char **)rilParams->data)[QDP_RIL_APN];
  ril_user = ((const char **)rilParams->data)[QDP_RIL_NAI];
  ril_pass = ((const char **)rilParams->data)[QDP_RIL_PASSWORD];
  ril_auth_pref = ((const char **)rilParams->data)[QDP_RIL_AUTH];
  ril_ipfamily = ((const char **)rilParams->data)[QDP_RIL_IP_FAMILY];
#if (QCRIL_RIL_VERSION >= 15)
  ril_ip_roaming = ((const char **)rilParams->data)[QDP_RIL_IP_ROAMING];
  ril_apn_types = ((const char **)rilParams->data)[8];
  ril_roaming_allowed = ((const char **)rilParams->data)[14];
#endif

  qdpParams[QDP_RIL_TECH] = ril_tech;
  qdpParams[QDP_RIL_PROFILE_ID] = ril_profile;
  qdpParams[QDP_RIL_APN] = ril_apn;
  qdpParams[QDP_RIL_NAI] = ril_user;
  qdpParams[QDP_RIL_PASSWORD] = ril_pass;
  qdpParams[QDP_RIL_AUTH] = ril_auth_pref;
  qdpParams[QDP_RIL_IP_FAMILY] = ril_ipfamily;
  qdpParams[QDP_RIL_IP_ROAMING] = ril_ip_roaming;
}

boolean setEmergencyDataCallParams(
  int profileId,
...
)
{
  ...
  memset(&ipfamily_info, 0, sizeof(ipfamily_info));

  /* set dsi profile id */
  apn_info.buf_val = NULL;
  apn_info.num_val = profileId;
  switch (techType) {
    case QDP_3GPP:
      info_tbl[infoTblIndex].qdp_3gpp_profile_pdn_type = pdnType;
      info_tbl[infoTblIndex].qdp_umts_profile_id = profileId;
      QCRIL_LOG_VERBOSE("Setting umts profile id [%d] on info_tbl [%d]",
                        profileId, infoTblIndex);
      if ( ( dsi_set_data_call_param( info_tbl[infoTblIndex].dsi_hndl ,
                                      DSI_CALL_INFO_UMTS_PROFILE_IDX,
                                      &apn_info ) ) != DSI_SUCCESS )
      {
        QCRIL_LOG_ERROR("unable to set umts profile id on info_tbl  index [%d]",
                        info_tbl[infoTblIndex].index );
        return FALSE;
      }
      break;
    case QDP_3GPP2:
      info_tbl[infoTblIndex].qdp_3gpp2_profile_pdn_type = pdnType;
      info_tbl[infoTblIndex].qdp_cdma_profile_id = profileId;
      ...
      break;
    default:
      QCRIL_LOG_ERROR("%s", "profile lookup failed");
      return FALSE;
  }
  /* set dsi ip family */
  ...
  info_tbl[infoTblIndex].dsi_ip_version = ipfamily_info.num_val;
  if ( ( dsi_set_data_call_param( info_tbl[infoTblIndex].dsi_hndl ,
                                  DSI_CALL_INFO_IP_VERSION,
                                  &ipfamily_info) ) != DSI_SUCCESS )
  {
    QCRIL_LOG_ERROR("unable to set ip family, index [%d]", info_tbl[infoTblIndex].index );
    return FALSE;
  }
  return TRUE;
}
...
/*===========================================================================
  ...
  FUNCTION:  allocate_cid

===========================================================================*/
/*!
    @brief
    Allocate dynamic memory for 'cid' & copy input request's cid into it.

    @return
    None.
*/
/*=========================================================================*/
void allocate_cid( const int cid, qcril_reqlist_u_type* u_info)
{
  int *cid_ptr = NULL;

  if(u_info)
  {
    QCRIL_LOG_DEBUG("allocate_cid:: memory allocated to info object");
    u_info->ds.info = (void *) malloc( sizeof(int) );
    cid_ptr = (int*)(u_info->ds.info);
    if( (cid_ptr) != NULL )
    {
      *cid_ptr = cid;
    }
  ...
  {
    QCRIL_LOG_ERROR("allocate_cid:: 'qcril_reqlist_u_type' object is NULL"
                    " memory not allocated to 'info' object");
  }
}

/*===========================================================================

  FUNCTION:  deallocate_cid

===========================================================================*/
/*!
    @brief
    Dellocate the dynamic memory for 'cid' entry in reqlist

    @return
    None.
*/
/*=========================================================================*/
void deallocate_cid( const qcril_instance_id_e_type instance_id, const RIL_Token token )
{
  ...
  if ( (qcril_ret = qcril_reqlist_query( instance_id, token,
                                                         &reqlist_info ) ) != E_SUCCESS )
  {
    QCRIL_LOG_INFO("unable to find reqlist entry. token with params->t[0x%x]",
                        token);
    return;
  }
  cid_ptr = (int *)( reqlist_info.sub.ds.info);

  QCRIL_LOG_INFO("deallocate_cid::[RID %d] ReqList entry : %s(%p) token id %" PRId32 " at (%p)", instance_id,
  qcril_log_lookup_event_name( reqlist_info.request ), reqlist_info.request, reqlist_info.t_id, reqlist_info.t );

  if( cid_ptr != NULL )
  {
    QCRIL_LOG_INFO("deallocate_cid::[RID %d] cid = %d",instance_id, *cid_ptr);
    free(cid_ptr);
    cid_ptr = NULL;
    reqlist_info.sub.ds.info = NULL;
  }
  else
  {
    QCRIL_LOG_INFO(" deallocate_cid::cid_ptr is NULL");
  }
}
...
)
{
  ...
  const char               *ril_apn_types = NULL;
  const char               *ril_roaming_allowed = NULL;
  const char               *ril_access_network = NULL;
  const char               *ril_reason = NULL;
  const char               *ril_ipaddresses = NULL;
  const char               *ril_dnsaddresses = NULL;
  char                      tmp_tech[ QCRIL_DATA_TECHNOLOGY_MAX_LEN + 1 ];
  char                      tmp_apn[ DS_CALL_INFO_APN_MAX_LEN + 1 ];
  char                      tmp_reason[ QCRIL_REASON_STR_MAX + 1 ];
  qcril_reqlist_u_type      u_info;
  qcril_reqlist_public_type reqlist_entry;
  int                       umts_profile_id = QCRIL_INVALID_PROFILE_ID;
  int                       cdma_profile_id = QCRIL_INVALID_PROFILE_ID;
  ...
  unsigned int               emergencyProfileId = QCRIL_INVALID_PROFILE_ID;
  qdp_profile_pdn_type       emergencyPdnType = QDP_PROFILE_PDN_TYPE_INVALID;
  qdp_tech_t                 emergencyTechType = QDP_NOTECH;
  boolean useEmergencyProfile = FALSE;
  uint8_t reported_tech_valid = FALSE;
  int reported_tech = 0;
  uint8_t data_rat_status = FALSE;
  int tmp_radiotech;

  QCRIL_LOG_VERBOSE( "%s", "qcril_data_request_setup_data_call: ENTRY" );

  /* emergency call scenario, bypass validation and use emergency supported profile */
  if (isEmergencyCall(params_ptr)) {
    const char * qdpParams[QDP_RIL_PARAM_MAX];
    memset(qdpParams, 0, sizeof(qdpParams));
    prepareEmergencyLookupParams(params_ptr, qdpParams);

    if (QDP_SUCCESS == emergencyProfileLookup(&qdpParams[0],
                                              &emergencyProfileId,
                                              &emergencyPdnType,
                                              &emergencyTechType))
    {
  ...
  memset( tmp_tech,   0, QCRIL_DATA_TECHNOLOGY_MAX_LEN + 1 );
  memset( tmp_apn,   0, DS_CALL_INFO_APN_MAX_LEN + 1 );
  memset( tmp_reason,   0, QCRIL_REASON_STR_MAX + 1 );
  ...
  /* Radio HAL version 1.2 setupDataCall_1_2 contains access network
   * instead of radio technology. If ril_access_network is not NULL,
   * setupDataCall_1_2 is used, and ril_tech should not be considered.
   * ril_tech in info_tbl_ptr->call_params->ril_tech is set with current
   * RAT which is reported in DATA_REGISTRATION_STATE to be used
   * in qcril_data_validate_call_technology() to validate 1x to eHRPD case */
  if (strlen(((const char **)params_ptr->data)[15]) != 0) {
    /* If access network is available, use current data technology which is
     * the latest reported in DATA_REGISTRATION_STATE indication */
    ril_access_network = ((const char **)params_ptr->data)[15];
    data_rat_status  = qcril_qmi_nas_fetch_data_reg_rat(&reported_tech_valid, &reported_tech);
    if(data_rat_status)
    {
      tmp_radiotech = QCRIL_DATA_CONVERT_RIL_RADIOTECH(reported_tech);
    } else {
      tmp_radiotech = QCRIL_DATA_CONVERT_RIL_RADIOTECH(RADIO_TECH_UNKNOWN);
    }
    snprintf(tmp_tech, QCRIL_DATA_TECHNOLOGY_MAX_LEN + 1, "%d", tmp_radiotech);
    RIL_RadioAccessNetworks apan = qcril_data_get_numeric_access_network(ril_access_network);
    RIL_RadioAccessNetworks mpan = qcril_data_get_access_network_from_radio_tech(tmp_radiotech);

    /*TODO:
    The checks which were added as part of OS's requirement
    have been modified to address specific use-cases of Customer.
    If needed, these will be updated
    */

    if (mpan == UNKNOWN) {
      QCRIL_LOG_DEBUG("Radio Technology received is UNKNOWN!!");
    }

    /* allow emergency call regardless of RAT */
    if ( ( apan != UNKNOWN ) && ( apan != mpan ) && (!useEmergencyProfile) ) {
      QCRIL_LOG_ERROR("Requested access network [%d] is not current data technology [%d]",
        apan, tmp_radiotech);
      info_tbl[i].call_info.inactiveReason = PDP_FAIL_OEM_DCFAILCAUSE_4;
      goto err_label;
    }

    /* ril_tech is set to current data radio technology */
    ril_tech = (const char *)tmp_tech;
  }
  else
  {
    /* If access network is not available, use radio technoloty */
    QCRIL_LOG_DEBUG ("RIL - access network is NULL");

    /* ril_tech is set to given radio technology */
    ril_tech = ((const char **)params_ptr->data)[0];
  }

  /* Radio HAL version 1.2 setupDataCall_1_2 contains data call request reason
   * if it is for normal, shutdown, or handover. Handover reason is returned
   * as not supported unless IWLAN refactoring is supported */
  if (strlen(((const char **)params_ptr->data)[16]) != 0) {
    ril_reason = ((const char **)params_ptr->data)[16];
    snprintf(tmp_reason, QCRIL_REASON_STR_MAX + 1, "%d", HANDOVER);
    if(strcmp(ril_reason, tmp_reason) == 0) {
      QCRIL_LOG_ERROR("Requested reason Handover is not supported [%s]", ril_reason);
      info_tbl[i].call_info.inactiveReason = PDP_FAIL_FEATURE_NOT_SUPP;
      goto err_label;
    }
  }
  else
  {
    QCRIL_LOG_DEBUG ("RIL - data request reason is NULL");
  }

  if (strlen(((const char **)params_ptr->data)[17]) != 0) {
    ril_ipaddresses = ((const char **)params_ptr->data)[17];
  }
  else
  {
    QCRIL_LOG_DEBUG ("RIL - ip address is NULL");
  }

  if (strlen(((const char **)params_ptr->data)[18]) != 0) {
    ril_dnsaddresses = ((const char **)params_ptr->data)[18];
  }
  else
  {
    QCRIL_LOG_DEBUG ("RIL - dns address is NULL");
  }
  ...
  else
  {
    ...
    if (useEmergencyProfile) {
      /* Set DSI params and info_tbl and skip to start data call. Only need
         to set profile id, pdn type, and tech type */
      if (!setEmergencyDataCallParams((int)emergencyProfileId,
                                      emergencyPdnType,
                                      emergencyTechType,
                                      i))
      {
        QCRIL_LOG_ERROR("%s", "unable to set all dsi parameters");
        goto err_label;
      }
    }
    else {
      ...
      if (ril_apn != NULL)
      {
        ...
        if(umts_profile_id > QCRIL_INVALID_PROFILE_ID)
        {
        ...
        if (cdma_profile_id > QCRIL_INVALID_PROFILE_ID)
        {
        ...
        if (cdma_profile_id == QCRIL_INVALID_PROFILE_ID || umts_profile_id == QCRIL_INVALID_PROFILE_ID)
        {
        ...
        else if (strcmp(ril_ipfamily, QCRIL_DATA_IP_FAMILY_V6) == 0)
        {
          ...
          if( global_qcril_clat_supported )
          {
            if (ril_apn != NULL && ril_apn_types != NULL &&
                      atoi(ril_apn_types) == RIL_APN_TYPE_DEFAULT) {
                QCRIL_LOG_DEBUG("%s(): Automatic IPv4 request for APN %s",
                                __func__, ril_apn_types);
                ipfamily_info.num_val = DSI_IP_VERSION_4_6;
            } else {
  ...
  if ( ( dsi_start_data_call( info_tbl[i].dsi_hndl ) ) != DSI_SUCCESS )
  {
    ...
    /* Remove the entry from ReqList */
    REMOVE_ENTRY_FROM_REQLIST( instance_id , params_ptr->t );
    goto err_label;
  }
...
/*!
    ...
    @param
      ...
      ril_access_network [in] - Access Network
...
)
{
  ...
  {
    QCRIL_LOG_DEBUG("copying ril_access_network=%s", ril_access_network);

    strlcpy(info_tbl_ptr->call_params->ril_access_network,
            ril_access_network,
            sizeof(info_tbl_ptr->call_params->ril_access_network));
  }
}
...
)
{
  ...
  /* Create the timer */
  if (-1 == timer_create(CLOCK_MONOTONIC, &sigev, &info_tbl_ptr->retry_timer_id))
  {
...
)
{
  ...
  /* Radio HAL version 1.2 deactivateDataCall_1_2 contains data call request reason
   * if it is for normal, shutdown, or handover. Handover reason is returned
   * as not supported unless IWLAN refactoring is supported */
  if (strlen(((const char **)params_ptr->data)[2]) != 0) {
    ril_reason = ((const char **)params_ptr->data)[2];
    snprintf(tmp_reason, QCRIL_REASON_STR_MAX + 1, "%d", HANDOVER);
    if(strcmp(ril_reason, tmp_reason) == 0) {
      QCRIL_LOG_ERROR("Requested reason Handover is not supported [%s]", ril_reason);
      call_end_cause = RIL_E_REQUEST_NOT_SUPPORTED;
      goto err_label;
    }
  }
  else
  ...
  u_info.ds.info = NULL;
  allocate_cid(info_tbl[i].cid, &u_info);
  qcril_reqlist_default_entry( params_ptr->t,
                               QCRIL_EVT_RIL_REQUEST_DEACTIVATE_DATA_CALL,
                               modem_id,
                               QCRIL_REQ_AWAITING_MORE_AMSS_EVENTS,
  ...
  if (E_SUCCESS != stop_data_err)
  {
    ...
    {
      call_end_cause = RIL_E_OEM_ERROR_3;
      QCRIL_LOG_DEBUG("%s", "Not cleaning up call in info_tbl cache & reqlist ");
      QCRIL_DATA_MUTEX_UNLOCK(INFO_TBL_MUTEX);
      QCRIL_LOG_DEBUG("%s", "qcril_data_request_deact_data_call:"
                            "EXIT.Not Sending response to telephony");
      return;
    }
  }
  ...
  do
  {
    ...
    for(j = 0; j < umts_profile_list_len; j++)
    {
      ...
      if(tech_type == QDP_3GPP)
      {
        ...
        //Compare the 3 parameters retrieved from profile through QMI_WDS query & stored in new_p_params fields
        if ( ril_maxConnstime > 0 )
        {
          if ( new_p_params.max_pdn_conn_timer != ril_maxConnstime )
          {
            QCRIL_LOG_DEBUG("Update needed due to change in maxpdn_conn_timer");
            updateNeeded = TRUE;
        ...
        if ( ril_maxConns > 0 )
        {
          if ( new_p_params.max_pdn_conn_per_block != ril_maxConns )
          {
            QCRIL_LOG_DEBUG("Update needed due to change in maxpdn_conn_block");
            updateNeeded = TRUE;
        ...
        if ( ril_waitTime > 0 )
        {
          if ( new_p_params.pdn_req_wait_interval != ril_waitTime )
          {
            QCRIL_LOG_DEBUG("Update needed due to change in pdn_req_wait_interval");
            updateNeeded = TRUE;
            new_p_params.pdn_req_wait_interval_valid = TRUE;
            new_p_params.pdn_req_wait_interval = ril_waitTime;
          }
        }
        if ( new_p_params.apn_disabled_flag == ril_enabled )
        {
          QCRIL_LOG_DEBUG("Update needed due to change in apn_disabled_flag ");
          updateNeeded = TRUE;
      ...
      if( updateNeeded )
      {
        QCRIL_LOG_DEBUG("Updating Profile Params");
      ...
      {
        QCRIL_LOG_DEBUG("Update of Profile Params is not Needed. Skipping profile updation");
      }
    }
  ...
  )
{
  ...
  if ((params_ptr == NULL) || (ret_ptr == NULL) || (params_ptr->data == NULL)) {
  ...
  if (ret != RIL_E_SUCCESS) {
    ...
    if (e == NULL) {
      ...
      e->type = req->type;
      strlcpy(e->sourceAddress, req->sourceAddress, MAX_INADDR_LEN+1);
      e->sourcePort = req->sourcePort;
      strlcpy(e->destinationAddress, req->destinationAddress, MAX_INADDR_LEN+1);
...
void qcril_data_handle_keepalive_ind(wds_ka_result_type_enum_v01 status, uint32_t handle)
{
  ...
  if (status == WDS_KEEP_ALIVE_RESULT_OK_V01) {
    QCRIL_LOG_DEBUG("Keepalive active for handle %d", handle);
    code = KEEPALIVE_ACTIVE;
  } else {
    QCRIL_LOG_DEBUG("Keepalive inactive for handle %d", handle);
...
/*===========================================================================
    ...
    FUNCTION: qcril_data_reqlist_free

===========================================================================*/
/*!
    @brief
    API to delete request from Reqlist cache mantained in framework

    @return
    IxErrnoType value
    E_FAILURE : Upon failure in deleting entry in reqlist
    E_SUCCESS : Upon success in deleting entry in reqlist
*/
/*=========================================================================*/
IxErrnoType qcril_data_reqlist_free(const qcril_request_params_type *const params_ptr)
{
  if(params_ptr == NULL)
  {
    QCRIL_LOG_DEBUG("qcril_data_reqlist_free:: params is NULL returning Error");
    return E_FAILURE;
  }

  REMOVE_ENTRY_FROM_REQLIST( params_ptr->instance_id , params_ptr->t );
