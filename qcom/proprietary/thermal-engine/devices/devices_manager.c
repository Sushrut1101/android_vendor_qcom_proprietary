...
static pthread_mutex_t clnt_mtx = THERMAL_RECURSIVE_MUTEX_INITIALIZER;
...
static int update_dev_state(struct devices_manager_dev *dev_mgr)
{
	...
	if ((dev_mgr->dev_info.dev_type != DEVICE_GENERIC_TYPE) &&
	    ...
	    (dev_mgr->dev_info.dev_type != DEVICE_CDEV_TYPE))
		return -(EFAULT);
			   ...
			   struct device_state_info *info)
{
	...
	if (dev_mgr->current_op)
		dev_mgr->current_op(dev_mgr, (int *)&info->current_op.lvl);
