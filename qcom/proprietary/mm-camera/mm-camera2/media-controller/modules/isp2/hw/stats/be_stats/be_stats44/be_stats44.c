...
#define BE_STATS44_MIN_WIDTH  4
#define BE_STATS44_MIN_HEIGHT 2
#define BE_STATS44_MAX_HNUM   32
#define BE_STATS44_MAX_VNUM   24
#define BE_STATS44_MIN_HVNUM 1
 ...
 * Return void
 **/
static void be_stats44_debug(ISP_StatsBe_CfgCmdType *pcmd)
{
  if (!pcmd) {
    return;
  }
...
/** be_stats44_fill_stats_parser_params:
 *
 ...
 **/
static boolean be_stats44_fill_stats_parser_params(
  isp_sub_module_t *isp_sub_module, be_stats44_t *be_stats,
  isp_sub_module_output_t *sub_module_output, uint32_t camif_window_w,
  uint32_t camif_window_h)
{
  ISP_StatsBe_CfgCmdType      *pcmd = NULL;
  ...
  stats_config->stats_mask |= (1 << MSM_ISP_STATS_BE);
  stats_config->aec_config = be_stats->aec_config;

  roi_params =
    &stats_params->stats_roi_params[MSM_ISP_STATS_BE];
  ...
  rgns_stats = &stats_params->rgns_stats[MSM_ISP_STATS_BE];
  ...
  if (isp_sub_module->tintless_enabled == TRUE) {
    tintless_stats_config =
      &algo_params->tintless_stats_config[MSM_ISP_STATS_BE];
    ...
    tintless_stats_config->stat_elem_w =
      FLOOR2(camif_window_w / ISP_STATS_BE_GRID_H);
    tintless_stats_config->stat_elem_h =
      FLOOR2(camif_window_h / ISP_STATS_BE_GRID_V);
    tintless_stats_config->num_stat_elem_cols = ISP_STATS_BE_GRID_H;
    tintless_stats_config->num_stat_elem_rows = ISP_STATS_BE_GRID_V;
    tintless_stats_config->saturation_limit = 255 - PIX_VALUE_OFFSET;
    tintless_stats_config->stats_type = MSM_ISP_STATS_BE;
  }

  return TRUE;
}

/** be_stats44_store_hw_update:
 *
 *  @isp_sub_module: isp sub module handle
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean be_stats44_store_hw_update(isp_sub_module_t *isp_sub_module,
  be_stats44_t *be_stats)
{
  boolean                      ret = TRUE;
  ISP_StatsBe_CfgCmdType      *pcmd = NULL;
  struct msm_vfe_cfg_cmd2     *cfg_cmd = NULL;
  struct msm_vfe_reg_cfg_cmd  *reg_cfg_cmd = NULL;
  ISP_StatsBe_CfgCmdType      *copy_cmd = NULL;
  ...
  copy_cmd = (ISP_StatsBe_CfgCmdType *)CAM_MALLOC(sizeof(*copy_cmd));
  if (!copy_cmd) {
 ...
 * Return void
 **/
static boolean be_stats44_split_config(be_stats44_t *be_stats,
  ...
  void *data)
{
  ...
  be_stats = (be_stats44_t *)isp_sub_module->private_data;
  if (!be_stats) {
  ...
  if ((isp_sub_module->submod_enable == FALSE) ||
    ...
    if (sub_module_output->stats_params) {
      ret = be_stats44_fill_stats_parser_params(isp_sub_module, be_stats,
        sub_module_output, camif_window_w, camif_window_h);
      if (ret == FALSE) {
        ISP_ERR("failed: be_stats44_fill_stats_parser_params");
      }
    }
    PTHREAD_MUTEX_UNLOCK(&isp_sub_module->mutex);
    return TRUE;
  ...
  if ((be_config->roi.left + be_config->roi.width) > camif_window_w) {
  ...
  if ((be_config->roi.top + be_config->roi.height) > camif_window_h) {
  ...
  if ((be_config->grid_info.h_num != 0) && (be_config->grid_info.v_num != 0)) {
    ...
    rgnwidth = Clamp(rgnwidth, BE_STATS44_MIN_WIDTH, Maxwidth);

    rgnheight = FLOOR2(be_config->roi.height / be_config->grid_info.v_num);
    Maxheight = FLOOR2(camif_window_h/be_config->grid_info.v_num);
    rgnheight = Clamp(rgnheight,BE_STATS44_MIN_HEIGHT, Maxheight);

    pcmd->rgnWidth = rgnwidth - 1;
    pcmd->rgnHeight = rgnheight - 1;
    be_config->roi.width = (rgnwidth) * be_config->grid_info.h_num;
    be_config->roi.height = (rgnheight)* be_config->grid_info.v_num;
    pcmd->rgnHOffset = FLOOR2(be_config->roi.left);
    pcmd->rgnVOffset = FLOOR2(be_config->roi.top);
    pcmd->rgnHNum    = Clamp(be_config->grid_info.h_num, BE_STATS44_MIN_HVNUM,
      BE_STATS44_MAX_HNUM) - 1;
    pcmd->rgnVNum    = Clamp(be_config->grid_info.v_num, BE_STATS44_MIN_HVNUM,
      BE_STATS44_MAX_VNUM) - 1;
    ...
    ISP_INFO("warning: Denomitor is zero hnum %d vnum %d",
      be_config->grid_info.h_num, be_config->grid_info.v_num);

  /* Default configuration */
    pcmd->rgnWidth   = FLOOR2(camif_window_w / ISP_STATS_BE_GRID_H) - 1;
    pcmd->rgnHeight  = FLOOR2(camif_window_h / ISP_STATS_BE_GRID_V) - 1;
    pcmd->rgnHOffset = FLOOR2((camif_window_w - ISP_STATS_BE_GRID_H *
      (pcmd->rgnWidth + 1)) / 2);
    pcmd->rgnVOffset = FLOOR2((camif_window_h - ISP_STATS_BE_GRID_V *
      (pcmd->rgnHeight + 1)) / 2);
    pcmd->rgnHNum    = ISP_STATS_BE_GRID_H - 1;
    pcmd->rgnVNum    = ISP_STATS_BE_GRID_V - 1;
    ...
    /* update be_config what we applied */
    be_config->roi.width = (pcmd->rgnWidth + 1) * ISP_STATS_BE_GRID_H;
    be_config->roi.height = (pcmd->rgnHeight + 1) * ISP_STATS_BE_GRID_V;
    be_config->roi.left = pcmd->rgnHOffset;
    be_config->roi.top = pcmd->rgnVOffset;
    be_config->grid_info.h_num = pcmd->rgnHNum + 1;
    be_config->grid_info.v_num = pcmd->rgnVNum + 1;
  ...
  if (be_stats->isp_out_info.is_split == TRUE) {
     ret =  be_stats44_split_config(
       be_stats, pcmd->rgnHNum + 1, pcmd->rgnHOffset, pcmd->rgnWidth + 1);
     if (ret == FALSE) {
       ISP_ERR("failed be_stats44_split_config");
       goto ERROR;
     }
  }
  ...
  ret = be_stats44_store_hw_update(isp_sub_module, be_stats);
  if (ret == FALSE) {
    ISP_ERR("failed: be_stats44_store_hw_update");
    goto ERROR;
  }
...
/** be_stats44_stats_config_validate:
 *
 *  @isp_sub_module: isp sub module handle
 ...
 *  Return TRUE on success and FALSE on failure
 **/
boolean be_stats44_stats_config_validate(be_stats44_t *bg_stats,
...

{
    ...
    be_config = &aec_config->be_config;
    if ((be_config->grid_info.h_num == 0) ||
      (be_config->grid_info.v_num == 0)) {
    ...
    if((be_config->roi.top + be_config->roi.height > camif_window_h_t) ||
      (be_config->roi.left + be_config->roi.width > camif_window_w_t)){
      ISP_ERR("Invalid BG ROI from 3A %d %d %d %d", be_config->roi.left,
          be_config->roi.top, be_config->roi.width, be_config->roi.height);
      goto ERROR;
    }
    return TRUE;
...
/** be_stats44_stats_config_update:
 *
 *  @isp_sub_module: isp sub module handle
 *  @data: handle to stats_config_t
 *
  ...
  void *data)
{
  ...
  if (memcmp(aec_config, &be_stats->aec_config,
    sizeof(aec_config_t))) {
...
boolean be_stats44_init(isp_sub_module_t *isp_sub_module)
{
  ...
  be_stats = (be_stats44_t *)CAM_MALLOC(sizeof(be_stats44_t));
  if (!be_stats) {
    ISP_ERR("failed: be_stats %p", be_stats);
    PTHREAD_MUTEX_UNLOCK(&isp_sub_module->mutex);
    return FALSE;
