...
int main()
{
    ...
    len = __system_property_get("vendor.qti.nfc.chipid", valueStr);
    if (len > 0)
        printf ("%s: vendor.qti.nfc.chipid: %s \n", __FUNCTION__, valueStr);

    snprintf(chip_ver, 5, "0x%02x", chip_version);
    __system_property_set("vendor.qti.nfc.chipid", chip_ver);
    len = __system_property_get("vendor.qti.nfc.chipid", valueStr);
    printf ("After system property set : vendor.qti.nfc.chipid: %s \n", valueStr);
    ...
    __system_property_set("vendor.qti.nfc.fwver", firmware_version);
    len = __system_property_get("vendor.qti.nfc.fwver", valueStr);
    printf ("After system property set : vendor.qti.nfc.fwver: %s \n", valueStr);
    close(fp);
