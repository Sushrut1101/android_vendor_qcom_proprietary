...
public:
  ...
  */
  bool IsBatchCompleted(slimServiceEnumT eService,
                          int iSamplesLen,
                          uint64_t tFrameTime,
                          int iBatchType,
                          int iBatchValue
                       );
  uint64_t m_tLastCANPktTime;
  ...
  : public VehicleNetworkMessageProcessor
{
  public:
    ...
    ~VehicleNetworkCONF1GyroMP()
    {
    }
    virtual void OnNewCanFrameEvent(CanFrameEvent* pzEvent);
    virtual void ToString(char* pStr, size_t sLen);
    virtual void CheckForError(float InputVal,GyroCONF1Config* m_pzCfg,boolean* ErrorFound);
  protected:
    GyroCONF1Config* m_pzCfg;
    static const int msGYRO_VEL_AX=3;
};
...
namespace BitModify
{
  void ChangeBit(long long *pInput,long long off,long long bit,int NBits);
  void  ReverseBits(long long *pInput,long long LBitPos,long long RBitPos,int NBits);
}
