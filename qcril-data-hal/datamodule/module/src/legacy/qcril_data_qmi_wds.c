...
)
{
  ...
  QCRIL_DATA_ALLOC_STORAGE(event_data,
      sizeof(qcril_data_wds_event_data_t), bail);
  ...
  switch(ind_id)
  {
    ...
    case QMI_WDS_MODEM_ASSISTED_KA_STATUS_IND_V01:
      {
        QCRIL_LOG_DEBUG("%s", "process QMI_WDS_MODEM_ASSISTED_KA_STATUS_IND_V01");

        ind_data_ptr = &event_data->ind_data.ka_status;
        ind_data_len = sizeof(event_data->ind_data.ka_status);
      }
      break;
#endif
    case QMI_WDS_DATA_BEARER_TYPE_IND_V01:
      {
        QCRIL_LOG_DEBUG("%s", "process QMI_WDS_DATA_BEARER_TYPE_IND_V01");

        ind_data_ptr = &event_data->ind_data.bearer_type_ind;
        ind_data_len = sizeof(event_data->ind_data.bearer_type_ind);
      }
      break;
