...
/* NPU Cooling device */
static pthread_mutex_t npu_mtx = PTHREAD_MUTEX_INITIALIZER;
static int npu_max_state = -1;

int npu_get_op(int *lvl, int cdev_id)
{
	...
	if (read_line_from_file(node_buf, buf, UINT_BUF_MAX) <= 0) {
		msg("Could not read %s cur state\n", NPU_DEV_NAME);
		return -1;
	}
	...
	if (curr_lvl == LONG_MIN || curr_lvl == LONG_MAX ||
		...
		msg("Invalid %s cur state %d\n", NPU_DEV_NAME, curr_lvl);
		return -1;
	}
	*lvl = curr_lvl;
...
int npu_freq_set(int value, int cdev_id)
{
	char buf[UINT_BUF_MAX] = {0};
	char node_buf[MAX_PATH] = {0};
	...
	if (write_to_file(node_buf, buf, strlen(buf)) > 0) {
		dbgmsg("ACTION: NPU - "
		     "Setting NPU Level to %d\n", value);
		thermalmsg(LOG_LVL_DBG,
			(LOG_LOGCAT | LOG_LOCAL_SOCKET | LOG_TRACE),
			"%s:NPU:%d\n", MITIGATION, value);
	} else {
		msg("Unable to set NPU cdev state to %d\n", value);
		return -1;
	}
...
int npu_device_init(void)
{
	...
	cdev_id = get_cdevn(NPU_DEV_NAME);
	if (cdev_id < 0) {
	...
	if (max_state == LONG_MIN || max_state == LONG_MAX ||
		...
		msg("Invalid %s max state %d\n", NPU_DEV_NAME, max_state);
		return -1;
	}
	npu_max_state = max_state;
	dbgmsg("NPU device initialized. max state:%d\n", npu_max_state);
	npu_freq_set(0, cdev_id);

	return max_state;
}

/*===========================================================================
FUNCTION npu_freq_request

Action function to request NPU Freq throlling.

ARGUMENTS
	value - NPU cooling device level
	cdev_id - cooling device ID

RETURN VALUE
	0 on success, -1 on failure.
===========================================================================*/
int npu_freq_request(int value, int cdev_id)
{
	static int prev;

	if (npu_max_state == -1) {
		msg("NPU Cooling device not initialized\n");
		return -1;
	}

	if (value < 0) {
		msg("Incorrect value %d value for NPU Frequency level\n", value);
		return -1;
	}

	pthread_mutex_lock(&npu_mtx);

	if (value > npu_max_state)
		value = npu_max_state;

	if (prev != value) {
		if (!npu_freq_set(value, cdev_id))
			prev = value;
	}
	pthread_mutex_unlock(&npu_mtx);
...
static int check_devfreq_gpu_node(void)
{
	...
	if (!ngpus) {
		ngpus = 1;
		if (snprintf(gpu_devfreq_avail_freq, MAX_PATH, "%s%s", temp,
			 DEVFREQ_AVAIL_FREQ) < 0)
...
static void wlan_init_node_name(void)
{
	...
	if (!ret) {
		if (snprintf(wlan_sysfs_node, MAX_PATH, "%s%s", temp,
...
ARGUMENTS
	cpu - CPU to hotplug
	offline - offline/online request
	cdev_id - cooling device ID

RETURN VALUE
	0 on success, -1 on failure.
===========================================================================*/
int hotplug_request(int cpu, int offline, int cdev_id)
{
	int ret = 0;
	char node_buf[MAX_PATH] = {0};
	char val_buf[UINT_BUF_MAX] = {0};
	...
	if (((uint32_t)offline) != HOTPLUG_CPU_OFFLINE(cpu)) {
		if (cdev_id != -1) {
			snprintf(node_buf, MAX_PATH, CDEV_CUR_STATE, cdev_id);
			snprintf(val_buf, UINT_BUF_MAX, "%d", offline);
			ret = write_to_file(node_buf, val_buf, strlen(val_buf));
			if (ret >= 0)
				ret = 0;
		} else if (hotplug_ktm_supported) {
...
int venus_request(int level)
{
	...
	if (!venus_init_done) {
		...
		if (!ret) {
			if (snprintf(venus_sysfs_node, MAX_PATH, "%s%s", temp,
				VENUS_MITIGATION_NODE) < 0)
				msg("%s: node truncated", __func__);
		} else {
...
RETURN VALUE
	0 on success, -1 on failure.
===========================================================================*/
int hvx_request(int level, enum hvx_dev_type dev, int cdev_id)
{
	int ret = -1;
	...
	if ((unsigned int)level != cur_hvx[dev]) {
		if (cdev_id != -1) {
			if (snprintf(node_buf, MAX_PATH, CDEV_CUR_STATE, cdev_id) < 0)
				msg("%s: truncation error\n", __func__);
			if (snprintf(buf, UINT_BUF_MAX, "%d", level) < 0)
				msg("%s: truncation error\n", __func__);
			if (write_to_file(node_buf, buf, strlen(buf)) > 0)
				ret = 0;
		} else {
#ifdef ENABLE_HVX_FASTRPC_MITIGATION
			if (dev == HVX_HW_DEV)
				...
				ret = sysmonhvxthrottle_update_thermal_enabled(
					...
					HVX_THERM_CLT_ID);
#else
			msg("CDSP/HVX mitigation is not supported\n");
			ret = -1;
#endif /* ENABLE_HVX_FASTRPC_MITIGATION */
		}
