...
using ILocHidlFlpServiceV1_1 = ::vendor::qti::gnss::V1_1::ILocHidlFlpService;
using ILocHidlFlpServiceV2_1 = ::vendor::qti::gnss::V2_1::ILocHidlFlpService;
...
static jmethodID sOnServiceDeath = NULL;
...
static void FlpServiceProvider_class_init(JNIEnv* env, jclass clazz)
{
    ...
    sOnBatchingStatusChange = env->GetMethodID(clazz,
                                         "onBatchingStatusCb",
                                         "(I[I)V");
    sOnServiceDeath = env->GetMethodID(clazz, "reportFLPServiceDied", "()V");
    getFlpServiceIface();
    registerDeathRecipient("FlpService", []() {
            gnssFlpServiceProviderIface = nullptr;
            ALOGE("ILocHidlFlpService died");
            FlpServiceProvider_init(nullptr, nullptr);
            JNIEnv* env = android::AndroidRuntime::getJNIEnv();
            env->CallVoidMethod(sCallbacksObj, sOnServiceDeath);
    });
}
                                                ...
                                                jlong tbm_ms)
{
    ...
    // Invoke with power mode for 2.1
    sp<ILocHidlFlpServiceV2_1> iface_2_1 = ILocHidlFlpServiceV2_1::castFrom(iface);
    int32_t result = -1;
    if (iface_2_1 != nullptr) {
        TO_HIDL_SERVICE();
        auto r = iface_2_1->startFlpSession_2_1(
                (int32_t)id,
                (uint32_t)flags,
                (int64_t)period_ns,
                (int32_t)distance_ms,
                                                 ...
                                                 jlong tbm_ms)
{
    ...
    if (iface_2_1 != nullptr) {
        TO_HIDL_SERVICE();
        auto r = iface_2_1->updateFlpSession_2_1(
                (int32_t)id,
                (uint32_t)flags,
                (int64_t)period_ns,
                (int32_t)distance_ms,
                                                  ...
                                                  jboolean enabled)
{
    ...
    sp<ILocHidlFlpServiceV1_1> iface = ILocHidlFlpServiceV1_1::castFrom(getFlpServiceIface());
    if (iface == nullptr) {
        ALOGE("FLP Iface NULL !");
        return;
    }
