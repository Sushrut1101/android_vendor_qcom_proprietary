      ...
      if(line_state == 1)
      {
        if(!rmnet_state_config->dtr_enabled && !is_ssr_in_progress)
        {
              ...
              is_fusion_target)
          {
            ...
            {
              LOG_MSG_INFO1("QTI:DFS ind registration succeeded", 0, 0, 0);
            }
          }
          else
          {
...
----------------------------------------------------------------------------*/
        ...
        {
          LOG_MSG_INFO1("will be handle later, DTR status:%d & is_ssr_in_progress:%d ",
                      rmnet_state_config->dtr_enabled, is_ssr_in_progress, 0);
        }
      }
/*----------------------------------------------------------------------------
...
)
{
 ...
 /*-------------------------------------------------------------------------*/
  ...
  ph_rx_buf = (char *)malloc(RMNET_MAX_TRANSFER_SIZE);
  if(ph_rx_buf == NULL)
  {
    LOG_MSG_ERROR("No memory to allocate buffer to peripheral", 0, 0, 0);
    return QTI_FAILURE;
  }
  memset(ph_rx_buf, 0, RMNET_MAX_TRANSFER_SIZE);
...
----------------------------------------------------------------------------*/
    ...
    {
      /*for new fusions also let it go through this path*/
...
/*-------------------------------------------------------------------------*/
  ...
  if ((DS_TARGET_WARLOCK != rmnet_state_config->target) ||
...
/*------------------------------------------------------------------------*/
  ...
  memset(&(rmnet_state_config->data_buf_info), 0, sizeof(rmnet_state_config->data_buf_info));

  LOG_MSG_INFO1_6("ul fifo size %d, dl fifo size %d, dl buf size %d \n",
...
--------------------------------------------------------------------------*/
  while (retry_count < QTI_USB_RETRY)
  {
    if ( rmnet_state_config->ph_iface[PH_DRIVER_TYPE_USB].ph_enabled )
    {
      ...
      ret_val = ioctl(ph_iface_fd,
                      FRMNET_CTRL_DATA_BUF_INFO_LOOKUP,
                      &(rmnet_state_config->data_buf_info));
  ...
  snprintf(command, sizeof(command), "echo QTI Peripheral type %d, peripheral id %d, ipa consumer EP %d, ipa producer EP %d > /dev/kmsg",
  ...
  LOG_MSG_INFO1_6("Peripheral ul fifo size %d, dl fifo size %d, dl buf size %d \n",
                  rmnet_state_config->data_buf_info.epout_total_buf_len,
                  rmnet_state_config->data_buf_info.epin_total_buf_len,
                  rmnet_state_config->data_buf_info.epin_buf_len,0,0,0);
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    if(line_state == 1)
    {
      ...
      {
        if(DS_TARGET_FUSION4_5_PCIE == dpl_state_config->target ||
           DS_TARGET_OFF_TARGET == dpl_state_config->target ||
           DS_TARGET_FUSION_8084_9X45 == dpl_state_config->target ||
           DS_TARGET_FUSION_8096_9X55 == dpl_state_config->target ||
           is_fusion_target)
        {
          if (is_fusion_sw_path)
          {
        ...
        else
        {
          ...
          /*DPM port open for DPL and DPM port close for rmnet, which sequence causing the device
          crash. Hence added a sleep*/
          sleep(ONE_SEC);
        }
...
)
{
 ...
 /*-------------------------------------------------------------------------*/
  ...
  if(ph_rx_buf == NULL)
  {
    LOG_MSG_ERROR("No memory to allocate buffer to DPL", 0, 0, 0);
    return QTI_FAILURE;
  }
  memset(ph_rx_buf, 0, RMNET_MAX_TRANSFER_SIZE);
