...
QOMXImageCodec::QOMXImageCodec()
{
  ...
  m_callbacks = NULL;
  memset(&m_inputScaleInfo, 0x00, sizeof(m_inputScaleInfo));
  memset(&m_outputScaleInfo, 0x00, sizeof(m_outputScaleInfo));
  memset(&m_rotation, 0x00, sizeof(m_rotation));
  SetDynamicLogging("1");
}
...
{
  QIDBG_MED("%s:%d] From Current State: %d To New state: %d",
  ...
  case OMX_StateLoaded: {
    if (OMX_StateIdle == m_state) {
      ...
      if (!m_inPort->bEnabled && !m_outPort->bEnabled) {
        ...
        m_state = aState;
        QIDBG_MED("%s: State change from OMX_StateIdle to OMX_StateLoaded",
          __func__);
        sem_post(&m_cmdLock);
        ...
        QIDBG_MED("%s:%d] State change from OMX_StateIdle to OMX_StateLoaded "
      ...
      m_state = aState;
      QIDBG_MED("%s:%d] State change from OMX_StateWaitForResources to "
  ...
  case OMX_StateIdle: {
    if (OMX_StateLoaded == m_state) {
      ...
      if (!m_inPort->bEnabled && !m_outPort->bEnabled) {
        m_state = aState;
        QIDBG_MED("%s:%d] State change from OMX_StateLoaded to "
        ...
        if (m_dataAllocated) {
          ...
          QIDBG_MED("%s:%d] State  change incomplete.. Moving to "
      ...
      if (!m_inPort->bEnabled && !m_outPort->bEnabled) {
        m_state = aState;
        QIDBG_MED("%s:%d] Ports disabled: State change from "
        ...
        QIDBG_MED("%s:%d] Set abort flag to true", __func__, __LINE__);
        pthread_mutex_unlock(&m_abortlock);
        abortExecution();
        m_state = aState;
        m_compTransState = OMX_StateNone;
        QIDBG_MED("%s: %d] State change from OMX_StateExecuting to "
            "OMX_StateIdle", __func__, __LINE__);
        sem_post(&m_cmdLock);
        m_callbacks->EventHandler(m_compHandle, m_appData,
            OMX_EventCmdComplete, OMX_CommandStateSet, aState, NULL);
  ...
  case OMX_StateExecuting: {
    if (OMX_StateIdle == m_state) {
      ...
      */
      if (!m_inPort->bEnabled && !m_outPort->bEnabled) {
        m_state = aState;
        QIDBG_MED("%s:%d] State change from OMX_StateIdle to "
  ...
  case OMX_StateWaitForResources: {
    if (OMX_StateLoaded == m_state) {
      m_state = aState;
      QIDBG_MED("%s:%d] transition from OMX_StateLoaded to "
...
* thread
==============================================================================*/
OMX_ERRORTYPE QOMXImageCodec::postMessage(QIQueueObject *a_Message)
{
  ...
  OMX_IN OMX_PTR cmdData __unused)
{
  ...
  case OMX_CommandStateSet: {
    ...
    l_message->iData = (int32_t)param1;
    break;
  }
...
==============================================================================*/
void QOMXImageCodec::fillBufferDone(OMX_BUFFERHEADERTYPE *abufferHdr __unused)
{
  QIDBG_MED("%s %d]", __func__, __LINE__);
