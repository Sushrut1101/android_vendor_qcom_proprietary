/*============================================================================
  @file LinearAcceleration.cpp

  @brief
  LinearAcceleration class implementation.
...
============================================================================*/
LinearAcceleration::LinearAcceleration(int handle)
    :SAMSensor(handle)
{
    if (getAttribOK() == true) {
        ...
        setName("Linear Acceleration");
        setVendor("QTI");
        setType(SENSOR_TYPE_LINEAR_ACCELERATION);

        if(handle == HANDLE_LINEAR_ACCEL_WAKE_UP){
            bWakeUp = true;
            setName("Linear Acceleration -Wakeup");
            setFlags(SENSOR_FLAG_CONTINUOUS_MODE|SENSOR_FLAG_WAKE_UP);
        } else if (handle == HANDLE_LINEAR_ACCEL) {
            bWakeUp = false;
...
/*===========================================================================
  setSensorInfo
===========================================================================*/
void LinearAcceleration::setSensorInfo()
{
...
/*============================================================================
  enable
============================================================================*/
int LinearAcceleration::enable(int en)
{
    int err;
    ...
    if (enabled == en) {
        HAL_LOG_INFO("LinearAcceleration is already enabled/disabled %d", enabled);
        return 0;
    }
...
int LinearAcceleration::sendBatchReq()
{
    sensor1_error_e  error;
    sensor1_msg_header_s req_hdr;
...
===========================================================================*/
void LinearAcceleration::processResp(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
...
/*===========================================================================
  FUNCTION:  processInd
===========================================================================*/
void LinearAcceleration::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if(bWakeUp == false) {
            ...
            la_sample.sensor = HANDLE_LINEAR_ACCEL_WAKE_UP;
            HAL_LOG_VERBOSE("%s:sensor %s (wake_up)",__FUNCTION__,
                            Utility::SensorTypeToSensorString(getType()));
        }

        la_sample.acceleration.x = curr_sample->data[5];
        la_sample.acceleration.y = curr_sample->data[4];
        la_sample.acceleration.z = -curr_sample->data[6];
        ...
        HAL_LOG_VERBOSE("%s: LinearAcceleration: X: %f Y: %f Z: %f acc: %d SAM TS: %u HAL TS:%" PRId64 " elapsedRealtimeNano:%" PRId64 "",
