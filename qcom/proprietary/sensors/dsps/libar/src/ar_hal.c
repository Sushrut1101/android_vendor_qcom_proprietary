...
#ifdef WEAR_SUPPORT
#define HANDLE_HIKE 5
#define HANDLE_INACTIVE 6
#define HANDLE_WALK_STEP_RATE 7
#define HANDLE_JOG_STEP_RATE 8
#define HANDLE_RUN_STEP_RATE 9
#endif
...
/* Helper functions */
int hal_get_handle_for_cmc_motionstate( uint32_t state);
...
#ifdef WEAR_SUPPORT
/* List of supported activities */
const char *g_activity_list_v3[MAX_NUM_ACTIVITIES] = {
    "android.activity_recognition.in_vehicle",
    "android.activity_recognition.on_bicycle",
    "android.activity_recognition.walking",
    "android.activity_recognition.running",
    "android.activity_recognition.still",
    "com.activity_recognition.hiking",
    "com.activity_recognition.inactive",
    "com.activity_recognition.walkrate",
    "com.activity_recognition.jograte",
    "com.activity_recognition.runrate",
};
#endif
...
/*!
*/
static void hal_sam_send_cancel( hal_ar_ctl_t* ar_ctl, uint32_t svc_num )
{
    ...
    if( SENSOR1_SUCCESS != error ) {
        ...
        if( SENSOR1_SUCCESS !=
            (error = sensor1_write( ar_ctl->hndl, &req_hdr, cancel_msg )) ) {
            sensor1_free_msg_buf( ar_ctl->hndl, cancel_msg );
            HAL_LOG_ERROR( "%s: sensor1_write() error: %d", __FUNCTION__, error );
        }

        /* waiting for response */
        if (hal_wait_for_response( CANCEL_TIME_OUT_MS,
                                   &ar_ctl->cb_mutex,
                                   &ar_ctl->cb_arrived_cond,
                                   &g_ar_control->is_resp_arrived ) == false ) {
            HAL_LOG_ERROR("%s: ERROR: No reponse from CMC cancel request %d",
                           __FUNCTION__, SNS_SAM_CMC_CANCEL_REQ_V02);
            return;
        }
        HAL_LOG_DEBUG("%s: Received CMC cancel response: %d", __FUNCTION__, ar_ctl->error);
    }
}
...
static bool hal_sam_cmc_send_enable_req( hal_ar_ctl_t* ar_ctl )
{
    ...
    hal_sam_send_cancel( ar_ctl, CMC_SVC_NUM );
    ...
    sam_req->enabled_motion_states_bit_mask_valid = true;
    sam_req->enabled_motion_states_bit_mask =
            ( 1 << SNS_SAM_CMC_MS_STATIONARY_V02) |
            ( 1 << SNS_SAM_CMC_MS_MOVE_V02 ) |
            ( 1 << SNS_SAM_CMC_MS_FIDDLE_V02 ) |
            ( 1 << SNS_SAM_CMC_MS_PEDESTRIAN_V02 ) |
            ( 1 << SNS_SAM_CMC_MS_VEHICLE_V02 ) |
            ( 1 << SNS_SAM_CMC_MS_WALK_V02 ) |
            ( 1 << SNS_SAM_CMC_MS_BIKE_V02 ) |
            ( 1 <<  SNS_SAM_CMC_MS_HIKE_V02 ) |
            ( 1 << SNS_SAM_CMC_MS_INACTIVE_V02 ) |
            ( 1 << SNS_SAM_CMC_MS_WALK_STEP_RATE_V02 ) |
            ( 1 << SNS_SAM_CMC_MS_JOG_STEP_RATE_V02 ) |
            ( 1 << SNS_SAM_CMC_MS_RUN_STEP_RATE_V02 );
...
#ifdef WEAR_SUPPORT
        case HANDLE_HIKE:
            handle = SNS_SAM_CMC_MS_HIKE_V02;
            break;
        case HANDLE_INACTIVE:
            handle = SNS_SAM_CMC_MS_INACTIVE_V02;
            break;
        case HANDLE_WALK_STEP_RATE:
            handle = SNS_SAM_CMC_MS_WALK_STEP_RATE_V02;
            break;
        case HANDLE_JOG_STEP_RATE:
            handle = SNS_SAM_CMC_MS_JOG_STEP_RATE_V02;
            break;
        case HANDLE_RUN_STEP_RATE:
            handle = SNS_SAM_CMC_MS_RUN_STEP_RATE_V02;
            break;
#endif
        default:
...
static void hal_process_sam_cmc_ind( sensor1_msg_header_s *msg_hdr, void *msg_ptr )
{
    ...
    if (SNS_SAM_CMC_REPORT_IND_V02 == msg_hdr->msg_id) {
        ...
        if (NULL == sample_list) {
            ...
            HAL_LOG_DEBUG("%s(Line%d): ms state=%d ms event=%d timestamp=%d",
                          __FUNCTION__, __LINE__, sam_ind->report_data.motion_state,
                          sam_ind->ms_event, sam_ind->timestamp);
            int handle = -1;
            handle = hal_get_handle_for_cmc_motionstate((uint32_t)sam_ind->report_data.motion_state);
            if(handle == -1) {
                HAL_LOG_ERROR( "%s:unsupported motion state(%d) for CMC ind reply",
                                __FUNCTION__, sam_ind->report_data.motion_state);
            }
            if((handle != -1 &&
                       sam_ind->ms_event == SNS_SAM_CMC_MS_EVENT_ENTER_V02 &&
                       g_ar_control->hal_track_activity[handle].enter == true) ||
                      (handle != -1 &&
                       sam_ind->ms_event == SNS_SAM_CMC_MS_EVENT_EXIT_V02 &&
                       g_ar_control->hal_track_activity[handle].exit == true)) {
                    count = 1;
        ...
        if (NULL == sample_list) {
            ...
            for(i = 0; i < sam_ind->items_len; i++) {
                HAL_LOG_DEBUG("%s(Line%d): ms state=%d ms event=%d timestamp=%d",
                              __FUNCTION__, __LINE__, sam_ind->items[ i ].report.motion_state,
                              sam_ind->items[ i ].ms_event, sam_ind->items[ i ].timestamp);
                int handle = -1;
                handle = hal_get_handle_for_cmc_motionstate((uint32_t)sam_ind->items[ i ].report.motion_state);
                if(handle == -1) {
                    HAL_LOG_ERROR( "%s:unsupported motion state(%d) for CMC batch ind reply",
    ...
    for (i = 0; i < count; i++) {
        ...
        /*curr_sample->data[0] is mapped to motion state*/
        activity    = hal_get_handle_for_cmc_motionstate(curr_sample->data[0]);
        if(activity != -1) {
            activity_data[i].activity = activity;
        } else {
             HAL_LOG_ERROR("%s: Unknown Activity/reported motion state : %d",
                    __FUNCTION__, curr_sample->data[0]);
             continue;
        }
                                                   ...
                                                   int64_t max_batch_report_latency_ns )
{
    ...
    /* Always cancel first, just in case */
    hal_sam_send_cancel( ar_ctl, TILT_DETECTOR_SVC_NUM );
...
/*===========================================================================
  FUNCTION: hal_get_handle_for_cmc_motionstate

    this function returns the handle for a given motion state
    returns -1 if motion state is not correct
===========================================================================*/

int hal_get_handle_for_cmc_motionstate( uint32_t state)
{
    HAL_LOG_INFO("%s: state %u", __FUNCTION__, state);

    switch ( state ) {
        case SNS_SAM_CMC_MS_VEHICLE_V02:
            return HANDLE_IN_VEHICLE;

        case SNS_SAM_CMC_MS_BIKE_V02:
            return HANDLE_ON_BICYCLE;

        case SNS_SAM_CMC_MS_WALK_V02:
            return HANDLE_WALKING;
...
#ifdef WEAR_SUPPORT
        case SNS_SAM_CMC_MS_HIKE_V02:
            return HANDLE_HIKE;

        case SNS_SAM_CMC_MS_INACTIVE_V02:
             return HANDLE_INACTIVE;

        case SNS_SAM_CMC_MS_WALK_STEP_RATE_V02:
            return HANDLE_WALK_STEP_RATE;

        case SNS_SAM_CMC_MS_JOG_STEP_RATE_V02:
            return HANDLE_JOG_STEP_RATE;

        case SNS_SAM_CMC_MS_RUN_STEP_RATE_V02:
            return HANDLE_RUN_STEP_RATE;
#endif
        default :
            HAL_LOG_ERROR("%s: improper motion state %d", __FUNCTION__, state);
            return -1;
    }
}
                                                ...
                                                char const* const** activity_list)
{
    ...
    if( ar_list_len != 0 ) {
        ...
        *activity_list = g_activity_list;
        ...
        /*8909W supports new CMC version start handing it*/
#ifdef WEAR_SUPPORT
        if( g_ar_control->cmc_version == 3 ) {
            /* Hard coding in the initial implementation as SAM doesn't support */
            /* Need to switch to algo attrib req once SAM supports */
            *activity_list = g_activity_list_v3;
            ar_list_len = ARRAY_SIZE(g_activity_list_v3);
        }
#endif
    }
    ...
    for( i = 0; i < ar_list_len; i++ ) {
        if(g_ar_control->cmc_version == 3)
#ifdef WEAR_SUPPORT
            HAL_LOG_VERBOSE( "%s: handle:%d activity:%s", __FUNCTION__, i, g_activity_list_v3[i] );
        else
#endif
