...
#define MIN(a, b) ((a) < (b) ? (a) : (b))
    ...
    OperationStatus LogTxtParser::LogParserInit(const char* stringConversionPath, CpuType cpuType,
        const module_level_info* modulesInfo)
    {
        SetStates();
        ...
        */
        ...
        if (!ReadStringsConversionFile(converionFileName.str().c_str(), str_sz, extra_space))
        {
        ...
        snprintf(m_stringsConversionBuffer.data() + str_sz, extra_space, "%s", CORRUPTED_PARAM_MARK);
        ReadModuleNames();
    ...
    bool LogTxtParser::ReadStringsConversionFile(const char* name, size_t& outRealSize, size_t extra)
    {
        outRealSize = 0;
        ...
        m_stringsConversionBuffer.clear();
        m_stringsConversionBuffer.resize(outRealSize + extra);
        strings_file.seekg(0, strings_file.beg);
        strings_file.read(m_stringsConversionBuffer.data(), size);
        strings_file.close();
        return true;
    }
    ...
    {
        if (stringOffset >= str_sz || m_stringsConversionBuffer.empty())
        {
            std::stringstream invalidSs;
            invalidSs << timeStamp << " >>>> Invalid string offset: " << stringOffset << std::endl;
            return invalidSs.str();
        }
        const char *fmt = m_stringsConversionBuffer.data() + stringOffset;
    ...
    {
        const char* mod = m_stringsConversionBuffer.data();
...
#define S(x)((x) - 'A')
    ...
    bool LogTxtParser::pop_arg(int type, const std::vector<int> & params, unsigned int &lastParamIndex, arg_t &newArg)
    {
        ...
        case PTR:
            ...
            newArg.p = m_stringsConversionBuffer.data() +
