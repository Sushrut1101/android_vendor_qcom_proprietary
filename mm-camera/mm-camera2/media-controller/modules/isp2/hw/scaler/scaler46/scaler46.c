...
#define ROUNDING_PDAF_OFFSET(X,Y) (\
ceil((X-(Y-1.0))/Y)*Y)
...
static boolean scaler46_get_module_enable_mask(scaler46_t *scaler,
 ...
 * Return: nothing
 **/
static void scaler46_cmd_debug(ISP_ScaleCfgCmdType* cmd, uint8_t index)
{
  ISP_DBG(" scaler[%d](%d-Enc/%d-View/%d-Video/%d-Ideal)", index,
  ...
  ISP_DBG(" scaler[%d] Y horiz_interp_Resolution = %d",
     index, cmd->Y_ScaleCfg.horizInterResolution);
  ISP_DBG(" scaler[%d] Y horiz_phase_Multipli_Factor = %u",
     index, cmd->Y_ScaleCfg.horizPhaseMult);
  ISP_DBG(" scaler[%d] Y vert_interp_Resolution = %d",
     index, cmd->Y_ScaleCfg.vertInterResolution);
  ISP_DBG(" scaler[%d] Y vert_phase_Multipli_Factor = %u",
  ...
  ISP_DBG(" scaler[%d] Y scale_YInWidth = %d",
    index, cmd->Y_ScaleCfg.scaleYInWidth);
  ISP_DBG(" scaler[%d] Y horiz_SkipCount = %d",
    index, cmd->Y_ScaleCfg.hSkipCount);
  ISP_HIGH(" scaler[%d] Y right_pad_en = %d",
    index, cmd->Y_ScaleCfg.rightPadEnable);
  ISP_DBG(" scaler[%d] Y scale_YInHeight = %d",
    index, cmd->Y_ScaleCfg.scaleYInHeight);
  ISP_HIGH("scaler[%d] Y vert_SkipCount = %d",
    index, cmd->Y_ScaleCfg.vSkipCount);
  ISP_HIGH("scaler[%d] Y bottom_pad_en = %d",
    index, cmd->Y_ScaleCfg.bottomPadEnable);
  ...
  ISP_DBG(" scaler[%d] CbCr horiz_interp_Resolution = %d",
     index, cmd->CbCr_ScaleCfg.horizInterResolution);
  ISP_DBG(" scaler[%d] CbCr horiz_phase_Multipli_Factor = %u",
     index, cmd->CbCr_ScaleCfg.horizPhaseMult);
  ISP_DBG(" scaler[%d] CbCr vert_interp_Resolution = %d",
     index, cmd->CbCr_ScaleCfg.vertInterResolution );
  ISP_DBG(" scaler[%d] CbCr vert_phase_Multipli_Factor = %u",
...
/** scaler46_handle_set_chromatix_ptr
 *
...
/** calculate_scaler46_factor
 *    @scale_factor_horiz: horizontal scale factor
 *    @scale_factor_vert:  vertical scale factor
 *    @hFactor: output
 *    @vFactor: output
 *
 * calculate scaler46 factor
 *
 * Return: TRUE
 **/
static boolean calculate_scaler46_factor(int scale_factor_horiz,
  int scale_factor_vert, uint32_t *h_inter_resol, uint32_t *v_inter_resol)
{
  ...
  if (scale_factor_horiz >= 1 && scale_factor_horiz < 4) {
    ...
    *h_inter_resol = 1;
             ...
             ISP_SCALER46_HW_SCALER_LIMIT) {
    *h_inter_resol = 0;
  } else {
    ISP_DBG("scale_factor_vert %d is greater than max supported ratio %d",
      scale_factor_vert, ISP_SCALER46_HW_SCALER_LIMIT);
    return FALSE;
  }
...
/** scaler46_is_stripe_offset_on_rollover_point
 *
 * DESCRIPTION:
 *
 **/
boolean scaler46_is_stripe_offset_rollover_point(
 ...
 **/
void scale46_fetch_engine_adjust(scaler46_t *scaler, uint32_t *stripe_offset)
{
...
/** scaler46_compute_stripe_offset
 *    @scaler: scaler module control struct
 ...
 **/
static int scaler46_compute_stripe_offset(scaler46_t *scaler,
  isp_stripe_request_t *stripe_request_info)
{
  ...
  right_stripe = split_point - limits->min_overlap_left -
       (limits->min_left_split - limits->smallest_split_point);
  ...
  stripe_offset = split_point - limits->min_overlap_left -
       (limits->min_left_split - limits->smallest_split_point);
  ...
  /* Adjust the stripe offset for rolloff grid line - 36 requirement
     only adjust if rolloff has posted limitation. */
  ...
  if (stripe_request_info->offline_mode) {
    scale46_fetch_engine_adjust(scaler, &stripe_offset);
  }

  stripe_offset = FLOOR2(stripe_offset);

   /* For PDAF enable, adjust the stripe offset to meet the pdaf
     offset requirement and block width */
  if(limits->pdaf_enable
     && limits->pdaf_blockwidth !=0 ){
     stripe_offset =
        ROUNDING_PDAF_OFFSET(stripe_offset -limits->pdaf_hoffset,
        limits->pdaf_blockwidth) +limits->pdaf_hoffset;
   }

  if (scaler->is_zzhdr_mode){
      ISP_DBG("zzhdr mode offset to 4");
      stripe_offset = FLOOR4(stripe_offset);
      ISP_DBG("Offset for zzhdr floor of 4 %d", stripe_offset);
  }


  /*scaler 46 dont have stripe offset limitation
  ...
  stripe_request_info->ispif_out_info.split_point =
    ((limits->min_left_split - limits->smallest_split_point) / 2) +
 ...
 * Return: 0 - success and negative value - failure
 **/
static int scaler46_trigger_enable(scaler46_t *scaler,
 ...
 * Return: 0 - success and negative value - failure
 **/
static int scaler46_enable(scaler46_t *scaler, isp_mod_set_enable_t *enable,
...
/** scaler46_calculate_zoom_scaling
 ...
 * Return: nothing
 **/
static void scaler46_calculate_zoom_scaling(scaler46_t *scaler,
 ...
 * Return: nothing
 **/
static void scaler46_check_hw_limit(scaler46_t *scaler,
  ISP_Y_ScaleCfgCmdType* y_cfg, int in_aspect_ratio)
{
  ...
  if (scaler->rtb_mode){
    ISP_HIGH("RTB mode is set can not downscale\n");
    scaler->request_crop = 1;
    return;
  }
  if (scale_factor > scaler->max_scale_factor &&
  ...
  if ((y_cfg->hOut * ISP_SCALER46_MAX_SCALER_FACTOR) < y_cfg->hIn) {
    ISP_ERR(" hOut / hIn < 1/%d",ISP_SCALER46_MAX_SCALER_FACTOR);
    y_cfg->hOut = (y_cfg->hIn + ISP_SCALER46_MAX_SCALER_FACTOR -1) /
      ISP_SCALER46_MAX_SCALER_FACTOR;
    y_cfg->vOut = y_cfg->hOut * Q12 / in_aspect_ratio;
  }

  if ((y_cfg->vOut * ISP_SCALER46_MAX_SCALER_FACTOR) < y_cfg->vIn) {
    ISP_ERR(" vOut / vIn < 1/%d", ISP_SCALER46_MAX_SCALER_FACTOR);
    y_cfg->vOut = (y_cfg->vIn + ISP_SCALER46_MAX_SCALER_FACTOR -1)/
      ISP_SCALER46_MAX_SCALER_FACTOR;
  ...
  if (down_scaling_factor < SCALE_RATIO_LIMIT) {
    if (scaler->crop_factor_applied != Q12) {
 ...
 * Return: None
 **/
static void scaler46_get_camif_sizes(scaler46_t *scaler, int32_t *camif_w,
    int32_t *camif_h)
{
...
void scaler46_calculate_dis(scaler46_t *scaler)
{
  ...
  if (scaler->dis_enable) {
    scaler46_get_camif_sizes(scaler, &camif_w, &camif_h);
    if(scaler->hw_stream_info.width > camif_w) {
      ISP_DBG(" over camif limit!!!");
  ...
  uint32_t *h_skip_cnt)
{
  ...
  *phase_init = (*mn_init << (INTERP_SHIFT + interp_reso)) / M;
}
...
boolean scaler46_config(scaler46_t *scaler)
{
  ...
  isp_scaler46_entry_t*     entry    = &scaler->scalers;
  ...
  ISP_HIGH("y_cfg->hOut %d y_cfg->vOut %d", y_cfg->hOut, y_cfg->vOut);
  /* In RTB mode, RTB algo need 1x frames even we zoom,
   * to support this feature ISP will disable zooming feature
   */
  if (!scaler->rtb_mode){
     scaler46_calculate_zoom_scaling(scaler, y_cfg, scaler->crop_factor_applied);
  }
  scaler46_check_hw_limit(scaler, y_cfg, input_ratio);
  ...
  rc = calculate_scaler46_factor(y_scale_factor_horiz,
    y_scale_factor_vert, &y_hFactor, &y_vFactor);
  if (rc == FALSE) {
    ISP_ERR("Can not set luma scale factor");
    return rc;
  }

  y_cfg->hSkipCount = h_skip_cnt;

  phase_increment = INTERP_SHIFT + y_hFactor;
  y_cfg->horizInterResolution = y_hFactor;
  y_cfg->horizPhaseMult = (y_cfg->hIn << phase_increment) / y_cfg->hOut;
  y_cfg->horizMNInit = 0;
  y_cfg->horizPhaseInit = 0;

  phase_increment = INTERP_SHIFT + y_vFactor;
  y_cfg->vertInterResolution = y_vFactor;
  y_cfg->vertPhaseMult = (y_cfg->vIn << phase_increment) / y_cfg->vOut;
  y_cfg->vertMNInit = 0;
  y_cfg->vertPhaseInit = 0;

  y_cfg->scaleYInWidth    = y_cfg->hIn;
  y_cfg->scaleYInHeight   = y_cfg->vIn;
  y_cfg->hSkipCount       = 0;
  y_cfg->vSkipCount       = 0;
  y_cfg->rightPadEnable   = 0;
  y_cfg->bottomPadEnable  = 0;
  ...
  rc = calculate_scaler46_factor(cbcr_scale_factor_horiz,
    cbcr_scale_factor_vert, &cbcr_hFactor, &cbcr_vFactor);
  if (rc == FALSE) {
    ISP_ERR("Can not set CbCr scale factor");
    return rc;
  }

  phase_increment = INTERP_SHIFT + cbcr_hFactor;
  ...
  phase_increment = INTERP_SHIFT + cbcr_vFactor;
  ...
  y_cfg->rightPadEnable   = 1;
  y_cfg->bottomPadEnable  = 1;
 ...
 **/
static int scaler46_update_zoom_params(scaler46_t *scaler,
  ...
  if (!scaler->session_based_identity) {
    ...
    for (j = 0; j < ISP_MAX_STREAMS; j++) {
      if (scaler->identity[j] &&
        ...
        for (i = 0; i < ISP_MAX_STREAMS; i++) {
          zoom_params = &upd_zoom_params[i];
          if ((zoom_params->identity) && (zoom_params->identity != scaler->identity[j])) {
            continue;
          }
          ...
          ISP_DBG("zoom_debug::i %d\
            ide %x scale %f out %d %d\n",
            i, identity,
            zoom_params->scaling_ratio, zoom_params->scaler_output.width,
            zoom_params->scaler_output.height);
          break;
...
/** scaler46_do_hw_update
 * @scaler: scaler module struct data
 * @hw_update_list: linked list ent from pipeline layer which
 *      scaler46 will populate with register address and data info
 ...
 * Return: nothing
 **/
static boolean scaler46_do_hw_update(scaler46_t *scaler,
  isp_sub_module_output_t *output, uint32_t identity)
{
  ...
  if (scaler->update_enable_bit == TRUE) {
    scaler46_get_module_enable_mask(scaler,
  ...
  if (scaler->scalers.hw_update_pending == TRUE) {
    scaler46_cmd_debug(&scaler->scalers.reg_cmd, scaler->entry_idx);

    reg_cmd = (ISP_ScaleCfgCmdType *)CAM_MALLOC(sizeof(ISP_ScaleCfgCmdType));
    if (!reg_cmd) {
    ...
    /* Values of hIn,hOut, vIn and vOut are decreased by 1  *
     * because, for this values if we write n means n+1     *
     */
    reg_cmd->Y_ScaleCfg.hIn      -= 1;
    reg_cmd->Y_ScaleCfg.hOut     -= 1;
    reg_cmd->Y_ScaleCfg.vIn      -= 1;
    reg_cmd->Y_ScaleCfg.vOut     -= 1;
    reg_cmd->CbCr_ScaleCfg.hIn   -= 1;
    reg_cmd->CbCr_ScaleCfg.hOut  -= 1;
    reg_cmd->CbCr_ScaleCfg.vIn   -= 1;
    reg_cmd->CbCr_ScaleCfg.vOut  -= 1;
    ...
    reg_cfg_cmd = (struct msm_vfe_reg_cfg_cmd *)
      CAM_MALLOC(sizeof(struct msm_vfe_reg_cfg_cmd) * 2);
    if (!reg_cfg_cmd) {
    ...
    reg_cfg_cmd[0].u.rw_info.reg_offset = scaler->reg_off;
    reg_cfg_cmd[0].u.rw_info.len = scaler->reg_len * sizeof(uint32_t);
    ...
    ret = scaler46_util_append_cfg(&output->hw_update_list, hw_update);
    if (ret == FALSE) {
  ...
  output->stats_params->zoom = scaler->zoom_value;
  ISP_DBG(" user zoom %d stream type %d", scaler->zoom_value.user_zoom, scaler->stream_type);

  rc = scaler46_update_zoom_params(scaler, output->stats_params->zoom_params,
    identity);
  if (rc < 0) {
    ISP_ERR("failed: scaler46_update_zoom_params");
    return FALSE;
  }
  ...
  if (output->frame_meta) {
    ...
    ISP_HIGH(" frame_meta: crop window: [left, top, Width, Height]= [%d, %d, %d, %d]",
      scaler->applied_crop_window.left,
      scaler->applied_crop_window.top,
      scaler->applied_crop_window.width,
      scaler->applied_crop_window.height);
      output->frame_meta->zoom_value  = scaler->zoom_value;
      output->frame_meta->fovc_zoom  = scaler->fovc_zoom_val;
  }
  return ret;
}

boolean scaler46_parm_crop_region(mct_module_t *module,
  scaler46_t *scaler, cam_crop_region_t *crop_window, uint32_t identity)
{
  boolean ret = TRUE;
  ...
  if ((tmp_crop_window.width  + tmp_crop_window.left) > camif_w ||
      (tmp_crop_window.height + tmp_crop_window.top) > camif_h) {
       ISP_ERR("Invalid Crop window left %d top %d w x h = %d x %d CAMIF wxh %d x %d",
         tmp_crop_window.left, tmp_crop_window.top, tmp_crop_window.width, tmp_crop_window.height,
         camif_w, camif_h);
       return TRUE;
  }
  ...
  scaler->crop_factor_applied = MIN(temp1, temp2);
  ...
  if (is_stream_on == TRUE) {
    ret = module_scaler46_pass_scale_factor(module, scaler, is_stream_on, identity);
  }
...
boolean scaler46_parm_zoom(mct_module_t *module,
  scaler46_t *scaler, cam_zoom_info_t *zoom_val,
  uint32_t identity, mct_event_type type)
{
  boolean ret = TRUE;
  boolean is_stream_on = FALSE;
  int temp_fovc = 0;
  ...
  ISP_DBG("uzoom %d zoom valid %d", zoom_val->user_zoom,
    scaler->zoom_value.is_stream_zoom_info_valid);

  if (!scaler->zoom_value.is_stream_zoom_info_valid) {
    ret = isp_zoom_get_crop_factor(&scaler->zoom, scaler->zoom_value.user_zoom,
      &scaler->crop_factor_applied);
    if (ret == FALSE) {
      ISP_ERR("failed: isp_zoom_get_crop_factor");
    }
    scaler->crop_factor_calculate_crop = scaler->crop_factor_applied;
  } else {
    for (i = 0; i < scaler->zoom_value.num_streams; i++) {
      /* For Dual camera cases specifically, we need to support per stream
       * zoom control. The limitation is that all different stream zoom are
       * expected on mapped on different HW stream. This today is specific
       * to preview/snapshot which are guaranteed to be mapped to different
       * HW stream */
      if (scaler->stream_type == zoom_val->stream_zoom_info[i].stream_type) {
        ret = isp_zoom_get_crop_factor(&scaler->zoom,
          scaler->zoom_value.stream_zoom_info[i].isp_zoom,
          &scaler->crop_factor_applied);
        if (ret == FALSE) {
          ISP_ERR("failed: isp_zoom_get_crop_factor");
        }
        ret = isp_zoom_get_crop_factor(&scaler->zoom,
          scaler->zoom_value.stream_zoom_info[i].stream_zoom,
          &scaler->crop_factor_calculate_crop);
        if (ret == FALSE) {
          ISP_ERR("failed: isp_zoom_get_crop_factor");
        }
        break;
  ...
  scaler->crop_factor_applied += (uint32_t)((scaler->fovc_zoom_val *
    (scaler->crop_factor_applied/4096.0)) * Q12);
 ...
 * Return: nothing
 **/
void scaler46_reset(scaler46_t *scaler)
{
  ...
  /* In RTB mode isp zoom will disabled */
  scaler->rtb_mode = 0;
 ...
 * Return: 0 always
 **/
boolean scaler46_init(scaler46_t *scaler)
{
  scaler46_reset(scaler);
  scaler->crop_factor_applied = Q12;
  scaler->crop_factor_calculate_crop = Q12;
  scaler->fovc_zoom_val = 0.0f;
  return TRUE;
}

/** scaler46_get_module_enable_mask:
 *
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean scaler46_get_crop_factor(scaler46_t *scaler,
  isp_crop_factors_t *crop_factor)
{
  ...
  PTHREAD_MUTEX_LOCK(&scaler->mutex);
  crop_factor->crop_applied= scaler->crop_factor_applied;
  crop_factor->crop_to_calculate = scaler->crop_factor_calculate_crop;
  PTHREAD_MUTEX_UNLOCK(&scaler->mutex);

  return TRUE;
}

static uint32_t scaler46_calculate_right_split_width(uint32_t M, uint32_t N,
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean scaler46_set_max_scale_factor(scaler46_t *scaler)
{
...
static boolean scaler46_loop_saved_events(mct_module_t *module,
  scaler46_t *scaler, isp_saved_events_t *saved_events, uint32_t identity)
{
  ...
  if (saved_events->set_params[ISP_SET_PARM_DIS_ENABLE] &&
    ...
    hal_param =
      ...
      control_event_data;
    port_scaler46_handle_hal_set_parm(module, scaler, hal_param, identity);
  }
  ...
  if (saved_events->module_events[ISP_MODULE_EVENT_STATS_FOVC_MAGNIFICATION_FACTOR] &&
    (saved_events->module_events_valid[ISP_MODULE_EVENT_STATS_FOVC_MAGNIFICATION_FACTOR] == TRUE)) {
    port_scaler46_handle_lens_position_update(module,scaler, identity,
  ...
  if (saved_events->module_events[ISP_MODULE_EVENT_CDS_REQUEST] &&
    ...
    port_scaler46_handle_cds_request(scaler,
      saved_events->module_events[ISP_MODULE_EVENT_CDS_REQUEST]);
  }
  if (saved_events->set_params[ISP_SET_PARM_BOKEH_MODE] &&
    (saved_events->set_params_valid[ISP_SET_PARM_BOKEH_MODE] == TRUE)) {
    hal_param =
      saved_events->set_params[ISP_SET_PARM_BOKEH_MODE]->u.ctrl_event.
      control_event_data;
    port_scaler46_handle_hal_set_parm(module, scaler, hal_param, identity);
  }
  return TRUE;
...
/** scaler46_handle_isp_private_event:
 *
  ...
  scaler46_t *scaler, void *data, uint32_t identity)
{
  ...
  case ISP_PRIVATE_SET_MOD_ENABLE:
    rc = scaler46_enable(scaler,
      (isp_mod_set_enable_t *)private_event->data, private_event->data_size);
    if (rc < 0) {
      ISP_ERR("failed: scaler46_enable rc %d", rc);
      ret = FALSE;
    }
    break;
  ...
  case ISP_PRIVATE_REQUEST_STRIPE_OFFSET:
    rc = scaler46_compute_stripe_offset(scaler, private_event->data);
    if (rc < 0) {
      ISP_ERR("failed: scaler46_compute_stripe_offset rc %d", rc);
      ret = FALSE;
    }
    break;
  ...
  case ISP_PRIVATE_SET_STREAM_SPLIT_INFO: {
    ...
    scaler46_set_max_scale_factor(scaler);
    scaler46_config(scaler);
  }
    break;

  case ISP_PRIVATE_SET_TRIGGER_ENABLE:
    rc = scaler46_trigger_enable(scaler, private_event->data,
      private_event->data_size);
    if (rc < 0) {
      ISP_ERR("failed: scaler46_trigger_enable rc %d", rc);
      ret = FALSE;
    }
    break;
  ...
  case ISP_PRIVATE_SET_TRIGGER_UPDATE: {
    ...
    if (output->saved_events) {
      scaler46_loop_saved_events(module, scaler, output->saved_events,
        identity);
    }
    PTHREAD_MUTEX_LOCK(&scaler->mutex);
    ret = scaler46_do_hw_update(scaler,
      (isp_sub_module_output_t *)private_event->data, identity);
    if (ret == FALSE) {
      ISP_ERR("failed: scaler46_do_hw_update ret %d", ret);
    }
    PTHREAD_MUTEX_UNLOCK(&scaler->mutex);
  }
    break;
  ...
  case ISP_PRIVATE_FETCH_SCALER_OUTPUT: {
    ...
    if (scaler->scalers.is_used) {
      ...
      if (scaler->ispif_out_info.is_split == TRUE) {
        ...
        ISP_DBG(" scaler_input_width %d right_stripe_offset %d",
          scaler_input_width, scaler->ispif_out_info.right_stripe_offset);

        scaler_output->right_split_y_width = scaler46_calculate_right_split_width(
          Y_ScaleCfg->hOut, Y_ScaleCfg->hIn, Y_ScaleCfg->horizMNInit,
          scaler->scalers.reg_cmd.Y_ScaleCfg.hSkipCount, scaler_input_width);

        scaler_output->right_split_cbcr_width =
          scaler46_calculate_right_split_width(CbCr_ScaleCfg->hOut,
          CbCr_ScaleCfg->hIn, CbCr_ScaleCfg->horizMNInit,
          CbCr_ScaleCfg->hSkipCount, scaler_input_width);
        ISP_DBG("scaler output y %d cbcr %d",
  ...
  case ISP_PRIVATE_FETCH_CROP_FACTOR:
    ret = scaler46_get_crop_factor(scaler,
      (isp_crop_factors_t *)private_event->data);
    if (ret == FALSE) {
      ISP_ERR("failed: scaler46_get_crop_factor");
    }
    break;

  case ISP_PRIVATE_REQUEST_ZOOM_PARAMS:
     rc = scaler46_update_zoom_params(scaler,
        (isp_zoom_params_t *)private_event->data, identity);
    if (rc < 0) {
      ISP_ERR("failed: scaler46_update_zoom_params rc %d", rc);
      ret = FALSE;
    }
    break;
 ...
 *    @version: hw version
 *
 *  scaler 46 module open and create func table
 *
 * Return: scaler module ops struct pointer
 **/
boolean scaler46_open(uint32_t version)
{
  scaler46_t *scaler = CAM_MALLOC(sizeof(scaler46_t));
  ...
  memset(scaler, 0, sizeof(scaler46_t));
