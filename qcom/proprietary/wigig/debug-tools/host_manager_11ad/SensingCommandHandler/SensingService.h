...
public:
    ...
    OperationStatus OpenSensing(bool autoRecovery);
    ...
    OperationStatus ReadData(uint32_t maxSizeBytes, std::vector<unsigned char>& sensingData, std::vector<uint64_t>& driTsfVec) const;
    OperationStatus ReadStats(uint32_t& dropCntFromLastRead, uint32_t& numRemainingBursts) const;
    OperationStatus ClearData() const;
    OperationStatus SetReadTimeout(uint32_t readTimeoutMsec) const;
    OperationStatus CancelRead() const;
