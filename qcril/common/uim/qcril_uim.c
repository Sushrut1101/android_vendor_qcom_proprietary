...
05/23/15   yt      Asynchronously read EF-PBR during refresh registration
05/15/15   yt      Use IPC routing for MSM platform
...
#define QCRIL_UIM_PROP_BASEBAND_VALUE_SDA         "sda"
#define QCRIL_UIM_PROP_BASEBAND_VALUE_SDM         "sdm"
#define QCRIL_UIM_PROP_BASEBAND_VALUE_SDX         "sdx"
...
extern void SecureElementInit();
extern void RadioConfigRegisterService();
extern void SecureElementProcessApduInd(const qcril_uim_indication_params_type  * param_ptr);
  ...
  *total_size_ptr = sizeof(qcril_uim_callback_params_type);
  ...
  switch(rsp_data_ptr->rsp_id)
  {
    ...
    case QMI_UIM_SRVC_REMOTE_UNLOCK_RSP_MSG:
      *total_size_ptr += rsp_data_ptr->rsp_data.remote_unlock_rsp.data_len;
      break;
  ...
  switch(rsp_data_ptr->rsp_id)
  {
    ...
    case QMI_UIM_SRVC_REMOTE_UNLOCK_RSP_MSG:
      out_ptr->qmi_rsp_data.rsp_data.remote_unlock_rsp.data_ptr =
        (uint8*)&out_ptr->payload;
      if (out_ptr->qmi_rsp_data.rsp_data.remote_unlock_rsp.data_ptr != NULL &&
          rsp_data_ptr->rsp_data.remote_unlock_rsp.data_ptr != NULL &&
          rsp_data_ptr->rsp_data.remote_unlock_rsp.data_len > 0)
      {
        memcpy(out_ptr->qmi_rsp_data.rsp_data.remote_unlock_rsp.data_ptr,
               rsp_data_ptr->rsp_data.remote_unlock_rsp.data_ptr,
               rsp_data_ptr->rsp_data.remote_unlock_rsp.data_len);
      }
      break;
...
)
{
  ...
  switch(ind_id)
  {
    ...
    case QMI_UIM_SRVC_SAP_CONNECTION_IND_MSG:
      ind_params_ptr =
...
)
{
      ...
      (strcmp(prop_value_ptr, QCRIL_UIM_PROP_BASEBAND_VALUE_CSFB)     == 0))
  {
    qmi_modem_port = QMI_CLIENT_QMUX_RMNET_SDIO_INSTANCE_0;
           ...
           (strcmp(prop_value_ptr, QCRIL_UIM_PROP_BASEBAND_VALUE_SDX)   == 0)  ||
  ...
  else if ((strcmp(prop_value_ptr, QCRIL_UIM_PROP_BASEBAND_VALUE_APQ)    == 0) ||
           (strcmp(prop_value_ptr, QCRIL_UIM_PROP_BASEBAND_VALUE_SDA)    == 0) ||
...
)
{
  ...
  char                           baseband_prop_value[PROPERTY_VALUE_MAX] = {0};
  char                           vcc_prop_value[PROPERTY_VALUE_MAX] = {0};
  char                           procedure_bytes_prop_value[PROPERTY_VALUE_MAX] = {0};
  char                           temp_unlock_status[PROPERTY_VALUE_MAX] = {0};
  ...
  property_get(QCRIL_UIM_PROP_PROCEDURE_BYTES,
               procedure_bytes_prop_value,
               QCRIL_UIM_PROP_RETRUN_PROCEDURE_BYTES_VALUE);

  QCRIL_LOG_INFO("Procedure bytes : %s", procedure_bytes_prop_value);

  if (strcasecmp(procedure_bytes_prop_value, QCRIL_UIM_PROP_SKIP_PROCEDURE_BYTES_VALUE)  == 0)
  {
...
)
{
  ...
  ret = qcril_uim_remote_server_register_service(qmi_ril_get_process_instance_id());
...
)
{
  ...
  switch(callback_params_ptr->qmi_rsp_data.rsp_id)
  {
    ...
    case QMI_UIM_SRVC_READ_RECORD_RSP_MSG:
      if (callback_params_ptr->orig_req_data->request_id == QCRIL_UIM_REQUEST_REFRESH_REGISTER)
      {
    ...
    case QMI_UIM_SRVC_TERMINAL_CAPABILITY_RSP_MSG:
      if(callback_params_ptr->orig_req_data->request_id == QCRIL_EVT_HOOK_GET_TERMINAL_CAPABILITY_LPA_TLV_REQ)
      {
        qcril_uim_get_terminal_capability_resp(callback_params_ptr);
      }
      else if(callback_params_ptr->orig_req_data->request_id == QCRIL_EVT_HOOK_SET_TERMINAL_CAPABILITY_LPA_TLV_REQ)
      {
      ...
      else
      {
        QCRIL_LOG_ERROR("Invalid request id 0x%x corresponding to terminal capability response",
                        callback_params_ptr->orig_req_data->request_id);
      }
      break;
...
)
{
  ...
  switch(ind_param_ptr->ind_id)
  {
    ...
    case QMI_UIM_SRVC_SEND_APDU_IND_MSG:
      ...
      SecureElementProcessApduInd(ind_param_ptr);
      break;
#endif /* RIL_REQUEST_SIM_APDU || RIL_REQUEST_SIM_TRANSMIT_CHANNEL ||
          RIL_REQUEST_SIM_TRANSMIT_APDU_BASIC || RIL_REQUEST_SIM_TRANSMIT_APDU_CHANNEL */
    ...
    case QMI_UIM_SRVC_SAP_CONNECTION_IND_MSG:
      qcril_qmi_sap_ind_hdlr(&ind_param_ptr->ind_data.sap_ind);
      break;
