    ...
    if (unsolicited_msg_info->message_specific_payload_len > 0 && NULL != unsolicited_msg_info->message_specific_payload)
        ...
        switch (call_info_msg->call_info_len)
            ...
            case 1: // single call, simple output
                ...
                snprintf(call_state_buf, sizeof(call_state_buf), "call state %s", call_state_str);
            ...
            default: // multiple calls
                snprintf(call_state_buf, sizeof(call_state_buf), "voice call state update, total calls %d", call_info_msg->call_info_len);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, call_state_buf);
    
                for (idx = 0; idx < call_info_msg->call_info_len; idx++)
                    {
                    snprintf(call_state_buf, sizeof(call_state_buf), "call %d in state %s", idx + 1, qmi_simple_ril_voice_get_call_state_str(call_info_msg->call_info[0].call_state));
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, call_state_buf);
...
int qmi_simple_ril_voice_unsolicited_qmi_message_handler (qmi_util_service_message_info* unsolicited_msg_info, 
    ...
    unsigned int call_state_buf_size = 32 + 20; //extra 20 characters for formatting the output
    char call_state_buf[call_state_buf_size];
    char * call_state_str;
    uint32_t idx;
    ...
    if (unsolicited_msg_info->message_specific_payload_len > 0 && NULL != unsolicited_msg_info->message_specific_payload)
        ...
        switch (call_info_msg->call_info_len)
            ...
            case 1: // single call, simple output
                call_state_str = qmi_simple_ril_voice_get_call_state_str(call_info_msg->call_info[0].call_state);
                qmi_simple_ril_core_set_cond_var("call_state", call_state_str);
                snprintf(call_state_buf, call_state_buf_size, "call state %s",call_state_str );
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, call_state_buf);
                if (0x02 == call_info_msg->call_info[0].call_state)
                    {
                    simple_ril_voice_svc_info.ringing_call_id = call_info_msg->call_info[0].call_id;
            ...
            default: // multiple calls
                snprintf(call_state_buf, call_state_buf_size, "voice call state update, total calls %d", call_info_msg->call_info_len);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, call_state_buf);
    
                for (idx = 0; idx < call_info_msg->call_info_len; idx++)
                    {
                    snprintf(call_state_buf, call_state_buf_size, "call %d in state %s", idx + 1, qmi_simple_ril_voice_get_call_state_str(call_info_msg->call_info[0].call_state));
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, call_state_buf);
                    if (0x02 == call_info_msg->call_info[idx].call_state)
                        {
                        simple_ril_voice_svc_info.ringing_call_id = call_info_msg->call_info[idx].call_id;
...
{
    unsigned int temp_char_array_size = 100;
    char temp_char_array[temp_char_array_size];
    ...
    if (uplink_message->error_code == QMI_NO_ERR || uplink_message->error_code == QMI_IDL_LIB_MISSING_TLV)
        ...
        {
        snprintf(temp_char_array, temp_char_array_size, "[VOICE_TEST] ERROR in sending the SEND_CDMA_BURST_DTMF Request, ERROR CODE:%d\n",uplink_message->error_code);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    return 0;
}
