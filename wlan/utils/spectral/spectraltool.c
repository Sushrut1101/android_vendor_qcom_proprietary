...
void dump_samp_msg_umac(struct spectral_samp_msg_info_umac *msg)
{
	int i;
	...
	if (header->cmd == WLAN_NL_MSG_SPECTRAL_SCAN) {
		...
		if (tb_vendor[CLD80211_ATTR_DATA]) {
			...
			if (IS_UMAC_SPECTRAL_SAMPLE_MSG(
				...
				struct spectral_samp_msg_info_umac *msg_umac =
					nla_data(tb_vendor[CLD80211_ATTR_DATA]);
				if (msg_umac->signature != SPECTRAL_SIGNATURE)
					return -1;
				dump_samp_msg_umac(msg_umac);
			} else {
	...
	while (num_rbuff_received < config_params[SPECTRAL_PARAM_SCAN_COUNT]) {
		...
		if (read_bytes < 0) {
			...
			if (IS_UMAC_SPECTRAL_SAMPLE_MSG(
				...
				msg = (struct spectral_samp_msg_info *)
					NLMSG_DATA(nlh);
...
struct spectral_samp_msg_info_umac *g_msg_umac;
		...
		if (tb_vendor[CLD80211_ATTR_DATA]) {
			if (IS_UMAC_SPECTRAL_SAMPLE_MSG(
				...
				g_msg_umac =
					(struct spectral_samp_msg_info_umac *)
					nla_data(tb_vendor[CLD80211_ATTR_DATA]);
				if (g_msg_umac->signature != SPECTRAL_SIGNATURE)
					return -1;
			} else {
				g_msg = (struct spectral_samp_msg_info *)
					nla_data(tb_vendor[CLD80211_ATTR_DATA]);
...
int spectral_scan_getsample(int ifidx, int num_raw_data_to_cap)
{
	...
	while (num_buf_written < num_raw_data_to_cap) {
		...
		if (IS_UMAC_SPECTRAL_SAMPLE_MSG(
			...
			info->sub_version,
			nlh->nlmsg_len))
			g_msg_umac = (struct spectral_samp_msg_info_umac *)
					NLMSG_DATA(nlh);
		else
			g_msg = (struct spectral_samp_msg_info *)
				NLMSG_DATA(nlh);
		...
		if (IS_UMAC_SPECTRAL_SAMPLE_MSG(
			...
			sampinfo[num_buf_written * 2 + 1] =
					g_msg_umac->samp_data.bin_pwr_count;

			memcpy(buf_ptr, g_msg_umac->samp_data.bin_pwr,
					g_msg_umac->samp_data.bin_pwr_count);
			buf_ptr += MAX_RAW_SPECT_DATA_SZ;

			timeStp[num_buf_written * 3] =
					g_msg_umac->samp_data.spectral_tstamp;
			timeStp[num_buf_written * 3 + 1] =
					g_msg_umac->samp_data.spectral_rssi;
			timeStp[num_buf_written * 3 + 2] =
					g_msg_umac->samp_data.noise_floor;
			{
				static int nf_cnt;
			...
			if (g_msg_umac->samp_data.ch_width == CH_WIDTH_160MHZ) {
				is_160[num_buf_written] = 1;

				memcpy(buf_ptr_sec80,
					   g_msg_umac->samp_data.bin_pwr_sec80,
					   g_msg_umac->samp_data.
					   bin_pwr_count_sec80);
				...
				rssi_nf_sec80[num_buf_written * 2 + 1] =
					g_msg_umac->samp_data.noise_floor_sec80;
			}
		} else {
			if (g_msg->signature != SPECTRAL_SIGNATURE)
				continue;
