...
void __attribute__ ((constructor)) initialize()
{
     ...
     */
    asyncsOfInterest.push_back(VENDOR_FEEDBACK_INPUT_HINT_BEGIN);
    asyncsOfInterest.push_back(VENDOR_FEEDBACK_INPUT_HINT_END);
...
int perfmodule_submit_request(mpctl_msg_t *msg) {
    ...
    if(msg != 0) {
        ...
        ret = LearningModule::getInstance().trigger(payload);
    }
    return ret;
}
