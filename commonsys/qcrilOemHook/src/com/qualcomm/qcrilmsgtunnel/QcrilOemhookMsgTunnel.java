...
public class QcrilOemhookMsgTunnel {
    ...
    private void broadcastAdnInitDone(byte[] data) {
        ...
        putPhoneIdAndSubIdExtra(intent, mInstanceId);
        mContext.sendBroadcast(intent);
    ...
    private void putPhoneIdAndSubIdExtra(Intent intent, int phoneId) {
        ...
        if (subIds != null && subIds.length > 0) {
            intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subIds[0]);
            intent.putExtra(SubscriptionManager.EXTRA_SUBSCRIPTION_INDEX, subIds[0]);
