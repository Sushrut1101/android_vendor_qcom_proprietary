...
{
    QCRIL_LOG_FUNC_ENTRY();
    char* radio_config_thread_name = (char*)qcril_malloc(QCRIL_QMI_RADIO_CONFIG_SOCKET_MAX_THREAD_NAME_SIZE);

    if(radio_config_thread_name)
    {
        snprintf(radio_config_thread_name, QCRIL_QMI_RADIO_CONFIG_SOCKET_MAX_THREAD_NAME_SIZE,
        ...
        QCRIL_LOG_INFO("...Created Radio Config thread on SUB %d with name: %s",
                qmi_ril_get_process_instance_id(), radio_config_thread_name);

        qcril_qmi_radio_config_socket_agent::get_instance()->init(radio_config_thread_name);
        qcril_free(radio_config_thread_name);
    }
    else
    {
