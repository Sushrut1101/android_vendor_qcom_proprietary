 ...
 * already made successfully before exercising any API here except where noted.
 *
...
interface ISessionManagerService {
     ...
     * intent with state=1 to notify interested components. Can be called
     * without establishing a P2P connection.
     *
   ...
   /**
     ...
     * blocking call for a sink device and it will wait until
     * it has connected to peer device through RTSP. For source
     * it will not wait for the peer to connect and may be invoked
     * before a P2P connection is established.
     *
    ...
    /**
     * Create a WFD session
     *
     * @param listener IWfdActionListenere on which callbacks will be given
     * @param localDev local WfdDevice
     * @param peerDev peer WfdDevice
     * @return a Binder token to a WFDSession whose interface can be retireved using
     *         IWFDSession.Stub.asInterface(token)
     *
     * @throws RemoteException
     */
    IBinder createWiFiDisplay(IWfdActionListener listener, in WfdDevice localDev, in WfdDevice peerDev);

    /**
     * Get an inactive WFDSession instance
     *
     * @return a Binder token to a WFDSession whose interface can be retireved using
     *         IWFDSession.Stub.asInterface(token)
     *
     * @throws RemoteException
     */
    IBinder getWiFiDisplaySession();

    /**
     * Get the internally managed WFDSession instance for legacy clients
     *
     * @return a Binder token to a WFDSession whose interface can be retireved using
     *         IWFDSession.Stub.asInterface(token)
     *
