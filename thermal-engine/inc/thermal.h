/*===========================================================================
  ...
  Copyright (c) 2010-2016,2018-2020 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
#define TZ_ALGO "/sys/class/thermal/thermal_zone%d/policy"
#define TZ_POLLING_DELAY "/sys/class/thermal/thermal_zone%d/polling_delay"
#define TZ_PASSIVE_DELAY "/sys/class/thermal/thermal_zone%d/passive_delay"
#define TZ_SET_TEMP "/sys/class/thermal/thermal_zone%d/trip_point_%d_temp"
#define TZ_HYST "/sys/class/thermal/thermal_zone%d/trip_point_%d_hyst"
#define TZ_CDEV_TYPE "/sys/class/thermal/thermal_zone%d/cdev%d/type"
#define TZ_CDEV_TRIP_POINT "/sys/class/thermal/thermal_zone%d/cdev%d_trip_point"
#define TZ_CDEV_UPPER_LIM "/sys/class/thermal/thermal_zone%d/cdev%d_upper_limit"
#define TZ_CDEV_LOWER_LIM "/sys/class/thermal/thermal_zone%d/cdev%d_lower_limit"
#define TZ_MODE "/sys/class/thermal/thermal_zone%d/mode"
#define TZ_TEMP "/sys/class/thermal/thermal_zone%d/temp"
#define TZ_TYPE "/sys/class/thermal/thermal_zone%d/type"
#define TZ_DIR_FMT "thermal_zone%d"
#define THERMAL_TYPE "/sys/class/thermal/%s/type"
...
/* constant values for thermal configuration structures */
#define MAX_TZ_NAME_LEN 32
#define MAX_TRIP_TEMP_NUM 16
#define MAX_CDEV_NUM 32
...
enum print_output_type {STD_OUT, DYNAMIC_QUERY};
...
/*structures and function definitions for kernel thermal conf info */

struct cooling_device {
	int cdevID;
	char cdev_name[MAX_TZ_NAME_LEN];
	int upper_limit;
	int lower_limit;
};
...
struct trip_temp {
	...
	int clr_temp;
	uint cdev_num;
	uint cdev_print_space;
	struct cooling_device cdev_list[MAX_CDEV_NUM];
};

struct thermal_zone_info {
	bool had_error;
	bool mode;
	char tz_name[MAX_TZ_NAME_LEN];
	char algo_type[MAX_TZ_NAME_LEN];
	char sensor[MAX_TZ_NAME_LEN];
	uint polling_delay;
	uint passive_delay;
	uint trip_temp_num;
	struct trip_temp trip_temp_list[MAX_TRIP_TEMP_NUM];
};

struct thermal_zone_list {
	uint num_tz;
	struct thermal_zone_info tz_info_list[MAX_NUM_TZ];
};

struct thermal_zone_list tz_list;
...
#define thermalmsg(loglevel, logtype, format, ...)				\
	do {									\
		...
		if (logtype & LOG_LOCAL_SOCKET) {				\
			...
			if (snprintf(logstring,REPORT_MSG_MAX,format, ## __VA_ARGS__) >= 0)\
				write_to_local_socket(SOCKET_RPT_LOG, \
...
static inline int wlan_bw_request(int level)
{
	return -(EFAULT);
}
#endif
...
#else
static inline void print_thermal_zones(void){}
static inline void get_thermal_zone_info(void){}
#endif
