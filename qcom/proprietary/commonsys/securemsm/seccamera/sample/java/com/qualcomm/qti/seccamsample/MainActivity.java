...
public class MainActivity extends Activity implements SecCamServiceClient.ClientCallback {
    ...
    private static boolean isSessionActive_ = false;

    private static final String TZ_APP_NAME = "seccamdemo64";
    private static boolean is_2dfa = false; //enable for 2dfa
    private static final Integer destination =
        (is_2dfa) ? SecCamServiceClient.DESTINATION_MLVM : SecCamServiceClient.DESTINATION_QTEE;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SecCamServiceClient.getInstance().start(this, this, TZ_APP_NAME, TZ_APP_BUFFER_SIZE, destination);
    ...
    public void serviceDisconnected() {
        ...
        // If the camera was already started and the service disconnected in
        // the middle, stop the session
        if (isSessionActive_) {
            // Same Integer code as given in startActivityForResult
            finishActivity(1);
        }
    }
}
