/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
Copyright (c) 2007-2020 Qualcomm Technologies, Inc.
All Rights Reserved.
...
#define MAX_MASK_SYNC_COUNT 1000
...
void diag_qshrink4_init(void)
{
	...
	for (i = 0; i < NUM_PROC; i++) {
		qshrink4_data[i].header_length = 10;
...
{
	static unsigned char send_buf[MAX_SOCKET_CHANNELS][USER_SPACE_DATA];
	unsigned char offset = 4;
	int i, j;
	int start = 0, end = 0;
	int s_char = 0, e_char = 0;
	int copy_bytes;
	int success = 0, local_hdlc_disabled = 0;
	int packet_len_index = 0, end_index = 0;
	uint16_t packet_len = 0;

	if ((id < 0) || id >= MAX_SOCKET_CHANNELS || num_bytes < MIN_CMD_PKT_SIZE) {
	...
	*(int *)send_buf[id] = USER_SPACE_DATA_TYPE;
	if (socket_token[id] != 0  ) {
		*(int *)(send_buf[id] + offset) = socket_token[id];
		offset += sizeof(int);
	}
	...
	for (i = 0; i < num_bytes; i++) {
		if (buf[i] == CONTROL_CHAR) {
			s_char = buf[i];
			if (i + 2 < num_bytes) {
				packet_len_index = i + 2;
				packet_len = (uint16_t)(*(uint16_t *)(buf + packet_len_index));
				end_index = i + DIAG_NON_HDLC_HEADER_SIZE + packet_len;
				if (end_index < num_bytes)
					e_char = buf[end_index];
			}

			if (s_char == e_char) {
				local_hdlc_disabled = 1;
				copy_bytes = packet_len + DIAG_NON_HDLC_HEADER_SIZE + 1;
				if (offset + copy_bytes >= USER_SPACE_DATA) {
					DIAG_LOGE("diag_socket_log: %s:non_hdlc: command too large, dropping pkt\n", __func__);
					return success;
				}
				memset(send_buf[id]+offset, 0, USER_SPACE_DATA-offset);
				memcpy(send_buf[id]+offset, buf+start, copy_bytes);
				diag_send_data(send_buf[id], copy_bytes+offset);
				i = start + packet_len + (DIAG_NON_HDLC_HEADER_SIZE);
				start = i+1;
				end = i+1;
				continue;
			} else {
				...
				if (offset + copy_bytes >= USER_SPACE_DATA) {
					DIAG_LOGE("diag_socket_log: %s: hdlc: command too large, dropping pkt\n", __func__);
					return success;
				}
				memset(send_buf[id]+offset, 0, USER_SPACE_DATA-offset);
				memcpy(send_buf[id]+offset, buf+start, copy_bytes);
...
static void parse_data_for_command_rsp(uint8* ptr, int count_received_bytes, int index)
{

	if (diag_id_state)
		parse_data_for_diag_id_rsp(ptr, count_received_bytes, index);

	if (qdss_state)
		parse_data_for_qdss_rsp(ptr, count_received_bytes);

	if (parser_state)
		parse_data_for_qsr4_db_file_op_rsp(ptr, count_received_bytes);

	if (adpl_parser_state)
		parse_data_for_adpl_rsp(ptr, count_received_bytes);
}
...
static boolean fill_pkt_buffer(unsigned char *ptr)
{
	...
	for (i = 0; i < num_data_fields; i++) {
		...
		if (count_received_bytes > 0) {
			parse_data_for_command_rsp(ptr, count_received_bytes, index);
...
static int process_ext_header_packet(int dci_proc, unsigned char* read_buffer) {
	...
	case DCI_EVENT_TYPE:
		...
		break;
	default:
		DIAG_LOGE("diag: dci: received invalid packet type %lu in extended packet\n",
				pkt_cmd_type);
		break;
	}
	read_bytes += len;
	...
	if (!callback_ptr) {
		DIAG_LOGE("diag: dci: no callback function registered for packet type %lu\n",
				pkt_cmd_type);
		return 0;
	}
...
void log_to_device(unsigned char *ptr, int logging_mode, int size, int type)
{
	...
	if (CALLBACK_MODE == logging_mode) {
		for (i = 0; i < num_data_fields && bytes_remaining > 0; i++) {
			...
			if (bytes_remaining < (bytes_read + count_received_bytes)) {
				DIAG_LOGE("diag: In %s received packet of invalid length bytes remaining:%u bytes_read:%u count_received_bytes:%lu\n",
	...
	if (fd_dev != -1) {
		if(logging_mode == MEMORY_DEVICE_MODE ){
			errno = 0;
			if ((use_qmdl2_v2 || hdlc_disabled) && write_qshrink_header) {
				write_qshrink_header = 0;
				if (use_qmdl2_v2 && (type >= 0)) {
...
{
	int err, remote_mask, success, dev_idx, device_mask = 1;
	...
	diag_hw_accel_query_sub_payload_rsp_t *query_rsp, uint8 operation)
{
	...
	if (operation == DIAG_HW_ACCEL_OP_QUERY) {
		err = ioctl(diag_fd, DIAG_IOCTL_QUERY_PD_FEATUREMASK, query_rsp,
			sizeof(diag_hw_accel_query_sub_payload_rsp_t), NULL, 0, NULL, NULL);
		if (err) {
			DIAG_LOGE("diag: Error in querying PD featuremask, err: %d, errno: %d\n",err, errno);
			return -1;
		}
	} else {
		err = ioctl(diag_fd, DIAG_IOCTL_PASSTHRU_CONTROL, pReq,
			sizeof(diag_hw_accel_cmd_req_t), NULL, 0, NULL, NULL);
		if (err) {
...
boolean Diag_LSM_Init (uint8_t* pIEnv)
{
   ...
   pthread_mutex_lock(&lsm_init_count_mutex);
	if(lsm_init_count == 0) {
		...
		ret = ioctl(diag_fd, DIAG_IOCTL_QUERY_CON_ALL, &params_con, sizeof(struct diag_con_all_param_t), NULL, 0, NULL, NULL);
...
boolean Diag_LSM_DeInit(void)
{
	...
	if (diag_fd != DIAG_INVALID_HANDLE) {
		...
		DIAG_LOGE("diag: %s, Joining the read thread\n", __func__);
...
static void *CreateWaitThread(void* param)
{
	...
	if (rc != 0)
		DIAG_LOGE("diag:%s: Failed to block signal for read thread\n", __func__);
