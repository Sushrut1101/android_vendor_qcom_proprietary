...
import com.qualcomm.qti.wigig.WpsResult;
import com.qualcomm.qti.wigig.WpsResult.Status;
...
import static com.qualcomm.qti.wigig.WigigManager.SENSING_STATE_DISABLED;
import static com.qualcomm.qti.wigig.WigigManager.SENSING_STATE_ENABLED;
import static com.qualcomm.qti.wigig.WigigManager.SENSING_STATE_FAILED;
import static com.qualcomm.qti.wigig.WigigManager.SENSING_STATE_UNKNOWN;
...
public class WigigStateMachine extends StateMachine {
    ...
    private int mConnectedPeriodicScanToken = 0;
    private boolean mConnectedPeriodicScan;
    ...
    /* RSSI threshold for triggering periodic scan to look for roam candidates */
    private static final int WIGIG_CONNECTED_PERIODIC_SCAN_RSSI_THRESHOLD = -50;
    /* minimal diff between current RSSI and roaming candidate's RSSI for triggering roam */
    private static final int WIGIG_ROAM_MIN_RSSI_DIFF = 5;
    private static final int CONNECTED_PERIODIC_SCAN_INTERVAL_MSECS = 10000;
    /**
    ...
    /* IpClient created notification */
    static final int CMD_IP_CLIENT_CREATED_NOTIFICATION                 = BASE + 31;
    ...
    /**
     * Used to handle messages bounced between WigigStateMachine and IpClient.
     */
    ...
    /* periodic scan while connected for the sake of roaming */
    static final int CMD_CONNECTED_PERIODIC_SCAN                        = BASE + 412;

    /* For DHCP notifications. */
    static final int CMD_PRE_DHCP_ACTION                                = BASE + 413;
    static final int CMD_PRE_DHCP_ACTION_COMPLETE                       = BASE + 414;
    static final int CMD_POST_DHCP_ACTION                               = BASE + 415;

    /* Enable / disable sensing */
    static final int CMD_ENABLE_SENSING                                 = BASE + 500;
    static final int CMD_DISABLE_SENSING                                = BASE + 501;
    ...
    /* pre-initial state, waiting for android to be ready (mainly IpClient creation) */
    private State mPreInitialState = new PreInitialState();
    ...
    /* Roaming to another AP in same ESS */
    ...
    /* Sensing Stand alone */
    private State mSensingStandaloneState = new SensingStandaloneState();
    ...
    /**
     * One of {@link WigigManager#SENSING_STATE_DISABLED},
     * {@link WigigManager#SENSING_STATE_ENABLED},
     * {@link WigigManager#SENSING_STATE_UNKNOWN},
     * {@link WigigManager#SENSING_STATE_FAILED}
     */
    private final AtomicInteger mSensingState = new AtomicInteger(SENSING_STATE_DISABLED);
    ...
    private WigigManager.SoftApCallback mSoftApCallback;
    private int mNumAssociatedStations = 0;

    /**
     * Called from WigigService to register a callback for notifications
     */
    public void registerSoftApCallback(WigigManager.SoftApCallback callback) {
    ...
    public WigigStateMachine(Context context, String wlanInterface, WigigNetworkMonitor networkMonitor) {
        ...
        mTcpBufferSizes = mCapConfigStoreHal.getConfigString(
            "npt.tcp_buffer_sizes", mContext.getResources().getString(
                com.android.internal.R.string.config_wifi_tcp_buffers));

        addState(mDefaultState);
            addState(mPreInitialState, mDefaultState);
            ...
            addState(mSensingStandaloneState, mDefaultState);

        setInitialState(mPreInitialState);
    ...
    class IpClientCallback extends IpClientCallbacks {

        @Override
        public void onIpClientCreated(IIpClient ipClient) {
            sendMessage(CMD_IP_CLIENT_CREATED_NOTIFICATION, ipClient);
        }
     ...
     * TODO: doc + sync
     */
    public int syncGetSensingState() {
        return mSensingState.get();
    }

    /**
     * TODO: doc + sync
     */
    public boolean getSensingEn() {
        return mSensingState.get() == SENSING_STATE_ENABLED;
    }
     ...
     * TODO: doc
     */
    public String syncGetSensingStateByName() {
        switch (mSensingState.get()) {
            case SENSING_STATE_DISABLED:
                return "disabled";
            case SENSING_STATE_ENABLED:
                return "enabled";
            case SENSING_STATE_FAILED:
                return "failed";
            case SENSING_STATE_UNKNOWN:
                return "unknown state";
            default:
                return "[invalid state]";
        }
    ...
    public void setSensing(boolean enable) {
        log("setSensing " + enable);

        if (enable) {
            sendMessage(CMD_ENABLE_SENSING);
        } else {
            sendMessage(CMD_DISABLE_SENSING);
        }
    }
    ...
    String smToString(int what) {
        ...
        switch (what) {
            ...
            case CMD_TETHER_NOTIFICATION_TIMED_OUT:
                ...
                break;
            case CMD_IP_CLIENT_CREATED_NOTIFICATION:
                s = "CMD_IP_CLIENT_CREATED_NOTIFICATION";
                break;
            ...
            case CMD_COUNTRY_CODE_CHANGED:
                ...
                break;
            case CMD_CONNECTED_PERIODIC_SCAN:
                s = "CMD_CONNECTED_PERIODIC_SCAN";
                break;
            case CMD_ROAM_WATCHDOG_TIMER:
                s = "CMD_ROAM_WATCHDOG_TIMER";
                break;
            case WigigMonitor.AP_NON_P2P_STA_CONNECTED_EVENT:
                s = "AP_NON_P2P_STA_CONNECTED_EVENT";
                break;
            case WigigMonitor.AP_NON_P2P_STA_DISCONNECTED_EVENT:
                s = "AP_NON_P2P_STA_DISCONNECTED_EVENT";
                break;
            default:
    ...
    private void setWigigApState(int wigigApState, int reason) {
        ...
        if (mSoftApCallback != null)
            mSoftApCallback.onStateChanged(wigigApState, reason);
    ...
    private void setSensingState(int sensingState) {
        final int previousSensingState = mSensingState.get();

        mSensingState.set(sensingState);

        if (DBG) {
            log("setSensingState: " + syncGetSensingStateByName());
            log("sending SENSING_STATE_CHANGED_ACTION, new state=" + sensingState);
        }
        final Intent intent = new Intent(WigigManager.SENSING_STATE_CHANGED_ACTION);
        intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
        intent.putExtra(WigigManager.EXTRA_SENSING_STATE, sensingState);
        intent.putExtra(WigigManager.EXTRA_PREVIOUS_SENSING_STATE, previousSensingState);
        mContext.sendStickyBroadcastAsUser(intent, UserHandle.ALL);
    }
    ...
    private void handleSupplicantConnectionLoss() {
        ...
        setSensingState(SENSING_STATE_DISABLED);
    }
    ...
    private void setFstPropEnabled(boolean enabled) {
        ...
        // TODO uncomment when QCOM WIFI framework code is merged
        ...
        if (enabled) {
            // update framework classes from config store HAL
            // TODO temporarily commented out due to compile error on some framework versions
            // TetheringConfiguration.fstInterfaceName is initialized to bond0 by default,
            // so it should still work.
            //String fstInterfaceName = mCapConfigStoreHal.getConfigString("fst.data.interface", "bond0");
            //TetheringConfiguration.setFstInterfaceName(fstInterfaceName);

            mWigigNative.setIPAOffload(false);
            mWigigNative.setSensingOverSPI(false);
...
//        }
    ...
    private void startSoftApWithConfig(final WifiConfiguration configuration) {
        ...
        Mutable<Integer> edmgChannel = new Mutable<>(0);
        ...
        if (config.apChannel == 0) {
            edmgChannel.value = SystemProperties.getInt("persist.vendor.wigig.softap.edmg.channel",
                                                        10);
            config.apChannel = SystemProperties.getInt("persist.vendor.wigig.softap.channel",
                                                       2);
        }
        ...
        new Thread(new Runnable() {
            public void run() {
                boolean res;
                int maxNumSta = mCapConfigStoreHal.getConfigInteger("max_num_sta",
                                                                    DEFAULT_MAX_NUM_STA);
                ...
                res = mWigigNative.setSoftAp(mInterfaceName, config.SSID, config.hiddenSSID,
                        config.apChannel, edmgChannel.value,
                ...
                String triggers = mCapConfigStoreHal.getConfigString("sap.wowlan_triggers", "any");
                ...
                mWigigMonitor.setHostapdOverride(true);
    ...
    class PreInitialState extends State {
        @Override
        public void enter() {
            IpClientCallback clientCallback = new IpClientCallback();
            IpClientUtil.makeIpClient(mContext, mDataInterfaceName, clientCallback);
        }
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case CMD_IP_CLIENT_CREATED_NOTIFICATION:
                    mIpClient = (IIpClient)message.obj;
                ...
                case CMD_AIRPLANE_TOGGLED:
                case CMD_COUNTRY_CODE_CHANGED:
                case CMD_ENABLE_SENSING:
                case CMD_DISABLE_SENSING:
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case CMD_START_SUPPLICANT:
                    ...
                    if (SystemProperties.getInt("persist.vendor.wigig.spi_sensing.support", 0) == 1) {
                        sensingOverSPI = true;
                    }
                    // must be called before loading the driver
                    loge("call setSensingOverSPI(" + sensingOverSPI + ")");
                    mWigigNative.setSensingOverSPI(sensingOverSPI);
                    mWigigNative.setIPAOffload(false);
                    if (mWigigNative.loadDriver()) {
                        try {
                ...
                case CMD_START_RATE_UPGRADE:
                    ...
                    break;
                case CMD_START_AP:
                    boolean ipaOffload = false;
                    if (SystemProperties.getBoolean("persist.vendor.wigig.ipa.offload", false) &&
                            Settings.Global.getInt(
                                mContext.getContentResolver(), TETHER_OFFLOAD_DISABLED, 0) == 0) {
                        ipaOffload = true;
                    }
                    // must be before loading the driver
                    mWigigNative.setIPAOffload(ipaOffload);
                    mWigigNative.setSensingOverSPI(false);
                ...
                case CMD_AIRPLANE_TOGGLED:
                    ...
                    break;
                case CMD_ENABLE_SENSING:
                    loge("Handling CMD_ENABLE_SENSING in InitialState");
                    sensingOverSPI = false;
                    if (SystemProperties.getInt("persist.vendor.wigig.spi_sensing.support", 0) == 1) {
                        sensingOverSPI = true;
                    }
                    // must be called before loading the driver, which will be done in SensingStandaloneState
                    loge("call setSensingOverSPI(" + sensingOverSPI + ")");
                    mWigigNative.setSensingOverSPI(sensingOverSPI);

                    if (sensingOverSPI) {
                        transitionTo(mSensingStandaloneState);
                    } else {
                        setSensingState(SENSING_STATE_DISABLED);
                    }
                    break;
    ...
    class SupplicantStartingState extends State {
        ...
        private void initializeWowlanTriggers() {
            String triggers = mCapConfigStoreHal.getConfigString("sta.wowlan_triggers", "any");
            // continue even if we fail to set wowlan_triggers, this is not critical
            // empty value means remove all triggers
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case WigigMonitor.SUP_DISCONNECTION_EVENT:
                    ...
                    if (mSensingState.get() == SENSING_STATE_ENABLED) {
                        transitionTo(mSensingStandaloneState);
                    } else {
                        transitionTo(mInitialState);
                    }
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case WigigMonitor.SUP_DISCONNECTION_EVENT:  /* Supplicant connection lost */
                    ...
                    if (mSensingState.get() == SENSING_STATE_ENABLED) {
                        transitionToState = mSensingStandaloneState;
                    } else {
                        transitionToState = mInitialState;
                    }
                ...
                case CMD_COUNTRY_CODE_CHANGED:
                    ...
                    break;
                case CMD_DISABLE_SENSING:
                    loge("Handling CMD_DISABLE_SENSING in SupplicantStartedState");
                    if (!mWigigNative.killSensing()) {
                        loge("failed to kill sensing daemon");
                    }
                    setSensingState(SENSING_STATE_DISABLED);
                    break;
                case CMD_ENABLE_SENSING:
                    loge("Handling CMD_ENABLE_SENSING in SupplicantStartedState");
                    if (SystemProperties.getInt("persist.vendor.wigig.spi_sensing.support", 0) == 1) {
                        if (!mWigigNative.startSensing()) {
                                loge("failed to start sensing daemon");
                                setSensingState(SENSING_STATE_FAILED);
                        } else {
                                setSensingState(SENSING_STATE_ENABLED);
                        }
                    } else {
                        loge("Sensing is not supported");
                        setSensingState(SENSING_STATE_DISABLED);
                    }
                    break;
                default:
    ...
    class WaitForP2pDisableState extends State {
        ...
        public void enter() {
            switch (getCurrentMessage().what) {
                case WigigMonitor.SUP_DISCONNECTION_EVENT:
                    if (mSensingState.get() == SENSING_STATE_ENABLED) {
                        mTransitionToState = mSensingStandaloneState;
                    } else {
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case WigigManager.START_WPS:
                    ...
                    break;
                case CMD_ENABLE_SENSING:
                    if (DBG) log("P2P in use, ignore CMD_ENABLE_SENSING");
                    setSensingState(SENSING_STATE_FAILED);
                    break;
                default:
                    handled = NOT_HANDLED;
    ...
    class L2ConnectedState extends State {
        ...
        public void enter() {
            ...
            mConnectedPeriodicScan = false;
            if (mNetworkAgent != null) {
                loge("Have NetworkAgent when entering L2Connected");
                setNetworkDetailedState(DetailedState.DISCONNECTED);
            }
            setNetworkDetailedState(DetailedState.CONNECTING);
            int score = mCapConfigStoreHal.getConfigInteger("net.score", DEFAULT_SCORE);
            ...
            switch (message.what) {
                ...
                case CMD_PRE_DHCP_ACTION_COMPLETE:
                    try {
                        ...
                        loge("Error stopping completing pre dhcp action", e);
                    }
                    break;
                case CMD_POST_DHCP_ACTION:
                    handlePostDhcpSetup();
                ...
                case CMD_RSSI_POLL:
                    if (message.arg1 == mRssiPollToken) {
                        ...
                        // start/stop periodic scan according to new rssi level
                        boolean runPeriodicScan =
                            SystemProperties.getBoolean("persist.vendor.wigig.connected.periodic.scan", false) &&
                            (mWifiInfo.getRssi() != mWifiInfo.INVALID_RSSI &&
                                mWifiInfo.getRssi() < WIGIG_CONNECTED_PERIODIC_SCAN_RSSI_THRESHOLD);
                        if (runPeriodicScan == mConnectedPeriodicScan) {
                            break;
                        }

                        logd(runPeriodicScan ? "Starting" : "Stopping" + " connected periodic scan");
                        mConnectedPeriodicScan = runPeriodicScan;
                        if (runPeriodicScan) {
                            sendMessage(CMD_CONNECTED_PERIODIC_SCAN, ++mConnectedPeriodicScanToken);
                        } else {
                            removeMessages(CMD_CONNECTED_PERIODIC_SCAN);
                        }
                    }
                    break;
                case CMD_ASSOCIATED_BSSID:
        ...
        public void enter() {
            ...
            if (!TextUtils.isEmpty(mTcpBufferSizes)) {
                try {
                    ...
                    loge("Error setting up tcp buffer sizes", e);
                }
            }
            ...
            if (!mWigigConfigStore.isUsingStaticIp(mLastNetworkId)) {
                ...
                try {
                    ...
                    loge("Error starting provisioning on IpClient", e);
                }
            ...
            switch (message.what) {
                ...
                case WigigMonitor.NETWORK_CONNECTION_EVENT:
                    if (mAssociated) {
                        ...
                        // keep current IP (skip ObtainingIpState)
                        transitionTo(mConnectedState);
                    }
                    break;
                case WigigMonitor.NETWORK_DISCONNECTION_EVENT:
                    ...
                    String target = (TextUtils.isEmpty(mTargetRoamBSSID) ? "" : mTargetRoamBSSID);
    ...
    class ConnectedState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case CMD_CONNECTED_PERIODIC_SCAN:
                    if (message.arg1 == mConnectedPeriodicScanToken) {
                        startScan();
                        sendMessageDelayed(obtainMessage(CMD_CONNECTED_PERIODIC_SCAN,
                                ++mConnectedPeriodicScanToken, 0),
                                CONNECTED_PERIODIC_SCAN_INTERVAL_MSECS);
                    }
                    break;
                case WigigMonitor.SCAN_RESULTS_EVENT:
                case WigigMonitor.SCAN_FAILED_EVENT:
                    ...
                    // see if we have any roam candidate
                    synchronized (mScanResultCache) {
                        ScanResult roamCandidate = null;
                        for (ScanResult result : mScanResults) {
                            if (!result.wigigSsid.toString().equals(mWifiInfo.getWifiSsid().toString())) {
                                // skip different SSID
                                continue;
                            }

                            if (result.BSSID.equals(mWifiInfo.getBSSID())) {
                                // skip connected AP
                                continue;
                            }

                            logd("roam candidate: bssid " + result.BSSID + ", rssi " + result.level);
                            int highestLevel = (roamCandidate != null ? roamCandidate.level : WifiInfo.MIN_RSSI);
                            if (result.level > highestLevel) {
                                roamCandidate = result;
                            }
                        }
                        ...
                        if (roamCandidate.level > mWifiInfo.getRssi() + WIGIG_ROAM_MIN_RSSI_DIFF) {
                            logd("trying to roam to bssid " + roamCandidate.BSSID +
                                    ", rssi " + roamCandidate.level +
                                    " (current rssi " + mWifiInfo.getRssi() + " )");
                            boolean res = mWigigNative.roamToNetwork(roamCandidate.BSSID);
                            if (res) {
                                mTargetRoamBSSID = roamCandidate.BSSID;
            ...
            switch(message.what) {
                ...
                case CMD_START_AP_SUCCESS:
                    ...
                    if (mSoftApCallback != null) {
                        ...
                        mSoftApCallback.onStaConnected("", mNumAssociatedStations);
                    }
                ...
                case CMD_START_AP_FAILURE:
                    ...
                    mWigigMonitor.setHostapdOverride(false);
                    transitionTo(mInitialState);
                    break;
                default:
                    return NOT_HANDLED;
    ...
    class SoftApStartedState extends State {
        ...
        private void setConnectedStations(String Macaddr) {
            ...
            log("Num connected stations " + mNumAssociatedStations);
            if (mSoftApCallback != null) {
                mSoftApCallback.onStaConnected(Macaddr, mNumAssociatedStations);
            } else {
                log("SoftApCallback is null. Dropping onStaConnected event.");
            }
        }
        ...
        private void setDisConnectedStations(String Macaddr) {
            ...
            log("Num connected stations " + mNumAssociatedStations);
            if (mSoftApCallback != null) {
                mSoftApCallback.onStaDisconnected(Macaddr, mNumAssociatedStations);
            } else {
                log("SoftApCallback is null. Dropping onStaDisconnected event.");
            }
        }

        @Override
        public void enter() {
            log("Resetting num stations on start");
        ...
        public void exit() {
            mWigigMonitor.stopMonitoring();
            mWigigMonitor.setHostapdOverride(false);
            log("Resetting num stations on stop");
            mNumAssociatedStations = 0;
        }
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case WigigMonitor.AP_NON_P2P_STA_CONNECTED_EVENT:
                    if (message.obj == null) {
                        log("Invalid Macaddr of connected station: " + message.obj);
                        break;
                    }
                    log("Setting connected stations");
                    setConnectedStations((String) message.obj);
                    break;
                case WigigMonitor.AP_NON_P2P_STA_DISCONNECTED_EVENT:
                    if (message.obj == null) {
                        log("Invalid Macaddr of disconnected station: " + message.obj);
                        break;
                    }
                    log("Setting disconnected stations");
                    setDisConnectedStations((String) message.obj);
                    break;
                case WigigMonitor.SUP_DISCONNECTION_EVENT:
                    log("hostapd terminated");
                    mWigigMonitor.stopMonitoring();
                    mWigigMonitor.setHostapdOverride(false);
                    setWigigApState(WIGIG_AP_STATE_DISABLED, 0);
                    transitionTo(mInitialState);
    ...
    class SensingStandaloneState extends State {
        @Override
        public void enter() {
           if (mWigigNative.loadDriver()) {
                try {
                    ...
                    loge("calling Interface up");
                    mNwService.setInterfaceUp(mInterfaceName);

                    /* TODO - allow to finish interface up */
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case CMD_START_RATE_UPGRADE:
                    /* Reject */
                    loge("Failed to start Rate Upgrade when sensing is enabled");
                    break;
                case CMD_START_AP:
                    /* Reject */
                    loge("Failed to start soft AP when sensing is enabled");
                    setWigigApState(WIGIG_AP_STATE_FAILED, WigigManager.SAP_START_FAILURE_GENERAL);
                    break;
                case CMD_AIRPLANE_TOGGLED:
                    boolean newWigigMode = (message.arg1 == 1);
                ...
                case CMD_DISABLE_SENSING:
                    ...
                    break;
                case CMD_P2P_REQUEST_RADIO:
                        loge("Reject P2P_REQUEST_RADIO when sensing is enabled");
                        break;
                default:
                    return NOT_HANDLED;
            }
