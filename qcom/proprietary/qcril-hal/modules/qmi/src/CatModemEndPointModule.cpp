...
std::map<uint32_t, void *>  CatModemEndPointModule::mEnvList;
std::map<uint32_t, void *>  CatModemEndPointModule::mTrMsgPtr;
...
)
{
  ...
  else if (ind_msg_ptr->run_at_command_valid)
  {
    ...
    pc_data_len   = ind_msg_ptr->run_at_command.pc_run_at_command_len;
    pc_data_ptr   = ind_msg_ptr->run_at_command.pc_run_at_command;
    unique_ref_id = ind_msg_ptr->run_at_command.uim_ref_id;
    cmd_type      = QCRIL_GSTK_QMI_CMD_STK_RUN_AT_CMD;
  }
  else
  {
...
{
  CatModemEndPointModule *module = (CatModemEndPointModule *)notify_cb_data;
...
{
  std::shared_ptr<GstkQmiCatResponseMsg>    resp_msg_ptr   = nullptr;
  std::shared_ptr<qmi_cat_rsp_data_type>    resp_data_ptr  = std::make_shared<qmi_cat_rsp_data_type>();

  QCRIL_LOG_INFO("onQmiAsyncCb, msg_id: 0x%x", msg_id);
  ...
  {
    resp_data_ptr->transp_err = transp_err;

    /* Process valid RESP messages */
    switch (msg_id)
    {
      case QMI_CAT_SCWS_DATA_AVAILABLEA_RESP_V02:
        resp_data_ptr->rsp_id = QMI_CAT_SRVC_SCWS_DATA_AVAILABLE_RSP_MSG;
        if (resp_data_ptr->transp_err == QMI_NO_ERR &&
            resp_c_struct != nullptr)
        {
          cat_scws_data_available_resp_msg_v02 * data_avail_resp_ptr =
                                 (cat_scws_data_available_resp_msg_v02 *)resp_c_struct;

          resp_data_ptr->qmi_err = data_avail_resp_ptr->resp.error;
        }
        break;

      case QMI_CAT_SEND_EVENLOPE_CMD_RESP_V02:
        resp_data_ptr->rsp_id = QMI_CAT_SRVC_SEND_ENVELOPE_COMMAND_RSP_MSG;
        QCRIL_LOG_INFO("Envelope command response Transport err: 0x%x", resp_data_ptr->transp_err);
            ...
            resp_c_struct != nullptr)
        {
          ...
          if (env_resp_ptr->indication_token_valid)
          {
            ...
            mEnvList.insert({env_resp_ptr->indication_token, resp_cb_data});
            QCRIL_GSTK_QMI_FREE_PTR(resp_c_struct);
            return;
          }

          resp_data_ptr->qmi_err = env_resp_ptr->resp.error;
          ...
          if (env_resp_ptr->resp.result == QMI_RESULT_SUCCESS_V01)
          {
            if (env_resp_ptr->env_resp_data_valid)
            {
              ...
              {
                memcpy(resp_data_ptr->rsp_data.env_rsp_data.env_resp_data,
                       env_resp_ptr->env_resp_data.env_resp_data,
                       env_resp_ptr->env_resp_data.env_resp_data_len);

                resp_data_ptr->rsp_data.env_rsp_data.env_resp_data_len =
                           env_resp_ptr->env_resp_data.env_resp_data_len;
              }

              resp_data_ptr->rsp_data.env_rsp_data.sw1 = env_resp_ptr->env_resp_data.sw1;
              resp_data_ptr->rsp_data.env_rsp_data.sw2 = env_resp_ptr->env_resp_data.sw2;
            }
          }
        }
        break;
      ...
      case QMI_CAT_SEND_TR_RESP_V02:
        resp_data_ptr->rsp_id = QMI_CAT_SRVC_SEND_TERMINAL_RESPONSE_RSP_MSG;
        QCRIL_LOG_INFO("TR response Transport err: 0x%x", resp_data_ptr->transp_err);
            ...
            resp_c_struct != nullptr)
        {
          ...
          if (tr_resp_ptr->indication_token_valid)
          {
            ...
            mTrMsgPtr.insert({tr_resp_ptr->indication_token, resp_cb_data});
            QCRIL_GSTK_QMI_FREE_PTR(resp_c_struct);
            return;
          }

          resp_data_ptr->qmi_err = tr_resp_ptr->resp.error;
          ...
          if (tr_resp_ptr->resp.result == QMI_RESULT_SUCCESS_V01)
          {
                ...
                (tr_resp_ptr->tr_response_data.tr_response_len <= QMI_CAT_TERMINAL_RESPONSE_MAX_LENGTH))
            {
              ...
              memcpy(resp_data_ptr->rsp_data.tr_rsp_data.tr_response,
                     tr_resp_ptr->tr_response_data.tr_response,
                     tr_resp_ptr->tr_response_data.tr_response_len);

              resp_data_ptr->rsp_data.tr_rsp_data.tr_response_len =
                         tr_resp_ptr->tr_response_data.tr_response_len;

              resp_data_ptr->rsp_data.tr_rsp_data.sw1 = tr_resp_ptr->tr_response_data.sw1;
              resp_data_ptr->rsp_data.tr_rsp_data.sw2 = tr_resp_ptr->tr_response_data.sw2;
            }
          }
        }
        break;
  ...
  resp_msg_ptr = std::make_shared<GstkQmiCatResponseMsg>(resp_data_ptr, resp_cb_data);
...
{
  auto ind_ptr     = std::make_shared<qcril_gstk_qmi_ind_params_type>();;
  ...
  switch(ind_id)
  {
    case QMI_CAT_SEND_TR_IND:
      it = mTrMsgPtr.find(((cat_send_tr_ind_msg_v02 *)ind_data_ptr)->indication_token);

      if (it != mTrMsgPtr.end())
      {
        cat_send_tr_ind_msg_v02 * tr_ind_ptr    = (cat_send_tr_ind_msg_v02 *)ind_data_ptr;
        auto                      resp_data_ptr = std::make_shared<qmi_cat_rsp_data_type>();

        if (resp_data_ptr != nullptr)
        {
          resp_data_ptr->rsp_id     = QMI_CAT_SRVC_SEND_TERMINAL_RESPONSE_RSP_MSG;
          resp_data_ptr->transp_err = QMI_NO_ERR;
          resp_data_ptr->qmi_err    = tr_ind_ptr->resp.error;
          ...
          if (tr_ind_ptr->resp.result == QMI_RESULT_SUCCESS_V01)
          {
                ...
                (tr_ind_ptr->tr_response_data.tr_response_len <= QMI_CAT_TERMINAL_RESPONSE_MAX_LENGTH))
            {
              ...
              resp_data_ptr->rsp_data.tr_rsp_data.sw1 = tr_ind_ptr->tr_response_data.sw1;
              resp_data_ptr->rsp_data.tr_rsp_data.sw2 = tr_ind_ptr->tr_response_data.sw2;
            }
          }
        }

        auto  resp_msg_ptr  = std::make_shared<GstkQmiCatResponseMsg>(resp_data_ptr, it->second);
    ...
    case QMI_CAT_SEND_EVENLOPE_CMD_IND:
      it = mEnvList.find(((cat_send_envelope_cmd_ind_msg_v02 *)ind_data_ptr)->indication_token);
      ...
      if (it != mEnvList.end())
      {
        ...
        if (resp_data_ptr != nullptr)
        {
          resp_data_ptr->rsp_id = QMI_CAT_SRVC_SEND_ENVELOPE_COMMAND_RSP_MSG;
          resp_data_ptr->transp_err = QMI_NO_ERR;
          resp_data_ptr->qmi_err    = env_ind_ptr->resp.error;
          ...
          if (env_ind_ptr->resp.result == QMI_RESULT_SUCCESS_V01)
          {
            if (env_ind_ptr->env_resp_data_valid)
            {
              ...
              resp_data_ptr->rsp_data.env_rsp_data.sw1 = env_ind_ptr->env_resp_data.sw1;
              resp_data_ptr->rsp_data.env_rsp_data.sw2 = env_ind_ptr->env_resp_data.sw2;
            }
          }
        }
    ...
    case QMI_CAT_EVENT_REPORT_IND:
      ...
      if (!mTrMsgPtr.empty())
      {
        for (auto it = mTrMsgPtr.begin(); it != mTrMsgPtr.end();)
        {
          auto  resp_msg_ptr  = std::make_shared<GstkQmiCatResponseMsg>(nullptr, it->second);

          if (resp_msg_ptr != NULL)
          {
            resp_msg_ptr->dispatch();
          }
          it = mTrMsgPtr.erase(it);
        }
      }
      qmi_cat_process_event_report_ind(
...
(
  std::shared_ptr<QmiCatSetupRequestSync> msg
)
{
  ...
  if (mServiceObject == nullptr)
  {
    ...
    if (rc == QMI_NO_ERR)
    {
      rc = qmi_client_register_notify_cb(mNotifyHandle, CatModemEndPointModule::onQmiNotifyCb, this);
      if (rc != QMI_NO_ERR)
      {
...
)
{
  ...
  for (auto it = CatModemEndPointModule::mTrMsgPtr.begin(); it != CatModemEndPointModule::mTrMsgPtr.end();)
  {
    auto  resp_msg_ptr  = std::make_shared<GstkQmiCatResponseMsg>(nullptr, it->second);
    ...
    it = CatModemEndPointModule::mTrMsgPtr.erase(it);
  }

  for (auto it = CatModemEndPointModule::mEnvList.begin(); it != CatModemEndPointModule::mEnvList.end();)
  {
    auto  resp_msg_ptr  = std::make_shared<GstkQmiCatResponseMsg>(nullptr, it->second);
    ...
    it = CatModemEndPointModule::mEnvList.erase(it);
  }
...
)
{
  ...
  qmi_client_recv_msg_async_cb      cb_ptr    = NULL;
  ...
  do
  {
    if (mOwner->getState() != ModemEndPoint::State::OPERATIONAL ||
        respPtr == NULL ||
        (msg_ptr->get_rsp_data() == NULL &&
         msg_ptr->get_user_data() == NULL))
    {
      QCRIL_LOG_ERROR(" Failed to handle request resp: %p, req: 0x%x", respPtr.get(), msg_ptr.get());
      break;
    }

    if (msg_ptr->get_rsp_data() == NULL &&
        msg_ptr->get_user_data() != NULL)
    {
      cb_ptr = CatModemEndPointModule::onQmiAsyncCb;
    }
    ...
    *respPtr = qmi_cat_process_qcril_gstk_request(mQmiSvcClient,
                                                  ...
                                                  msg_ptr->get_message(),
                                                  cb_ptr,
                                                  msg_ptr->get_user_data(),
                                                  msg_ptr->get_rsp_data());
  } while(0);

  msg_ptr->sendResponse(msg_ptr, status, respPtr);
} /* CatModemEndPointModule::handleQcrilGstkRequest */


/*===========================================================================
  FUNCTION  cleanUpQmiSvcClient - Cleanup QMI cat clients
===========================================================================*/
...
} /* CatModemEndPointModule::cleanUpQmiSvcClient */
