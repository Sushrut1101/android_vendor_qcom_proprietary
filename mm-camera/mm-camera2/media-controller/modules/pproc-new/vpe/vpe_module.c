...
static vpe_module_ctrl_t* vpe_module_create_vpe_ctrl(void)
{
  ...
  mct_queue_t *q;
  int32_t rc;
  ctrl = (vpe_module_ctrl_t *) CAM_MALLOC(sizeof(vpe_module_ctrl_t));
  if(!ctrl) {
    CDBG_ERROR("%s:%d, CAM_MALLOC failed", __func__, __LINE__);
    return NULL;
  }
  memset(ctrl, 0x00, sizeof(vpe_module_ctrl_t));

  /* create real-time queue */
  ctrl->realtime_queue.q = (mct_queue_t*) CAM_MALLOC(sizeof(mct_queue_t));
  ...
  ctrl->offline_queue.q = (mct_queue_t*) CAM_MALLOC(sizeof(mct_queue_t));
...
void vpe_module_set_mod(mct_module_t *module, mct_module_type_t module_type,
  uint32_t identity)
{
...
boolean vpe_module_start_session(mct_module_t *module, uint32_t sessionid)
{
  int32_t rc;
    ...
    if(ctrl->session_params[i] == NULL) {
      ctrl->session_params[i] =
        (vpe_module_session_params_t*)
           CAM_MALLOC(sizeof(vpe_module_session_params_t));
...
boolean vpe_module_stop_session(mct_module_t *module, uint32_t sessionid)
{
  int32_t rc;
  if(!module) {
    CDBG_ERROR("%s:%d, failed", __func__, __LINE__);
  ...
  mct_event_t* event)
{
  ...
  if(unproc_required == TRUE) {
    vpe_event[event_idx] = (vpe_module_event_t*)
                              CAM_MALLOC(sizeof(vpe_module_event_t));
    if(!vpe_event[event_idx]) {
      CDBG_ERROR("%s:%d, CAM_MALLOC() failed\n", __func__, __LINE__);
      PTHREAD_MUTEX_UNLOCK(&(stream_params->mutex));
  ...
  mct_event_t* event)
{
  ...
  hw_strm_buff_info.buffer_info = (vpe_hardware_buffer_info_t *)CAM_MALLOC(
  ...
  vpe_module_ack_key_t key, int32_t buf_dirty, int32_t ref_count)
{
  ...
  vpe_module_ack_t *vpe_ack =
    (vpe_module_ack_t *) CAM_MALLOC (sizeof(vpe_module_ack_t));
  if(!vpe_ack) {
    CDBG_ERROR("%s:%d, CAM_MALLOC failed\n", __func__, __LINE__);
    return -ENOMEM;
  }
  ...
  mct_event_t* event)
{
  ...
  case MCT_EVENT_MODULE_EVENT: {
    ...
    case MCT_EVENT_MODULE_IFACE_REQUEST_PP_DIVERT: {
      CDBG_LOW("%s:%d: MCT_EVENT_MODULE_IFACE_REQUEST_PP_DIVERT: identity=0x%x"
               , __func__, __LINE__, identity);
          ...
          if(ctrl->session_params[i]->stream_params[j] == NULL) {
            ctrl->session_params[i]->stream_params[j] =
              (vpe_module_stream_params_t *)
                 CAM_MALLOC (sizeof(vpe_module_stream_params_t));
