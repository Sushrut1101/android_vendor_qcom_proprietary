...
class SyncApiSession {
 public:
  ...
  class CallbackSync : public GenericCallback<R> {
   ...
   public:
    ...
    inline void onResponse(std::shared_ptr<Message> solicitedMsg,
      ...
      std::lock_guard<qtimutex::QtiSharedMutex> lock(mOwner.mSyncApiMutex);
      (void)solicitedMsg;
  ...
  inline void waitForSyncApiResponse() {
    std::unique_lock<qtimutex::QtiSharedMutex> mylock(mSyncApiMutex);
  ...
  inline void notifyTimeout() {
    std::lock_guard<qtimutex::QtiSharedMutex> lock(mSyncApiMutex);
    misPeerResponded = true;
  ...
  qtimutex::QtiSharedMutex mSyncApiMutex;
  std::condition_variable_any mSyncApiCv;
