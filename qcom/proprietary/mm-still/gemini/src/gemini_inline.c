...
gemini_params_t gemini_params = {
	...
	QCAMERA_DUMP_FRM_LOCATION"test.yuv", // char *input_yuv_filename;
	QCAMERA_DUMP_FRM_LOCATION"Inline", // char *output_bs_filename;
...
void gemini_app_output_buf_enq (gmn_obj_t gmn_obj)
{
	...
	output_buf[i].alloc_ion.heap_id_mask = 0x1 << ION_IOMMU_HEAP_ID;
  ...
  struct gemini_buf *buf)
{
  ...
  if (input_p != NULL && input_p->y_len != 0) {
    ...
    input_p->alloc_ion.heap_id_mask = 0x1 << ION_IOMMU_HEAP_ID;
