...
typedef struct
{
  ...
  OS_EVENT *sns_time_clnt_mutex; /* Mutext protecting the sns_time_clients */
...
sns_time_calc_thread( void *p_arg )
{
  ...
  sns_time_client *clnt_l_head = NULL;
  uint8_t os_err;

  /*Need to protect g_time_control->time_clients b/w delete and this thread
   sns_time_clnt_mutex can't be aquired for sns_time_mr_send_ind , it makes
   dead lock in QMI.send_ind and sns_time_mr_csi_disconnect holds same
   resource lock in QMI*/

  sns_os_mutex_pend( g_time_control->sns_time_clnt_mutex, 0, &os_err );
  if( OS_ERR_NONE != os_err )
  {
    SNS_PRINTF_STRING_ERROR_1( SNS_DBG_MOD_APPS_TIME,
                               "Error acquiring clnt mutex %i", os_err );
  }
  ...
  while( NULL != client )
  {
    sns_time_client *clnt_l = SNS_OS_MALLOC(SNS_DBG_MOD_APPS_TIME, sizeof(sns_time_client));
    if(clnt_l != NULL) {
      clnt_l->client_hndl = client->client_hndl;
      clnt_l->next = NULL;
      if(clnt_l_head == NULL) {
        clnt_l_head = clnt_l;
      } else {
        clnt_l->next = clnt_l_head;
        clnt_l_head = clnt_l;
      }
    }
    client = client->next;
  ...
  sns_os_mutex_post(g_time_control->sns_time_clnt_mutex);
  client = clnt_l_head;
  while( NULL != client )
  {
    sns_time_mr_send_ind( client->client_hndl );
    sns_time_client *clnt_ptr = client;
    client = client->next;
    SNS_OS_FREE(clnt_ptr);
  }
}
...
sns_time_init()
{
  ...
  if( NULL == g_time_control )
  {
    ...
    return (sns_err_code_e)SENSOR1_ENOMEM;
  }
  ...
  g_time_control->sns_time_clnt_mutex = sns_os_mutex_create( SNS_TIME_APPS_MUTEX, &os_err );
  if( OS_ERR_NONE != os_err )
  {
    SNS_PRINTF_STRING_FATAL_0( SNS_DBG_MOD_APPS_TIME, "Clnt Mutex create failed" );
    sns_em_delete_timer_obj( g_time_control->sns_time_calc_tmr );
    // Delete time mutex
    sns_os_mutex_del( g_time_control->sns_time_mutex, 0, &os_err );
    if( 0 != os_err )
    {
      SNS_PRINTF_STRING_FATAL_1( SNS_DBG_MOD_APPS_TIME, "can't delete time mutex %i", os_err );
    }
    SNS_OS_FREE( g_time_control );
    return SNS_ERR_FAILED;
  }
  ...
  {
    SNS_PRINTF_STRING_ERROR_1( SNS_DBG_MOD_APPS_TIME,
                               "Failed to open B family drivers, err=%d",
                               errno );
    return SNS_ERR_FAILED;
  }
#else
...
sns_time_deinit( void )
{
  ...
  if( NULL != g_time_control )
  {
    ...
    // Delete time client mutex
    sns_os_mutex_del( g_time_control->sns_time_clnt_mutex, 0, &err );
    if( 0 != err )
    {
      SNS_PRINTF_STRING_FATAL_1( SNS_DBG_MOD_APPS_TIME, "can't delete mutex %i", err );
    }
