...
static boolean module_hdr_port_set_meta_event(mct_port_t *port,
...
static boolean module_hdr_port_up_buf_divert_ack_event(mct_port_t *port,
  mct_event_t *event, boolean *forward_event);
static boolean module_hdr_port_qry_dvrt_event(mct_port_t *port,
  mct_event_t *event, boolean *forward_event);
static boolean module_hdr_port_stats_aec_update_event(mct_port_t *port,
  mct_event_t *event, boolean *forward_event);
static boolean module_hdr_port_stats_awb_update_event(mct_port_t *port,
  mct_event_t *event, boolean *forward_event);
static boolean module_hdr_port_isp_awb_update_event(mct_port_t *port,
  mct_event_t *event, boolean *forward_event);
...
static boolean module_hdr_port_stream_buf_event(mct_port_t *port,
  mct_event_t *event, boolean *forward_event);
static boolean module_hdr_port_set_parm_event(mct_port_t *port,
  mct_event_t *event, boolean *forward_event);
static boolean module_hdr_dump_pproc_meta(mct_port_t *port,
 ...
 *
 **/
#define MODULE_HDR_PROPERTY_IN_OUT_DUMP_ENABLE "inout"
...
static module_hdr_port_event_lut_entry_t module_hdr_port_event_lut[] = {
  { { MCT_EVENT_MODULE_SET_META_PTR }, module_hdr_port_set_meta_event },
  ...
  { { MCT_EVENT_MODULE_QUERY_DIVERT_TYPE }, module_hdr_port_qry_dvrt_event },
  { { MCT_EVENT_MODULE_STATS_AEC_UPDATE }, module_hdr_port_stats_aec_update_event},
  { { MCT_EVENT_MODULE_STATS_AWB_UPDATE }, module_hdr_port_stats_awb_update_event},
  { { MCT_EVENT_MODULE_ISP_AWB_UPDATE }, module_hdr_port_isp_awb_update_event},
...
static module_hdr_port_event_lut_entry_t module_hdr_port_cmd_lut[] = {
  ...
  { { MCT_EVENT_CONTROL_SET_PARM }, module_hdr_port_set_parm_event },
// Dummy entry. It needs to be always at the end of the LUT
 ...
 *  @rotation: rotation that needs to be applied
 *
  ...
  if (CAM_FORMAT_YUV_420_NV21 == info->fmt) {
    ...
    if (buf_divert->native_buf) {
      ...
      if (img_buf) {
        ...
        fd = img_buf->buf_planes[0].fd;
      }
    }

    if (ptr) {
      img_frame->frame[0].plane_cnt =
        (uint32_t)info->buf_planes.plane_info.num_planes;

      for (i = 0; i < img_frame->frame[0].plane_cnt; i++) {

        img_frame->frame[0].plane[i].fd = fd;
        img_frame->frame[0].plane[i].plane_type = i;

        if ((0 == rotation) || (180 == rotation)) {
          img_frame->frame[0].plane[i].stride =
             (uint32_t)info->buf_planes.plane_info.mp[i].stride;
          img_frame->frame[0].plane[i].scanline =
            (uint32_t)info->buf_planes.plane_info.mp[i].scanline;
          img_frame->frame[0].plane[i].width = (uint32_t)info->dim.width;
          img_frame->frame[0].plane[i].height = (uint32_t)info->dim.height;
        } else {
          img_frame->frame[0].plane[i].stride =
             (uint32_t)info->buf_planes.plane_info.mp[i].scanline;
          img_frame->frame[0].plane[i].scanline =
            (uint32_t)info->buf_planes.plane_info.mp[i].stride;
          img_frame->frame[0].plane[i].width = (uint32_t)info->dim.height;
          img_frame->frame[0].plane[i].height = (uint32_t)info->dim.width;
          if (CAM_FORMAT_YUV_420_NV21 == info->fmt) {
            img_frame->frame[0].plane[i].scanline /= (i + 1);
            img_frame->frame[0].plane[i].stride *= (i + 1);
          }
        }
        ...
        img_frame->frame[0].plane[i].length =
          ...
          * (uint32_t)img_frame->frame[0].plane[i].stride;

        if (img_buf) {
          /*
           * Offset refers to data offset from each plane base.
           * MCT sends length of each plane and offset separately.
           * Use length of each plane along with offset for
           * correct data offset.
           */
          img_frame->frame[0].plane[i].offset =
            (((uint32_t)img_buf->buf_planes[i].offset) + len);
          len += (uint32_t)img_buf->buf_planes[i].size;
        } else {
      ...
      img_frame->timestamp = (uint64_t)buf_divert->buffer.timestamp.tv_sec * 1000000
        + (uint64_t)buf_divert->buffer.timestamp.tv_usec;

      img_frame->frame_cnt = 1;
      img_frame->info.width = (uint32_t)info->dim.width;
      img_frame->info.height = (uint32_t)info->dim.height;
  ...
  int32_t rotation)
{
  ...
  if (CAM_FORMAT_YUV_420_NV21 == info->fmt) {
    img_frame->frame[0].plane_cnt = (uint32_t)mct_stream_map_buf->num_planes;

    for (i = 0; i < (uint32_t)mct_stream_map_buf->num_planes; i++) {
      img_frame->frame[0].plane[i].plane_type = i;
      img_frame->frame[0].plane[i].addr = mct_stream_map_buf->buf_planes[i].buf;

      if ((0 == rotation) || (180 == rotation)) {
        img_frame->frame[0].plane[i].stride =
          (uint32_t)mct_stream_map_buf->buf_planes[i].stride;
        img_frame->frame[0].plane[i].scanline =
          (uint32_t)mct_stream_map_buf->buf_planes[i].scanline;
        img_frame->frame[0].plane[i].height = (uint32_t)info->dim.height;
        img_frame->frame[0].plane[i].width = (uint32_t)info->dim.width;
      } else {
        img_frame->frame[0].plane[i].stride =
      ...
      img_frame->frame[0].plane[i].length = img_frame->frame[0].plane[i].height
        * img_frame->frame[0].plane[i].stride;
      img_frame->frame[0].plane[i].offset =
        (uint32_t)mct_stream_map_buf->buf_planes[i].offset;
    }

    gettimeofday(&timestamp, NULL);
    img_frame->timestamp = (uint64_t)timestamp.tv_sec * 1000000 +
    ...
    img_frame->info.analysis = 0;
    img_frame->idx = (uint32_t)mct_stream_map_buf->buf_index;
 ...
 * Returns Handler to mapped buffer or NULL
 **/
static boolean __unused module_hdr_port_find_stream(void *list_data, void *user_data)
{
  boolean ret_val = FALSE;
  ...
  if (buf_divert && port && port->port_private && MCT_PORT_PARENT(port) &&
    ...
    if (ret_val) {
      ...
      if (ret_val->img_frame) {
        ...
        ret_val->buffer_access = buf_divert->buffer_access;
        ...
        if (buf_divert->native_buf) {
          ...
          stream_info =
            (mct_stream_info_t *)mct_module_get_stream_info(module,
              ...
              (int32_t)info->reprocess_config.online.input_stream_id);
        ...
        if (stream_info) {
            ...
            buf_divert, stream_info, private_data->rotation);
    ...
    // pproc needs buf divert ack flag to be 0
    // for indication that there is no piggy back ack
    buf_divert->ack_flag = FALSE;
  } else {
      ...
      if (stream_info) {
        ...
        if (list && list->data) {
          if (module_hdr_port_fill_buffer_handler(ret_val->img_frame,
            list->data, stream_info, private_data->rotation)) {
      ...
      if (ret_val->img_frame) {
        ...
        if (ret >= 0) {
          ...
          if (list && list->data) {
            ...
            ret_val->is_native = FALSE;
#ifdef CACHE_PHASE2
            ret_val->buffer_access = buff.flags;
#else
            // For now assume the buffers are clean
            ret_val->buffer_access = 0;
#endif

            if (module_hdr_port_fill_buffer_handler(ret_val->img_frame,
                list->data, info, private_data->rotation)) {
              done = TRUE;
            } else {
              IDBG_ERROR("Unsupported buffer in %s", __func__);
            }
...
static boolean module_hdr_port_release_buffer(module_hdr_buf_t **buffer,
  uint32_t identity, boolean buff_done, int ion_fd, mct_port_t *port)
{
  ...
  if (buffer && *buffer && identity) {
    ...
    if (buff->img_frame) {
       if (!buff->is_native) {
        ...
        buff_info.timestamp.tv_sec =
          (int32_t)(buff->img_frame->timestamp / 1000000);
        buff_info.timestamp.tv_usec =
          (int32_t)((int64_t)buff->img_frame->timestamp
...
#ifdef CACHE_PHASE2
          buff_info.flags = buff->buffer_access;
#else
          img_common_handle_bufdone_frame_cache_op(
            buff->img_frame, ion_fd, &buff->buffer_access);
#endif
          ret = module_hdr_dump_pproc_meta(port, buff);
          if (!ret) {
           IDBG_ERROR("Failed to dump pproc meta data in %s", __func__);
          }
        } else {
  ...
  module_hdr_buf_t *buff)
{
  ...
  if (buff && port && port->port_private && MCT_PORT_PARENT(port) ) {
    ...
    isp_buf_divert.buffer.length = (uint32_t)buff->img_frame->frame[0].plane_cnt;
    ...
    isp_buf_divert.buffer_access = buff->buffer_access;
  ...
  if (out_buff && in_buff && port && port->port_private && out_crop &&
    ...
    for (i = 0; i < HDR_LIB_OUT_BUFFS + HDR_LIB_INPLACE_BUFFS; i++) {
      ...
      if (private_data->dump_output_frame) {
        img_dump_frame(private_data->out_buff[i]->img_frame,
          "hdr_output", i, NULL);
        private_data->out_buff[i]->buffer_access |= CPU_HAS_READ;
      }
      ...
      if ((int32_t)i >= HDR_LIB_OUT_BUFFS) {
      ...
      if (send_frame_to_next_module) {
        ...
        module_hdr_port_release_buffer(&private_data->out_buff[i],
          private_data->reserved_identity, TRUE, private_data->ion_fd, port);
      }
 ...
 **/
static boolean __unused module_hdr_port_negative_check_linked_peer(void *data,
  void *user_data)
{
  boolean ret_val = FALSE;
  mct_port_t *port = (mct_port_t *)data;
  ...
  mct_event_t *event, boolean *forward_event)
{
  ...
  if (port && event && port->port_private &&
    ...
    rc = module_imglib_common_meta_set_gamma(&private_data->meta_data_list,
      event->u.module_event.module_event_data,
...
/** module_hdr_port_qry_dvrt_event:
 *  @port: the Port to handle the event
 *  @event: the Event to be handled
 *  @forward_event: output flag indicating whether
 *                  event need to be forwarded to the peer
...
/** module_hdr_port_stats_aec_update_event:
 *  @port: the Port to handle the event
 *  @event: the Event to be handled
 *  @forward_event: output flag indicating whether
 *                  event need to be forwarded to the peer
 *
 * Function to handle aec update event for the port
 *
 * Returns TRUE in case of success
 **/
static boolean module_hdr_port_stats_aec_update_event(mct_port_t *port,
  mct_event_t *event, boolean *forward_event)
{
  ...
  if (port && event && port->port_private &&
    ...
    rc = module_imglib_common_meta_set_aec(&private_data->meta_data_list,
      event->u.module_event.module_event_data,
      event->u.module_event.current_frame_id);
...
/** module_hdr_port_stats_awb_update_event:
 *  @port: the Port to handle the event
 *  @event: the Event to be handled
 *  @forward_event: output flag indicating whether
 *                  event need to be forwarded to the peer
  ...
  mct_event_t *event, boolean *forward_event)
{
  ...
  if (port && event && port->port_private &&
    ...
    if (stats_update->awb_update.awb_dst == STATS_UPDATE_DST_ALL) {
      ...
      rc = module_imglib_common_meta_set_awb(&private_data->meta_data_list,
        event->u.module_event.module_event_data,
        event->u.module_event.current_frame_id);

      ret_val = GET_STATUS(rc);
    } else {
      IDBG_ERROR("%s:%d] skipping AWB update due to the dst flag", __func__, __LINE__);
    }
  } else {
    IDBG_ERROR("%s:%d] Null pointer detected\n", __func__, __LINE__);
  }
...
/** module_hdr_port_isp_awb_update_event:
 *  @port: the Port to handle the event
 *  @event: the Event to be handled
 *  @forward_event: output flag indicating whether
 *                  event need to be forwarded to the peer
  ...
  mct_event_t *event, boolean *forward_event)
{
  ...
  if (port && event && port->port_private) {
    ...
    //reset previous rotation setting
    private_data->rotation = 0;

#ifdef _ANDROID_
    property_get("persist.vendor.camera.imglib.hdr.dump",
      value,
      MODULE_HDR_PROPERTY_DUMP_DISABLE);
  ...
  mct_event_t *event, boolean *forward_event)
{
  ...
  if (port && event && port->port_private) {
    ...
    mct_list_traverse(private_data->config_list, module_imglib_free_func, NULL);
    ...
    mct_list_free_all(private_data->meta_data_list, module_imglib_free_func);
    private_data->meta_data_list = NULL;

    mct_list_free_list(private_data->hal_meta_data_list);
    private_data->hal_meta_data_list = NULL;
    ...
    for (i = 0; i < IMGLIB_ARRAY_SIZE(private_data->in_buff); i++) {
      ...
      module_hdr_port_release_buffer(&private_data->in_buff[i],
        event->identity, FALSE, private_data->ion_fd, port);
    }
      ...
      if (stream && private_data->stream_on) {
        ...
        if (0 == private_data->hdr_burst_counter) {
          private_data->hdr_need_1x =
            (boolean)info->reprocess_config.pp_feature_config.hdr_param.hdr_need_1x;
            ...
            if (private_data->in_buff[hdr_lib_buf_idx]) {
              ...
              if (private_data->dump_input_frame) {
                img_dump_frame(
                  ...
                  "hdr_input", hdr_lib_buf_idx, NULL);
                private_data->in_buff[hdr_lib_buf_idx]->buffer_access |=
                  CPU_HAS_READ;
              }

              img_common_handle_input_frame_cache_op(private_data->exec_mode,
                private_data->in_buff[hdr_lib_buf_idx]->img_frame,
                private_data->ion_fd, private_data->force_cache_op,
                &private_data->in_buff[hdr_lib_buf_idx]->buffer_access);
              ...
              if (hdr_lib_buf_idx == private_data->hdr_port_in_buffs - 1) {
                ...
                for (i = 0; i < HDR_LIB_OUT_BUFFS+HDR_LIB_INPLACE_BUFFS; i++) {
                  ...
                  img_common_handle_output_frame_cache_op(private_data->exec_mode,
                    private_data->out_buff[i]->img_frame, private_data->ion_fd,
                    private_data->force_cache_op,
                    &private_data->out_buff[i]->buffer_access);
                ...
                if (private_data->hdr_need_1x) {
                  ...
                  if (!private_data->non_hdr_buf) {
                    ...
                    img_common_handle_output_frame_cache_op(private_data->exec_mode,
                      private_data->non_hdr_buf->img_frame, private_data->ion_fd,
                      private_data->force_cache_op,
                      &private_data->non_hdr_buf->buffer_access);
                  }
                } else {
                ...
                rc = module_imglib_common_add_imglib_debug_meta(
                  private_data->hal_meta_data_list,
                  private_data->meta_data_list);
                if (IMG_ERROR(rc)) {
                  IDBG_ERROR("Cannot add imglib debug in %s\n", __func__);
                }

                if (ret_val) {
                  ret_val = module_hdr_lib_process(private_data->lib_instance,
                    private_data->out_buff, private_data->in_buff,
                    private_data->meta_data_list, port,
                    module_hdr_port_notify_cb);
                }
              }
...
/** module_hdr_port_set_parm_event:
 *  @port: the Port to handle the event
 *  @event: the Event to be handled
 *  @forward_event: output flag indicating whether
 *                  event need to be forwarded to the peer
 *
 * Function to handle set param event for the port
 *
 * Returns TRUE in case of success
 **/
static boolean module_hdr_port_set_parm_event(mct_port_t *port,
  mct_event_t *event, boolean *forward_event)
{
  boolean ret_val = FALSE;
  module_hdr_port_t *private_data;
  mct_event_control_parm_t *parm_buf;
  ...
  if (event && event->u.ctrl_event.control_event_data && port &&
    ...
    if (CAM_INTF_PARM_ROTATION == parm_buf->type) {
      if (parm_buf->parm_data) {
        cam_rotation_info_t *rotation_info = (cam_rotation_info_t *)parm_buf->parm_data;
        switch (rotation_info->rotation) {
        case ROTATE_0:
          private_data->rotation = 0;
          break;
        case ROTATE_90:
          private_data->rotation = 90;
          break;
        case ROTATE_180:
          private_data->rotation = 180;
          break;
        case ROTATE_270:
          private_data->rotation = 270;
          break;
        default:
          IDBG_ERROR("Not supported rotation in %s\n", __func__);
          ret_val = FALSE;
          break;
        }
      } else {
        IDBG_ERROR("Null pointer detected for param %d in %s\n", parm_buf->type,
          __func__);
        ret_val = FALSE;
      }
    }
...
/**
 * Function: module_hdr_dump_pproc_meta
 *
 * Description: function called to dump hdr pproc metadata
 *
 * Arguments:
 *   @port: mct port handler
 *   @buff: image buffer handler
 *
 * Returns TRUE in case of success
 **/
static boolean module_hdr_dump_pproc_meta(mct_port_t *port, module_hdr_buf_t *buff)
{
  boolean ret_val = FALSE;
  module_hdr_port_t *private_data;
  mct_event_t event;
  ...
  private_data = port->port_private;
  metadata_dump.frame_id = buff->img_frame->frame_id;;
  metadata_dump.meta_data = buff->meta_data;
  event.identity = private_data->reserved_identity;
  event.type = MCT_EVENT_MODULE_EVENT;
  event.direction = MCT_EVENT_UPSTREAM;
  event.u.module_event.type = MCT_EVENT_MODULE_PPROC_DUMP_METADATA;
  event.u.module_event.module_event_data = &metadata_dump;

  ret_val = MCT_PORT_EVENT_FUNC(MCT_PORT_PEER(port)) (MCT_PORT_PEER(port),
      &event);
  if (!ret_val) {
    IDBG_ERROR("Cannot send MCT_EVENT_MODULE_PPROC_DUMP_METADATA in %s\n",
      __func__);
  }
  IDBG_MED("%s -", __func__);
  ...
  module_hdr_buf_t *buff, boolean buff_done)
{
  ...
  if (buff && buff->img_frame && port && port->port_private &&
    ...
    isp_buf_divert_ack.timestamp.tv_sec =
      (int32_t)(buff->img_frame->timestamp / 1000000);
    isp_buf_divert_ack.timestamp.tv_usec =
      (int32_t)((int64_t)buff->img_frame->timestamp
      - isp_buf_divert_ack.timestamp.tv_sec * 1000000);
    isp_buf_divert_ack.buffer_access = buff->buffer_access;

    IDBG_HIGH("isp_buf_divert_ack.is_buf_dirty %d",
      isp_buf_divert_ack.is_buf_dirty);
    ...
    ret_val = module_hdr_dump_pproc_meta(port, buff);
    if (!ret_val) {
      IDBG_ERROR("Cannot dump pproc meta data in %s\n",
        __func__);
    }
    event.identity = private_data->reserved_identity;
    event.type = MCT_EVENT_MODULE_EVENT;
  ...
  if (out_buff && in_buff && port && port->port_private && out_crop &&
    ...
    for (i = 0; i < HDR_LIB_OUT_BUFFS + HDR_LIB_INPLACE_BUFFS; i++) {
      ...
      if (found) {
        ...
        if (private_data->dump_output_frame) {
          img_dump_frame(out_buff[i]->img_frame, "hdr_output", i, NULL);
          out_buff[i]->buffer_access |= CPU_HAS_READ;
        }
...
/** module_hdr_port_set_meta_event:
 *  @port: the Port to handle the event
 *  @event: the Event to be handled
 *  @forward_event: output flag indicating whether
 *                  event need to be forwarded to the peer
 *
 * Function to handle set meta event for the port
 *
 * Returns TRUE in case of success
 **/
static boolean module_hdr_port_set_meta_event(mct_port_t *port,
  mct_event_t *event, boolean *forward_event)
{
  ...
  if (port && event && port->port_private) {
    ...
    private_data->hal_meta_data_list = mct_list_append(
      private_data->hal_meta_data_list, event->u.module_event.module_event_data,
      NULL, NULL);
          ...
          if (private_data->in_buff[hdr_lib_buf_idx]) {
            ...
            if (private_data->dump_input_frame) {
              rc = module_imglib_common_meta_find_by_frameid(
                private_data->in_buff[hdr_lib_buf_idx]->img_frame->frame_id,
                private_data->meta_data_list, &p_meta);

              if (rc != IMG_SUCCESS) {
                IDBG_ERROR("Cannot find meta buffer in %s\n", __func__);
                ret_val = FALSE;
              }

              img_dump_frame(
                private_data->in_buff[hdr_lib_buf_idx]->img_frame, "hdr_input",
                hdr_lib_buf_idx, p_meta);
            ...
            if (hdr_lib_buf_idx == private_data->hdr_port_in_buffs - 1) {
              ...
              for (i = 0; i < HDR_LIB_OUT_BUFFS + HDR_LIB_INPLACE_BUFFS; i++) {
                ...
                if (i < HDR_LIB_OUT_BUFFS) {
                  ...
                  if (!module_hdr_lib_get_output_inplace_index(
                    (uint32_t)i - HDR_LIB_OUT_BUFFS, &j)) {
                      IDBG_ERROR("Cannot get index of inplace buffer in %s\n",
                        __func__);
                      break;
                  }
              ...
              if (ret_val) {
                ...
                if ((private_data->non_hdr_extra_buf_needed || native_input)
                  && private_data->hdr_need_1x) {
...
void module_hdr_port_deinit(mct_port_t *port)
{
  ...
  if (port && port->port_private && MODULE_HDR_VALIDATE_NAME(port)) {
    ...
    if (private_data->ion_fd >= 0) {
      img_buffer_close();
      private_data->ion_fd = -1;
    }
  ...
  uint32_t *sessionid, void* lib_handle)
{
  ...
  module_hdr_port_t *private_data;
  mct_pipeline_cap_t caps;
  mct_pipeline_imaging_cap_t* img_caps = &caps.imaging_cap;
  uint32_t i;
    ...
    if (MODULE_HDR_VALIDATE_NAME(port)) {
      ...
      if (private_data) {
        ...
        if (private_data->lib_instance) {
          ...
          private_data->state = MODULE_HDR_PORT_STATE_CREATED;
          private_data->exec_mode = IMG_EXECUTION_SW;
          private_data->ion_fd = img_buffer_open();
          private_data->force_cache_op = FALSE;
          ...
          if (module_hdr_lib_query_mod(&caps)) {
            ...
            for (i=0; i<img_caps->hdr_bracketing_setting.num_frames; i++) {
              if (!img_caps->hdr_bracketing_setting.exp_val.values[i]) {
