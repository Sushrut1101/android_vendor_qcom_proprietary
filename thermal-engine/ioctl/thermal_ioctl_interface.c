...
int thermal_ioctl_get_cluster_voltage_plan(int cluster_num,
				uint32_t *arr, uint32_t *len)
{
	...
	if (ioctl_fd < 0) {
		msg("Thermal IOCTL not initialized\n");
		ret = -1;
		goto VOLTAGE_PLAN_EXIT;
	}
	init_msm_thermal_ioctl(&query_req);
	query_req.voltage.cluster_num = (uint32_t)cluster_num;

	ret = ioctl(ioctl_fd, (int)MSM_THERMAL_GET_CLUSTER_VOLTAGE_PLAN, &query_req);
	if (ret) {
		msg("Error in voltage plan len query for cluster[%d].err %d\n",
			cluster_num, ret);
		goto VOLTAGE_PLAN_EXIT;
	}
	if (!*len) {
		*len = query_req.voltage.voltage_table_len;
		goto VOLTAGE_PLAN_EXIT;
	}
	if (arr == NULL) {
		ret = -1;
		msg("Null pointer for voltage array for cluster[%d].ret %d\n",
			cluster_num, ret);
		goto VOLTAGE_PLAN_EXIT;
	}

	set_idx = query_req.voltage.voltage_table_len / MSM_IOCTL_FREQ_SIZE;
	if (query_req.voltage.voltage_table_len % MSM_IOCTL_FREQ_SIZE)
		set_idx++;

	while (set_idx--) {
		query_req.voltage.set_idx = set_idx;
		ret = ioctl(ioctl_fd, (int)MSM_THERMAL_GET_CLUSTER_VOLTAGE_PLAN, &query_req);
		if (ret) {
			msg("Error in voltage plan query for cluster[%d].err %d\n",
				cluster_num, ret);
			goto VOLTAGE_PLAN_EXIT;
		}
		memcpy(&arr[len_index], query_req.voltage.voltage_table,
			query_req.voltage.voltage_table_len * sizeof(unsigned int));
		len_index += query_req.voltage.voltage_table_len;
	}
