...
{
  int i, j, k, tag_val_len, ret, size, err = 0;
  ...
  for (i = 0; (i < MCT_MAX_TAG_CMD) && (cmds[i][0] != TAG_END); i++) {
    ...
    if ( err < 0) {
      ...
      tag_val_len = cmds[i][MCT_TAG_VALUE_LENGTH_OFFSET];
      ALOGD("%s: TAG Length = %d", __func__, tag_val_len);
      for(j = 0, k = 0; j < tag_val_len; j++) {
        ret = snprintf(&tag_val[k], sizeof(tag_val)- k, "0x%02x ", cmds[i][MCT_TAG_BDADDR_OFFSET+j]);
        k=k+5;
      }
      tag_val[k+1] ={'\0'};
      ALOGD("%s: TAG Value = %s", __func__, tag_val);
    }
