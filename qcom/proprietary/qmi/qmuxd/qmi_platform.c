...
)
{
  ...
  if((signal_ptr)->clock == CLOCK_MONOTONIC)
  {
    clock_gettime(CLOCK_MONOTONIC, &wait_till_time);
    /* Set wait time seconds to current + the number of seconds needed for timeout */
    wait_till_time.tv_sec =  wait_till_time.tv_sec + (timeout_milli_secs/1000);
    wait_till_time.tv_nsec = wait_till_time.tv_nsec +  ((timeout_milli_secs % 1000) * 1000 * 1000);
  }
  else
  {
...
)
{
  ...
  qmi_connection_id_type rc = QMI_CONN_ID_INVALID;
  int conn_id = -1;
  int max_ifaces = QMI_PLATFORM_RMNET_DATA_MAX_IFACES;
  int num_forward_conn_ids = QMI_PLATFORM_NUM_FORWARD_CONN_IDS;
  ...
  *mux_id = -1;
  ...
  /* Hana55 and Kona targets support additional ifaces */
  if ((DS_TARGET_FUSION_SDA855_SDX55 == target) ||
      (DS_TARGET_FUSION_SDA865_SDX55 == target) ||
      (DS_TARGET_LITO == target) ||
      (DS_TARGET_FUSION_SDA855_SDXMARMOT == target))
  {
    max_ifaces = QMI_PLATFORM_RMNET_DATA_MAX_IFACES_EXT;
    num_forward_conn_ids = QMI_PLATFORM_NUM_FORWARD_CONN_IDS_EXT;
  }
               ...
               strlen(QMI_PLATFORM_RMNET_DATA_PREFIX)))
  {
    ...
    if (0 > conn_id || max_ifaces <= conn_id)
    {
    ...
    if( DS_TARGET_MSM8994 == target ||
        ...
        DS_TARGET_TABASCO == target ||
        DS_TARGET_MSM8996 == target ||
        DS_TARGET_MSM8998 == target ||
        DS_TARGET_ELDARION == target ||
        DS_TARGET_JACALA == target ||
        DS_TARGET_FEERO6 == target ||
        DS_TARGET_FEEROLITE6 == target ||
        DS_TARGET_MSMNILE == target ||
        DS_TARGET_SDM845 == target ||
        DS_TARGET_FUSION_SM8150_SDX50 == target ||
        DS_TARGET_STARLORD == target ||
        DS_TARGET_STARHAWK == target ||
        DS_TARGET_WARLOCK == target ||
        DS_TARGET_MSMSTEPPE == target ||
        DS_TARGET_LITO == target)
    {
    ...
    else if (DS_TARGET_MDM == target)
    {
      ...
      rc = (int) QMI_CONN_ID_RMNET_USB_0 + conn_id;
             ...
             DS_TARGET_LE_TESLA == target ||
             DS_TARGET_LE_ATLAS == target)
    {
    ...
    else if(DS_TARGET_MSM == target)
    {
      ...
      rc = (int) QMI_CONN_ID_RMNET_0 + conn_id;
             ...
             DS_TARGET_LYKAN == target ||
             DS_TARGET_FEERO == target)
    {
      /* DPM 2.0 */
      rc = QMI_CONN_ID_RMNET_0;
      *mux_id = conn_id + 1;
