...
typedef struct {
  ott_rat_enum_type_v01 rat; /* key */
  qcril_data_lce_rpt_criteria criteria; /* value */
} lqe_reporting_criteria_t;

// Update when ott_rat_enum_type_v01 adds more types
#define LQE_MAX_OTT_RAT 3
...
typedef struct
{
  ...
  /* added for threshold based reporting */
  int ul_report_enabled;
  int dl_report_enabled;
  lqe_reporting_criteria_t criteria_list[LQE_MAX_OTT_RAT];
} lqe_start_req_t;
...
struct lqe_client_config {
  ...
  lqe_qmi_service_type     lqe_qmi_service;
};
