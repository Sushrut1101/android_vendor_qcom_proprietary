...
** Type declarations
*****************************************************************************/
qmi_qmux_dpm_state_t qmi_qmux_dpm_state;
  ...
  */
  ...
  else
  {
    ...
    else
    {
      ...
      /* Handle releasing of QMUX clients */
      else if ( msg_hdr.msg_id == QMI_QMUX_IF_RELEASE_QMI_CLIENT_ID_MSG_ID )
      {

        /* Save QMI client ID and service ID for cleanup in fail case */
        msg_hdr.qmi_client_id = cmd_data.qmi_qmux_if_release_client_req.delete_client_id;
        msg_hdr.qmi_service_id = cmd_data.qmi_qmux_if_release_client_req.delete_service_id;
...
/*===========================================================================
  FUNCTION  qmi_qmux_dpm_service_init
===========================================================================*/
/*!
@brief
  Init DPM service clients

@return
  TRUE if DPM client init succeeded, FALSE otherwise
...
*/
/*=========================================================================*/
int qmi_qmux_dpm_service_init(void)
{
#ifdef FEATURE_QMUXD_DEPRECATION
  ...
  QMI_DEBUG_MSG_1("%s: ENTRY", __func__);
  /* Start clean */
  memset(&qmi_qmux_dpm_state, 0x0, sizeof(qmi_qmux_dpm_state_t));

  dpm_svc_obj = dpm_get_service_object_v01();
  if ( NULL == dpm_svc_obj)
  {
    QMI_DEBUG_MSG_0("Unable to allocate dpm service object");
    goto bail;
  }

  QMI_DEBUG_MSG_1("%s: Unregistering other transports ", __func__);
  ...
  do
  {
    memset(&dpm_os_params, 0x0,
           sizeof(dpm_os_params));
    rc = qmi_client_init_instance(dpm_svc_obj,
                                  QMI_CLIENT_INSTANCE_ANY,
                                  NULL,
                                  NULL,
                                  &dpm_os_params,
                                  QMI_QMUX_DPM_TIMEOUT,
                                  &qmi_qmux_dpm_state.dpm_clid);

    if (QMI_TIMEOUT_ERR == rc)
    {
      ++retry_counter;
      QMI_ERR_MSG_2("%s: DPM client init timed out. Retry attempt[%d]",
                    __func__, retry_counter);
    }
    else if (QMI_NO_ERR != rc)
    {
      QMI_ERR_MSG_2("%s: DPM client init failed with error code[%d].",
                    __func__, rc);
      goto bail;
    }
    else
    {
      QMI_DEBUG_MSG_1("%s: dpm_inited", __func__);
      qmi_qmux_dpm_state.dpm_inited = TRUE;
      rc = QMI_NO_ERR;
    }
  }while( QMI_TIMEOUT_ERR == rc && retry_counter < QMI_QMUX_DPM_MAX_RETRIES);

  if (QMI_QMUX_DPM_MAX_RETRIES == retry_counter)
  {
    QMI_ERR_MSG_2("%s: unable to init DPM client after max retries [%d]",
                    __func__, retry_counter);
    goto bail;
  }

bail:
  QMI_DEBUG_MSG_1("%s: EXIT", __func__);
  return rc;
#else
  return 0;
#endif
...
int qmi_qmux_process_modem_capabilities(void)
{
  ...
  cap_req = (dpm_get_supported_capabilities_req_msg_v01 *)
            malloc(sizeof(dpm_get_supported_capabilities_req_msg_v01));
  if ( NULL == cap_req)
  {
    QMI_ERR_MSG_1("%s: Out of memory.", __func__);
    goto bail;
  }
  ...
  do
  {
    if (TRUE != qmi_qmux_dpm_state.dpm_inited)
    {
      rc = qmi_qmux_dpm_service_init();
      if ( QMI_NO_ERR != rc)
      {
        QMI_ERR_MSG_1("%s: dpm service init failed.", __func__);
        break;
      }
    }

    /* Client init complete query for capabilities now */
    rc = qmi_client_send_msg_sync(qmi_qmux_dpm_state.dpm_clid,
    ...
    {
      ++retry_count;
      /* Possible SSR scenario */
      QMI_ERR_MSG_3("%s: get_supported_caps failed rc[%d] on Attempt[%d]",
                    __func__, rc, retry_count);
      qmi_client_release(qmi_qmux_dpm_state.dpm_clid);
      qmi_qmux_dpm_state.dpm_inited = FALSE;
      sleep(QMI_QMUX_DPM_TIMEOUT_SEC);
      continue;
    }

    if (QMI_RESULT_SUCCESS_V01 == cap_resp.resp.result &&
        TRUE == cap_resp.is_shim_supported_valid &&
        TRUE == cap_resp.is_shim_supported)
    {
      qmi_qmux_dpm_state.is_shim_supported = TRUE;
      QMI_DEBUG_MSG_1("%s: Shim layer supported by modem",__func__);
    }

  }while(rc != QMI_NO_ERR && QMI_QMUX_DPM_MAX_RETRIES > retry_count );

  if (QMI_NO_ERR != rc &&
      QMI_QMUX_DPM_MAX_RETRIES == retry_count )
  {
    QMI_ERR_MSG_2("%s: Query modem_caps failed after [%d] attempts",
                  __func__, retry_count);
  }
bail:
  if (FALSE == qmi_qmux_dpm_state.is_shim_supported)
  {
    /*Register back qmux transports in case of shim not present */
    QMI_DEBUG_MSG_1("%s: Shim not present. Registering QMUX transports", __func__);
...
)
{
  ...
  /* Remove client in QMUX.  Only do so if the QMUX client ID is valid.  QMUX
  ...
  */
  if (txn->qmux_if_hdr.qmux_client_id != QMI_QMUX_INVALID_QMUX_CLIENT_ID)
  {
  ...
  if ( rc == QMI_NO_ERR )
  {
    QMI_DEBUG_MSG_4("qmi_ctl_handle_release_client_id_rsp: Success "
                    "qmux_client_id: %d, qmi_conn_id: %d, qmi_service_id: %d"
                    "qmi_client_id: %d",
  ...
  {
    QMI_ERR_MSG_4("qmi_ctl_handle_release_client_id_rsp: FAILED "
                  "qmux_client_id: %d, qmi_conn_id: %d, qmi_service_id: %d"
                  "qmi_client_id: %d",
                  txn->qmux_if_hdr.qmux_client_id,
                  txn->qmux_if_hdr.qmi_conn_id,
...
)
{
  ...
  switch (rx_msg_id)
  {
     ...
     case QMI_CTL_RELEASE_CLIENT_ID_MSG_ID:
       rc = qmi_ctl_handle_release_client_id_rsp (txn, &qmi_err_code);
     break;
