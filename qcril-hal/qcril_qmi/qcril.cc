...
typedef struct
{
  qtimutex::QtiSharedMutex pending_unsol_resp_mutex;
...
)
{
  ...
  do
  {
          ...
          {
            if (!qmi_ril_get_req_details_from_oem_req(
                &oem_hook_req_details,
                &audit_result,
...
{
  const char *version = "Qualcomm Technologies RIL 1.0";
  ...
  QCRIL_LOG_CF_PKT_RIL_FN( instance_id, "getVersion() - Qualcomm Technologies RIL 1.0 (QMI)" );
...
{
  qmi_ril_fw_android_request_flow_control_overview.lock_mutex.lock();
} // qmi_ril_fw_android_request_flow_control_info_lock
//===========================================================================
// qmi_ril_fw_android_request_flow_control_info_unlock
//===========================================================================
...
{
  qmi_ril_fw_android_request_flow_control_overview.lock_mutex.unlock();
} // qmi_ril_fw_android_request_flow_control_info_unlock
//===========================================================================
...
void qmi_ril_fw_android_request_flow_control_init()
{
  ...
  if ( NULL != qmi_ril_fw_android_request_flow_control_overview.android_request_kind_info )
  {
    ...
    for ( android_request_id = QMI_RIL_ZERO; android_request_id <= QMI_RIL_FW_ANDROID_REQUEST_HNDL_MAX_EVT_ID; android_request_id++ )
    {
      ...
      {
        holders = qcril_malloc2(holders, caps_nof_slots);
        if ( NULL != holders )
        {
          rec_overview->holders.extra_holders = holders;
          holder_iter = holders;
...
void qmi_ril_fw_android_request_flow_control_drop_legacy_book_records( int voice_calls_related_only, int is_unbind_cleanup )
{
  ...
  for ( android_request_id = QMI_RIL_ZERO; android_request_id <= QMI_RIL_FW_ANDROID_REQUEST_HNDL_MAX_EVT_ID; android_request_id++ )
  {
    ...
    // Do not drop reqlist entry for RIL_REQUEST_RADIO_POWER as part of SSR.
    if ( (cmp_mask & android_request_caps) && (android_request_id != RIL_REQUEST_RADIO_POWER) )
    {
...
{
  qmi_ril_android_pending_unsol_resp.pending_unsol_resp_mutex.lock();
} // qmi_ril_android_pending_unsol_resp_lock
...
{
  qmi_ril_android_pending_unsol_resp.pending_unsol_resp_mutex.unlock();
} // qmi_ril_android_pending_unsol_resp_unlock
...
void qmi_ril_init_android_unsol_resp_pending_list()
{
  ...
  qmi_ril_android_pending_unsol_resp.pending_unsol_resp_list
    = qcril_malloc2(qmi_ril_android_pending_unsol_resp.pending_unsol_resp_list,
...
void qmi_ril_add_unsol_resp_to_pending_list(qcril_unsol_resp_params_type *param_ptr)
{
  ...
  do
  {
    ...
    if ( NULL != param_ptr->logstr )
    {
      ...
      resp->logstr = qcril_malloc2(resp->logstr, len+1);
      if ( NULL != resp->logstr )
      {
