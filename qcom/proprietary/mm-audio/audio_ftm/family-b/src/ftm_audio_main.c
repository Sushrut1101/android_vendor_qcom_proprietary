...
)
{
 ...
 rsp = ftm_audio_dispatch(req_pkt, TRUE);
...
int main(int argc, char *argv[])
{
  ...
  /* analysis ==1, measure frequency and tone level withour recording */
  int tone_analysis = 1;
  /* test pass if freq diff is acceptable */
  int freq_tolerance = 0;
  int ret = 0;
    ...
    if (grp == NULL) {
        if (errno == 0) {
            printf("No group 'audio' found: %s", strerror(errno));
        } else {
            printf("Error getting gid for audio: %s", strerror(errno));
        }
        return -1;
    }
    gid_t gid_audio = grp->gr_gid;

    ret = setgid(gid_audio);
    if(ret < 0)
        printf("failed to setuid for selinux. err = %d\n", ret);
#endif
    ...
    if((fp = fopen("/proc/asound/cards","r")) == NULL) {
      ...
      return -1;
        ...
        /* skip first line due to unstable string format */
        if((fgets(soundCardInfo, sizeof(soundCardInfo), fp) == NULL)) {
            printf("/proc/asound/cards is empty. Abort\n");
            return -1;
        }
        /* read second line as it contains sound card name only. */
        while((fgets(soundCardInfo, sizeof(soundCardInfo), fp) != NULL)) {
            ...
            if (strstr(soundCardInfo, "msm8930-sitar-snd-card")) {
                ...
                total_test = 20;
                break;
            } else if ((fp_config = fopen(config_path,"r")) != NULL) {
                printf("Use the soundcard %s file\n", config_path);
                codec = CODEC_CONFIG_SUPPORT;
                fclose(fp_config);
                fp_config = NULL;
                break;
	...
	char *test_r;
        ...
        static char *myfifo = "/data/vendor/audio/ftm_commands";
        char *token = NULL;
        ...
        while(1) {
           ...
           while(token) {
               ...
               } else if (!strncmp(token, "-tv", 3)) {
                    token = strtok_r(NULL, " ", &test_r);
                     if (!token) {
                        printf("tx volume is missing");
                        break;
                     } else {
                        tx_volume = atoi(token);
                        if (tx_volume < 0 || tx_volume > 100)
                            tx_volume = DEFAULT_VOLUME;
                        printf("\n tx volume %d", tx_volume);
                     }
               } else if (!strncmp(token, "-rv", 3)) {
                    token = strtok_r(NULL, " ", &test_r);
                     if (!token) {
                        printf("rx volume is missing");
                        break;
                     } else {
                        rx_volume = atoi(token);
                        if (rx_volume < 0 || rx_volume > 100)
                            rx_volume = DEFAULT_VOLUME;
                        printf("\n rx volume %d", rx_volume);
                     }
               } else if (!strncmp(token, "-analyze", 8)) {
                    token = strtok_r(NULL, " ", &test_r);
                     if (!token) {
                        printf("analyze swtich is missing");
                        break;
                     } else {
                        tone_analysis = atoi(token);
                        printf("\n tone_analysis = %d", tone_analysis);
                     }
               } else if (!strncmp(token, "-fdiff", 6)) {
                    token = strtok_r(NULL, " ", &test_r);
                     if (!token) {
                        printf("Allowed frequency difference is missing");
                        break;
                     } else {
                        freq_tolerance= atoi(token);
                        printf("\n allowed freq diff is = %d", freq_tolerance);
                     }
               }
               token = strtok_r(NULL, " ", &test_r);
           }
    ...
    /* Config file mode */
        ...
        while((i + 1) < count) {
           if (!strncmp(argv[i], "-tc", 3)) {
                ...
                printf("\n  rx volume set to %d", rx_volume);
           } else if (!strncmp(argv[i], "-analyze", 8)) {
                i++;
                tone_analysis= atoi(argv[i++]);
                printf("\n  tone_analysis is set to %d", tone_analysis);
           } else if (!strncmp(argv[i], "-fdiff", 6)) {
                i++;
                freq_tolerance = atoi(argv[i++]);
                printf("\n  freq tolerance is set to %d", freq_tolerance);
    ...
    if (!g_config_test && !fp) {
        if (!(fp = fopen(config_path,"rb"))) {
            printf("\n Failed to open %s", config_path);
            return -1;
        }
        else
            g_config_test = 1;
