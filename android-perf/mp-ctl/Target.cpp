...
int Target::getFirstCoreIndex(int cluster) {
    if (mLastCoreIndex) {
        if (cluster == 0) {
            return 0;
        } else if (cluster >=1 && cluster < mNumCluster) {
...
void Target::mLogicalPerfMapPowerCluster() {
    int i, cluster = (mNumCluster > MIN_CLUSTERS) ? MIN_CLUSTERS - 1 : mNumCluster-1;
    int clustersToMap = (mNumCluster > MIN_CLUSTERS) ? MIN_CLUSTERS : mNumCluster;
    mPhysicalClusterMap = new int[mNumCluster];
    if(mPhysicalClusterMap) {
        ...
        for(i=MIN_CLUSTERS; i < mNumCluster; i++) {
            mPhysicalClusterMap[i] = i ;
        }
    ...
    if (fd != -1) {
        ...
        if (n > 0) {
            ...
            if (w <= 800 && h <= 1280) {
                ...
                ret = PerfDataStore::MAP_RES_TYPE_HD_PLUS;
            }
