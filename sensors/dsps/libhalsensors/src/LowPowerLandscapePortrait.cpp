...
int LowPowerLandscapePortrait::enable(int en)
{
    ...
    if (en) {
        ...
        sam_req->streaming_item[0].SampleQuality = SNS_SMGR_SAMPLE_QUALITY_ACCURATE_TIMESTAMP_V01;
    ...
    switch(msg_hdr->msg_id) {
      case SNS_SAM_EVENT_GATED_SENSOR_REPORT_IND_V01:
        ...
        HAL_LOG_DEBUG("%s: Samples_len=%d Items=%d max_reports_per_index=%d",
            ...
            (uint32_t)max_reports_per_index);
        ...
        HAL_LOG_VERBOSE("%s: LP2: X: %f Y: %f Z: %f SAM TS: %u HAL TS:%" PRId64 " elapsedRealtimeNano:%" PRId64 "",
                        __FUNCTION__, sensor_data.acceleration.x, sensor_data.acceleration.y,
                        sensor_data.acceleration.z, smgr_data->TimeStamp, sensor_data.timestamp,
