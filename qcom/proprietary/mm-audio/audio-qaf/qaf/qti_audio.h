...
typedef enum audio_event_id {
    ...
    /* This event is used by wrapper module to notify
    * the delay for each device output.
    */
    AUDIO_DELAY_EVENT,
} audio_event_id_t;

typedef enum {
    /* This event is used to notify client that a new
     * input buffer can be sent to the input stream.
     */
    AUDIO_STREAM_SEND_INPUT_BUFFER_EVENT = 1,

    /* This event is used to notify client that a new
     * input format is detected in the input stream.
     */
    AUDIO_STREAM_INPUT_MEDIA_FORMAT_EVENT = 2,
    AUDIO_STREAM_MAX_EVENT,
} audio_stream_event_id_t;

/* Payload Structure associated with AUDIO_STREAM_SEND_INPUT_BUFFER_EVENT event.*/
typedef struct {
    uint32_t bytes_available;
} audio_qaf_send_buffer_t;

/* Payload Structure associated with AUDIO_STREAM_INPUT_MEDIA_FORMAT_EVENT event.*/
typedef struct {
    uint32_t sample_rate;
    uint32_t channels;
    uint32_t bit_width;
} audio_qaf_input_media_format_t;
...
/* Payload Structure associated with AUDIO_DELAY_EVENT event.
 ...
 * */
typedef struct audio_qaf_delay {
    uint32_t algo_delay;
...
/**
 * Callback to receive event data from stream
 * @param stream_handle- stream handle
 * @param priv_adev     - Private data that client passes while registering this call. This can be used to access any client specific
                          data from the call back. For Ex: HAL can check if there are new devices connected etc.
 * @param data          - pointer to the data buffer
 ...
 * @param size          - size of the buffer
 **/
typedef void (*stream_notify_event_callback_t)(audio_stream_handle_t stream_handle, void* priv_data, void* data, audio_stream_event_id_t event_id, int size);
 ...
 *
 **/
void register_stream_event_callback(audio_stream_handle_t stream_handle, void *priv_data, stream_notify_event_callback_t event_callback);
