...
public:
    ...
    BufferInfo* getNonSecBufferInfo()
    {
        std::unique_lock<std::mutex> lock(mBufMutex);
        return &mNonSecBufferInfo;
    }
#endif
...
#ifdef ENABLE_SECURE_MEM_DUMP
    int AllocateOneNonSecureBuffer(
        uint32_t           width,
        uint32_t           height,
        uint32_t           format,
        uint64_t           producerUsageFlags,
...
void FreeNonSecureBuffers();
#endif

    buffer_handle_t     mBuffers[BUFFER_QUEUE_DEPTH];    ///< Max Buffer pool
    BufferInfo          mBufferinfo[BUFFER_QUEUE_DEPTH];
#ifdef ENABLE_SECURE_MEM_DUMP
    buffer_handle_t     mNonSecBuffer;
    BufferInfo          mNonSecBufferInfo;
    uint32_t            mNonSecBufferStride;                 ///< Buffer stride
#endif
