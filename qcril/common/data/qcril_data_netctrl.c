...
/*===========================================================================
  ...
  Copyright (c) 2008-2017, 2019 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
boolean qmi_wds_clat_query_is_supported = FALSE;
...
#define QCRIL_DATA_PROP_DONT_USE_DSD "persist.vendor.radio.dont_use_dsd"
...
extern void qcril_dataconnectionservice_register_service (int instanceId);
#endif
...
)
{
  ...
  do
  {
    ...
    if ( !VALIDATE_LOCAL_DATA_OBJ( struct_ptr) )
    {
     ...
     QCRIL_LOG_DEBUG("Debug : apn_type_mask [%d] enabled ", struct_ptr->apn_type_mask);
...
)
{
  ...
  switch (msg_id)
  {
    ...
    case QMI_DSD_CURRENT_DDS_IND_V01:
      ...
      int sub_id = (int) qcril_data_dsd_to_ril_sub_id(dsd_ind_curr_dds_msg->dds);
...
/*!
    ...
    @return
    TRUE: If ADB property persist.vendor.radio.dont_use_dsd is set to TRUE
...
boolean qcril_data_dont_use_dsd()
{
  ...
  {
    QCRIL_LOG_DEBUG("%s", "persist.vendor.radio.dont_use_dsd = true");
    return TRUE;
  }
  else
  {
    QCRIL_LOG_DEBUG("%s", "persist.vendor.radio.dont_use_dsd = false");
    return FALSE;
  }
}
...
)
{
  ...
  {
    QCRIL_LOG_INFO("%s", "DSD not used due to persist.vendor.radio.dont_use_dsd");
...
)
{
  ...
  case DSI_EVT_NET_NO_NET:
    ...
    if ( dual_ip && last_call_end_reason == PDP_FAIL_ONLY_IPV6_ALLOWED )
    {
      ipf = DSI_IP_FAMILY_V6;
      ...
      if ( dsi_get_call_end_reason(info_tbl_ptr->dsi_hndl,
                                 ...
                                 ipf ) == DSI_SUCCESS )
      {
        QCRIL_LOG_DEBUG("dual_ip dsi_get_call_end_reason returned [%d]",
                        tmp_end_reason.reason_code);
        if (SUCCESS != qcril_data_get_ril_ce_code(&tmp_end_reason,
                                                  &last_call_end_reason))
        {
...
void qcril_data_qmi_wds_init(boolean from_ssr)
{
  ...
  int ret = DSI_ERROR;
  boolean dsd_bind_supported = FALSE;
  ...
  do
  {
    ...
    qcril_data_qmi_dsd_init(&dsd_bind_supported);

    /* Call flow for data registration indications:
     *
     * If persist.vendor.radio.dont_use.dsd is TRUE
     *   Use NAS indications
    ...
    if (TRUE == qcril_data_dont_use_dsd())
    {
      QCRIL_LOG_DEBUG("%s", "ADB override for persist.vendor.radio.dont_use_dsd, using NAS indications");
    ...
    else if (NULL != global_qmi_dsd_hndl)
    {
      dsd_get_system_status_resp_msg_v01  sys_resp_msg;
      dsd_system_status_ind_msg_v01 *ind_data = NULL;
    ...
    {
      qmi_wds_clat_query_is_supported = true;
...
#ifndef QCRIL_DATA_OFFTARGET
  qcril_dataconnectionservice_register_service(global_instance_id);
#endif
...
/*===========================================================================
  ...
  FUNCTION:  qcril_data_get_dds_sub_id
...
int qcril_data_get_dds_sub_id(void)
{
  ...
  {
    if (resp.dds_valid) {
      sub_id = (int) qcril_data_dsd_to_ril_sub_id(resp.dds);
    } else {
...
*/
/*=========================================================================*/
RIL_Errno qcril_data_request_dds_switch(int dds_sub_id)
{
  ...
  if (dds_sub_id < 0) {
    QCRIL_LOG_ERROR("invalid sub id %d received", dds_sub_id);
  ...
  memset(&req, 0, sizeof(req));
  memset(&resp, 0, sizeof(resp));
  req.subscription = qcril_data_ril_to_dsd_sub_id(dds_sub_id);
...
exit:
  ...
  if (DSI_SUCCESS == ret)
  {
    QCRIL_LOG_DEBUG("%s","qcril_data_qmi_dpm_init: exit with success");
  }
  else
  {
    QCRIL_LOG_ERROR("%s","qcril_data_qmi_dpm_init: exit with error");
  }
}
