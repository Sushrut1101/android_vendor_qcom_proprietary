...
public:
  ...
  void uimLpaHttpTxnCompletedResponse(int32_t token, qcril_uim_http_result_type result);
  void uimLpaUserResponse(int32_t token, qcril_uim_lpa_user_resp_type * user_resp);
  void uimLpaAddProfileProgressInd(qcril_uim_lpa_add_profile_progress_ind_type * progInd);
  void uimLpaHttpTxnIndication(qcril_uim_http_transaction_ind_type * txnInd);
};
