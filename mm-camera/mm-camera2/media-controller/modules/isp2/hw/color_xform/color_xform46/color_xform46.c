...
/* #define COLOR_XFORM_DEBUG */
#ifdef COLOR_XFORM_DEBUG
#undef ISP_DBG
 ...
 *  Return: None
 **/
static void color_xform46_cmd_debug(ISP_ColorXformCfgCmdType *cmd,
  isp_color_xform_type color_xform_path)
{
  if (color_xform_path == ISP_COLOR_XFORM_ENCODER) {
    ISP_DBG("=== xform coefficients for ENCODER path ===");
  } else if (color_xform_path == ISP_COLOR_XFORM_VIEWFINDER) {
    ISP_DBG(" === xform coefficients for VIEWFINDER path ===");
  } else if (color_xform_path == ISP_COLOR_XFORM_VIDEO) {
    ISP_DBG("=== xform coefficients for VIDEO path ===");
  } else {
    return;
  }

  ISP_DBG("Coefficients: \n"
 ...
 * Return: None
 **/
static boolean color_xform46_update_s_value(color_xform46_t *color_xform,
 ...
 **/
static boolean color_xform46_store_hw_update(isp_sub_module_t *isp_sub_module,
  color_xform46_t *color_xform)
{
  boolean                       ret = TRUE;
  ...
  reg_cmd = (ISP_ColorXformCfgCmdType *)CAM_MALLOC(sizeof(*reg_cmd));
  if (!reg_cmd) {
  ...
  *reg_cmd = color_xform->reg_cmd;
  ...
  color_xform46_cmd_debug(&color_xform->reg_cmd, color_xform->color_xform_path);
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean color_xform46_config(isp_sub_module_t *isp_sub_module,
  color_xform46_t *color_xform)
{
  ...
  ret = color_xform46_update_s_value(color_xform,isp_sub_module);
  if (ret == FALSE) {
    ISP_ERR("failed: upadte_s_value %d", ret);
    return FALSE;
  }
  ret = color_xform46_store_hw_update(isp_sub_module, color_xform);
  if (ret == FALSE) {
    ISP_ERR("failed: ret %d", ret);
  }

  return ret;
} /* color_xform46_config */
 ...
 *  Return TRUE on success and FALSE on failure
 **/
boolean color_xform46_scaler_output_update(
    mct_module_t     *module,
    isp_sub_module_t *isp_sub_module,
    mct_event_t      *event)
{
  ...
  color_xform46_t         *color_xform   = NULL;
  ...
  if (!isp_sub_module->private_data) {
    ISP_ERR("failed: color_xform %p", isp_sub_module->private_data);
    return FALSE;
  }

  color_xform = (color_xform46_t *)isp_sub_module->private_data;
 ...
 *  Return TRUE on success and FALSE on failure
 **/
boolean color_xform46_trigger_update(
    mct_module_t     *module,
    isp_sub_module_t *isp_sub_module,
    mct_event_t      *event)
{
  ...
  private_event = (isp_private_event_t *)
    event->u.module_event.module_event_data;
  if (!private_event || !isp_sub_module->private_data) {
    ISP_ERR("failed: private_event %p", private_event);
    return FALSE;
  }
  ...
  if (isp_sub_module->trigger_update_pending) {
    ret = color_xform46_config(isp_sub_module, color_xform);
    if (ret == FALSE) {
      ISP_ERR("failed: color_xform46_config ret %d", ret);
      return FALSE;
    }
 ...
 **/
boolean color_xform46_init(mct_module_t *module, isp_sub_module_t *isp_sub_module)
{
  color_xform46_t *color_xform = NULL;
  ...
  color_xform = (color_xform46_t *)CAM_MALLOC(sizeof(color_xform46_t));
  if (!color_xform) {
    ISP_ERR("failed: color_xform %p", color_xform);
    return FALSE;
  }
  ...
  if (!strncmp(MCT_MODULE_NAME(module),
    ...
    color_xform->reg_offset = ISP_COLOR_XFORM46_VIEW_OFF;
    color_xform->reg_len = ISP_COLOR_XFORM46_VIEW_LEN;
    ...
    color_xform->reg_offset = ISP_COLOR_XFORM46_ENC_OFF;
    color_xform->reg_len = ISP_COLOR_XFORM46_ENC_LEN;
  } else if (!strncmp(MCT_MODULE_NAME(module),
              COLOR_XFORM46_VIDEO_NAME(COLOR_XFORM46_VERSION),
              strlen(MCT_MODULE_NAME(module)))) {
    ISP_HIGH("video");
    color_xform->color_xform_path = ISP_COLOR_XFORM_VIDEO;
    color_xform->reg_offset = ISP_COLOR_XFORM46_VIDEO_OFF;
    color_xform->reg_len = ISP_COLOR_XFORM46_VIDEO_LEN;
  } else {
 ...
 *  Return none
 **/
void color_xform46_destroy(mct_module_t *module, isp_sub_module_t *isp_sub_module)
{
  if (!module || !isp_sub_module) {
    ISP_ERR("failed: module %p isp_sub_module %p", module, isp_sub_module);
    return;
