               ...
               void                 *msg_ptr )
{
    ...
    if ( msg_hdr->service_number == SNS_SMGR_SVC_ID_V01
        ...
        strlcpy( (char *)&pResp->SensorInfo[0].SensorShortName, "ACCEL", strlen("ACCEL"));

        pResp->SensorInfo[1].SensorID = SNS_SMGR_ID_GYRO_V01;
        pResp->SensorInfo[1].SensorShortName_len = 4;
        strlcpy( (char *)&pResp->SensorInfo[1].SensorShortName, "GYRO", strlen("GYRO"));

        pResp->SensorInfo[2].SensorID = SNS_SMGR_ID_MAG_V01;
        pResp->SensorInfo[2].SensorShortName_len = 3;
        strlcpy( (char *)&pResp->SensorInfo[2].SensorShortName, "MAG", strlen("MAG"));
        ...
        strlcpy( (char *)pResp->SensorInfo.data_type_info[0].SensorName, "sensor_name", strlen("sensor_name"));
        pResp->SensorInfo.data_type_info[0].VendorName_len = 11;
        strlcpy( (char *)pResp->SensorInfo.data_type_info[0].VendorName, "vendor_name", strlen("vendor_name"));
