/******************************************************************************
  Copyright (c) 2010, 2012, 2018 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
int qmi_simple_ril_misc_dms_modem_mode_qmi_message_handler (qmi_util_service_message_info* qmi_msg_info, 
    ...
    const unsigned int mode_buf_size = 150;
    char mode_buf[mode_buf_size];
    ...
    switch (qmi_msg_info->message_id)
        ...
        case QMI_DMS_GET_OPERATING_MODE_RESP_V01:
            ...
            snprintf(mode_buf, mode_buf_size, "modem is %s", qmi_simple_ril_dms_get_modem_mode_str(get_mode_resp->operating_mode) );
        ...
        case QMI_DMS_GET_TIME_RESP_V01:
            ...
            snprintf(mode_buf, mode_buf_size, "seconds elapsed from start of GPS time = %lf, time_source = %d", elapsed_time,get_time_resp->device_time.time_source);            
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, mode_buf);
            break;
...
)
{
    ...
    if (resp->esn_valid)
    {
        snprintf(buf, buf_size, "esn is %s", resp->esn);
    ...
    if (resp->imei_valid)
    {
        snprintf(buf, buf_size, "imei is %s", resp->imei);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, buf);
    ...
    {
        snprintf(buf, buf_size, "imeisv_svn is %s", resp->imeisv_svn);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, buf);
    ...
    if (resp->meid_valid)
    {
        snprintf(buf, buf_size, "meid is %s", resp->meid);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, buf);
...
int qmi_simple_ril_misc_uim_get_imsi_handler(qmi_simple_ril_cmd_input_info* cmd_params, qmi_simple_ril_cmd_ack_info* ack_info)
{
    ...
    uint8_t gw_path[] = {0x3F, 0x00, 0x7F, 0x20, 0x00};
...
int qmi_simple_ril_misc_get_fdn_status_handler(qmi_simple_ril_cmd_input_info* cmd_params, qmi_simple_ril_cmd_ack_info* ack_info)
    ...
    do
        ...
        if (cmd_params->info_set->nof_entries == 3)
            ...
            strlcpy(verify_pin_req_msg.verify_pin.pin_value, arg, sizeof(verify_pin_req_msg.verify_pin.pin_value));
    ...
    if (qmi_msg_info->error_code == QMI_NO_ERR)
        ...
        if(NULL != qmi_msg_info->message_specific_payload)
            ...
            if (send_apdu_resp->resp.result == QMI_RESULT_SUCCESS_V01)
                {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "send_apdu request returned success\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);

                if(send_apdu_resp->apdu_valid)
                    {
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "length of APDU response: 0x%x\n", send_apdu_resp->apdu_len);
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);

                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "APDU response data: \n");
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);

                    for (i = 0; i < (int)send_apdu_resp->apdu_len; i++)
                        {
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "0x%x ", send_apdu_resp->apdu[i]);
                        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                        }
                    }
                }
                ...
                {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "send_apdu request returned error code: 0x%x\n", send_apdu_resp->resp.error);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                }
            }
        else
            {
            snprintf(temp_char_array,TEMP_ARRAY_SIZE, "send_apdu response has no message payload\n");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);            
            }                                                
        }
    else
        {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE,  "ERROR in sending APDU, error code :0x%x\n",qmi_msg_info->error_code);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    ...
    if (qmi_msg_info->error_code == QMI_NO_ERR)
        ...
        if(NULL != qmi_msg_info->message_specific_payload)
            ...
            if (logical_chan_resp->resp.result == QMI_RESULT_SUCCESS_V01)
                {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "logical_channel request returned success\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);

                if(logical_chan_resp->channel_id_valid)
                    {
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "channel_id: 0x%x\n", logical_chan_resp->channel_id);
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    }

                if(logical_chan_resp->card_result_valid)
                    {
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "SW1: 0x%x\n", logical_chan_resp->card_result.sw1);
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);

                    snprintf(temp_char_array,TEMP_ARRAY_SIZE, "SW2: 0x%x\n", logical_chan_resp->card_result.sw2);
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    }

                if(logical_chan_resp->select_response_valid)
                    {
                    snprintf(temp_char_array,TEMP_ARRAY_SIZE, "SELECT response: \n");
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);

                    for (i = 0; i < (int)logical_chan_resp->select_response_len && i < QMI_UIM_SELECT_RESPONSE_MAX_V01; i++)
                        {
                        snprintf(temp_char_array, TEMP_ARRAY_SIZE,  "0x%x ", logical_chan_resp->select_response[i]);
                        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                        }
                    }
                }
                ...
                {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "logical_channel req returned error code: 0x%x\n", logical_chan_resp->resp.error);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                }
            }
        else
            {
            snprintf(temp_char_array,TEMP_ARRAY_SIZE, "logical_channel response has no message payload\n");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);            
            }                                                
        }
    else
    ...
    if (qmi_msg_info->error_code == QMI_NO_ERR)
        ...
        if(NULL != qmi_msg_info->message_specific_payload)
            ...
            if (uim_resp->resp.result == QMI_RESULT_SUCCESS_V01)
                {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] Get Service Status returned success\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                if (TRUE == uim_resp->fdn_status_valid)
                    {
                        if (uim_resp->fdn_status == 0)
                            {
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] FDN not available\n");
                            }
                        else if (uim_resp->fdn_status == 1)
                            {
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] FDN available but disabled\n");
                            }
                        else if (uim_resp->fdn_status == 2)
                            {
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] FDN available and enabled\n");
                            }
                        else
                            {
                            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] FDN status unknown\n");
                            }
                        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    }
                }
                ...
                {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] Get Service Status returned error code: %d\n", uim_resp->resp.error);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                }
            }
        else
            {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] Get Service status callback has no message payload\n");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);            
            }                                                
        }
    else
        {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_TEST] ERROR in sending Get Service Status request, error code :%d\n",qmi_msg_info->error_code);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    return 0;
}
...
int qmi_simple_ril_misc_uim_verify_pin_callback (qmi_util_service_message_info* qmi_msg_info, 
    ...
    if (qmi_msg_info->error_code == QMI_NO_ERR)
        ...
        if(NULL != qmi_msg_info->message_specific_payload)
            ...
            if (uim_resp->resp.result == QMI_RESULT_SUCCESS_V01)
                {
                snprintf(temp_char_array,TEMP_ARRAY_SIZE, "[UIM_Test] Verify PIN returned success\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                if (TRUE == uim_resp->retries_left_valid)
                    {
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] Unblocking attempts left: %d\n", uim_resp->retries_left.unblock_left);
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] Verifying attempts left: %d\n", uim_resp->retries_left.verify_left);
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                    }
                }
            else
                {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] Verify PIN returned error code: %d\n", uim_resp->resp.error);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                }
            }
        else
            {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] Verify PIN callback has no message payload\n");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);            
            }                                                
        }
    else
        {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_TEST] ERROR in sending Verify PIN request, error code :%d\n",qmi_msg_info->error_code);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    return 0;
}
    ...
    if (qmi_msg_info->error_code == QMI_NO_ERR)
        ...
        if(NULL != qmi_msg_info->message_specific_payload)
            ...
            if (set_srv_status_resp->resp.result == QMI_RESULT_SUCCESS_V01)
                {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] Set Service Status returned success\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                }
            else
                {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] Set Service Status returned error code: %d\n", set_srv_status_resp->resp.error);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                }
            }
        else
            {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_Test] Set Service status callback has no message payload\n");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);            
            }                                                
        }
    else
        {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[UIM_TEST] ERROR in sending Set Service Status request, error code :%d\n",qmi_msg_info->error_code);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    return 0;
}
...
int qmi_simple_ril_indication_register_callback (qmi_util_service_message_info* callback_msg_info,qmi_simple_ril_cmd_completion_info* uplink_message)
{
    if (uplink_message->error_code == QMI_NO_ERR || uplink_message->error_code == QMI_IDL_LIB_MISSING_TLV)
        ...
        {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[PB_TEST] ERROR in sending the Indication_Register Request, ERROR CODE:%d\n",uplink_message->error_code);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    return 0;
}
...
static void pb_print_indication_register_resp(pbm_indication_register_resp_msg_v01* pb_arg_resp_msg,qmi_simple_ril_cmd_completion_info* uplink_message)
{
    ...
    if(QMI_RESULT_SUCCESS_V01 == pb_arg_resp_msg->resp.result)
    {
        ...
        {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[PB_TEST] Registration MASK was set to : %d\n",pb_arg_resp_msg->reg_mask);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);        
        }
        else
        {
    ...
    {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[PB_TEST] ERROR in Indication_Register Response, ERROR CODE:%d",pb_arg_resp_msg->resp.error);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    }
}
...
static void pb_print_record_update_ind(pbm_record_update_ind_msg_v01* pb_arg_ind_msg,qmi_simple_ril_cmd_completion_info* uplink_message)
{
    ...
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[PB_TEST] Session Type : %d\n",pb_arg_ind_msg->record_update_info.session_type);
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[PB_TEST] PB Type : %d\n",pb_arg_ind_msg->record_update_info.pb_type);
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[PB_TEST] Operation : %d\n",pb_arg_ind_msg->record_update_info.operation);
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "[PB_TEST] Record ID : %d\n",pb_arg_ind_msg->record_update_info.record_id);
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
}
