                    ...
                    intptr_t cb_data)
{
    ...
    if (sensor1_cb->sensor1_handle == (sensor1_handle_s*)-1) {
        ...
        if (err == SENSOR1_SUCCESS) {
            HAL_LOG_DEBUG("%s: Sensor1 reopen success.", __FUNCTION__);
        }
                    ...
                    intptr_t cb_data)
{
    ...
    HAL_LOG_DEBUG("%s: The BrokenPipe handle is %d", __FUNCTION__, mSensor->getHandle());
    ...
    while (err == SENSOR1_EWOULDBLOCK && retry > 0) {
        HAL_LOG_WARN("%s: open sensor1 failed at SENSOR1_EWOULDBLOCK, retrying...%d", __FUNCTION__, retry);
