...
{
   pthread_mutex_lock(&shared_q_mutex);
   ...
   pthread_mutex_unlock(&shared_q_mutex);
   return handle;
...
int internal_perf_lock_acq_apply(int handle, Request *req) {
    ...
    static int display_hint_handle = 0;
    ...
    if (req->GetPriority() == HIGH_PRIORITY) {
        ...
        if (disp_hint_req_typ_on) {
            if (display_hint_handle) {
                ...
                QLOGI("Disp_hint_resource released handle %d\n",display_hint_handle);
                handle = display_hint_handle;
                display_hint_handle = 0;
                return handle;
            }
            else {
                QLOGI("Disp_hint_req_typ_on :Display state already on.\n");
                return handle;
            }
        }
        //Display off, if value Passed is 0
        else {
            //If Display off request exists, return previous handle
            QLOGI("DispCallFlow:disp_hint_resource released handle %d\n",display_hint_handle);
            if (display_hint_handle) {
                QLOGI("Disp_hint_req_typ :Display state already off.\n");
                return display_hint_handle;
            }
...
static void *mpctl_server(void *data) {
    ...
    int ksm = 0, value = 0, boot_count = 0;
    ...
    do {
        ...
        if (!value){
            QLOGI("Waiting for 1st Boot 0.1 sec...");
            usleep(SLEEP_TIME_PER_ITR);
        }
        if (boot_count > TOTAL_ITR) {
            QLOGE("First Boot check over.Going to 2nd Boot check.");
            break;
        }
        boot_count++;
    } while(!value);
    ...
    while(!value) {
        ...
        if (!value){
            QLOGI("Waiting for 2nd Boot 60 sec...");
            usleep(SLEEP_TIME_PER_ITR_SL);
        }
        if (boot_count > TOTAL_ITR_SL) {
            QLOGE("Second Boot check attempt reached. Exiting.....");
            return NULL;
        }
        boot_count++;
    }
...
static void *chk_core_online_poll (void *data) {
    int rc = 0;
    int one_cpu_per_clstr_online = FAILED;
    struct pollfd fds;
    char buf[NODE_MAX];
    Target &t = Target::getCurTarget();
    ...
    while (true) {
        ...
        do {
            ...
            if (rc < 0) {
                ...
                rc = pread(fds.fd, buf, NODE_MAX, 0);
                for (int i=0; i < t.getNumCluster(); i++) {
                      if (!i) {
...
int perfmodule_submit_request(mpctl_msg_t *msg) {
    ...
    switch (cmd) {
        case MPCTL_CMD_PERFLOCKHINTACQ:
             ...
             */
            if (pMsg->hint_id == VENDOR_HINT_FIRST_LAUNCH_BOOST && pMsg->hint_type == LAUNCH_TYPE_ATTACH_APPLICATION) {
            ...
            if (pMsg->pl_time < 0) {
                QLOGE("No valid timer value for perflock request");
                handle = -1;
                break;
            }
            pMsg->data = size;
