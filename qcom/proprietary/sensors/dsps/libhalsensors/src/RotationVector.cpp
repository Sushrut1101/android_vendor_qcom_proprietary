/*============================================================================
  @file RotationVector.cpp

  @brief
  RotationVector class implementation.
...
/*============================================================================
  RotationVector Constructor
============================================================================*/
RotationVector::RotationVector(int handle)
    :SAMSensor(handle)
{
    if ((getAttribOK() == true)) {
        ...
        setName("Rotation Vector");
        setVendor("QTI");
        setType(SENSOR_TYPE_ROTATION_VECTOR);

        if(handle == HANDLE_ROTATION_VECTOR_WAKE_UP){
            bWakeUp = true;
            setName("Rotation Vector -Wakeup");
            setFlags(SENSOR_FLAG_CONTINUOUS_MODE|SENSOR_FLAG_WAKE_UP);
        } else if (handle == HANDLE_ROTATION_VECTOR) {
            bWakeUp = false;
...
/*============================================================================
  enable
============================================================================*/
int RotationVector::enable(int en)
{
    int err;
    ...
    if (enabled == en) {
        HAL_LOG_INFO("RotationVector is already enabled/disabled %d", enabled);
        return 0;
    }
...
int RotationVector::sendBatchReq()
{
    sensor1_error_e  error;
    sensor1_msg_header_s req_hdr;
...
===========================================================================*/
void RotationVector::processResp(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
...
/*===========================================================================
  FUNCTION:  processInd
===========================================================================*/
void RotationVector::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if(bWakeUp == false) {
            ...
            la_sample.sensor = HANDLE_ROTATION_VECTOR_WAKE_UP;
        ...
        HAL_LOG_VERBOSE( "%s: ROTV [0]: %f [1]: %f [2]: %f [3]: %f [4]: %f acc: %d SAM TS: %u HAL TS:%" PRId64 " elapsedRealtimeNano:%" PRId64 "",
                          __FUNCTION__, la_sample.data[0], la_sample.data[1], la_sample.data[2], la_sample.data[3], la_sample.data[4],
