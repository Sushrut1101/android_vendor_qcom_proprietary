...
extern void vfe_diag_update(void *ctrl_obj, int config);
...
vfe_status_t vfe_stats_init(stats_mod_t *stats, vfe_params_t *params)
{
  ...
  if(!stats->use_bayer_stats) {
    ...
    if (VFE_SUCCESS != stats->aec_stats.ops.init(mod_id,
      &(stats->aec_stats), params)) {
        CDBG_ERROR("%s: vfe_aec_stats_init error, rc = %d",
          __func__, rc);
        return VFE_ERROR_GENERAL;
    }
    if (VFE_SUCCESS != stats->af_stats.ops.init(mod_id,
      &(stats->af_stats), params)) {
        CDBG_ERROR("%s: vfe_af_stats_init error, rc = %d",
          __func__, rc);
        return VFE_ERROR_GENERAL;
    }
  } else {
  ...
  vfe_params_t *params)
{
  ...
  if ((params->current_config & VFE_MOD_BCC)) {
  ...
  if ((params->current_config & VFE_MOD_CHROMA_SS)) {
    status = vfe_module->chroma_ss_mod.ops.init(module_id,
      &(vfe_module->chroma_ss_mod), params);
    if (VFE_SUCCESS != status) {
  ...
  if((params->current_config & VFE_MOD_FRAME_SKIP)) {
    status = vfe_module->frame_skip_mod.ops.init(module_id,
      &(vfe_module->frame_skip_mod), params);
    if (VFE_SUCCESS != status) {
  ...
  if ((params->current_config & VFE_MOD_ASF)) {
    status = vfe_module->asf_mod.ops.init(module_id,
  ...
  if ((params->current_config & VFE_MOD_CLF)) {
...
vfe_status_t vfe_params_init(vfe_ctrl_info_t *vfe_obj, vfe_params_t *params)
{
  ...
  params->effects_params.spl_effect = CAMERA_EFFECT_OFF;
...
vfe_status_t vfe_modules_deinit(vfe_module_t *vfe_module, vfe_params_t *params)
{
  ...
  status = vfe_module->asf_mod.ops.deinit(mod_id,
    &(vfe_module->asf_mod), params);
  if(status != VFE_SUCCESS) {
    CDBG_ERROR("%s: asf_mod de init failed", __func__);
    return VFE_ERROR_GENERAL;
  }
  ...
  int* p_update)
{
  ...
  *p_update = FALSE;
  ...
  status = mod_obj->asf_mod.ops.update(module_id,
             &(mod_obj->asf_mod), params);
  if (VFE_SUCCESS != status) {
  ...
  status = stats->af_stats.ops.update(module_id,
             &stats->af_stats, params);
  if (VFE_SUCCESS != status) {
  ...
  if (params->update) {
    int enable_mobicat = ((params->vfe_op_mode == VFE_OP_MODE_SNAPSHOT)
      || (params->vfe_op_mode == VFE_OP_MODE_ZSL)) &&
      params->enable_mobicat;

    if (enable_mobicat)
      vfe_diag_update((void *)vfe_ctrl_obj, FALSE);
...
vfe_status_t vfe_trigger_update_for_awb(vfe_ctrl_info_t *vfe_ctrl_obj)
{
  ...
  status = mod_obj->asf_mod.ops.trigger_update(module_id,
             &(mod_obj->asf_mod), params);
  if (VFE_SUCCESS != status) {
...
static void vfe_operation_config(vfe_ctrl_info_t *vfe_ctrl_obj)
{
  ...
  if (IS_BAYER_FORMAT(params)) {
    ...
    if(vfe_use_bayer_stats(vfe_ctrl_obj)) {
      vfe_ctrl_obj->vfe_op_cfg.op_cmd.vfeStatsCfg.bayerFocusSelect = 1;
      vfe_ctrl_obj->vfe_op_cfg.op_cmd.vfeStatsCfg.bayerGridSelect = 1;
...
static vfe_status_t vfe_stats_config(stats_mod_t *stats, vfe_params_t *params)
{
  ...
  if(IS_BAYER_FORMAT(params) && stats->use_bayer_stats) {
    ...
    if (VFE_SUCCESS != stats->bg_stats.ops.enable(mod_id,
      ...
      CDBG("%s: vfe_bg_stats_enable error = %d", __func__, rc);
      return VFE_ERROR_GENERAL;
    }
    ...
    if (VFE_SUCCESS != stats->bf_stats.ops.enable(mod_id,
      ...
      CDBG("%s: vfe_bf_stats_enable error = %d", __func__, rc);
      return VFE_ERROR_GENERAL;
    }
    ...
    if (VFE_SUCCESS != stats->bhist_stats.ops.enable(mod_id,
      ...
      CDBG("%s: vfe_bhist_stats_enable error = %d", __func__, rc);
      return VFE_ERROR_GENERAL;
    }
    if (VFE_SUCCESS != stats->bg_stats.ops.config(mod_id,
      &(stats->bg_stats), params)) {
      CDBG("%s: vfe_bg_stats_config error = %d", __func__, rc);
      return VFE_ERROR_GENERAL;
    }
    if (VFE_SUCCESS != stats->bhist_stats.ops.config(mod_id,
      &(stats->bhist_stats), params)) {
      CDBG("%s: vfe_bhist_stats_config error = %d", __func__, rc);
      return VFE_ERROR_GENERAL;
    }
  } else {
  ...
  enable = ((params->current_config & VFE_MOD_AEC_STATS) != 0);
  if (VFE_SUCCESS != stats->aec_stats.ops.enable(mod_id,
    &(stats->aec_stats), params, enable, FALSE))
    return VFE_ERROR_GENERAL;

  enable = ((params->current_config & VFE_MOD_AF_STATS) != 0);
  if (VFE_SUCCESS != stats->af_stats.ops.enable(mod_id,
    &(stats->af_stats), params, enable, FALSE))
    return VFE_ERROR_GENERAL;
  ...
  if (VFE_SUCCESS != stats->aec_stats.ops.config(mod_id,
    &(stats->aec_stats), params))
    return VFE_ERROR_GENERAL;
  }
  ...
  int hw_write)
{
  ...
  status = mod_obj->asf_mod.ops.enable(module_id,
    &(mod_obj->asf_mod), params, enable, hw_write);
  if (VFE_SUCCESS != status) {
  ...
  status = mod_obj->chroma_ss_mod.ops.enable(module_id,
    &(mod_obj->chroma_ss_mod), params, enable, hw_write);
  if (VFE_SUCCESS != status) {
  ...
  status = mod_obj->frame_skip_mod.ops.enable(module_id,
    &(mod_obj->frame_skip_mod), params, enable, hw_write);
  if (VFE_SUCCESS != status) {
...
static vfe_status_t vfe_reload_chromatix(vfe_ctrl_info_t *vfe_ctrl_obj)
{
  ...
  status = vfe_module->asf_mod.ops.reload_params(mod_id,
    &(vfe_module->asf_mod), params);
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: asf_mod reload_params failed", __func__);
    return VFE_ERROR_GENERAL;
  }
...
static vfe_status_t vfe_config_pipeline(vfe_ctrl_info_t *vfe_ctrl_obj)
{
  ...
  status = mod_obj->asf_mod.ops.config(module_id,
    &(mod_obj->asf_mod), params);
  if (VFE_SUCCESS != status) {
  ...
  status = mod_obj->frame_skip_mod.ops.config(module_id,
    &(mod_obj->frame_skip_mod), params);
  if (VFE_SUCCESS != status) {
    CDBG_HIGH("%s: Frame skip config failed", __func__);
    return status;
  }
  ...
  status = mod_obj->chroma_ss_mod.ops.config(module_id,
    &(mod_obj->chroma_ss_mod), params);
  if (VFE_SUCCESS != status) {
  ...
  if (enable_mobicat)
    vfe_diag_update((void *)vfe_ctrl_obj, TRUE);
  ...
  vfe_params_t *params)
{
  ...
  switch (params->prev_spl_effect) {
    ...
    case CAMERA_EFFECT_FADED: {
      status = mod_obj->chroma_enhan_mod.ops.set_spl_effect(mod_id,
        &(mod_obj->chroma_enhan_mod), params, CAMERA_EFFECT_OFF);
      if (VFE_SUCCESS != status) {
        CDBG_HIGH("%s: CV reset %d failed", __func__,
          params->prev_spl_effect);
        return VFE_ERROR_GENERAL;
      }
      break;
    ...
    case CAMERA_EFFECT_ACCENT_GREEN:
    case CAMERA_EFFECT_ACCENT_ORANGE: {
      status = mod_obj->sce_mod.ops.set_spl_effect(mod_id,
        &(mod_obj->sce_mod), params, CAMERA_EFFECT_OFF);
      if (VFE_SUCCESS != status) {
        CDBG_HIGH("%s: SCE reset %d failed", __func__,
          params->prev_spl_effect);
	return VFE_ERROR_GENERAL;
      }
      break;
  ...
  switch (params->effects_params.spl_effect) {
    ...
    case CAMERA_EFFECT_ACCENT_ORANGE: {
      status = mod_obj->sce_mod.ops.set_spl_effect(mod_id,
        &(mod_obj->sce_mod), params, params->effects_params.spl_effect);
      if (VFE_SUCCESS != status) {
        CDBG_HIGH("%s: SCE reset %d failed", __func__,
          params->prev_spl_effect);
        return VFE_ERROR_GENERAL;
  ...
  camera_bestshot_mode_type mode)
{
  ...
  status = module->asf_mod.ops.set_bestshot(mod_id, &module->asf_mod,
    params, mode);
  if (VFE_SUCCESS != status) {
  ...
  vfe_stats_af_params_t *p_obj)
{
  ...
  if(IS_BAYER_FORMAT(params) &&
    ...
    status = af_stats->ops.config(mod_id, af_stats, params);
    if(status != VFE_SUCCESS) {
  ...
  vfe_frame_skip *frm_skip)
{
  vfe_status_t status = VFE_SUCCESS;
  int mod_id =0;
  status = p_obj->vfe_module.frame_skip_mod.ops.update(mod_id,
...
vfe_status_t vfe_command_ops(vfe_ctrl_info_t *p_obj, void *parm)
{
  ...
  switch(cmd.mod_cmd_ops) {
    case MOD_CMD_START:
      rc = isp_send_hw_cmd(p_obj->vfe_params.camfd, CMD_GENERAL, cmd.cmd_data, cmd.length, VFE_CMD_START);
      break;
    case MOD_CMD_STOP:
      ...
      rc = isp_send_hw_cmd(p_obj->vfe_params.camfd, CMD_GENERAL, cmd.cmd_data, cmd.length, VFE_CMD_STOP);
      break;
    case MOD_CMD_RESET:
      rc = isp_send_hw_cmd(p_obj->vfe_params.camfd, CMD_GENERAL, cmd.cmd_data, cmd.length, VFE_CMD_RESET);
      break;
    default:
...
vfe_status_t vfe_module_ops_init(vfe_ctrl_info_t *p_obj)
{
  ...
  vfe_asf_ops_init(&(p_obj->vfe_module.asf_mod));
  ...
  vfe_chroma_subsample_ops_init(&(p_obj->vfe_module.chroma_ss_mod));
  ...
  vfe_frame_skip_ops_init(&(p_obj->vfe_module.frame_skip_mod));
  ...
  vfe_aec_stats_ops_init(&(p_obj->vfe_module.stats.aec_stats));
  vfe_awb_stats_ops_init(&(p_obj->vfe_module.stats.awb_stats));
  vfe_af_stats_ops_init(&(p_obj->vfe_module.stats.af_stats));
...
vfe_status_t vfe_module_ops_deinit(vfe_ctrl_info_t *p_obj)
{
  ...
  vfe_asf_ops_deinit(&(p_obj->vfe_module.asf_mod));
  ...
  vfe_chroma_subsample_ops_deinit(&(p_obj->vfe_module.chroma_ss_mod));
  ...
  vfe_frame_skip_ops_deinit(&(p_obj->vfe_module.frame_skip_mod));
  ...
  vfe_aec_stats_ops_deinit(&(p_obj->vfe_module.stats.aec_stats));
  vfe_awb_stats_ops_deinit(&(p_obj->vfe_module.stats.awb_stats));
  vfe_af_stats_ops_deinit(&(p_obj->vfe_module.stats.af_stats));
...
static int vfe_process(void *client, int event, void *parm)
{
  ...
  case VFE_CONFIG_MODE:
    ...
    LOGE("%s: op_mode : %d", __func__, op_mode);
  ...
  case VFE_RELEASE_STATS:
   ...
   break;
  case VFE_PLUGIN_MOD_UPDATE: {
    vfe_plugin_module_params_t *plugin_params = (vfe_plugin_module_params_t *)parm;
    rc = vfe_module_plugin_update(vfe_ctrl_obj, plugin_params);
    if(rc != VFE_SUCCESS)
      CDBG_ERROR("%s: VFE_PLUGIN_MOD_UPDATE failed\n", __func__);
  break;
  }
  case VFE_PLUGIN_REG_UPDATE: {
    rc = vfe_module_plugin_reg_update(vfe_ctrl_obj);
    if(rc != VFE_SUCCESS)
      CDBG_ERROR("%s: VFE_PLUGIN_REG_UPDATE failed\n", __func__);
  break;
  }
  default:
    CDBG_ERROR("%s: Invalid mode  %d\n", __func__, event);
  ...
  void *parm_out)
{
  ...
  case VFE_SET_MOBICAT: {
    uint32_t *p_mobicat_enable = (uint32_t *)parm_in;
    vfe_ctrl_obj->vfe_params.enable_mobicat = *p_mobicat_enable;
    CDBG_HIGH("%s:[%d] Enable mobicat %d", __func__, __LINE__,
      vfe_ctrl_obj->vfe_params.enable_mobicat);
    break;
  }
  ...
  void *parm, int parm_len)
{
  ...
  break;
  case VFE_GET_MOBICAT_ISP_INFO: {
    QISPInfo_t *p_info = (QISPInfo_t *)parm;
    *p_info = vfe_ctrl_obj->isp_info;
    break;
  }
  default:
    CDBG_ERROR("%s: Invalid mode \n", __func__);
  ...
  vfe_set_hal_stream *tmp)
{
  ...
  case MSM_STATS_TYPE_AEC:
    p_obj->vfe_module.stats.aec_stats.use_hal_buf = tmp->use_hal_buff;
    break;
  case MSM_STATS_TYPE_AWB:
    p_obj->vfe_module.stats.awb_stats.use_hal_buf = tmp->use_hal_buff;
    break;
  case MSM_STATS_TYPE_AF:
    p_obj->vfe_module.stats.af_stats.use_hal_buf = tmp->use_hal_buff;
    break;
...
/*===========================================================================
 * FUNCTION    - vfe_module_plugin_update -
 *
 * DESCRIPTION:
 *
 * DEPENDENCY: update the vfe modules from plugin.
 *==========================================================================*/
vfe_status_t vfe_module_plugin_update(vfe_ctrl_info_t *vfe_obj,
  vfe_plugin_module_params_t *tmp)
{
  int mod_id = 0;

  switch(tmp->type & 0xFFFFFFFF) {
  case VFE_MOD_LINEARIZATION:
    vfe_linearization_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_ROLLOFF:
    vfe_rolloff_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_DEMUX:
    vfe_demux_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_DEMOSAIC:
    vfe_demosaic_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_BPC:
    vfe_bpc_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_ABF:
    vfe_abf_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_ASF:
    vfe_asf_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_COLOR_CONV:
    vfe_color_conv_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_COLOR_CORRECT:
    vfe_color_correct_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_CHROMA_SS:
    vfe_chroma_subsample_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_CHROMA_SUPPRESS:
    vfe_chroma_suppress_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_LA:
    vfe_la_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_MCE:
    vfe_mce_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_SCE:
    vfe_sce_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_CLF:
    vfe_clf_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_WB:
    vfe_wb_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_GAMMA:
    vfe_gamma_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_AWB_STATS:
    vfe_awb_stats_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_FOV:
    vfe_fov_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_SCALER:
    vfe_scaler_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_BCC:
    vfe_bcc_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_CLAMP:
    vfe_clamp_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  case VFE_MOD_FRAME_SKIP:
    vfe_frame_skip_plugin_update(mod_id, tmp->mod_ptr,
      &(vfe_obj->vfe_params));
  break;
  default:
    CDBG_HIGH("%s: Unsupported module : %d\n", __func__, tmp->type);
    break;
  }
  return VFE_SUCCESS;
}
...
/*===========================================================================
 * FUNCTION    - vfe_module_plugin_reg_update -
 *
 * DESCRIPTION:
 *
 * DEPENDENCY: update the vfe reg update modules from plugin.
 *==========================================================================*/
vfe_status_t vfe_module_plugin_reg_update(vfe_ctrl_info_t *vfe_obj)
{
  vfe_status_t status = VFE_SUCCESS;

  status = vfe_util_write_hw_cmd (vfe_obj->vfe_params.camfd, CMD_GENERAL, NULL, 0,
      VFE_CMD_UPDATE);
  if(status != VFE_SUCCESS) {
