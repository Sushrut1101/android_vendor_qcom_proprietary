...
#define LOC_GPS_GEOFENCE_DWELL_INSIDE   (1<<3L)
#define LOC_GPS_GEOFENCE_DWELL_OUTSIDE  (1<<4L)
...
static jmethodID method_onServiceDeath = NULL;
...
Return<void> GnssGeofenceServiceProviderCallback::gnssGeofenceBreachCb(
    ...
    for (int32_t i = 0; i < count; i++) {
        uint32_t transition;
        switch(breachType) {
            case (uint32_t)LocHidlGeofenceBreachType::ENTER:
                transition = LOC_GPS_GEOFENCE_ENTERED;
                break;
            case (uint32_t)LocHidlGeofenceBreachType::EXIT:
                transition = LOC_GPS_GEOFENCE_EXITED;
                break;
            case (uint32_t)LocHidlGeofenceBreachType::DWELL_IN:
                transition = LOC_GPS_GEOFENCE_DWELL_INSIDE;
                break;
            case (uint32_t)LocHidlGeofenceBreachType::DWELL_OUT:
...
static void GeofenceServiceProvider_class_init(JNIEnv* env, jclass clazz)
{
    ...
    method_onServiceDeath = env->GetMethodID(clazz, "reportServiceDied", "()V");
    ...
    registerDeathRecipient("GeofenceService", []() {
            gnssGeofenceServiceProviderIface = nullptr;
            ALOGE("ILocHidlGeofenceService died");
            GeofenceServiceProvider_init(nullptr, nullptr);
            JNIEnv* env = android::AndroidRuntime::getJNIEnv();
            env->CallVoidMethod(sCallbacksObj, method_onServiceDeath);
    });
}
