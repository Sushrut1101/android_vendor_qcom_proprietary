...
static int get_bt_soc_type()
{
    ...
    if (ret != 0) {
        DEBUG("vendor.qcom.bluetooth.soc set to %s\n", bt_soc_type);
        if (!strncasecmp(bt_soc_type, "rome", sizeof("rome"))) {
            ...
            DEBUG("vendor.qcom.bluetooth.soc not set, so using default.\n");
...
static bool intf_name_check(char *intf_name)
{
    int size;
    char *name_prefix[] = {
        "/dev/at_usb",
    ...
    for(size = 0; size < (sizeof name_prefix / sizeof name_prefix[0]); size++) {
        if(strstr(intf_name, name_prefix[size]))
          return true;
    }
    return false;
}
...
static int parse_options(wdsdaemon *wds, int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "abfunhstmi:")) != -1) {
        switch (opt) {
        case 'a':
        ...
        case 's':
            ...
            break;
#ifdef ANDROID
      if (soc_type == BT_SOC_ROME || soc_type == BT_SOC_CHEROKEE || soc_type == BT_SOC_HASTINGS
          || soc_type == BT_SOC_GENOA) {
          case 'u':
...
#endif
      case 'i':
            if (!intf_name_check(optarg)) {
                ERROR("interface name must be /dev/at_usb{xx},/dev/ttyHSL{xx} or /dev/ttyGS{xx}\n");
                ret = STATUS_ERROR;
                break;
            }
            DEBUG("PC interface name: %s\n", optarg);
            wds->pc_if.uart.intf = optarg;
            break;
      case 'h':
      ...
      if (soc_type == BT_SOC_ROME || soc_type == BT_SOC_CHEROKEE || soc_type == BT_SOC_HASTINGS
          || soc_type == BT_SOC_GENOA) {
          DEBUG("Use -u for opening only UART Channel for BT\n");
      }
      if (soc_type == BT_SOC_CHEROKEE || soc_type == BT_SOC_HASTINGS) {
      ...
      DEBUG("Use -u for opening only UART Channel for BT (ROME)\n");
            ...
            DEBUG("Use -i <ifname> for setting interface name of PC-DUT\n");
            DEBUG("Use -h to print help\n");
            ret = STATUS_ERROR;
            break;
        default:
            DEBUG("Usage %s [-abfunhmsti:]\n", argv[0]);
            ret = STATUS_ERROR;
            break;
        }
    }
