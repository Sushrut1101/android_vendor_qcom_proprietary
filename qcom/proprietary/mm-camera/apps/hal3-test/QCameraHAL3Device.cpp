...
void QCameraHAL3Device::openCamera()
{
    ...
    snprintf(camera_name, sizeof(camera_name) - 1, "%d", mCameraId);
...
bool QCameraHAL3Device::IsVideoStream(int index)
{
    bool result = false;
    if (0 != (GRALLOC_USAGE_HW_VIDEO_ENCODER & mStreams[index]->usage))
    {
        result = true;
    }
    return result;
...
bool QCameraHAL3Device::IsPreviewStream(int index)
{
    bool result = false;
    if ((CAMERA3_STREAM_OUTPUT == mStreams[index]->stream_type) &&
        (GRALLOC_USAGE_HW_VIDEO_ENCODER != (GRALLOC_USAGE_HW_VIDEO_ENCODER & mStreams[index]->usage)) &&
        ((HAL_PIXEL_FORMAT_YCBCR_420_888 == mStreams[index]->format) ||
        (HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED == mStreams[index]->format)) &&
        ((GRALLOC_USAGE_HW_TEXTURE == (GRALLOC_USAGE_HW_TEXTURE & mStreams[index]->usage)) ||
        (GRALLOC_USAGE_HW_CAMERA_WRITE == (GRALLOC_USAGE_HW_CAMERA_WRITE & mStreams[index]->usage))))
    {
        result = true;
    }
    return result;
...
bool QCameraHAL3Device::IsSnapShotStream(int index)
{
    bool result = false;
    if ((CAMERA3_STREAM_OUTPUT == mStreams[index]->stream_type) &&
        (GRALLOC_USAGE_SW_READ_OFTEN == (GRALLOC_USAGE_SW_READ_OFTEN & mStreams[index]->usage)) &&
        (HAL_DATASPACE_JFIF != mStreams[index]->data_space))
    {
        result = true;
    }
    return result;
...
bool QCameraHAL3Device::IsRawStream(int index)
{
    bool result = false;
    if ((CAMERA3_STREAM_OUTPUT  == mStreams[index]->stream_type) &&
        ((HAL_PIXEL_FORMAT_RAW10 == mStreams[index]->format) ||
        ((HAL_PIXEL_FORMAT_RAW16 == mStreams[index]->format))))
        {
            result = true;
        }
    return result;
...
bool QCameraHAL3Device::IsJPEGSnapshotStream(int index)
{
    bool isJPEGSnapshotStream = false;
    if ((CAMERA3_STREAM_OUTPUT       == mStreams[index]->stream_type) &&
        (HAL_PIXEL_FORMAT_BLOB       == mStreams[index]->format)      &&
        ((HAL_DATASPACE_JFIF         == mStreams[index]->data_space)  ||
        (HAL_DATASPACE_V0_JFIF      == mStreams[index]->data_space)))
    {
