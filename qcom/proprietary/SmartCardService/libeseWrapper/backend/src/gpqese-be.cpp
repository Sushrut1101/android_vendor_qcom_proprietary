...
static uint32_t gpqese_transceive(struct EseInterface *ese, const uint8_t *const tx_buf,
  ...
  if (tx_len > GPQESE_MAX_CAPDU_SIZE){
    ALOGE("APDU too long %lu. max is %lu", (unsigned long)tx_len, (unsigned long)GPQESE_MAX_CAPDU_SIZE);
    return ret;
  }
  ...
  if (DBG)
    print_text("tApdu to be sent :", tx_buf, tx_len);
  ...
  memscpy(input.buffer,input.size,(void *) &tx_buf[0], tx_len);
  operation.params[0].memref.parent = &input;
  operation.params[0].memref.offset = 0;
  operation.params[0].memref.size = tx_len;
  ...
  memscpy(rx_buf, rx_len, (uint8_t*)output.buffer, size_read);
  if (DBG)
    print_text("rApdu received :", rx_buf, size_read);
  TEEC_ReleaseSharedMemory(&input);
  TEEC_ReleaseSharedMemory(&output);
  return size_read;
}
