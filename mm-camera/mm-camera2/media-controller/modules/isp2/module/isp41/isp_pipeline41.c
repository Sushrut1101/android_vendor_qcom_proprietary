...
#include "isp_pipeline41_parser.h"
#include "isp_defs.h"
#include "isp_log.h"
...
#define ISP_PIX41_CLK_USER_MAX ISP_PIX_CLK_320MHZ
...
static boolean isp_pipeline41_update_module_cfg(int32_t fd,
  uint8_t *curr_submod_enable,
  isp_module_enable_info_t *enable_bit_info, cam_format_t sensor_fmt);

typedef enum {
  ISP_PIPELINE41_MODULE_CFG,
  ISP_PIPELINE41_MCE_CS,
  ISP_PIPELINE41_STATS_CFG,
  ISP_PIPELINE41_BPC_BCC_ABF,
  ISP_PIPELINE41_MOD_EN_MAX,
} isp_pipeline41_mod_en_type_t;
...
/** isp_pipeline41_adrc_hw_module_mask:
 *
 *  @fd: file descriptor
 *
 *  Calculate module mask which has ADRC support.
 *
 *  Return adrc module mask
 **/
uint32_t adrc_hw_module_mask()
{
  return (1 << ISP_ADRC_MOD_LA) | (1 << ISP_ADRC_MOD_LTM);
}

/** isp_pipeline41_pipeline_update_stats_mask_for_mono_sensor:
 *
 ...
 **/
static boolean isp_pipeline41_pipeline_update_stats_mask_for_mono_sensor(
  uint32_t *stats_mask)
{
  if (!stats_mask)
    return FALSE;
...
/** isp_pipeline41_calculate_roi_map:
 *
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean isp_pipeline41_calculate_roi_map(
...
static isp_pipeline_stats_func_table_t func_table = {
  .pipeline_fill_stats_info = isp_pipeline41_parser_fill_stats_info,
  .pipeline_calculate_roi_map = isp_pipeline41_calculate_roi_map,
  .pipeline_update_module_cfg = isp_pipeline41_update_module_cfg,
  .pipeline_stats_parse[MSM_ISP_STATS_BG] = isp_pipeline41_parser_bg_stats,
  .pipeline_stats_parse[MSM_ISP_STATS_BF] = isp_pipeline41_parser_bf_stats,
  .pipeline_stats_parse[MSM_ISP_STATS_BE] = isp_pipeline41_parser_be_stats,
  .pipeline_stats_parse[MSM_ISP_STATS_CS] = isp_pipeline41_parser_cs_stats,
  .pipeline_stats_parse[MSM_ISP_STATS_RS] = isp_pipeline41_parser_rs_stats,
  .pipeline_stats_parse[MSM_ISP_STATS_BHIST] = isp_pipeline41_parser_bhist_stats,
  .pipeline_stats_parse[MSM_ISP_STATS_IHIST] = isp_pipeline41_parser_ihist_stats,
...
/** isp_pipeline41_set_register_mask_val_default:
 *
 ...
 *  Return void
 **/
static inline void isp_pipeline41_set_register_mask_val_default(
  uint32_t *register_mask, uint32_t *register_val,
  isp_pipeline41_mod_en_type_t mod_en_type,
  uint32_t register_bit, uint8_t enable)
{
  if (!register_mask || !register_val ||
    (mod_en_type >= ISP_PIPELINE41_MOD_EN_MAX) || (register_bit > 31)) {
...
/** isp_pipeline41_set_register_mask_val:
 *
 *  @register_mask: mask to store bits that need change
 *  @register_val: for masked bit, enable / disable bit in
 *               actual register
 ...
 *  Return void
 **/
static inline void isp_pipeline41_set_register_mask_val(
  uint32_t *register_mask, uint32_t *register_val,
  isp_pipeline41_mod_en_type_t mod_en_type, isp_module_enable_info_t *bit_info,
  isp_hw_module_id_t module_id, uint32_t register_bit)
{
  if (!register_mask || !register_val || !bit_info ||
    (mod_en_type >= ISP_PIPELINE41_MOD_EN_MAX) ||
...
/** isp_pipeline41_create_module_cfg_mask:
 *
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean isp_pipeline41_create_module_cfg_mask(
  uint8_t *curr_submod_enable,
  ...
  for (module_id = 0; module_id < ISP_MOD_MAX_NUM; module_id++) {
    if (input_enable_bit_info->submod_mask[module_id]) {
      ...
      case ISP_MOD_LINEARIZATION:
        isp_pipeline41_set_register_mask_val(register_mask, register_val,
          ISP_PIPELINE41_MODULE_CFG, input_enable_bit_info, module_id,
          MODULE_CFG_BLACK_EN);
        break;
      ...
      case ISP_MOD_BPC:
        isp_pipeline41_set_register_mask_val(register_mask,
          register_val, ISP_PIPELINE41_BPC_BCC_ABF, input_enable_bit_info,
      ...
      case ISP_MOD_ABF:
        isp_pipeline41_set_register_mask_val(register_mask, register_val,
          ISP_PIPELINE41_BPC_BCC_ABF, input_enable_bit_info, module_id,
          MODULE_CFG_ABF_EN);
        break;
      ...
      case ISP_MOD_SKIN_BHIST_STATS:
        ...
        isp_pipeline41_set_register_mask_val(register_mask, register_val,
          ISP_PIPELINE41_STATS_CFG, input_enable_bit_info, module_id,
      ...
      case ISP_MOD_CHROMA_SUPPRESS:
        ...
        if (curr_submod_enable[ISP_MOD_CHROMA_SUPPRESS] ||
          ...
          isp_pipeline41_set_register_mask_val_default(register_mask,
            register_val, ISP_PIPELINE41_MODULE_CFG,
        ...
        isp_pipeline41_set_register_mask_val(register_mask, register_val,
          ISP_PIPELINE41_MCE_CS, input_enable_bit_info, module_id,
          MODULE_CFG_CS_EN);
        break;
...
/** isp_pipeline41_update_module_cfg:
 *
 *  @fd: file descriptor
  ...
  cam_format_t sensor_fmt)
{
  ...
  struct msm_vfe_reg_cfg_cmd   reg_cfg_cmd[ISP_PIPELINE41_MOD_EN_MAX];
  uint32_t                     register_mask[ISP_PIPELINE41_MOD_EN_MAX] = {0},
                               register_val[ISP_PIPELINE41_MOD_EN_MAX] = {0};
  uint32_t                     num_cfg = 0;
  isp_pipeline41_mod_en_type_t mod_en_type = 0;
  ...
  ret = isp_pipeline41_create_module_cfg_mask(curr_submod_enable,
    enable_bit_info, register_mask, register_val);
  if (ret == FALSE) {
    ISP_ERR("failed: isp_pipeline41_create_module_cfg_mask");
    return FALSE;
  }

  num_cfg = 0;
  for (mod_en_type = 0; mod_en_type < ISP_PIPELINE41_MOD_EN_MAX;
    mod_en_type++) {
...
isp_pipeline_t *isp_pipeline41_open(uint32_t isp_version, uint32_t isp_id)
{
  ...
  if (isp_version == ISP_MSM8952) {
    ...
    curr_pipeline->is_mono_stats_supported = 0;

    if (isp_id == ISP_HW_0) {
      curr_pipeline->max_width = 3504;
      curr_pipeline->max_height = 4096;
      curr_pipeline->hw_stream_info[ISP_HW_STREAM_ENCODER].max_width = 3504;
      curr_pipeline->hw_stream_info[ISP_HW_STREAM_ENCODER].max_height = 4096;
      curr_pipeline->hw_stream_info[ISP_HW_STREAM_VIEWFINDER].max_width = 2112;
      curr_pipeline->hw_stream_info[ISP_HW_STREAM_VIEWFINDER].max_height = 4096;
    ...
    curr_pipeline->max_pix_clk_rate[ISP_CX_LEVEL_NOMINAL] = ISP_PIX41_CLK_USER_MAX;
    curr_pipeline->max_pix_clk_rate[ISP_CX_LEVEL_TURBO] = ISP_PIX_CLK_320MHZ;
    } else if (isp_version == ISP_MSM8956) {
      ...
      curr_pipeline->max_pix_clk_rate[ISP_CX_LEVEL_NOMINAL] = ISP_PIX41_CLK_USER_MAX;
      curr_pipeline->max_pix_clk_rate[ISP_CX_LEVEL_TURBO] = ISP_PIX_CLK_465MHZ;
    } else {
