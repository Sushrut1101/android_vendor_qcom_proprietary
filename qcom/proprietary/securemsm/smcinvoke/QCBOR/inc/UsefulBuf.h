...
/*===================================================================================
 ...
 --------           ----            ---------------------------------------------------
 01/30/19           rnance          A few small refinements
 05/04/18           llundbla        Better strategy for endian conversion and floats to
                                    avoid compiler warnings.
 04/11/18           llundbla        Documentation only fixes -- clean up Doxygen complaints
...
/**
 @brief Get a uint16_t out of the buffer, byte alignment safe

 @param[in] me: the UsefulBuf.
 @param[in] offset offset to the 2 bytes in the UsefulBuf.

 @return The uint16_t

 The input bytes must be in network order (big endian).
 */
static inline uint16_t UsefulBuf_GetUint16(UsefulBufC me, size_t offset)
{
   const uint8_t* pByte = ((const uint8_t*) me.ptr) + offset;
   return   ((uint16_t)pByte[0]<<8)  |
            pByte[1];
}

/**
 @brief Get a uint32_t out of the buffer, byte alignment safe

 @param[in] me: the UsefulBuf.
 @param[in] offset offset to the 4 bytes in the UsefulBuf.

 @return The uint32_t

 The input bytes must be in network order (big endian).
 */
static inline uint32_t UsefulBuf_GetUint32(UsefulBufC me, size_t offset)
{
   const uint8_t* pByte = ((const uint8_t*) me.ptr) + offset;
   return   ((uint32_t)pByte[0]<<24) |
            ((uint32_t)pByte[1]<<16) |
            ((uint32_t)pByte[2]<<8)  |
            pByte[3];
}
...
/**
 @brief Get a uint64_t out of the buffer, byte alignment safe

 @param[in] me: the UsefulBuf.
 @param[in] offset offset to the 8 bytes in the UsefulBuf.

 @return The uint64_t

 The input bytes must be in network order (big endian).
 */
static inline uint64_t UsefulBuf_GetUint64(UsefulBufC me, size_t offset)
{
   const uint8_t* pByte = ((const uint8_t*) me.ptr) + offset;
   return   ((uint64_t)pByte[0]<<56) |
            ((uint64_t)pByte[1]<<48) |
            ((uint64_t)pByte[2]<<40) |
            ((uint64_t)pByte[3]<<32) |
            ((uint64_t)pByte[4]<<24) |
...
/**
 ...
 The general calling flow is like this:
