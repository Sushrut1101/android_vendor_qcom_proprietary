...
#ifndef UTIL_UNUSED
#define UTIL_UNUSED(name) name __attribute__((unused))
#endif
                                      ...
                                      util_bit_field_type list_bit_field)
{
    ...
    if(!list_info)
    {
        if(util_bit_field_is_bits_set(list_bit_field,
                                      ...
                                      FALSE))
        {
            list_info = static_cast<util_list_info_type *>(util_memory_alloc(sizeof(*list_info)));
        }
        else
        {
                  ...
                  util_bit_field_type node_bit_field)
{
    ...
    if(list_info)
    {
        ...
        do
        {
            ...
            temp_node = static_cast<util_list_node_type *>(util_memory_alloc(sizeof(*temp_node)));
            if(!temp_node)
            {
                break;
            }
...
util_list_node_data_type* util_list_find_data_in_list_with_param(const util_list_info_type *list_info, int (*find_evaluator)(const util_list_node_data_type *to_be_found_data, const void *compare_data), const void* compare_data)
{
    util_list_node_type *iter;
    util_list_node_type *found_node;
    ...
    if(list_info && find_evaluator)
    {
        util_list_auto_lock_list((util_list_info_type *)list_info);
        iter = list_info->list_head;
        while(iter && !((*find_evaluator) (&iter->node_data, compare_data)))
        {
            iter = iter->next;
        }
        util_list_auto_unlock_list((util_list_info_type *)list_info);
    }
    found_node = iter;
...
***************************************************************************************************/
int util_list_enque_add_evaluator(UTIL_UNUSED(util_list_node_data_type *to_be_added_data),
                                  UTIL_UNUSED(util_list_node_data_type *to_be_evaluated_data))
{
    return TRUE;
}
