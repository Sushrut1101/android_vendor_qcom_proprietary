...
void * db_maintainer_cleanup_thread(void * arg) {
    ...
    while(1)
    {
        property_get("vendor.prefetcher.pkg_cleanup_intrvl",property, "2880");
        pkg_cleanup_interval = strtod(property, NULL) * 60;

        property_get("vendor.prefetcher.pkg_retention_mins",property, "28800");
        pkg_retention_threshold = strtod(property, NULL) * 60;

        property_get("vendor.iop.uxe.last_launch_threshold",property, "21600");
