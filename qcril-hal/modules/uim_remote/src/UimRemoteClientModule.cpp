...
UimRemoteClientModule::UimRemoteClientModule()
{
  ...
  mMessageHandler = {
      ...
      HANDLER(UimRmtCardAppInitStatusIndMsg, UimRemoteClientModule::handleUimRemoteCardAppInitStatusIndMessage),
  };
} /* UimRemoteClientModule::UimRemoteClientModule */
...
)
{
  ...
  switch (ind_data_ptr->ind_id)
  {
    case QMI_UIM_REMOTE_SRVC_UP_IND_MSG:
      mReady = TRUE;
      uimRemoteClient->uimRemoteClientServiceInd(true);
      break;
    case QMI_UIM_REMOTE_SRVC_DOWN_IND_MSG:
      mReady = FALSE;
      isUimRemoteSimConnected = false;
      uimRemoteClient->uimRemoteClientServiceInd(false);
      break;
...
*/
/*=========================================================================*/
void UimRemoteClientModule::handleUimRemoteCardAppInitStatusIndMessage
(
...
)
{
  ...
  if (msgPtr == nullptr || uimRemoteClient == nullptr)
  {
  ...
  {
    uimRemoteClient->uimRemoteClientCardAppInitStatusIndMessage(indDataPtr);
  }
  else
  {
    QCRIL_LOG_ERROR("UIM remote SIM is not in connected Mode - Not sending CardAppInitStatusInd to remote clients ");
    return;
  }
} /* UimRemoteClientModule::handleUimRemoteCardAppInitStatusIndMessage */
#endif
