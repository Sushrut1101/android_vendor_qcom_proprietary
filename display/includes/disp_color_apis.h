...
struct csc_config_data {
  int32_t warmness;
  struct disp_pa_config_data data;
};

struct csc_setting_param {
  int warmness, hue, saturation, intensity, contrast;
};

struct CONV_CSC_CFG_PARAMS {
  int pre_bv[3];
  int post_bv[3];
  uint32_t pre_lv[6];
  uint32_t post_lv[6];
  float mv[9]; //3x3 conversion matrix
};
