 ...
 *
 * This is compatible with version 00.09.01 of the IDL.
 *
 * ---------------------------------------------------------------------------
...
public class EmbmsOemHook extends Handler{
    ...
    /*
     * Size in bytes.
     * For example, size in bytes of the length within V field of TLV.
    ...
    //getPLMNList
    private static final byte TLV_TYPE_GET_PLMN_LIST_RESP_PLMN_LIST = 0x02;
    //getEmbmsStatus
    private static final short TLV_TYPE_GET_EMBMS_STATUS_RESP = 0x02;
    //getInterestedTmgiListResponse
    private static final byte TLV_TYPE_GET_INTERESTED_TMGI_LIST_RESP_TMGI = 0x03;
    ...
    private static final short EMBMSHOOK_MSG_ID_UNSOL_GET_INTERESTED_TMGI_LIST = 0x22;
    private static final short EMBMSHOOK_MSG_ID_GET_INTERESTED_TMGI_LIST_RESP = 0x23;
    ...
    public static final int UNSOL_TYPE_GET_INTERESTED_TMGI_LIST = 0x0D;
    ...
    private void handleResponse(HashMap<Integer, Object> map) {
        ...
        switch (msgId) {
            ...
            case EMBMSHOOK_MSG_ID_UNSOL_GET_INTERESTED_TMGI_LIST: {
                RequestIndication ind = new RequestIndication(respByteBuf);
                notifyUnsol(UNSOL_TYPE_GET_INTERESTED_TMGI_LIST, ind, phoneId);
                break;
            }
            case EMBMSHOOK_MSG_ID_GET_INTERESTED_TMGI_LIST_RESP:
                // Clients are not expecting response, so not parsing.
                Log.v(LOG_TAG, " getInterestedTmgiListResponse ack successStatus="
                        + successStatus);
                break;
            default:
    ...
    /**
     * Send the get interested tmgi list response from client to RIL.
     */
    public int getInterestedTMGIListResponse(int traceId, byte callId, byte[] tmgiList,
            int phoneId, Message msg) {
        try {
            GetInterestedTmgiResponse req = new GetInterestedTmgiResponse(traceId, callId,
                    tmgiList);

            mQmiOemHook.sendQmiMessageAsync(EMBMS_SERVICE_ID,
                    EMBMSHOOK_MSG_ID_GET_INTERESTED_TMGI_LIST_RESP, req.getTypes(),
                    req.getItems(), msg, phoneId);
        } catch (IOException e) {
    ...
    /**
     * Represents a general request indication to the client that has only the following parameter
     * - traceId.
     * When the indication is sent, the client is expected to send the response.
     * For example, UNSOL_TYPE_GET_INTERESTED_TMGI_LIST.
     */
                ...
                try {
                    ...
                    switch (type) {
                        case TLV_TYPE_COMMON_RESP_TRACE_ID:
                            ...
                            break;
                        default:
                            Log.e(LOG_TAG, "RequestIndication: Unexpected Type " + type);
                            break;
                    }
                } catch (BufferUnderflowException e) {
                    Log.e(LOG_TAG, "Unexpected buffer format when parsing for RequestIndication");
                }
            }
        }
    }
    ...
    /**
     * Utility to convert a tmgi list byte array to a QmiArray of QmiBytes.
     * @param vSize Size of length within V field of TLV
     */
    private QmiArray<QmiByte> tmgiListArrayToQmiArray (short vSize, byte[] tmgiList){
        int length = (tmgiList == null)? 0 : tmgiList.length;

        int numOfTmgi = length / SIZE_OF_TMGI;
        length += (ONE_BYTE*numOfTmgi);
        QmiByte[] qmiByteArray = new QmiByte[length];
        int index = 0;

        // Convert tmgiList array to an array that has size of tmgi followed by tmgi.
        // So array will look like - [tmgiLength][tmgi][tmgiLength][tmgi]...

        for(int i = 0; i < numOfTmgi; i++) {
            qmiByteArray[index++] = new QmiByte(SIZE_OF_TMGI);
            for (int j = (i*SIZE_OF_TMGI); j < ((i+1)*SIZE_OF_TMGI) ; j++) {
                qmiByteArray[index++] = new QmiByte(tmgiList[j]);
            }
        }

        // Here the length within V of TLV contains the number of tmgis. Therefore the
        // numOfElements to be considered in one set of the qmi array is (SIZE_OF_TMGI + ONE_BYTE),
        // ONE_BYTE is needed to account for the length of tmgi that is added in V of the TLV.
        short numOfElements = (short)SIZE_OF_TMGI + ONE_BYTE;
        return new QmiArray<QmiByte>(qmiByteArray, QmiByte.class, vSize, numOfElements);
    }
    ...
    /**
     * Represents the response sent to RIL after receiving the get
     * interested tmgi list unsol.
     *
     */
     public class GetInterestedTmgiResponse extends BaseQmiStructType {
         ...
         public GetInterestedTmgiResponse(int traceId, byte callId, byte[] tmgiList) {
             this.traceId = new QmiInteger(traceId);
             this.callId = new QmiByte(callId);
             this.tmgiList = tmgiListArrayToQmiArray(ONE_BYTE, tmgiList);
         }

        @Override
        public BaseQmiItemType[] getItems() {
            return new BaseQmiItemType[] {traceId, callId, tmgiList};
        }
        ...
        public short[] getTypes() {
            return new short[] {TLV_TYPE_COMMON_REQ_TRACE_ID,
                                TLV_TYPE_COMMON_REQ_CALL_ID,
                                TLV_TYPE_GET_INTERESTED_TMGI_LIST_RESP_TMGI};
        }
     }
    ...
    public class TimeResponse {
        ...
        public TimeResponse(int traceId, int status, long timeMseconds, boolean additionalInfo,
                ...
                try {
                    ...
                    switch (type) {
                        case TLV_TYPE_GET_PLMN_LIST_RESP_PLMN_LIST:
                            ...
                            // The client expects each plmn of size 6 bytes (mcc - 3 bytes,
                            // mnc - 3 bytes). But mnc received from RIL can be of 2 bytes. In
                            // this case, add a space as the as the 3rd byte to make the mnc of
                            // 3 bytes.
                            byte space = ' ';
                            this.plmnList = new byte[SIZE_OF_EACH_PLMN_IN_BYTES * numOfPlmn];
                            int index = 0;
                            for (int i = 0; i < numOfPlmn; i++) {
                                byte mccLen = buf.get();
                                buf.get(plmnList, index, mccLen);
                                index += mccLen;
                                byte mncLen = buf.get();
                                buf.get(plmnList, index, mncLen);
                                index += mncLen;
                                if (mncLen == TWO_BYTES){
                                    plmnList[index++] = space;
                                }
                            }
