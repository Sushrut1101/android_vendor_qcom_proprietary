...
public class CaseManager implements CaseOpCallback, ControllerInterface {
    ...
    private void initCasesAndResult(TestMode type) {
        ...
        Map<String, PlatformMapInfo> platformMap = new HashMap<String, PlatformMapInfo>();
        String platform = null;

        ConfigParser.parsePlatformMapFile(mContext.getResources().getXml(
                R.xml.platform_mapping_config), platformMap);
        platform = Utils.getSystemProperties("ro.board.platform", "default");
        LogUtils.logi("ro.board.platform is:" + platform);
        if (platformMap.containsKey(platform)) {
            int testCaseId = mContext.getResources().getIdentifier(platformMap.get(platform).getTestCaseFile(), "xml", mContext.getPackageName());
            int caseListId = mContext.getResources().getIdentifier(platformMap.get(platform).getCaseListFile(), "xml", mContext.getPackageName());
            /**Load case from configure*/
            ConfigParser.parseCaseConfigFile(mContext.getResources().getXml(testCaseId), allTestCasesList, allTestCasesMap);
            ConfigParser.parsePlatformConfigFile(mContext.getResources().getXml(caseListId), platformInfoMap, platform);
        } else {
            /**Load case from configure*/
            ...
            ConfigParser.parsePlatformConfigFile(mContext.getResources().getXml(
                    R.xml.platform_case_config), platformInfoMap, ConfigParser.PLATFORM_DEFAULT);
        }

        PlatformInfo platformInfo = null;
        if (platformInfoMap.containsKey(platform)) {
            platformInfo = platformInfoMap.get(platform);
        } else {
            platformInfo = platformInfoMap.get(ConfigParser.PLATFORM_DEFAULT);
        }
        ...
        if (uiCasesList.size() < 1 && platformInfo.getUiCases() != null) {
        ...
        if (pcbaCasesList.size() < 1 && platformInfo.getPcbaCases() != null) {
        ...
        if (assmebleCasesList.size() < 1 && platformInfo.getAssembleCases() != null) {
