...
03/03/15   yt      Support for QMI_UIM_GET_CONFIGURATION
02/24/15   yt      Set qmi_err_code correctly in case of transport error
02/11/15   at      Support for GBA APIs
...
static qmi_client_os_params                qmi_uim_client_init_os_params;
...
)
{
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    else if (qmi_response->long_select_response_valid)
    {
      ...
      /* For sync calls this is freed by client */
      rsp_data->rsp_data.logical_channel_rsp.select_response.data_ptr =
        qcril_malloc(qmi_response->long_select_response_len);
      if(rsp_data->rsp_data.logical_channel_rsp.select_response.data_ptr == NULL)
      {
        QCRIL_LOG_ERROR("%s", "data pointer NULL");
        rsp_data->qmi_err_code = QMI_SERVICE_ERR;
...
)
{
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    else if(qmi_response->long_status_response_valid)
    {
      ...
      rsp_data->rsp_data.send_status_rsp.status_response.data_ptr = qcril_malloc(qmi_response->long_status_response_len);
      if(rsp_data->rsp_data.send_status_rsp.status_response.data_ptr == NULL)
      {
        QCRIL_LOG_ERROR("%s", "data pointer NULL");
        rsp_data->qmi_err_code = QMI_SERVICE_ERR;
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    if(qmi_response->ks_naf_valid)
    {
      ...
      rsp_data->rsp_data.gba_rsp.ks_naf.data_ptr =
        qcril_malloc(sizeof(qmi_response->ks_naf));
    ...
    else if(qmi_response->encrypted_ks_naf_valid)
    {
      ...
      /* For sync calls this is freed by client */
      rsp_data->rsp_data.gba_rsp.ks_naf.data_ptr =
        qcril_malloc(qmi_response->encrypted_ks_naf_len);
      if(rsp_data->rsp_data.gba_rsp.ks_naf.data_ptr == NULL)
      {
        QCRIL_LOG_ERROR("%s", "data pointer NULL");
        rsp_data->qmi_err_code = QMI_SERVICE_ERR;
    ...
    if(qmi_response->btid_valid)
    {
      ...
      rsp_data->rsp_data.gba_rsp.b_tid.data_ptr =
        qcril_malloc(qmi_response->btid_len);
    ...
    if(qmi_response->lifetime_valid)
    {
      ...
      rsp_data->rsp_data.gba_rsp.key_lifetime.data_ptr =
        qcril_malloc(qmi_response->lifetime_len);
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    if(qmi_response->encrypted_impi_valid)
    {
      ...
      rsp_data->rsp_data.gba_impi_rsp.impi.data_ptr =
        qcril_malloc(qmi_response->encrypted_impi_len);
    ...
    else if(qmi_response->impi_valid)
    {
      ...
      /* For sync calls this is freed by client */
      rsp_data->rsp_data.gba_impi_rsp.impi.data_ptr =
        qcril_malloc(qmi_response->impi_len);
      if(rsp_data->rsp_data.gba_impi_rsp.impi.data_ptr == NULL)
      {
        QCRIL_LOG_ERROR("%s", "data pointer NULL");
        rsp_data->qmi_err_code = QMI_SERVICE_ERR;
...
)
{
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    if(qmi_response->temporary_depersonalization_status_valid)
    {
      ...
      for(i = 0; i < qmi_response->temporary_depersonalization_status[0].temporary_unlock_status_len &&
                 i < QMI_UIM_MAX_PERSO_FEATURES; i++)
      {
        rsp_data->rsp_data.get_configuration_rsp.temp_perso_status[i].feature =
          qmi_response->temporary_depersonalization_status[0].temporary_unlock_status[i].feature;
  ...
  if(qmi_response->physical_slot_status_valid)
  {
    ...
    for(slot_id = 0; slot_id < qmi_response->physical_slot_status_len; slot_id++)
    {
      ...
      rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].logical_slot=
                                      qmi_response->physical_slot_status[slot_id].logical_slot;
      rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].iccid_len =
                                      qmi_response->physical_slot_status[slot_id].iccid_len;
      memcpy(rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].iccid, qmi_response->physical_slot_status[slot_id].iccid,
      ...
      for (i = 0; i < qmi_response->terminal_capability_tlv_len; i++)
      {
        ...
        if (qmi_response->terminal_capability_tlv[i].value_len > 0)
        {
          ...
          rsp_data->rsp_data.terminal_capability_rsp.tlvs[i].value.data_ptr =
            qcril_malloc(qmi_response->terminal_capability_tlv[i].value_len);
...
)
{
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    {
      rsp_data->rsp_data.remote_simlock_version.major_version =
        qmi_response->maximum_version.major_version;
      rsp_data->rsp_data.remote_simlock_version.minor_version =
    ...
    {
      rsp_data->rsp_data.remote_unlock_timer = qmi_response->delay_timer_value;
    }

    if (resp_ptr != NULL && resp_len != 0)
    {
      rsp_data->rsp_data.remote_unlock_rsp.data_ptr =
            (uint8_t *)qcril_malloc(sizeof(uint8_t) * resp_len);
      if (rsp_data->rsp_data.remote_unlock_rsp.data_ptr != NULL)
      {
        memcpy(rsp_data->rsp_data.remote_unlock_rsp.data_ptr,
               resp_ptr,
               resp_len);
        rsp_data->rsp_data.remote_unlock_rsp.data_len = resp_len;
      }
    }
  }
  else
  ...
  if(ind_data_ptr != NULL && ind_data != NULL)
  {
       ...
       ind_data_ptr->physical_slot_status_len <= 5)
    {
      ...
      for (i = 0; i < ind_data->no_of_slots && i < QMI_UIM_MAX_SLOT_COUNT; i++)
      {
        ...
        memcpy(ind_data->slot_status[i].iccid, ind_data_ptr->physical_slot_status[i].iccid, QMI_UIM_MAX_ICCID_LEN);
      }
    }
    else
    {
    ...
    if (QMI_NO_ERR == qmi_err)/* what is going on here */
    {
      switch(msg_id)
      {
        ...
        case QMI_UIM_SAP_CONNECTION_IND_V01:
          ...
          ind_id = QMI_UIM_SRVC_SAP_CONNECTION_IND_MSG;
          if (decoded_payload != NULL)
          {
            uim_sap_connection_ind_msg_v01 * msg_ptr = (uim_sap_connection_ind_msg_v01 *)decoded_payload;
            qmi_uim_sap_connection_ind_msg * ind_msg_ptr = (qmi_uim_sap_connection_ind_msg *)ind_data;

            ind_msg_ptr->sap_connection_event_valid = msg_ptr->sap_connection_event_valid;
            ind_msg_ptr->sap_connection_event.slot = (qmi_uim_slot_type)msg_ptr->sap_connection_event.slot;
            ind_msg_ptr->sap_connection_event.sap_state = msg_ptr->sap_connection_event.sap_state;
          }
          break;
...
)
{
  ...
  switch (msg_id)
  {
    ...
    case QMI_UIM_REMOTE_UNLOCK_RESP_V01:
      if (rsp_data->rsp_data.remote_unlock_rsp.data_ptr)
      {
        qcril_free(rsp_data->rsp_data.remote_unlock_rsp.data_ptr);
        rsp_data->rsp_data.remote_unlock_rsp.data_ptr = NULL;
        rsp_data->rsp_data.remote_unlock_rsp.data_len = 0;
      }
      break;
...
)
{
  ...
  QCRIL_NOTUSED(user_rx_ind_msg_hdlr);
  QCRIL_NOTUSED(user_rx_ind_msg_hdlr_user_data);
  ...
  memset(&qmi_uim_client_init_os_params, 0x00, sizeof(qmi_client_os_params));
...
)
{
  ...
  if(params->apdu.data_len > QMI_UIM_APDU_DATA_MAX_V01)
  {
    ...
    if(!is_get_config_done)
    {
      ...
      rc = qcril_qmi_uim_get_configuration(qcril_uim.qmi_handle,
                                            &get_config_params,
                                            NULL,
                                            NULL,
                                            &get_data_rsp) ;
