...
char boot_fail_property_string[PROPERTY_VALUE_MAX];
char boot_fail_property_string_copy[PROPERTY_VALUE_MAX];
char *saveptr, *token;
#endif
/*
		...
		if (devinfo.mdm_list[i].type == MDM_TYPE_EXTERNAL) {
			...
			if (!strcmp(devinfo.mdm_list[i].mdm_link,
				...
				strlcpy(dev[count].mdm_link,
					...
					sizeof(dev[count].mdm_link));
			strlcpy(dev[count].mdm_link_extra_data,
					devinfo.mdm_list[i].mdm_link_extra_data,
					sizeof(dev[count].mdm_link_extra_data));
			strlcpy(dev[count].mdm_port,
					devinfo.mdm_list[i].drv_port,
					sizeof(dev[count].mdm_port));
			dev[count].device_descriptor = -1;
...
int mdm_helper_init()
{
	...
	for (i = 0; i < mdm_helper.num_modems; i++) {
		if (!mdm_helper.dev[i].ops.power_up ||
				!mdm_helper.dev[i].ops.monitor) {
			ALOGE("power_up/monitor undefined for %s\n",
	...
	do {
		...
		case MDM_HELPER_STATE_POWERUP:
			...
			if (dev->ops.power_up(dev)
				...
				ALOGI("%s : finished powerup init, moving to monitor state", dev->mdm_name);
				state = MDM_HELPER_STATE_MONITOR;
            }
			break;
		case MDM_HELPER_STATE_SAHARA:
			ALOGI("%s : switching state to SAHARA:", dev->mdm_name);
			if (dev->ops.sahara(dev)
				!= RET_SUCCESS){
				ALOGI("%s : SAHARA op returned exception. Check whether we retry or fail.", dev->mdm_name);
				if (dev->required_action ==
					...
					state = MDM_HELPER_STATE_MONITOR;
					dev->required_action = MDM_REQUIRED_ACTION_NONE;
					ALOGI("%s : SAHARA requesting RETRY. Setting state back to MONITOR.", dev->mdm_name);
				}
				else{
					state = MDM_HELPER_STATE_FAIL;
					dev->required_action = MDM_REQUIRED_ACTION_NONE;
					ALOGI("%s : SAHARA returned failure. Moving to FAIL state.", dev->mdm_name);
                }
            }
            else
                state = MDM_HELPER_STATE_MONITOR;
			break;
		case MDM_HELPER_STATE_MONITOR:
			ALOGI("%s : switching state to POST POWERUP/monitor",
					dev->mdm_name);
			if (dev->ops.monitor(dev) !=
					RET_SUCCESS) {
			...
			ALOGE("%s : monitor returned unsupported action:%d",
					dev->mdm_name,
					dev->required_action);
			state = MDM_HELPER_STATE_FAIL;
			break;
...
int main(int argc, char *argv[])
{
	...
	//Get the boot fail action property (defaults to panic) and start the tokenizer
	//Copy of the property string is required as strtok_r injects null characters in original string
#ifdef ESOC_BOOT_FAIL_ACTION
	property_get(BOOT_FAIL_PROPERTY, boot_fail_property_string, "panic");
	...
	if (mdm_helper_init()) {
		...
		//Initialization failing indicates something
		//fundamentally wrong in the lower layers and
		//is not something that we can recover from by
		//restarting the daemon which was happenning
		//(repeatedly) in the earlier approach. We now
		//just print a error and go to sleep.
		goto bail;
	}
	if (argc > 1) {
