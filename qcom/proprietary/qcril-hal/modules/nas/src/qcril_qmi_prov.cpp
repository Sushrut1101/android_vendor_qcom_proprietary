...
)
{
    ...
    qmi_client_error =
        qmi_client_nas_send_sync(QMI_NAS_GET_MODE_PREF_REQ_MSG_V01,
                                   NULL,
    ...
    {
        if ( UIM_SUBSCRIPTION_1 == select_uicc_sub.sub_type )
        {
            mode_pref =
        ...
        else if ( UIM_SUBSCRIPTION_2 == select_uicc_sub.sub_type )
        {
            mode_pref =
        ...
        memcpy( &req_info_ptr->select_uicc_sub, &select_uicc_sub, sizeof( RIL_UIM_SelectUiccSub ) );
...
int qcril_qmi_prov_get_app_index_for_deactivation(void)
{
    ...
    // Deactivate SIM app in reverse order of activation to make sure there
    // are no race conditions
    if ( cdma_provision_state == QCRIL_APP_STATUS_PENDING_DEPROVISIONING )
    {
        app_index = qcril_qmi_prov_get_cdma_app_index();
    }
    else if ( gw_provision_state == QCRIL_APP_STATUS_PENDING_DEPROVISIONING )
    {
        app_index = qcril_qmi_prov_get_gw_app_index();
    }
...
)
{
    ...
    RIL_UIM_SelectUiccSub                    *sub_ptr;
    RIL_UIM_SubscriptionType                  sub_num;
    qcril_modem_id_e_type                     modem_id;
    ...
    if( provision_info_ptr != NULL )
    {
        ...
        {
          sub_ptr = (RIL_UIM_SelectUiccSub *)&req_info.sub.nas.select_uicc_sub;
          if ( ( req_info.request == QCRIL_EVT_RIL_REQUEST_MANUAL_PROVISIONING ) &&
               ( sub_ptr->act_status == UIM_UICC_SUBSCRIPTION_ACTIVATE ) )
          {
              ...
              else
              {
                  ...
                  switch ( provision_info_ptr->session_type )
                  {
                      ...
                      case UIM_SESSION_TYPE_NON_PROV_SLOT_3:
                      case UIM_SESSION_TYPE_CARD_SLOT_3:
                          sub_num = UIM_SUBSCRIPTION_3;
                          break;

                      case UIM_SESSION_TYPE_SEC_GW_PROV:
                      case UIM_SESSION_TYPE_SEC_1X_PROV:
                      case UIM_SESSION_TYPE_NON_PROV_SLOT_2:
                      case UIM_SESSION_TYPE_CARD_SLOT_2:
                          sub_num = UIM_SUBSCRIPTION_2;
                          break;

                      case UIM_SESSION_TYPE_PRI_GW_PROV:
                      case UIM_SESSION_TYPE_PRI_1X_PROV:
                      case UIM_SESSION_TYPE_NON_PROV_SLOT_1:
                      case UIM_SESSION_TYPE_CARD_SLOT_1:
                      default:
                          sub_num = UIM_SUBSCRIPTION_1;
                          break;
                  }
              }
              ...
              ** any other apps to activate.*/
                   ...
                   provision_info_ptr->status == QCRIL_PROVISION_STATUS_SUCCESS )
              {
                  qcril_qmi_prov_update_app_provision_status_using_app_index(sub_ptr->app_index,
                                                                             sub_ptr->act_status,
                                                                             (RIL_Errno)provision_info_ptr->err_code);
                  ...
                  ** for another app */
                  ...
                  {
                      // this function might return failure due to get mode pref failure.
                      // However, app status will be updated when this function returns with failure.
                      res = qcril_qmi_prov_activate_deactivate_app( req_info.t,
                                                                    req_info.request,
                                                                    UIM_UICC_SUBSCRIPTION_ACTIVATE );
                  }

                  // If second app activation fails or no other app to activate send response.
                  if ( res == RIL_E_GENERIC_FAILURE ||
                       qcril_qmi_prov_is_any_app_being_provisioned() == FALSE )
                  {
...
)
{
    ...
    if( provision_info_ptr != NULL )
    {
        if ( qcril_reqlist_query_by_event( instance_id, modem_id, QCRIL_EVT_CM_DEACTIVATE_PROVISION_STATUS, &req_info ) == E_SUCCESS )
        {
            sub_ptr = (RIL_UIM_SelectUiccSub *)&req_info.sub.nas.select_uicc_sub;
            if ( ( req_info.request == QCRIL_EVT_RIL_REQUEST_MANUAL_PROVISIONING ) &&
                 ( sub_ptr->act_status == UIM_UICC_SUBSCRIPTION_DEACTIVATE ) )
            {
              if ( provision_info_ptr->status == QCRIL_PROVISION_STATUS_FAILURE )
              {
                   ...
                   provision_info_ptr->status == QCRIL_PROVISION_STATUS_SUCCESS )
              {
                  ...
                  ** any other apps to deactivate.*/
                  if ( qcril_qmi_prov_is_any_app_being_deprovisioned() )
                  {
                      res = qcril_qmi_prov_activate_deactivate_app( req_info.t,
                                                                    req_info.request,
                                                                    UIM_UICC_SUBSCRIPTION_DEACTIVATE );
                  }

                  // if second app deactivation fails or no other app to deactivate send response.
                  if ( res == RIL_E_GENERIC_FAILURE ||
                       qcril_qmi_prov_is_any_app_being_deprovisioned() == FALSE )
                  {
                    prov_state = qcril_qmi_prov_get_prov_state();
