...
public class WFDSession extends IWfdSession.Stub implements UIBCManager.HIDCEventListener, UIBCManager.EventListener {
    ...
    private static final String sDebugKey = "persist.vendor.debug.wfd.wfdsvc";
    ...
    private WFDSession(Context c) {
        ...
        mReceiver = new BroadcastReceiver() {
            public void onReceive(Context context, Intent intent) {
                ...
                synchronized (mLock) {
                    if (action.equals(WindowManagerPolicyConstants.ACTION_HDMI_PLUGGED)) {
    ...
    private void eventCallback(String eventName, Object[] objectArray) {
        ...
        if ("Error".equalsIgnoreCase(eventName)) {
            ...
            if (objectArray.length >= 1) {
                ...
                notify(b, -1);
                Log.d(mTAG, eventName + "Angle = " + angle);
            }
                    ...
                    if (array_length > 4) {
                        ...
                        try {
                            if (objectArray[4] != null) {
                                Log.d(mTAG, objectArray[4].getClass().getName());
                ...
                if (mDeviceType != WFDDeviceType.SOURCE) {
                    if(mSurface != null) {
                        ...
                        if(mSurface.isValid()) {
                            ...
                            mSurface = null;
                            Log.d(mTAG,"Released Surface during TEARDOWN Done!");
                        }
                    }
                } else {
                    releaseVDS();
    ...
    private void releaseVDS() {
        synchronized (mLock) {
            if (mVirtualDisplay != null) {
                Log.d(mTAG, "Releasing Virtual Display");
    ...
    private void broadcastWifiDisplayAudioIntent(boolean flag) {
        ...
        if (flag) {
            if (mRequestProxyRouting) {
                Log.d(mTAG, "Calling Audio System Proxy enable");
