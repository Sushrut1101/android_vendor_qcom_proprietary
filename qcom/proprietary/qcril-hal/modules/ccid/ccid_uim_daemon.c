...
/* Define default slot number for reading sap_state */
uint8 ccid_uim_slot = 0;
/* Global mutex */
...
)
{
  ...
  if (is_connect)
  {
    ...
    sap_connect_params.connection_condition_valid      = TRUE;
    sap_connect_params.connection_condition            = UIM_SAP_CONNECTION_COND_BLOCK_NONE_V01;
  }
  else
  {
...
)
{
  ...
  if (sap_conn_ind_ptr->sap_connection_event.sap_state != ccid_uim.sap_state)
  {
    ccid_uim.sap_state = sap_conn_ind_ptr->sap_connection_event.sap_state;
...
)
{
  ...
  /* Update global in case of new card state or error code
    Checking only for slot 1
  */

  if (qmi_response_ptr->card_status.card_info[ccid_uim_slot].card_state == UIM_CARD_STATE_ERROR_V01 &&
    qmi_response_ptr->card_status.card_info[ccid_uim_slot].error_code == UIM_CARD_ERROR_CODE_SAP_CONNECTED_V01)
  {
    ccid_uim.sap_state = UIM_SAP_STATE_CONNECTED_SUCCESSFULLY_V01;
  }
...
)
{
  ...
  {
    if (ccid_uim.sap_state == UIM_SAP_STATE_CONNECTED_SUCCESSFULLY_V01)
    {
...
{
  /* checking if slot number is passed as arg */
  if (argc > 1)
  {
    ccid_uim_slot = atoi(argv[1]);
  }
