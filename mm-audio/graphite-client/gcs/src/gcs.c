...
{
    bool valid = false;
    if (pdata->state >= GCS_STATE_MAX) {
        OSAL_LOGE("%s: invalid state %d\n", __func__, pdata->state);
        return valid;
    }
    switch (pdata->state) {
    case GCS_GRAPH_IDLE:
...
static int32_t __get_config(struct gcs_graph_pdata *pdata,
                            struct gcs_config_rsp *response)
{
    int32_t ret = OSAL_SUCCESS;
    struct pktzr_cmd_rsp *cmd_rsp;
    struct graphite_basic_rsp_result *basic_rsp;

    ret = gcs_allocate_cmd_rsp(pdata, &cmd_rsp);
    if (ret)
        goto done;

    ret = pktzr_get_config(pdata->payload, pdata->payload_data_size, cmd_rsp);
    if (ret) {
        OSAL_LOGE("%s: get_config command failed, ret = %d\n",
                  __func__, ret);
        goto cleanup_cmd_rsp;
    }

    if (!cmd_rsp->rsp_size) {
        OSAL_LOGE("%s: 0 size rsp received\n", __func__);
        goto cleanup_cmd_rsp;
    }

    /* Basic response is returned in case of errors */
    if (cmd_rsp->is_basic_rsp == true) {
        basic_rsp = (struct graphite_basic_rsp_result *) cmd_rsp->buf;
        ret = gcs_check_graphite_response(basic_rsp->status);
        goto cleanup_cmd_rsp;
    }

    /* Caller did not allocate enough memory to do copy */
    if (cmd_rsp->rsp_size > response->rsp_size_requested) {
        response->rsp_size_requested = cmd_rsp->rsp_size;
        ret = OSAL_ENOMEMORY;
        goto cleanup_cmd_rsp;
    }

    osal_mem_cpy(response->rsp_buf, response->rsp_size_requested,
                 cmd_rsp->buf, cmd_rsp->rsp_size);
    response->rsp_size_returned = cmd_rsp->rsp_size;
...
static int32_t gcs_stop_read(struct gcs_graph_pdata *pdata)
{
    ...
    /*
     * Even if __graph_close fails, go ahead and switch the state machine
     * so that further close calls (gcs_close) etc can cleanup the session
     */
...
static int32_t __gcs_config(uint32_t graph_handle, void *payload,
                            uint32_t payload_size, enum gcs_fsm_cmd cmd,
                            struct gcs_config_rsp *response)
{
    int32_t ret = OSAL_SUCCESS;
    struct gcs_graph_pdata *pdata;

    /* __gcs_config should only be called for GCS_CMD_[GET|SET]_CONFIG */
    if (cmd != GCS_CMD_SET_CONFIG && cmd != GCS_CMD_GET_CONFIG) {
        OSAL_LOGE("%s: Invalid config cmd %d\n", __func__, cmd);
        ret = OSAL_EBADPARAM;
        goto exit;
    }
    ...
    if (!payload || !payload_size || (payload_size > MAX_GCS_PLD_SIZE)) {
        OSAL_LOGE("%s: Invalid %s_config payload %s. Payload size = %d\n",
                  __func__, gcs_fsm_cmd_name[cmd],
    ...
    if (!gcs_is_valid_state(pdata, cmd, __func__)) {
        ret = OSAL_EBADPARAM;
    ...
    if (ret ) {
        OSAL_LOGE("%s: %s_config payload copy failed, ret = %d\n",
                  __func__, gcs_fsm_cmd_name[cmd], ret);
        goto reset_payload_data_size;
    }

    /* Send get/set Config command to IPC layer */
    if (cmd == GCS_CMD_SET_CONFIG)
        ret = __set_config(pdata);
    else
        ret = __get_config(pdata, response);
    if (ret) {
        OSAL_LOGE("%s: %s_config failed, ret = %d\n", __func__,
                  gcs_fsm_cmd_name[cmd], ret);
        goto reset_payload_data_size;
    }
...
/*
 * gcs_get_config - Get configuration parameters from an active
 *                  module in graph
 *
 * @graph_handle:    handle to the graph in GCS
 * @req_pld:         Payload for the request command
 * @req_pld_size:    size of the request command payload
 * @response:        structure to hold the response of the
 *                   config items to be read.
 *
 * Returns success or error code. The request payload will
 * be sent to server as-is. If the size of parameter data
 * returned from graphite server is larger than the size
 * requested by caller, OSAL_ENOMEMORY error will be
 * returned to caller.
 *
 * Prerequisites
 *     Graph must be opened and enabled.
 */
int32_t gcs_get_config(uint32_t graph_handle, void *req_pld,
                       uint32_t req_pld_size,
                       struct gcs_config_rsp *response)
{
    if (!response) {
        OSAL_LOGE("%s: response structure required for get_config\n",
                  __func__);
        return OSAL_EBADPARAM;
    }

    return __gcs_config(graph_handle, req_pld, req_pld_size,
                        GCS_CMD_GET_CONFIG, response);
}
...
{
        return __gcs_config(graph_handle, payload, payload_size,
                            GCS_CMD_SET_CONFIG, NULL);
}
