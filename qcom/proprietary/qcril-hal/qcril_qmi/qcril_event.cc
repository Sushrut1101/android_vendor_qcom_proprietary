...
{
  pthread_t tid;
  qtimutex::QtiSharedMutex list_mutex;
  qtimutex::QtiSharedMutex startup_mutex;
  ...
  qtimutex::QtiSharedMutex activity_lock_mutex;
} qcril_event_static_data_type;

static qcril_event_static_data_type qcril_event;

static std::condition_variable_any qcril_event_startupCond;
...
class qcril_event_module: public Module
{
    public:
        ...
        void handle_qcril_event(std::shared_ptr<QcrilEventMessage> msg) {
            ...
            int res = qcril_event.activity_lock_mutex.try_lock();
            ...
            if (res)
            {
              qcril_event.activity_lock_mutex.unlock();
            }
...
void qcril_event_start( void )
{
  ...
  qcril_event_startupCond.notify_all();
...
{
  qcril_event.activity_lock_mutex.lock();
} // qcril_event_suspend
//===========================================================================
//qcril_event_resume
//===========================================================================
...
)
{
  ...
  if ( E_SUCCESS != err_no )
  {
    if (qcril_is_event_in_group(params.event_id, &QCRIL_GRP_OEM_HOOK))
    {
