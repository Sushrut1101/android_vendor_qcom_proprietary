...
qti_audio_proxy(const sp<IBinder>& impl) : BpInterface<Iqti_audio_server>(impl) {
    ...
    g_audio_binder = interface_cast<IQASClient>(binder);
    data.writeInterfaceToken(Iqti_audio_server::getInterfaceDescriptor());
    data.writeStrongBinder(IInterface::asBinder(g_audio_binder));
    status_t status = remote()->transact(REGISTER_CLIENT, data, &reply);
    if (status)
        ALOGD("qti_audio_proxy: Client registration failed");
}
...
~qti_audio_proxy() {
    ...
    if (status)
        ALOGD("qti_audio_proxy: Unregistering client failed");
}
...
ssize_t qahw_out_write(qahw_stream_handle_t *stream,
    ...
    ALOGV("Taking shared mem mutex %p", &p_stream->shmem_lock);
    ...
    //copy buff
    if (out_buf->bytes > shmem_data->hal_buf_bytes) { //some freaking problem
        ALOGE("Received more bytes %d than allocated bytes %d",
               out_buf->bytes, shmem_data->hal_buf_bytes);
        goto exit;
    }

    memscpy((void *)shmem_data_buff, shmem_data->hal_buf_bytes, (char *)out_buf->buffer, out_buf->bytes);
    ...
    rc = *return_value;
    ALOGV("Unlock shared mem mutex %p", &p_stream->shmem_lock);
    pthread_mutex_unlock(&p_stream->shmem_lock);
...
int qahw_out_get_presentation_position(const qahw_stream_handle_t *out_handle,
    ...
    // is the server thread waiting for commands
    ALOGV("Taking command mutex %zd", &shmem_data->c_mutex);
    rc = pthread_mutex_lock(&shmem_data->c_mutex);

    shmem_data->status = -1;

    //wakeup server stream thread.
    ALOGV("Waking up stream thread(%zd:%zd)",
        p_stream, p_stream->n_stream_handle);
    rc = pthread_cond_signal(&shmem_data->c_cond);
    ALOGV("Woken up stream thread(%zd:%zd) %d",
        p_stream, p_stream->n_stream_handle, rc);

    while (shmem_data->status == -1) {
        ALOGV("Waiting for status signal stream(%zd:%zd)",
            p_stream, p_stream->n_stream_handle);

        rc = pthread_cond_wait(&shmem_data->c_cond, &shmem_data->c_mutex);

        ALOGV("Waiting ended for status signal stream(%zd:%zd) %d",
            p_stream, p_stream->n_stream_handle, rc);
    }
    rc = pthread_mutex_unlock(&shmem_data->c_mutex);
    *frames = shmem_pts_buff->frames;
    memscpy(timestamp, sizeof(*timestamp), &shmem_pts_buff->timestamp, sizeof(*timestamp));
    ALOGD("%s: frames %lld played at %lld", __func__, (long long int)*frames,
...
ssize_t qahw_in_read(qahw_stream_handle_t *stream,
    ...
    if (in_buf->bytes > shmem_data->hal_buf_bytes) {
        ALOGE(" cannot read bytes %d than allocated bytes %d",
                in_buf->bytes, shmem_data->hal_buf_bytes);
        goto exit;
    }
...
int qahw_get_param_data(const qahw_module_handle_t *hw_module,
    ...
    status_t status = data.writeBlob(param_size, false, &w_blob);
    if (status) {
        ALOGE("%s: Error in writeblob, status %d", __func__, status);
        return -EINVAL;
    }
    memset(w_blob.data(), 0x0, param_size);
    memscpy(w_blob.data(), param_size, payload, param_size);
...
int prepare_for_write(qahw_stream_handle_t *stream,
    ...
    ptr->hal_buf_bytes = shmem_data_buff_size;
    ...
    rc = reply.readInt32();
    if (rc) {
        ALOGE("%s: PREPARE_FOR_WRITE failed %d", __func__, rc);
        goto exit;
    }
