...
int16_t VTPlayer::setFarEndSurface(JNIEnv * env, jobject jSurfacefar)
{
  int16_t slStatus = -1;
  sp<ANativeWindow> nativeWindow = NULL;
  sp<IGraphicBufferProducer> producer = NULL;
  sp<Surface> surfaceFar = NULL;

  do {
    if (NULL == jSurfacefar) {
      ERROR1("setFarEndSurface is NULL so not setting it");
    ...
    surfaceFar = reinterpret_cast<Surface *>(env->GetLongField(jSurfacefar,
                 surfaceID));

    if (NULL == surfaceFar.get()) {
      ERROR1("setFarEndSurface ignoring as surfaceFar is NULL");
      break;
    }

    producer = surfaceFar->getIGraphicBufferProducer();

    if (NULL == producer.get()) {
      ERROR1("setFarEndSurface ignoring as producer is NULL");
      break;
    }
    ...
    if (NULL == nativeWindow.get() ) {
      ERROR1("setFarEndSurface: nativeWindow is NULL");
      break;
    }

    CRITICAL4_STR("setFarEndSurface jSurfacefar %p, producer %p, nativewindow %p",
                  jSurfacefar, producer.get(),nativeWindow.get());
    SetNativeWindow(nativeWindow.get());
    slStatus = 0;
  } while (0);
