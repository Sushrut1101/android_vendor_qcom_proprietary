...
Copyright (c) 2010,2011,2013,2018 Qualcomm Technologies, Inc.
All Rights Reserved.
...
#ifndef ANDROID
extern bool hidl_client_initialize(int mode, int *tool_fd);
#endif
...
boolean ftm_bt_hci_pfal_set_transport(void)
{
    if (soc_type == BT_SOC_ROME || soc_type == BT_SOC_CHEROKEE || soc_type == BT_SOC_NAPIER
                || soc_type == BT_SOC_HASTINGS || soc_type == BT_SOC_GENOA) {
