...
static inline void deinitialize_cld80211lib()
{
}
static inline void initialize_nl_fd ()
{
}
#endif
...
static int nl_loop_process_msg(struct nlmsghdr *nlh, int len)
{
	...
	while (NLMSG_OK(nlh, (unsigned int)len)) {
		...
		if (len < NLMSG_ALIGN(nlh->nlmsg_len))
			break;
...
int nl_loop_run(void)
{
	...
	pfd[2].fd = cnss_user_get_fd();
	pfd[2].events = POLLIN;

	pfd[3].fd = cnss_genl_get_fd();
	pfd[3].events = POLLIN;

	pfd[4].fd = cnss_nl80211_get_socket();
	pfd[4].events = POLLIN;
	...
	while (!nl_loop.terminate) {
		...
		if (pfd[2].revents & (POLLIN | POLLHUP | POLLERR)) {
			wsvc_printf_dbg("pfd[2].fd: %d has data to process\n",
					pfd[2].fd);
			handle_cnss_user_event(pfd[2].fd);
		}

		if (pfd[3].revents & (POLLIN | POLLHUP | POLLERR)) {
			wsvc_printf_dbg("pfd[3].fd: %d has data to process\n", pfd[3].fd);
			cnss_genl_recvmsgs();
		}

		if (pfd[4].revents & (POLLIN | POLLHUP | POLLERR)) {
			wsvc_printf_dbg("pfd[4].fd: %d has data to process\n",
					pfd[4].fd);
			cnss_nl80211_handle_event();
		}
	}
