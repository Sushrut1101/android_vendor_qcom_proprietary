...
struct msm_jpeg_hw_cmd hw_cmd_fe_cfg[] = {
  ...
  {MSM_JPEG_HW_CMD_TYPE_WRITE, 1, JPEG_FE_CFG_ADDR,
   JPEG_FE_CFG_BMSK, {0xa01a0f0}},//cbcr , PLANAR and pln0 enabled as default.
};
#else
struct msm_jpeg_hw_cmd hw_cmd_fe_cfg[] = {
  /*
...
struct msm_jpeg_hw_cmds *jpege_lib_hw_fe_cfg(jpege_cmd_input_cfg *p_input_cfg,
  uint8_t mcus_per_blk, uint32_t version __unused)
{
  uint32_t reg_val = 0;
  uint32_t memory_format =0;
  uint32_t pln0_enable = 0, pln1_enable = 0, pln2_enable = 0;
  ...
  reg_val |= ((uint32_t)mcus_per_blk << JPEG_FE_CFG__MCU_BOUNDARY_SHFT);
...
struct msm_jpeg_hw_cmds *jpege_lib_hw_fe_buffer_cfg
  ...
  JPEG_HW_INFO("%s:%d] w %d h %d stride %d scanline %d",
  ...
  if (p_scale_cfg->scale_enable) {
    image_width = p_scale_cfg->scale_input_width + p_scale_cfg->h_offset;
    image_height = p_scale_cfg->scale_input_height + p_scale_cfg->v_offset;
  } else {
  ...
  switch(p_input_cfg->inputFormat) {
    case JPEGE_INPUT_H2V1:
          chroma_width = image_width;
          chroma_height = image_height;
          break;
    case JPEGE_INPUT_H2V2:
          chroma_width = image_width;
          chroma_height = (uint32_t)floor(image_height / 2);
          break;
    case JPEGE_INPUT_H1V2:
          chroma_width = image_width * 2;
          chroma_height = (uint32_t)floor(image_height / 2);
          c_stride = c_stride * 2;
          break;
  ...
  if (p_input_cfg->num_of_input_plns == 3) {
      chroma_height = (uint32_t)floor(chroma_height);
      chroma_width = (uint32_t)floor(chroma_width/2);
      c_stride /= 2;
  }
  ...
  uint32_t pln1_hoffset = 0, pln1_voffset =0;
   ...
   p_hw_cmds =
    ...
    if (!p_hw_cmds) {
     ...
     switch(p_input_cfg->inputFormat) {
       case JPEGE_INPUT_H2V2:
         pln1_hoffset = (uint32_t)floor(scale_cfg->h_offset/2);
         pln1_voffset = (uint32_t)floor(scale_cfg->v_offset/2);
         break;
       case JPEGE_INPUT_H2V1:
         pln1_hoffset = (uint32_t)floor(scale_cfg->h_offset/2);
...
struct msm_jpeg_hw_cmds *jpege_lib_hw_scale_cfg(jpege_cmd_scale_cfg *scale_cfg,
  jpege_cmd_input_cfg * pIn __unused)
{
  uint32_t reg_val =0;
  struct msm_jpeg_hw_cmds *p_hw_cmds;
  struct msm_jpeg_hw_cmd *p_hw_cmd;
...
struct msm_jpeg_hw_cmds *jpege_lib_hw_scale_reg_cfg(jpege_cmd_scale_cfg
  ...
  if (!p_hw_cmds) {
   ...
   case JPEGE_INPUT_FORMAT_MAX:
    ...
    hor_scale_ratio = (uint32_t)FLOAT_TO_Q(21,
      (double)scale_cfg->scale_input_width /
      (double)scale_cfg->output_width) ;
    vert_scale_ratio = (uint32_t)FLOAT_TO_Q(21,
      (double)scale_cfg->scale_input_height /
                                                ...
                                                jpege_cmd_scale_cfg *scale_cfg)
{
  ...
  uint32_t hor_subsampling =0, vert_subsampling =0;
 ...
 /*Encoded image size is the image size in MCUs after scaling if scaling is
  ...
  encoded_height = (uint32_t)((padded_height / (8*vert_subsampling)) - 1);
  encoded_width = (uint32_t)((padded_width /(8 *hor_subsampling)) - 1);
...
uint32_t jpege_lib_hw_lookup_we_buf_height(uint32_t buf_size)
{
  ...
  for (i = 0; i < HW_BUF_SIZE_TBL_ENTRIES - 1; i++) {
     if (buf_size > lBufferRangeTbl[i] &&
       ...
       l_hw_buf_height = (uint32_t)(pow(2, i+2));
       break;
     }
  }
...
struct msm_jpeg_hw_cmds *jpege_lib_hw_we_bffr_cfg (jpege_cmd_input_cfg * pIn,
  jpege_cmd_scale_cfg *scale_cfg __unused,
  uint32_t version)
{
  uint32_t l_hw_buf_height = 0;
  uint32_t nRegVal = 0;
...
struct msm_jpeg_hw_cmds *jpege_lib_hw_we_cfg (uint32_t version __unused)
{
  uint32_t nRegVal = 0;
