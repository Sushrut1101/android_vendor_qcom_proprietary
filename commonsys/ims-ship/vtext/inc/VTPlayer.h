...
struct FrameStateMap {
  ...
  uint64_t rtp_tstamp;
  /* AV Timer at which this frame is given to Decoder */
  uint64_t av_tstamp_in_us;
  /* Corresponding System clock at which AVTimer is generated */
  uint64_t app_tstamp;
};
...
class VTPlayer : public VideoCodecBase
{
  ...
  public:
    ...
    virtual void SetNativeWindow(ANativeWindow* surfaceFar) = 0;
    virtual QPE_VIDEO_ERROR UpdateNalHeader(uint8_t* pVolHeader,
                                            uint16_t pVolHeaderLen) = 0;
    virtual QPE_VIDEO_ERROR StartPlayer();
    virtual void UpdateCodecConfigInfo(QP_VIDEO_CONFIG* pCodecConfig);
    virtual int16_t setFarEndSurface(JNIEnv * env, jobject jSurfacefar);
};
