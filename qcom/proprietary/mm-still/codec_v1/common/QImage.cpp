/**********************************************************************
*  Copyright (c) 2012,2016-2017 Qualcomm Technologies, Inc.
*  All Rights Reserved.
  ...
  QIFormat aFormat)
{
  ...
  case QI_YUV2:
    return aSize.Length() +
      (uint32_t)((float)aSize.Length() * lChromaFactor * 2);
  default:
    return 0;
  }
}
 ...
 * Notes: none
 *==========================================================================*/
int QImage::setDefaultPlanes(uint32_t aPlaneCnt __unused, uint8_t *aAddr,
  int aFd,
  uint32_t *aOffset, uint32_t *aPhyOffset)
{
  ...
  case QI_YCBCR_SP: {
    ...
    lPlaneAddr[1] = aAddr;
    lSize[1] = QISize((uint32_t)((float)mSize.Width() *
      getChromaWidthFactor(mSubSampling) * 2),
      (uint32_t)((float)mSize.Height() *
      getChromaHeightFactor(mSubSampling)));
    lActualSize[1] = QISize((uint32_t)((float)mActualSize.Width() *
      getChromaWidthFactor(mSubSampling) * 2),
      (uint32_t)((float)mActualSize.Height() *
      getChromaHeightFactor(mSubSampling)));
  ...
  case QI_IYUV: {
    ...
    lPlaneAddr[1] = aAddr;
    lSize[1] = QISize((uint32_t)((float)mSize.Width() *
      getChromaWidthFactor(mSubSampling)),
      (uint32_t)((float)mSize.Height() *
      getChromaHeightFactor(mSubSampling)));
    lActualSize[1] = QISize((uint32_t)((float)mActualSize.Width() *
      getChromaWidthFactor(mSubSampling)),
 ...
 * Function: Dump
 *
 * Description: Dumps the image
 *
 * Input parameters:
 *   aFileName - output filename
 *   aFrameId - frame_id
 *
 * Return values:
 *   none
 *
 * Notes: need to add support for 3 planes
 *==========================================================================*/
void QImage::Dump(const char* aFileName, int32_t aFrameId)
{
  uint32_t i;
  size_t lLength;
  size_t lWrittenSize;
  ...
  case 2:
    ...
    break;
  default:;
    QIDBG_MED("%s:%d] Not supported", __func__, __LINE__);
    return;
  }

  /* get the address and size */
  for (i = 0; i < PlaneCount(); i++) {
    lPlane = getPlane(lType[i]);
    if (NULL == lPlane) {
      QIDBG_ERROR("%s:%d] Invalid input", __func__, __LINE__);
      return;
    }
    lAddr[i] = lPlane->ActualAddr();
    lSize[i] = lPlane->ActualSize();
  }

  snprintf(lName, sizeof(lName), "%s%s_%dx%d_%d.yuv", "/data/", aFileName,
    lSize[0].Width(), lSize[0].Height(), aFrameId);

  lOutFileFd = open(lName, O_RDWR | O_CREAT, 0777);

  if (lOutFileFd >= 0) {
    for (i = 0; i < PlaneCount(); i++) {
      lLength = (size_t)lSize[i].Length();
      lWrittenSize = (size_t)write(lOutFileFd,
        lAddr[i],
        lLength);
      if (lLength != lWrittenSize)
        QIDBG_ERROR("%s:%d failed: Cannot write data to file %s\n", __func__,
          __LINE__, lName);
    }

    close(lOutFileFd);
  } else {
    QIDBG_ERROR("%s:%d failed: Cannot open file\n", __func__, __LINE__);
  }
}
