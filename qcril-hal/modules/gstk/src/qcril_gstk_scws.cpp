...
#define QCRIL_SCWS_ENTER_CRITICAL_SECTION                      \
           qcril_scws_mutex.lock();

#define QCRIL_SCWS_LEAVE_CRITICAL_SECTION                      \
           qcril_scws_mutex.unlock();


/*=========================================================================
  Global mutex
===========================================================================*/
static qtimutex::QtiSharedMutex              qcril_scws_mutex;
...
)
{
  ...
  if (server_ptr == NULL)
  {
    QCRIL_LOG_ERROR( "Invalid input: NULL server_ptr\n");
    QCRIL_ASSERT(0);
    return;
  }
...
)
{
  ...
  if (server_ptr->connected_socket[incoming_index].buffer_size <= SSIZE_MAX - received_len)
  {
    total_size = received_len + server_ptr->connected_socket[incoming_index].buffer_size;
    temp_buffer = (uint8 *)qcril_malloc(total_size);
  }
...
)
{
  ...
  QCRIL_LOG_DEBUG("qcril_scws initalized");
...
)
{
  ...
  QCRIL_LOG_DEBUG("qcril_scws de-initalized");
...
*/
/*=========================================================================*/
static int qcril_scws_add_bip_id_to_server_socket
(
  qcril_scws_server_socket_type * server_ptr,
  uint32                          bip_id
)
...
*/
/*=========================================================================*/
static int qcril_scws_create_new_server_socket
(
  qcril_scws_server_socket_type * server_ptr,
  uint16                          local_port,
...
)
{
  ...
  {
    error = errno;
    QCRIL_LOG_ERROR("Failure to create the socket : %d", error);
    memset(server_ptr, 0, sizeof(qcril_scws_server_socket_type));
    return error;
  }

  /* Set socket options */
  setsockopt(server_ptr->server_sd, SOL_SOCKET, SO_REUSEADDR, (void *)&reuse_addr, sizeof(reuse_addr));

  /* Set Linger time to 0 to close the socket immediately */
  so_linger.l_onoff  = TRUE;
  so_linger.l_linger = 0;
  setsockopt(server_ptr->server_sd, SOL_SOCKET, SO_LINGER, (void *)&so_linger, sizeof(so_linger));
  ...
  if (bind(server_ptr->server_sd, (struct sockaddr *)&server_address, sizeof(server_address)) < 0 )
  {
    error = errno;
    memset(server_ptr, 0, sizeof(qcril_scws_server_socket_type));
    QCRIL_LOG_ERROR("Failure to bind the socket");
    close(server_ptr->server_sd);
    return error;
  }
  ...
  {
    error = errno;
    QCRIL_LOG_ERROR("Failure to listen on the socket : %d", error);
    close(server_ptr->server_sd);
    memset(server_ptr, 0, sizeof(qcril_scws_server_socket_type));
    return error;
  }
  ...
  {
    error = errno;
    QCRIL_LOG_ERROR("Failure to start the thread for the new socket :%d", error);
    close(server_ptr->server_sd);
    memset(server_ptr, 0, sizeof(qcril_scws_server_socket_type));
    return error;
  }
...
)
{
  ...
  for (i = 0; i < QCRIL_SCWS_MAX_SERVER_SOCKETS; i++)
  {
    if (server_socket[i].local_port == 0)
    {
      int res = qcril_scws_create_new_server_socket(&server_socket[i], local_port, slot_id, bip_id);
  ...
  QCRIL_LOG_ERROR("Error opening the new server socket");
...
)
{
  ...
  QCRIL_LOG_ERROR("No matching BIP id found to close the socket");
...
)
{
  ...
  QCRIL_LOG_ERROR("No matching BIP id found to send the data");
