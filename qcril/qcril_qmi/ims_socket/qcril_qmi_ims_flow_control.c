...
void qcril_ims_flow_control_pre_init()
{
  ...
  int dtmf_pause_interval = QCRIL_QMI_VOICE_DTMF_INTERVAL_VAL;
  ...
  //Read DTMF pause interval from adb property
  QCRIL_SNPRINTF( property_name, sizeof( property_name ), "%s",
      QCRIL_QMI_VOICE_DTMF_PAUSE_INTERVAL);
  property_get( property_name, args, "" );
  property_param_len = strlen( args );
  if ( property_param_len > 0 )
  {
    ...
    if ( ( errno == ERANGE ) && ( ret_val == ULONG_MAX ) )
    {
      QCRIL_LOG_ERROR( "QCRIL QMI VOICE Fail to convert QCRIL_QMI_VOICE_DTMF_PAUSE_INTERVAL %s",
          args );
    }
    else
    {
      dtmf_pause_interval = ret_val;
    }
  }
  QCRIL_LOG_DEBUG("QCRIL_QMI_VOICE_DTMF_PAUSE_INTERVAL=%d", dtmf_pause_interval);
  ...
  if(qcril_ims_flow_control_family_ring != NULL)
  {
    ...
    qcril_ims_flow_control_family_ring->dtmf_pause_timer = dtmf_pause_interval;
    qcril_ims_flow_control_family_ring->ims_flow_control_family_ring =
               (int *)ims_flow_control_family_ring_dtmf_req;
...
boolean qcril_qmi_is_flow_control_required(qcril_evt_e_type event_id)
{
  ...
  while (req_grp_list)
  {
    for(itr=0; itr<req_grp_list->max_req; itr++)
    {
      if((int)event_id == req_grp_list->ims_flow_control_family_ring[itr])
      {
        ret = TRUE;
        break;
      }
        ...
        *req_action = req_grp_list->action_on_dup_req;
        switch(event_id)
        {
          ...
          case QCRIL_EVT_IMS_SOCKET_REQ_DTMF_STOP:
            *timer = req_grp_list->dtmf_pause_timer;
            break;
  ...
  while(req_overview)
  {
    ...
    while(req_list)
    {
       if(token == req_list->req_node->token &&
          (int)req_id == req_list->req_node->req_id)
       {
           req_list_found = TRUE;
           break;
       }
