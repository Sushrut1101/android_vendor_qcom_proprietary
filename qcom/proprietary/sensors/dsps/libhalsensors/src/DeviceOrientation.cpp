     ...
     is_report_time_created(false)
{
    if(getAttribOK() == true) {
        ...
        is_lastsample_reset = false;
        if (bWakeUp == false) {
            sigev.sigev_notify_function = timeOutReporterForLastSample;
            /* Create timer for Device Orientation. */
            if (initReportTimer()) {
                HAL_LOG_ERROR("%s:Device Orientation initReportTimer failed", __FUNCTION__);
            } else {
                is_report_time_created = true;
            }
        }
...
{
    if (is_report_time_created) {
        if (delReportTimer()) {
            HAL_LOG_ERROR(" %s:Device Orientation report timer delete failed", __FUNCTION__);
        }
    }
}
...
===========================================================================*/
void DeviceOrientation::reportSingleEvent(sensors_event_t sensor_data)
{
    if (is_lastsample_reset == false) {
        is_lastsample_reset = true;
    }
    pthread_mutex_lock(&data_cb->data_mutex);
    if (Utility::insertQueue(&sensor_data, bWakeUp, data_cb)) {
        Utility::signalInd(data_cb);
        /* backup the last reported sensor event */
...
===========================================================================*/
void DeviceOrientation::timeOutReporterForLastSample(sigval_t val)
{
    DeviceOrientation *dev_ori_handle = (DeviceOrientation *)val.sival_ptr;
    sensors_event_t curr_sample = dev_ori_handle->latest_event;
    dev_ori_handle->reportSingleEvent(curr_sample);
}
/*============================================================================
  enable
============================================================================*/
int DeviceOrientation::enable(int en)
{
    ...
    if (en) {
        dev_ori_last_event.data[0] = SNS_SAM_DEV_ORI_UNKNOWN_V01;
        if (0 == freq) {
...
void DeviceOrientation::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    /* No error */
    if (error == false) {
        HAL_LOG_DEBUG( "%s: DeviceOrientation: Orientation: %f",__FUNCTION__, sensor_data.data[0]);
        ...
        memcpy(&latest_event, &sensor_data, sizeof(sensors_event_t));
        ...
        /* Drop event if orientation is not change */
        if (sensor_data.data[0] == dev_ori_last_event.data[0]) {
            HAL_LOG_VERBOSE( "%s: DeviceOrientation: orientation not change, Orientation: %f wakeup: %d  HAL TS:%" PRId64"",
                __FUNCTION__, sensor_data.data[0], bWakeUp, sensor_data.timestamp);
            /* Cancel the delayed report */
            if (is_report_time_created) {
                if (unsetReportTimer()) {
                    HAL_LOG_ERROR( "%s: unsetReportTimer failed", __FUNCTION__);
                }
            }
            return;
        }
        /* Check sample is not reported before timeout elapsed for non-wakeup sensor
         * Drop it for now and report valid sample after timeout elapsed. */
        if (bWakeUp == false && is_lastsample_reset == true) {
            if (!bComplySampleRate(dev_ori_last_event.timestamp, sensor_data.timestamp)) {
                float time = HZ_TO_NSEC(freq) - sensor_data.timestamp + dev_ori_last_event.timestamp;
                if (is_report_time_created) {
                    if (unsetReportTimer()) {
                        HAL_LOG_ERROR( "%s: unsetReportTimer failed", __FUNCTION__);
                    }
                    if (setReportTimer(time)){
                        HAL_LOG_ERROR( "%s: setReportTimer failed", __FUNCTION__);
                    }
                    return;
                }
            }
        }
        reportSingleEvent(sensor_data);
    }
}
