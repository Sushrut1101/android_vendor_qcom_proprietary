...
boolean qcril_qmi_npb_get_data_size(const pb_field_t fields[], size_t* dataSize)
{
    ...
    if ( ! qcril_qmi_npb_field_init(&iter, fields, NULL) || ! dataSize) {
        return FALSE;
    }
    ...
    *dataSize = 0;
    ...
    {
        if ( ! iter.pos ) {
            QCRIL_LOG_ERROR("Invalid iterator");
            return FALSE;
        }
        ...
        *dataSize += new_field_size - iter.pos->size_offset;
...
void qcril_qmi_npb_release(const pb_field_t fields[], void *dest_struct)
{
    ...
    do
    {
        ...
        {
            qcril_qmi_npb_release_binary_data( static_cast<qcril_binary_data_type*>(
                    (static_cast<pb_callback_t*>(iter.cur_data_ptr))->arg) );
        }
        else if (PB_LTYPE(type) == PB_LTYPE_SUBMESSAGE)
        {
