...
static uint32_t u32ConvertParams(char *cpInput, uint32_t *u32pOutput)
{
    *u32pOutput = 0;
    ...
    else if(strcasecmp(cpInput, "low") == 0)
    {
        *u32pOutput = HQV_FRC_MODE_LOW;
    }
    else if(strcasecmp(cpInput, "med") == 0)
    {
        *u32pOutput = HQV_FRC_MODE_MED;
    }
    else if(strcasecmp(cpInput, "high") == 0)
    {
        *u32pOutput = HQV_FRC_MODE_HIGH;
    }
...
void vFuncTestInit(struct test_ctx *pstTestCtx, struct functional_test_ctx *pFuncTestCtx)
{
    ...
    for(i = 0; i < MAX_RECONFIG_STREAMS; i++)
    {
        u32Ret = buf_params_init_default(&pFuncTestCtx->reconfig_streams[i].reconfig_pool_params, &pstTestCtx->port_param);
        if(u32Ret)
        {
            LOGE("Error initializing buffer parameters!");
            return;
...
int main(int argc, char **argv)
{
    ...
    if(argc > 1)
        u32GetArgs(argc, argv, pstTestCtx);
