 ...
 * Notes: none
 *==========================================================================*/
int QImageQ6Encoder::Start(QIThread *a_pUseEncThread)
{
...
void QImageQ6Encoder::Encode()
{
  ...
  register_buf(p_output_buffer, mOutputImage->Length(),
      mOutputImage->Fd());

  QIDBG_HIGH("%s:%d] adsp_jpege_q6_process START ++++", __func__, __LINE__);

  remote_set_mode(REMOTE_MODE_SERIAL);
  ...
  register_buf(p_output_buffer, mOutputImage->Length(), -1);

#ifdef DUMP_JPEG_BITSTREAM
  {
    FILE *fout;
    fout = fopen(QCAMERA_DUMP_FRM_LOCATION"jpeg_q6.bs", "wb");
    if (fout) {
