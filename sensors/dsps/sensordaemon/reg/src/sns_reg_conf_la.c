  ...
  if( (dirent->d_type == DT_REG) &&
    ...
    if( strncmp(dirent->d_name+strlen(dirent->d_name)-5, sns_reg_default_postfix, SNS_REG_DEFAULT_POSTFIX_LEN) == 0) {
      ...
      if( uid_found ) {
        ...
        int i;
        struct dirent *dir;
        char *dir_path[] = {SNS_REG_DEFAULT_CONF_PATH,
                            SNS_REG_DEFAULT_CONF_PATH_LEGACY};
        for (i = 0; i < 2; i++) {
          d = opendir(dir_path[i]);
          if (d) {
            while ((dir = readdir(d)) != NULL) {
              if ((dir->d_type == DT_REG) && (strncasecmp(dir->d_name, sns_reg_default_prefix, SNS_REG_DEFAULT_PREFIX_LEN ) == 0) &&
                  (strncmp(dir->d_name + strlen(dir->d_name) - 5, sns_reg_default_postfix,
                   SNS_REG_DEFAULT_POSTFIX_LEN) == 0)) {
                strlcpy(path, dir_path[i], strlen(dir_path[i]) + 1);
                ++i;
                break;
              }
            }
...
#define xstr(x)         str(x)
  ...
  if( NULL == buf ) {
    ...
    while( fgets( buf, sz, file ) != NULL ) {
      ...
      char key_meta[META_KEY_LEN+1];
      char val_meta[META_VAL_LEN+1];
      char val_meta2[META_VAL_LEN+1];
...
bool sns_reg_update_external_defaults( sns_reg_settings_group_s *settings )
{
   ...
   * to avoid warnings */
  num_files = scandir( path, &file_list,
                       sns_reg_lib_filter,
