...
#define ERR_GET_FREQ_VAL    NULL
#define ERR_AUTO_VAL_LB     0.0f
#define ERR_TEMP_SCALE_FACTOR_LB 0
#define ERR_FREQ_SCALE_FACTOR_LB 0
#define ERR_PERSIST_VAL     NULL
#define ERR_GAIN_LB         0.0f
...
static int parse_logic(void *value);
static int parse_bw_sensors(void *value);
static int parse_device(void *value);
static int parse_thresh(void *value);
static int parse_thresh_clr(void *value);
static int parse_tm_actions(void *value);
static int parse_bwlm_actions(void *value);
static int parse_tm_action_info(void *value);
static int parse_bwlm_action_info(void *value);
static int parse_weights(void *value);
static int parse_ints(void *value);
static int parse_ib_thresh(void *value);
static int parse_ib_thresh_clr(void *value);
static int parse_ab_thresh(void *value);
static int parse_ab_thresh_clr(void *value);
static int parse_utilize_thresh(void *value);
static int parse_utilize_thresh_clr(void *value);
...
#define PARSE_THRESHOLD(ptr, field, err_val, ret)                   \
	do {                                                        \
		uint32_t i;                                         \
		for (i = 0; lookahead == INTEGER && i < THRESHOLDS_MAX; i++) {\
			ptr[i].field = strtoul(tokenText, NULL, 0); \
		...
		if (i == 0) {                                       \
			...
			while (i < THRESHOLDS_MAX) {                \
				ptr[i].field = (uint64_t)err_val;   \
				i++;                                \
			}                                           \
		}                                                   \
	} while (0)                                                 \

#define PARSE_ACTION(t_ptr, ret) \
do {                       \
	uint32_t i, j; \
...
#define PARSE_MULTI_THRESHOLD(ptr, field, err_val)                  \
	do {                                                        \
		uint32_t i, j;                                      \
		for (i = 0; lookahead == INTEGER && i < THRESHOLDS_MAX; i++) { \
			t_ptr[i].field[0] = strtoul(tokenText, NULL, 0); \
			lookahead = next_token();                   \
			for (j = 1; lookahead == PLUS && j < MAX_NUM_SENSOR; j++) { \
				lookahead = next_token();           \
				if (lookahead == INTEGER) {         \
					t_ptr[i].field[j] = strtoul(tokenText, NULL, 0); \
					lookahead = next_token();   \
				}                                   \
			}                                           \
			while (j < MAX_NUM_SENSOR) {                   \
				t_ptr[i].field[j] = (uint64_t)ERR_INFO_VAL; \
				j++;                                \
			}                                           \
		}                                                   \
		if (i == 0)                                         \
			return 1;                                   \
		while (i < THRESHOLDS_MAX) {                        \
			t_ptr[i].field[0] = (uint64_t)ERR_INFO_VAL;         \
			i++;                                        \
		}                                                   \
	} while (0)                                                 \

#define PARSE_ACTION_INFO(t_ptr, ret) \
do { \
	uint32_t i, j; \
	for (i = 0; lookahead == INTEGER && i < THRESHOLDS_MAX; i++) { \
...
static struct table tm_table[] = {
	...
	{.field_name = "actions", .offset = offsetof(struct setting_info,
		data.tm.t),
		.callback_func = &parse_tm_actions},
	{.field_name = "action_info", .offset = offsetof(struct setting_info,
		data.tm.t),
		.callback_func = &parse_tm_action_info},
...
static struct table ss_table[] = {
	...
	{.field_name = "device_max_limit", .offset = offsetof(struct setting_info,
		...
		.callback_func = &parse_int_opt},
	{.field_name = "device_perf_floor", .offset = offsetof(struct setting_info,
		data.ss.device_perf_floor),
		.callback_func = &parse_int_opt}
};

static struct table tb_table[] = {
	{.field_name = "time_constant", .offset = offsetof(struct setting_info,
		data.tb.time_constant),
		.callback_func = &parse_int},
	{.field_name = "sampling", .offset = offsetof(struct setting_info,
		data.tb.sampling_period_ms),
		.callback_func = &parse_int},
	{.field_name = "disable", .offset = offsetof(struct setting_info,
		disable),
		.callback_func = &parse_int_opt},
	...
	{.field_name = "set_point", .offset = offsetof(struct setting_info,
		...
		.callback_func = &parse_int},
	{.field_name = "set_point_clr", .offset = offsetof(struct setting_info,
		data.tb.set_point_clr),
		.callback_func = &parse_int},
	{.field_name = "override", .offset = offsetof(struct setting_info,
		data.tb.override),
		.callback_func = &parse_int_opt},
	{.field_name = "up_loop_gain", .offset = offsetof(struct setting_info,
		data.tb.up_loop_gain),
		.callback_func = &parse_float},
	{.field_name = "down_loop_gain", .offset = offsetof(struct setting_info,
		data.tb.down_loop_gain),
		.callback_func = &parse_float},
	{.field_name = "auto_penalty", .offset = offsetof(struct setting_info,
		data.tb.auto_penalty),
		.callback_func = &parse_float},
	{.field_name = "auto_reward", .offset = offsetof(struct setting_info,
		data.tb.auto_reward),
		.callback_func = &parse_float},
	{.field_name = "temp_scale_factor", .offset = offsetof(struct setting_info,
		data.tb.temp_scale_factor),
		.callback_func = &parse_int},
	{.field_name = "freq_scale_factor", .offset = offsetof(struct setting_info,
		data.tb.freq_scale_factor),
		.callback_func = &parse_int},
	{.field_name = "quadratic_reward", .offset = offsetof(struct setting_info,
		data.tb.quadratic_reward),
		.callback_func = &parse_int},
	{.field_name = "quadratic_penalty", .offset = offsetof(struct setting_info,
		data.tb.quadratic_penalty),
		.callback_func = &parse_int}
};
...
static struct table v_table[] = {
	...
	{.field_name = "thresholds_clr", .offset = offsetof(struct setting_info,
		...
		.callback_func = &parse_thresh_clr},
	{.field_name = "descending", .offset = offsetof(struct setting_info,
		data.v.descending_thresh),
		.callback_func = &parse_int_opt},
	{.field_name = "tracks_low", .offset = offsetof(struct setting_info,
		data.v.tracks_low),
		.callback_func = &parse_int_opt},
	{.field_name = "trip_sensor_logic", .offset = offsetof(struct setting_info,
		data.v.trip_sensor_logic),
		.callback_func = &parse_logic},
	{.field_name = "math", .offset = offsetof(struct setting_info,
		data.v.math_type),
		.callback_func = &parse_int},
	{.field_name = "trip_equal_logic", .offset = offsetof(struct setting_info,
		data.v.trip_equal_logic),
		.callback_func = &parse_int_opt},
	{.field_name = "trip_always_monitor", .offset = offsetof(struct setting_info,
		data.v.trip_always_mon),
		.callback_func = &parse_int_opt}
};
...
static struct table bwlm_table[] = {
	{.field_name = "disable", .offset = offsetof(struct setting_info,
		disable),
		.callback_func = &parse_int_opt},
	{.field_name = "sensor_list", .offset = offsetof(struct setting_info,
		data.bwlm.sensor_list[0]),
		.callback_func = &parse_bw_sensors},
	{.field_name = "bus", .offset = offsetof(struct setting_info,
		data.bwlm.bus),
		.callback_func = &parse_int_opt},
	{.field_name = "ib_thresholds", .offset = offsetof(struct setting_info,
		data.bwlm.t),
		.callback_func = &parse_ib_thresh},
	{.field_name = "ib_thresholds_clr", .offset = offsetof(struct setting_info,
		data.bwlm.t),
		.callback_func = &parse_ib_thresh_clr},
	{.field_name = "ab_thresholds", .offset = offsetof(struct setting_info,
		data.bwlm.t),
		.callback_func = &parse_ab_thresh},
	{.field_name = "ab_thresholds_clr", .offset = offsetof(struct setting_info,
		data.bwlm.t),
		.callback_func = &parse_ab_thresh_clr},
	{.field_name = "utilize_thresholds", .offset = offsetof(struct setting_info,
		data.bwlm.t),
		.callback_func = &parse_utilize_thresh},
	{.field_name = "utilize_thresholds_clr", .offset = offsetof(struct setting_info,
		data.bwlm.t),
		.callback_func = &parse_utilize_thresh_clr},
	{.field_name = "actions", .offset = offsetof(struct setting_info,
		data.bwlm.t),
		.callback_func = &parse_bwlm_actions},
	{.field_name = "action_info", .offset = offsetof(struct setting_info,
		data.bwlm.t),
		.callback_func = &parse_bwlm_action_info},
};
...
static struct algorithms algos[] = {
	...
	{.name = "tb", .type = TB_ALGO_TYPE},
	{.name = "virtual", .type = VIRTUAL_SENSOR_TYPE},
	{.name = "bwlm", .type = BWLM_ALGO_TYPE}
};
...
static int verify_logic(char *value)
{
	uint32_t i;

	for (i = 0; i < sensor_info_arr_len; i++) {
		if (strncasecmp(value, "&&", 2) == 0)
			return 0;
		if (strncasecmp(value, "||", 2) == 0)
			return 0;
	}
...
static int parse_logic(void *value)
{
	char **logic_list = *(char ***)value;
	uint32_t i;

	for (i = 0; lookahead == STRING_DATA && i < sensor_info_arr_len
		    && verify_logic(tokenText) == 0; i++) {
		strlcpy(logic_list[i], tokenText, MAX_SENSOR_NAME_LEN);
		lookahead = next_token();
	}
	while (i < sensor_info_arr_len) {
		logic_list[i][0] = ERR_SENSOR_VAL;
		i++;
	}
...
static int parse_bw_sensors(void *value)
{
	...
	for (i = 0; lookahead == STRING_DATA && i < MAX_NUM_SENSOR; i++) {
		strlcpy(&sensor_list[i * MAX_SENSOR_NAME_LEN], tokenText, MAX_SENSOR_NAME_LEN);
		lookahead = next_token();
	}
	if (i == 0)
		return 1;
	while (i < MAX_NUM_SENSOR) {
		sensor_list[i * MAX_SENSOR_NAME_LEN] = ERR_SENSOR_VAL;
		i++;
	}
...
{
	int ret = 0;
	struct bwlm_threshold_t *t_ptr = (struct bwlm_threshold_t *)value;

	PARSE_THRESHOLD(t_ptr, ib_trig, ERR_TRIG_VAL, ret);
...
static int parse_ib_thresh_clr(void *value)
{
	...
	PARSE_THRESHOLD(t_ptr, ib_clr, ERR_CLR_VAL, ret);
...
static int parse_utilize_thresh(void *value)
{
	...
	PARSE_THRESHOLD(t_ptr, utilize_trig, ERR_TRIG_VAL, ret);
...
static int parse_utilize_thresh_clr(void *value)
{
	...
	PARSE_THRESHOLD(t_ptr, utilize_clr, ERR_CLR_VAL, ret);
...
static int parse_ab_thresh(void *value)
{
	...
	PARSE_MULTI_THRESHOLD(t_ptr, ab_trig, ERR_TRIG_VAL);
	return 0;
}
...
static int parse_ab_thresh_clr(void *value)
{
	...
	PARSE_MULTI_THRESHOLD(t_ptr, ab_clr, ERR_CLR_VAL);
...
static int *init_weight_list(uint8_t dynamic, int *prev,
...
static void init_tb_settings(struct setting_info *section)
{
	if (section->dynamic) {
		section->data.tb.sampling_period_ms = ERR_SAMPLING_VAL;
		section->data.tb.set_point = ERR_SETPOINT_VAL;
		section->data.tb.set_point_clr = ERR_SETPOINTCLR_VAL;
	}
}
...
static int init_v_settings(struct setting_info *section)
{
	...
	section->data.v.num_thresholds = 0;
	if (section->dynamic) {
		for (; i < THRESHOLDS_MAX; i++) {
			section->data.v.t[i].lvl_trig = ERR_TRIG_VAL;
			section->data.v.t[i].lvl_clr = ERR_CLR_VAL;
		}
	...
	temp = init_sensor_list(section->data.v.trip_sensor_list,
		 section->data.v.trip_cnt, section->data.v.list_size);
	if (temp == NULL) {
		msg("Failed to alloc v trip_sensor_list\n");
		return 1;
	}
	section->data.v.trip_sensor_list = temp;

	temp = init_sensor_list(section->data.v.trip_sensor_logic,
		 section->data.v.trip_cnt, section->data.v.list_size);
	if (temp == NULL) {
		msg("Failed to alloc v trip_sensor_logic\n");
		return 1;
	}
	section->data.v.trip_sensor_logic = temp;
	section->data.v.list_size = (uint8_t)sensor_info_arr_len;

	itemp = init_weight_list(section->dynamic, section->data.v.weight_list,
...
static void init_bwlm_settings(struct setting_info *section)
{
	uint32_t i, j;

	for (i = section->data.bwlm.num_thresholds; i < THRESHOLDS_MAX; i++) {
		section->data.bwlm.t[i].ib_trig = (uint64_t)ERR_TRIG_VAL;
		section->data.bwlm.t[i].ib_clr = (uint64_t)ERR_CLR_VAL;
		section->data.bwlm.t[i].utilize_trig = (uint64_t)ERR_TRIG_VAL;
		section->data.bwlm.t[i].utilize_clr = (uint64_t)ERR_CLR_VAL;
		for (j = section->data.bwlm.num_sensors; j < MAX_NUM_SENSOR;
			j++) {
			section->data.bwlm.t[i].ab_trig[j] = (uint64_t)ERR_TRIG_VAL;
			section->data.bwlm.t[i].ab_clr[j] = (uint64_t)ERR_CLR_VAL;
		}
		for (j = section->data.bwlm.t[i].num_actions; j < ACTIONS_MAX; j++) {
			section->data.bwlm.t[i].actions[j].device[0] = ERR_DEVICE_VAL;
			section->data.bwlm.t[i].actions[j].info = ERR_INFO_VAL;
		}
	}
}
...
static void finalize_v_settings(struct setting_info *section)
{
	...
	section->data.v.trip_cnt = 0;
	section->data.v.weight_cnt = 0;
	section->data.v.logic_cnt = 0;
	for (i = 0 ; i < sensor_info_arr_len; i++) {
	...
	for (i = 0; i < sensor_info_arr_len; i++) {
		if (section->data.v.weight_list[i] == ERR_WEIGHT_VAL)
			break;
		section->data.v.weight_cnt++;
	}

	for (i = 0 ; i < sensor_info_arr_len; i++) {
		if (section->data.v.trip_sensor_list[i][0] == ERR_SENSOR_VAL)
			break;
		section->data.v.trip_cnt++;
	}

	for (i = 0; i < THRESHOLDS_MAX ; i++) {
		if (section->data.v.t[i].lvl_trig == ERR_TRIG_VAL
		   || section->data.v.t[i].lvl_clr == ERR_CLR_VAL)
			break;
		section->data.v.num_thresholds++;
	}

	for (i = 0 ; i < sensor_info_arr_len; i++) {
		if (section->data.v.trip_sensor_logic[i][0] == ERR_SENSOR_VAL)
			break;
		section->data.v.logic_cnt++;
	}
}
...
static void finalize_bwlm_settings(struct setting_info *section)
{
	uint32_t i = 0, j = 0;

	section->data.bwlm.num_thresholds = 0;
	for (i = 0; i < THRESHOLDS_MAX; i++) {
		if (section->data.bwlm.t[i].ib_trig == (uint64_t)ERR_TRIG_VAL
		   || section->data.bwlm.t[i].ib_clr == (uint64_t)ERR_CLR_VAL
		   || section->data.bwlm.t[i].utilize_trig == (uint64_t)ERR_TRIG_VAL
		   || section->data.bwlm.t[i].utilize_clr == (uint64_t)ERR_CLR_VAL)
			break;

		section->data.bwlm.num_sensors = 0;
		for (j = 0; j < MAX_NUM_SENSOR; j++) {
			if (!strlen(section->data.bwlm.sensor_list[j]))
				break;
			section->data.bwlm.num_sensors++;
		}
		section->data.bwlm.t[i].num_actions = 0;
		for (j = 0; j < ACTIONS_MAX; j++) {
			if (section->data.bwlm.t[i].actions[j].device[0]
			    == ERR_DEVICE_VAL
			   || section->data.bwlm.t[i].actions[j].info
			    == ERR_INFO_VAL)
				break;
			section->data.bwlm.t[i].num_actions++;
		}
		section->data.bwlm.num_thresholds++;
	}
}
...
static void parse_tb_settings(struct setting_info *section)
{
	info("Algo Type 'tb'\n");
	init_tb_settings(section);
	if (parse_settings((void *)section, section->desc, tb_table,
			   ARRAY_SIZE(tb_table)))
		section->err_disable = 1;
}
...
static void parse_v_settings(struct setting_info *section)
{
	...
	if (section->data.v.logic_cnt < (section->data.v.trip_cnt - 1)) {
		msg("Not enough trip_sensor_logic field content\n");
		section->err_disable = 1;
	}
...
static void parse_bwlm_settings(struct setting_info *section)
{
	info("Algo Type 'bwlm'\n");
	init_bwlm_settings(section);
	if (parse_settings((void *)section, section->desc, bwlm_table,
		ARRAY_SIZE(bwlm_table)))
		section->err_disable = 1;
	finalize_bwlm_settings(section);
}
...
static void parse_section(struct thermal_setting_t *settings, int flag)
{
	...
	case VIRTUAL_SENSOR_TYPE:
		...
		break;
	case BWLM_ALGO_TYPE:
		parse_bwlm_settings(in_section);
		break;
	default:
...
static int bwlm_threshold_array_compare(const void *x, const void *y)
{
	struct bwlm_threshold_t *i = (struct bwlm_threshold_t *)x;
	struct bwlm_threshold_t *j = (struct bwlm_threshold_t *)y;

	if (i->ib_trig < j->ib_trig)
		return -1;
	else if (i->ib_trig > j->ib_trig)
		return 1;
	return 0;
}
...
{
	if (s->data.v.trip_cnt == 0
	   || (s->data.v.trip_cnt - 1) > s->data.v.logic_cnt
...
static int validate_tb(struct setting_info *s)
{
	if (s->data.tb.sensor[0] == ERR_SENSOR_VAL
		|| s->data.tb.device[0] == ERR_DEVICE_VAL
		|| s->data.tb.sampling_period_ms == ERR_SAMPLING_VAL
		|| s->data.tb.set_point == ERR_SETPOINT_VAL
		|| s->data.tb.set_point_clr == ERR_SETPOINTCLR_VAL
		|| s->data.tb.auto_penalty < ERR_AUTO_VAL_LB
		|| s->data.tb.auto_reward < ERR_AUTO_VAL_LB
		|| s->data.tb.temp_scale_factor <= ERR_TEMP_SCALE_FACTOR_LB
		|| s->data.tb.freq_scale_factor <= ERR_FREQ_SCALE_FACTOR_LB
		|| s->data.tb.up_loop_gain <= ERR_GAIN_LB
		|| s->data.tb.down_loop_gain <= ERR_GAIN_LB) {
		return 1;
	}
	if (s->data.tb.sampling_period_ms < SAMPLING_MS_MINIMUM)
		s->data.tb.sampling_period_ms = SAMPLING_MS_MINIMUM;
	return 0;
}
	...
	for (j = 0; j < s->data.tm.num_thresholds; j++) {
		for (k = 0; k < s->data.tm.t[j].num_actions; k++) {
			if ((k < s->data.tm.t[j].num_actions-1)
			    && (s->data.tm.t[j].actions[k].device[0] != ERR_DEVICE_VAL)
...
static int validate_bwlm(struct setting_info *s)
{
	uint32_t i;

	/* Disable no threshold entries */
	if (s->data.bwlm.num_thresholds == 0)
		return 1;
	if (s->data.bwlm.sensor_list[0][0] == ERR_SENSOR_VAL)
		return 1;
	for (i = 0; i < s->data.bwlm.num_thresholds; i++) {
		if (s->data.bwlm.t[i].num_actions == 0)
			return 1;
	}

	/* Ensure thresholds are in ascending or descending order
		depending on algorithm instance configuration */
	if (s->data.bwlm.num_thresholds > 1) {
		qsort(s->data.bwlm.t, s->data.bwlm.num_thresholds,
			sizeof(struct bwlm_threshold_t),
			bwlm_threshold_array_compare);
	}
...
static int load_fields(void)
{
	...
	uint32_t max_fields = ARRAY_SIZE(global_table) + ARRAY_SIZE(pid_table) +
			 ...
			 ARRAY_SIZE(ss_table) + ARRAY_SIZE(v_table) + ARRAY_SIZE(tb_table);
	...
	get_unique_fields(tb_table, ARRAY_SIZE(tb_table), &unique_fields);
	get_unique_fields(v_table, ARRAY_SIZE(v_table), &unique_fields);
	get_unique_fields(bwlm_table, ARRAY_SIZE(bwlm_table), &unique_fields);
...
static void print_ss_setting(struct setting_info *setting, int flag)
{
	...
	if (ss->device_perf_floor)
		PRINT_SETTING(flag, "device_perf_floor %d", ss->device_perf_floor);
}

static void print_tb_setting(struct setting_info *setting, int flag)
{
	struct tb_setting *tb = &setting->data.tb;

	PRINT_SETTING(flag, "[%s]", setting->desc);
	if (setting->dynamic)
		PRINT_SETTING(flag, "algo_type tb");
	else
		PRINT_SETTING(flag, "#algo_type tb");
	PRINT_SETTING(flag, "sampling %d\nsensor %s\ndevice %s",
	     tb->sampling_period_ms, tb->sensor, tb->device);
	PRINT_SETTING(flag, "set_point %d\nset_point_clr %d\ntime_constant %d",
	     tb->set_point,
	     tb->set_point_clr,
	     tb->time_constant);
	PRINT_SETTING(flag, "up_loop_gain %f\ndown_loop_gain %f",
		tb->up_loop_gain, tb->down_loop_gain);
	PRINT_SETTING(flag, "auto_penalty %f\nauto_reward %f",
		tb->auto_penalty, tb->auto_reward);
	PRINT_SETTING(flag, "temp_scale_factor %f", tb->temp_scale_factor);
	PRINT_SETTING(flag, "freq_scale_factor %f", tb->freq_scale_factor);
	PRINT_SETTING(flag, "quadratic_reward %d (nonzero true)\nquadratic_penalty %d (nonzero true)",
		tb->quadratic_reward, tb->quadratic_penalty);
	PRINT_SETTING(flag, "use_timeout %d (nonzero true)", tb->use_timeout);
	if (tb->use_timeout)
		PRINT_SETTING(flag, "timeout %d (num interrupts)", tb->timeout);
	PRINT_SETTING(flag, "unified_rail %d (nonzero true)", tb->unified_rail);
	if (tb->override)
		PRINT_SETTING(flag, "override %d", tb->override);
}
...
static void print_vs_setting(struct setting_info *setting, int flag)
{
	...
	snprintf(info_buf, sizeof(info_buf), "trip_sensors");
	for (idx = 0; idx < vs->trip_cnt; idx++) {
		if (strlcat(info_buf, " ", sizeof(info_buf)) >=
		    sizeof(info_buf))
			break;
		if (strlcat(info_buf, vs->trip_sensor_list[idx],
			    sizeof(info_buf)) >= sizeof(info_buf))
			break;
	}
	info_buf[sizeof(info_buf) - 1] = '\0';
	...
	if (vs->trip_cnt > 1) {
		snprintf(info_buf, sizeof(info_buf), "trip_sensors_logic");
		for (idx = 0; idx < vs->trip_cnt - 1; idx++) {
			if (strlcat(info_buf, " ", sizeof(info_buf)) >=
			    sizeof(info_buf))
				break;
			if (strlcat(info_buf, vs->trip_sensor_logic[idx],
				sizeof(info_buf)) >= sizeof(info_buf))
				break;
		}
		info_buf[sizeof(info_buf) - 1] = '\0';
	...
	for (idx = 0; idx < vs->trip_cnt; idx++) {
		if (strlcat(info_buf, " ", sizeof(info_buf)) >=
		    sizeof(info_buf))
			break;
		snprintf(int_buf, sizeof(int_buf), "%d", vs->t[idx].lvl_trig);
		if (strlcat(info_buf, int_buf, sizeof(info_buf)) >=
			    sizeof(info_buf))
			break;
	}
	...
	for (idx = 0; idx < vs->trip_cnt; idx++) {
		...
		snprintf(int_buf, sizeof(int_buf), "%d", vs->t[idx].lvl_clr);
		if (strlcat(info_buf, int_buf,
			    sizeof(info_buf)) >= sizeof(info_buf))
			break;
	}
	...
	if (vs->descending_thresh)
		PRINT_SETTING(flag, "descending");
	if (vs->tracks_low)
		PRINT_SETTING(flag, "tracks_low");
	PRINT_SETTING(flag, "sampling %d", vs->sampling_period_ms);
	PRINT_SETTING(flag, "math %d", vs->math_type);
	if (vs->trip_equal_logic)
		PRINT_SETTING(flag, "trip_equal_logic");
	if (vs->trip_always_mon)
		PRINT_SETTING(flag, "trip_always_monitor");
}

static void print_bwlm_setting(struct setting_info *setting, int flag)
{
	char info_buf[1000];
	char int_buf[UINT_BUF_MAX];
	uint8_t idx, action_idx, sensor_idx;
	struct bwlm_setting *bwlm = &setting->data.bwlm;

	PRINT_SETTING(flag, "[%s]", setting->desc);
	if (setting->dynamic)
		PRINT_SETTING(flag, "algo_type bwlm");
	else
		PRINT_SETTING(flag, "#algo_type bwlm");

	snprintf(info_buf, sizeof(info_buf), "sensor_list");
	for (idx = 0; idx < bwlm->num_sensors; idx++) {
		if (strlcat(info_buf, " ", sizeof(info_buf)) >=
		    sizeof(info_buf))
			break;

		if (strlcat(info_buf, bwlm->sensor_list[idx], sizeof(info_buf))
			>= sizeof(info_buf))
			break;
	}
	info_buf[sizeof(info_buf) - 1] = '\0';
	...
	snprintf(info_buf, sizeof(info_buf), "bus  %d", bwlm->bus);
	PRINT_SETTING(flag, "%s", info_buf);

	snprintf(info_buf, sizeof(info_buf), "ib_thresholds");
	for (idx = 0; idx < bwlm->num_thresholds; idx++) {
		if (strlcat(info_buf, " ", sizeof(info_buf)) >=
		    sizeof(info_buf))
			break;
		snprintf(int_buf, sizeof(int_buf), "%lu",
			(unsigned long int)bwlm->t[idx].ib_trig);
		if (strlcat(info_buf, int_buf, sizeof(info_buf)) >=
			    sizeof(info_buf))
			break;
	}
	...
	snprintf(info_buf, sizeof(info_buf), "ib_thresholds_clr");
	for (idx = 0; idx < bwlm->num_thresholds; idx++) {
		if (strlcat(info_buf, " ", sizeof(info_buf)) >=
		    sizeof(info_buf))
			break;
		...
		snprintf(int_buf, sizeof(int_buf), "%lu",
			(unsigned long int)bwlm->t[idx].ib_clr);
		if (strlcat(info_buf, int_buf,
			    sizeof(info_buf)) >= sizeof(info_buf))
			break;
	}
	...
	snprintf(info_buf, sizeof(info_buf), "ab_threshold");
	for (idx = 0; idx < bwlm->num_thresholds; idx++) {
		if (strlcat(info_buf, " ", sizeof(info_buf)) >=
		    sizeof(info_buf))
			break;
		for (sensor_idx = 0; sensor_idx <
		      bwlm->num_sensors; sensor_idx++) {
			if (sensor_idx > 0) {
				if (strlcat(info_buf, "+",
					    sizeof(info_buf)) >=
				    sizeof(info_buf))
			...
			snprintf(int_buf, sizeof(int_buf), "%lu",
			(unsigned long int)bwlm->t[idx].ab_trig[sensor_idx]);
			if (strlcat(info_buf, int_buf, sizeof(info_buf))
			     >= sizeof(info_buf))
				break;
		}
	...
	snprintf(info_buf, sizeof(info_buf), "ab_threshold_clr");
	for (idx = 0; idx < bwlm->num_thresholds; idx++) {
		if (strlcat(info_buf, " ", sizeof(info_buf)) >=
		    sizeof(info_buf))
			break;
		for (sensor_idx = 0; sensor_idx <
			...
			snprintf(int_buf, sizeof(int_buf), "%lu",
			(unsigned long int)bwlm->t[idx].ab_clr[sensor_idx]);
			if (strlcat(info_buf, int_buf, sizeof(info_buf))
			     >= sizeof(info_buf))
				break;
		}
	...
	snprintf(info_buf, sizeof(info_buf), "utilize_thresholds");
	for (idx = 0; idx < bwlm->num_thresholds; idx++) {
		if (strlcat(info_buf, " ", sizeof(info_buf)) >=
		    sizeof(info_buf))
			break;
		snprintf(int_buf, sizeof(int_buf), "%lu",
			(unsigned long int)bwlm->t[idx].utilize_trig);
		if (strlcat(info_buf, int_buf, sizeof(info_buf)) >=
			    sizeof(info_buf))
			break;
	}
	...
	snprintf(info_buf, sizeof(info_buf), "utilize_thresholds_clr");
	for (idx = 0; idx < bwlm->num_thresholds; idx++) {
		if (strlcat(info_buf, " ", sizeof(info_buf)) >=
		    sizeof(info_buf))
			break;
		...
		snprintf(int_buf, sizeof(int_buf), "%lu",
			(unsigned long int)bwlm->t[idx].utilize_clr);
		if (strlcat(info_buf, int_buf,
			    sizeof(info_buf)) >= sizeof(info_buf))
			break;
	}
	...
	for (idx = 0; idx < bwlm->num_thresholds; idx++) {
		...
		for (action_idx = 0; action_idx <
		      bwlm->t[idx].num_actions; action_idx++) {
			if (action_idx > 0) {
				if (strlcat(info_buf, "+",
					    sizeof(info_buf)) >=
				    sizeof(info_buf))
			...
			if (strlcat(info_buf, bwlm->t[idx].actions[action_idx].\
				    device, sizeof(info_buf)) >=
			    sizeof(info_buf))
				break;
		}
	...
	for (idx = 0; idx < bwlm->num_thresholds; idx++) {
		...
		for (action_idx = 0; action_idx <
			...
			snprintf(int_buf, sizeof(int_buf), "%d",
				 bwlm->t[idx].actions[action_idx].info);
			if (strlcat(info_buf, int_buf, sizeof(info_buf))
			     >= sizeof(info_buf))
				break;
		}
...
void print_setting(struct setting_info *setting)
{
	...
	case SS_ALGO_TYPE:
		...
		break;
	case TB_ALGO_TYPE:
		print_tb_setting(setting, PRINT_TO_INFO);
		break;
	case EQUILIBRIUM_TYPE:
	...
	case VIRTUAL_SENSOR_TYPE:
		...
		break;
	case BWLM_ALGO_TYPE:
		print_bwlm_setting(setting, PRINT_TO_INFO);
		break;
	default:
...
void print_settings(struct thermal_setting_t *settings)
{
	...
	while (setting != NULL) {
		...
		case SS_ALGO_TYPE:
			...
			break;
		case TB_ALGO_TYPE:
			print_tb_setting(setting, PRINT_TO_STDOUT);
			break;
		case EQUILIBRIUM_TYPE:
		...
		case VIRTUAL_SENSOR_TYPE:
			...
			break;
		case BWLM_ALGO_TYPE:
			print_bwlm_setting(setting, PRINT_TO_STDOUT);
			break;
		default:
			msg("%s: Uknown algo type", __func__);
			break;
