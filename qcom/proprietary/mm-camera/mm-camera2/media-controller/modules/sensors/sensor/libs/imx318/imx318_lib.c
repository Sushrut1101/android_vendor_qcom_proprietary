...
#ifdef IMX318_DPHY
#include "imx318_DPHY_lib.h"
#else
#include "imx318_lib.h"
#endif
...
#define PDAF_CONFIDENCE_THRESH                 (1023)
...
int32_t imx318_pdaf_calculate_defocus
  (void * arg1,
   signed long (*get_defocus)(void *input, void *output))
{
  ...
  for(i=0;i<POINT_NUM;i++) {
    /*conf = 1024* 145(sony min conf)/250(qc min conf)*/
    conf[i] = 594;
  ...
  for(j = 0;j < pd_win_y;j++)
    for(i = 0;i < pd_win_x;i++) {
     ...
     SLOW("PDAF INPUT[%d][%d]: window: X_STA %d X_END %d, Y_STA %d Y_END %d",
       i, j,
      ...
      if (output.DefocusConfidenceLevel > PDAF_CONFIDENCE_THRESH) {
        output.DefocusConfidenceLevel = PDAF_CONFIDENCE_THRESH;
      }
...
/**
 * FUNCTION: imx318_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static unsigned int imx318_real_to_register_gain(float real_gain)
{
    unsigned int reg_gain = 0;

    if (real_gain < IMX318_MIN_AGAIN)
    {
        real_gain = IMX318_MIN_AGAIN;
        SLOW("IMX318 Requested gain(%f) is lower than the sensor minimum(%d)",
            real_gain, IMX318_MIN_AGAIN);
    }
    else if (real_gain > IMX318_MAX_AGAIN)
    {
        real_gain = IMX318_MAX_AGAIN;
    }
...
/**
 * FUNCTION: imx318_register_to_real_gain
 *
 * DESCRIPTION: Calcuate sensor real gain value
 **/
static float imx318_register_to_real_gain(unsigned int reg_gain)
{
    float gain;

    if(reg_gain > IMX318_MAX_AGAIN_REG_VAL)
        reg_gain = IMX318_MAX_AGAIN_REG_VAL;
...
/**
 * FUNCTION: imx318_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static unsigned int imx318_digital_gain_calc(
 float real_gain, float sensor_real_gain)
{
    unsigned int reg_dig_gain = IMX318_MIN_DGAIN_REG_VAL;
    float real_dig_gain = IMX318_MIN_DGAIN;

    if(real_gain > IMX318_MAX_AGAIN)
    {
        real_dig_gain = real_gain / sensor_real_gain;
    }
    else
    {
        real_dig_gain = IMX318_MIN_DGAIN;
    }

    if(real_dig_gain > IMX318_MAX_DGAIN)
    {
        real_dig_gain = IMX318_MAX_DGAIN;
    }
  ...
  unsigned int line_count, sensor_exposure_info_t *exp_info, float s_real_gain)
{
    ...
    exp_info->reg_gain = imx318_real_to_register_gain(real_gain);
    exp_info->sensor_real_gain =
      imx318_register_to_real_gain(exp_info->reg_gain);
    exp_info->sensor_digital_gain =
      imx318_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
    exp_info->sensor_real_dig_gain =
      (float)exp_info->sensor_digital_gain / IMX318_MAX_DGAIN_DECIMATOR;
    ...
    exp_info->s_reg_gain = imx318_real_to_register_gain(sensor_s_real_gain);

    return 0;
}

/**
 * FUNCTION: imx318_fill_exposure_array
 *
  ...
  __attribute__((unused)) int is_hdr_enabled)
{
    ...
    if(luma_delta != 0){
      ...
      reg_setting->reg_setting[reg_count].reg_addr = IMX318_EXP_RATIO_ADDR;
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX318_DIG_GAIN_GLOBAL_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX318_DIG_GAIN_GLOBAL_ADDR + 1;
  ...
  unsigned short awb_gain_b, struct camera_i2c_seq_reg_setting* reg_setting)
{
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX318_ABS_GAIN_R_WORD_ADDR;
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX318_ABS_GAIN_B_WORD_ADDR;
