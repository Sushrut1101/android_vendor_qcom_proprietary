    ...
    if(sample_list != NULL) {
        if( sample_list->data[3] ) { /* If report.step_event is true */
            for (i = 0; i < count; i++) {
                ...
                HAL_LOG_VERBOSE("%s: STEP DETECTOR: step event %f SAM TS: %u HAL TS:%" PRId64 " elapsedRealtimeNano:%" PRId64 "",
                                 __FUNCTION__, la_sample.data[0], curr_sample->timestamp, la_sample.timestamp,
