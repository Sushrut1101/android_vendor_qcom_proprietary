...
/**
 * wlan_has_npt() : check if network performance tuner
 * service is available.
 *
 * Return: 1 if service is available, 0 if not.
 */
static int wlan_has_npt(void)
{
	int sock;

	if (!checked_has_npt) {
		sock = socket_local_client("wigig/wigignpt",
		...
		if (sock >= 0)
			close(sock);
		checked_has_npt = 1;
		wsvc_printf_info("%s : has_npt is %d", __func__, has_npt);
	}
