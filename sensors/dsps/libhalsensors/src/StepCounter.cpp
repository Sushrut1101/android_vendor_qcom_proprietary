...
void StepCounter::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    if (SNS_SAM_PED_REPORT_IND_V01 == msg_hdr->msg_id) {
        ...
        if (NULL == sample_list) {
            ...
            memset(sample_list, 0, sizeof(hal_sam_sample_t));
            count = 1;
        ...
        if (NULL == sample_list) {
            ...
            memset(sample_list, 0, sam_ind->items_len * sizeof(hal_sam_sample_t));
    ...
    for (i = 0; i < count; i++) {
        ...
        if (SNS_SAM_PED_GET_REPORT_RESP_V01 == msg_hdr->msg_id &&
            step_counter_current_instance != curr_sample->data[0]) {
