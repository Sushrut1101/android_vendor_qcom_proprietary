...
static int annouce_servers(struct context *ctx, struct sockaddr_qrtr *sq)
{
	...
	map_for_each(&nodes, node_me) {
		...
		map_for_each(&node->services, srv_me) {
			srv = map_iter_data(srv_me, struct server, mi);
...
static void cfg_nid(uint32_t addr)
{
	struct {
		struct nlmsghdr nh;
...
static void usage(void)
{
	...
	fprintf(stderr, "\t-i,\tConfigure local node id to <node-id>\n");
	fprintf(stderr, "\t-h,\tHelp option to print usage\n");
	exit(1);
}
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "fhi:")) != -1) {
		...
		case 'f':
			...
			break;
		case 'i':
			addrul = strtoul(optarg, &ep, 10);
			if (*ep != '\0' || addrul >= UINT_MAX)
				usage();
			addr = addrul;
			break;
		default:
	...
	rc = setsockopt(ctx.sock, SOL_SOCKET, SO_RCVBUF, &rx_buf_sz, sizeof(int));
	if (rc < 0)
		err(1, "set rx buffer size");
