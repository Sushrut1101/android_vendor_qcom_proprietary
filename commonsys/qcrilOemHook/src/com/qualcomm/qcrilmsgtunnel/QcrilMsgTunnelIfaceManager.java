...
public class QcrilMsgTunnelIfaceManager extends IQcrilMsgTunnel.Stub {
    ...
    private static QcrilOemhookMsgTunnel[] mQcrilOemhookMsgTunnel = null;
    ...
    QcrilMsgTunnelIfaceManager(QcrilMsgTunnelService service) {
        ...
        mQcrilOemhookMsgTunnel = new QcrilOemhookMsgTunnel[numPhones];

        for (int i = 0; i < numPhones; i++) {
            mQcrilOemhookMsgTunnel[i] = new QcrilOemhookMsgTunnel(i);
            Message msg =  Message.obtain();
            msg.arg1 = i;
            mQcrilOemhookMsgTunnel[i].setOnUnsolOemHookExtApp(mMessageHandler,
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case CMD_INVOKE_OEM_RIL_REQUEST:
                    ...
                    if (request.sub >= 0 && request.sub < getPhoneCount()) {
                        mQcrilOemhookMsgTunnel[request.sub].invokeOemRilRequestRaw(
                                (byte[]) request.argument, onCompleted);
                    } else {
                        Log.e(TAG, "Invalid Phone Id sent by client. Bail " + request.sub);
                    }
                    break;
                ...
                case CMD_INVOKE_OEM_RIL_REQUEST_ASYNC:
                    ...
                    if (requestAsync.sub >= 0 && requestAsync.sub < getPhoneCount()) {
                        mQcrilOemhookMsgTunnel[requestAsync.sub].invokeOemRilRequestRaw(
                            (byte[]) requestAsync.arg1, onCompleted);
                    } else {
                        Log.e(TAG, "Invalid Phone Id sent by client. Bail " + requestAsync.sub);
                    }
                    break;
                ...
                case EVENT_INVOKE_OEM_RIL_REQUEST_ASYNC_DONE:
                    ...
                    try {
                        //TODO: change interface to send AsyncResult instead of
                        //return-value/buffer of api call. For example how would
                        //a client know if his async call resulted in failure.
                        if (ar.exception != null) {
                            Log.e(TAG, "OEM_RIL_REQUEST_ASYNC_DONE Error received :"
                                    + ar.exception);
                            cb.onOemHookException(requestAsync.sub);
                        } else {
                            requestAsync.result = ar.result;
                            Log.d(TAG, "OEM_RIL_REQUEST_ASYNC_DONE requestAsync.sub :"
                                    + requestAsync.sub);
    ...
    /**
     * Utility copy a byte array
     *
     * @param source source array of bytes to copy from
     * @param dest destination array of bytes to copy to
     * @param length length to be copied
     *
     * @return null
     */
    private void copyByteArray(byte[] source, byte[] dest, int length) {
        if (source == null || dest == null) {
            throw new NullPointerException("source = " + source + " destination=" + dest);
        }
        int sourcelen = source.length;
        int destlen = dest.length;
        if (length < 0 || sourcelen < 0 || destlen < 0 || destlen < length || sourcelen < length) {
            throw new ArrayIndexOutOfBoundsException("source length = " + sourcelen +
                        " destination length = " + destlen);
        }
        ...
        try {
            ...
            if (result.exception == null) {
                if (result.result != null) {
                    ...
                    copyByteArray(responseData, response, responseData.length);
