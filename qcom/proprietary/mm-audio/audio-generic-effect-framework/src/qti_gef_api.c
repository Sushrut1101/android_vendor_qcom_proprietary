...
typedef int (*audio_extn_gef_send_audio_cal_t)(void*, int,
    int, int, int, int, uint32_t, uint16_t, uint32_t, void*, int, bool);

typedef int (*audio_extn_gef_get_audio_cal_t)(void*, int,
    int, int, int, int, uint32_t, uint16_t, uint32_t, void*, int*, bool);

typedef int (*audio_extn_gef_store_audio_cal_t)(void*, int,
    int, int, int, int, uint32_t, uint16_t, uint32_t, void*, int);
...
int gef_enable_effect(gef_handle_t* handle)
{
    ...
    event_value value = {0, EFFECT_UUID_NULL_, 0, 0, 0, 0, 0};
        ...
        effect_data_in* data)
{
    ...
    if (gp_data->send_cal && gp_data->hal_data)
        ret = gp_data->send_cal(gp_data->hal_data, params->device_id,
            ...
            params->sample_rate, params->module_id, 0,
            params->param_id, data->data, data->length, params->persist);
#else
    if (gp_data->send_cal && gp_data->hal_data)
        ret = gp_data->send_cal(gp_data->hal_data, params->device_id,
            params->cal_type, params->app_type, params->topo_id,
 ...
 *       ENOSYS: Sending parameters is not supported
 */
int gef_set_param_v2(gef_handle_t* handle,
        effect_config_params_v2* params,
        effect_data_in* data)
{
    int ret = -ENOSYS;
    effect_private_data* effect_data = (effect_private_data*)handle;
    ...
    if (gp_data->send_cal && gp_data->hal_data)
        ret = gp_data->send_cal(gp_data->hal_data, params->device_id,
            params->cal_type, params->app_type, params->topo_id,
            params->sample_rate, params->module_id, params->instance_id,
            params->param_id, data->data, data->length, params->persist);
#endif
...
/*
 * This function copies data from version 2 format to version 1.
 * Version 1 format: Module ID, Param ID, ...
 * Version 2 format: Module ID, Instance ID, Param ID, ...
 */
void copy_v2_to_v1(void* input_data_v1, void* input_data_v2, int len_v1)
{
    uint32_t* data_v1 = (uint32_t*) input_data_v1;
    uint32_t* data_v2 = (uint32_t*) input_data_v2;

    if (NULL == data_v1 || NULL == data_v2 || len_v1 <= 0) {
        return;
    }
    ...
    memcpy(data_v1, data_v2, len_v1 - sizeof(uint32_t));
}
...
#ifdef INSTANCE_ID_ENABLED
    data_len_v2 = *(data->length) + sizeof(uint32_t);
    data_v2 = (char*) calloc(data_len_v2, sizeof(char));
    if (gp_data->get_cal && gp_data->hal_data)
        ret = gp_data->get_cal(gp_data->hal_data, params->device_id,
            params->cal_type, params->app_type, params->topo_id,
            params->sample_rate, params->module_id, 0,
            params->param_id, data_v2, &data_len_v2, params->persist);

    copy_v2_to_v1(data->data, data_v2, *(data->length));
    free(data_v2);
#else
    if (gp_data->get_cal && gp_data->hal_data)
        ret = gp_data->get_cal(gp_data->hal_data, params->device_id,
 ...
 * returns:- ENOSYS: Retrieving parameters is not supported
 */
int gef_get_param_v2(const gef_handle_t* handle,
        effect_config_params_v2* params,
        effect_data_out* data)
{
    int ret = -ENOSYS;
        ...
        effect_data_in* data)
{
    ...
    if (gp_data->store_cal && gp_data->hal_data)
        ret = gp_data->store_cal(gp_data->hal_data, params->device_id,
                params->cal_type, params->app_type, params->topo_id,
                params->sample_rate, params->module_id, 0,
                params->param_id, data->data, data->length);
#else
    if (gp_data->store_cal && gp_data->hal_data)
        ret = gp_data->store_cal(gp_data->hal_data, params->device_id,
                params->cal_type, params->app_type, params->topo_id,
 ...
 *      ENOSYS: Sending parameters is not supported
 */
int gef_store_cacheparam_v2(gef_handle_t* handle,
        effect_config_params_v2* params,
        effect_data_in* data)
{
    int ret = -ENOSYS;
...
void gef_device_config_cb(void* handle, audio_devices_t device,
        audio_channel_mask_t cmask, int sample_rate,
        int acdb_dev_id, int app_type)
{
    struct listnode *node;
