...
extern "C" jint Java_com_qualcomm_qti_seccamservice_SecCamService_doStartTzApp(
    ...
    seccam_if_destination_type_t destination =
        (seccam_if_destination_type_t) j_destination;

    if (destination >= SECCAM_IF_DESTINATION_MAX ||
        destination < SECCAM_IF_DESTINATION_QTEE) {
        ALOGE("doStartTzApp - invalid j_destination: %d", destination);
        return (jint)BAD_VALUE;
    }

    int32_t ret = secCamStartTzApp(app_name.c_str(),
            j_buff_size, &if_version_maj, &if_version_min, destination);
    if (OK == ret) {
        jfieldID field_id;
        field_id = env->GetFieldID(cls, "verMaj_", "I");
...
//=========================================================================
extern "C" jboolean Java_com_qualcomm_qti_seccamservice_SecCamService_isDualStreamSupported(
    JNIEnv* env, jobject thiz) {
    return (jboolean)secCamIsDualStreamSupported();
}
//=========================================================================
...
extern "C" jboolean Java_com_qualcomm_qti_seccamservice_SecCamService_releaseSecurePreviewSurface(
    ...
    return (jboolean)secCamReleaseSecurePreviewSurface(nullptr, j_capture_surface_id);
}

//=========================================================================
extern "C" void Java_com_qualcomm_qti_seccamservice_SecCamService_releaseAllSecureSurface(
    JNIEnv* env, jobject thiz) {

    ALOGD("Releasing all Pending surfaces");
    return secCamReleaseAllSecureSurface();
}
