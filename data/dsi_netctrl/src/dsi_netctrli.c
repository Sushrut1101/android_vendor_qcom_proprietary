...
/*Set to true when default WDS handle for a modem initialized*/
boolean is_default_handle_inited[DSI_MAX_MODEMS] = {DSI_FALSE, DSI_FALSE};
...
char * dsi_qmi_port_names[DSI_MAX_IFACES] =
{
    ...
    DSI_QMI_PORT_RMNET_8,
    DSI_QMI_PORT_RMNET_9,
    DSI_QMI_PORT_RMNET_10,
    DSI_QMI_PORT_RMNET_11,
    DSI_QMI_PORT_RMNET_12,
    DSI_QMI_PORT_RMNET_13,
    DSI_QMI_PORT_RMNET_14,
    DSI_QMI_PORT_RMNET_15,
    DSI_QMI_PORT_RMNET_16,
    ...
    DSI_QMI_PORT_RMNET_SDIO_8,
    DSI_QMI_PORT_RMNET_SDIO_9,
    DSI_QMI_PORT_RMNET_SDIO_10,
    DSI_QMI_PORT_RMNET_SDIO_11,
    DSI_QMI_PORT_RMNET_SDIO_12,
    DSI_QMI_PORT_RMNET_SDIO_13,
    DSI_QMI_PORT_RMNET_SDIO_14,
    DSI_QMI_PORT_RMNET_SDIO_15,
    DSI_QMI_PORT_RMNET_SDIO_16,
};
...
)
{
  ...
  {
    DSI_LOG_DEBUG("Executing Callback function %p with %p data",
...
)
{
  ...
  *link_state_v6 = WDS_CONNECTION_STATUS_DISCONNECTED_V01;
  ...
  if(dsi_config.single_qmux_ch_enabled == DSI_TRUE)
  {
    ...
    ds_get_epid(dsi_config.phys_net_dev, (ds_ep_type_t *) &bind_params.ep_id.ep_type, (int *) &bind_params.ep_id.iface_id);
...
DSI_INLINE void dsi_detach_dsi_iface(dsi_store_t * st)
{
  ...
  if ( !DSI_IS_IFACE_VALID(st->priv.dsi_iface_id) )
  {
    DSI_LOG_ERROR("%s","invalid dsi iface");
    return;
  }
...
)
{
  ...
  do
  {
    ...
    if (dsi_hndl->priv.start_nw_params.bring_up_by_apn_name_valid)
    {
      pparams->params_mask |= DSI_QMI_WDS_ROUTE_LOOKUP_BRINGUP_BY_APN_NAME_PARAM;
      pparams->bringup_by_apn_name = dsi_hndl->priv.start_nw_params.bring_up_by_apn_name;
    }
    else if (dsi_hndl->priv.start_nw_params.bring_up_by_apn_type_valid)
    {
      pparams->params_mask |= DSI_QMI_WDS_ROUTE_LOOKUP_BRINGUP_BY_APN_TYPE_PARAM;
      pparams->bringup_by_apn_type = dsi_hndl->priv.start_nw_params.bring_up_by_apn_type;
    }
    ...
    if (dsi_hndl->priv.start_nw_params.ext_technology_preference_valid)
    {
      ...
      pparams->xtended_tech_pref =
        (wds_technology_name_enum_v01) dsi_hndl->priv.start_nw_params.ext_technology_preference;
    ...
    if (dsi_hndl->priv.start_nw_params.apn_type_valid)
    {
      pparams->params_mask |= DSI_QMI_WDS_ROUTE_LOOKUP_APN_TYPE_PARAM;
      pparams->apn_type = dsi_hndl->priv.start_nw_params.apn_type;
    }
    else
    {
      pparams->apn_type = WDS_APN_TYPE_ENUM_MIN_ENUM_VAL_V01;
    }

    if (dsi_hndl->priv.start_nw_params.apn_type_mask_valid)
    {
      pparams->params_mask |= DSI_QMI_WDS_ROUTE_LOOKUP_APN_TYPE_MASK_PARAM;
      pparams->apn_type_mask = dsi_hndl->priv.start_nw_params.apn_type_mask;
    }
    else
    {
      pparams->apn_type_mask = WDS_APN_TYPE_MASK_UNSPECIFIED_V01;
    }
...
)
{
  ...
  for(i=0; i<DSI_MAX_DATA_CALLS; i++)
  {
       ...
       )
    {
      ...
      DSI_LOG_VERBOSE("data store [0x%p] updated", (unsigned int*)st);
      handle_found = TRUE;
    }
  }
...
)
{
  ...
  for(i = 0; i < DSI_MAX_DATA_CALLS; i++)
  {
       ...
       )
    {
      ...
      DSI_LOG_VERBOSE("data store [0x%p] updated with call technology:%x",
...
*/
/*=========================================================================*/
static DSI_INLINE void dsi_invalidate_modems(boolean from_ssr)
{
  int m=0;
  ...
  for(m = 0; m<DSI_MAX_MODEMS; m++)
  {
    ...
    if(is_default_handle_inited[m] == DSI_FALSE)
    {
...
static DSI_INLINE void dsi_invalidate_iface(int dsi_iface)
{
    ...
    DSI_SET_SUBS_ID(dsi_iface,DSI_DATA_DEFAULT_SUBS);
...
void dsi_modem_out_of_service_hdlr(int conn_id)
{
  ...
  {
    is_default_handle_inited[modem] = DSI_FALSE;
...
void dsi_modem_in_service_hdlr(int conn_id)
{
  ...
  if(DSI_MODEM_GET_DEFAULT_IFACE(modem) == conn_id)
  {
    if(DSI_MODEM_GET_DEFAULT_WDS_HNDL(modem) == DSI_INVALID_WDS_HNDL)
    {
      if(is_default_handle_inited[modem] == DSI_FALSE)
      {
        if(DSI_SUCCESS == dsi_qmi_wds_init_qmi_client(DSI_GET_WDS_STR(conn_id),
                                               &wds_hndl))
        {
          DSI_MODEM_SET_DEFAULT_WDS_HNDL(modem, wds_hndl);
          is_default_handle_inited[modem] = DSI_TRUE;
          DSI_LOG_DEBUG( "dsi_qmi_wds_init_qmi_client for %d modem: %p wds_hndl",modem, DSI_MODEM_GET_DEFAULT_WDS_HNDL(modem));
        }
        else
        {
...
static DSI_INLINE int dsi_init_ifaces()
{
  ...
  dsi_qmi_port_names[8]  = DSI_QMI_PORT_RMNET_0;
  dsi_qmi_port_names[9]  = DSI_QMI_PORT_RMNET_0;
  dsi_qmi_port_names[10] = DSI_QMI_PORT_RMNET_0;
  dsi_qmi_port_names[11] = DSI_QMI_PORT_RMNET_0;
  dsi_qmi_port_names[12] = DSI_QMI_PORT_RMNET_0;
  dsi_qmi_port_names[13] = DSI_QMI_PORT_RMNET_0;
  dsi_qmi_port_names[14] = DSI_QMI_PORT_RMNET_0;
  dsi_qmi_port_names[15] = DSI_QMI_PORT_RMNET_0;
  dsi_qmi_port_names[16] = DSI_QMI_PORT_RMNET_0;

  dsi_qmi_port_names[17] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[18] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[19] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[20] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[21] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[22] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[23] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[24] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[25] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[26] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[27] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[28] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[29] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[30] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[31] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[32] = DSI_QMI_PORT_RMNET_INVALID;
  dsi_qmi_port_names[33] = DSI_QMI_PORT_RMNET_INVALID;
...
int dsi_init_internal(dsi_mni_vtable_t * mni_vtbl, boolean from_ssr)
{
  ...
  do
  {
    ...
    dsi_invalidate_modems(from_ssr);
    ...
    DSI_LOG_DEBUG("%s","dsi_init init modem DSI_RADIO_4GMODEM" \
                  ...
                  " cotains iface ids 17..33");

    /* modem DSI_RADIO_4GMODEM contains iface 17..33 */
    ...
    /* set this modem index on iface 17..33 */
    DSI_SET_MODEM(17, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(18, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(19, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(20, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(21, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(22, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(23, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(24, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(25, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(26, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(27, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(28, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(29, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(30, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(31, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(32, DSI_RADIO_4GMODEM);
    DSI_SET_MODEM(33, DSI_RADIO_4GMODEM);
    ...
    /* modem DSI_RADIO_4GMODEM has default iface 17 */
    DSI_MODEM_SET_DEFAULT_IFACE(DSI_RADIO_4GMODEM, 17);

    DSI_LOG_DEBUG("%s","dsi_init init modem DSI_RADIO_3GMODEM" \
                  " with 1X technology support " \
                  " contains iface id 0..16");
    /* modem DSI_RADIO_3GMODEM contains iface 0..16 */
    ...
    /* set this modem index on iface 0..16*/
    ...
    DSI_SET_MODEM(8, DSI_RADIO_3GMODEM);
    DSI_SET_MODEM(9, DSI_RADIO_3GMODEM);
    DSI_SET_MODEM(10, DSI_RADIO_3GMODEM);
    DSI_SET_MODEM(11, DSI_RADIO_3GMODEM);
    DSI_SET_MODEM(12, DSI_RADIO_3GMODEM);
    DSI_SET_MODEM(13, DSI_RADIO_3GMODEM);
    DSI_SET_MODEM(14, DSI_RADIO_3GMODEM);
    DSI_SET_MODEM(15, DSI_RADIO_3GMODEM);
    DSI_SET_MODEM(16, DSI_RADIO_3GMODEM);
