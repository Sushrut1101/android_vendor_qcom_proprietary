...
void LearningModule::runPeriodic() {
    DEBUGV(LOG_TAG_LM, "Running periodic algorithm");
    bool shouldRun;
    unsigned long int sleepDurationInMs = -1;
    while (runPeriodicThreadInterrupt == false) {
        shouldRun = false;
        for (Feature* currFeature : mFeatureList) {
            shouldRun |= currFeature->mRunFlag;
            // take hcf instead of min
            sleepDurationInMs = min(sleepDurationInMs, currFeature->mSleepDurationInMs);
        }
        DEBUGV(LOG_TAG_LM, "Starting threads, shouldRun = %d", shouldRun);
        if (!shouldRun) {
            unique_lock<mutex> lk(mFeatureRunner.mWaitForThreads);
            DEBUGV(LOG_TAG_LM, "Waiting for threads to signal");
            mFeatureRunner.mRunPeriodic.wait(lk);
            DEBUGV(LOG_TAG_LM, "Signal received from thread");
            lk.unlock();
        }
        if (sleepDurationInMs == (unsigned long int)-1)
            continue;

        this_thread::sleep_for(chrono::milliseconds(sleepDurationInMs));
        for (Feature* currFeature : mFeatureList) {
            if(currFeature and currFeature->mRunFlag) {
                DEBUGV(LOG_TAG_LM,"Calling runAsyncPeriodic for %s", currFeature->getFeatureName().c_str());
                if(mEnableLMtracing) {
                    string traceLog = "LM-runPeriodic() " + currFeature->getFeatureName();
                    ATRACE_ASYNC_BEGIN(traceLog.c_str(), 0);
                }
                currFeature->runAsyncPeriodic(false);
                if(mEnableLMtracing) {
                    string traceLog = "LM-runPeriodic() " + currFeature->getFeatureName();
                    ATRACE_ASYNC_END(traceLog.c_str(), 0);
                }
                DEBUGV(LOG_TAG_LM,"runPeriodic for %s returned", currFeature->getFeatureName().c_str());
            }
        }
    }
}
...
void LearningModule::lmMain() {
    ...
    /* register feature list with periodic feature runner */
    ...
    /* Create an algo thread which can call feature algo periodically */
    thread periodicAlgoThread = thread(&LearningModule::runPeriodic, this);
    ...
    if(periodicAlgoThread.joinable()) {
        runPeriodicThreadInterrupt = true;
        periodicAlgoThread.join();
    }
