...
int HciUartTransport::ClockOperation(int cmd)
{
  ...
  switch (cmd) {
    case USERIAL_OP_CLK_ON:
      ...
      BtState::Get()->AddLogTag(dst_buff, tv, (char *)"UART Clock On");
      BtState::Get()->SetUARTClockOn(dst_buff);
      vote_counter++;
      if(vote_counter == 1)
    ...
    case USERIAL_OP_CLK_OFF:
      gettimeofday(&tv, NULL);
      BtState::Get()->AddLogTag(dst_buff, tv, (char *)"UART Clock Off");
      BtState::Get()->SetUARTClockOff(dst_buff);
      vote_counter--;
      if(vote_counter < 0)
        vote_counter = 0;
...
int HciUartTransport::Ioctl(userial_vendor_ioctl_op_t op, int *p_data)
{
  ...
  switch (op) {
    ...
    case USERIAL_OP_FLOW_ON:
      ...
      gettimeofday(&tv, NULL);
      *p_data |= TIOCM_RTS;
      BtState::Get()->AddLogTag(dst_buff, tv, (char *)"UART Flow On");
      BtState::Get()->SetUARTFlowOn(dst_buff);
    ...
    case USERIAL_OP_FLOW_OFF:
      ...
      gettimeofday(&tv, NULL);
      BtState::Get()->AddLogTag(dst_buff, tv, (char *)"UART Flow Off");
      BtState::Get()->SetUARTFlowOff(dst_buff);
...
bool HciUartTransport::Init(BluetoothSocType soc_type, bool need_reload)
{
  ...
  char dst_buff[MAX_BUFF_SIZE];
  vote_counter = 0;
  tUSERIAL_CFG normal_cfg = userial_init_cfg;

  ALOGD("%s:> soc_type: %d, need_reload: %d", __func__, soc_type, need_reload);

  this->force_to_exit = false;
  ...
  switch (soc_type) {
    ...
    case BT_SOC_HASTINGS:
      if (!need_reload)
        /* Keep baudrate consistent with firmware, i.e. 3M or 3.2M */
        userial_init_cfg.baud = GetMaxBaudrate();
      InitTransport(&userial_init_cfg);
    break;

    case BT_SOC_CHEROKEE: {
      if (!need_reload) {
        normal_cfg.baud = GetMaxBaudrate();
        if (!InitTransport(&normal_cfg)) {
      ...
      gettimeofday(&tv, NULL);
      BtState::Get()->AddLogTag(dst_buff, tv, (char *)"Wakelock :: Acquire during transport init");
      BtState::Get()->SetWakeLockAcqTimeName(dst_buff);
      Wakelock :: Acquire();
#endif
...
#ifdef WAKE_LOCK_ENABLED
        gettimeofday(&tv, NULL);
        BtState::Get()->AddLogTag(dst_buff, tv, (char *)"Wakelock :: Release after hammer reset");
        BtState::Get()->SetWakeLockRelTimeName(dst_buff);
        Wakelock :: Release();
#endif
        break;
      }
...
#ifdef WAKE_LOCK_ENABLED
      gettimeofday(&tv, NULL);
      BtState::Get()->AddLogTag(dst_buff, tv, (char *)"Wakelock :: Release after clock off");
      BtState::Get()->SetWakeLockRelTimeName(dst_buff);
      Wakelock :: Release();
#endif
...
bool HciUartTransport::CleanUp(bool need_reload)
{
  int len, retval = 0;
  char shutdown_val = 0xC0;
  bool status = false;

  ALOGD("%s:> soc_type: %d, need_reload: %d", __func__, soc_type_, need_reload);
  ...
  health_info->CommitStatInfoBuff();
...
void HciUartTransport::Disconnect(bool need_reload) {
  if (ctrl_fd_ > 0) {
