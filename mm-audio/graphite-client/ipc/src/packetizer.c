        ...
        osal_list_for_each_entry_safe(struct pktzr_node, wnode, n, &ppriv->wait_list, list) {
            ...
            if (wnode->token == token) {
                ...
                if (wnode->cmd == GRAPHITE_CMD_DATA) {
                    ...
                    if (ret != OSAL_SUCCESS) {
                        OSAL_LOGE("%s: data_cmd_cb failed, ret = %d, token = %d\n",
                                  __func__, ret, token);
                        continue;
                    }
                } else {
                              ...
                              uint16_t cmd, bool use_island, bool sync_cmd)
{
    ...
    if (ret < 0) {
        ...
        /* Delete current node for all failures */
        osal_list_del(&wnode->list);
        osal_mem_free(wnode);

        /* Delete all other pending async commands in case of ENETRESET */
        if (ret == -ENETRESET) {
