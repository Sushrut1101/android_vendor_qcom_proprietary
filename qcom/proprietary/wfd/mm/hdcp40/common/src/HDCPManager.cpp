...
#define SINK_GENERIC_TRACK_ID 2
...
void* HDCPManager::m_hHDCPRptHandle = NULL;
void* HDCPManager::m_hHDCPTransportHandle = NULL;
...
// keeps track of propagation status from ARM
...
namespace {
    std::list<HDCPManager*> gHDCPManagerList;
...
*/
    ...
    static pthread_mutex_t sHDCPManagerLock =
...
#define LOCK_MUTEX do {\
        if(pthread_mutex_lock(&sHDCPManagerLock))\
        {\
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,\
            "Failed to acquire mutex due to %s", strerror(errno));\
        }\
    }while(0);


#define UNLOCK_MUTEX do {\
        if(pthread_mutex_unlock(&sHDCPManagerLock))\
        {\
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, \
            "Failed to acquire mutex due to %s", strerror(errno));\
        }\
...
#ifdef WFD_HDCP_ENABLED
    void WFD_ARMHDCP_Events_Receiver(ArmHdcpEvent* pEvtInfo, void* pvClientData)
    {
        if(!pEvtInfo)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                        "Invalid repeater callback args");
            return;
        }


        for(std::list<HDCPManager*>::iterator it = gHDCPManagerList.begin();
             it != gHDCPManagerList.end(); it++)
        {
            if(*it != NULL)
            {
                (*it)->HDCPEventCallBack((int)pEvtInfo->eventType,
                                         (void*)pEvtInfo->deviceHandle);
            }
        }
    }
#endif
...
HDCPManager* HDCPManager::CreateHDCPManager()
{
    LOCK_MUTEX

#ifdef WFD_HDCP_ENABLED
    if(gHDCPManagerList.empty())
    {

        unsigned long nStatus;

        nStatus = ARM_HDCP_Engine_Init("/vendor/etc/ArmHDCP_QTI_Android.cfg");

        if(nStatus)
        {
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
               "HDCPManager HDCP Engine Init Fail %u", nStatus);
        }
    }
#endif

    HDCPManager *inst = MM_New(HDCPManager);

    if(!inst)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
           "HDCPManager create fail");
    }
    else
    {
        gHDCPManagerList.push_front(inst);
    }
...
void HDCPManager::DestroyHDCPManager(HDCPManager* obj)
{
    if(!obj)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
           "HDCPManager bad object to destroy");
        return;
    }
    LOCK_MUTEX

    gHDCPManagerList.remove(obj);
...
bool HDCPManager::isHDCPAvailable()
{
#ifdef WFD_HDCP_ENABLED
    return true;
#else
    ...
    switch(m_eState)
    {
        ...
        case HDCP_STATE_INIT:
        {
            ...
            */
            ...
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                        "HDCPManager SignalQ Released, all threads and resources cleared");
...
)
{
    ...
    MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_HIGH,
                    "HDCPManager Setup HDCP Session %d %x %d",
                    eMode, nIpAddr, portNum);
    HDCPMAN_CRITICAL_SECT_ENTER
    ...
    */
    ...
    m_eMode = eMode;
    // Following logic blocks this thread(and RTSP) to wait for HDCP Listen
    // to complete
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,
                "createHDCPSession() waiting for HDCP listen");
    //Choice of counter and sleep values is arbitrary but needs to be
    //such that counter*sleep time < 5s (since this blocks RTSP thread)
    int counter = 80;
    while (counter >= 0 && m_eState == HDCP_STATE_CONNECTING)
    {
        HDCPMAN_CRITICAL_SECT_LEAVE
        MM_Timer_Sleep(50);
        HDCPMAN_CRITICAL_SECT_ENTER
...
)
{
    ...
    if(m_pDisconnectSignal)
    {
        ...
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
            "HDCPManager:teardownHDCPSession Arm Close");

        if(m_hHDCPRptHandle != NULL)
        {
            nStatus = ARM_HDCP_Repeater_Destroy(
                        (ArmHdcpDeviceHandle *)&m_hHDCPRptHandle);
            if(nStatus)
            {
               MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                  "HDCPManager:teardownHDCPSession ArmClose failed %lu",
                   nStatus);
            }
        }
            ...
            switch ( *pEvent )
            {
                ...
                case m_nHDCPManListenEvent:
                {
                   ...
                   //if(HDCP1X_COMM_hdmi_status())
                   {
                    ...
                    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                                                  "HDCPManager: Init ARM");

                    nStatus = ARM_HDCP_Transport_Create_Tcp(m_ipAddrArray,
                                 (uint16_t) m_nPortNum,
                                 ARM_HDCP_TRANSPORT_DIRECTION_UPSTREAM,
                              (ArmHdcpTransportHandle*) &m_hHDCPTransportHandle);

                    if(nStatus ||m_hHDCPTransportHandle == NULL)
                    {
                       MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                       "HDCPManager Transport Create Fail %u", nStatus);
                       m_eState = HDCP_STATE_ERROR;
                       break;
                    }

                    nStatus = ARM_HDCP_Repeater_Create(WFD_ARMHDCP_Events_Receiver,
                                        NULL, //pass NULL to clientData for now
                                       (ArmHdcpDeviceHandle *)&m_hHDCPRptHandle);

                    if(nStatus)
                    {
                        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                        "HDCPManager:ARM_HDCP_Repeater_Create failed %ld",nStatus);
                        m_eState = HDCP_STATE_ERROR;
                        break;
                    }
                    ...
                    {
                        /*In some use cases ARM Init return little late
                    ...
                    /*After ARM init, send a request to display/hdmi with Request Topology
                      ...
                      as ARM update topology will be done in CB
                    */
                    if(HDCP1X_COMM_hdmi_status())
                    {
                        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                    ...
                    bool bSuccess = false;
                     // do not call ARM API's
                    bool bContinueToARMCalls = false;
                    int nIteration = 0;
                    if(HDCP1X_COMM_hdmi_status())
                    {
                        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                    ...
                    {
                        bSuccess = true;
                        bContinueToARMCalls = true;
                    }

                    /* Warning!: In some use cases, DIsplay HDCP manager can give us
                       callback little late depending on when it authenticated the device
                       and so on. So we might have to wait here till
                    ...
                    while (!bSuccess) // Loop here till we get the CB from display
                    {
                        ...
                        // then it is set as part of Topology update with in ARM
                    ...
                    nStatus = ARM_HDCP_Repeater_Upstream_Listen(
                                     (ArmHdcpTransportHandle) m_hHDCPTransportHandle,
                                     (ArmHdcpDeviceHandle) m_hHDCPRptHandle);
                    if(nStatus)
                    {
                        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                                   "HDCPManager:Start Listen failed %ld",nStatus);
                        m_eState = HDCP_STATE_ERROR;
                        break;
                    }
                    else
                    {
                        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                               "HDCPManager:Start Listen SUCCESS");
                        m_eState = HDCP_STATE_LISTENING;
                    }

                    bool bAuthenticated = false;
                    bool bAuthenticatedRepeater = false;
                    while(m_eState == HDCP_STATE_LISTENING)
                    {

                        nStatus = ARM_HDCP_Repeater_Upstream_Get_Status(
                                          (ArmHdcpDeviceHandle)m_hHDCPRptHandle,
                                          &bAuthenticated,
                                          &bAuthenticatedRepeater);


                        if(nStatus)
                        {
                            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                                   "HDCPManager: Upstream status failed %ld",
                                    nStatus);
                        }
                        else
                        {
                            if(bAuthenticated || bAuthenticatedRepeater)
                            {
                                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                ...
                case m_nHDCPManDisconnectEvent:
                {
                   ...
                   nStatus = ARM_HDCP_Transport_Destroy(
                             (ArmHdcpTransportHandle*)  &m_hHDCPTransportHandle);
                    if(nStatus)
                    {
                       MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                       "HDCPManager Transport Destroy Fail %u", nStatus);
                    }
                   m_hHDCPTransportHandle = NULL;
...
/*==========================================================================
   ...
   Return Value : return 0 for success else Arm_HDCP error code
===========================================================================*/
                                   ...
                                   int nStreamId)
{
    ...
    /* After ARM Update topology, the topology information is propagated
       with in ARM and ARM notifies us with CIPHER_DISABLED and we should
    ...
    if( pIn && pOut && nLen )
    {
        ...
        EArmHdcpStreamMediaType lStream = ARM_HDCP_STREAM_MEDIA_TYPE_UNKNOWN;

        if (nStreamId == HDCP_AUDIO_TRACK_ID)
        {
            lStream = (EArmHdcpStreamMediaType)m_nAudioStreamType;
        }
        else if (nStreamId == HDCP_VIDEO_TRACK_ID)
        {
            lStream = (EArmHdcpStreamMediaType)m_nVideoStreamType;
        }
        ...
        ArmHdcpCipherTxtInfo sInBufInfo;
        sInBufInfo.bBypassCipher = nPesPvtDataLen ? false: true;
        sInBufInfo.handle = (uint64_t)pIn;
        sInBufInfo.type = ARM_HDCP_BUFFER_TYPE_SHARED_MEMORY_HANDLE;
        if(pesPrivateData && nPesPvtDataLen)
        {
            memcpy(sInBufInfo.PES_PrivateData, pesPrivateData,
                   nPesPvtDataLen);

        }

        ArmHdcpPlainTextInfo sOutBufInfo;

        sOutBufInfo.handle = (uint64_t)pOut;
        sOutBufInfo.type   = ARM_HDCP_BUFFER_TYPE_SHARED_MEMORY_HANDLE;
        sOutBufInfo.offset = nOutputOffset;


        {
#ifdef SYSTRACE_PROFILE
            ATRACE_NAME("Decrypt frame");
#endif
            nStatus = ARM_HDCP_Repeater_Decrypt(
                                    (ArmHdcpDeviceHandle)m_hHDCPRptHandle,
                                    lStream,
                                    nLen,
                                    &sInBufInfo,
...
//    nStatus = ARM_HDCP_Rpt_Set_Parameter((EArmHdcpConfigParam)paramID,
//    paramData);
...
void HDCPManager::HDCPEventCallBack(int status, void * deviceHandle)
{
    UNUSED(status);
    UNUSED(deviceHandle);
    if(deviceHandle != m_hHDCPRptHandle)
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                    "HDCPEventCb Not intended for this instance");
        return;
    }
#ifdef WFD_HDCP_ENABLED
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                  "HDCPManager: ARMCallBack Entry %x", status);
    switch(status)
    {

      case ARM_HDCP_PROCESS_EVENT_AUTHETICATION_START:
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                     "HDCPManager: ARMCallBack AUTHENTICATION START");
           break;
      case ARM_HDCP_PROCESS_EVENT_AUTHETICATION_SUCCESS:
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                     "HDCPManager: ARMCallBack AUTHENTICATION SUCCESS");
           break;
       case ARM_HDCP_PROCESS_EVENT_AUTHETICATION_FAILURE:
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                     "HDCPManager: ARMCallBack AUTHENTICATION FAILURE !!");
           break;
      case ARM_HDCP_STATUS_EVENT_CONNECTION_AUTHENTICATED:
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                     "HDCPManager: ARMCallBack CONNECTION AUTHENTICATED");
           break;
       case ARM_HDCP_STATUS_EVENT_CONNECTION_UNAUTHENTICATED:
           m_bPropagationStatus = true;
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                     "HDCPManager: ARMCallBack CONNECTION UNAUTHENTICATED");
           break;
       case ARM_HDCP_ERROR_EVENT_INTERNAL:
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                     "HDCPManager: ARMCallBack ERROR !!");
           break;
       case ARM_HDCP_CONNECTIVITY_EVENT_UPSTREAM_LISTEN_READY:
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                     "HDCPManager: ARMCallBack UPSTREAM LISTEN READY");
           break;
       case ARM_HDCP_STATUS_EVENT_SESSION_CIPHER_ENABLED:
           m_bPropagationStatus = false;
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                     "HDCPManager: ARMCallBack SESSION CIPHER ENABLED");
           break;
       case ARM_HDCP_STATUS_EVENT_SESSION_CIPHER_DISABLED:
           m_bPropagationStatus = true;
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                     "HDCPManager: ARMCallBack SESSION CIPHER DISABLED");
           break;
       default:
           MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                     "HDCPManager: ARMCallBack Default Reached");
           break;
    }
#endif
}
        ...
        case EV_SEND_TOPOLOGY:
           {
                ...
                std::list<HDCPManager*>::iterator it;

                for(it = gHDCPManagerList.begin();
                     it != gHDCPManagerList.end(); it++)
                {
                    if(*it != NULL)
                        (*it)->hdcp_update_topology(param);
                }
           }
           break;
...
/*==========================================================================
   ...
   DESCRIPTION  : Updates the topology information from Display to ARM
...
void HDCPManager::hdcp_update_topology(void *msg1)
{
    ...
    MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_HIGH,"dev_count = %d depth = %d "
    "max_cascade_exceeded = %d max_dev_exceeded = %d", msg->dev_count,
    msg->depth, msg->max_cascade_exceeded, msg->max_dev_exceeded);
    ...
    {
        m_bPropagationStatus = true;
        /* Update the ARM Topology */
        unsigned long nStatus = ARM_HDCP_Common_Update_Converters_HDCP1_Topology(
                                 (ArmHdcpReceiverId_t *)&msg->ksv_list[0],
                                  (uint32_t) msg->dev_count,
                                  (uint32_t) msg->depth,
...
/*==========================================================================
   ...
   DESCRIPTION  : Constuct the codec+stream information,
                  which is needed by ARM Decrypt API
   ...
   Return Value : bool
===========================================================================*/
void HDCPManager::constructCodecAndStreamType(int nStream, HDCPCodecType eCodec)
{
#ifdef WFD_HDCP_ENABLED
    HDCPMAN_CRITICAL_SECT_ENTER

    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                "yx HDCPManager::constructCodecAndStreamType [%d] -> %d",
                nStream,eCodec);

    if (nStream == HDCP_AUDIO_TRACK_ID) // Audio Stream
    {
        // then its a LPCM
        m_nAudioStreamType = ARM_HDCP_STREAM_MEDIA_TYPE_AUDIO;
        m_bAudioCodecInfoAvailable = true;
    }
    else if (nStream == HDCP_VIDEO_TRACK_ID) // Video stream
    {
        m_nVideoStreamType = ARM_HDCP_STREAM_MEDIA_TYPE_VIDEO;
