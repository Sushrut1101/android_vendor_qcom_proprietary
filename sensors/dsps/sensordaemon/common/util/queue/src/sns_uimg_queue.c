...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 1990-2009,2014-2015,2018 Qualcomm Technologies, Inc.  All Rights Reserved.
...
RETURN VALUE
  A pointer to the initialized queue or NULL if invalid argument passed.
...
RETURN VALUE
  A pointer to the initialized link or  NULL if link pointer passed as NULL.
...
RETURN VALUE
  A pointer to the next item on the queue if queue is not NULL. If the end of the queue is reached,
...
{
   if( q_delete_ptr        != NULL )
   {
      if( q_delete_ptr->q_ptr != NULL )
      {
