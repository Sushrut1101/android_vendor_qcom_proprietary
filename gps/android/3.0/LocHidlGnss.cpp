...
LocHidlGnss::LocHidlGnss() {
    ENTRY_LOG();
#ifdef GNSS_HIDL_VERSION
    IGnss* gnss = getIGnssFromLib("android.hardware.gnss@" GNSS_HIDL_VERSION "-impl-qti.so");
    mGnss = gnss;
#else
    ALOGE("GNSS_HIDL_VERSION not defined!");
#endif
}
...
// Methods from ::vendor::qti::gnss::V2_1::ILocHidlGnss follow.
Return<sp<V3_0::ILocHidlIzatConfig>> LocHidlGnss::getExtensionLocHidlIzatConfig() {
    FROM_HIDL_CLIENT();
    if (mIzatConfig == nullptr) {
        mIzatConfig = new LocHidlIzatConfig();
    }
    return mIzatConfig;
}
