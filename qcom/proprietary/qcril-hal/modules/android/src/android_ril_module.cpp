...
/* If property is "1", then enable some additional logging. */
...
std::recursive_mutex outstandingReqsLock;
std::unordered_set<qcril_event_context *> outstandingReqs;
...
void qcril_debug_logger(std::string msg) {
    QCRIL_LOG_DEBUG("%s", msg.c_str());
}

void qcril_deadlock_logger(std::string msg) {
    QCRIL_LOG_FATAL("%s", msg.c_str());
}

class MutexLoggerInit {
	public:
    MutexLoggerInit() {
        unsigned long res;
        char *end_ptr = NULL;
        int qcril_mutex_logging_enabled = 0;
        char property_value[PROPERTY_VALUE_MAX];
        memset(property_value, 0, sizeof(property_value));
        property_get( QCRIL_LOG_ADDITIONAL_LOG_ON, property_value, "0" );
        int temp_len = strlen( property_value );
        if ( temp_len > 0 )
        {
            res = strtoul( property_value, &end_ptr, 0 );
            if (((errno == ERANGE) &&
                 (res == ULONG_MAX)) ||
                  *end_ptr)
            {
                qcril_mutex_logging_enabled = QMI_RIL_ZERO;
            }
            else
            {
                qcril_mutex_logging_enabled = (int) res;
            }
        }
        QCRIL_LOG_DEBUG("enable mutex logging - %d", qcril_mutex_logging_enabled);
        if (qcril_mutex_logging_enabled) {
            QtiMutexDefaults::getInstance().setDebugFlag(true);
        } else {
            QtiMutexDefaults::getInstance().setDebugFlag(false);
        }
        QtiMutexDefaults::getInstance().setDefaultDebugLogger(qcril_debug_logger);
        QtiMutexDefaults::getInstance().setDefaultDeadlockLogger(qcril_deadlock_logger);
    }
};

static MutexLoggerInit log_init;
...
bool android_ril_module::dispatch_request(qcril_instance_id_e_type instance_id, int req_id, void* data, size_t size, RIL_Token t)
{
    ...
    if (!ctx) {
        QCRIL_LOG_ESSENTIAL("Unable to allocate context");
        android_ril_on_request_complete(instance_id, t, RIL_E_NO_MEMORY, NULL, 0);
    } else {
        QCRIL_LOG_DEBUG("Allocated context for request %d. ctx: %p", req_id, ctx);
        std::lock_guard<std::recursive_mutex> lock(outstandingReqsLock);
        outstandingReqs.insert(ctx);
    }
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                android_ril_on_request_complete(instance_id, t, RIL_E_REQUEST_NOT_SUPPORTED, NULL, 0);
                delete ctx;
                ret = false;
            }
...
void android_ril_module::send_response(qcril_instance_id_e_type instance_id, RIL_Token token_id, RIL_Errno e, void *response, size_t response_len)
{
    ...
    if (ctx)
    {
        ...
        {
            std::lock_guard<std::recursive_mutex> lock(outstandingReqsLock);
            valid = (outstandingReqs.count(ctx) > 0);
            if (valid) {
                outstandingReqs.erase(ctx);
            }
        }
        ...
        if (!valid) {
            QCRIL_LOG_ESSENTIAL("Trying to send response for invalid request. ctx: %p. Ignoring", ctx);
            QCRIL_LOG_FUNC_RETURN();
            return;
        }
