...
public class WigigNative {
    ...
    public boolean startSensing() {
        ...
        // mark mSuppTunnelHal to run only sensing daemon
        ret = mSuppTunnelHal.setConfigOption("sensing_daemon_only", "1");
        if (!ret) {
             Log.e(mTAG, "sensing_daemon_only config not supported");
             return ret;
        }

        try {
             ret = mSuppTunnelHal.startSupplicant();
        } finally {
            mSuppTunnelHal.setConfigOption("sensing_daemon_only", "0");
        }

        return ret;
    }

    public boolean killSensing() {
        boolean ret;

        // mark mSuppTunnelHal to kill only sensing daemon
        ret = mSuppTunnelHal.setConfigOption("sensing_daemon_only", "1");
        if (!ret) {
             Log.e(mTAG, "sensing_daemon_only config not supported");
             return ret;
        ...
        try {
             ret = mSuppTunnelHal.killSupplicant();
        } finally {
            mSuppTunnelHal.setConfigOption("sensing_daemon_only", "0");
        }
     ...
     */
    public boolean roamToNetwork(String bssid) {
        return doBooleanCommand("ROAM " + bssid);
    }
    ...
    public boolean setSoftAp(String ifname, String ssid, boolean hidden, int channel,
                             int edmgChannel, boolean secured, String key, int maxNumSta,
                             String countryCode) {
        ...
        if (edmgChannel != 0 &&
            (!mSuppTunnelHal.setHostapdConfigOption("enable_edmg", "1") ||
            !mSuppTunnelHal.setHostapdConfigOption("edmg_channel", Integer.toString(edmgChannel)))) {
            Log.e(TAG, "failed to set hostapd EDMG config options");
            return false;
        }
    ...
    public boolean setIPAOffload(boolean enable) {
        String val = Integer.toString(enable ? 1 : 0);
        if (!mSuppTunnelHal.setHostapdConfigOption("ipa_offload", val)) {
            Log.e(TAG, "failed to set ipa_offload to " + val);
            return false;
        }
    ...
    public boolean setSensingOverSPI(boolean enable) {
        String val = Integer.toString(enable ? 1 : 0);
        if (!mSuppTunnelHal.setConfigOption("sensing_over_spi", val)) {
            Log.e(TAG, "failed to set sensing_over_spi to " + val);
            return false;
        }
