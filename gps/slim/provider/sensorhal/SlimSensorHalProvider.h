...
#include <fstream>
#include <vector>
void SearchInPath(char* parentDir,  char* subFileRead, std::string contentVerified, int maxLevel,char* outputPath);
void parseBMIBufferData(slimServiceEnumT aeSensorType,slimSensorSampleStructT *azEventBuffer);
void parseIAMBufferData(slimServiceEnumT aeSensorType,slimSensorSampleStructT *azEventBuffer);
namespace slim
{
...
{
  TIME_BASED_BATCHING,
  FIXED_COUNT_BASED_BATCHING,
  VARIABLE_COUNT_BASED_BATCHING
} eBatchCfgType;
...
public:
  ...
  int readTempIAM();
  int readTempSMI(void);
  void bufferDataScaling( slimServiceEnumT aeSensorType, slimSensorSampleStructT *zEventBuffer );
 /**
