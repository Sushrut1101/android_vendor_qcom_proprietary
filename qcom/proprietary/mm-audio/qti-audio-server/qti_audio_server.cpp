...
int32_t qti_audio_server::onTransact(uint32_t code,
    ...
    switch(code) {
        ...
        case LOAD_HAL: {
            ...
            qti_audio_server_add_hal_handle((void *)hal_handle, HAL_HANDLE_TYPE_AHAL);
        ...
        case OPEN_OUTPUT_STREAM: {
            ...
            qti_audio_server_add_io_stream_handle((void *)n_stream, IO_HANDLE_TYPE_PLAYBACK);
            return rc;
        }
        break;
        ...
        case CLOSE_OUTPUT_STREAM: {
            ...
            if (n_stream != NULL) {
                ...
                qti_audio_server_remove_io_stream_handle((void *)n_stream, IO_HANDLE_TYPE_PLAYBACK);
                n_stream->sh_mem_handle.clear();
                free(n_stream);
        ...
        case OPEN_INPUT_STREAM: {
            ...
            qti_audio_server_add_io_stream_handle((void *)n_stream, IO_HANDLE_TYPE_RECORD);
            return rc;
        }
        break;
        ...
        case CLOSE_INPUT_STREAM: {
            ...
            if (n_stream != NULL) {
                ...
                qti_audio_server_remove_io_stream_handle((void *)n_stream, IO_HANDLE_TYPE_RECORD);
                n_stream->sh_mem_handle.clear();
                free(n_stream);
        ...
        case PREPARE_FOR_WRITE: {
            ...
            ALOGV("shmem_data buff header(%p) buff(%p)buff size %zd",
                        ...
                        sizeof(sh_mem_data));
            rc = prepare_for_write((qahw_stream_handle_t *)n_stream);
            reply->writeInt32(rc);
        }
        break;
        ...
        case PREPARE_FOR_READ:
        {
            ...
            ALOGV("shmem_data buff header(%p) buff(%p)buff size %zd",
                        ...
                        n_stream->shmem_data_buff,
                        sizeof(sh_mem_data));
            rc = prepare_for_read((qahw_stream_handle_t *)n_stream);
            reply->writeInt32(rc);
        }
        break;
        ...
        case GET_PARAM_DATA: {
            ...
            uint32_t blob_size;
            data.readUint32(&blob_size);
            status_t status = data.readBlob(blob_size, &r_blob);
            if (status) {
                ALOGE("%s: Error in readblob, status %d", __func__, status);
                return -EINVAL;
            }
            memset(&param_payload, 0x0, sizeof(qahw_param_payload));
            memscpy(&param_payload, sizeof(qahw_param_payload), r_blob.data(), blob_size);
            r_blob.release();
        ...
        switch(shmem_data->cmd) {
            case WRITE:
            {
                ...
                *return_value = rc;
                ...
                ALOGV("%s:after cond signal", __func__);
                break;
            }
            case GET_PRESENTATION_TIME:
            {
                ...
                //make sure proxy side waits for server to wait on c_mutex
                pthread_cond_signal(&shmem_data->c_cond);
                ALOGD("%s:GET_PRESENTATION_TIME: after cond signal", __func__);
                break;
            }
            case GET_LATENCY:
            {
                ...
                *latency = qahw_out_get_latency_l(
                ...
                //make sure proxy side waits for server to wait on c_mutex
                pthread_cond_signal(&shmem_data->c_cond);
                ALOGV("%s:GET_LATENCY: after cond signal", __func__);
                break;
            }
            case STOP:
            {
                ALOGD("%s: stop cmd", __func__);
                shmem_data->status = 0;
                pthread_cond_signal(&shmem_data->c_cond);
...
int qti_audio_server::prepare_for_write(qahw_stream_handle_t *stream,
    ...
    ALOGD("create Write Stream Thread ret %d", rc);
    if (rc)
...
ssize_t qti_audio_server::qahw_in_stop(qahw_stream_handle_t *in_handle __unused) {
//dummy funtion
    return -ENOSYS;
}
