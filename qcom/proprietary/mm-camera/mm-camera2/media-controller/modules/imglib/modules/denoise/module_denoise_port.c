  ...
  if (port && event && port->port_private
    && event->u.module_event.module_event_data) {
      ...
      if (sizeof(session_meta->isp_stats_awb_data.private_data)
        ...
        CDBG_ERROR("session_meta->isp_stats_awb_data.private_data size %zu\n",
          sizeof(session_meta->isp_stats_awb_data.private_data));
        CDBG_ERROR("awb_update_t size %zu\n", sizeof(awb_update_t));
      }
  } else {
    CDBG_ERROR("Null pointer detected in %s\n", __func__);
  }
  ...
  if (port && event && port->port_private
    && event->u.module_event.module_event_data) {
      ...
      if (sizeof(session_meta->isp_gamma_data.private_data)
        ...
        CDBG_ERROR("%s: Private data size need to be increased\n", __func__);
        CDBG_ERROR("session_meta->isp_gamma_data.private_data size %zu\n",
  ...
  img_frame_t* img_frame, isp_buf_divert_t *buf_divert, mct_stream_info_t* info)
{
  ...
  if ((CAM_FORMAT_YUV_420_NV21 == info->fmt) ||
    (CAM_FORMAT_YUV_422_NV61 == info->fmt)) {
      ...
      img_frame->frame[0].plane_cnt = (uint32_t)info->buf_planes.plane_info.num_planes;
      ...
      for (i=0; i<img_frame->frame[0].plane_cnt; i++) {
        ...
        if (i)
          ptr += img_frame->frame[0].plane[1].stride * (uint32_t)info->dim.height;
        img_frame->frame[0].plane[i].addr = ptr;
      }

      img_frame->timestamp =
        (uint64_t)buf_divert->buffer.timestamp.tv_sec * 1000000
 ...
 **/
static boolean __unused module_denoise_port_release_native_buffer(
  ...
  module_denoise_buf_t *buff, uint32_t identity, boolean buff_done)
{
  ...
  if (buff && identity) {
    if (buff->img_frame) {
      buff_info.index = (uint32_t)buff->img_frame->idx;
      ...
      buff_info.timestamp.tv_sec = (__kernel_time_t)buff->img_frame->timestamp / 1000000;
      buff_info.timestamp.tv_usec = (__kernel_time_t)buff->img_frame->timestamp
        - (__kernel_time_t)buff_info.timestamp.tv_sec * 100000;
      if (buff_done)
  ...
  mct_event_t *event, boolean *forward_event)
{
  ...
  metadata_buffer_t* cam_metadata_info;
          ...
          if (stream) {
            ...
            if (denoise2d->denoise_enable) {
              ...
              private->in_buff = module_denoise_port_get_input_buffer(port,
                event, info);
              if (private->in_buff) {
                private->out_buff = module_denoise_port_get_buffer(event, info);
                if (private->out_buff) {
                  cam_metadata_info =
                    module_denoise_port_get_metadata_buffer(port, event, info);
                  privateData = POINTER_OF_META(CAM_INTF_META_PRIVATE_DATA, cam_metadata_info);
                  if (cam_metadata_info && privateData) {
  ...
  module_denoise_buf_t *buff)
{
  ...
  if (buff && port && port->port_private && MCT_PORT_PARENT(port)) {
      ...
      isp_buf_divert.buffer.index = (uint32_t)buff->img_frame->idx;
  ...
  mct_event_t *event, boolean *forward_event)
{
  ...
  if (port && event) {
    ...
    ret_val = event_handler ? event_handler(port, event, forward_event) : TRUE;
  } else {
    CDBG_ERROR("Null pointer detected in %s\n", __func__);
  }
