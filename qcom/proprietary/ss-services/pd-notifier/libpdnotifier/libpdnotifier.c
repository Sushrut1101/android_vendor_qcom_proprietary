...
#define PD_NOTIFIER_DEBUG_PROP "persist.vendor.pd_locater_debug"
...
static qmi_servreg_notif_set_ack_resp_msg_v01 dummy_resp = {0};
		...
		void *hndl)
{
	...
	retVal = qmi_client_send_msg_async(user_handle,
			...
			&ack,
			sizeof(ack),
                        /*dummy_resp is written to but the value isn't read/used
                          for any purpose and might be overwritten if there are
                          multiple instances which is fine.*/
			&dummy_resp,
			sizeof(dummy_resp),
			null_cb,
                        NULL,
