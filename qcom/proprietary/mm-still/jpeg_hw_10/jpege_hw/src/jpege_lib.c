...
#define MSM_JPEGE_NAME_0 "/dev/jpeg0"
#define MSM_JPEGE_NAME_1 "/dev/jpeg1"

#define JPEG_CLK_RATE_NORMAL (266670000)
#define JPEG_CLK_RATE_HIGH (320000000)

#define JPEG_CLK_RATE_NORMAL_8994 (320000000)
#define JPEG_CLK_RATE_HIGH_8994 (465000000)

#define JPEG_CLK_RATE_NORMAL_8996 (320000000)
#define JPEG_CLK_RATE_HIGH_8996 (480000000)

#define JPEG_CLK_RATE_NORMAL_8998 (480000000)
#define JPEG_CLK_RATE_HIGH_8998 (480000000)

#define SOC_ID_QM215 386
#define SOC_ID_QM2150 436

#ifdef TARGET_ENABLE_INSTRUMENTATION
...
/**
 * Function: jpege_lib_set_userdata
 *
 * Description: This method is to set the userdata
 *
 * Input parameters:
 *   jpege_hw_obj: hardware object got from lib init
 ...
 * Notes: none
 **/
int jpege_lib_set_userdata(jpege_hw_obj_t *jpege_hw_obj, void *p_userdata)
{
  if (jpege_hw_obj != NULL && *jpege_hw_obj != NULL && p_userdata != NULL) {
    ((__jpege_hw_obj_t * ) *jpege_hw_obj)->p_userdata = p_userdata;
    return 0;
  }
  return -1;
}
  ...
  int (*input_handler) (jpege_hw_obj_t, struct jpege_hw_buf *))
{
  ...
  property_get("persist.vendor.camera.mmstill.logs", prop, "1");
  ...
  property_get("vendor.debug.camera.jpegencoder", value, "-1");
  jpegeName = atoi(value);
#endif

  if (jpegeName != -1) {
    /* Select encoder based on setprop */
    jpege_hw_obj_p->msm_jpege_name =
      (jpegeName == 1)? MSM_JPEGE_NAME_1 : MSM_JPEGE_NAME_0;

    jpegefd = open (jpege_hw_obj_p->msm_jpege_name, O_RDWR);
    JPEG_HW_DBG("open %s: fd = %d", jpege_hw_obj_p->msm_jpege_name, jpegefd);
    if (jpegefd < 0) {
      JPEG_HW_PR_ERR ("Cannot open %s", jpege_hw_obj_p->msm_jpege_name);
      goto jpege_init_err;
    }
  } else {
    /* Attempt to open jpeg0 first and if fails then open jpeg1*/
    for (p = jpeg_devs; *p; p++ ) {
      jpegefd = open (*p, O_RDWR);
      jpege_hw_obj_p->msm_jpege_name = *p;
      JPEG_HW_DBG("open %s: fd = %d", jpege_hw_obj_p->msm_jpege_name, jpegefd);
      if (jpegefd < 0) {
        JPEG_HW_PR_ERR ("Cannot open %s", jpege_hw_obj_p->msm_jpege_name);
      } else {
  ...
  if (fd >= 0) {
    if (read(fd, buf, sizeof(buf) - 1) == -1) {
      JPEG_HW_PR_ERR("Unable to read soc_id. Jpeg will use default freq");
    } else {
      jpege_hw_obj_p->soc_id = atoi(buf);
    }
    close(fd);
  }
...
int jpege_lib_release(jpege_hw_obj_t jpege_hw_obj)
{
  ...
  JPEG_HW_DBG("%s:%d] closed %s", __func__, __LINE__, jpege_hw_obj_p->msm_jpege_name);
  ...
  jpege_cmd_scale_cfg *p_scale_cfg)
{
  ...
  if (IS_8974_V1(version))
    return 0;

  if (p_scale_cfg->scale_enable) {
    downscale_ratio = (float)(p_scale_cfg->scale_input_width /
 ...
 *
 **/
uint32_t jpege_get_clk_rate(uint32_t version,
  uint32_t width __unused,
  uint32_t height __unused,
  JPEG_SPEED_MODE_T speed_mode,
  JPEGE_HW_INPUT_FORMAT format __unused)
{
  uint32_t clk_rate;
  /*ToDo: update clock rate based on dimensions */
  /* get clock rates from DTSI */
  /* current logic is to take userinput and set the mode*/
  if (IS_8994(version)) {
    clk_rate = (speed_mode == JPEG_SPEED_NORMAL) ?
      JPEG_CLK_RATE_NORMAL_8994 :
      JPEG_CLK_RATE_HIGH_8994;
  } else if (IS_8996(version)) {
    clk_rate = (speed_mode == JPEG_SPEED_NORMAL) ?
      JPEG_CLK_RATE_NORMAL_8996 :
      JPEG_CLK_RATE_HIGH_8996;
  } else if(IS_8998(version)) {
     clk_rate = (speed_mode == JPEG_SPEED_NORMAL) ?
      JPEG_CLK_RATE_NORMAL_8998 :
      JPEG_CLK_RATE_HIGH_8998;
  } else {
    clk_rate = (speed_mode == JPEG_SPEED_NORMAL) ?
      JPEG_CLK_RATE_NORMAL :
      JPEG_CLK_RATE_HIGH;
  }
  return clk_rate;
}
  ...
  jpege_cmd_scale_cfg *p_scale_cfg)
{
  ...
  p_hw_cmds = jpege_lib_hw_fe_cfg(p_input_cfg, 0, jpege_hw_obj_p->version);
  if (!p_hw_cmds)
    goto fail;
  result = ioctl (jpegefd, MSM_JPEG_IOCTL_HW_CMDS, p_hw_cmds);
  free (p_hw_cmds);
  ...
  p_hw_cmds = jpege_lib_hw_we_cfg (jpege_hw_obj_p->version);
  if (!p_hw_cmds)
    goto fail;
  result = ioctl (jpegefd, MSM_JPEG_IOCTL_HW_CMDS, p_hw_cmds);
  free (p_hw_cmds);
 ...
 if ((jpege_hw_obj_p->soc_id == SOC_ID_QM215 || jpege_hw_obj_p->soc_id == SOC_ID_QM2150)) {
    property_get("vendor.camera.jpeg.set.clock", value, "266");
    clk_rate = atoi(value);
  } else {
    property_get("vendor.camera.jpeg.set.clock", value, "0");
    clk_rate = atoi(value);
  }

  if (p_encode_cfg->speed_mode == JPEG_SPEED_HIGH || clk_rate) {
    if (clk_rate > 0) {
      jpeg_clk = clk_rate * 1000000;
    } else {
      jpeg_clk = jpege_get_clk_rate(jpege_hw_obj_p->version, 0, 0,
        p_encode_cfg->speed_mode,
        JPEGE_INPUT_H2V2);
    }
...
int jpege_lib_encode (jpege_hw_obj_t jpege_hw_obj)
{
  ...
  if (p_hw_cmds) {
    ...
    JPEG_HW_DBG("ioctl %s: rc = %d", jpege_hw_obj_p->msm_jpege_name, result);
    free (p_hw_cmds);
  }
