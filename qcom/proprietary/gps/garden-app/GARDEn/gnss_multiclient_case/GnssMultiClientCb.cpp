...
void GnssMultiClientCb::gnssLocationCb(Location& location) {
    ...
    CONDPRINTLN("Client: %p (gnssLocationCb) - LAT: %f, LON: %f, ALT: %f ACC: %f, TIME: %llu",
             mClient, location.latitude, location.longitude, location.altitude,
    ...
    if (mGnssCase && mGnssCase->hasFlag(GNSS_CASE_SESSION) &&
        ...
        lock_guard lock(mGnssCase->mLock);
