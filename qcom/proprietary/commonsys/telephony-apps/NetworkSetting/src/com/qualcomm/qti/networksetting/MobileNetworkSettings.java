...
public class MobileNetworkSettings extends Activity  {
    ...
    private static boolean VDBG = Log.isLoggable(LOG_TAG, Log.VERBOSE);
    ...
    /**
     * An int specifying an invalid slot index
     */
    private static final int INVALID_SLOT_INDEX = -1;

    /**
     * The slot ID retrieved from the activity's intent extra {@link EXTRA_SLOT_ID}.
     */
    private static int mSimSlotIndexFromIntent;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mSimSlotIndexFromIntent = intent != null
                ? intent.getIntExtra(EXTRA_SLOT_ID, INVALID_SLOT_INDEX)
                : INVALID_SLOT_INDEX;

        Log.d(LOG_TAG, "mSimSlotIndexFromIntent: " + mSimSlotIndexFromIntent);

        int numOfSlots = ((TelephonyManager) this
                .getSystemService(Context.TELEPHONY_SERVICE))
                .getPhoneCount();

        if (mSimSlotIndexFromIntent < 0 || mSimSlotIndexFromIntent >= numOfSlots) {
            mSimSlotIndexFromIntent = 0;
        }
    ...
    public static class MobileNetworkFragment extends PreferenceFragment implements
        ...
        private static final String BUTTON_5G_SWITCH_KEY             = "5g_switch";
        ...
        private static final String PRIMARY_4G_CARD_PROPERTY_NAME =
                "persist.vendor.radio.detect4gcard";
        ...
        private static final String HIDE_PREFERRED_NW_MODE_PROPERTY_NAME =
                "persist.vendor.radio.hideprefernwmode";
        private static final String HIDE_ENHANCE_4G_LTE_PROPERTY_NAME =
                "persist.vendor.radio.hideenhanced4glte";
        private static final String SHOW_5G_SWITCH_PROPERTY_NAME =
                "persist.vendor.radio.show5gswitch";
        //Carrier mode
        ...
        private static final boolean PRIMCARYCARD_L_W_ENABLED =
                SystemProperties.getBoolean("persist.vendor.radio.lw_enabled", false);

        private final BroadcastReceiver mPhoneChangeReceiver = new PhoneChangeReceiver();

        private int mPreferredNetworkMode = Phone.PREFERRED_NT_MODE;
        private int mCachePreferredNetworkMode = Phone.PREFERRED_NT_MODE;
        ...
        private SubscriptionManager mSubscriptionMgrforPhone;
        ...
        private SwitchPreference mButton5g;
        ...
        public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
            ...
            if (preference.getKey().equals(BUTTON_4G_LTE_KEY)) {
                return true;
            } else if (preference.getKey().equals(BUTTON_5G_SWITCH_KEY)) {
                return true;
                        ...
                        for(SubscriptionInfo newSi : newSil) {
                            ...
                            if (si.getSimSlotIndex() == newSi.getSimSlotIndex()) {
                                if (DBG) log("onSubscriptionsChanged: Slot matched SimSlotIndex: "
                                        + si.getSimSlotIndex());
                                if (newSi.getSubscriptionId() != si.getSubscriptionId() ||
                                        !newSi.getDisplayName().equals(si.getDisplayName()) ||
                                        !newSi.getCarrierName().equals(si.getCarrierName())) {
            ...
            switch (state) {
                case UPDATE: {
                    ...
                    if (mTabHost != null) {
                        ...
                        log("mTabHost is not null, using existing tab: " + currentTab);
                    } else {
                        currentTab = mSimSlotIndexFromIntent;
                        log("mTabHost is null, using tab from intent: " + currentTab);
                    }
        ...
        private void updatePhone(int slotId) {
            ...
            mPreferredNetworkMode = getPreferredNetworkModeForPhoneId();
            logi("updatePhone:- slotId=" + slotId);
            mSubscriptionMgrforPhone = (SubscriptionManager) (mPhone.getContext())
                    .getSystemService(Context.TELEPHONY_SUBSCRIPTION_SERVICE);
            mImsMgr = ImsManager.getInstance(mPhone.getContext(), mPhone.getPhoneId());
            if (mImsMgr == null) {
                logi("updatePhone :: Could not get ImsManager instance!");
            } else if (DBG) {
                logi("updatePhone :: mImsMgr=" + mImsMgr);
            }
        }
        ...
        public void onCreate(Bundle icicle) {
            ...
            if (activity == null || activity.isDestroyed()) {
                loge("onCreate:- with no valid activity.");
                return;
            }
            ...
            mButton5g = (SwitchPreference)findPreference(BUTTON_5G_SWITCH_KEY);
            mButton5g.setOnPreferenceChangeListener(this);
            ...
            if (getResources().getBoolean(R.bool.config_enabled_4g)) {
            ...
            mPhoneStateListener =
                    new PhoneStateListener[mTelephonyManager.getPhoneCount()];
        ...
        private boolean isCmccCardBySubId (int subId) {
            SubscriptionInfo subInfo = mSubscriptionMgrforPhone.getActiveSubscriptionInfo(subId);
            boolean isCmccCard = false;
            if (subInfo != null ) {
        ...
        private int getNetworkTypeForEnable4gChange() {
            int networkType = Phone.PREFERRED_NT_MODE;
            if (isCtModeAndCtCard()) {
                networkType = mButtonEnable4g.isChecked()
        ...
        private void handleEnable4gChange() {
            int networkType = getNetworkTypeForEnable4gChange();
            //Disable the option until response is received
            mButtonEnable4g.setEnabled(false);

            log("Enable 4G option: isCtCard - " + isCtModeAndCtCard() + ", set networkType - " +
                    networkType);
            ...
            UpdatePreferredNetworkModeSummary(getNetworkTypeForEnable4gChange());
            UpdateEnabledNetworksValueAndSummary(getNetworkTypeForEnable4gChange());
        }

        private boolean isNwModeLte() {
            //if present nwMode is LTE return true.
            int type = getPreferredNetworkModeForSubId();
            if (type == TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA
                    || type == TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_GSM_WCDMA
                    || type == TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_WCDMA
                    || type == TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_GSM
                    || type == TelephonyManager.NETWORK_MODE_LTE_TDSCDMA
                    || type == TelephonyManager.NETWORK_MODE_LTE_WCDMA
                    || type == TelephonyManager.NETWORK_MODE_LTE_ONLY
                    || type == TelephonyManager.NETWORK_MODE_LTE_CDMA_EVDO_GSM_WCDMA
                    || type == TelephonyManager.NETWORK_MODE_LTE_GSM_WCDMA
                    || type == TelephonyManager.NETWORK_MODE_LTE_CDMA_EVDO
                    ) {
                return true;
            }
            return false;
        ...
        private void updateButtonEnable4g() {
            ...
            int simState = mTelephonyManager.getSimState(mPhone.getPhoneId());
            ...
            List<SubscriptionInfo> sirList = mSubscriptionMgrforPhone
        ...
        public void onResume() {
            ...
            if (mUnavailable) {
                logi("onResume:- ignore mUnavailable == false");
                return;
            }
            final Activity activity = getActivity();
            if (activity == null || activity.isDestroyed()) {
                loge("onResume:- with no valid activity.");
                return;
            }
            final int currentTab = mTabHost != null
                    ? mTabHost.getCurrentTab()
                    : mSimSlotIndexFromIntent;
            updatePhone(currentTab);

            final PersistableBundle carrierConfig = mConfigManager
                    .getConfigForSubId(mPhone.getSubId());

            if (VDBG) {
                log("onResume: carrierConfig for subId: " + mPhone.getSubId()
                        + ", phoneId: " + mPhone.getPhoneId());
                dumpConfig(carrierConfig);
            }
            ...
            if ((getPreferenceScreen().findPreference(BUTTON_PREFERED_NETWORK_MODE) != null
                    && mButtonPreferredNetworkMode.isEnabled())
                    || (getPreferenceScreen().findPreference(BUTTON_ENABLED_NETWORKS_KEY)
                    != null && mButtonEnabledNetworks.isEnabled()))  {
            ...
            context.registerReceiver(mCarrierConfigReceiver, new IntentFilter(
            ...
            PreferenceScreen prefeSet = getPreferenceScreen();
            if (prefeSet != null) {
                if (phoneType == PhoneConstants.PHONE_TYPE_GSM
                        || (carrierConfig != null && carrierConfig
                        .getBoolean(CarrierConfigManager.KEY_WORLD_PHONE_BOOL) == true)) {
                    updateGsmUmtsOptions(this, prefeSet, mPhone.getSubId(), mNetworkQueryService);
                }
            } else {
                loge("onresume with no null prefeSet.");
            }
        ...
        private boolean isEnhanced4gLteEnabled() {
            return mImsMgr.isEnhanced4gLteModeSettingEnabledByUser()
        ...
        private void updateBodyBasicFields(Activity activity, PreferenceScreen prefSet,
            ...
            log("updateBodyBasicFields, phoneSubId: " + phoneSubId
        ...
        private void updateBody() {
            ...
            if (activity == null || activity.isDestroyed()) {
                loge("updateBody with no valid activity.");
                return;
            }

            if (prefSet == null) {
                loge("updateBody with no null prefSet.");
                return;
            }
        ...
        public static boolean hideEnhanced4gLteSettings(Context context,
            ...
            boolean isCConfigHideEnhanced4gLte = carrierConfig.getBoolean(
                    CarrierConfigManager.KEY_HIDE_ENHANCED_4G_LTE_BOOL);
            boolean isPropertyHideEnhanced4gLte = SystemProperties.getBoolean(
                    HIDE_ENHANCE_4G_LTE_PROPERTY_NAME, false);
            try {
                if ((mImsMgr.getImsServiceState() != ImsFeature.STATE_READY)
                        || !(mImsMgr.isVolteEnabledByPlatform()
                        && mImsMgr.isVolteProvisionedOnDevice())
                        || isCConfigHideEnhanced4gLte
                        || isPropertyHideEnhanced4gLte) {
                    hideSettings = true;
                    if (VDBG) {
                        Log.d(LOG_TAG, "hideEnhanced4gLteSettings: "
                                + "ImsServiceState: " + mImsMgr.getImsServiceState()
                                + "isVolteEnabledByPlatform: " + mImsMgr.isVolteEnabledByPlatform()
                                + "isVolteProvisioned: " + mImsMgr.isVolteProvisionedOnDevice()
                                + "isCConfigHideEnhanced4gLte: " + isCConfigHideEnhanced4gLte
                                + "isPropertyHideEnhanced4gLte: " + isPropertyHideEnhanced4gLte);
                    }
                }
        ...
        /**
          * Whether to show the 5G switch
          * By default hide 5G switch, show the settings if carreir app set "show_5g_switch_bool"
          * to true and it's DDS.
         */
        private boolean hide5gSettings(Context context, PersistableBundle carrierConfig) {
            final int phoneId = mPhone.getPhoneId();
            final int dataPhoneId = mSubscriptionManager.getDefaultDataPhoneId();
            return (!carrierConfig.getBoolean("show_5g_switch_bool")
                        && !SystemProperties.getBoolean(SHOW_5G_SWITCH_PROPERTY_NAME, false))
                    || (phoneId != dataPhoneId);
        }

        private void update5gSwitchStatus(int networkMode) {
            if (mButton5g != null) {
                mButton5g.setChecked(isNrNetworkModeType(networkMode));
            }
        }

        private boolean isNrNetworkModeType(int type) {
            if (type == TelephonyManager.NETWORK_MODE_NR_ONLY
                    || type == TelephonyManager.NETWORK_MODE_NR_LTE
                    || type == TelephonyManager.NETWORK_MODE_NR_LTE_CDMA_EVDO
                    || type == TelephonyManager.NETWORK_MODE_NR_LTE_GSM_WCDMA
                    || type == TelephonyManager.NETWORK_MODE_NR_LTE_CDMA_EVDO_GSM_WCDMA
                    || type == TelephonyManager.NETWORK_MODE_NR_LTE_WCDMA
                    || type == TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA
                    || type == TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_GSM
                    || type == TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_WCDMA
                    || type == TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_GSM_WCDMA
                    || type == TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA) {
            //TODO: This is being merged due to urgency and it will be redesigned using SDK APIs.
                return true;
            }
            return false;
        }

        private void enableNr() {
            int nrType;
            int perferNetworkMode = getPreferredNetworkModeForSubId();
            switch (perferNetworkMode) {
                case TelephonyManager.NETWORK_MODE_LTE_CDMA_EVDO:
                       nrType = TelephonyManager.NETWORK_MODE_NR_LTE_CDMA_EVDO;
                       break;
                case TelephonyManager.NETWORK_MODE_LTE_GSM_WCDMA:
                       nrType = TelephonyManager.NETWORK_MODE_NR_LTE_GSM_WCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_LTE_CDMA_EVDO_GSM_WCDMA:
                       nrType = TelephonyManager.NETWORK_MODE_NR_LTE_CDMA_EVDO_GSM_WCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_LTE_ONLY:
                       nrType = TelephonyManager.NETWORK_MODE_NR_LTE;
                       break;
                case TelephonyManager.NETWORK_MODE_NR_ONLY:
                       nrType = TelephonyManager.NETWORK_MODE_NR_ONLY;
                       break;
                case TelephonyManager.NETWORK_MODE_LTE_WCDMA:
                       nrType = TelephonyManager.NETWORK_MODE_NR_LTE_WCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_LTE_TDSCDMA:
                       nrType = TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_GSM:
                       nrType = TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_GSM;
                       break;
                case TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_WCDMA:
                       nrType = TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_WCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_GSM_WCDMA:
                       nrType = TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_GSM_WCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA:
                     nrType = TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA;
                     break;
                default:
                     nrType = TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA;
                     break;
            ...
            //TODO: This is being merged due to urgency and it will be redesigned using SDK APIs.
            Log.d(LOG_TAG, "enable NR from " + perferNetworkMode + " -> " +  nrType);
            if (getPreferenceScreen().findPreference(BUTTON_PREFERED_NETWORK_MODE) != null) {
                mButtonPreferredNetworkMode.setValue(Integer.toString(nrType));
            } else {
                //if preferred network mode is hidden, cache the value
                mCachePreferredNetworkMode = nrType;
            }
            if (getPreferenceScreen().findPreference(BUTTON_ENABLED_NETWORKS_KEY) != null) {
                mButtonEnabledNetworks.setValue(Integer.toString(nrType));
            }
            UpdatePreferredNetworkModeSummary(nrType);
            UpdateEnabledNetworksValueAndSummary(nrType);
            mPhone.setPreferredNetworkType(nrType, mHandler
                    .obtainMessage(MyHandler.MESSAGE_SET_PREFERRED_NETWORK_TYPE));
            mButtonPreferredNetworkMode.setEnabled(false);

        }

        private void disableNr() {
            int newNetworkMode;
            int oldNetworkMode = getPreferredNetworkModeForSubId();
            switch (oldNetworkMode) {
                case TelephonyManager.NETWORK_MODE_NR_LTE_CDMA_EVDO:
                       newNetworkMode = TelephonyManager.NETWORK_MODE_LTE_CDMA_EVDO;
                       break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_GSM_WCDMA:
                       newNetworkMode = TelephonyManager.NETWORK_MODE_LTE_GSM_WCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_CDMA_EVDO_GSM_WCDMA:
                       newNetworkMode = TelephonyManager.NETWORK_MODE_LTE_CDMA_EVDO_GSM_WCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_NR_ONLY:
                       newNetworkMode = TelephonyManager.NETWORK_MODE_NR_ONLY;
                       break;
                case TelephonyManager.NETWORK_MODE_NR_LTE:
                       newNetworkMode = TelephonyManager.NETWORK_MODE_LTE_ONLY;
                       break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_WCDMA:
                       newNetworkMode = TelephonyManager.NETWORK_MODE_LTE_WCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA:
                       newNetworkMode = TelephonyManager.NETWORK_MODE_LTE_TDSCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_GSM:
                       newNetworkMode = TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_GSM;
                       break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_WCDMA:
                       newNetworkMode = TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_WCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_GSM_WCDMA:
                       newNetworkMode = TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_GSM_WCDMA;
                       break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA:
                     newNetworkMode = TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA;
                     break;
                default:
                     newNetworkMode = TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA;
                     break;
            ...
            //TODO: This is being merged due to urgency and it will be redesigned using SDK APIs.
            Log.d(LOG_TAG, "disable NR from " + oldNetworkMode + " -> " +  newNetworkMode);
            if (getPreferenceScreen().findPreference(BUTTON_PREFERED_NETWORK_MODE) != null) {
                mButtonPreferredNetworkMode.setValue(Integer.toString(newNetworkMode));
            } else {
                //if preferred network mode is hidden, cache the value
                mCachePreferredNetworkMode = newNetworkMode;
            }
            if (getPreferenceScreen().findPreference(BUTTON_ENABLED_NETWORKS_KEY) != null) {
                mButtonEnabledNetworks.setValue(Integer.toString(newNetworkMode));
            }
            UpdatePreferredNetworkModeSummary(newNetworkMode);
            UpdateEnabledNetworksValueAndSummary(newNetworkMode);
            mPhone.setPreferredNetworkType(newNetworkMode, mHandler
                    .obtainMessage(MyHandler.MESSAGE_SET_PREFERRED_NETWORK_TYPE));
            mButtonPreferredNetworkMode.setEnabled(false);
        }
        ...
        private void updateBodyAdvancedFields(Activity activity, PreferenceScreen prefSet,
            ...
            final int dataPhoneId = mSubscriptionManager
                    .getSlotIndex(mSubscriptionManager.getDefaultDataSubscriptionId());
            int settingsNetworkMode = getPreferredNetworkModeForSubId();
            PersistableBundle carrierConfig =
                    mConfigManager.getConfigForSubId(phoneSubId);

            TelephonyManager telephonyManagerForSubId = mTelephonyManager
                    .createForSubscriptionId(phoneSubId);
            ...
            if (VDBG) {
                log("updateBodyAdvancedFields: carrierConfig for subId: " + phoneSubId
                        + ", phoneId: " + phoneId);
                dumpConfig(carrierConfig);
            }
            ...
            if (!hide5gSettings(getActivity(), carrierConfig)) {
                if (DBG) log("updateBody: update 5G switch");
                prefSet.addPreference(mButton5g);
                mButton5g.setChecked(isNrNetworkModeType(settingsNetworkMode));
            }

            if (showEuiccSettings(getActivity())) {
                prefSet.addPreference(mEuiccSettingsPref);
                String spn = telephonyManagerForSubId.getSimOperatorName();
            ...
            if (getResources().getBoolean(R.bool.config_enabled_4g)) {
                ...
                updateButtonEnable4g();
            } else if (activity.getResources().getBoolean(com.android.internal.R.bool
                    .config_primarycard) && !SystemProperties.getBoolean(
            ...
            if (carrierConfig.getBoolean(
                ...
                prefSet.removePreference(mLteDataServicePref);
                    ...
                    .KEY_HIDE_PREFERRED_NETWORK_TYPE_BOOL)
                        || SystemProperties.getBoolean(
                            HIDE_PREFERRED_NW_MODE_PROPERTY_NAME, false))
                ...
                settingsNetworkMode = mPreferredNetworkMode;
            ...
            if (hideEnhanced4gLteSettings(getActivity(), carrierConfig)) {
                if (DBG) log("hiding Enhanced 4G Lte preference");
            ...
            if (hide5gSettings(getActivity(), carrierConfig)) {
                if (DBG) log("hiding 5G switch preference");
                Preference pref = prefSet.findPreference(BUTTON_5G_SWITCH_KEY);
                if (pref != null) {
                    prefSet.removePreference(pref);
                }
            }
            ...
            if (isCtModeAndCtCard()) {
                enhanced4glteModeTitleId = R.string.enhanced_4g_lte_mode_title_variant;
            }
            int enhanced4glteModeSummaryId = isCtModeAndCtCard() ?
                R.string.enhanced_4g_lte_mode_summary_variant :
                R.string.enhanced_4g_lte_mode_summary;

            updatePreferredNetworkType(phoneId);
            mOnlyAutoSelectInHomeNW = carrierConfig.getBoolean(
                    CarrierConfigManager.KEY_ONLY_AUTO_SELECT_IN_HOME_NETWORK_BOOL);

            mButton4glte.setTitle(enhanced4glteModeTitleId);
            mButton4glte.setSummary(enhanced4glteModeSummaryId);
            ...
            if (ps != null) {
                ...
                if (mPhone.getServiceState().getRoaming()) {
                    ...
                    if (mOnlyAutoSelectInHomeNW) {
                        String operatorName = telephonyManagerForSubId.getSimOperatorName();
                        if (TextUtils.isEmpty(operatorName)) {
                            logi("SimOperatorName is blank, use NetworkOperatorName.");
                            operatorName = telephonyManagerForSubId.getNetworkOperatorName();
                        }
            ...
            boolean isValidSlot = currentPrimarySlot >= 0
                    && currentPrimarySlot < mTelephonyManager.getPhoneCount();
            ...
            if (SystemProperties.getBoolean(PRIMARY_4G_CARD_PROPERTY_NAME, false)) {
                if (isValidSlot && (phoneId != currentPrimarySlot)) {
                    if (PRIMCARYCARD_L_W_ENABLED) {
                        mButtonPreferredNetworkMode.setEntries(
                        ...
                        mButtonEnabledNetworks.setEntryValues(
                                R.array.preferred_network_mode_gsm_wcdma_values);
                    } else if (getPreferredNetworkModeForPhoneId()
        ...
        private void registerPhoneStateListeners() {
            ...
            if (mTelephonyManager == null || mSubscriptionMgrforPhone == null) {
                loge("TelephonyManager or SubscriptionManager is null");
                return;
            }

            for (int i = 0; i < mPhoneStateListener.length; i++) {
                final SubscriptionInfo subInfo =
                        mSubscriptionMgrforPhone.getActiveSubscriptionInfoForSimSlotIndex(i);
                if (subInfo == null) {
                ...
                mPhoneStateListener[i]  = new PhoneStateListener() {
                    ...
                    */
                    ...
                    public void onCallStateChanged(int state, String incomingNumber) {
                        ...
                        PersistableBundle carrierConfig =
                                mConfigManager.getConfigForSubId(subId);

                        if (VDBG) {
                            log("onCallStateChanged: carrierConfig for subId: " + subId
                                    + ", phoneId: " + phoneId);
                            dumpConfig(carrierConfig);
                        }
        ...
        public boolean onPreferenceChange(Preference preference, Object objValue) {
            ...
            if (preference == mButtonPreferredNetworkMode) {
                ...
                mImsMgr.setEnhanced4gLteModeSetting(enhanced4gModePref.isChecked());
                Intent intent = new Intent("org.codeaurora.intent.action.ACTION_ENHANCE_4G_SWITCH");
                intent.addFlags(Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND);
                context.sendBroadcast(intent);
            } else if (preference == mButton5g) {
                if (DBG) log("onPreferenceTreeClick: preference == mButton5g.");
                boolean isChecked = !mButton5g.isChecked();
                Log.d(LOG_TAG, "isChecked : " + isChecked);
                mButton5g.setChecked(isChecked);
                if (isChecked) {
                    enableNr();
                } else {
                    disableNr();
                ...
                if (!mButtonDataRoam.isChecked()) {
                    PersistableBundle carrierConfig =
                            mConfigManager.getConfigForSubId(mPhone.getSubId());

                    if (VDBG) {
                        log("onPreferenceTreeClick: carrierConfig for subId: " + mPhone.getSubId()
                                + ", phoneId: " + mPhone.getPhoneId());
                        dumpConfig(carrierConfig);
                    }
        ...
        private boolean isCtModeAndCtCard() {
            int subId = mPhone.getSubId();
            SubscriptionInfo subInfoNonDds = mSubscriptionMgrforPhone
            ...
            return getContext().getResources().getBoolean(R.bool.config_enhanced4glte_status)
                && isCtCard;
        }
        ...
        private class MyHandler extends Handler {
            ...
            private void handleSetPreferredNetworkTypeResponse(Message msg) {
                ...
                log("handleSetPreferredNetworkTypeResponse: Sucess");
                    ...
                    if (getPreferenceScreen().findPreference(BUTTON_5G_SWITCH_KEY) != null)  {
                        if (getPreferenceScreen().findPreference(
                                    BUTTON_PREFERED_NETWORK_MODE) != null)  {
                            networkMode =  Integer.parseInt(
                                    mButtonPreferredNetworkMode.getValue());
                        } else {
                            //If preferred network mode is hidden, set it as cache value
                            networkMode = mCachePreferredNetworkMode;
                        }
                        log("handleSetPreferredNetworkTypeResponse : 5G switch with type --> "
                                + networkMode);
                        update5gSwitchStatus(networkMode);
                        setPreferredNetworkMode(networkMode);
                    }
                    if (getPreferenceScreen().findPreference(
        ...
        private void updatePreferredNetworkUIFromDb() {
            ...
            update5gSwitchStatus(settingsNetworkMode);
            UpdatePreferredNetworkModeSummary(settingsNetworkMode);
            UpdateEnabledNetworksValueAndSummary(settingsNetworkMode);
        ...
        private void UpdatePreferredCMCCNetworkModeSummary(int NetworkMode) {
            if (TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA == NetworkMode)
        ...
        private void UpdatePreferredNetworkModeSummary(int NetworkMode) {
            switch(NetworkMode) {
                ...
                case TelephonyManager.NETWORK_MODE_NR_ONLY:
                    mButtonPreferredNetworkMode.setSummary(
                            R.string.preferred_network_mode_nr_summary);
                    break;
                case TelephonyManager.NETWORK_MODE_NR_LTE:
                    mButtonPreferredNetworkMode.setSummary(
                ...
                case TelephonyManager.NETWORK_MODE_NR_LTE_CDMA_EVDO:
                    mButtonPreferredNetworkMode.setSummary(
                            R.string.preferred_network_mode_nr_lte_cdma_evdo_summary);
                    break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_GSM_WCDMA:
                    mButtonPreferredNetworkMode.setSummary(
                            R.string.preferred_network_mode_nr_lte_gsm_wcdma_summary);
                    break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_CDMA_EVDO_GSM_WCDMA:
                    mButtonPreferredNetworkMode.setSummary(
                            R.string.preferred_network_mode_nr_lte_cdma_evdo_gsm_wcdma_summary);
                    break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_WCDMA:
                    mButtonPreferredNetworkMode.setSummary(
                            R.string.preferred_network_mode_nr_lte_wcdma_summary);
                    break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA:
                    mButtonPreferredNetworkMode.setSummary(
                            R.string.preferred_network_mode_nr_lte_tdscdma_summary);
                    break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_GSM:
                    mButtonPreferredNetworkMode.setSummary(
                            R.string.preferred_network_mode_nr_lte_tdscdma_gsm_summary);
                    break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_WCDMA:
                    mButtonPreferredNetworkMode.setSummary(
                            R.string.preferred_network_mode_nr_lte_tdscdma_wcdma_summary);
                    break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_GSM_WCDMA:
                    mButtonPreferredNetworkMode.setSummary(
                            R.string.preferred_network_mode_nr_lte_tdscdma_gsm_wcdma_summary);
                    break;
                case TelephonyManager.NETWORK_MODE_NR_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA:
                    mButtonPreferredNetworkMode.setSummary(R.string.
                            preferred_network_mode_nr_lte_tdscdma_cdma_evdo_gsm_wcdma_summary);
                    break;
                case TelephonyManager.NETWORK_MODE_LTE_TDSCDMA_GSM:
                    mButtonPreferredNetworkMode.setSummary(
        ...
        private void updatePreferredNetworkType(int phoneId) {
            if (mCallState == null) return;
            if (phoneId >= 0 && phoneId < mCallState.length) {
                boolean enabled = mCallState[phoneId] == TelephonyManager.CALL_STATE_IDLE
                        && hasActiveSubscriptions();
                logi("updatePreferredNetworkType: " + enabled);
        ...
        private void log(String msg) {
            Log.d(LOG_TAG, "[" + mPhoneId + "] " + msg);
        }

        private void logi(String msg) {
            Log.i(LOG_TAG, "[" + mPhoneId + "] " + msg);
        }

        private void loge(String msg) {
            Log.e(LOG_TAG, "[" + mPhoneId + "] " + msg);
        }
        ...
        private void dumpConfig(PersistableBundle config) {
            List<String> configKeys = new ArrayList<String>(config.keySet());
            for (String key : configKeys) {
                if (config.get(key) != null && config.get(key) instanceof Object[]) {
                    log(key + " = " +
                            Arrays.toString((Object[]) config.get(key)));
                } else if (config.get(key) != null && config.get(key) instanceof int[]) {
                    log(key + " = " + Arrays.toString((int[]) config.get(key)));
                } else {
                    log(key + " = " + config.get(key));
                }
            }
        }
    }
