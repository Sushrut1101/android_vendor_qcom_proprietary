 ...
 * Notes: none
 *==========================================================================*/
int QJpegEncoderTest::EncodeComplete(QImage *aOutputImage __unused)
{
 ...
 * Notes: none
 *==========================================================================*/
int QJpegEncoderTest::OutputFragment(QIBuffer &aBuffer __unused)
{
  QIDBG_MED("%s:%d] ", __func__, __LINE__);
  return QI_SUCCESS;
}
 ...
 * Notes: none
 *==========================================================================*/
void QJpegEncoderTest::WriteComplete(QIBuffer &aBuffer __unused)
{
  QIDBG_MED("%s:%d] ", __func__, __LINE__);
}
 ...
 * Notes: none
 *==========================================================================*/
void QJpegEncoderTest::WriteFragmentDone(QIBuffer &aBuffer __unused)
{
  QIDBG_MED("%s:%d] ", __func__, __LINE__);
}
...
int QJpegEncoderTest::Init()
{
  ...
  mEncodeParams.setNumOfComponents(m_numOfComponents);
...
int QJpegEncoderTest::Start()
{
  ...
  lrc = mInput->setDefaultPlanes(m_numOfPlanes, mInputdata->Addr(),
    mInputdata->Fd(), NULL, NULL);
  if (lrc != QI_SUCCESS) {
    QIDBG_ERROR("%s:%d] failed", __func__, __LINE__);
    return QI_ERR_GENERAL;
  }
  ...
  QI_LOCK(&mMutex);
  mError = QI_SUCCESS;
  lrc = mpEncoder->Start(m_pEncoderThread);
  if (lrc != QI_SUCCESS) {
    QIDBG_ERROR("%s:%d] failed", __func__, __LINE__);
    QI_UNLOCK(&mMutex);
    return QI_ERR_GENERAL;
  ...
  if (!mTimeoutFlag && lrc != QI_SUCCESS) {
    QIDBG_ERROR("%s:%d] failed", __func__, __LINE__);
    QI_UNLOCK(&mMutex);
    return QI_ERR_GENERAL;
  }
...
int main(int argc, char* argv[])
{
  ...
  while ((c = getopt(argc, argv, "I:O:W:H:Q:T:PZ")) != -1) {
    switch (c) {
    case 'O':
    ...
    case 'H':
      ...
      break;
    case 'T':
      lEncoder->mTimeout = atoi(optarg);
      lEncoder->mTimeoutFlag = 1;
