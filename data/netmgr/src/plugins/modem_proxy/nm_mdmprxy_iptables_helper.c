...
)
{
  ...
  {
    case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES, nfo->sport, nfo->mark);
      break;

    case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES, nfo->sport, nfo->mark);
      break;
...
)
{
  ...
  {
    case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES,
               nfo->sport, nfo->dport, nfo->mark);
      break;

    case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES,
               nfo->sport, nfo->dport, nfo->mark);
      break;
...
)
{
  ...
  {
    case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES, nfo->dport, nfo->mark);
      break;

    case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES, nfo->dport, nfo->mark);
      break;
...
)
{
  ...
  {
    case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES, nfo->mark);
      break;

    case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES, nfo->mark);
      break;
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_mark_drop_formatter
===========================================================================*/
/*!
@brief
  Formatter used for preparing rules used to drop spurious packets marked
  with custom mark
...
)
{
  ...
  {
    case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES, nfo->proto, nfo->mark);
      break;

    case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES, nfo->proto, nfo->mark);
      break;
...
)
{
  ...
  {
    case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES, nfo->iif);
      break;

    case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES, nfo->iif);
      break;
...
)
{
  ...
  switch(af)
  {
    case AF_INET:
      ...
      snprintf(buff, bufflen, formatstr, IPTABLES, daddr, gw, nfo->oif);
      break;
    ...
    case AF_INET6:
      ...
      snprintf(buff, bufflen, formatstr, IP6TABLES, daddr, gw, nfo->oif);
      break;
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_dst_drop_formatter
===========================================================================*/
/*!
@brief
  Formatter for dropping packets after TEE
...
*/
/*=========================================================================*/
int nm_mdmprxy_dst_drop_formatter
(
  char *buff,
  int bufflen,
  sa_family_t af,
...
)
{
  ...
  if (!buff)
  {
    netmgr_log_err("%s(): buff not provided, invalid input!", __func__);
    return NM_MDMPRXY_ERR;
  }

  if (!formatstr)
  {
    netmgr_log_err("%s(): formatstr not provided, invalid input!", __func__);
    return NM_MDMPRXY_ERR;
  }
  ...
  case AF_INET:
    ...
    inet_ntop(af, &(nfo->daddr4), daddr, INET6_ADDRSTRLEN);
    snprintf(buff, bufflen, formatstr, IPTABLES, daddr);
    break;
  ...
  case AF_INET6:
    ...
    snprintf(buff, bufflen, formatstr, IP6TABLES, daddr);
...
)
{
  ...
  {
    case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES, NM_MDMPRXY_IIFACE, nfo->oif);
      break;

    case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES, NM_MDMPRXY_IIFACE, nfo->oif);
      break;
...
)
{
  ...
  {
  case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES, nfo->oif, NM_MDMPRXY_IIFACE);
      break;

  case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES, nfo->oif, NM_MDMPRXY_IIFACE);
      break;
...
)
{
  ...
  {
    case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES, nfo->spi, nfo->mark);
      break;

    case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES, nfo->spi, nfo->mark);
      break;
...
)
{
  ...
  switch(af)
  {
    case AF_INET:
      ...
      snprintf(buff, bufflen, formatstr, IPTABLES, gw, nfo->oif);
      break;

    case AF_INET6:
      inet_ntop(af, &(nfo->gw6), gw, sizeof(gw));
      snprintf(buff, bufflen, formatstr, IP6TABLES, gw, nfo->oif);
      break;
...
)
{
  ...
  {
    case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES, NM_MDMPRXY_LBO_MARK);
      break;

    case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES, NM_MDMPRXY_LBO_MARK);
      break;
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_masquerade_skip_formatter
===========================================================================*/
/*!
@brief
  Formatter for masquerade skip rules

@arg buff - char buffer to store the resulting string
@arg bufflen - length of buff in bytes
@arg af - AF_INET/AF_INET6
@arg formatstr - format string; you know, the thing with the %s and %d
@arg in_addr - IPv4 address of IMS PDN
...
)
{
   ...
   struct in_addr *inaddr4 = (struct in_addr *) info;

   if (af == AF_INET)
   {
      inet_ntop(af, inaddr4, saddr, INET_ADDRSTRLEN);
      snprintf(buff, bufflen, formatstr, IPTABLES, saddr);
   }
   else
   {
     return NM_MDMPRXY_SKIP;
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_static_rule_masquerade_formatter
===========================================================================*/
/*!
@brief
  Formatter for masquerade jump rules
...
)
{
  ...
  if (af == AF_INET)
  {
    return nm_mdmprxy_basic_formatter(buff, bufflen, af, formatstr, info);
  }
  return NM_MDMPRXY_SKIP;
}
...
)
{
  ...
  int tunnel_af = nfo->tunnel_family;
  ...
  {
    case AF_INET:
      snprintf(buff, bufflen, formatstr, IPRULE, nfo->local_addr, nfo->dest_addr);
      break;

    case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6RULE, nfo->local_addr, nfo->dest_addr);
      break;
...
)
{
  ...
  {
    case AF_INET:
      inet_ntop(af, &(nfo->saddr.saddr4), saddr, INET6_ADDRSTRLEN);
      if (nfo->mask)
      {
        snprintf(prefix, 8, "/%u", nfo->mask);
        strlcat(saddr, prefix, INET6_ADDRSTRLEN*2);
      }

      snprintf(buff, bufflen, formatstr, IPTABLES, saddr, nfo->mss);
      break;

    case AF_INET6:
      inet_ntop(af, &(nfo->saddr.saddr6), saddr, INET6_ADDRSTRLEN);
      if (nfo->mask)
      {
        snprintf(prefix, 8, "/%u", nfo->mask);
        strlcat(saddr, prefix, INET6_ADDRSTRLEN*2);
      ...
      snprintf(buff, bufflen, formatstr, IP6TABLES, saddr, nfo->mss);
      break;
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_process_rule_return_result
===========================================================================*/
/*!
@brief
  Install / remove an iptable rule (really just runs the cmd specified,
  does not have to be an iptables rule). Returns the result of the command
  back to the caller
...
*/
/*=========================================================================*/
int nm_mdmprxy_process_rule_return_result
(
  const struct nm_mdmprxy_cmd *cmd,
  int use_antirule,
  sa_family_t af,
...
)
{
  ...
  DS_SYSCALL_DEFAULTS(opts);
  opts.stderr = cmd_result;
  opts.stderr_length = cmd_result_len;
  opts.log_options |= DS_EXEC_LOG_EXECTIME;
...
*/
/*=========================================================================*/
int nm_mdmprxy_install_iiface_rules(const char *ifname, uint32_t mark)
{
  int i, handle, rc;
  size_t cpyres;
  ...
  if (handle == -1)
  {
    netmgr_log_err("%s(): could not find a free handle!", __func__);
    return NETMGR_FAILURE;
  }
  ...
  {
    netmgr_log_err("%s(): failed to allocate memory on handle [%d]", __func__, handle);
    return NETMGR_FAILURE;
  }
  ...
  if (rc != NETMGR_SUCCESS)
  {
    netmgr_log_err("%s(): failed to install iiface rules for IPv4 on handle [%d]!",
                   __func__, handle);
    goto error;
  }
  ...
  if (rc != NETMGR_SUCCESS)
  {
    netmgr_log_err("%s(): failed to install iiface rules for IPv6 on handle [%d]!",
                   __func__, handle);
...
*/
/*=========================================================================*/
int nm_mdmprxy_uninstall_oiface_rules(int handle, int slot, int ip_family)
{
  ...
  if (!installed_rules[handle])
  {
    netmgr_log_err("%s(): no valid iif rules present on handle [%d]", __func__, handle);
    return NETMGR_FAILURE;
  }
  ...
  if (!orule->valid4 && !orule->valid6)
  {
    /* Clear the oiface rule struct only when both address family rules
...
*/
/*=========================================================================*/
int nm_mdmprxy_uninstall_iiface_rules(int handle)
{
  int i = 0;
  ...
  _nm_mdmprxy_uninstall_iiface_rules(handle,
                                     AF_INET6,
                                     route_prov_cmd_tbl->per_iiface_rule_list_len - 1);
  /* Partial retry on this handle can result in bad things. Reset the handle
   * even if the uninstall fails */
  if (installed_rules[handle] != NULL)
  {
    free(installed_rules[handle]);
    installed_rules[handle] = NULL;
  }
...
*/
/*=========================================================================*/
int nm_mdmprxy_install_oiface_rules
(
  int             handle,
  int             slot,
  const char      *ifname,
...
)
{
  ...
  if(valid4)
  {
    if (orule->valid4 == 1)
    {
      netmgr_log_err("%s(): Already have a v4 oiface rule for [%s]", __func__, ifname);
      return NETMGR_FAILURE;
    }
    orule->valid4 = 1;
    netmgr_log_low("%s(): Installing v4 oiface rules for [%s]", __func__, ifname);
    orule->gw4 = gw4;
    orule->daddr4 = daddr4;
    rc = _nm_mdmprxy_install_destination_rules(handle, AF_INET, slot);
    if (rc != NETMGR_SUCCESS) {
      orule->valid4 = 0;
      netmgr_log_err("%s(): Failed to install v4 dest rules for [%s]", __func__, ifname);
      goto error;
    }
    ...
    {
      orule->valid4 = 0;
      netmgr_log_err("%s(): Failed to install v4 forwarding rules for [%s]", __func__, ifname);
      goto error;
    }
  }
  ...
  if(valid6)
  {
    if (orule->valid6 == 1)
    {
      netmgr_log_err("%s(): Already have a v6 oiface rule for [%s]", __func__, ifname);
      return NETMGR_FAILURE;
    }
    orule->valid6 = 1;
    netmgr_log_low("%s(): Installing v6 oiface rules for [%s]", __func__, ifname);
    ...
    {
      orule->valid6 = 0;
      netmgr_log_err("%s(): Failed to install v dest rules for [%s]", __func__, ifname);
      goto error;
    }
    ...
    {
      orule->valid6 = 0;
      netmgr_log_err("%s(): Failed to install v6 forwarding rules for [%s]", __func__, ifname);
      goto error;
    }
  }
...
nm_mdmprxy_flush_connmark_rules(sa_family_t af)
{
  ...
  flush_conntrack_rules.rule = "%s" NM_MDMPRXY_FLAGS " -t mangle -F nm_mdmprxy_icmp_pkt_marker";
...
nm_mdmprxy_flush_spi_rules(sa_family_t af)
{
  ...
  flush_spi_rules.rule = "%s" NM_MDMPRXY_FLAGS " -t mangle -F nm_mdmprxy_mngl_pre_spi";
...
nm_mdmprxy_check_iptables_socket_match_support(void)
{
  ...
  { "%s" NM_MDMPRXY_FLAGS " -t mangle -A PREROUTING -m socket --nowildcard --restore-skmark"
                          " -j ACCEPT",
    "%s" NM_MDMPRXY_FLAGS " -t mangle -D PREROUTING -m socket --nowildcard --restore-skmark"
                          " -j ACCEPT",
    nm_mdmprxy_basic_formatter
  };
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_install_masquerade_skip_rule
===========================================================================*/
/*!
@brief
  Installs ip rule to skip masquerade rules for iWLAN packets

@arg link_name  - Interface name
@arg tech_type  - Local breakout or S2B
@arg saddr4 - V4 address of IMS
...
/*=========================================================================*/
int
nm_mdmprxy_install_masquerade_skip_rule
(
  const char          *link_name,
  int                 tech_type,
  int                 ip_family,
...
)
{
  ...
  NM_MDMPRXY_NULL_CHECK(route_prov_cmd_tbl->masquerade_skip_rule_list);
  NM_MDMPRXY_NULL_CHECK(saddr4);
  ...
  for (i = 0; i < route_prov_cmd_tbl->masquerade_skip_rule_list_len; i++)
  {
    if (!route_prov_cmd_tbl->masquerade_skip_rule_list[i].rule)
      continue;

    rc = nm_mdmprxy_process_rule(&route_prov_cmd_tbl->masquerade_skip_rule_list[i],
                                 NM_MDMPRXY_RUN_RULE, ip_family, saddr4);
...
error:
  nm_mdmprxy_remove_masquerade_skip_rule(link_name, tech_type, ip_family, saddr4);
  return NETMGR_FAILURE;
}

/*===========================================================================
  FUNCTION  nm_mdmprxy_remove_masquerade_skip_rule
===========================================================================*/
/*!
@brief
  Removes ip rule to skip masquerade rules for iWLAN traffic

@arg link_name  - Interface name
@arg tech_type  - Local breakout or S2B
@arg saddr4     - address of IMS PDN
...
/*=========================================================================*/
int
nm_mdmprxy_remove_masquerade_skip_rule
(
  const char          *link_name,
  int                 tech_type,
  int                 ip_family,
...
)
{
  ...
  for (i = 0; i < route_prov_cmd_tbl->masquerade_skip_rule_list_len; i++)
  {
    ...
    rc = nm_mdmprxy_process_rule(&route_prov_cmd_tbl->masquerade_skip_rule_list[i],
                                 NM_MDMPRXY_RUN_ANTIRULE, ip_family, saddr4);
...
nm_mdmprxy_flush_doze_rules(sa_family_t af)
{
  ...
  flush_doze_rules.rule = "%s" NM_MDMPRXY_FLAGS " -t filter -F nm_mdmprxy_doze_mode_skip";
  flush_doze_rules.antirule = NULL;
  flush_doze_rules.formatter = nm_mdmprxy_basic_formatter;

  rc = nm_mdmprxy_process_rule(&flush_doze_rules, NM_MDMPRXY_RUN_RULE, af, NULL);
