...
public class OsAgent
{
    ...
    public static OsAgent GetInstance(Context context, Looper looperObj,
                                      IzatProviderContext izatProviderContext) {
        synchronized (mLock) {
            if (null == mInstance) {
                mInstance = new OsAgent(context, looperObj, izatProviderContext);
            }
        }
        return mInstance;
    }
    private OsAgent(Context context, Looper looperObj, IzatProviderContext izatProviderContext)
    {
        ...
        // listen for location mode change
        Message msgObjInstallModeChangeReceiver = Message.obtain(mHandler,
                                                      MSG_LOCATION_MODE_CHANGE);
        mHandler.sendMessage(msgObjInstallModeChangeReceiver);
            ...
            switch (newdataItem)
            {
                ...
                case ENH_DATA_ITEM_ID:
                    ...
                    if ( mEulaState != eula_state) {
                        ...
                        if (consentChanged && (null != mIzatProviderContext)) {
                            mIzatProviderContext.setUserConsent(consentAccepted);
                        }
    ...
    private void installModeChangeReceiver()
    {
        ...
        mContext.registerReceiver(mProviderChangedReceiver, intentFilter, null, mHandler);
    ...
    private void setRegulatedFeatureAllowed()
    {
        if(0 != (mDeviceInRegulatedArea & DEVICE_IN_REGULATED_REGION_KNOWN)) {
            ...
            if (null != mIzatProviderContext) {
                mIzatProviderContext.setRegionalNpRegulate(regulated);
            }
        }
    }
    ...
    private static final int MSG_LOCATION_MODE_CHANGE =              113;
