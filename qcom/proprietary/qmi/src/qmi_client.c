/******************************************************************************
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2007-2010,2012,2014,2018 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
)
{
    ...
    {
        QMI_ERR_MSG_2("Invalid conn_id:%d range = [0,%d]\n",conn_id, QMI_MAX_CONN_IDS);
        return QMI_INTERNAL_ERR;
    }
...
)
{
  ...
  uint32_t reply_max_buf_len;
  int qmi_error,ret_val;
  uint32_t req_max_msg_len;
  uint32_t max_msg_len;
  qmi_service_id_type service_id;
  int encode_ret_val = 0;
