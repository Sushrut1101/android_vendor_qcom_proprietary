...
void Dispatcher::registerHandler(message_id_t id, Module* module) {
  ...
  if (is_handle_valid(id) && module) {
    ...
    if (h) {
      auto found = std::find(h->module_list.begin(), h->module_list.end(), module);
      if (found == h->module_list.end()) {
        h->module_list.push_back(module);
      } else {
        Log::getInstance().d("module already present in the list at position: " +
                        std::to_string(found - h->module_list.begin() + 1));
      }
    }
  }
  else {
...
void Dispatcher::suppressMessage(message_id_t id, bool status) {
    ScopedLock lock(mMutex);
