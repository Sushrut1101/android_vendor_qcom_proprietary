...
static void *msg_waiting_thread(void *s) {
    ...
    while(1) {
        ...
        msg = (msg_t *) malloc(sizeof(msg_t) + 1);
        if(msg == NULL) {
...
bool start_mmid() {
    ...
    while(retry > 0) {
        property_get("init.svc.vendor.mmid", svc_mmid_status, "stopped");
        ALOGE("mmid status:%s", svc_mmid_status);
        if(!strcmp(svc_mmid_status, "running")) {
            ...
            property_set("ctl.start", "vendor.mmid");
            usleep(5*1000); //5ms
            retry--;
        }
    }
