...
public:
    ...
    inline virtual void apListUpdate(std::vector<APInfo>* ap_list_ptr,
                              const int apListStatus,
                              const UlpLocation& ulpLoc,
                              const bool bUlpLocValid) override {
        if (nullptr != ap_list_ptr) {
            ...
            if (ap_list_arr!=NULL) {
                ...
                if (true == completeList) {
                    mApListAvailCb(ap_list_arr, ap_list.size(), apListStatus,
                                   ulpLoc, bUlpLocValid, (void *)mClientData);
                }
                for (size_t ii = 0; ii < ap_list.size(); ++ii) {
                    if (ap_list_arr[ii]!=NULL) {
...
static void DBReceiverApListUpdateCb(APInfo_s *ap_list[], size_t ap_list_size,
                                     int apListStatus, const UlpLocation& gpsLoc,
                                     bool bUlpLocValid, void *clientData) {
    ...
    sCallback->apListUpdateCallback(apInfoList, apListSize);
    (void) apListStatus;
    (void) gpsLoc;
    (void) bUlpLocValid;
    (void) clientData;
...
Return<void> LocHidlWiFiDBReceiver::pushAPWiFiDB(
    ...
    ((WiFiDBUpdaterWrapper*)mIzatWiFiDBReceiverApiClient)->pushWiFiDB(&loc_list,
                                                                      &ap_list,
                                                                      daysValid,
                                                                      0);
    return Void();
