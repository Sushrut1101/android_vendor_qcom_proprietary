...
package com.qualcomm.qti.confdialer.data;
...
public class Contact {
    ...
    private static final String TAG = "dialer Contact";
    private static ContactsCache sContactCache;
    private static final String SELF_ITEM_KEY = "Self_Item_Key";
    private static final String SQLITE_EXCEPTION_LOW_MEMORY_MESSAGE =
...
/*            if (MessageUtils.isAlias(number)) {
        ...
        private Contact getContactInfoForPhoneNumber(String number) {
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(minMatch)) {
                ...
                Cursor cursor = mContext.getContentResolver().query(
                        PHONES_WITH_PRESENCE_URI, CALLER_ID_PROJECTION, selection, null, null);
                if (cursor == null) {
        ...
        private static boolean isLowMemoryException(SQLiteException e) {
            return e.getMessage().equals(SQLITE_EXCEPTION_LOW_MEMORY_MESSAGE);
        }

        private void raiseLowMemoryToast() {
            Toast.makeText(mContext, com.android.internal.R.string.low_memory,
                ...
                try {
                    while (cursor.moveToNext()) {
                        ...
                        synchronized (entry) {
                            ...
                            if (!TextUtils.isEmpty(name)) {
                                entry.mName = name;
                                if (Log.isLoggable("contacts", Log.DEBUG)) {
        ...
        private Contact internalGet(String numberOrEmail, boolean isMe) {
            synchronized (ContactsCache.this) {
                ...
                final boolean isNotRegularPhoneNumber = isMe || false ;
