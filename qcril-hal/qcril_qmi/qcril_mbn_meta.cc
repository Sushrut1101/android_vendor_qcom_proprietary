...
#define QCRIL_MBN_META_DATA_LEN_SIZE (4)
    ...
    do {
        ...
        if ((*meta_data_len < QCRIL_MBN_META_DATA_LEN_SIZE) || (*meta_data_len > ((unsigned int)meta_start_offset)))
        {
            break;
        }

        *meta_data = static_cast<unsigned char *>(calloc(1, *meta_data_len));
        if (!(*meta_data))
        {
            break;
        }
...
#define QCRIL_MBN_CONFIG_INFO_MAGIC_LEN  (8 + 8)
    ...
    do {
        ...
        while (index < meta_data_len)
        {
            ...
            /* Since we are reading 2 bytes, need to consider index +1 */
            if (index + 1 >= meta_data_len) break;
        ...
        tlv_len = *((int16_t*)&meta_data[index]);
        *length = tlv_len;
        *value = static_cast<unsigned char *>(calloc(1, tlv_len));

        if(*value)
        {
            index += 2;
            if ( (index + tlv_len) >= meta_data_len ) {
                QCRIL_LOG_DEBUG("Trying to read beyond available data");
                free(*value);
                *value = NULL;
                break;
            }
...
)
{
    ...
    do
    {
        ...
        *iin_list_count = iin_size;
        ...
        if(iin_size <= 0)
        {
            QCRIL_LOG_DEBUG("Empty IIN list(or) invalid size");
            if(*iin_flag == 1) {
                QCRIL_LOG_DEBUG("wild_entry: add to db");
                ret = E_SUCCESS;
                break;
            } else {
                ret = E_FAILURE;
        ...
        *iin_list = static_cast<int *>(malloc(iin_size*sizeof(int32_t)));
        if(*iin_list)
        {
            index++;
            if (index >= length)
...
)
{
    ...
    do
    {
        ...
        *long_iin_list = static_cast<char *>(calloc(1, (length + 1 ) * sizeof(char)));
        if (!(*long_iin_list)) {
            ret = E_FAILURE;
            break;
        }
...
)
{
    ...
    do
    {
        ...
        *mcc_mnc_list = static_cast<uint16_t *>(malloc(2*mcc_mnc_size*sizeof(uint16_t)));
...
)
{
    ...
    do
    {
        ...
        if (value)
        {
            *carrier_name = static_cast<char *>(malloc(sizeof(char)*length+1));
...
)
{
    ...
    do
    {
        ...
        if (value)
        {
            *hwid_name = static_cast<char *>(malloc((sizeof(char)*(length+1))));
            if(*hwid_name)
            {
