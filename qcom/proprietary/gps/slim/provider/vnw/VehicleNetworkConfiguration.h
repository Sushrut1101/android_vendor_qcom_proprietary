...
*/
#define SLIM_VNW_BATCH_TYPE_TIME      1
#define SLIM_VNW_BATCH_TYPE_COUNT     2
#define SLIM_VNW_BATCH_TYPE_DEFAULT   SLIM_VNW_BATCH_TYPE_TIME
#define SLIM_VNW_BATCH_VAL_DEFAULT    100
...
public:
  ...
  */
  ...
  /**
  @brief Load batch configuration

  This method stores batch configuration string into internal field.The method must
  be overwritten in inherited class to implement functionality to parse
  configuration string to initialise necessary values.
  ...
  @param[in] sLen length of the buffer
  */
  virtual void loadBatchConfig(const char* pStrBatchType, const char* pStrBatchValue);
...
public:
  ...
  void loadBatchConfig(const char* pStrBatchType, const char* pStrBatchValue);
...
typedef struct
{
  int m_iBitWiseErrorStartBit;
  int m_iBitWiseErrorLength;
  int m_iBitWiseErrorValue;
...
public:
  ...
  bool CfgValid() const;
  void ToString(char* pStr, size_t sLen) const;
  void GetGVecMetaByAxis(GyroValRead* pz_ValConfig,char** pFBegin, int iDefault);
  void GetNextError(BitErrorStructType* pz_ErrValConfig,char** pFBegin, int iDefault);
  ~GyroCONF1Config();
  void loadBatchConfig(const char* pStrBatchType, const char* pStrBatchValue);

  int                  m_iFrameId;
  int                  m_iByteOrder;
  int                  m_iSign;               //0-signed, 1-unsigned
  int                  m_iSignStartBit;
  int                  m_iSignLength;
  int                  m_iSignVal;
  float                m_fGyroMin;
  float                m_fGyroMax;
  float                m_fAdditiveOffset;
  float                m_fScalingOffset;
  int                  m_iUnit;               // 0 is deg/sec, 1 - rad/sec
  int                  m_iNumAbsError;
  int                  m_iNumBitWiseError;
  float*               m_pfAbsErrors;
  BitErrorStructType*  m_pzBitWiseErrors;
  GyroValRead          m_zGyroConfig[3];
  int                  m_iBatchSize;
  int                  m_iBatchType;
  };
