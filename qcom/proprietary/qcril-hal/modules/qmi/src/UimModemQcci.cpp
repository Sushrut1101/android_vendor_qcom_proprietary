...
#define QMI_UIM_DEFAULT_TIMEOUT 125000
...
)
{
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    if (qmi_response->extended_card_info_valid)
    {
      for (uint8 index  =0; index < qmi_response->extended_card_info_len && index < rsp_data->rsp_data.get_card_status_rsp.card_status.num_slots && index < QMI_UIM_MAX_SLOT_COUNT; index++)
      {
        rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].iccid_len =
            MIN(MIN(QMI_UIM_MAX_ICCID_LEN, QMI_UIM_ICCID_LEN_MAX_V01),
                qmi_response->extended_card_info[index].iccid_len);

        if(rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].iccid_len)
        {
          memcpy(rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].iccid,
                 qmi_response->extended_card_info[index].iccid,
                 rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].iccid_len);
        }

        rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].atr_len =
            MIN(MIN(QMI_UIM_MAX_ATR_LEN, QMI_UIM_ATR_DATA_MAX_V01),
                qmi_response->extended_card_info[index].atr_len);

        if(rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].atr_len)
        {
          memcpy(rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].atr,
                 qmi_response->extended_card_info[index].atr,
                 rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].atr_len);
        }

        rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].eid_len =
            MIN(MIN(QMI_UIM_MAX_EID_LEN, QMI_UIM_EID_LEN_V01),
                qmi_response->extended_card_info[index].eid_len);

        if(rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].eid_len)
        {
          memcpy(rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].eid,
                 qmi_response->extended_card_info[index].eid,
                 rsp_data->rsp_data.get_card_status_rsp.card_status.card[index].eid_len);
        }
      }
    }
...
)
{
  ...
  if(qmi_response->card_result_valid)
  {
    rsp_data->rsp_data.logical_channel_rsp.sw1_sw2_valid = QMI_UIM_TRUE;
  ...
  {
    rsp_data->rsp_data.logical_channel_rsp.sw1_sw2_valid = QMI_UIM_FALSE;
  }
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    else if (qmi_response->long_select_response_valid)
    {
      if(qmi_response->long_select_response_len > QMI_UIM_LONG_SELECT_RESPONSE_MAX_V01)
      {
        QCRIL_LOG_ERROR("%s", "data length too long");
        rsp_data->qmi_err_code = QMI_SERVICE_ERR;
        return;
      ...
      /* For sync calls this is freed by client */
      rsp_data->rsp_data.logical_channel_rsp.select_response.data_ptr =
        (unsigned char *)qcril_malloc(qmi_response->long_select_response_len);
      if(rsp_data->rsp_data.logical_channel_rsp.select_response.data_ptr == NULL)
      {
        QCRIL_LOG_ERROR("%s", "data pointer NULL");
        rsp_data->qmi_err_code = QMI_SERVICE_ERR;
      ...
      memcpy((rsp_data->rsp_data.logical_channel_rsp.select_response.data_ptr),
             (unsigned char*)qmi_response->long_select_response,
             (unsigned short)qmi_response->long_select_response_len);
...
)
{
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    else if (qmi_response->long_status_response_valid)
    {
      if((qmi_response->long_status_response_len) > QMI_UIM_LONG_SELECT_RESPONSE_MAX_V01)
      {
        QCRIL_LOG_ERROR("%s", "data length too long");
        rsp_data->qmi_err_code = QMI_SERVICE_ERR;
        return;
      ...
      rsp_data->rsp_data.send_status_rsp.status_response.data_ptr = (unsigned char *)qcril_malloc(qmi_response->long_status_response_len);
      if(rsp_data->rsp_data.send_status_rsp.status_response.data_ptr == NULL)
      {
        QCRIL_LOG_ERROR("%s", "data pointer NULL");
        rsp_data->qmi_err_code = QMI_SERVICE_ERR;
      ...
      memcpy(rsp_data->rsp_data.send_status_rsp.status_response.data_ptr,
             (unsigned char*)qmi_response->long_status_response,
             (unsigned short)qmi_response->long_status_response_len);

      rsp_data->rsp_data.send_status_rsp.status_response.data_len = (unsigned short)qmi_response->long_status_response_len;
    }
  }
  else
  {
...
)
{
  ...
  if(qmi_response->card_result_valid)
  {
    ...
    rsp_data->rsp_data.reselect_rsp.sw1_sw2_valid = QMI_UIM_TRUE;
  }
  else
  {
    rsp_data->rsp_data.reselect_rsp.sw1_sw2_valid = QMI_UIM_FALSE;
  }
...
)
{
  ...
  if(qmi_response->physical_slot_status_valid)
  {
    ...
    for(slot_id = 0; slot_id < qmi_response->physical_slot_status_len && slot_id < QMI_UIM_MAX_SLOT_COUNT; slot_id++)
    {

     if(!qmi_response->extended_card_state_valid)
     {
     ...
     else if(slot_id < qmi_response->extended_card_state_len && slot_id < QMI_UIM_MAX_SLOT_COUNT)
     {
       switch(qmi_response->extended_card_state[slot_id].card_state)
       {
         case UIM_EXTENDED_CARD_STATE_ABSENT_V01:
         case UIM_EXTENDED_CARD_STATE_UNKNOWN_V01:
           rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].card_state = QMI_UIM_SLOT_CARD_STATE_ABSENT;
           break;
         case UIM_EXTENDED_CARD_STATE_PRESENT_V01:
           rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].card_state = QMI_UIM_SLOT_CARD_STATE_PRESENT;
           break;
         case UIM_EXTENDED_CARD_STATE_ERROR_V01:
           if(qmi_response->extended_card_state[slot_id].error_code == UIM_CARD_ERROR_CODE_NO_ATR_RECEIVED_V01 ||
              qmi_response->extended_card_state[slot_id].error_code == UIM_CARD_ERROR_CODE_POSSIBLY_REMOVED_V01||
              qmi_response->extended_card_state[slot_id].error_code == UIM_CARD_ERROR_CODE_POWER_DOWN_V01)
           {
             rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].card_state = QMI_UIM_SLOT_CARD_STATE_ABSENT;
           }
           else
           {
             rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].card_state = QMI_UIM_SLOT_CARD_STATE_ERROR;
           }
           break;
         default:
           rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].card_state = QMI_UIM_SLOT_CARD_STATE_ERROR;
      ...
      rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].iccid_len =
                                      MIN(qmi_response->physical_slot_status[slot_id].iccid_len, QMI_UIM_MAX_ICCID_LEN);
      memcpy(rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].iccid,
             qmi_response->physical_slot_status[slot_id].iccid,
             rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].iccid_len);
    }
  }
  if(qmi_response->physical_slot_information_valid &&
     rsp_data->rsp_data.slots_status_rsp.no_of_slots == (MIN(qmi_response->physical_slot_information_len,
                                                          QMI_UIM_MAX_SLOT_COUNT)))
  {
    for(slot_id = 0; slot_id < qmi_response->physical_slot_information_len && slot_id < QMI_UIM_MAX_SLOT_COUNT; slot_id++)
    {
      rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].atr_len =
                                      MIN(qmi_response->physical_slot_information[slot_id].atr_value_len, QMI_UIM_MAX_ATR_LEN);
      memcpy(rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].atr,
             qmi_response->physical_slot_information[slot_id].atr_value,
             rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].atr_len);
    }
  }

  if (qmi_response->slot_eid_info_valid &&
      rsp_data->rsp_data.slots_status_rsp.no_of_slots == (MIN(qmi_response->slot_eid_info_len,
                                                          QMI_UIM_MAX_SLOT_COUNT)))
  {
    for (slot_id = 0; slot_id < rsp_data->rsp_data.slots_status_rsp.no_of_slots; slot_id++)
    {
      rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].eid_len =
        MIN(MIN(qmi_response->slot_eid_info[slot_id].eid_len, QMI_UIM_MAX_EID_LEN),
            QMI_UIM_EID_LEN_V01);

      if(rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].eid_len)
      {
        memcpy(rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].eid,
               qmi_response->slot_eid_info[slot_id].eid,
               rsp_data->rsp_data.slots_status_rsp.slot_status[slot_id].eid_len);
      }
    }
  }
...
)
{
  if(ind_data_ptr != NULL)
  {
    ...
    if (ind_data_ptr->extended_card_info_valid)
    {
      for (uint8 index  =0; index < ind_data_ptr->extended_card_info_len && index < ind_data->status_change_ind.card_status.num_slots && index < QMI_UIM_MAX_SLOT_COUNT; index++)
      {
        ind_data->status_change_ind.card_status.card[index].iccid_len =
            MIN(MIN(QMI_UIM_MAX_ICCID_LEN, QMI_UIM_ICCID_LEN_MAX_V01),
                ind_data_ptr->extended_card_info[index].iccid_len);

        if(ind_data->status_change_ind.card_status.card[index].iccid_len)
        {
          memcpy(ind_data->status_change_ind.card_status.card[index].iccid,
                 ind_data_ptr->extended_card_info[index].iccid,
                 ind_data->status_change_ind.card_status.card[index].iccid_len);
        }

        ind_data->status_change_ind.card_status.card[index].atr_len =
            MIN(MIN(QMI_UIM_MAX_ATR_LEN, QMI_UIM_ATR_DATA_MAX_V01),
                ind_data_ptr->extended_card_info[index].atr_len);

        if(ind_data->status_change_ind.card_status.card[index].atr_len)
        {
          memcpy(ind_data->status_change_ind.card_status.card[index].atr,
                 ind_data_ptr->extended_card_info[index].atr,
                 ind_data->status_change_ind.card_status.card[index].atr_len);
        }

        ind_data->status_change_ind.card_status.card[index].eid_len =
            MIN(MIN(QMI_UIM_MAX_EID_LEN, QMI_UIM_EID_LEN_V01),
                ind_data_ptr->extended_card_info[index].eid_len);

        if(ind_data->status_change_ind.card_status.card[index].eid_len)
        {
          memcpy(ind_data->status_change_ind.card_status.card[index].eid,
                 ind_data_ptr->extended_card_info[index].eid,
                 ind_data->status_change_ind.card_status.card[index].eid_len);
        }
      }
    }
...
)
{
  ...
  if(ind_data_ptr->refresh_event_valid)
  {
    ...
    if (ind_data->refresh_ind.refresh_event.num_files > 0)
    {
...
)
{
  ...
  if(ind_data_ptr != NULL && ind_data != NULL)
  {
       ...
       ind_data_ptr->physical_slot_status_len <= 5)
    {
      ...
      {
        if(!ind_data_ptr->extended_card_state_valid)
        {
          ind_data->slot_status[i].card_state =
             (qmi_uim_slot_card_state_type)ind_data_ptr->physical_slot_status[i].physical_card_status;
        }
        else if(i < ind_data_ptr->extended_card_state_len && i < QMI_UIM_MAX_SLOT_COUNT)
        {
          switch(ind_data_ptr->extended_card_state[i].card_state)
          {
            case UIM_EXTENDED_CARD_STATE_ABSENT_V01:
            case UIM_EXTENDED_CARD_STATE_UNKNOWN_V01:
              ind_data->slot_status[i].card_state = QMI_UIM_SLOT_CARD_STATE_ABSENT;
              break;
            case UIM_EXTENDED_CARD_STATE_PRESENT_V01:
              ind_data->slot_status[i].card_state = QMI_UIM_SLOT_CARD_STATE_PRESENT;
              break;
            case UIM_EXTENDED_CARD_STATE_ERROR_V01:
              if(ind_data_ptr->extended_card_state[i].error_code == UIM_CARD_ERROR_CODE_NO_ATR_RECEIVED_V01 ||
                 ind_data_ptr->extended_card_state[i].error_code == UIM_CARD_ERROR_CODE_POSSIBLY_REMOVED_V01||
                 ind_data_ptr->extended_card_state[i].error_code == UIM_CARD_ERROR_CODE_POWER_DOWN_V01)
              {
                ind_data->slot_status[i].card_state = QMI_UIM_SLOT_CARD_STATE_ABSENT;
              }
              else
              {
                ind_data->slot_status[i].card_state = QMI_UIM_SLOT_CARD_STATE_ERROR;
              }
              break;
            default:
              ind_data->slot_status[i].card_state = QMI_UIM_SLOT_CARD_STATE_ERROR;
    ...
    if(ind_data_ptr->physical_slot_information_valid &&
       ind_data_ptr->physical_slot_status_len == ind_data_ptr->physical_slot_information_len)
    {
      for (i = 0; i < ind_data->no_of_slots && i < QMI_UIM_MAX_SLOT_COUNT; i++)
      {
        ind_data->slot_status[i].atr_len    =
           MIN(ind_data_ptr->physical_slot_information[i].atr_value_len, QMI_UIM_MAX_ATR_LEN);
        memcpy(ind_data->slot_status[i].atr,
               ind_data_ptr->physical_slot_information[i].atr_value,
               ind_data->slot_status[i].atr_len);
      }
    }

    if (ind_data_ptr->slot_eid_info_valid &&
        ind_data_ptr->slot_eid_info_len == ind_data_ptr->physical_slot_status_len)
    {
      for (i = 0; i < ind_data->no_of_slots && i < QMI_UIM_MAX_SLOT_COUNT; i++)
      {
        ind_data->slot_status[i].eid_len =
          MIN(MIN(ind_data_ptr->slot_eid_info[i].eid_len, QMI_UIM_MAX_EID_LEN),
              QMI_UIM_EID_LEN_V01);

        if(ind_data->slot_status[i].eid_len)
        {
          memcpy(ind_data->slot_status[i].eid,
                 ind_data_ptr->slot_eid_info[i].eid,
                 ind_data->slot_status[i].eid_len);
        }
      }
    }
...
)
{
  ...
  /*set procedure bytes*/
  qmi_request->procedure_bytes_valid = TRUE;
  qmi_request->procedure_bytes       = (uim_apdu_procedure_bytes_enum_v01)params->procedure_bytes;
...
)
{
  ...
  /* update Access Point Network info */
  if(params->apn.data_len != 0 && params->apn.data_len <= QMI_UIM_APN_NAME_MAX_V01)
  {
    qmi_request->apn_name_valid = 1;
    qmi_request->apn_name_len   = params->apn.data_len;
    memcpy(qmi_request->apn_name, params->apn.data_ptr, qmi_request->apn_name_len);
  }
