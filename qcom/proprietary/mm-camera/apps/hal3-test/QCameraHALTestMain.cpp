...
#else
#define CAMERA_HAL_LIBERAY "/vendor/lib/hw/camera.sdm660.so"
#endif
...
void print_version()
{
    ...
    if (buf) {
        snprintf(buf, (sizeof(char)*1024) - 1, "\
            ======================= Camera Test Version  =======================\n\
             CAMTEST_SHA1    : %s\n\
             CAMTEST_BUILD_TS: %s\n\
             CAMTESTHOSTNAME : %s\n\
...
int main(int argc, char *argv[])
{
    ...
    while (!stop){
        ...
        case 'a':{
            ...
            if (ops[0] == 'X')
            {
                printf("secure mode enable\n");
                testConf->mIsSecureMode = true;
            }
            ...
            case TESTMODE_VIDEO_PREVIEW:{
                ALOGD("TESTMODE_VIDEO_PREVIEW");
                QCameraHAL3TestVideo* testVideo = new QCameraHAL3TestVideo(CamModule,
                    Camera[CurCameraId]->mDevice,testConf);
                testVideo->PreinitVideoPreviewStreams();
                testVideo->mDevice->openCamera();
                testVideo->run();
                HAL3Test[testConf->mCameraId] = testVideo;
                break;
        ...
        case 'P':{
            ...
            if (HAL3Test[CurCameraId]->mConfig->mTestMode == TESTMODE_SNAPSHOT) {
                ...
                testVideo->dumpPreview(num);
                testVideo->dumpVideo(num);
            } else if (HAL3Test[CurCameraId]->mConfig->mTestMode == TESTMODE_VIDEO_PREVIEW) {
                ALOGI("Dump Preview and video");
                QCameraHAL3TestVideo* testVideo =
                    (QCameraHAL3TestVideo *)HAL3Test[CurCameraId];
                // Have 2 preview stream
                testVideo->dumpPreview(num);
                testVideo->dumpVideo(num);
            }
            break;
