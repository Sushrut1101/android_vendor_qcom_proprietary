...
public:
    ...
    GARDEN_RESULT gnssIfaceAPIUpdateSvTypeConfig(uint64_t enabledMask, uint64_t disabledMask);
    GARDEN_RESULT gnssIfaceAPIGetSvTypeConfig();
    GARDEN_RESULT gnssIfaceAPIResetSvTypeConfig();

private:
    static const GnssInterface* loadLocationInterface(const char* library, const char* name);
