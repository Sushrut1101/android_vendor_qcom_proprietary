/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
Copyright (c) 1998-2008, 2017, 2018 Qualcomm Technologies, Inc.
All Rights Reserved.
      ...
      */
      if( cl_bd_ptr->message_id.udh_present == TRUE )
      {
       ...
       /* Concatenate the result of encoding headers with the Characters */
        if( cl_bd_ptr->user_data.encoding == WMS_ENCODING_GSM_7_BIT_DEFAULT &&
            cl_bd_ptr->user_data.number_of_digits < WMS_CDMA_USER_DATA_MAX)
        {
    ...
    for( i = 0; i < cl_bd_ptr->scpt_data_ptr->num_entries; i ++ )
    {
      ...
      */
      ...
      if( digit_size == 16 )
      {
        ...
        {
          if (k > WMS_BC_MM_SRV_LABEL_SIZE - 2)
          {
            break;
          }
...
(
  ...
  wms_client_bd_s_type               * cl_bd_ptr       /* OUT */
    ...
    return wms_ts_decode_CDMA_bd_from_uint8(raw_bd_ptr->data, raw_bd_ptr->len,
            is_unpacked_user_data, decode_other, cl_bd_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*---- checking ---- */
  if ( p_bd_data == NULL || cl_bd_ptr == NULL )
  {
    LOGE("wms_ts_decode_CDMA_bd null ptr!");
    return WMS_NULL_PTR_S;
  }

  bd_len = (uint8) MIN(len,WMS_MAX_LEN);
  bd_data = (uint8*) p_bd_data;
...
} /* end of wms_ts_decode_CDMA_bd_from_uint8() */
...
//#ifdef FEATURE_CDSMS
  ...
  if ( ud == NULL || buf == NULL)
  {
      LOGE("wms_ts_unpack_ascii null ptr!",0,0,0);
    return 0;
  }
