...
Copyright (c)2007-2012, 2014-2019 Qualcomm Technologies, Inc.
All Rights Reserved.
...
extern int use_qmdl2_v2 = 0;
extern int use_qmdl2_v2_hdlc_disable = 0;
extern int diagid_guid_mapping = 0;
...
static int diag_reconfigure_masks(int proc_type);
...
static void usage(char *progname)
{
	...
	printf("\n-h, --help:\t usage help\n");
	printf("\n-i, --adpl\t Enable ADPL logging over ODL\n");
...
static void parse_args(int argc, char **argv)
{
	...
	while ((command = getopt_long(argc, argv, "f:m:l:o:p:s:w:n:t:x:g:q:j:cdkebrahiu", longopts, NULL))
			!= -1) {
		...
		switch (command) {
			...
			case 'l':
				...
				break;
			case 'm':
				strlcpy(mask_file_proc[MDM], optarg, FILE_NAME_LEN);
...
static void stop_mdlog(int pid)
{
	int fd;
	int ret;
	char pid_buff[10];
...
int main(int argc, char *argv[])
{
	...
	int diag_fd_temp = DIAG_INVALID_HANDLE, pid_read = 0, local_device_mask = 1;
	...
	if (peripheral_mask) {
		...
		query_pid.pd_mask = 0;
	} else if (peripheral_id_mask) {
		if (peripheral_id_mask > DIAG_CON_NONE &&
			peripheral_id_mask < DIAG_CON_ALL) {
			query_pid.peripheral_mask = peripheral_id_mask;
			query_pid.pd_mask = 0;
		} else if (peripheral_id_mask >= DIAG_CON_UPD_WLAN &&
					peripheral_id_mask < DIAG_CON_UPD_ALL) {
			query_pid.peripheral_mask = 0;
			query_pid.pd_mask = peripheral_id_mask;
		}
	} else if (upd_mask != 0) {
		query_pid.peripheral_mask = 0;
...
fall_back_con_all:
	...
	if (status)
		goto fail_deinit;
	diag_get_diag_id_info();
	diag_wait_for_diag_id_info();
	if (qdss_mask) {
		/****************************************************
