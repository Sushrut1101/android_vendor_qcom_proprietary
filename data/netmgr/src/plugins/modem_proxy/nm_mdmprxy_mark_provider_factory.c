...
static qmi_client_type   qmi_dfs_clnt_hndl = NETMGR_QMI_CLIENT_INVALID;
...
{
  static modem_caps_t  modem_caps = CAPABILITY_INVALID;
  int rc;
  dfs_get_capability_req_msg_v01  request;
  dfs_get_capability_resp_msg_v01 response;

  /* If querying modem caps is done before, just return the result directly. */
  if (modem_caps != CAPABILITY_INVALID)
  {
    goto bail;
  }
...
bail:
  if (qmi_dfs_clnt_hndl)
  {
    ...
    qmi_dfs_clnt_hndl = NETMGR_QMI_CLIENT_INVALID;
