/*============================================================================
 * Copyright (c) 2013-2014, 2016, 2018-2019 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
JNIEXPORT jint JNICALL Java_com_qualcomm_listen_ListenSoundModel_verifyUserRecording
                 (JNIEnv *env, jclass clazz __unused,  // ListenSoundModel
                  ...
                  jobject rec )               // ShortBuffer
{
   ...
   if (modelInfo.version >= SM_VERSION_2p0) {
      ...
      status = verifyUserRecording(&keywordModel, kwString,
                 ...
                 jstring userName)
{
   ...
   if (modelInfo.version >= SM_VERSION_2p0) {
      ...
      status = getSoundModelHeader(&keywordModel, &modelHeader);
      if (kSucess != status) {
      ...
      status = getUserKeywordModelSize(&keywordModel,
...
cleanup_for_get_size_when_extended:
   ...
   if (modelInfo.version >= SM_VERSION_2p0) {
      status = releaseSoundModelHeader(&modelHeader);
      if (kSucess != status) {
                 ...
                 jobject qualityData)        // ListenTypes$ConfidenceData
{
   ...
   //    allocate array of listen_user_recording userRecsArray[]
   //    then place into i-th element of userRecordings struct array
   userRecsArray = (listen_user_recording **)calloc(nRecs, sizeof(listen_user_recording *));
   if (NULL == userRecsArray) {
      ALOGE("_extend: ERROR listen_user_recording[] allocation failed");
   ...
   for (uint32_t i=0; i < nRecs; i++) {
       ...
       userRecsArray[i] = (listen_user_recording *)calloc(1, sizeof(listen_user_recording));
       if (NULL == userRecsArray[i]) {
          ALOGE("_extend: ERROR listen_user_recording[%d] allocation failed", i);
          retStatus = LISTEN_ENO_MEMORY;
          goto cleanup_for_extend;
       }
   ...
   if (modelInfo.version >= SM_VERSION_2p0) {
      ...
      status = createUserKeywordModel(&keywordModel,
               kwString, userString,
 ...
 *          LISTEN_EFAILURE
 */
JNIEXPORT jint JNICALL Java_com_qualcomm_listen_ListenSoundModel_verifyUdkRecording__Ljava_nio_ByteBuffer_2Ljava_nio_ShortBuffer_2
            ...
            jobject jUserRecording) //short buffer
{
    ...
    ALOGV("call checkUserRecording");
    status = checkUserRecording(&languageModel, NULL,
 ...
 * Class:     com_qualcomm_listen_ListenSoundModel
 * Method:    verifyUdkRecording
 * Signature: (Ljava/nio/ByteBuffer;I[Ljava/nio/ShortBuffer;)I
 *
 * Returns the detection algorithm's confidence that the last user recording
 * is viable for creating a user-defined keyphrase sound model.
 * Consistency among all user recordings will also be checked.
 *
 * Param [in]  env - ptr to JNIEnv
 * Param [in]  clazz - java class (not an object instance) of this static method
 * Param [in]  jLanguageModel - contains language data for a specific language
 * Param [in]  jUserRecordings  - an array of recordings of user speaking keyword
 *
 * Return - percent confidence level that user recording matches keyword
 *          Negative error number is returned if error occurred.
 *          LISTEN_EBAD_PARAM
 ...
 *          LISTEN_ENO_MEMORY
 */
JNIEXPORT jint JNICALL Java_com_qualcomm_listen_ListenSoundModel_verifyUdkRecording__Ljava_nio_ByteBuffer_2_3Ljava_nio_ShortBuffer_2
    (JNIEnv *env, jclass clazz __unused,
            jobject jLanguageModel,
            jobjectArray jUserRecordings)
{
    listen_language_model_type      languageModel;
    listen_user_recording **        userRecordings = NULL;
    float                           snr;
    jint                            jSnr = 0;
    listen_status_enum              status = kSucess;
    listen_status_enum_t            eStatus = LISTEN_EFAILURE;
    ...
    ALOGV("_verifyUdkRecording_1 entered");
    if ( (NULL == jLanguageModel) || (NULL == jUserRecordings) ) {
        ALOGE("_verifyUdkRecording_1: ERROR Null input ptrs");
        return LISTEN_EBAD_PARAM;
    }

    shortBufferClazz = env->FindClass("[Ljava/nio/ShortBuffer;");
    if (NULL == shortBufferClazz) {
       ALOGE("_verifyUdkRecording_1: ERROR FindClass([java/nio/ShortBuffer) failed");
       return LISTEN_EFAILURE;
    }

    if( !(env->IsInstanceOf(jUserRecordings, shortBufferClazz))){
        ALOGE("_verifyUdkRecording: ERROR jUserRecordings is not an ShortBuffer[] instance");
        return LISTEN_EBAD_PARAM;
    }

    nRecs = env->GetArrayLength(jUserRecordings);
    if( 0 == nRecs) {
        ALOGE("_verifyUdkRecording_1: ERROR zero recordings");
        return LISTEN_EBAD_PARAM;
    }

    userRecordings = (listen_user_recording **)calloc(nRecs, sizeof(listen_user_recording *));
    if (NULL == userRecordings) {
        ALOGE("_verifyUdkRecording_1: ERROR listen_user_recording[] allocation failed");
    ...
    eStatus = getLanguageModelFromByteBuff(env, jLanguageModel, &languageModel.size, &languageModel.data);
    if (LISTEN_SUCCESS != eStatus) {
       ALOGE("_verifyUdkRecording_1: ERROR getLanguageModelFromByteBuff returned %d", eStatus);
       goto cleanup_for_verify_rec;
    }
    ALOGV("_verifyUdkRecording_1: user independent SM (ptr,size)=(%p,%d), nRecs=%d",
            languageModel.data, languageModel.size, nRecs);

    for (size_t i = 0; i < nRecs; i++) {
        userRecordings[i] = (listen_user_recording *)calloc(1, sizeof(listen_user_recording));
        if (NULL == userRecordings[i]) {
          ALOGE("_verifyUdkRecording_1: ERROR listen_user_recording[%d] allocation failed", i);
          retStatus = LISTEN_ENO_MEMORY;
          goto cleanup_for_verify_rec;
        }

        jobject userRecObj =  env->GetObjectArrayElement(jUserRecordings, i);
        if (NULL == userRecObj) {
           ALOGE("_verifyUdkRecording_1: ERROR GetObjectArrayElement of jUserRecordings[%d] failed", i);
           retStatus = LISTEN_EBAD_PARAM;
           goto cleanup_for_verify_rec;
        }

        // Extract data and numsamples from recording jobject & put in userRecording struct.
        eStatus = getRecFromShortBuff(env, userRecObj, &userRecordings[i]->n_samples, &userRecordings[i]->data);
        if ( LISTEN_SUCCESS != eStatus) {
           ALOGE("_verifyUdkRecording_1: ERROR getSMFromShortBuff returned %d", eStatus);
           goto cleanup_for_verify_rec;
        }
        ALOGV("_verifyUdkRecording_1: user recording[%d] (ptr,size)=(%p,%d)",
               i, userRecordings[i]->data, userRecordings[i]->n_samples);
    }

    // Call SML function equivalent checkRecordingsQuality.
    // Passing null for EPD uses default EPD params.
    ALOGD("_verifyUserRecording_1: call checkRecordingsQuality");
    status = checkRecordingsQuality(&languageModel, NULL,
            nRecs, userRecordings, &snr);
    if (kSucess != status) {
        ALOGE("_verifyUdkRecording_1: ERROR checkRecordingsQuality() failed status %d", (int)status);
        MAP_ERR(status, eStatus);
    } else {
        jSnr = (jint)snr;
        ALOGD("_verifyUserRecording_1: checkRecordingsQuality snr %d", (int)jSnr);
    }
...
cleanup_for_verify_rec:
    ...
    if (userRecordings) {
       for (size_t i = 0; i < nRecs; i++) {
          if (userRecordings[i]) {
              if (userRecordings[i]->data != NULL)
                 free((void *) userRecordings[i]->data);
              free((void *)userRecordings[i]);
          }
       }
       free((void *)userRecordings);
    }
    // return error is error occurred otherwise return confidence level
    if (LISTEN_SUCCESS != eStatus) {
        ALOGE("_verifyUdkRecording_1: returns error %d", eStatus);
        return eStatus;
    }
    return (jint) jSnr;
}
          ...
          jobject jLanguageModel) // ByteBuffer
{
    ...
    //    allocate array of listen_user_recording userRecsArray[]
    //    then place into i-th element of userRecordings struct array
    userRecsArray = (listen_user_recording **)calloc(numberUdkRecordings, sizeof(listen_user_recording *));
    if (NULL == userRecsArray) {
      ALOGE("_getUdkSmSize: ERROR listen_user_recording[] allocation failed");
      retStatus = LISTEN_ENO_MEMORY;
      goto cleanup_for_get_udk_sm_size;
    }
    ...
    for (uint32_t i=0; i < numberUdkRecordings; i++) {
        ...
        userRecsArray[i] = (listen_user_recording *)calloc(1, sizeof(listen_user_recording));
        if (NULL == userRecsArray[i]) {
            ALOGE("_getUdkSmSize: ERROR listen_user_recording[%d] allocation failed", i);
            retStatus = LISTEN_ENO_MEMORY;
            goto cleanup_for_get_udk_sm_size;
        }
    ...
    status = getUserDefinedKeywordSize(NULL, kwString, userString,
          ...
          jobject        jConfidenceData) // ListenTypes$ConfidenceData
{
    ...
    userRecsArray = (listen_user_recording **)calloc(nRecs, sizeof(listen_user_recording *));
    if (NULL == userRecsArray) {
      ALOGE("_createUdkSm: ERROR listen_user_recording[] allocation failed");
      retStatus = LISTEN_ENO_MEMORY;
      goto cleanup_for_create_udk_sm;
    }
    ...
    for (uint32_t i=0; i < nRecs; i++) {
       ...
       userRecsArray[i] = (listen_user_recording *)calloc(1, sizeof(listen_user_recording));
       if (NULL == userRecsArray[i]) {
          ALOGE("_createUdkSm: ERROR listen_user_recording[%d] allocation failed", i);
          retStatus = LISTEN_ENO_MEMORY;
          goto cleanup_for_create_udk_sm;
       }
    ...
    status = createUserDefinedKeywordModel(NULL, kwString, userString,
                 ...
                 jobject outDataStruct)          // ListenTypes$VoiceWakeupDetectionDataV2
{
   ...
   memset(&modelHeader, 0, sizeof(listen_sound_model_header));
   status = getSoundModelHeader(&soundModel, &modelHeader);
   if (kSucess != status) {
   ...
   status = parseDetectionEventData (
   ...
   status = getKeywordPhrases(&soundModel, &numKWs, keywordPhrases);
   if (kSucess != status) {
   ...
   if (numUsers > 0) {
      ...
      status = getUserNames(&soundModel, &numUsers, userNames);
      if (kSucess != status) {
                 ...
                 jobject soundModelInfo)         // ListenTypes$SoundModelInfo
{
   ...
   if (modelInfo.type == 4) {
      ...
      // SVA 3.0 Big sound model
      if (modelInfo.version == VERSION_0300)
          version = VERSION_0300;
      else
          version = VERSION_0400;
   }
                 ...
                 jobjectArray soundModelsArr)    // array of ByteBuffer
{
   ...
   status = getMergedModelSize(numSoundModels, pInputModels, &sizeOfMergedModel );
   if (kSucess != status) {
                 ...
                 jobject      mergedModel)       // ByteBuffer
{
   ...
   status = mergeModels(numSoundModels, pInputModels, &outputModel );
   if (kSucess != status) {
                 ...
                 jstring         user)
{
   ...
   //
   memset(&modelHeader, 0, sizeof(listen_sound_model_header));
   status = getSoundModelHeader(&model, &modelHeader);
   if (kSucess != status) {
   ...
   status = getSizeAfterDeleting(&model,
                 ...
                 jobject         outSoundModel )  // ByteBuffer
{
   ...
   memset(&modelHeader, 0, sizeof(listen_sound_model_header));
   status = getSoundModelHeader(&inputModel, &modelHeader);
   if (kSucess != status) {
   ...
   status = getSizeAfterDeleting(&inputModel,
   ...
   // call LSMLib function
   status = deleteFromModel(&inputModel,
