...
static void usage(char *progname)
{
	...
	printf("  -p, --pcie_mode: test suite for pcie mode switch\n");
	printf("  -h, --help:    help\n");
...
static void parse_args(int argc, char **argv)
{
	...
	while ((command = getopt_long(argc, argv, "t:p:vh", longopts, NULL)) != -1) {
		switch (command) {
		case 't':
		...
		case 'v':
			...
			break;
		case 'p':
			pcie_test = atoi(optarg);
			if (pcie_test != PCIE_MODE && pcie_test != USB_MODE) {
				printf("test_diag: Invalid switch mode, %d\n", pcie_test);
				exit(0);
			}
			break;
		case 'h':
...
int main(int argc, char *argv[])
{
	...
	if (pcie_test) {
		if (pcie_test == USB_MODE)
			logging_mode = PCIE_MODE;
		diag_switch_logging(pcie_test, NULL);
		printf("test_diag: Switched to mode %s\n",
		(pcie_test == 1 ? "USB" : "PCIE"));
	}
