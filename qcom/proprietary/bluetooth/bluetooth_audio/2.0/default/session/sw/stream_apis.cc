...
using ::vendor::qti::bluetooth_audio::BluetoothAudioPortOut;
using ::vendor::qti::bluetooth_audio::utils::GetAudioParamString;
using ::vendor::qti::bluetooth_audio::utils::ParseAudioParams;
...
constexpr unsigned int kMaximumOutWriteSleepTimeMs = 15;
...
static int out_dump(const struct audio_stream* stream, int fd __unused) {
  const auto* out = reinterpret_cast<const BluetoothStreamOut*>(stream);
  LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState();
  return 0;
}
...
static uint32_t out_get_latency(const struct audio_stream_out* stream,
  ...
  if ((delay_report_ns / 1000000) <= 0)
    sink_latency = 200;
  else
    sink_latency = (uint32_t)(delay_report_ns / 1000000);

  LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState()
               << ", latency_ms + sink_latency=" << latency_ms + sink_latency;
  return latency_ms + sink_latency;
}
...
static ssize_t out_write(struct audio_stream_out* stream, const void* buffer,
  ...
  lock.unlock();
  sent = out->bluetooth_output_.WriteData(buffer, bytes);
  lock.lock();
...
finish:
  ...
  if (sent == -1) {
    ...
    if (sleep_time > 0) {
      int64_t effective_sleep_time = sleep_time;
      if (effective_sleep_time > kMaximumOutWriteSleepTimeMs * 1000) {
        effective_sleep_time = kMaximumOutWriteSleepTimeMs * 1000;
      }

      LOG(INFO) << __func__ << ": calculated sleep time is" << (sleep_time / 1000)
                <<" ms, effectively sleep " << (effective_sleep_time / 1000)
                << " ms when writting FMQ datapath";
      lock.unlock();
      usleep(effective_sleep_time);
      lock.lock();
    } else {
...
static int out_get_presentation_position(const struct audio_stream_out* stream,
  ...
  uint64_t latency_frames = ((uint64_t)out_get_latency(stream, timestamp) * out->sample_rate_) / 1000;
  if (out->frames_presented_ >= latency_frames) {
    clock_gettime(CLOCK_MONOTONIC, timestamp);
    *frames = out->frames_presented_ - latency_frames;
    LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState()
...
int open_output_stream(struct audio_hw_device* dev,
                            audio_io_handle_t handle __unused, audio_devices_t devices,
  ...
  *stream_out = nullptr;
  ...
  struct bluetooth_audio_device* bt_dev =
                    reinterpret_cast<bluetooth_audio_device*>(dev);
  ...
  *stream_out = &out->stream_out_;
  bt_dev->output = reinterpret_cast<audio_stream_out*>(out);
  LOG(INFO) << __func__ << ": state=" << out->bluetooth_output_.GetState()
...
void close_output_stream(struct audio_hw_device* dev,
  ...
  LOG(VERBOSE) << __func__ << ": state=" << out->bluetooth_output_.GetState()
               << ", stopped";
  bt_dev->output = nullptr;
  delete out;
}

size_t stream_input_buffer_size(const struct audio_hw_device* dev __unused,
                                  const struct audio_config* config __unused) {
  return 320;
}

int open_input_stream(struct audio_hw_device* dev __unused,
...
void close_input_stream(struct audio_hw_device* dev __unused,
                             struct audio_stream_in* stream_in __unused) {}
