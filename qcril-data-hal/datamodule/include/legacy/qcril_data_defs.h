...
/*===========================================================================
    Copyright (c) 2011,2012,2015,2017,2018 Qualcomm Technologies, Inc.
    All Rights Reserved.
...
===========================================================================*/
/* According to Reference T3380 [3GPP TS24.008 section 6.1.3.1.5] PDN bringup
   timeout at NAS layer in modem is (30 seconds*5).
   Default timeout in Qcril Data is slightly more than 150.( 150+5 = 155)
*/
...
// \todo this may need to be defined by framework
...
} qcril_data_pcscf_addr_string_t;
...
#define DS_CALL_INFO_DEV_NAME_MAX_LEN 12 /* Device name format: rmnetXX or rmnet_sdioXX */
  ...
  qcril_data_pcscf_addr_string_t pcscf;
...
typedef struct
{
  ...
  char  ril_profile[QCRIL_IP_FAMILY_STR_MAX + 1];

  char  ril_apn[DS_CALL_INFO_APN_MAX_LEN];

  char  ril_user[QMI_WDS_USER_NAME_MAX_V01 + 1];

  char  ril_pass[QMI_WDS_PASSWORD_MAX_V01 + 1];

  char  ril_auth_pref[QCRIL_DATA_AUTH_PREF_MAX_LEN];

  char  ril_ipfamily[QCRIL_IP_FAMILY_STR_MAX + 1];

  char  ril_ip_roaming[QCRIL_IP_FAMILY_STR_MAX + 1];

  char  ril_apn_types[QCRIL_APN_TYPES_STR_MAX];

  char  ril_roaming_allowed[QCRIL_ROAMING_ALLOWED_STR_MAX];

  char  ril_access_network[QCRIL_ROAMING_ALLOWED_STR_MAX];
...
typedef struct
{
  ...
  dsi_hndl_t               dsi_hndl;
  /* set to either qmi_wds_hndl_v4 or qmi_wds_hndl_v6 */
  qmi_client_type          qmi_wds_hndl;
  /* flag to indicate whether dev_name and call_index is valid */
  unsigned char            info_flg;

  /* store separate handles for v4 and v6 call */
  qmi_client_type          qmi_wds_hndl_v4;
  qmi_client_type          qmi_wds_hndl_v6;
...
typedef struct qcril_data_call_response_s
{
  ...
  OmhProfileInfo omh_profile_info;
...
/* Modem assisted KeepAlive types */

/**
 * Types used by the Keepalive request
 */
 typedef uint32_t qcril_data_ka_v4_address_t;
 typedef uint16_t qcril_data_ka_port_t;

 /**
 * @brief     Modem assisted keep alive types
 *            Options: UNKNOWN, NATV4, NATV6
 *            Mirrors wds_keep_alive_type_enum_v01
 */
typedef enum {
  QCRIL_DATA_MA_KA_TYPE_UNKNOWN = -2147483647,
  QCRIL_DATA_MA_KA_TYPE_NAT_V4 = 0,
  QCRIL_DATA_MA_KA_TYPE_NAT_V6 = 1,

}qcril_data_ma_keepalive_t ;

typedef struct {
  qcril_data_ma_keepalive_t ka_type;
  ...
  uint32_t call_id;
}qcril_data_keepalive_request_t;

/**
 * Type for the status of the Modem assisted keepalive timer.
 */
typedef enum {
  QCRIL_DATA_KA_STATUS_NONE, /* not started, failed, stopped */
  QCRIL_DATA_KA_STATUS_REQUESTED, /* requested, and awaiting indication from modem */
  QCRIL_DATA_KA_STATUS_RUNNING,
} qcril_data_ma_ka_status_t;
...
typedef enum {
  lce_report_off,
  lce_report_on
} qcril_data_lce_rpt_state;

typedef struct {
  int32_t hysteresisMs;
  int32_t hysteresisDlKbps;
  int32_t hysteresisUlKbps;
  int32_t thresholdsDownlinkKbps[QMI_OTT_DL_THRESHOLD_LIST_LEN_V01];
  int thresholdsDownlinkKbps_size;
  int32_t thresholdsUplinkKbps[QMI_OTT_UL_THRESHOLD_LIST_LEN_V01];
  int thresholdsUplinkKbps_size;
  ott_rat_enum_type_v01 ran;
} qcril_data_lce_rpt_criteria;

/**
 * Element that is used to cache data related to
 * modem adssisted keepalive requests.
 */
typedef struct qcril_data_ma_ka_list_elem_t{
    RIL_Token token;
    RIL_KeepaliveType type;
    char sourceAddress[MAX_INADDR_LEN+1];      /* Source address in network-byte order */
    int sourcePort;                          /* Source port if applicable, or 0x7FFFFFFF;
                                                the maximum value is 65535 */
    char destinationAddress[MAX_INADDR_LEN+1]; /* Destination address in network-byte order */
    ...
    struct qcril_data_ma_ka_list_elem_t *next;
