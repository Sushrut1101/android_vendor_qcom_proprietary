...
void *platform_stdev_init(sound_trigger_device_t *stdev)
{
    ...
    audio_hw_acdb_init_v2_t audio_hw_acdb_init_v2 = NULL;
    ...
    if (my_data->audio_hw_open_snd_mixer) {
        ...
        ALOGE("%s: ERROR. dlsym error %s for audio_extn_utils_open_snd_mixer", __func__, dlerror());
    ...
    audio_hw_acdb_init_v2 = (audio_hw_acdb_init_v2_t)dlsym(stdev->audio_hal_handle,
                                              "acdb_init_v2");
    if (audio_hw_acdb_init_v2 == NULL) {
        ALOGE("%s: ERROR. dlsym error %s for acdb_init_v2", __func__, dlerror());
    ...
    if (audio_hw_acdb_init_v2) {
        ALOGD("call audio_hw_acdb_init_v2 ");
        ret = audio_hw_acdb_init_v2(stdev->mixer);
        if (ret) {
             ALOGE("%s: ERROR. audio_hw_acdb_init_v2 failed status %d", __func__, ret);
             goto cleanup;
        }
    } else if (audio_hw_acdb_init) {
