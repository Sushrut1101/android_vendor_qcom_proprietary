...
/*Remove entry from AdaptLaunchState db*/
void FeatureState::deleteRow(std::string appName){
    if(!mStateDb) {
        DEBUGE(LOG_TAG_FS,"%s:Fatal: StateDb is NULL", mFeatureName.c_str());
        return;
    }
    LMDBDataRowKey key(appName);
    DbRowVect dbRow = mStateDb->readAllocateDR<AppState>(key, 0);
    if (dbRow.size() == 1) {
        DEBUGV(LOG_TAG_FS,"Deleting entry from adaptlaunchstate");
        mStateDb->deleteDeallocateDR<AppState>(dbRow);
        return;
    }
    DEBUGE(LOG_TAG_FS,"Entry not found in db");
    return;
}
