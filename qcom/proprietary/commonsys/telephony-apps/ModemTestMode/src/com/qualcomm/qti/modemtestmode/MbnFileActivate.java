...
public class MbnFileActivate extends Activity {
    ...
    private Handler mHandler = new Handler() {
        ...
        public void dispatchMessage(Message msg) {
            ...
            case EVENT_QCRIL_HOOK_READY:
                ...
                if (!getMbnInfo()) {
                    showConfigPrompt();
                } else if (MbnAppGlobals.getInstance().getSelectMode().equals(MbnTestUtils.AUTO_MODE)
                        || MbnAppGlobals.getInstance().getSelectMode().equals(MbnTestUtils.CT_MODE)) {
                    setAutoModeView();
                }
                break;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = this;
        if (MbnAppGlobals.getInstance().getSelectMode().equals(MbnTestUtils.MANUAL_MODE)
            || MbnAppGlobals.getInstance().getSelectMode().equals(MbnTestUtils.DEFAULT_MODE)) {
    ...
    private boolean getMbnInfo() {
        ...
        String mbn2 = MbnAppGlobals.getInstance().getMbnConfig(1);
        String meta2 = MbnAppGlobals.getInstance().getMetaInfoForConfig(mbn2);
        ...
        mCurMbn2 = new MbnMetaInfo(mContext, mbn2, meta2);
        ...
        log("Current Sw Mbn in slot2:" + mCurMbn2);
    ...
    private void setAutoModeView() {
        TextView hwMbnTextView = (TextView) findViewById(R.id.hw_mbn_id);
        hwMbnTextView.setText(getString(R.string.mbn_id) + " " +mCurHwMbn.getMbnId());

        TextView swMbnTextView = (TextView) findViewById(R.id.sw_mbn_id);

        swMbnTextView.setText(getString(R.string.mbn_id) + " "
            + getString(R.string.sub1) + " " + mCurMbn.getMetaInfo() + "\n"
            + "           " + getString(R.string.sub2) + " " + mCurMbn2.getMetaInfo());
    }
