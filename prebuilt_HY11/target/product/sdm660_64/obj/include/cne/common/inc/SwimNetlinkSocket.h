...
public:
   ...
   * SIDE EFFECTS  None
   *--------------------------------------------------------------------------*/
  int NetlinkSend( struct iovec& iov );
  ...
  int getFd( ) const
  {
    ...
    /*----------------------------------------------------------------------------
   * FUNCTION      Accessor for Netlink token
   *
   * DESCRIPTION   Used for getting Netlink token
   *
   * DEPENDENCIES  None
   *
   * RETURN VALUE  None
   *
   * SIDE EFFECTS  Netlink token will be increased by 1 for each calling
   *--------------------------------------------------------------------------*/
  int getNextToken()
  {
      return ++nltoken;
