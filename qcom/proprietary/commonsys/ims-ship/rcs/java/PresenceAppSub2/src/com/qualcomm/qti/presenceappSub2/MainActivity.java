...
public class MainActivity extends ListActivity {
    private final static String TAG = "Presence_UI2: MainActivity";
    ...
    private void getDefaultIccId() {
       ...
       if (subInfoList != null && subInfoList.size() > 1) {
           ...
           AppGlobalState.mIccId = subInfoList.get(1).getIccId();
           AppGlobalState.mSubId = subInfoList.get(1).getSubscriptionId();
           AppGlobalState.mPhoneid = SubscriptionManager.getPhoneId(AppGlobalState.mSubId);
           
           //return;
       }
       else {
       Log.d(TAG, "Subinfo List is null, SIM not inserted");
       }
       }
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try
        {
            ...
            Log.d(TAG, "before my prop is:");
            String sMode = reader.readLine();
            Log.d(TAG, "my prop is: " + sMode);
            ...
            if (sMode != null)
            {
                Log.d(TAG, "LauncherrActivity : OnCreate : sMode " + sMode);
                AppGlobalState.setOperatorMode(Integer.parseInt(sMode));
                Log.d(TAG, "LauncherrActivity : OnCreate : getOperatorMode " + AppGlobalState.getOperatorMode());
            }
            ...
            Log.d(TAG, "before my sModeCd is:");

            String sModeCd = readerCd.readLine();
            if(sModeCd != null && sModeCd.toString().equals("")) {
               sModeCd = "0";
            }
            Log.d(TAG, "After my sModeCd is: " + sModeCd);

            if (sModeCd != null && Integer.parseInt(sModeCd) == 1)
            {
                Log.d(TAG, "LauncherrActivity : OnCreate : sModeCd " + sModeCd);
                AppGlobalState.setCdByOption(true);
                Log.d(TAG, "LauncherrActivity : OnCreate : isCdByOption " + AppGlobalState.isCdByOption());
            }
            else
            {
        ...
        if(AppGlobalState.mPhoneid != AppGlobalState.INVALID_PHONE_ID) {
        ImsManager imsManager = ImsManager.getInstance(mContext,AppGlobalState.mPhoneid);
        if (imsManager != null) {
           Settings.isVt_calling_enabled = imsManager.isVtEnabledByUser();
        ...
        receiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
             ...
             Log.d(TAG, "intent.getAction():"+intent.getAction().toString());

                if(intent.getAction() != null && intent.getAction().equals("RCS_SERVICE_AVAILABLE"))
                {
                    Log.d(TAG, "MainActivity : onCreate : onReceive  RCS_SERVICE_AVAILABLE");
                    if(!isIMSServBound)
                    {
                        bindSettingService();
                        Log.d(TAG, "After Bind service isIMSServBound "+isIMSServBound);
                    }
                    }
                if(intent.getAction() != null && intent.getAction().equals("RCS_SERVICE_UNAVAILABLE"))
                {
                    Log.d(TAG, "MainActivity : onCreate : onReceive  RCS_SERVICE_UNAVAILABLE");

                }

                if(intent.getAction() != null && intent.getAction().equals(ImsUceManager.ACTION_UCE_SERVICE_DOWN))
                {
                    Log.d(TAG, "MainActivity : ACTION_UCE_SERVICE_DOWN");
                    releaseServices();
                ...
                {
                    //ToDo: RcsService died retry service connection
                    Log.d(TAG, "MainActivity : ACTION_UCE_SERVICE_UP");
                    bindRcsService();
                }
            }
        };
        ...
        if (NewLaunch == 1)
        {
            ...
            Log.d(TAG, "LAUNCHING NOW... NewLaunch = " + NewLaunch);
            ...
            Log.d(TAG, "MainActivity : onCreate : telephonyManagerNetworkType " + telephonyManagerNetworkType);

            switch (telephonyManagerNetworkType)
            {
                case TelephonyManager.NETWORK_TYPE_LTE:
                    Log.d(TAG, "MainActivity : onCreate : NETWORK_TYPE_LTE ");
                    ContactInfo.networkTypeLTE = true;
                    ContactInfo.vopsEnabled = true;
                    break;

                case TelephonyManager.NETWORK_TYPE_EHRPD:
                    Log.d(TAG, "MainActivity : onCreate : NETWORK_TYPE_EHRPD ");
    ...
    void SetCustomFeatureTags() {
        ...
        try
        {
            ...
            Log.d(TAG, "SetCustomFeatureTags :After my sModeCd is: " + sModeCd);
            ...
            e.printStackTrace();
            Log.d(TAG, "RemoteException, Unable to set New Feature Tag");
        }
    }
    ...
    void startPresenceService(IUceService rcsService) {
        try {
            ...
            if ( presenceSerrviceHandle == 0) {
                Log.d(TAG, "MainActivity : Creating New Presence Service");
                presenceSerrviceHandle = rcsService.createPresenceServiceForSubscription(
                                            AppGlobalState.getListenerHandler().presListener,
                                            AppGlobalState.getPresenceListenerHandle(),
                                            AppGlobalState.mIccId);


            } else {
                Log.d(TAG, "MainActivity : PresenceServiceHandle already exists, destroy current handle");
            ...
            Log.d(TAG, "MainActivity : presenceSerrviceHandle[" + presenceSerrviceHandle +"]");
			IPresenceService presService = rcsService.getPresenceServiceForSubscription(AppGlobalState.mIccId);
      if(presService != null){
            Log.d(TAG,"getPresenceService not null | handle: "+ String.valueOf(presenceSerrviceHandle));
            AppGlobalState.setPresenceService(presService);
      }
            AppGlobalState.setPresenceSerrviceHandle(presenceSerrviceHandle);
            SetCustomFeatureTags();
        } catch (RemoteException e) {
            Log.d(TAG, " MainActivity : startOptionsService :  DeadObjectException dialog  ");
            e.printStackTrace();
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    void startOptionsService(IUceService rcsService){
        try {
            ...
            if(optionsSrvHandle == 0) {
                Log.d(TAG, "MainActivity : Creating New Options Service");
                optionsSrvHandle = rcsService.createOptionsServiceForSubscription(
                    AppGlobalState.getListenerHandler().cdServiceListener,
                    AppGlobalState.getOptionsListenerHandle(),
                    AppGlobalState.mIccId);

            } else {
                Log.d(TAG, "MainActivity : OptionsServiceHandle already exists, destroy current handle");
            ...
            Log.d(TAG, "MainActivity : OptionsServiceHandle[" + optionsSrvHandle +"]");
            AppGlobalState.setCdServiceHandle(optionsSrvHandle);
            IOptionsService optionsService = rcsService.getOptionsServiceForSubscription(AppGlobalState.mIccId);
    ...
    void initAllServices(IUceService rcsService) {
        ...
        Log.d(TAG, "MainActivity initAllServices  : start");
    Settings.isVt_calling_enabled = getVtCallingStatus();
    Log.d(TAG,"Settings.isVt_calling_enabled nonVZW" + String.valueOf(Settings.isVt_calling_enabled));
        ...
        try {
            if (rcsService != null) {
                ...
                if (true == serviceStatus) {
                    Log.d(TAG, "MainActivity : Rcs Service Status is TRUE");
                    startPresenceService(rcsService);
                    startOptionsService(rcsService);
                }
             Log.d(TAG, "MainActivity : setMobileDataContentObserver");
            ...
            Log.d(TAG, " MainActivity : initAllServices :  DeadObjectException dialog  ");
            e.printStackTrace();
        }
        catch (Exception e) {
            e.printStackTrace();
  ...
  private ImsConfigListener.Stub imsConfigListener = new ImsConfigListener.Stub() {
      ...
      Log.d(TAG, " imsconfigsettings isVt_calling_enabled_valid " + vtCallEnabledVal);
      ...
      SharedPreferences preferences = mContext.getSharedPreferences("presencedata", Context.MODE_PRIVATE);
                     ...
                     {
             Log.d(TAG,"publish was getting invoked from here | imsconfiglistener");
                     }
  ...
  public void initImsConfigSettings(){
           new Thread(new Runnable(){
         public void run(){
       try{

       //phone id default 0
        Log.d(TAG, " imsconfigsettings | initializing settingsvariable for phone id 0");
        ImsConfig imsConfig = AppGlobalState.getImsConfig();
        if (imsConfig == null)
        {
           Log.d("PRESENCE_UI", " imsconfig is null, not initialized");
           return;
        }
     //mobile data config settings
       int mobDataEnablVal = imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.MOBILE_DATA_ENABLED);
        if(mobDataEnablVal == 1)
          Settings.isMobile_data_enabled = true;
        else
          Settings.isMobile_data_enabled = false;
             Log.d(TAG, " imsconfigsettings isMobile_data_enabled " + mobDataEnablVal);

        //presence settings
    Settings.volteUserOptInStatus = imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.VOLTE_USER_OPT_IN_STATUS);
    Log.d(TAG, " imsconfigsettings isPresenceEnabled " + Settings.volteUserOptInStatus);
            if(AppGlobalState.getOperatorMode() == AppGlobalState.ATT_MODE)
                         {
                         Log.d(TAG, "For ATT , make Presence check  = "
        ...
        //vtcalling settings
             Settings.isVt_calling_enabled = getVtCallingStatus();
             Log.d(TAG,"Settings.isVt_calling_enabled VZW/ATT/TMO" + String.valueOf(Settings.isVt_calling_enabled));
             Log.d(TAG, "Current operator : " + AppGlobalState.getOperatorMode());
             if(AppGlobalState.getOperatorMode() != AppGlobalState.TMO_MODE)
                     {
            Log.d(TAG,"publish was getting invoked from here | nonvzw");
                     }

        Log.d(TAG, " imsconfigsettings isVolte_enabled " + imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.VLT_SETTING_ENABLED ));

        Settings.publishExpiryTimer = imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.PUBLISH_TIMER);
                Log.d(TAG, " imsconfigsettings getPublish_expiry_timer " + Settings.publishExpiryTimer);

                Settings.publishExtendedExpiryTimer = imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.PUBLISH_TIMER_EXTENDED);
          Log.d(TAG, " imsconfigsettings getPublish_extended_expiry_timer " + Settings.publishExtendedExpiryTimer);

        Settings.listSubscriptionExpiryTimer = imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.CAPAB_POLL_LIST_SUB_EXP);
          Log.d(TAG, " imsconfigsettings getCapability_poll_list_subscription_expiry_timer " + Settings.listSubscriptionExpiryTimer);

        Settings.capabilityDiscoveryEnable = imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.CAPABILITY_DISCOVERY_ENABLED);
          Log.d(TAG, " imsconfigsettings isCapability_discovery_enable " + Settings.capabilityDiscoveryEnable);

        Settings.capabilityCacheExpiration = imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.CAPABILITIES_CACHE_EXPIRATION);
          Log.d(TAG, " imsconfigsettings getCapabilites_cache_expiration " + Settings.capabilityCacheExpiration);
        ...
        Settings.availabilityCacheExpiration = imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.AVAILABILITY_CACHE_EXPIRATION);
          Log.d(TAG, " imsconfigsettings getAvailability_cache_expiration " + Settings.availabilityCacheExpiration);

        Settings.capabilityPollInterval = imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.CAPABILITIES_POLL_INTERVAL);
          Log.d(TAG, " imsconfigsettings getCapability_poll_interval " + Settings.capabilityPollInterval);

        Settings.maxSubscriptionListEntries = imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.MAX_NUMENTRIES_IN_RCL);
          Log.d(TAG, " imsconfigsettings getMax_subcription_list_entries " + Settings.maxSubscriptionListEntries);

                Log.d(TAG, "Settings.inSettingScreen "
                    ...
                    Log.d(TAG, " runOnUiThread(updateDataSetting)");
                    Log.d(TAG, "For Video Settings.inSettingScreen "+ Settings.inSettingScreen);
                    ...
                    if (Settings.inSettingScreen) {
                        Runnable updateVideoSetting = new Runnable() {
                            public void run() {
                                try {
                                    Log.d(TAG, " Before updateVideoSetting");
                                    Settings.settingsObject.updateVideoSetting();
                                    Log.d(TAG, " After updateVideoSetting");
                                } catch (Exception e) {
                                    Log.d(TAG, " In Setting screen Exception");
                                    e.printStackTrace();
                                }
                            }
                        };
                        runOnUiThread(updateVideoSetting);
                        Log.d(TAG, " runOnUiThread(updateVideoSetting)");
                    }


           Log.d(TAG, "AppStart = " + AppStart);
                    ...
                    if (AppStart == 1 && (AppGlobalState.getOperatorMode() == AppGlobalState.VZW_MODE)
                        ...
                        try {
                            ...
                            Log.d(TAG, "data from Settings  UI" + data);
                        } catch (Exception e) {
                            // Some problem accessible private API
                        }

                        Log.d(TAG, "Data value from setting UI = " + data);
            int temp;
            if(data)
              temp = 1;
            else
              temp = 0;
                        ...
                        if (imsConfig.getProvisionedValue(ImsConfig.ConfigConstants.MOBILE_DATA_ENABLED) != temp) {
                            Log.d(TAG, "Data value from setting UI does not match with the resp_msg.isMobile_data_enabled()");
                            if (data) {
                                Log.d(TAG, "Setting Data Enabled value from Preference ");
                                imsConfig.setProvisionedValue(ImsConfig.ConfigConstants.MOBILE_DATA_ENABLED,ImsConfig.FeatureValueConstants.ON);
                            } else {
                                Log.d(TAG, "Setting Data Disabled value from Preference ");
                                imsConfig.setProvisionedValue(ImsConfig.ConfigConstants.MOBILE_DATA_ENABLED,ImsConfig.FeatureValueConstants.OFF);
                            }
                            Log.d(TAG, "Before QrcsImsSettings_SetQipcallConfig");
                            Log.d(TAG, "After QrcsImsSettings_SetQipcallConfig");
                            AppGlobalState.isDataSettingNvSame = false;
                            ContactInfo.firstPublish = false;
                        } else {
                            AppGlobalState.isDataSettingNvSame = true;
                            Log.d(TAG,"Data value from setting UI match with the resp_msg.isMobile_data_enabled()");
                        }
                    }
          ...
          Runnable invokePublishMethod = new Runnable() {
                            ...
                            {
                Log.d(TAG,"publish was getting invoked from here | contactfirstpublish");
                            }
                            else
                            {
             ...
             Log.d(TAG, " getImsSettingsResp   2  ");

                         Runnable updateSetting = new Runnable() {
                             public void run() {
                                 try {
                                     Log.d(TAG, " In Setting screen"
                                             + Settings.inSettingScreen);
                                     if (Settings.inSettingScreen) {
                                         Log.d(TAG, " Before Populating values");
                                         Settings.settingsObject.updateSettingValues();
                                         Log.d(TAG, "After Populating values ");
                                     }
                         ...
                         Log.d(TAG, " runOnUiThread(updateSetting)");

       }catch(Exception e){
               Log.d(TAG, "imsconfig thread exception");
         e.printStackTrace();
       }
           }
             }).start();
  ...
  private boolean getVtCallingStatus() {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.publish_rich:
                ...
                Log.d(TAG, "isCdByOption " + AppGlobalState.isCdByOption());
          ...
          Log.d(TAG,"invoking publish from isCdbyoption true");
                  invokePublish(true);
                } else {
          Log.d(TAG,"invoking publish from isCdbyoption false");
                  invokePublish(false);
                }
    ...
    private void startPublishActivity() {
        Intent i = new Intent();
        i.setClassName("com.qualcomm.qti.presenceappSub2",
                "com.qualcomm.qti.presenceappSub2.Publish");
        startActivity(i);
    }
    ...
    private void startMyInfoActivity() {
        Intent i = new Intent();
        i.setClassName("com.qualcomm.qti.presenceappSub2",
                "com.qualcomm.qti.presenceappSub2.MyInfo");
        startActivity(i);
    }
    ...
    private void startParActivity() {
        Intent i = new Intent();
        i.setClassName("com.qualcomm.qti.presenceappSub2",
                "com.qualcomm.qti.presenceappSub2.Par");
        startActivity(i);
    }
    ...
    private void startSettingsActivity() {
        Intent i = new Intent();
        i.setClassName("com.qualcomm.qti.presenceappSub2",
                "com.qualcomm.qti.presenceappSub2.Settings");
        startActivity(i);
    ...
    private void startLiveLoggingActivity() {
        Intent i = new Intent();
        i.setClassName("com.qualcomm.qti.presenceappSub2",
                "com.qualcomm.qti.presenceappSub2.LiveLoggingActivity");
        startActivity(i);
    ...
    private void startContactInfoActivity(int index) {
        Intent i = new Intent();
        i.setClassName("com.qualcomm.qti.presenceappSub2",
                "com.qualcomm.qti.presenceappSub2.ContactInfo");
    ...
    {
        if(AppGlobalState.getIqRcsService() != null){
        Log.d(TAG,"starting imsconfigsetting");
        initImsConfigSettings();
        }
    return true;
    }
    ...
    //initializing the imsconfigsetting from here once rcs service has been set.
        ...
        Log.d(TAG, " bindRcsService l.size =   "+l.size());
        if(l.size() > 0)
        {
        ...
        {
            Log.d(TAG, " bindRcsService    no active setting service ");
            return false;
        } **/
