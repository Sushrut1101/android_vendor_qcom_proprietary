...
int create_database()
{
    ...
    else
    {
        ...
        iop_query_exec(IO_PREFETCHER_QUERY_CREATE_TBL_FILE_LIST);
        iop_query_exec(IO_PREFETCHER_QUERY_CREATE_TBL_PKG_LIST);
        iop_query_exec(UX_ENGINE_QUERY_CREATE_TBL_PKG_LIST);
        iop_query_exec(UX_LATENCY_QUERY_CREATE_TBL_PKG_LIST);
...
int get_ux_lat_pkg(char *pkg_name, pkg_ux_lat_details *pkg_info)
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        const char *db_pkg_name = (const char *)sqlite3_column_text(stmt, 0);
        if (db_pkg_name == NULL)
            continue;
        QLOGI("pkg_name = %s\n", db_pkg_name);
        strlcpy(pkg_info->pkg_name, db_pkg_name, PKG_NAME_LEN);
        pkg_info->bindApp_dur = sqlite3_column_int(stmt,1);
        pkg_info->disAct_dur = sqlite3_column_int(stmt,2);
        pkg_info->memory_st = sqlite3_column_int(stmt,3);
        pkg_info->bindApp_ct = sqlite3_column_int(stmt,4);
        counter++;
    }
