...
/**
 * Class iMultipleIovData defines an interface to an object that can store
 * multiple arrays of struct iovec entries.
 */
class iMultipleIovData
{
public:
    iMultipleIovData() { }
    virtual ~iMultipleIovData() { }

    /**
     * Reset object for reuse.
     */
    virtual void ResetMultipleIovData() = 0;
     ...
     */
    virtual bool AddIovec(struct iovec *pIov, uint32 nNumIovs) = 0;
     ...
     */
    virtual void PrintMultipleIovData() const = 0;

private:
    iMultipleIovData(const iMultipleIovData&);
    iMultipleIovData& operator=(const iMultipleIovData&);
};

/**
 * iMultipleDataAllocator defines an interface to allocate and
 * release an object of type iMultipleIovData.
 */
class iMultipleIovDataAllocator
{
public:
    iMultipleIovDataAllocator() { }
    virtual ~iMultipleIovDataAllocator() { }
     ...
     */
    virtual iMultipleIovData *AllocateMultipleIovData(uint32 maxCount, uint32 maxIovEntriesPerIov) = 0;

    /**
     * Release an instance of iMultipleIovData.
     */
    virtual void ReleaseMultpleIovData(iMultipleIovData *ptr) = 0;

private:
    iMultipleIovDataAllocator(const iMultipleIovDataAllocator&);
    iMultipleIovDataAllocator& operator=(const iMultipleIovDataAllocator&);
};
...
public:
  ...
  */
  virtual DataSourceReturnCode Readable(
        /*in*/ iReadNotificationHandler const* /*pNotificationHandler*/) = 0;
...
/**
  ...
  */
  ...
  /**
     ...
     * return DS_SUCCESS on success
     *        DS_INTR    Interrupted while sending data
     *        DS_FAIL    non-recoverable error
     */
  virtual DataSourceReturnCode EnqueueForTransmit(
     uint32 /*nStreamNumber*/,
     const iMultipleIovData * /*pMultipleIovData*/)
  {return video::iSourcePort::DS_FAILURE;}
