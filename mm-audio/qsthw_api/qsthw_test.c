...
#define SOUNDTRIGGER_TEST_USAGE \
    ...
    "Example: qti_sound_trigger_test -ns 1 -id 1 -sm /data/HS.uim -nk 1\n" \
    "OPTIONS:\n" \
...
static int kb_duration_ms = 2000; //2000msec is default duration
...
static const char *get_param_key_array[] =
{
    ...
    QSTHW_PARAMETER_ESP_ENERGY_LEVELS,
};
...
static void eventCallback(struct sound_trigger_recognition_event *event, void *sessionHndl __unused)
{
    ...
    if (sm_data->esp_levels) {
        rc = qsthw_get_param_data(st_mod_handle, sm_handle, param,
...
static void process_detection_event(struct sm_session_data *sm_data)
{
    ...
    printf("%s: detection event timestamp %lld\n", __func__, event->timestamp);
                                   ...
                                   size_t param_data_size)
{
    if (!strncmp(param, QSTHW_PARAMETER_DIRECTION_OF_ARRIVAL, sizeof(param))) {
        ...
        printf("%s: processing esp energy levels params %p %d\n", __func__, payload, param_data_size);
        ...
        printf("%s: ESP: voice energy %d, ambient noise energy %d\n", __func__,
                energy_levels_params.voiceEnergy, energy_levels_params.ambientNoiseEnergy);
    }
}
...
static void qti_sva_server_death_notify_cb(void *ctxt)
{
    printf("qas died\n");
    struct sm_session_data *s_data = (struct sm_session_data *)ctxt;
    exit_loop = true;
}
...
static void fill_opaque_data(struct sound_trigger_recognition_config *rc_config,
      ...
      unsigned int cnn_kw_conf, unsigned int vop_user_conf, bool kw_buffer)
{
...
int main(int argc, char *argv[])
{
    ...
    printf("registering qas callback\n");
    qsthw_register_qas_death_notify_cb((qsthw_error_callback)qti_sva_server_death_notify_cb, &sound_trigger_info);
