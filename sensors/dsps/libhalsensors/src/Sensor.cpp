...
float Sensor::calcSampleRate(uint64_t ns)
{
    ...
    if ((this->trigger_mode != SENSOR_MODE_CONT) &&
        (this->trigger_mode != SENSOR_MODE_EVENT)) {
...
void Sensor::flushSendCmplt()
{
    ...
    if (flush_requested > 0) {
        ...
        flush_requested--;
        if (Utility::insertQueue(&flush_evt, bWakeUp, data_cb)){
...
float Sensor::getMaxFreqBkp()
{
    return this->max_sample_freq_bkp;
}
...
int Sensor::getResBufferedSamples()
{
    return this->reserved_buffered_samples;
}
...
void Sensor::setMaxFreqBkp(float max_sample_freq_bkp)
{
    this->max_sample_freq_bkp = max_sample_freq_bkp;
}
...
void Sensor::setResBufferedSamples(int reserved_buffered_samples)
{
    this->reserved_buffered_samples = reserved_buffered_samples;
}
...
bool Sensor::isBatchingEnable()
{
    return this->batching;
}

bool Sensor::bComplySampleRate(int64_t prv_ts_ns, int64_t present_ts_ns)
{
    if ((present_ts_ns - prv_ts_ns) >= (int64_t)HZ_TO_NSEC(freq))
        return true ;
    else
        return false;
}
...
/*============================================================================
  setReportTimer
============================================================================*/
int Sensor::setReportTimer(float report_delay_sec)
{
    struct itimerspec timer;
    int ret = 0;
    report_delay_sec = NSEC_TO_SEC(report_delay_sec);
    timer.it_value.tv_sec = (unsigned int)floor(report_delay_sec);
    timer.it_value.tv_nsec = (unsigned long)((report_delay_sec - (unsigned int)floor(report_delay_sec)) * NSEC_PER_SEC);
    timer.it_interval.tv_sec = 0;
    timer.it_interval.tv_nsec = 0;
    if (timer_settime( report_timer, 0, &timer, NULL ))
        ret = -1;
    return ret;
}
...
int Sensor::unsetReportTimer()
{
    ...
    timer.it_interval.tv_sec = 0;
    timer.it_interval.tv_nsec = 0;
    if (timer_settime( report_timer, TIMER_ABSTIME, &timer, NULL ))
        ret = -1;
    return ret;
}
