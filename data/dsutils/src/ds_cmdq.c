...
/*===========================================================================
  FUNCTION  ds_cmdq_flush
===========================================================================*/
/*!
@brief
  Purges all pending commands in queue.
...
)
{
  ...
  ds_log_high("[%d][%d] ds_cmdq_flush Enter", getpid(),gettid());

  /* validate input param */
  if(NULL == cmdq) {
    ds_log_err("ds_cmdq_flush Bad Param cmdq NULL\n");
    return -1;
  }
  ...
  ds_log_high("[%d][%d] ds_cmdq_flush Exit", getpid(),gettid());
  return 0;
}
