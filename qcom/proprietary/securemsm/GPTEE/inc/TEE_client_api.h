 ...
 * or a Command invocation operation. As this is a synchronous API, this function
 * must be called from a thread other than the one executing the TEEC_OpenSession
 * or TEEC_InvokeCommand function.
 *
 *
 * This function just sends a cancellation signal to the TEE and returns
 * immediately; the operation is not guaranteed to have been cancelled when this
 * function returns. In addition, the cancellation request is just a hint;
 * the TEE or the Trusted Application MAY ignore the cancellation request.
 * It is valid to call this function using a TEEC_Operation structure any
 * time after the Client Application has set the started field of an Operation
 * structure to zero. In particular, an operation can be cancelled before it is
 * actually invoked, during invocation, and after invocation. Note that the
 * Client Application MUST reset the started field to zero each time an
 * Operation structure is used or re-used to open a Session or invoke a
 * Command if the new operation is to be cancellable.
 *
 ...
 * @brief
 *
 * This function registers a UUID with the library. The TA binary corresponding
 * to this UUID is passed in a pre-allocated buffer.
 * The library does not make a copy of the passed buffer, which has therefore
 * to be guaranteed as valid by the caller for the whole validity of the active
 * context.
 *
 * @param[in] context: pointer to an initialized TEE Context.
 * @param[in] uuid: pointer to a structure containing the UUID for the Trusted
 *            Application being registered.
 * @param[in] buffer: buffer containing the TA binary.
 * @param[in] size: its size.
 *
 * @return: TEEC_SUCCESS is the Trusted Application was successfully registered
 *          with the initialized context.
 */
TEEC_Result TEEC_RegisterTA(TEEC_Context *context,
                            TEEC_UUID const * uuid,
                            void const * buffer,
                            size_t size);
