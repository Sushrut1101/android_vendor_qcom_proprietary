...
--------   ---     ----------------------------------------------------------
01/06/15   tl      Fix comment for FCI fallback
...
)
{
  ...
  if (qcril_uim.card_status.card[slot].card_state != QMI_UIM_CARD_STATE_PRESENT)
  {
    ...
    return QCRIL_UIM_RIL_E_SIM_ABSENT;
  }
  ...
  QCRIL_LOG_WARN("%s", "Error extracting appropriate session !!");
...
)
{
  ...
  if(qcril_uim.card_iccid_info.iccid_len > 0)
  {
    if(qcril_uim.card_iccid_info.iccid_len >
       *iccid_len_ptr)
    {
    ...
    {
      file_data_len = qcril_uim.card_iccid_info.iccid_len;
    }
    memcpy(iccid_data_ptr,
           qcril_uim.card_iccid_info.iccid_data,
           file_data_len);
     ...
     (read_rsp_data.rsp_data.read_transparent_rsp.content.data_ptr == NULL))
  {
    ...
    if(read_rsp_data.qmi_err_code == QMI_ERR_DEVICE_NOT_READY_V01)
    {
...
(
  qmi_client_error_type          qcril_err_code
)
{
  RIL_Errno ril_error_value = QCRIL_UIM_RIL_E_INTERNAL_ERR;

  switch (qcril_err_code)
  {
    /* QMI_SERVICE_ERR_NONE is same as QMI_NO_ERR */
    case QMI_NO_ERR:
      ril_error_value = RIL_E_SUCCESS;
      break;

    case QMI_ERR_NO_MEMORY_V01:
      ril_error_value = QCRIL_UIM_RIL_E_NO_MEMORY;
      break;
    ...
    case QMI_ERR_INVALID_PB_TYPE_V01:
      ril_error_value = QCRIL_UIM_RIL_E_INVALID_ARGUMENTS;
      break;

    case QMI_ERR_CLIENT_IDS_EXHAUSTED_V01:
    case QMI_ERR_INSUFFICIENT_RESOURCES_V01:
      ril_error_value = QCRIL_UIM_RIL_E_NO_RESOURCES;
      break;

    case QMI_ERR_INCORRECT_PIN_V01:
      ril_error_value = QCRIL_UIM_RIL_E_PASSWORD_INCORRECT;
      break;
    ...
    case QMI_ERR_INCOMPATIBLE_STATE_V01:
      ril_error_value = QCRIL_UIM_RIL_E_INVALID_SIM_STATE;
      break;

    case QMI_ERR_ACCESS_DENIED_V01:
      ril_error_value = QCRIL_UIM_RIL_E_OPERATION_NOT_ALLOWED;
      break;

    case QMI_ERR_PIN2_RESTRICTION_V01:
      ril_error_value = QCRIL_UIM_RIL_E_SIM_PIN2;
      break;
    ...
    case QMI_ERR_NO_SIM_V01:
      ril_error_value = QCRIL_UIM_RIL_E_SIM_ERR;
      break;

    case QMI_ERR_DEVICE_NOT_READY_V01:
      ril_error_value = QCRIL_UIM_RIL_E_INVALID_MODEM_STATE;
      break;

    case QMI_ERR_DISABLED_V01:
      ril_error_value = QCRIL_UIM_RIL_E_SYSTEM_ERR;
      break;

    case QMI_CLIENT_INVALID_CLNT:
      ril_error_value = QCRIL_UIM_RIL_E_MODEM_ERR;
      break;

    case QMI_INVALID_TXN:
    case QMI_CLIENT_PARAM_ERR:
      ril_error_value = QCRIL_UIM_RIL_E_INVALID_ARGUMENTS;
      break;

    default:
      ril_error_value = QCRIL_UIM_RIL_E_INTERNAL_ERR;
      break;
  }
