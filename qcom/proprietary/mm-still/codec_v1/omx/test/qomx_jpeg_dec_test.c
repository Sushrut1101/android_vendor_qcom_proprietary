...
OMX_ERRORTYPE omx_test_dec_send_buffers(void *data)
{
  ...
  lbuffer_info.fd = (OMX_U32)p_client->in_buffer[i].p_pmem_fd;
...
OMX_ERRORTYPE omx_test_dec_set_io_ports(omx_dec_test_t *p_client)
{
  ...
  p_client->inputPort->format.image.nStride =
    (OMX_S32)p_client->inputPort->format.image.nFrameWidth;
...
OMX_ERRORTYPE omx_test_dec_set_scaling_params(omx_dec_test_t *p_client)
{
  ...
  recttype.nLeft = (OMX_S32)p_client->scale_cfg.h_offset;
  recttype.nTop = (OMX_S32)p_client->scale_cfg.v_offset;
...
OMX_ERRORTYPE omx_test_dec_enable_output_port(omx_dec_test_t *p_client)
{
  ...
  if (p_client->width && p_client->height) {
    ...
    p_client->outputPort->format.image.nStride =
        (OMX_S32)p_client->outputPort->format.image.nFrameWidth;
  ...
  p_client->out_buffer[0].size =
    (size_t)((float)p_client->outputPort->format.image.nStride *
    (float)p_client->outputPort->format.image.nSliceHeight *
    size_factor);
  ...
  lbuffer_info.fd = (OMX_U32)p_client->out_buffer[0].p_pmem_fd;
  ...
  omx_dec_test_args_t *p_test)
{
  ...
  while ((c = getopt(argc, argv, "I:O:W:H:F:Q:r:P:um:n:x:y:M:N:"
    ...
    case 'r':
      ...
      break;
    case 'W':
      p_test->width = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "Default width", p_test->width);
      break;
    case 'H':
      p_test->height = (uint32_t)atoi(optarg);
    ...
    case 'u':
      ...
      break;
    case 'm':
      p_test->scale_cfg.input_width = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "image scale input width",
        p_test->scale_cfg.input_width);
      break;
    case 'n':
      p_test->scale_cfg.input_height = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "image scale input height",
        p_test->scale_cfg.input_height);
      break;
    case 'M':
      p_test->scale_cfg.output_width = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "image scale output width",
        p_test->scale_cfg.output_width);
      break;
    case 'N':
      p_test->scale_cfg.output_height = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "image scale output height",
        p_test->scale_cfg.output_height);
      break;
    case 'x':
      p_test->scale_cfg.h_offset = (uint32_t)atoi(optarg);
      fprintf(stderr, "%-25s%d\n", "image scale h offset",
        p_test->scale_cfg.h_offset);
      break;
    case 'y':
      p_test->scale_cfg.v_offset = (uint32_t)atoi(optarg);
...
int main(int argc, char* argv[])
{
  ...
  OMX_Deinit();
  for (i = 0; i < test_args.instance_cnt; i++)
    fprintf(stderr, "Decoding %d completed %s in %llu milliseconds\n", i,
