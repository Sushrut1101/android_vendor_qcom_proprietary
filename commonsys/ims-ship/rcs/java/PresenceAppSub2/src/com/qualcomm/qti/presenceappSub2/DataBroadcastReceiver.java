...
public class DataBroadcastReceiver  {
    ...
    private static final String TAG = "Presence_UI2: DataBroadcastReceiver";
               ...
               for (int i = 0; i < mPhoneCount; i++) {
                ...
                Log.d(TAG ,"subIds length : "+subIds.length);
                 if (SubscriptionManager.isUsableSubIdValue(subIds[0])) {
                    mContext.getContentResolver().registerContentObserver(setUpUri(subIds),false, mObserver);
                }
                }
               }
    ...
    public void removeMobileDataContentObserver() {
     ...
     private Uri setUpUri(int [] subIds) {

       Uri dataUri = null;
       Log.d(TAG ,"Setup Uri subIds length : "+subIds.length);
    ...
    private class MobileDataContentObserver extends ContentObserver {
         ...
         public void onChange(final boolean selfChange, Uri uri) {
             ...
             if(phoneId == AppGlobalState.mPhoneid) {
             toggleQipcallConfig((enabled !=0),phoneId);
    ...
    private void toggleQipcallConfig(final boolean bInMobileDataEnabled,final int phoneId) {
        ...
        try
        {
            ...
            com.qualcomm.qti.presenceappSub2.Settings.isMobile_data_enabled = bInMobileDataEnabled;
