...
====================================================================================================
Copyright (c) 2011-2013, 2015-2018 Qualcomm Technologies, Inc.  All Rights Reserved.
...
typedef union
{
  ...
  AUD_FTM_EXT_LP_PARAM_T     ext_lp_param;
...
struct mixer_ctl *get_ctl(struct mixer *m, char *name)
{
    ...
    return mixer_get_ctl_by_name(m, name);
}

int mixer_ctl_select(struct mixer_ctl *ctl, const char *value)
{
    return mixer_ctl_set_enum_by_string(ctl, value);
}

static int mixer_ctl_set(struct mixer_ctl *ctl, int val)
{

    return mixer_ctl_set_value(ctl, 0, val);
}
...
int parse(struct test_params *commands)
{
    ...
    while((p = fgets(array, sizeof(array), fp))) {
        ...
        if (!strncmp(&p[0], "#", 1)) {
            ...
            } else if (!strncmp(&p[1], "ExternalLoop", len)) {
                commands->type_of_test = PATH_EXT_LB;
            } else
                commands->type_of_test = -1;
                ...
                if (temp) {
                    ...
                    if (val < 0) {
                        ...
                        if (strstr(ctlname, "Volume")) {
                            if (val < 0 || val > 100)
                                val = DEFAULT_VOLUME;
                            volume_range = mixer_ctl_get_range_max(ctl)
                                               - mixer_ctl_get_range_min(ctl);
                            /*
                             * At enable, volume is determined with following priority:
                             * 1. volume in command line.
                             * 2. volume in config file.
                             * At disable, volume always take value from config file.
                             */
                            if (commands->enable) {
                                if (PATH_EXT_LB == commands->type_of_test) {
                                    if(strstr(ctlname, "RX"))
                                        val = (commands->rx_volume > 0) ? commands->rx_volume : val;
                                    else if(strstr(ctlname, "DEC"))
                                        val = (commands->tx_volume > 0) ? commands->tx_volume : val;
                                } else {
                                    val = (commands->volume > 0) ? commands->volume : val;
                                }
                            }
                            val = (val * volume_range) / 100;
                        }
                        printf("\n Set %s %d", ctlname, val);
...
int taiko_tc_tx_handset_mic1_en(struct mixer *mxr, int enable)
{
    ...
    ret = mixer_ctl_set(ctl, val);
    if (taiko_i2s == TRUE)
        ctl = get_ctl(mxr, "MultiMedia1 Mixer MI2S_TX");
    else
        ctl = get_ctl(mxr, "MultiMedia1 Mixer SLIM_0_TX");
    if (!ctl)
        goto mixer_err;
...
int taiko_tc_rx_handset_earphone_en(struct mixer *mxr, int enable)
{
    ...
    if (enable)
        ...
        ret = mixer_ctl_select(ctl, "ZERO");
    if (taiko_i2s == TRUE)
        ctl = get_ctl(mxr, "MI2S_RX Audio Mixer MultiMedia1");
    else
        ctl = get_ctl(mxr, "SLIMBUS_0_RX Audio Mixer MultiMedia1");
    if (!ctl)
        goto mixer_err;
...
static AUD_FTM_PATH_MATRIX_T      g_current_path=
    {FTM_CASE_TONE_PLAY_HANDSET,   AUDIO_FTM_IN_INVALID, AUDIO_FTM_OUT_HANDSET,   TRUE,FALSE,FALSE,FALSE,0};
...
#ifdef VNDK_ENABLED
static char  *pcm_capture_file   = "/data/vendor/audio/ftm_pcm_record.wav";
#else
static char  *pcm_capture_file   = "/data/vendor/ftm_pcm_record.wav";
#endif
/*==================================================================================================
                                     GLOBAL VARIABLES
==================================================================================================*/
...
static DALBOOL ftm_audio_start_ext_loopback(uint16 low_freq, char *filename,
                                                    int16 tone_analysis, int16 freq_tolerance);
static DALBOOL ftm_audio_stop_ext_loopback();
...
)
{
    ...
    if (!g_config_test) {
        ...
        if (g_curr_ext_lb_device >= 0)
            ftm_tc_devices[g_curr_ext_lb_device].enable(mxr, 0);
    }
    ...
    if (g_config_test || !ftm_tc_devices[test_case].enable(mxr, 1)) {
        ...
        case PATH_FM:
            ...
            break;
        case PATH_EXT_LB:
            g_curr_ext_lb_device = test_case;
            break;
        default:
...

{
    ...
    ftm_param.ext_lp_param.path.inpath = g_current_path.iPath;
    ftm_param.ext_lp_param.path.outpath = g_current_path.oPath;
    ftm_param.ext_lp_param.gain = (uint16)g_volume_level;
    ftm_param.ext_lp_param.channel = g_current_path.channel;
    ftm_param.ext_lp_param.capturedevice = g_devicecapture;
    ftm_param.ext_lp_param.playbackdevice = g_deviceplayback;
    ftm_param.ext_lp_param.freq = low_freq;
    ftm_param.ext_lp_param.tone_analysis = tone_analysis;
    ftm_param.ext_lp_param.freq_tolerance = freq_tolerance;
    if(NULL == filename)
        ftm_param.ext_lp_param.filename = pcm_capture_file;
    else
        ftm_param.ext_lp_param.filename = filename;

    printf("\n%s: cdevice %d pdevice %d", __func__,
               g_devicecapture, g_deviceplayback);
    res=Audio_FTM_Attach(
        AUDIO_FTM_METHOD_EXT_LOOPBACK,
        &ftm_param,
        &g_active_ftm_driver_attach_hdl);
    if(res != AUDIO_FTM_SUCCESS)
    {
...
static DALBOOL ftm_audio_start_pcm_capture(uint8 seconds, const char *filename)
{
  AUDIO_FTM_STS_T res;
  AUD_FTM_DISPATCH_PARAM_T ftm_param;
...
//    ftm_pcm_buffer_size=ftm_param.pcm_rec_param.size;
  ...
  if(NULL == filename)
      ftm_param.pcm_rec_param.pFileName=(char *)pcm_capture_file;
  else
      ftm_param.pcm_rec_param.pFileName=(char *)filename;
...
static DALBOOL ftm_audio_start_file_play(struct test_params *params)
{
    AUDIO_FTM_STS_T res = AUDIO_FTM_SUCCESS;
    if (!params) {
    ...
    ftm_param.pcm_play_param.duration = params->duration;;
    res = Audio_FTM_Attach(
...
int test_ftm_afe_loopback_comm(uint32 path,  struct test_params *params)
{
    ...
    ftm_audio_dispatch((PACKED void *)&cmd, FALSE);
...
int test_ftm_ext_loopback_comm(uint32 path,  struct test_params *params)
{
    ftm_audio_pkt_type  cmd;
    struct mixer *mxr = 0;
    int result = 0;
    ...
    {
        if(params->fl >= MIN_FREQ && params->fl <= MAX_FREQ)
            printf("fl is valid.\n");
        else
        {
            params->fl = DEFAULT_FREQ;
            printf("fl is out of range. fl set to default value=%d\n", DEFAULT_FREQ);
        }
        ftm_audio_start_ext_loopback(params->fl, params->filename,
                                    params->tone_analysis, params->freq_tolerance);
    }
...
int test_ftm_pcm_record_comm(uint32 path, struct test_params *params)
{
    ...
    if (!params)
        ftm_audio_start_pcm_capture(2, NULL);
    else
        ftm_audio_start_pcm_capture(params->duration, params->filename);
done:
    return result;
}
    ...
    int tone_analysis, int freq_tolerance)
{
    ...
    if (fp){
        ...
        if (params.type_of_test == -1 || params.type_of_test > PATH_EXT_LB ||
