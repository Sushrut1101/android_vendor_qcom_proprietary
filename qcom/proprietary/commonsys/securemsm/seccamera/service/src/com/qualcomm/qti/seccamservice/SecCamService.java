...
public class SecCamService extends Service {
    ...
    private static final int MSG_GET_FORMAT_SUPPORT = 1007;

    /* Secure frame processing destination */
    ...
    private int destination = DESTINATION_QTEE;
    ...
    private native int doStartTzApp(String str, int buffSize, TZappInfo appInfo, int destination);
    private native int doShutdownTzApp();

    private static native boolean enableFrameCallback(long cSurfaceId, int returnParamsSize);
    private static native int isFrameAvailable(long cSurfaceId, int timeout,
    ...
    static class SecCamServiceHandler extends Handler {
        ...
        //
        //=========================================================================
        protected void handleMessage_MSG_GET_FORMAT_SUPPORT(Message msg) {
            Log.d(LOG_TAG, "::handleMessage_MSG_GET_FORMAT_SUPPORT");
            Messenger activityMessenger = msg.replyTo;
            Message replyMsg = Message.obtain();
            replyMsg.what = MSG_GET_FORMAT_SUPPORT;
            Bundle out_bundle = new Bundle();

            boolean result = isDualStreamSupported();
            out_bundle.putBoolean("result", result);
            replyMsg.setData(out_bundle);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_SEVICE_VERSION: {
                    ...
                    break;
                case MSG_GET_FORMAT_SUPPORT:
                    handleMessage_MSG_GET_FORMAT_SUPPORT(msg);
                    break;
                default: {
    ...
    public boolean init() {
        ...
        if (tzAppIfAvalable_ == false) {
            ...
            if (JNI_OK != doStartTzApp(taName_, taBufferSize_, tzAppInfo_, destination)) {
    ...
    public void extractTaParams(Intent intent) {
        ...
        if(null != intent.getExtras()) {
            ...
            destination = intent.getIntExtra("DESTINATION", DESTINATION_QTEE);
        }
        if(null == taName_) {
            taName_ = TZ_APP_NAME;
        }
