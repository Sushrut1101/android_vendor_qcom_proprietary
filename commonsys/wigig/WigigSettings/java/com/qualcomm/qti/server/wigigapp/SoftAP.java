...
public class SoftAP extends WigigActivity {
    ...
    private Preference mNumConnectedPreference;

    private String mModeChange = null;

    private WigigManager.SoftApCallback mSoftApCallback = new WigigManager.SoftApCallback() {
        @Override
        ...
        public void onStaConnected(String Macaddr, int numClients) {
            Log.d(TAG, "onStaConnected Macaddr " + Macaddr + " numClients " + numClients);
            mNumConnectedPreference.setSummary(
                    getResources().getQuantityString(R.plurals.num_connected, numClients, numClients));
        }

        @Override
        public void onStaDisconnected(String Macaddr, int numClients) {
            Log.d(TAG, "onStaDisconnected Macaddr " + Macaddr + " numClients " + numClients);
            mNumConnectedPreference.setSummary(
                    getResources().getQuantityString(R.plurals.num_connected, numClients, numClients));
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mNumConnectedPreference = findPreference("num_connected");

        mWigigManager.registerSoftApCallback(mSoftApCallback, null);
    ...
    private void getWigigApConfiguration() {
        ...
        if (mWifiConfig != null) {
            ...
            mApPreference.setSummary(mWifiConfig.SSID + " " + (mSecurityTypeIndex == OPEN_INDEX ?
                    getResources().getString(R.string.wifi_security_none) :
                    getResources().getString(R.string.wifi_security_wpa)));
            mNumConnectedPreference.setEnabled(true);
        }
    }
