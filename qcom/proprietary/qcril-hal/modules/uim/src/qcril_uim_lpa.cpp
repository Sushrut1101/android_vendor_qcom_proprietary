...
(
  std::shared_ptr<UimLpaReqMessage>   msg_ptr,
...
(
  ...
  qcril_uim_lpa_user_req_type         * in_ptr,
...
)
{
  ...
  if (lpa_rsp_ptr == NULL || in_ptr == NULL || req_ptr == NULL)
  {
  ...
  /* If request is GET profile, no need to process further */
    if (in_ptr->event == QCRIL_UIM_LPA_GET_PROFILE)
    {
      return;
    }
  }
  ...
  {
    qcril_uim_lpa_byte_to_iccid(iccid,
                                QCRIL_UIM_ICCID_LEN,
                                in_ptr->iccid,
                                in_ptr->iccid_len);
  }
  ...
  switch(in_ptr->event)
  {
    ...
    case QCRIL_UIM_LPA_DISABLE_PROFILE:
      result = qcril_uim_lpa_enable_disable_profile(req_ptr,
                                                    iccid,
                                                    in_ptr->event,
                                                    TRUE);
      break;
    case QCRIL_UIM_LPA_DELETE_PROFILE:
      result = qcril_uim_lpa_delete_profile(req_ptr, iccid, TRUE);
      break;
    default:
      break;
  }
...
)
{
  ...
  /* Sanity checks */
  if ((resp_ptr == NULL) || (req_ptr == NULL) || req_ptr->get_data_ptr() == NULL)
  {
  ...
  resp_data_ptr = std::make_shared<qcril_uim_lpa_response_type>();
  ...
  switch(resp_ptr->qmi_rsp_data.rsp_id)
  {
...
)
{
  ...
  switch(in_ptr->event)
  {
    case QCRIL_UIM_LPA_ADD_PROFILE:
      ...
      {
        result = qcril_uim_lpa_add_profile(req_ptr, in_ptr->activationCode, in_ptr->confirmationCode);
      }
      else
      {
    ...
    case QCRIL_UIM_LPA_DISABLE_PROFILE:
      if (in_ptr->iccid != NULL && in_ptr->iccid_len != 0)
      {
        qcril_uim_lpa_byte_to_iccid(iccid,
                                    ...
                                    in_ptr->iccid,
                                    in_ptr->iccid_len);
        result = qcril_uim_lpa_enable_disable_profile(req_ptr, iccid, in_ptr->event, FALSE);
      }
      break;
    ...
    case QCRIL_UIM_LPA_DELETE_PROFILE:
      if (in_ptr->iccid != NULL && in_ptr->iccid_len != 0)
      {
        qcril_uim_lpa_byte_to_iccid(iccid,
                                    ...
                                    in_ptr->iccid,
                                    in_ptr->iccid_len);
        result = qcril_uim_lpa_delete_profile(req_ptr, iccid, FALSE);
      }
      break;

    case QCRIL_UIM_LPA_EUICC_MEMORY_RESET:
      result = qcril_uim_lpa_memory_reset(req_ptr, in_ptr->resetMask);
      break;

    case QCRIL_UIM_LPA_GET_PROFILE:
      result = qcril_uim_lpa_get_profile(req_ptr);
      break;
    ...
    case QCRIL_UIM_LPA_UPDATE_NICKNAME:
      if (in_ptr->iccid != NULL && in_ptr->iccid_len != 0)
      {
        qcril_uim_lpa_byte_to_iccid(iccid,
                                    ...
                                    in_ptr->iccid,
                                    in_ptr->iccid_len);
        result = qcril_uim_lpa_update_nickname(req_ptr, iccid, in_ptr->nickname);
      }
      break;

    case QCRIL_UIM_LPA_GET_EID:
      result = qcril_uim_lpa_get_eid(req_ptr);
      break;

    case QCRIL_UIM_LPA_USER_CONSENT:
      result = qcril_uim_lpa_user_consent(req_ptr, in_ptr->userOk);
      break;

    case QCRIL_UIM_LPA_SRV_ADDR_OPERATION:
      result = qcril_uim_lpa_get_set_server_address(req_ptr, in_ptr->srvOpReq.smdpAddress, in_ptr->srvOpReq.opCode);
      break;
