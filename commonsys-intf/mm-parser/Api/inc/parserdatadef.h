...
// define Data cache size, recommended for audio container formats
#define DATA_CACHE_SIZE (1024 * 512)
#define MAX_ID3_TAG_SIZE  ( 3 * 1024 *1024)
...
//This is the minimum data requested for any container format
...
/*! Upper boundary to the file Size. This upper boundary will be helpful in
  ...
  ! this predefined value. local 60MB, streaming 10MB avoid ANR for large clip.
*/
#define FILE_UPPER_BOUNDARY (60 * 1000 * 1000)//60MB
#define FILE_UPPER_BOUNDARY_STREAMING (10 * 1000 * 1000)//10MB
...
/* This read status ENUMs are used to update the read API status.*/
typedef enum
{
  //! Data available and copied into destination buffer
  DATA_SUCCESS,
  //! Data read failed, stop the playback
  DATA_FAIL,
  //! Data is not available, indicates wait for data
  DATA_UNDERRUN,
  //! Data offset requested is overwritten, get new data offset. This happens
  //! only in WFD where circular buffer might overwritten data before consumed.
  DATA_OVERFLOW,
} eDataReadStatus;
...
//!Define ABS for 64 bit variables
#define  FILESOURCE_ABS64( x )  ( ( (int64)(x) > 0 ) ? (x) : -(x) )
...
//! Convert FOURCC to UINT32
#define FOURCC_TO_UINT32(a,b,c,d)    ((a) | ((b) << 8) | ((c) << 16) | ((unsigned)(d) << 24))
#define FOURCC_BE_TO_UTIN32(a,b,c,d) ((d) | ((c) << 8) | ((b) << 16) | ((unsigned)(a) << 24))
#define BAIL_ON_ERROR( eStatus) \
{ \
   if( PARSER_ErrorNone != eStatus) \
   { \
       MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_ERROR, \
        " Failure Status  %u", eStatus); \
       goto out; \
   } \
}
#define BAIL_ON_ERROR_IF_NULL( pValidPtr) \
{ \
   if( NULL == pValidPtr) \
   { \
       MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_ERROR, \
        " Failure Status  NULL Ptr"); \
       goto out; \
   } \
}
...
/*!
 *@brief OPUS File Format specific constant
 */
static const uint8 OPUS_SIG_BYTES[FOURCC_SIGNATURE_BYTES] ={0x4f,0x70,0x75,0x73};
...
/* OGG Metadata structure is used in FLAC file also. Instead of replicating
   ...
   common header file. */
static unsigned char const OGGFieldNames[MAX_FIELDNAMES_SUPPORTED][32] =
{
  "TITLE",
  "VERSION",
  "ALBUM",
...
// MP3 header mask for frame-sync/version/layer/sample-rate-index
#define MP3HDR_MASK 0xFFFE0C00
...
{
  uint32 header;                   ///< 4 byte mp3 header
...
static const uint8 EAC3_CHAN_LOC[9]= \
 { 2 /* Lc/Rc pair   */,
   2 /* Lrs/Rrs pair */,
   1 /* Cs           */,
   1 /* Ts           */,
...
/** DTS Frame duration
 *  The code represents the number of audio samples represented in a complete
 *  audio access unit at sampling frequency
 */
static const uint32 DTS_FRAME_DURATION[4] = { 512,1024, 2048, 4096 };

static const uint16 DTS_CHAN_LAYOUT[16] =
{
  1,/* Center in front of listener */
  2,/* Left/Right in front         */
  2,/* Left/Right surround on side in rear */
  1,/* Low frequency effects subwoofer */
  1,/* Center surround in rear */
  2,/* Left/Right height in front*/
  2,/* Left/Right surround in rear */
  1,/* Center height in front */
  1,/* Over the listener's head */
  2,/* Between left/right and center in front */
  2,/* Left/Right on side in front */
  2,/* Left/Right surround on side */
  1,/* Second low frequency effects subwoofer */
  2,/* Left/Right height on side */
  1,/* Center height in rear */
  2,/* Left/Right height in rear */
};
...
static const uint8 MP4_3GP_BRAND_ISML[FOURCC_SIGNATURE_BYTES] = { 'i','s','m','l' };
static const uint8 MP4_3GP_BRAND_PIFF[FOURCC_SIGNATURE_BYTES] = { 'p','i','f','f' };
static const uint8 QUICK_TIME_BRAND[FOURCC_SIGNATURE_BYTES]   = { 'q','t',' ',' ' };
/*!
...
/*!
 *@brief NAL unit constants
 */
static const uint8 NALU_SC_3BYTE[3] = {0x00,0x00,0x01};
static const uint8 NALU_SC_4BYTE[4] = {0x00,0x00,0x00,0x01};
