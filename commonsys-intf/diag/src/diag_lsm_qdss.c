...
int diag_qdss_init()
{
	...
	pthread_mutex_init(&(qdss_pools_mdm[0].write_mutex), NULL);
	pthread_cond_init(&(qdss_pools_mdm[0].write_cond), NULL);
	pthread_mutex_init(&(qdss_pools_mdm[0].read_mutex), NULL);
	pthread_cond_init(&(qdss_pools_mdm[0].read_cond), NULL);
	pthread_mutex_init(&(qdss_pools_mdm[1].write_mutex), NULL);
	pthread_cond_init(&(qdss_pools_mdm[1].write_cond), NULL);
	pthread_mutex_init(&(qdss_pools_mdm[1].read_mutex), NULL);
...
static int diag_set_diag_transport(int peripheral_type, int peripheral, uint8 stm_cmd)
{
	...
	if ((peripheral_type < MSM || peripheral_type > MDM_2) ||
		(peripheral <  DIAG_MODEM_PROC || peripheral >  DIAG_NPU_PROC)) {
...
void fill_qdss_buffer(void* ptr, int len, int type)
{
	...
	if (!type) {
		...
		if (len >= (DISK_BUF_SIZE - *bytes_in_buff)) {
			...
			pthread_mutex_unlock(&(qdss_pools_mdm[qdss_curr_read_mdm].write_mutex));
			qdss_curr_read_mdm = !qdss_curr_read_mdm;
			pthread_mutex_lock(&(qdss_pools_mdm[qdss_curr_read_mdm].read_mutex));
			if (!qdss_pools_mdm[qdss_curr_read_mdm].free) {
				pthread_mutex_unlock(&(qdss_pools_mdm[qdss_curr_read_mdm].write_mutex));
				pthread_cond_wait(
								  &(qdss_pools_mdm[qdss_curr_read_mdm].read_cond),
								  &(qdss_pools_mdm[qdss_curr_read_mdm].read_mutex));
				pthread_mutex_lock(&(qdss_pools_mdm[qdss_curr_read_mdm].write_mutex));
			}
			pthread_mutex_unlock(&(qdss_pools_mdm[qdss_curr_read_mdm].read_mutex));
			buffer = qdss_pools_mdm[qdss_curr_read_mdm].buffer_ptr[0];
			bytes_in_buff =
...
static void* qdss_read_thread_mdm(void* param)
{
	int num_bytes_read = 0, type = 1;
	(void) param;

	do {
		if (!qdss_init_done_mdm || diag_qdss_node_fd_mdm == DIAG_INVALID_HANDLE)
			continue;
		num_bytes_read = read(diag_qdss_node_fd_mdm, (void*)qdss_read_buffer_mdm,
							  READ_BUF_SIZE);
		if (!num_bytes_read) {
			if (qdss_kill_thread) {
				DIAG_LOGD("diag: In %s, Exit read thread for 0 length packet for mdm\n", __func__);
				return 0;
			}
			continue;
		}
...
static void* qdss_write_thread_mdm(void* param) {
	...
	while (1) {
		...
		if (!temp_ptr)
			return NULL;
		pthread_mutex_lock(&(qdss_pools_mdm[qdss_curr_write_mdm].write_mutex));
		if (!qdss_pools_mdm[qdss_curr_write_mdm].data_ready) {
			qdss_in_write_mdm = 1;
			pthread_cond_wait(&(qdss_pools_mdm[qdss_curr_write_mdm].write_cond),
							  &(qdss_pools_mdm[qdss_curr_write_mdm].write_mutex));
			qdss_in_write_mdm = 0;
		}
		...
		/* Free Read thread if waiting on same buffer */
		pthread_mutex_lock(&(qdss_pools_mdm[qdss_curr_write_mdm].read_mutex));
		pthread_cond_signal(&(qdss_pools_mdm[qdss_curr_write_mdm].read_cond));
		pthread_mutex_unlock(&(qdss_pools_mdm[qdss_curr_write_mdm].read_mutex));
		pthread_mutex_unlock(&(qdss_pools_mdm[qdss_curr_write_mdm].write_mutex));

		qdss_curr_write_mdm = !qdss_curr_write_mdm;
...
{
	int ret = 0, i, local_remote_mask, local_device_mask = 0;
	int dev_idx;
	uint16 z = 1, proc_type;
	...
	if (device_mask & local_device_mask) {
		...
		for (dev_idx = 1; dev_idx <=  NUM_PROC; dev_idx++) {
			if ((remote_mask & (1 << (dev_idx - 1))) &&
				...
				DIAG_LOGD("diag: In %s, Successful in killing config thread for mdm\n", __func__);
	...
	pthread_mutex_destroy(&(qdss_pools_mdm[0].write_mutex));
	pthread_cond_destroy(&(qdss_pools_mdm[0].write_cond));
	pthread_mutex_destroy(&(qdss_pools_mdm[0].read_mutex));
	pthread_cond_destroy(&(qdss_pools_mdm[0].read_cond));
	pthread_mutex_destroy(&(qdss_pools_mdm[1].write_mutex));
	pthread_cond_destroy(&(qdss_pools_mdm[1].write_cond));
	pthread_mutex_destroy(&(qdss_pools_mdm[1].read_mutex));
