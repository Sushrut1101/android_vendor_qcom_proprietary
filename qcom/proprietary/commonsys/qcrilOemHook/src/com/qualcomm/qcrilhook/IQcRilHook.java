...
public interface IQcRilHook {
    ...
    /** Abort ongoing nw scan request */
    int QCRIL_EVT_HOOK_ABORT_NW_SCAN = QCRILHOOK_BASE + 95;
    ...
    int QCRIL_EVT_HOOK_ENABLE_MODEM_UPDATE = QCRILHOOK_BASE + 34;

    /** Sim DePersonalization code */
    int QCRIL_EVT_HOOK_ENTER_DEPERSONALIZATION_CODE = QCRILHOOK_BASE + 216;

    /** Send Telephony UI READY status to RIL */
    int QCRIL_EVT_HOOK_SET_ATEL_UI_STATUS = QCRILHOOK_BASE + 26;

    /** Get Csg ID */
    int QCRIL_EVT_HOOK_GET_CSG_ID = QCRILHOOK_BASE + 24;
    ...
    /** Get current Uicc card Manual Provision status */
    int QCRIL_EVT_HOOK_GET_UICC_PROVISION_PREFERENCE = QCRILHOOK_BASE + 90;

    /** Set user preferred Uicc card Manual Provision preference */
    int QCRIL_EVT_HOOK_SET_UICC_PROVISION_PREFERENCE = QCRILHOOK_BASE + 91;

    /** Get Uicc Card ICCID value */
    int QCRIL_EVT_HOOK_GET_UICC_ICCID = QCRILHOOK_BASE + 92;

    /** Get Max Data Allowed value */
    int QCRIL_EVT_HOOK_GET_MAX_DATA_ALLOWED = QCRILHOOK_BASE + 93;

    /** Get L plus L support status*/
    int QCRIL_EVT_REQ_HOOK_GET_L_PLUS_L_FEATURE_SUPPORT_STATUS_REQ = QCRILHOOK_BASE + 94;

    /** Get OMH data profiles from modem */
    int QCRIL_EVT_HOOK_GET_OMH_CALL_PROFILE = QCRILHOOK_BASE + 202;

    /** Send protobuf msg id */
    int QCRIL_EVT_HOOK_PROTOBUF_MSG = QCRILHOOK_BASE + 101;

    /** Send lte direct discovery msg id */
    int QCRIL_EVT_HOOK_LTE_DIRECT_DISC_REQ = QCRILHOOK_BASE + 102;

    /** Unsol lte direct discovery msg id */
    int QCRIL_EVT_HOOK_UNSOL_LTE_DIRECT_DISC = QCRILHOOK_BASE + 103;

    /** Get card state */
    int QCRIL_EVT_HOOK_GET_CARD_STATE = QCRILHOOK_BASE + 210;

    /** Get ATR */
    int QCRIL_EVT_HOOK_GET_ATR = QCRILHOOK_BASE + 211;

    /** Send APDU command */
    int QCRIL_EVT_HOOK_SEND_APDU_CMD = QCRILHOOK_BASE + 212;

    /** Switch Slot mapping request */
    int QCRIL_EVT_HOOK_SWITCH_SLOT = QCRILHOOK_BASE + 213;

    /** Getu current slot mapping */
    int QCRIL_EVT_HOOK_GET_SLOT_MAPPING = QCRILHOOK_BASE + 214;

    /** Send card power request */
    int QCRIL_EVT_HOOK_CARD_POWER_REQ = QCRILHOOK_BASE + 215;

    /** Get Slot status request */
    int QCRIL_EVT_HOOK_GET_SLOTS_STATUS_REQ = QCRILHOOK_BASE + 218;

    /** Get LPA Terminal Capability */
    int QCRIL_EVT_HOOK_GET_TERMINAL_CAPABILITY_LPA_TLV_REQ = QCRILHOOK_BASE + 219;

    /** Set LPA Terminal Capability */
    int QCRIL_EVT_HOOK_SET_TERMINAL_CAPABILITY_LPA_TLV_REQ = QCRILHOOK_BASE + 220;

    /** get ADN record from the SIM card */
    int QCRIL_EVT_HOOK_GET_ADN_RECORD = QCRILHOOK_BASE + 221;

    /** Update ADN record to the SIM card */
    int QCRIL_EVT_HOOK_UPDATE_ADN_RECORD = QCRILHOOK_BASE + 222;
    ...
    /** send local call hold status */
    int QCRIL_EVT_SET_LOCAL_CALL_HOLD = QCRILHOOK_BASE + 7000;
    ...
    /** Unsol event for Uicc card Manual provision status change */
    int QCRILHOOK_UNSOL_UICC_PROVISION_STATUS_CHANGED = QCRILHOOK_UNSOL_BASE + 28;

    /** Unsol event for max data allowed change */
    int QCRILHOOK_UNSOL_MAX_DATA_ALLOWED_CHANGED = QCRILHOOK_UNSOL_BASE + 54;

    /** Unsol event card state update */
    int QCRILHOOK_UNSOL_CARD_STATE_CHANGED = QCRILHOOK_UNSOL_BASE + 30;

    /** Unsol event for RAC change indication */
    int QCRIL_EVT_HOOK_UNSOL_RAT_RAC_CHANGE_IND = QCRILHOOK_UNSOL_BASE + 53;

    /** Unsol event for SIM Lock Temp Unlock Expired */
    int QCRILHOOK_UNSOL_SIMLOCK_TEMP_UNLOCK_EXPIRED = QCRILHOOK_UNSOL_BASE + 29;

    /** Unsol event for MBN list report*/
    int QCRILHOOK_UNSOL_PDC_LIST_CONFIG = QCRILHOOK_UNSOL_BASE + 32;

    /** Unsol event for slot status change */
    int QCRILHOOK_UNSOL_SLOT_STATUS_CHANGE_IND = QCRILHOOK_UNSOL_BASE + 33;
     ...
     *
     * @deprecated Use qcRilGetAllConfigs
     */
    @Deprecated
    public String[] qcRilGetAvailableConfigs(String device);

    /**
     * Gets all MBNs maintained by modem
     *
     * @return true on SUCCESS and there will be subsequent unsol message to
     *         report MBN list; false on FAILURE and no subsequrent unsol message.
     */
    ...
    /**
     * Sends a QCRIL_EVT_HOOK_GET_CSG_ID request to qcril
     *
     * @return Csg Id
     */
    ...
    /**
     * Gets  L plus L feature support status form RIL
     * @return boolean true or false.
     */
    public boolean getLpluslSupportStatus();

    /**
     * Sends a QCRIL_EVT_HOOK_PROTOBUF_MSG request to qcril
     * ProtocolBuffer is used to communicate with RIL.
     *
     * @param protocolBuffer ProtoBuf converted to byte[]
     * @param phoneId PhoneId index of the Request.
     * @return byte[] protocol buffer byte array.
     */
    byte[] qcRilSendProtocolBufferMessage(byte[] protocolBuffer, int phoneId);
