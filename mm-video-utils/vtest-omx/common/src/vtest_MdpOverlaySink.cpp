...
MdpOverlaySink::MdpOverlaySink(OMX_BOOL bRotate)
    ...
    snprintf(m_pName, MAX_NAME_LENGTH, "%s", "MdpOverlaySink");
...
void MdpOverlaySink::OverlayUnset() {
    ...
    struct mdp_display_commit sDispCommit;
    sDispCommit.flags = MDP_DISPLAY_COMMIT_OVERLAY;
    sDispCommit.wait_for_finish = 0;
    sDispCommit.var = m_sVinfo;
    sDispCommit.l_roi.x = 0;
    sDispCommit.l_roi.y = 0;
    sDispCommit.l_roi.w = 0;
    sDispCommit.l_roi.h = 0;
    sDispCommit.r_roi.x = 0;
    sDispCommit.r_roi.y = 0;
    sDispCommit.r_roi.w = 0;
    sDispCommit.r_roi.h = 0;

    if (ioctl(m_nFbFd, MSMFB_DISPLAY_COMMIT, &sDispCommit) < 0) {
       VTEST_MSG_ERROR("MSMFB_DISPLAY_COMMIT failed");
    }
    VTEST_MSG_MEDIUM("MSMFB_OVERLAY_COMMIT successfull");
    m_nOverlayId = -1;
}
...
OMX_ERRORTYPE MdpOverlaySink::OverlaySet() {
    ...
    if (nWidth >= m_sVinfo.xres) {
        ...
        pOverlay->dst_rect.h = ((float)m_sVinfo.xres/pOverlay->src.width)*pOverlay->src.height;
    } else {
        pOverlay->dst_rect.x = (m_sVinfo.xres - nWidth)/2;
        if (ceil((float)m_sVinfo.yres /(pOverlay->dst_rect.w)) <= 4) {
            pOverlay->dst_rect.h = (m_sVinfo.xres/pOverlay->src.width)*pOverlay->src.height;
            pOverlay->dst_rect.w = m_sVinfo.xres;
            pOverlay->dst_rect.x = 0;
        } else {
            pOverlay->dst_rect.h = pOverlay->src.height * 4;
            pOverlay->dst_rect.w = pOverlay->src.width * 4;
            pOverlay->dst_rect.x = (m_sVinfo.xres - pOverlay->dst_rect.w)/2;
        }
    }
