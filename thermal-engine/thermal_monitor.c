/*===========================================================================
  ...
  Copyright (c) 2010-2015, 2020 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
enum temp_read_type {
	MONITOR_READ_TRIP_TEMP,
	MONITOR_READ_CUR_TEMP
};
...
static pthread_mutex_t cfg_set_mutex = PTHREAD_MUTEX_INITIALIZER;
...
#define THRESH_MASK_IS_SET(idx) ((idx > 31)?\
		(thresh_reached_mask[1] & (((uint32_t)0x1) << (idx - 32))):\
		(thresh_reached_mask[0] & (((uint32_t)0x1) << idx)))
#define DYNAMIC_CFG_MASK_SET(dynamic_cfg_mask, idx) ((idx > 31)?\
		(dynamic_cfg_mask[1] |= (((uint32_t)0x1) << (idx - 32))):\
		(dynamic_cfg_mask[0] |= (((uint32_t)0x1) << idx)))
#define DYNAMIC_CFG_ANY_SET(dynamic_cfg_mask) (dynamic_cfg_mask[0] | dynamic_cfg_mask[1])
#define DYNAMIC_CFG_IS_SET(dynamic_cfg_mask, idx) ((idx > 31)?\
		(dynamic_cfg_mask[1] & (((uint32_t)0x1) << (idx - 32))):\
		(dynamic_cfg_mask[0] & (((uint32_t)0x1) << idx)))
...
static int report_rule(device_clnt_handle clnt, const char *rule,
		      const char *sensor,
		      int temperature, int level, int is_trigger)
{
	char tempBuf[REPORT_MSG_MAX] = {0};
	union device_request req;

	info("ACTION: REPORT-RULE "
	    "Sensor '%s' - temperature %d, threshold level %d, is_trigger %s, rule '%s'\n",
	    sensor, temperature, level + 1,
	    is_trigger ? "true" : "false", rule);
				        ...
				        uint8_t disable)
{
	...
	if (!disable) {
		if (validate_disabled_config(tm_instance_info)) {
			msg("%s: validate_disabled_config error\n", __func__);
			return -1;
		}
		...
		for (i = 0; i < tm_info->num_thresholds; i++)
			tm_instance_info->lvl_alarm[i] = 0;
		tm_instance_info->last_lvl = 0;
		tm_instance_info->disable = disable;
	}
...
static int config_set_notify(int num_cfg_cnt, void *data, void *reserved)
{
	...
	int c_idx;
	int ret = 0;
	uint32_t dynamic_cfg_mask[2] = {0, 0};
...
algo_notify:
		if (disable_adjust_done ||
			...
			DYNAMIC_CFG_MASK_SET(dynamic_cfg_mask, a_idx[c_idx]);
		}
		continue;
...
error_handler:
	...
	pthread_mutex_lock(&wait_mutex);
	if (DYNAMIC_CFG_ANY_SET(dynamic_cfg_mask)) {
		for (c_idx = 0; (unsigned int)c_idx < tm_cnt; c_idx++)
			if (DYNAMIC_CFG_IS_SET(dynamic_cfg_mask, c_idx))
				THRESH_MASK_SET(c_idx);
		 pthread_cond_broadcast(&wait_cond);
	}
	pthread_mutex_unlock(&wait_mutex);
...
static void handle_thresh_sig(enum temp_read_type type)
{
	uint32_t j;
	int i;
	uint32_t idx;
	...
	/* Block all external request to modify config till we handle this */
	pthread_mutex_lock(&cfg_set_mutex);
	for (idx = 0; idx < tm_cnt; idx++) {
		...
		if (type == MONITOR_READ_CUR_TEMP)
			sensor_temp = sensors_manager_read(sensor->ts_clnt);
		else
				...
				case DEVICE_DIRECT_ACTION_TYPE:
					if (alarm_raised &&
					    ...
					    (strncasecmp(dev_info->name,
							"report_rule",
							DEVICES_MAX_NAME_LEN) == 0))
						report_rule(sensor->dev_clnt_list[action_idx],
							   info->desc, tm_info->sensor,
							   sensor_temp, i, 1);
					else if ((strncasecmp(dev_info->name, "report", DEVICES_MAX_NAME_LEN) != 0) &&
						(strncasecmp(dev_info->name, "report_rule", DEVICES_MAX_NAME_LEN) != 0))
			...
			for (j = 0; j < tm_info->t[lvl_min].num_actions; j++) {
				...
				if (strncasecmp(dev_info->name, "report",
					...
					report_msg(sensor->dev_clnt_list[action_idx],
						   ...
						   lvl_min, 0);
					break;
				/* if clearing all alarm case (lvl_min == 0) report level 0 and
				   triggered as true */
				} else if (strncasecmp(dev_info->name, "report_rule",
						DEVICES_MAX_NAME_LEN) == 0) {
					report_rule(sensor->dev_clnt_list[action_idx],
						   info->desc, tm_info->sensor,
						   sensor_temp, lvl_min == 0 ? lvl_min - 1 : lvl_min,
							lvl_min == 0 ? 1 : 0);
					break;
				}
			}
...
static void *sensor_monitor(void *data)
{
	...
	handle_thresh_sig(MONITOR_READ_CUR_TEMP);
	...
	while (exit_daemon != 1) {
		...
		handle_thresh_sig(MONITOR_READ_TRIP_TEMP);
	}
