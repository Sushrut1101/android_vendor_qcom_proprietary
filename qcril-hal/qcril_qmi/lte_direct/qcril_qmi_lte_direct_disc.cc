  ...
  do
  {
    ...
    if (ril_err == RIL_E_SUCCESS)
    {
      ...
      if (resp->AnnouncingPolicy_valid)
      {
        ...
        config->announcingPolicy = qcril_malloc2(config->announcingPolicy,
                config->announcingPolicy_count);

        if (config->announcingPolicy == NULL)
        {
          QCRIL_LOG_ERROR("Memory allocation failed");
        ...
        for(i = 0; i < resp->AnnouncingPolicy_len; i++)
        {
          ...
          config->announcingPolicy[i].plmn = qcril_malloc2(
      ...
      if (resp->MonitoringPolicy_valid)
      {
        ...
        config->monitoringPolicy = qcril_malloc2(config->monitoringPolicy,
                config->monitoringPolicy_count);

        if (config->monitoringPolicy == NULL)
        {
          QCRIL_LOG_ERROR("Memory allocation failed");
          ril_err = RIL_E_GENERIC_FAILURE;
          break;
        ...
        for(i = 0; i < resp->MonitoringPolicy_len; i++)
        {
          ...
          config->monitoringPolicy[i].plmn = qcril_malloc2(
 ...
 */
/*=========================================================================*/
static const char *qcril_qmi_lte_direct_disc_lookup_command_name
(
  unsigned long msg
)
{
...
)
{
  ...
  if(qmi_resp_callback)
  {
    ...
    instance_id = static_cast<qcril_instance_id_e_type>(
            QCRIL_EXTRACT_INSTANCE_ID_FROM_USER_DATA( user_data ));
...
)
{
  ...
  qmi_callback.data_buf = qcril_malloc2(qmi_callback.data_buf, ind_buf_len);
...
/*=========================================================================
FUNCTION: qcril_qmi_lte_direct_init

===========================================================================*/
void qcril_qmi_lte_direct_init
(
  void
)
{
  qcril_qmi_lte_direct_disc_event_map_init();
