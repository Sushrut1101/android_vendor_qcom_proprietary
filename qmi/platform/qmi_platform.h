...
/******************************************************************************
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2007-2010, 2013-2014, 2017-2018 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
#define QMI_PLATFORM_NUM_FORWARD_CONN_IDS     11
#define QMI_PLATFORM_NUM_FORWARD_CONN_IDS_EXT 17 /* Additional PDN support */
#ifdef FEATURE_QMI_IWLAN
  #define QMI_PLATFORM_REV_RMNET_DATA_PREFIX  "r_rmnet_data"
  #define QMI_PLATFORM_RMNET_DATA_MAX_IFACES      40
  #define QMI_PLATFORM_RMNET_DATA_MAX_IFACES_EXT  66
#else
  #define QMI_PLATFORM_RMNET_DATA_MAX_IFACES      22
  #define QMI_PLATFORM_RMNET_DATA_MAX_IFACES_EXT  34
...
#define QMI_LOG_ADB_PROP "persist.vendor.data.qmi.adb_logmask"
...
{ \
  int rc; \
  pthread_condattr_init(&(signal_ptr)->attr); \
  rc = pthread_condattr_setclock(&(signal_ptr)->attr, CLOCK_MONOTONIC); \
  if(!rc) \
  { \
    rc = pthread_cond_init(&(signal_ptr)->cond_var, &(signal_ptr)->attr); \
    if(!rc) \
    { \
      (signal_ptr)->clock = CLOCK_MONOTONIC; \
    } \
    else \
    { \
      pthread_cond_init (&(signal_ptr)->cond_var, NULL); \
      (signal_ptr)->clock = CLOCK_REALTIME; \
    } \
  } \
  else \
  { \
...
do \
{ \
  pthread_condattr_destroy(&(signal_ptr)->attr); \
