/* csd_client.c
 *
 * Copyright (c) 2012-2016, 2018-2020 Qualcomm Technologies, Inc. All Rights Reserved.
...
#define QCCI_IPCRTR_INSTANCE_0 0
...
#define CSD_VOICEMMODE1_SESSION_NAME "11C05000"
#define CSD_VOICEMMODE2_SESSION_NAME "11DC5000"
...
enum session_id {
    ...
    VOLTE_SESSION_ID,
    VOICE2_SESSION_ID,
    VOICEMMODE1_SESSION_ID,
    VOICEMMODE2_SESSION_ID,
    ALL_SESSION_ID
};
...
static int sockid;
static int i2s_sample_rate = SAMPLE_RATE_48KHZ;
...
static int get_sessionid_by_vsid(uint32_t vsid)
{
    ...
    case VOICEMMODE1_VSID:
        sessionid = VOICEMMODE1_SESSION_ID;
        break;

    case VOICEMMODE2_VSID:
        sessionid = VOICEMMODE2_SESSION_ID;
        break;
...
struct csd_service *get_service_by_sessionid(int sessionid)
{
    ...
    case VOICEMMODE1_SESSION_ID:
    case VOICEMMODE2_SESSION_ID:
    default:
...
static int initialize_session_data(struct csd_session *session)
{
    ...
    pthread_cond_init(&session->csd_async_wait_condition, &attr);
...
static int initialize_service_data(struct csd_service *service)
{
    ...
    strlcpy(service->sessions[VOICEMMODE1_SESSION_ID].name,
           CSD_VOICEMMODE1_SESSION_NAME, strlen(CSD_VOICEMMODE1_SESSION_NAME)+1);
    strlcpy(service->sessions[VOICEMMODE2_SESSION_ID].name,
           CSD_VOICEMMODE2_SESSION_NAME, strlen(CSD_VOICEMMODE2_SESSION_NAME)+1);
                           ...
                           int mute, int direction, uint16_t ramp_duration)
{
    int ret = 0;
    qmi_client_error_type rc = QMI_NO_ERR;
    qmi_csd_voice_mute_req_msg_v01 mute_req;
    qmi_csd_voice_mute_req_msg_v01 mute_resp;
    memset(&mute_resp, 0, sizeof(mute_resp));
    memset(&mute_req, 0, sizeof(mute_req));
    qmi_txn_handle txn = 0;

    mute_req.cmd_token = 0;
    mute_req.mode = CSD_VOICE_MUTE_MODE_DEVICE_V01;
    mute_req.enable = mute;
    mute_req.direction = direction;
    mute_req.ramp_duration_in_ms = ramp_duration;

    pthread_mutex_lock(&session->csd_async_cond_mutex);
    rc = qmi_client_send_msg_async(service->csd_handle,
                                   QMI_CSD_VOICE_MUTE_REQ_V01,
                                   &mute_req, sizeof(mute_req),
...
static int csd_service_init(struct csd_service *service)
{
    ...
    while (1) {
        ...
        LOGE("%s: Error %d for qmi_client_get_service_instance\n", __func__, rc);
...
int csd_client_start_voice(uint32_t vsid)
{
    ...
    int ec_dev_id = 0;
#ifdef PLATFORM_AUTO
    int remote_rx_dev_id = 0;
    int remote_tx_dev_id = 0;
    int ret = 0;
    qmi_txn_handle txn = 0;

    qmi_client_error_type qmi_rc = QMI_NO_ERR;
    qmi_csd_voice_start_req_msg_v01 start_req;
    qmi_csd_voice_cmd_resp_msg start_resp;
    memset(&start_resp, 0, sizeof(start_resp));
    memset(&start_req, 0, sizeof(start_req));
#endif
    int incall_rec_dev = 0;
...
#ifdef PLATFORM_AUTO
        service->sessions[sessionid].state = SESSION_CREATED;
#else
    rc = csd_open_session_handles(service, sessionid);
    if (rc < 0) {
    ...
    if (session->state == SESSION_CREATED ||
        ...
        start_req.cmd_token = 0;
        strlcpy(start_req.session_id,session->name,sizeof(session->name));

        start_req.tx_device_info_valid = 1;
        start_req.tx_device_info.dev_id = remote_tx_dev_id;
        start_req.tx_device_info.sample_rate = get_sample_rate(tx_dev_id);
        start_req.tx_device_info.bits_per_sample = BITS_PER_SAMPLE;

        start_req.rx_device_info_valid = 1;
        start_req.rx_device_info.dev_id = remote_rx_dev_id;
        start_req.rx_device_info.sample_rate = get_sample_rate(rx_dev_id);
        start_req.rx_device_info.bits_per_sample = BITS_PER_SAMPLE;

        start_req.ec_ref_device_info_valid = 0;
        start_req.ec_ref_device_info.dev_id = ec_dev_id;
        start_req.ec_ref_device_info.sample_rate =  get_sample_rate(ec_dev_id);
        start_req.ec_ref_device_info.bits_per_sample = BITS_PER_SAMPLE;

        pthread_mutex_lock(&session->csd_async_cond_mutex);
        qmi_rc = qmi_client_send_msg_async(service->csd_handle,
                                   QMI_CSD_VOICE_START_REQ_V01,
                                   &start_req, sizeof(start_req),
                                   &start_resp, sizeof(start_resp),
                                   csd_client_async_cb, session, &txn);
...
close_handles:
    csd_close_session_handles(service, sessionid);
...
int csd_client_stop_voice(uint32_t vsid)
{
    ...
    rc = csd_close_session_handles(service, sessionid);
    if (rc < 0) {
        LOGE("%s: Error %d closing session handles for session %d\n",
              __func__, rc, sessionid);
    }
    pthread_mutex_unlock(&service->csd_service_mutex);
...
/** Slim device not available in auto platform **/
#ifndef PLATFORM_AUTO
