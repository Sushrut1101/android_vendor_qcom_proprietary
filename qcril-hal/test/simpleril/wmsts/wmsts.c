/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
Copyright (c) 2001-2008, 2018 Qualcomm Technologies, Inc.
All Rights Reserved.
...
)
{
  ...
  if( dcs->alphabet == WMS_GW_ALPHABET_7_BIT_DEFAULT )
  {
    ...
    if(user_data->sm_len < WMS_MAX_LEN)
    {
  ...
  else
  {
    ...
    switch( client_ts_ptr->cb_page_hdr.cb_dcs.alphabet )
    {
      case WMS_GW_ALPHABET_7_BIT_DEFAULT:
        ...
        if(gw_user_data.sm_len < WMS_MAX_LEN)
        {
...
)
{
  ...
  */
  ...
  return wms_ts_decode_deliver_from_uint8(raw_ts_data_ptr->data, deliver, FALSE);
} /* wms_ts_decode_deliver() */

/*=========================================================================
FUNCTION
  wms_ts_decode_deliver_from_uint8
...
SIDE EFFECTS
  None
=========================================================================*/
wms_status_e_type wms_ts_decode_deliver_from_uint8
(
  const uint8                             * data,
  wms_gw_deliver_s_type                   * deliver,
  boolean                                 mti_reserved
)
{
  wms_status_e_type     st = WMS_OK_S;
  uint32                pos = 0, i;
  ...
  /* TP-MTI, TP-MMS, TP-SRI, TP-UDHI, TP-RP
  */
  if (data == NULL || deliver == NULL)
  {
    LOGE("null pointer in wms_ts_decode_deliver");
    return WMS_NULL_PTR_S;
  }
  else if ( !((data[pos] & 0x03) == 0x00 || /* check bit 0 and bit 1 for TP-MTI for deliver */
           (mti_reserved && (data[pos] & 0x03) == 0x11))) /* check TP-MTI for reserved if needed */
  {
