...
#define QSC_CHECKER "/data/vendor/mdmhelperdata/qschelpsocket"
...
#define DUMP_EMMC_DIR "/data/vendor/ramdump"
...
static void *uevent_mon(void* param)
{
	...
	while (1) {
		...
		if (count >= UEVENT_BUF_SIZE) {
			ALOGE("Uevent msg overflow; ignoring");
			continue;
		}
		uevent_buf[count] = '\0';
		uevent_buf[count+1] = '\0';
		/*
...
static void *rpm_log_thread(void* param)
{
	...
	while (1) {
		...
		if (count <= 0) {
			ALOGE("Failed to read rpm log OR zero bytes read, count:%d", count);
			free(read_buf);
			continue;
		}
...
static int ssr_search_pil_ramdump(void)
{
	...
	while ((dir_p = readdir(dir_s)) != NULL) {
		if (strncmp(dir_p->d_name, DUMP_HEAD_STR, 8) == 0) {
			if (i >= DUMP_NUM) {
				ALOGE("Exceed max pil number i:%d DUMP_NUM:%d",
								i, DUMP_NUM);
				pfd_num = 0;
				ret = -EINVAL;
				goto invalid_num;
...
static void ssr_tool_helper(void)
{
	...
	ALOGI("[arg1]: (1/2) Ramdump location: 1: eMMC: /data/vendor/ramdump or 2: SD card: /sdcard/ramdump");
...
static int parse_args(int argc, char **argv)
{
	...
	if (argc == 1) {
		...
		ALOGI(" Using default ramdump location 1: eMMC: /data/vendor/ramdump");
