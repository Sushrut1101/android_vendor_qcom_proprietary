...
class VideoCodecDecoder : public MediaCodecBase, public VTPlayer
{
  public:
    ...
    virtual void SetNativeWindow(ANativeWindow* surfaceFar);
    virtual ~VideoCodecDecoder();
    VideoCodecDecoder();
    /** Returns true if the thread started */
    bool StartPlayerThread() {
      CRITICAL1("Start videoPlayFrameLoop Thread ");
    ...
    void WaitforPlayerThread() {
      CRITICAL1("Waiting for videoPlayFrameLoop Thread to exit");
      (void) pthread_join(m_PlayerThread, NULL);
      CRITICAL1("videoPlayFrameLoop Thread exited");
    }
  ...
  private:
    ...
    void setRotation(uint8_t ucCvoInfo);
