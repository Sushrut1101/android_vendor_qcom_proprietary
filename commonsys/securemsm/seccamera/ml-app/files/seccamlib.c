...
int sock_wait_for_cmd(guest_socket socket_handle,
    ...
    guest_socket_recvfrom(&socket_handle, req, req_size,
                          rcv_sockaddr, &bytes);
    if (bytes <= 0) {
...
int sock_send_response(guest_socket socket_handle,
                       guest_sockaddr *rsp_sockaddr,
                       guest_sockaddr_len_type *rsp_addrlen,
                       void *rsp, uint32_t rsp_size) {
    int32_t bytes = 0;

    guest_socket_sendto(&socket_handle, rsp, rsp_size,
                        rsp_sockaddr, &bytes);
...
mlvmapp_error_t demoExchangeTimestamp(mlvmapp_if_timestamp_send_cmd_t *cmd,
                                      uint32_t cmd_size,
                                      mlvmapp_if_timestamp_send_cmd_rsp_t* rsp,
                                      uint32_t *rsp_size) {
    mlvmapp_error_t ret = MLVM_APP_E_SUCCESS;
...
mlvmapp_error_t is_buffer_secure(uint32_t buff_hyp_handle,
                                 int32_t* buff_fd) {
    mlvmapp_error_t ret = MLVM_APP_E_SUCCESS;
    ...
    *buff_fd = ioctl(scbuf_fd, IOCTL_COG_SCBUF_LOOKUP_HANDLE,
    ...
    printf("IOCTL received buffer_fd [%x %x] ret=%d \n",
           buff_hyp_handle, *buff_fd, ret);
#endif
    return ret;
}

static mlvmapp_error_t frame_notification_handler(mlvmapp_frame_cmd_req_t *cmd,
                                                  uint32_t cmd_size,
                                                  mlvmapp_frame_cmd_rsp_t* rsp,
                                                  uint32_t *rsp_size) {
    mlvmapp_error_t ret = MLVM_APP_E_SUCCESS;
    ...
    *rsp_size = sizeof(*rsp);
    ...
    local_handle = cmd->frame_data[0].in_buffer.buffer_handle;
    if (local_handle > INT_MAX) {
        printf("%s: handle is out of limit\n", __func__);
        ret = MLVM_APP_E_INVALID_PARAM;
        goto ERROR;
    }

    capture_buff_handle = (int32_t) local_handle;
    capture_buff_size = cmd->frame_data[0].in_buffer.size;

    // Prepare and validate custom request structure
    if (cmd->custom_req_offset != 0 &&
        cmd->custom_req_size != 0) {
        // Check that the start of custom region is after the
        // end of the mlvmapp_frame_cmd_req_t struct
    ...
    if (cmd->custom_rsp_offset != 0 &&
        cmd->custom_rsp_size != 0) {
        // Check that the start of custom region is after the
        // end of the seccam_frame_cmd_rsp_t struct
    ...
    // MMAP the buffs
    capture_buff_ipa = mmap(NULL, capture_buff_size,
    ...
    ret = process_frame(&frame_info, capture_buff_ipa, capture_buff_size,
                        &cam_buf_info, custom_req, custom_req_size,
                        custom_rsp, custom_req_size);
...
mlvmapp_error_t processCommand(void *rcv, uint32_t rcv_size,
                               void *rsp, uint32_t *rsp_size) {
    mlvmapp_error_t ret = MLVM_APP_E_INVALID_PARAM;
    mlvmapp_generic_req_t *gen_req = (mlvmapp_generic_req_t *) rcv;

    if (gen_req->cmd_id < MLVM_APP_CMD_NONE &&
        gen_req->cmd_id > MLVM_APP_CMD_LAST) {
...
void *hlos_rcv_cmds(void *arg) {
    uint8_t rcv_buf[MAX_PAYLOAD_SIZE] = {0};
    uint8_t rsp_buf[MAX_PAYLOAD_SIZE] = {0};
    ...
    while (1) {
        if((rcv_size = sock_wait_for_cmd(socket_handle, &rcv_sockaddr,
                                         ...
                                         sizeof(rcv_buf))) > 0) {
            if (processCommand((void *)rcv_buf, rcv_size,
                               (void *)rsp_buf, &rsp_size) ==
                MLVM_APP_E_DEINIT_COMMAND) {
                break;
            }
    ...
    while (1) {
        if ((rcv_size = sock_wait_for_cmd(socket_handle, &rcv_sockaddr,
            ...
            if (gen_req->cmd_id != MLVM_APP_CMD_HLOS_APP_INIT) {
                printf("invalid command %d, continue\n", gen_req->cmd_id);
                continue;
            }
            ret = init_cmd_handler(rcv_buf, rcv_size, rsp_buf, &rsp_size);
            if (rsp_size != 0 &&
                sock_send_response(socket_handle, &rcv_sockaddr,
                                   &rcv_sockaddrlen,
                                   rsp_buf, rsp_size) <= 0) {
                printf("Send response failed \n");
            }
            ...
            printf("invalid command size buffer received, exit\n");
            ret = MLVM_APP_E_UNKNOWN_COMMAND;
            goto EXIT;
        }
    }
