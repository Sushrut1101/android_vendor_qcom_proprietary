...
===========================================================================*/
#define MAX_UDP_ENCAP_SOCKETS 5
#define MAX_OPEN_SOCKETS 601
...
static int                    pr_qmi_state;
static qmi_client_type        pr_qmi_dfs_clnt_hndl;
static qmi_cci_os_signal_type pr_qcci_os_params;

/* List of open handles. Caching these for SSR / cleanup only */
static int  udp_encap_hndls[MAX_UDP_ENCAP_SOCKETS];
...
static socket_info_t remote_sockets[MAX_OPEN_SOCKETS];
static socket_state_t curr_sock_state = NM_MDMPRXY_SOCKET_STATE_SHUTDOWN;
...
static const struct nm_mdmprxy_cmd apps_ports_rule_list[] =
{
  {"%s" NM_MDMPRXY_FLAGS " -t mangle -A nm_mdmprxy_mngl_pre_ex -p tcp"
       " --dport " TOSTR(NM_MDMPRXY_IMS_PORT_START) ":" TOSTR(NM_MDMPRXY_IMS_PORT_END)
  ...
  {"%s" NM_MDMPRXY_FLAGS " -t mangle -A nm_mdmprxy_mngl_pre_ex -p udp"
       " --dport " TOSTR(NM_MDMPRXY_IMS_PORT_START) ":" TOSTR(NM_MDMPRXY_IMS_PORT_END)
       " -j MARK --set-mark 0x%x",
   "%s" NM_MDMPRXY_FLAGS " -t mangle -D nm_mdmprxy_mngl_pre_ex -p udp"
       " --dport " TOSTR(NM_MDMPRXY_IMS_PORT_START) ":" TOSTR(NM_MDMPRXY_IMS_PORT_END)
       " -j MARK --set-mark 0x%x",
   nm_mdmprxy_markjump_formatter },
...
static const struct nm_mdmprxy_cmd udp_encap_rule_list[] =
{
  {"%s" NM_MDMPRXY_FLAGS " -t mangle -I nm_mdmprxy_mark_prov_chain -p udp"
     " --sport 500 --dport %d -j MARK --set-mark 0x%x",
   "%s" NM_MDMPRXY_FLAGS " -t mangle -D nm_mdmprxy_mark_prov_chain -p udp"
     " --sport 500 --dport %d -j MARK --set-mark 0x%x",
   nm_mdmprxy_encaprule_dport_formatter},

  {"%s" NM_MDMPRXY_FLAGS " -t mangle -I nm_mdmprxy_mark_prov_chain -p udp"
     " --dport %d -m u32 --u32 0>>22&0x3C@8=0 -j MARK --set-mark 0x%x",
   "%s" NM_MDMPRXY_FLAGS " -t mangle -D nm_mdmprxy_mark_prov_chain -p udp"
     " --dport %d -m u32 --u32 0>>22&0x3C@8=0 -j MARK --set-mark 0x%x",
   nm_mdmprxy_encaprule_dport_formatter}
};

static int nm_mdmprxy_socket_mark_prov_init(uint32_t);

static int nm_mdmprxy_socket_mark_prov_is_hdlr(void);

static int nm_mdmprxy_socket_mark_prov_oos_hdlr(int);

static void nm_mdmprxy_socket_mark_prov_status(void);

static void nm_mdmprxy_socket_mark_prov_netd_oos_hdlr(void);

static void nm_mdmprxy_socket_mark_prov_netd_is_hdlr(uint32_t);

static int nm_mdmprxy_socket_mark_prov_netd_link_is_hdlr(void*);

static void nm_mdmprxy_socket_mark_prov_change_socket_state(socket_state_t);
...
nm_mdmprxy_mark_prov_t nm_mdmprxy_socket_mark_provider =
{
  ...
  .netd_oos_hdlr         = nm_mdmprxy_socket_mark_prov_netd_oos_hdlr,
  .netd_is_hdlr          = nm_mdmprxy_socket_mark_prov_netd_is_hdlr,
  .netd_is_hdlr_per_link = nm_mdmprxy_socket_mark_prov_netd_link_is_hdlr,
  .change_socket_state   = nm_mdmprxy_socket_mark_prov_change_socket_state,
...
struct mark_prov_iptables socket_mark_prov_cmd_tbl;
...
static uint32_t nm_mdmprxy_oem_mark = 0;
...
#define ADJUST_PORT_RANGE(base_port_value, max_port_value,                  \
                            min_range, max_range, port_range_valid)         \
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_set_pr_qmi_state
===========================================================================*/
/*!
@brief
  Sets the 'pr_qmi_state' to the passed value.
*/
/*=========================================================================*/
void nm_mdmprxy_set_pr_qmi_state(int state)
{
...
*/
/*=========================================================================*/
static void nm_mdmprxy_socket_mark_set_reserved_ports(void)
{
  FILE *fp = NULL;
  const char* enable_blocking_bind = "0";
  const char* port_range = TOSTR(NM_MDMPRXY_IMS_PORT_START) "-" TOSTR(NM_MDMPRXY_IMS_PORT_END) ","
  ...
  fp = fopen(FILEPATH_RESERVED_PORT_BIND, "we");
  if (!fp)
  {
...
@arg handle - remote_sockets handle, not fd
...
*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_get_socket_port(unsigned int handle)
{
  struct sockaddr_storage addr;
  socklen_t len = sizeof(addr);
  int rc;

  rc = getsockname(remote_sockets[handle].fd, (struct sockaddr *)&addr, &len);
  if (rc < 0)
  {
...
*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_get_socket_af(unsigned int handle)
{
  struct sockaddr_storage addr;
  socklen_t len = sizeof(addr);
  int rc;
...
*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_get_socket_type(unsigned int handle)
{
  int type, rc;
  socklen_t optlen = sizeof(type);

  rc = getsockopt(remote_sockets[handle].fd, SOL_SOCKET, SO_PROTOCOL, &type, &optlen);
  if (rc < 0)
  {
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_socket_mark_prov_change_socket_state
===========================================================================*/
/*!
@brief
  Change the state of all relevant sockets

*/
/*=========================================================================*/
static void nm_mdmprxy_socket_mark_prov_change_socket_state(socket_state_t new_sock_state)
{
  int rc = 0;
  int i;
  ...
  for_each_fd(i)
  {
    /* Search the entire list of sockets for TCP sockets and change the state
       on all of them */
    if (SOCK_STREAM != remote_sockets[i].type)
    {
      continue;
    }

    switch (new_sock_state)
    {
    case NM_MDMPRXY_SOCKET_STATE_SHUTDOWN:
      {
        if (NM_MDMPRXY_SOCKET_STATE_LISTENING == remote_sockets[i].sock_state)
        {
          /* If the socket is currently in listening mode we need to change it to shutdown
             mode */
          rc = shutdown(remote_sockets[i].fd, SHUT_RDWR);
          if (rc < 0)
          {
            netmgr_log_err("%s(): failed to socket handle '%d' in shutdown mode! Err: [%d:%s]",
                           __func__, i, errno, strerror(errno));
          }
          else
          {
            /* Socket was successfully shutdown */
            remote_sockets[i].sock_state = NM_MDMPRXY_SOCKET_STATE_SHUTDOWN;
          }
        }

        /* Update the current socket state to shutdown so that the same state is
           reflected for all new sockets */
        if (NM_MDMPRXY_SOCKET_STATE_SHUTDOWN != curr_sock_state)
        {
          netmgr_log_low("%s(): changing global socket state to SHUTDOWN", __func__);
          curr_sock_state = NM_MDMPRXY_SOCKET_STATE_SHUTDOWN;
        }
      }

      break; /* NM_MDMPRXY_SOCKET_STATE_SHUTDOWN */
    case NM_MDMPRXY_SOCKET_STATE_LISTENING:
      {
        if (NM_MDMPRXY_SOCKET_STATE_SHUTDOWN == remote_sockets[i].sock_state)
        {
          /* If the socket is currently in shutdown/bound mode we need to change it to listening
             mode */
          rc = listen(remote_sockets[i].fd, SOMAXCONN);
          if (rc < 0)
          {
            /* We were unable to put the socket back in listening mode, the socket is no
               longer usable so we can close it */
            netmgr_log_err("%s(): unable to put socket on handle '%d' back to listen mode, "
                           "closing it instead! Err: [%d:%s]", __func__, i, errno, strerror(errno));

            close(remote_sockets[i].fd);

            /* Clear the socket state since its been closed */
            memset(&remote_sockets[i], 0, sizeof(socket_info_t));
          }
          else
          {
            /* Socket was successfully put back in listening mode */
            remote_sockets[i].sock_state = NM_MDMPRXY_SOCKET_STATE_LISTENING;
          }
        }

        /* Update the current socket state to listening so that the same state is
           reflected for all new sockets */
        if (NM_MDMPRXY_SOCKET_STATE_LISTENING != curr_sock_state)
        {
          netmgr_log_low("%s(): changing global socket state to LISTENING", __func__);
          curr_sock_state = NM_MDMPRXY_SOCKET_STATE_LISTENING;
        }
      }

      break; /* NM_MDMPRXY_SOCKET_STATE_LISTENING */
    }
  }
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_socket_mark_prov_get_socket_state
===========================================================================*/
static char* nm_mdmprxy_socket_mark_prov_get_socket_state(int handle)
{
  if (remote_sockets[handle].fd != 0)
  {
    if (SOCK_STREAM == remote_sockets[handle].type)
    {
      switch (remote_sockets[handle].sock_state)
      {
      case NM_MDMPRXY_SOCKET_STATE_SHUTDOWN:
        return "SHUTDOWN";
      case NM_MDMPRXY_SOCKET_STATE_LISTENING:
...
static void nm_mdmprxy_socket_mark_prov_status(void)
{
  ...
  for_each_fd(i)
  {
    if (SOCK_STREAM == remote_sockets[i].type)
    {
      netmgr_log_low("sockets[%d]: fd:%d af:%d type:%d port:%d state: %s",
                     i,
                     remote_sockets[i].fd,
                     nm_mdmprxy_socket_mark_get_socket_af(i),
                     nm_mdmprxy_socket_mark_get_socket_type(i),
                     nm_mdmprxy_socket_mark_get_socket_port(i),
                     nm_mdmprxy_socket_mark_prov_get_socket_state(i));
    }
    else
    {
      netmgr_log_low("sockets[%d]: fd:%d af:%d type:%d port:%d",
                     i,
                     remote_sockets[i].fd,
                     nm_mdmprxy_socket_mark_get_socket_af(i),
                     nm_mdmprxy_socket_mark_get_socket_type(i),
...
/*===========================================================================
  FUNCTION nm_mdmprxy_socket_mark_if_udp_encap_cleanup_rules
===========================================================================*/
/*!
@brief
  Check if the handle passed in is for a UDP encap port, if yes cleanup
  the UDP encap rules

@arg handle - udp_encap handle to close
@arg port   - port to close

*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_if_udp_encap_cleanup_rules(int handle, int port)
{
  int i = 0;
  int rc = NETMGR_FAILURE;

  for (i = 0; i < MAX_UDP_ENCAP_SOCKETS; i++)
  {
    if (udp_encap_hndls[i] >= 0 && handle == udp_encap_hndls[i])
    {
      netmgr_log_med("%s(): cleanup UDP_ENCAP handle [%d], port [%d]",
                     __func__, udp_encap_hndls[i], port);

      /* Invalidate handle */
      udp_encap_hndls[i] = -1;

      /* Cleanup IKE port references */
      netmgr_main_cfg_cleanup_ike_port(port);

      rc = nm_mdmprxy_uninstall_udpencap_rules(port, NM_MDMPRXY_IKE_REMOTE_PORT,
                                               nm_mdmprxy_oem_mark);
...
*/
/*=========================================================================*/
static void nm_mdmprxy_socket_mark_close_socket(unsigned int handle)
{
  int port = 0;
  ...
  if (NETMGR_SUCCESS == nm_mdmprxy_socket_mark_if_udp_encap_cleanup_rules(handle, port))
  {
  ...
  close(remote_sockets[handle].fd);
  memset(&remote_sockets[handle], 0, sizeof(socket_info_t));
}
...
*/
/*=========================================================================*/
static void nm_mdmprxy_socket_mark_close_all_ports(void)
{
  int i;
...
/*!
@brief
  Simple linear search over remote_sockets[] to find an unused index
  It is the caller's responsibility to ensure that the return value
  is within range.

@return
  a positive value from 1 to MAX_OPEN_SOCKETS
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_socket_mark_find_empty_udp_encap_handle_index
===========================================================================*/
/*!
@brief
  Search the UDP encap handle array to check if the given handle already
  exists in the list

@return
  NETMGR_FAILURE if the UDP_ENCAP socket for the handle already exists
  NETMGR_SUCCESS otherwise

*/
/*=========================================================================*/
static int
nm_mdmprxy_socket_mark_check_if_udp_encap_port_exists(unsigned int handle)
{
  unsigned int i;

  for (i = 0; i < MAX_UDP_ENCAP_SOCKETS; i++)
  {
    if (udp_encap_hndls[i] == (int) handle)
    {
      netmgr_log_err("%s(): the requested UDP encap handle [%u] was already processed!"
                     " Duplicate request!",__func__, handle);
...
/*!
@brief
  Simple linear search over udp_encap_hndls[] to find an unused index
  It is the caller's responsibility to ensure that the return value
  is within range.

@return
  a positive value from 0 to MAX_UDP_ENCAP_SOCKETS

*/
/*=========================================================================*/
static unsigned int nm_mdmprxy_socket_mark_find_empty_udp_encap_handle_index(void)
{
  unsigned int i;

  for (i = 0; i < MAX_UDP_ENCAP_SOCKETS; i++)
  {
    if (udp_encap_hndls[i] == -1)
      return i;
  }
...
/*!
@brief
  Finds an empty slot in remote_sockets[], allocates a socket,
...
)
{
  ...
  if (source->ss_family == AF_UNSPEC)
    ...
    fd = socket(source->ss_family, type | SOCK_CLOEXEC, 0);
  ...
  rc = bind(fd, (struct sockaddr*)source, slen);
  if (rc < 0)
  {
    close(fd);
  ...
  memset(&remote_sockets[idx], 0, sizeof(socket_info_t));
  remote_sockets[idx].type = type;
  remote_sockets[idx].sock_state = NM_MDMPRXY_SOCKET_STATE_SHUTDOWN;

  if (SOCK_STREAM == type && NM_MDMPRXY_SOCKET_STATE_LISTENING == curr_sock_state)
  {
    ...
    remote_sockets[idx].sock_state = NM_MDMPRXY_SOCKET_STATE_LISTENING;
  }

  remote_sockets[idx].fd = fd;
...
)
{
  ...
  NM_MDMPRXY_VAR_UNUSED(even_port);
  NM_MDMPRXY_VAR_UNUSED(odd_port);
  NM_MDMPRXY_VAR_UNUSED(r_port);
  NM_MDMPRXY_VAR_UNUSED(idx_even_tcp);
  NM_MDMPRXY_VAR_UNUSED(idx_odd_tcp);
  NM_MDMPRXY_VAR_UNUSED(idx_even_udp);
  NM_MDMPRXY_VAR_UNUSED(idx_odd_udp);
...
@arg handle - remote_sockets[] handle
...
*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_set_udpencap(unsigned int handle, int should_set)
{
  int rc, val;
  ...
  rc = setsockopt(remote_sockets[handle].fd, SOL_IP, IP_TRANSPARENT, &val, sizeof(val));
  if (rc < 0)
  {
  ...
  if (should_set)
  {
    rc = setsockopt(remote_sockets[handle].fd, SOL_UDP, UDP_ENCAP, &type, sizeof(type));
    if (rc < 0)
    {
    ...
    /* Set SO_REUSEPORT option to allow CNE to bind to the same UDP encap port
       for NAT keep-alive offload */
    val = 1;
    rc = setsockopt(remote_sockets[handle].fd, SOL_SOCKET, SO_REUSEPORT, &val, sizeof(val));
    if (rc < 0)
    {
      netmgr_log_err("%s(): Unable to set SO_REUSEPORT: %d-%s\n",
                     __func__, errno, strerror(errno));
      return NETMGR_FAILURE;
    }
  }
...
)
{
  ...
  dfs_remote_socket_allocated_req_msg_v01  resp_req_msg;
  dfs_remote_socket_allocated_resp_msg_v01 *resp_resp_msg=NULL;
  ...
  if (TRUE == req_msg.port_pref_valid)
  {
    ...
    /* Use the base_port and max_port values passed by modem to determine the port
       range to be used */
    ADJUST_PORT_RANGE(base_port_value, max_port_value, min_range, max_range, port_range_valid);
    if (FALSE == port_range_valid)
    {
      netmgr_log_err("%s(): modem did not pass valid port range values!", __func__);
      return;
    }
  }
  else
  ...
  for (i = 0; i < req_msg.request_socket_list_len && i < QMI_DFS_MAX_ALLOCATED_SOCKETS_V01; i++)
  {
    ...
    case DFS_PORT_PROP_ADJACENT_START_EVEN_V01:
      ...
      adj_port_params.mark = nm_mdmprxy_oem_mark;
    ...
    case DFS_PORT_PROP_SYMMETRIC_V01:
      ...
      symm_port_params.mark = nm_mdmprxy_oem_mark;
    ...
    default:
      ...
      def_port_params.mark = nm_mdmprxy_oem_mark;
  ...
  resp_resp_msg = (dfs_remote_socket_allocated_resp_msg_v01*) malloc( sizeof(dfs_remote_socket_allocated_resp_msg_v01) );
  if( !resp_resp_msg )
  {
    netmgr_log_err("%s: Failed to allocate memory for resp_resp_msg \n",__func__);
    return;
  }
...
)
{
  ...
  if (req_msg.is_udp_encaps_valid)
  {
    ...
    {
      /* Check if we have already received a request for the same handle */
      if (NETMGR_SUCCESS != nm_mdmprxy_socket_mark_check_if_udp_encap_port_exists(handle))
      {
        return;
      }

      /* Find an empty slot for saving UDP_ENCAP handle information */
      udp_encap_hndl_idx = nm_mdmprxy_socket_mark_find_empty_udp_encap_handle_index();
      if (MAX_UDP_ENCAP_SOCKETS <= udp_encap_hndl_idx)
      {
        netmgr_log_err("%s(): could not find an empty slot for storing UDP_ENCAP handle"
      ...
      rc = nm_mdmprxy_install_udpencap_rules(port, NM_MDMPRXY_IKE_REMOTE_PORT,
                                             nm_mdmprxy_oem_mark);
      if (rc != NETMGR_SUCCESS)
      {
      ...
      netmgr_main_cfg_set_ike_port(port, FALSE);

      /* Store the UDP encap handle reference */
      udp_encap_hndls[udp_encap_hndl_idx] = handle;
    }
    else
    {
      ...
      rc = nm_mdmprxy_socket_mark_if_udp_encap_cleanup_rules(handle, port);
      if (rc != NETMGR_SUCCESS)
      {
        netmgr_log_err("%s(): Failed to uninstall UDP Encap rules", __func__);
      }
...
)
{
  ...
  switch(msg_id)
  {
    case QMI_DFS_REMOTE_SOCKET_REQUEST_IND_V01:
      nm_mdmprxy_socket_mark_qmi_do_request_ind(user_handle, msg_id, ind_buf,
                                                ind_buf_len, ind_cb_data);
      break;

    case QMI_DFS_REMOTE_SOCKET_RELEASE_IND_V01:
      nm_mdmprxy_socket_mark_qmi_do_release_ind(user_handle, msg_id, ind_buf,
                                                ind_buf_len, ind_cb_data);
      break;

    case QMI_DFS_REMOTE_SOCKET_SET_OPTION_IND_V01:
      nm_mdmprxy_socket_mark_qmi_do_set_option_ind(user_handle, msg_id, ind_buf,
                                                   ind_buf_len, ind_cb_data);
      break;
...
*/
/*=========================================================================*/
static void nm_mdmprxy_socket_mark_release_qmi_clients(void)
{
  ...
  nm_mdmprxy_set_pr_qmi_state(PR_QMI_STATE_UNINITIALIZED);
}
...
*/
/*=========================================================================*/
static void nm_mdmprxy_socket_mark_init_qmi_clients(void)
{
  int rc;
  ...
  nm_mdmprxy_set_pr_qmi_state(PR_QMI_STATE_INITIALIZED);
  return;
...
*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_uninstall_static_rules(sa_family_t af, int start)
{
  int i = 0;
  int rc = NETMGR_FAILURE;
  ...
  {
    netmgr_log_low("%s(): static rules are not present. Nothing to do.", __func__);
    return NETMGR_SUCCESS;
  }
...
*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_install_static_rules(sa_family_t af)
{
  int num_rules = 0, i = 0;
  int rc = NETMGR_FAILURE;
  ...
  {
    netmgr_log_low("%s(): static rules are already present. Nothing to do.", __func__);
    return NETMGR_SUCCESS;
  }
...
*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_uninstall_apps_rules(sa_family_t af, int start)
{
  int i = 0;
  int rc = NETMGR_FAILURE;
...
*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_install_apps_rules(sa_family_t af)
{
  int num_rules = 0, i = 0;
  int rc = NETMGR_FAILURE;
  struct nm_mdmprxy_iiface_rule_info nfo;
...
*/
/*=========================================================================*/
static void nm_mdmprxy_socket_mark_iptables_init(void)
{
...
*/
/*=========================================================================*/
static void nm_mdmprxy_socket_mark_iptables_uninit(int should_remove_static_rules)
{
  if (should_remove_static_rules)
  {
     /* This function is overloaded for handling more than one scenario
        ...
        * Modem SSR
        ...
        The static rules installed by socket_mark_prov depend on the packet
...
/*====================================================================================
  FUNCTION  nm_mdmprxy_socket_mark_prov_is_hdlr
====================================================================================*/
/*!
@brief
...
/*====================================================================================
  FUNCTION  nm_mdmprxy_socket_mark_prov_oos_hdlr
====================================================================================*/
/*!
@brief
...
@param
   should_remove_static_rules - determines whether static iptables rules installed
                                by provider should be cleaned up
...
*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_prov_oos_hdlr(int should_remove_static_rules)
{
  ...
  /* If modem subsystem is retarted all reverse links will be brought down and
     will be re-requested. Since links will be going down we will need to reset
     the global socket state to 'SHUTDOWN' so that when sockets are re-requested
     they go back to non-listening mode. Eventually LBO interface would come
     up and sockets will be put in the 'LISTENING' state.

     When netd is restarted, we will need to close all the previously opened
     remote sockets. However, we have observed that the LBO link is not torn
     down. This means we will not get any further trigger from modem using
     which we can force the new remote sockets in listenting mode. As a result,
     for netd restart scenario we will leave the socket in the state they were
     in prior to restart */
  if (!should_remove_static_rules)
  {
    curr_sock_state = NM_MDMPRXY_SOCKET_STATE_SHUTDOWN;
  ...
  /* Remove common rules */
  nm_mdmprxy_socket_mark_iptables_uninit(should_remove_static_rules);
  nm_mdmprxy_socket_mark_unlock();
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_socket_mark_prov_netd_oos_hdlr
===========================================================================*/
/*!
@brief
...
static void nm_mdmprxy_socket_mark_prov_netd_oos_hdlr(void)
{
  /* We are calling OOS handler function here because the impact of
     netd restarting now is similar to how SSR would be handled

     --- Socket Cleanup ---
     Netmgr needs to close sockets associated with the old packet mark
     otherwise routing will break.

     Releasing the QMI clients should result in cleanup of sockets on
     the modem. We assume that the sockets will be requested for again
     once new QMI clients are created

     --- IPTables cleanup ---
     Cleanup the iptables rules associated with the old packet mark */
  nm_mdmprxy_common_mark_prov_netd_oos_hdlr();
  nm_mdmprxy_socket_mark_prov_oos_hdlr(TRUE);

  nm_mdmprxy_oem_mark = NM_MDMPRXY_INVALID_PACKET_MARK;
}

/*===========================================================================
  FUNCTION  nm_mdmprxy_socket_mark_prov_netd_is_hdlr
===========================================================================*/
/*!
@brief
...
*/
/*=========================================================================*/
static void nm_mdmprxy_socket_mark_prov_netd_is_hdlr(uint32_t packet_mark)
{
  nm_mdmprxy_oem_mark = packet_mark;
  ...
  /* Use the new_packet_mark received from netd and re-install the
     iptables rules setup by common mark provider module */
  nm_mdmprxy_common_mark_prov_netd_is_hdlr(packet_mark);

  /* Re-initialize QMI clients to allow modem to request for new sockets */
  nm_mdmprxy_socket_mark_prov_is_hdlr();
}

/*===========================================================================
  FUNCTION  nm_mdmprxy_socket_mark_prov_netd_link_is_hdlr
===========================================================================*/
/*!
@brief
...
*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_prov_netd_link_is_hdlr(void* arg)
{
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_socket_mark_prov_init
===========================================================================*/
/*!
@brief
  Initialization routine for socket mark provider
...
*/
/*=========================================================================*/
static int nm_mdmprxy_socket_mark_prov_init(uint32_t packet_mark)
{
  int rc = NETMGR_SUCCESS;
  ...
  rc = nm_mdmprxy_common_mark_prov_init(&socket_mark_prov_cmd_tbl, packet_mark);
  if (NETMGR_SUCCESS != rc)
  {
  ...
  nm_mdmprxy_socket_mark_init_qmi_clients();
  nm_mdmprxy_socket_mark_lock();
  memset(udp_encap_hndls, -1, sizeof(udp_encap_hndls));
  nm_mdmprxy_socket_mark_iptables_init();
  nm_mdmprxy_socket_mark_unlock();
