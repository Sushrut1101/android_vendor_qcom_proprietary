...
)
{
  ...
  do
  {
      if ( params_ptr->datalen == 0 || params_ptr->data == NULL  )
      {
          ...
          qcril_default_request_resp_params( QCRIL_DEFAULT_INSTANCE_ID, params_ptr->t, params_ptr->event_id, RIL_E_INVALID_ARGUMENTS, &resp );
...
)
{
  ...
  else
  {
      ...
      else
      {
          ...
          if ( qcril_qmi_nas_get_modem_stack_id() == QCRIL_DEFAULT_MODEM_STACK_ID && RIL_E_SUCCESS == ril_req_res && TRUE == get_mode_pref_resp_msg.idx0_mode_pref_valid )
          {
              mode_pref_valid = TRUE;
              mode_pref       = get_mode_pref_resp_msg.idx0_mode_pref;
          }
          else if ( qcril_qmi_nas_get_modem_stack_id() == QCRIL_MODEM_SECONDARY_STACK_ID && RIL_E_SUCCESS == ril_req_res && TRUE == get_mode_pref_resp_msg.idx1_mode_pref_valid )
          {
              mode_pref_valid = TRUE;
              mode_pref       = get_mode_pref_resp_msg.idx1_mode_pref;
          }
          else if ( qcril_qmi_nas_get_modem_stack_id() == QCRIL_MODEM_TERTIARY_STACK_ID && RIL_E_SUCCESS == ril_req_res && TRUE == get_mode_pref_resp_msg.idx2_mode_pref_valid )
          {
              mode_pref_valid = TRUE;
...
)
{
  ...
  {
      QCRIL_LOG_ERROR("Invalid arguments..null params");
      qcril_default_request_resp_params( instance_id, params_ptr->t, params_ptr->event_id, RIL_E_INVALID_ARGUMENTS, &resp );
      qcril_send_request_response( &resp );
          break;
  }
  ...
  {
    if(roam_pref == set_system_selection_preference_req_msg.roam_pref)
    {
  ...
  else
  {
      if(set_system_selection_preference_resp_msg.resp.result == QMI_RESULT_FAILURE_V01)
      {
          ...
          qcril_default_request_resp_params( instance_id, params_ptr->t,
           params_ptr->event_id,
           qcril_qmi_util_convert_qmi_response_codes_to_ril_result(QMI_NO_ERR, &set_system_selection_preference_resp_msg.resp),
           &resp );
          qcril_send_request_response( &resp );
      }
      else
...
)
{
  ...
  else
  {
      if(get_system_selection_preference_resp_msg.resp.result == QMI_RESULT_FAILURE_V01)
      {
          ...
          qcril_default_request_resp_params( instance_id, params_ptr->t,
           params_ptr->event_id,
           qcril_qmi_util_convert_qmi_response_codes_to_ril_result(QMI_NO_ERR, &get_system_selection_preference_resp_msg.resp),
           &resp );
          qcril_send_request_response( &resp );
      }
      else
      {
          if(get_system_selection_preference_resp_msg.roam_pref_valid == TRUE)
          {
              ...
              else
              {
                  ...
                  qcril_default_request_resp_params( instance_id, params_ptr->t, params_ptr->event_id, RIL_E_MODEM_ERR, &resp );
                  qcril_send_request_response( &resp );
              }
          }
          else
...
RIL_Errno qcril_qmi_nas2_create_reqlist_setup_timer_helper( const qcril_request_params_type *const params_ptr )
{
    ...
    if ( NULL != params_ptr )
    {
        ...
        if ( (internal_err = qcril_reqlist_new( instance_id, &qcril_req_info_ptr )) == E_SUCCESS )
        {
            QCRIL_LOG_INFO( "Added entry for QCRIL_EVT_QMI_REQUEST_SET_SYS_SEL_PREF " );
            if( (internal_err = qcril_qmi_nas_sys_sel_pref_setup_timed_callback()) != E_SUCCESS )
            {
