...
private:
    ...
    int min_cores[MAX_CLUSTER];
    int max_cores[MAX_CLUSTER];
    ...
    int cluster2_interactive_node_storage_length[ MAX_INTERACTIVE_MINOR_OPCODE];
    char cluster2_interactive_node_storage[MAX_INTERACTIVE_MINOR_OPCODE][NODE_MAX];

    /*MEM_LAT Storage Variables*/
    int memlat_minfreq_node_strg_len[MAX_CLUSTER][MAX_MEMLAT_MINOR_OPCODE];
    ...
    int sch_upmigrate_sl[MAX_CLUSTER];

    char sch_downmigrate_s[MAX_CLUSTER][NODE_MAX];
    int sch_downmigrate_sl[MAX_CLUSTER];
...
public:
    ...
    unsigned int find_next_cpubw_available_freq(unsigned int freq);
     unsigned int find_next_llccbw_available_freq(unsigned int freq);
    int init_cpubw_hwmon_path();

    int init_llcbw_hwmon_path();

    /*Functions to handle new Opcode for llccbw and cpubw*/
    int init_llcbw_hwmon_path_newOpcode();
    int init_cpubw_hwmon_path_newOpcode();
