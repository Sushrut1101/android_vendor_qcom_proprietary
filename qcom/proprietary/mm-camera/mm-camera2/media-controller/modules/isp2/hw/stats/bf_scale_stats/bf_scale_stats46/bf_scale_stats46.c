 ...
 * Return void
 **/
static void bf_scale_stats46_debug(ISP_StatsBf_Scale_CfgCmdType *pcmd)
{
  ...
  ISP_DBG("b00        %d", pcmd->b00);
  ISP_DBG("b01        %d", pcmd->b01);
  ISP_DBG("b02        %d", pcmd->b02);
  ISP_DBG("b03        %d", pcmd->b03);
  ISP_DBG("b04        %d", pcmd->b04);
  ISP_DBG("b05        %d", pcmd->b05);
  ISP_DBG("b06        %d", pcmd->b06);
  ISP_DBG("b07        %d", pcmd->b07);
  ISP_DBG("b08        %d", pcmd->b08);
  ISP_DBG("b09        %d", pcmd->b09);
  ISP_DBG("b10        %d", pcmd->b10);
  ISP_DBG("b11        %d", pcmd->b11);
  ISP_DBG("b12        %d", pcmd->b12);
  ISP_DBG("b13        %d", pcmd->b13);
  ISP_DBG("b14        %d", pcmd->b14);
  ISP_DBG("b15        %d", pcmd->b15);
  ISP_DBG("b16        %d", pcmd->b16);
  ISP_DBG("b17        %d", pcmd->b17);
  ISP_DBG("b18        %d", pcmd->b18);
  ISP_DBG("b19        %d", pcmd->b19);
  ISP_DBG("b20        %d", pcmd->b20);
  ISP_DBG("b21        %d", pcmd->b21);
}
  ...
  uint32_t *phase_init, uint32_t *phase_mult)
{
  ...
  *mn_init = offset * M % N;
  *phase_init = (*mn_init << (14 + *interp_reso)) / M;
  *phase_mult = (N << (14 + *interp_reso)) / M;
}

/** bf_scale_stats46_store_hw_update:
 *
 ...
 **/
static int bf_scale_stats46_store_hw_update(isp_sub_module_t *isp_sub_module,
  bf_scale_stats46_t *bf_scale_stats)
{
  boolean                        ret = TRUE;
  ISP_StatsBf_Scale_CfgCmdType  *pcmd = NULL;
  struct msm_vfe_cfg_cmd2       *cfg_cmd = NULL;
  struct msm_vfe_reg_cfg_cmd    *reg_cfg_cmd = NULL;
  ISP_StatsBf_Scale_CfgCmdType  *copy_cmd = NULL;
  ...
  copy_cmd = (ISP_StatsBf_Scale_CfgCmdType *)CAM_MALLOC(sizeof(*copy_cmd));
  if (!copy_cmd) {
  ...
  /*
   * Values of hIn,hOut,scaleYInWidth,vIn,vOut
   * and scaleYInHeightare decreased by 1
  ...
  *copy_cmd = *pcmd;
  ...
  copy_cmd->hIn            -= 1;
  copy_cmd->hOut          -= 1;
  copy_cmd->scaleYInWidth  -= 1;
  copy_cmd->vIn            -= 1;
  copy_cmd->vOut           -= 1;
  copy_cmd->scaleYInHeight -= 1;
  ...
  reg_cfg_cmd[1].u.rw_info.reg_offset = BF_SCALE_STATS_OFF;
  reg_cfg_cmd[1].u.rw_info.len = BF_SCALE_STATS_LEN * sizeof(uint32_t);
...
/** bf_scale_stats46_update_min_stripe_overlap
 *
 *  @isp_sub_module: isp sub module handle
...
{
  boolean                        ret = TRUE;
  bf_scale_stats46_t                   *bf_scale_stats = NULL;
  ...
  bf_scale_stats = (bf_scale_stats46_t *)isp_sub_module->private_data;
  if (!bf_scale_stats) {
 ...
 * Return void
 **/
static boolean bf_scale_stats46_split_config(bf_scale_stats46_t *bf_scale_stats,
  ...
  uint32_t single_rgnWidth, uint32_t scale_ratio)
{
  ...
  if (isp_out->is_split == TRUE) {
    ...
    if (isp_out->stripe_id == ISP_STRIPE_LEFT) {
      ...
      pcmd->rgnHNum =  (bf_scale_stats->num_left_rgns > 0) ?
        bf_scale_stats->num_left_rgns - 1 : 0;
      pcmd->rgnHOffset = (bf_scale_stats->num_left_rgns > 0) ?
        FLOOR2(single_rgnHOffset) : MIN_RGN_H_OFFSET;
    } else { /* ISP_STRIPE_RIGHT */
      pcmd->rgnHNum = (bf_scale_stats->num_right_rgns > 0) ?
        bf_scale_stats->num_right_rgns - 1 : 0;
      pcmd->rgnHOffset = (upscale_single_rgn_HOffset +
        (bf_scale_stats->num_left_rgns * upscale_single_rgn_width) -
        isp_out->right_stripe_offset) / scale_ratio;
      pcmd->rgnHOffset = (bf_scale_stats->num_right_rgns > 0) ?
...
/** bf_scale_stats46_fill_stats_parser_params:
 *
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean bf_scale_stats46_fill_stats_parser_params(bf_scale_stats46_t *bf_scale_stats,
...
/** bf_scale_stats46_trigger_update:
 *
 *  @isp_sub_module: isp sub module handle
  ...
  void *data)
{
  ...
  if ((isp_sub_module->submod_enable == FALSE) ||
    ...
    if (sub_module_output->stats_params) {
      ret = bf_scale_stats46_fill_stats_parser_params(bf_scale_stats,
        sub_module_output->stats_params);
      if (ret == FALSE) {
        ISP_ERR("failed: bf_scale_stats46_fill_stats_parser_params");
      }
    }
  ...
  pcmd->hIn = MIN(ISP_STATS_BF_SCALE_MAX_H_CFG, bf_scale_stats->camif.width);
  pcmd->vIn = MIN(ISP_STATS_BF_SCALE_MAX_V_CFG, bf_scale_stats->camif.height);

  pcmd->hOut = pcmd->hIn / scale_ratio;
  pcmd->vOut = pcmd->vIn / scale_ratio;

  bf_scale_stats_calculate_phase(pcmd->hOut, pcmd->hIn, 0,
    &interp_reso, &mn_init, &phase_init, &phase_mult);
  pcmd->horizInterResolution = interp_reso;
  pcmd->horizMNInit = mn_init;
  pcmd->horizPhaseInit = phase_init;
  pcmd->horizPhaseMult = phase_mult;
  pcmd->scaleYInWidth = pcmd->hIn;
  pcmd->hSkipCount = 0;
  bf_scale_stats_calculate_phase(pcmd->vOut, pcmd->vIn, 0,
    &interp_reso, &mn_init, &phase_init, &phase_mult);
  pcmd->vertInterResolution = interp_reso;
  pcmd->vertMNInit = mn_init;
  pcmd->vertPhaseInit = phase_init;
  pcmd->vertPhaseMult = phase_mult;
  pcmd->scaleYInHeight = pcmd->vIn;
  pcmd->vSkipCount = 0;

  /* min of rgn_v_offset = 2, min of rgn_h_offset = 20
     rgn_v_offset = 2 + roi_top,  rgn_h_offset = 20 + roi_left */
  pcmd->rgnHOffset = FLOOR2(bf_scale_stats->scale_roi.bf_scaled_left);
  pcmd->rgnVOffset = FLOOR2(bf_scale_stats->scale_roi.bf_scaled_top);
  pcmd->rgnWidth   = FLOOR2(bf_scale_rgn_width) - 1;
  pcmd->rgnHeight  = FLOOR2(bf_scale_rgn_height) - 1;
  ...
  bf_scale_config->roi.left = pcmd->rgnHOffset;
  bf_scale_config->roi.top = pcmd->rgnVOffset;
  bf_scale_config->roi.width = (pcmd->rgnWidth + 1) *
    af_config->common.grid_info.h_num;
  bf_scale_config->roi.height = (pcmd->rgnHeight + 1) *
    af_config->common.grid_info.v_num;

  pcmd->r_fv_min   = bf_scale_config->r_min  << FV_MIN_SHIFT;
  pcmd->gr_fv_min  = bf_scale_config->gr_min << FV_MIN_SHIFT;
  pcmd->b_fv_min   = bf_scale_config->b_min  << FV_MIN_SHIFT;
  pcmd->gb_fv_min  = bf_scale_config->gb_min << FV_MIN_SHIFT;

  pcmd->b00 = bf_scale_config->hpf[0];
  pcmd->b01 = bf_scale_config->hpf[1];
  pcmd->b02 = bf_scale_config->hpf[2];
  pcmd->b03 = bf_scale_config->hpf[3];
  pcmd->b04 = bf_scale_config->hpf[4];
  pcmd->b05 = bf_scale_config->hpf[5];
  pcmd->b06 = bf_scale_config->hpf[6];
  pcmd->b07 = bf_scale_config->hpf[7];
  pcmd->b08 = bf_scale_config->hpf[8];
  pcmd->b09 = bf_scale_config->hpf[9];
  pcmd->b10 = bf_scale_config->hpf[10];
  pcmd->b11 = bf_scale_config->hpf[11];
  pcmd->b12 = bf_scale_config->hpf[12];
  pcmd->b13 = bf_scale_config->hpf[13];
  pcmd->b14 = bf_scale_config->hpf[14];
  pcmd->b15 = bf_scale_config->hpf[15];
  pcmd->b16 = bf_scale_config->hpf[16];
  pcmd->b17 = bf_scale_config->hpf[17];
  pcmd->b18 = bf_scale_config->hpf[18];
  pcmd->b19 = bf_scale_config->hpf[19];
  pcmd->b20 = bf_scale_config->hpf[20];
  pcmd->b21 = bf_scale_config->hpf[21];

  ret = bf_scale_stats46_split_config(bf_scale_stats, pcmd->rgnHNum +1,
    pcmd->rgnHOffset, pcmd->rgnWidth + 1, scale_ratio);
  if (ret == FALSE) {
    ISP_ERR("bf_scale_stats46_split_config failed");
    goto ERROR;
  }

  /* rgnHOffset needs to be at least 20 and at most camif_window_width - 2 */
  if (pcmd->rgnHOffset < MIN_RGN_H_OFFSET ||
    pcmd->rgnHOffset >= bf_scale_stats->camif.width - 2) {
  ...
  ret = bf_scale_stats46_store_hw_update(isp_sub_module, bf_scale_stats);
  if (ret == FALSE) {
    ISP_ERR("failed: bf_scale_stats46_store_hw_update");
    goto ERROR;
  }
...
/** bf_scale_stats46_stats_config_update:
 *
 *  @isp_sub_module: isp sub module handle
...
/** bf_scale_stats46_set_stripe_info:
 *
 *  @isp_sub_module: isp sub module handle
...
/** bf_scale_stats46_set_split_info:
 *
 *  @isp_sub_module: isp sub module handle
 *  @data: handle to sensor_out_info_t
 *
...
/** bf_scale_stats46_set_stream_config:
 *
 *  @isp_sub_module: isp sub module handle
 *  @data: handle to sensor_out_info_t
 *
  ...
  void *data)
{
  ...
  pcmd = &bf_scale_stats->pcmd;
  memset(pcmd, 0, sizeof(ISP_StatsBf_Scale_CfgCmdType));

  pcmd->rgnHNum    = 17;
  pcmd->rgnVNum    = 13;
  pcmd->r_fv_min   = 30 << FV_MIN_SHIFT;
  pcmd->gr_fv_min  = 30 << FV_MIN_SHIFT;
  pcmd->b_fv_min   = 30 << FV_MIN_SHIFT;
  pcmd->gb_fv_min  = 30 << FV_MIN_SHIFT;
  pcmd->b00        = -4;
  pcmd->b01        = -4;
  pcmd->b02        = 1;
  pcmd->b03        = 2;
...
boolean bf_scale_stats46_init(isp_sub_module_t *isp_sub_module)
{
  ...
  bf_scale_stats = (bf_scale_stats46_t *)CAM_MALLOC(sizeof(bf_scale_stats46_t));
  if (!bf_scale_stats) {
    ISP_ERR("failed: bf_scale_stats %p", bf_scale_stats);
    PTHREAD_MUTEX_UNLOCK(&isp_sub_module->mutex);
    return FALSE;
