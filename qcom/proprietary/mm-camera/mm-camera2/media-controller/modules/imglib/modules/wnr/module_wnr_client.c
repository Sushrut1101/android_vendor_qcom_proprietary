...
#ifdef CHROMATIX_304
#define MODULE_DENOISE_LIB_CHROMATIX_VERSION (0x304)
#elif defined CHROMATIX_306
#define MODULE_DENOISE_LIB_CHROMATIX_VERSION (0x306)
#elif defined CHROMATIX_307
#define MODULE_DENOISE_LIB_CHROMATIX_VERSION (0x307)
#elif defined CHROMATIX_308 || defined(CHROMATIX_308E)
#define MODULE_DENOISE_LIB_CHROMATIX_VERSION (0x308)
#elif defined CHROMATIX_309
#define MODULE_DENOISE_LIB_CHROMATIX_VERSION (0x309)
#elif defined CHROMATIX_310 || defined CHROMATIX_310E
#define MODULE_DENOISE_LIB_CHROMATIX_VERSION (0x310)
#else
  ...
  img_frame_t *pframe, int native_buf)
{
  ...
  if( (info->reprocess_config.pp_feature_config.rotation == ROTATE_270) ||
    ...
    pframe->info.width = (uint32_t)info->dim.height;
    pframe->info.height = (uint32_t)info->dim.width;
    ...
    pframe->info.width = (uint32_t)info->dim.width;
    pframe->info.height = (uint32_t)info->dim.height;
  ...
  size = (uint32_t)(pframe->info.width * pframe->info.height);
  padded_size = stride * scanline;

  /* check actually how many */
  pframe->frame[0].plane_cnt = (uint32_t)info->buf_planes.plane_info.num_planes;
  buf_idx = buf_divert->buffer.index;
  pframe->idx = buf_divert->buffer.index;
  ...
  pframe->timestamp = (uint64_t)buf_divert->buffer.timestamp.tv_sec * 1000000
...
static int module_wnr_update_offline_params(wnr_client_t *p_client)
{
  ...
  if (sizeof(local_meta->isp_stats_awb_data.private_data) <
    sizeof(awb_update_t)) {
    IDBG_ERROR("%s:%d] awb private data mismatch", __func__, __LINE__);
    return IMG_ERR_GENERAL;
  }
  ...
  if (sizeof(local_meta->isp_gamma_data.private_data) <
    sizeof(img_gamma_t)) {
    IDBG_ERROR("%s:%d] Gamma private data mismatch", __func__, __LINE__);
    return IMG_ERR_GENERAL;
  }
...
int module_wnr_configure_client(wnr_client_t *p_client)
{
  ...
  chromatix_version_type chromatix_version = 0xFFFF;
...
#if defined (CHROMATIX_309) || defined (CHROMATIX_310) || defined (CHROMATIX_310E)
    chromatix_version = chromatix->chromatix_version_info.chromatix_version;
#else
    chromatix_version = chromatix->chromatix_version;
#endif

    if (MODULE_DENOISE_LIB_CHROMATIX_VERSION != chromatix_version) {
  ...
  defined (CHROMATIX_308) || defined (CHROMATIX_309) || \
  (defined (CHROMATIX_310) && !defined(_USE_OEM_CHROMATIX_))
  int gamma_down_sample_ratio = GAMMA_TABLE_SIZE/GAMMA_TABLE_ENTRIES;
  for (i=0; i<IMGLIB_ARRAY_SIZE(low_gamma.table)-1; i++) {
    gamma.table[i] = (uint16_t)(chromatix_gamma->default_gamma_table.
      gamma_G[(int)i * gamma_down_sample_ratio] +
      ((chromatix_gamma->default_gamma_table.gamma_G[(int)i *
      gamma_down_sample_ratio + 1] -
      chromatix_gamma->default_gamma_table.gamma_G[(int)i *
      gamma_down_sample_ratio]) << 8));
  }
  gamma.table[i] = (uint16_t)(chromatix_gamma->default_gamma_table.
    gamma_G[GAMMA_TABLE_SIZE - 1] + 0xff00);
#elif defined(CHROMATIX_308E) || defined(CHROMATIX_310E)
    int gamma_down_sample_ratio = GAMMA_TABLE_SIZE/GAMMA_TABLE_ENTRIES;
    for (i=0; i<IMGLIB_ARRAY_SIZE(low_gamma.table)-1; i++) {
      gamma.table[i] = (uint16_t)(chromatix_gamma->gamma_table[DEFAULT_GAMMA_IDX].
        gamma_G[(int)i * gamma_down_sample_ratio] +
        ((chromatix_gamma->gamma_table[DEFAULT_GAMMA_IDX].gamma_G[(int)i *
        gamma_down_sample_ratio + 1] -
        chromatix_gamma->gamma_table[DEFAULT_GAMMA_IDX].gamma_G[(int)i *
        gamma_down_sample_ratio]) << 8));
    }
...
#else
    for (i=0; i<IMGLIB_ARRAY_SIZE(low_gamma.table)-1; i++) {
      gamma.table[i] = (uint16_t)(chromatix_gamma->gamma_table[DEFAULT_GAMMA_IDX].gamma[i] +
        ((chromatix_gamma->gamma_table[DEFAULT_GAMMA_IDX].gamma[i + 1] -
        chromatix_gamma->gamma_table[DEFAULT_GAMMA_IDX].gamma[i]) << 8));
    }
    gamma.table[i] = (uint16_t)(chromatix_gamma->gamma_table[DEFAULT_GAMMA_IDX].gamma[i] + 0xff00);
#endif
  ...
  for (i=0; i<IMGLIB_ARRAY_SIZE(low_gamma.table)-1; i++) {
    low_gamma.table[i] =
      (uint16_t)(chromatix_gamma->lowlight_gamma_table.gamma_G[(int)i *
      gamma_down_sample_ratio] +
      ((chromatix_gamma->lowlight_gamma_table.gamma_G[(int)i *
      gamma_down_sample_ratio + 1] -
      chromatix_gamma->lowlight_gamma_table.gamma_G[(int)i *
      gamma_down_sample_ratio]) << 8));
   }
   low_gamma.table[i] = (uint16_t)(chromatix_gamma->
     lowlight_gamma_table.gamma_G[GAMMA_TABLE_SIZE - 1] + 0xff00);
#elif defined(CHROMATIX_308E) || defined(CHROMATIX_310E)
  int gamma_down_sample_ratio = GAMMA_TABLE_SIZE/GAMMA_TABLE_ENTRIES;
  for (i=0; i<IMGLIB_ARRAY_SIZE(low_gamma.table)-1; i++) {
    low_gamma.table[i] =
      (uint16_t)(chromatix_gamma->gamma_table[LOWLIGHT_GAMMA_IDX].gamma_G[(int)i *
      gamma_down_sample_ratio] +
      ((chromatix_gamma->gamma_table[LOWLIGHT_GAMMA_IDX].gamma_G[(int)i *
      gamma_down_sample_ratio + 1] -
      chromatix_gamma->gamma_table[LOWLIGHT_GAMMA_IDX].gamma_G[(int)i *
      gamma_down_sample_ratio]) << 8));
   }
   low_gamma.table[i] = (uint16_t)(chromatix_gamma->
     gamma_table[LOWLIGHT_GAMMA_IDX].gamma_G[GAMMA_TABLE_SIZE - 1] + 0xff00);
#else
   for (i=0; i<IMGLIB_ARRAY_SIZE(low_gamma.table)-1; i++) {
     low_gamma.table[i] =
       (uint16_t)(chromatix_gamma-> gamma_table[LOWLIGHT_GAMMA_IDX].gamma[i] +
       ((chromatix_gamma->gamma_table[LOWLIGHT_GAMMA_IDX].gamma[i + 1] -
       chromatix_gamma->gamma_table[LOWLIGHT_GAMMA_IDX].gamma[i]) << 8));
    }
    low_gamma.table[i] =
      (uint16_t)(chromatix_gamma->gamma_table[LOWLIGHT_GAMMA_IDX].gamma[i] + 0xff00);
#endif
    ...
    IDBG_MED("%s : IMG_COMP_SET_PARAM (QWD_EARLY_CB) rc %d", __func__, rc);
    if (rc != IMG_SUCCESS) {
...
int module_wnr_configure_chroma_buffer(img_frame_t *p_frame)
{
  ...
  uint32_t chroma_width =0, chroma_height = 0, stride = 0;
  uint32_t i = 0, j = 0;
...
int module_wnr_client_exec(wnr_client_t *p_client)
{
  ...
  if (p_client->p_buf_divert_data->is_uv_subsampled) {
    p_frame[0].frame[0].plane[IC].width = p_frame[0].info.width;
    ...
    rc = module_wnr_configure_chroma_buffer(p_frame);
    if (rc != IMG_SUCCESS) {
      IDBG_ERROR("%s %d: Padding chroma buffer failed",  __func__, __LINE__);
    }
  }
  ...
  if (p_client->debug_info.camera_dump_enabled) {
    char filename[40];
    char *s = "Pre_WNR";
    img_get_timestamp(p_client->debug_info.timestamp,
        sizeof(p_client->debug_info.timestamp));
    snprintf(filename, sizeof (filename), "%s_%s",
      p_client->debug_info.timestamp, s);
    img_dump_frame(p_client->frame, filename,
      (uint32_t)p_frame[0].frame_id, NULL);
    p_client->p_buf_divert_data->buffer_access |= CPU_HAS_READ;
  }
  ...
  img_common_handle_input_frame_cache_op(p_client->exec_mode,
    p_frame, p_client->ion_fd, p_client->force_cache_op,
  ...
  mod_img_msg_buf_divert_t *p_divert)
{
  ...
  out_idx = module_imglib_common_get_buffer(subdev_fd,
    p_divert->identity, NULL, 0xFFFFFFFF);
  if (out_idx < 0) {
  ...
  p_addr = mct_module_get_buffer_ptr((uint32_t)out_idx,
    p_client->parent_mod,
  ...
  /* copy output buffer */
  if (p_addr) {
    ATRACE_CAMSCOPE_BEGIN(CAMSCOPE_WNR_MEMCPY);
    memcpy(p_addr, p_frame->frame[0].plane[0].addr, (size_t)padded_size * 3/2);
    ATRACE_CAMSCOPE_END(CAMSCOPE_WNR_MEMCPY);
    frame_id = p_divert->buf_divert.buffer.sequence;
    p_divert->buf_divert.buffer_access |= CPU_HAS_READ;
  }
  frame_id = p_divert->buf_divert.buffer.sequence;
  IDBG_MED("%s:%d] ", __func__, __LINE__);

  // Note : buffer_access is not handled in wnr module,
  // passing READ_WRITE for now
  status = module_imglib_common_release_buffer(subdev_fd, p_divert->identity,
    (uint32_t)out_idx, frame_id, TRUE,
    p_frame, p_client->ion_fd, p_divert->buf_divert.buffer_access);
  if (IMG_ERROR(status)) {
    IDBG_ERROR("%s:%d] Error getting output buffer %d", __func__, __LINE__,
      out_idx);
  }
  ...
  if (denoise_enable) {
    if( (p_client->stream_info->reprocess_config.pp_feature_config.rotation
      ...
      (p_client->stream_info->reprocess_config.pp_feature_config.rotation
        == ROTATE_90)) {
          info.width =
            (uint32_t)p_client->stream_info->buf_planes.plane_info.mp[0].width;
          info.height =
            (uint32_t)p_client->stream_info->buf_planes.plane_info.mp[0].scanline;
    } else {
      info.width =
        (uint32_t)p_client->stream_info->buf_planes.plane_info.mp[0].stride;
      info.height =
        (uint32_t)p_client->stream_info->buf_planes.plane_info.mp[0].height;
    }
...
void module_wnr_client_divert_exec(void *userdata, void *data)
{
  ...
  if (p_client->stream_off) {
    ...
    pthread_mutex_unlock(&p_client->mutex);
    pthread_mutex_lock(&p_client->stream_off_mutex);
    pthread_cond_signal(&p_client->stream_off_cond);
    pthread_mutex_unlock(&p_client->stream_off_mutex);
    return;
  }

  ATRACE_CAMSCOPE_BEGIN(CAMSCOPE_WNR);
  ...
  ATRACE_CAMSCOPE_END(CAMSCOPE_WNR);
  ...
  if (p_client->debug_info.camera_dump_enabled) {
    ...
    snprintf(filename, sizeof(filename), "%s_%s",
      p_client->debug_info.timestamp, s);
    img_dump_frame(p_client->frame, filename,
      (uint32_t)p_client->frame[0].frame_id, NULL);
    p_client->p_buf_divert_data->buffer_access |= CPU_HAS_READ;
  }

  img_common_handle_output_frame_cache_op(p_client->exec_mode,
    p_client->frame, p_client->ion_fd, p_client->force_cache_op,
    &p_client->p_buf_divert_data->buffer_access);
...
int module_wnr_client_stop(wnr_client_t *p_client)
{
  ...
  pthread_mutex_lock(&p_client->mutex);
  wait = p_client->stream_off && (IMGLIB_STATE_PROCESSING == p_client->state);
  if (wait) {
    pthread_cond_signal(&p_client->cond);
  }
  ...
  if (wait) {
    pthread_mutex_lock(&p_client->stream_off_mutex);
    img_wait_for_completion(&p_client->stream_off_cond,
      &p_client->stream_off_mutex, 100000);
    pthread_mutex_unlock(&p_client->stream_off_mutex);
  }
...
void module_wnr_client_destroy(wnr_client_t *p_client)
{
  ...
  if (IMGLIB_STATE_IDLE == p_client->state) {
    ...
    pthread_mutex_destroy(&p_client->stream_off_mutex);
    pthread_cond_destroy(&p_client->stream_off_cond);
  ...
  uint32_t identity, mct_stream_info_t *stream_info)
{
  ...
  p_client->exec_mode = IMG_EXECUTION_HW;
  p_client->force_cache_op = module_imglib_common_get_prop(
    "persist.vendor.camera.force_cache_ops", "0");
  ...
  p_client->debug_info.camera_dump_enabled = (uint8_t)atoi(value);
#endif
