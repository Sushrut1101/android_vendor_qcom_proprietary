...
static void buffer_exchange_obd_cb(__unused void *pv, struct vpp_buffer *buf)
{
    ...
    {
        if ((!u32ExpectedErrBufCnt) && (pstTestCtx->hqvCtrl.frc.mode == HQV_FRC_MODE_OFF))
...
static uint32_t get_uc_algo_enabled(void * p, uint32_t u32Algo)
{
    t_StVppCb *pstCb;
                           ...
                           uint32_t u32FrameType)
{
    ...
    while((pstTestCtx->u32OutRxCnt < u32FramesOutCnt) ||
          ...
          (pstTestCtx->hqvCtrl.frc.mode == HQV_FRC_MODE_OFF)))
    {
...
TEST_SETUP(BufferExchangeTestInit)
{
    ...
    u32 = ts_mgr_init(&tsmgr, TS_FREQ_30HZ, BUFFER_POOL_SZ);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
...
TEST(BufferExchangeFlush_Testcase)
{
    ...
    if(u32TestMask & FUNCTEST_FLUSH)
    {
        ...
        if(u32 == VPP_OK)
        {
            ...
            pthread_mutex_lock(&pstTestCtx->mutex);
            while((pstTestCtx->u32InHoldCnt != 0) || (pstTestCtx->u32OutHoldCnt != 0))
                pthread_cond_wait(&pstTestCtx->cond, &pstTestCtx->mutex);
            pthread_mutex_unlock(&pstTestCtx->mutex);
        ...
        if(u32 == VPP_OK)
        {
            ...
            for(i = 0; i < 2; i++)
            {
                for(j=0; j < pFuncTestCtx->u32StreamCount; j++)
                {
                    ...
                    param.width = pFuncTestCtx->reconfig_streams[j].reconfig_pool_params.u32Width;
                    param.height = pFuncTestCtx->reconfig_streams[j].reconfig_pool_params.u32Height;
            ...
            u32OutputFlushDoneFlag = 0;
            u32InputFlushDoneFlag = 0;
            param.width = buf_pool_params_temp.u32Width;
            param.height = buf_pool_params_temp.u32Height;
            param.stride = u32VppUtils_CalculateStride(param.width, param.fmt);
            param.scanlines = u32VppUtils_CalculateScanlines(param.height, param.fmt);
            u32 = vpp_reconfigure(p, param, param);
    ...
    if(u32TestMask & FUNCTEST_RECONFIG_ALL)
    {
        ...
        if(u32 == VPP_OK)
        {
            ...
            for(i=0; i < pFuncTestCtx->u32StreamCount; i++)
            {
                ...
                param.width = pFuncTestCtx->reconfig_streams[i].reconfig_pool_params.u32Width;
                param.height = pFuncTestCtx->reconfig_streams[i].reconfig_pool_params.u32Height;
                param.stride = u32VppUtils_CalculateStride(param.width, param.fmt);
                param.scanlines = u32VppUtils_CalculateScanlines(param.height, param.fmt);
                if((param.width > 1920) || (param.height > 1080))
                    continue;
...
TEST(BufferExchangeFrameRateBypass_Testcase)
{
    ...
    if(u32TestMask & FUNCTEST_BYPASS)
    {
        ...
        if(u32 == VPP_OK)
        {
            ...
            tsmgr.freq = TS_FREQ_30HZ;
            tsmgr.u64TsStep = 1000000 / tsmgr.freq;
            LOGD("Changing to 30Hz");
            if(pstTestCtx->u32FramesOut > pstTestCtx->u32OutRxCnt)
                rotate_buffers(p, pstTestCtx->u32FramesOut - pstTestCtx->u32OutRxCnt, ROTATE_OUT_IN,
                               u32MulFactor, u32FrameType);
...
TEST(BufferExchangeExtraStreamBypass_Testcase)
{
    ...
    if(u32TestMask & FUNCTEST_BYPASS)
    {
        ...
        u32 = vpp_set_parameter(p2, VPP_PORT_INPUT, param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        u32 = vpp_set_parameter(p2, VPP_PORT_OUTPUT, param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        u32 = setTestVppHqvCtrl(p3, pstTestCtx, &req);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        u32 = vpp_set_parameter(p3, VPP_PORT_INPUT, param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        u32 = vpp_set_parameter(p3, VPP_PORT_OUTPUT, param);
