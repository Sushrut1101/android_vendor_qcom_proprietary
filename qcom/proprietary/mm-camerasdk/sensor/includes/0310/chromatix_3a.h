...
typedef enum
{
    ASD_DISABLED = 0,    // Feature Disabled
    ASD_FULLY_ENABLED,   // Detection and Compensation
    ASD_DETECTION_ONLY   // Detection without Compensation
} asd_chrx_flag_t;
...
typedef struct
{
    ...
    asd_chrx_flag_t snow_scene_detection_enable;    // Possible values ASD_DISABLED, ASD_FULLY_ENABLED OR ASD_DETECTION_ONLY.
...
typedef struct
{
  ...
  chromatix_manual_white_balance_type initial_gain;
...
typedef struct
{
    ...
    trigger_point_type              exp_index_trigger[ADJ_GAIN_EXP_TRIGGER_NUM];
    int                             cct_trigger[AGW_AWB_MAX_LIGHT][ADJ_GAIN_EXP_TRIGGER_NUM];
    chromatix_awb_gain_adj_type     gain_adj_brightlight_shade[AGW_AWB_MAX_LIGHT];
    chromatix_awb_gain_adj_type     gain_adj_normallight_low[AGW_AWB_MAX_LIGHT];
    chromatix_awb_gain_adj_type     gain_adj_extreme_lowlight[AGW_AWB_MAX_LIGHT];
...
typedef struct{
    ...
    trigger_point_type         outlier_distance_bright_exp_index;
    trigger_point_type         outlier_distance_lowlight_exp_index;
    awb_outlier_distance_type  outlier_distance_bright;
    awb_outlier_distance_type  outlier_distance_normal;
    awb_outlier_distance_type  outlier_distance_lowlight;
} awb_outlier_distance_ext_type;
...
typedef struct
{
    ...
    awb_outlier_distance_ext_type             outlier_distances_ext;
...
typedef struct
{
    ...
    trigger_point_type  dark_threshold_exp_trigger;
...
typedef struct
{
    ...
    chromatix_awb_gain_adj_type               dual_led_gain_adj[2];
} awb_led_tuning_type;
...
typedef struct
{
    ...
    chromatix_awb_gain_adj_type  adjust_ratio;
    trigger_point_type           exp_index_trigger_shade;
    chromatix_awb_gain_adj_type  adjust_ratio_shade;
...
typedef struct
{
    ...
    awb_outdoor_special_type                awb_outdoor_artificial_grass_adjust;
    float                                   resv_dev_float1[RESERVED_DEV_NUM];
    float                                   resv_dev_float2[RESERVED_DEV_NUM];
...
typedef struct{
    ...
    awb_trigger_type_float    awb_misleading_sum_threshold;
    awb_detect_zone_type      awb_misleading_sum_aux_zones[ML_SUM_AUXZONE_NUM];
} awb_misleading_stats_sum_control;
...
typedef struct
{
    ...
    awb_misleading_stats_sum_control  awb_misleading_sum_control_zones[ML_SUM_CTRL_NUM];
    float                             resv_dev_float1[RESERVED_DEV_NUM];
    float                             resv_dev_float2[RESERVED_DEV_NUM];
...
typedef struct
{
    ...
    chromatix_awb_reference_type  h_cluster_cutoff_boundary;
...
typedef struct
{
    ...
    chromatix_awb_reference_type  awb_preflash_init_decision;
...
typedef struct
{
    awb_detect_zone_type          detection_zone[PREFLASH_DETECT_NUM];
    chromatix_CCT_trigger_type    cct_boundary;
    awb_trigger_type_float        k1_boundary;
    chromatix_awb_reference_type  target_ratio;
} awb_preflash_heuristic_type;
...
typedef struct
{
    ...
    trigger_point_type            outdoor_detection_exp_index;
    int                           awb_flash_stat_sat_threshold;
    int                           awb_preflash_outlier_distance;
    chromatix_awb_reference_type  awb_preFlash_min_ref;
    chromatix_awb_reference_type  awb_preFlash_max_ref;
    awb_trigger_type_int          valid_spl_trigger_for_index_selection;
    awb_trigger_type_int          flash_index_trigger;
    chromatix_CCT_trigger_type    flash_adjust_gain_cct_trigger[PREFLASH_CCT_TRIGGER];
    ...
    awb_trigger_type_int          valid_spl_trigger_for_gain_calculation;
    awb_preflash_heuristic_type   preflash_heuristic[PREFLASH_ENHANCE_HEURISTIC_NUM];
    chromatix_CCT_trigger_type    flash_adjust_ccm_cct_trigger[PREFLASH_CCT_TRIGGER];
...
typedef struct{
    ...
    chromatix_awb_reference_type  awb_extr_low_cct_detect_boundary;
    chromatix_awb_gain_adj_type   awb_extr_low_cct_adjust;
    ...
    chromatix_awb_reference_type  awb_extr_lowlight_exit_boundary;
    chromatix_awb_gain_adj_type   awb_extr_lowlight_adjust;
    int                           awb_extr_lowlight_target_decision;
    int                           awb_extr_high_cct_enable;
    int                           awb_extr_high_cct_wb_zone[EXTR_CCT_EXTRA_ZONE];
    awb_detect_zone_type          awb_extr_high_cct_aux_zone_detect[EXTR_CCT_EXTRA_ZONE];
    float                         awb_extr_cct_hightcct_conditions[EXTR_CCT_HIGH_CCT_COND][EXTR_CCT_HIGH_CCT_CAT];
    chromatix_awb_gain_adj_type   awb_extr_cct_hightcct_adjust[EXTR_CCT_HIGH_CCT_COND];
    float                         resv_dev_float[RESERVED_DEV_NUM];
    int                           resv_dev_int[RESERVED_DEV_NUM];
} awb_extreme_cct_type;
...
typedef struct
{
    ...
    awb_special_weights_type            awb_special_weights_for_lab;                //Special weights for lab detection
    awb_extreme_cct_type                awb_extreme_cct_control;                    //Extreme high/low cct condition
    awb_preflash_enhance_type           awb_preflash_enhance_control;               //Enhanced preflash
    awb_dynamic_frame_skip_type         awb_dynamic_frame_skip;                     // Dynamic frame skip control
    WBReferencePointCaliData_type       wb_refpnt_cali_data;                        // WBReferencePointCaliData
...
} LED_mix_calibration_entry_type;
...
typedef struct
{
    ...
    LED_mix_calibration_entry_type   calib_entries[MAX_CALIB_NUM];                       // Specific entries to calibrate
...
typedef struct
{
    ...
    LED_mix_type                cct_table[MAX_LED_POWER_LEVEL][MAX_LED_MIX_LEVEL];
    LED_mix_calibration_type    cct_table_calibration;      // Dual LED calibration feature
} aec_led_match_table_type;
...
typedef struct
{
    ...
    aec_led_match_table_type led_table_tuning;              // For both dual and single LED tuning, with de-rating support
...
typedef struct
{
    ...
    int             static_low_light_idx_thr;  //define low light threshold to make threshold less in low light default: 350
    float           static_low_light_comp;     // static threshold for low light default: 0.98
    float           band_dist_comp;            // how to define peak_dist value with calculated band_dist default: 0.90
    float           band_range_min;            // to decide flicker or not, the peak width range min default: 0.88
...
typedef struct _af_tuning_single_t
{
    ...
    af_single_frame_delay_t  frame_delay;
} af_tuning_single_t;
...
typedef struct _af_tuning_pd_pdlib_t
{
    int                               enable;
    int                               max_shift;
    int                               resize_ratio;
    ...
    float                             reserved[20];
} af_tuning_pd_pdlib_t;

typedef struct _af_tuning_pd_preproc_t
{
    int                                   enable;
    ...
    float                                 reserve[20];
} af_tuning_pd_preproc_t;

typedef struct _af_tuning_pdaf_internal_t
{
    unsigned int                          type;
...
typedef struct _af_tuning_pdaf_t
{
    ...
    af_tuning_pd_pdlib_t                  pd_lib;
    af_tuning_pd_preproc_t                pd_preproc;
    af_tuning_pd_internal_t               pd_int;
    float                                 reserve[200];
} af_tuning_pdaf_t;
...
typedef enum
{
    ...
    AF_DCIAF_FAR_RANGE,
    AF_DCIAF_MAX_RANGE
} af_dciaf_distance_range;


typedef struct _af_dciaf_fine_search_step_info
{
    int                                   stepSize;
    int                                   far_steps;
    int                                   near_steps;
} af_dciaf_fine_search_step_info;
...
typedef struct _af_tuning_dciaf_t
{
    ...
    af_dciaf_fine_search_step_info        normalLightFineSearchInfo[AF_DCIAF_MAX_RANGE];
    af_dciaf_fine_search_step_info        lowLightFineSearchInfo[AF_DCIAF_MAX_RANGE];
    float                                 reserve[100];
} af_tuning_dciaf_t;
...
typedef struct _af_tof_fine_scan_t
{
...
typedef struct _af_tof_frame_delay_t
{
...
} af_tof_frame_delay_t;

typedef struct _af_tof_scene_change_t
{
...
} af_tof_scene_change_t;

typedef struct _af_tof_flag_t
{
...
typedef struct _af_tof_filter_t
{
...
typedef struct _af_tuning_tof_t
{
    ...
    int                                   num_monitor_samples;
    int                                   tof_step_size;
    af_tof_filter_t                       tof_filter;
    af_tof_flag_t                         tof_flag;
    af_tof_frame_delay_t                  frame_delay;
    af_tof_scene_change_t                 scene_change;
    af_tof_focus_converge_t               focus_converge;
    af_tof_fine_scan_t                    fine_scan;
    float                                 reserve[100];
} af_tuning_tof_t;
...
typedef struct _af_tuning_haf_t
{
    ...
    af_fine_srch_extension_t  fine_srch_extension;
    af_tuning_sad_t           af_sad;
...
typedef struct _baf_tuning_scaler_coefficient_t
{
    int                                   a;
    int                                   b;
} baf_tuning_scaler_coefficient_t;
...
typedef struct _baf_tuning_filter_t
{
    ...
    baf_tuning_scaler_coefficient_t       coefficient;
} baf_tuning_filter_t;
