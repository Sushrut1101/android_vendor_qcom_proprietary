...
#define QSEECOM_RPMB_KEY_NOT_PROVISIONED		0xFFFFFF07
#define QSEECOM_RPMB_KEY_PROVISIONED_BUT_MAC_MISMATCH	0xFFFFFF0C
...
enum QSEECom_command_id	{
  ...
  QSEECOM_RPMB_CHECK_PROV_STATUS_COMMAND = 0x1B,/* mapping exists in qseecomi.h, do not change*/
 ...
 * mentioned offset.
 * This API is used to populate buffer with SG list's 32bit phy addr and sent to tzapp.
 * The HLOS client writes to the send_buf, where QSAPP writes to the rcv_buf.
 * This is a blocking call.
 *
 * @param[in] handle    The device handle
 ...
 *
 * This API is used to populate buffer with SG list's 32bit phy addr and sent to listener.
 * @param[in] handle,   The device handle
 * @param[in] send_buf, The buffer to be sent.
 *                      If using ion_sbuffer, ensure this
 *                      QSEECOM_BUFFER_ALIGN'ed.
...
/**
 * @brief get app info, including secure app arch type, sg_buffer_size, etc
 *
 * @param[in] handle    The device handle
 * @param[in/out] info   The pointer to qseecom_system_info
 *
 * @return Zero on success, negative on failure. errno will be set on
 *  error.
 */
int QSEECom_get_app_info(struct QSEECom_handle *handle,
				struct qseecom_app_info* info);

/**
 * @brief Send QSAPP a "user" defined buffer (may contain some message/
 * command request) and receives a response from QSAPP in receive buffer.
 * This API is used to populate buffer with SG list's 64bit phy addr and sent to tzapp;
 * while QSEECom_send_modified_cmd() populates buffer with SG list's 32bit phy addr.
 *
 * The HLOS client writes to the send_buf, where QSAPP writes to the rcv_buf.
 * This is a blocking call.
 *
...
/**
 * @brief qseecom_send_modified_resp_64
 *
 * A "user" defined API that contains the Ion fd allocated by the
 * listener service, to be communicated with TZ. This API is same as
 * "QSEECom_send_modified_cmd" but servicing the listener instead of app
 ...
 * address and modify the message in send_buf at the mentioned offset.
 *
 * This API is used to populate buffer with SG list's 64bit phy addr and sent to listener;
 * while QSEECom_send_modified_resp() populates buffer with SG list's 32bit phy addr.
 *
 * @param[in] handle,   The device handle
 * @param[in] send_buf, The buffer to be sent.
 *                      If using ion_sbuffer, ensure this
...
/**
 * @brief QSEECom_start_app_V2
 *
 * - Load a secure application from a memory buffer containing the Trusted App's
 *   mbn file, instead of load split imges from file system.image.
 *
 * @param[in/out] handle The device handle
 * @param[in] fname: TZ app name
 * @param [in] trustlet : memory buffer containing the Trusted Application image
 * @param [in] tlen : length of the memory buffer containing the Trusted Application image
 * @param[in] sb_size Size of the shared buffer memory  for sending requests.
 * @return Zero on success, negative on failure. errno will be set on error.
 */

int QSEECom_start_app_V2(struct QSEECom_handle **clnt_handle,
                         const char *fname,
                         unsigned char *trustlet,
                         uint32_t tlen,
                         uint32_t sb_length);
