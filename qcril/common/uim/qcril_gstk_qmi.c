...
--------   ---     ----------------------------------------------------------
06/02/15   hh      Support for retrieve cached proactive commands
05/14/15   yt      USE IPC routing for MSM platform
05/14/15   vdc     Use IPC Routing for registering with QMI UIM service
01/30/15   hh      Support for Class R related commands
...
#define QCRIL_GSTK_QMI_CMD_STK_RUN_AT_CMD             0x34
...
#define QCRIL_GSTK_PROP_BASEBAND_VALUE_SDM         "sdm"
#define QCRIL_GSTK_PROP_BASEBAND_VALUE_SDX         "sdx"
#define QCRIL_GSTK_PROP_BASEBAND_VALUE_SDA         "sda"
...
/* MAX number of envelope commands */
#define QCRIL_GSTK_MAX_ENVELOPE_COMMAND             9
...
#define QCRIL_GSTK_ADD_ENTRY_TO_REQUEST_LIST(params_ptr)                       \
          {                                                                    \
...
#define QCRIL_GSTK_CMD_TYPE_SETUP_MENU            0x25
#define QCRIL_GSTK_CMD_TYPE_SETUP_EVENT_LIST      0x05
#define QCRIL_GSTK_CMD_TYPE_SETUP_IDLE_MODE_TEXT  0x28
...
typedef struct
{
  ...
  boolean                                   scws_err_sent;
                                                          /*!< SCWS error sent flag */
  uint32                                    timer_id;
                                                          /*!< Timer ID for proactive commands */
  qmi_client_os_params                      qmi_cat_svc_os_params_primary;
                                                          /*!< QMI params for primary modem */
  qmi_client_os_params                      qmi_cat_svc_os_params_secondary;
                                                          /*!< QMI params for secondary modem */
...
/* -----------------------------------------------------------------------------
   STRUCT:      QCRIL_GSTK_SCWS_OPEN_CH_TIMER_TYPE

   DESCRIPTION:
     Structure used to cache the SCWS open channel retry
-------------------------------------------------------------------------------*/
static struct
{
  uint32                              timer_id;
  ...
  cat_scws_open_channel_ind_msg_v02  *scws_open_ch_msg_ptr;
} qcril_gstk_scws_open_ch_timer[QMI_UIM_MAX_CARD_COUNT];

static qcril_gstk_qmi_info_type qcril_gstk_qmi_info;

/* -----------------------------------------------------------------------------
   STRUCT:      QCRIL_GSTK_RESPONSE_IND_INFO_TYPE

   DESCRIPTION:
     Structure that holds the info that is required to handle the envelope or TR
     response indication.
-------------------------------------------------------------------------------*/
typedef struct
{
  boolean                             in_use;
  uint32                              token;
  qcril_gstk_original_request_type    orig_req;
} qcril_gstk_response_ind_info_type;

/* GSTK in Modem can process max QCRIL_GSTK_MAX_ENVELOPE_COMMAND at a time */
static qcril_gstk_response_ind_info_type  qcril_gstk_env_rsp_ind[QCRIL_GSTK_MAX_ENVELOPE_COMMAND];

/* Only one proactive command can be in process at a time */
static qcril_gstk_response_ind_info_type  qcril_gstk_tr_rsp_ind;
...
{
  qmi_client_qmux_instance_type qmi_modem_port = QMI_CLIENT_INSTANCE_ANY;
      ...
      (strcmp(prop_value_ptr, QCRIL_GSTK_PROP_BASEBAND_VALUE_CSFB)     == 0))
  {
    qmi_modem_port = QMI_CLIENT_QMUX_RMNET_SDIO_INSTANCE_0;
           ...
           (strcmp(prop_value_ptr, QCRIL_GSTK_PROP_BASEBAND_VALUE_SDX)    == 0) ||
  ...
  else if ((strcmp(prop_value_ptr, QCRIL_GSTK_PROP_BASEBAND_VALUE_APQ)   == 0) ||
           (strcmp(prop_value_ptr, QCRIL_GSTK_PROP_BASEBAND_VALUE_SDA)   == 0) ||
...
)
{
  ...
  {
    QCRIL_GSTK_QMI_RETURN_IF_OUT_OF_RANGE(ind_msg_ptr->contactless_state_changed.pc_contactless_state_changed_len,
                                          QCRIL_GSTK_QMI_COMMAND_MIN_SIZE,
                                          QMI_CAT_RAW_PROACTIVE_CMD_MAX_LENGTH_V02);

    QCRIL_LOG_INFO("%s", "Command will be handled by Android: Contactless State Changed TLV");

    user_ref                      = ind_msg_ptr->contactless_state_changed.uim_ref_id;
    ril_unsol_resp.ril_unsol_type = RIL_UNSOL_STK_PROACTIVE_COMMAND;
    qcril_gstk_qmi_prepare_unsol_response(&ril_unsol_resp.ril_unsol_resp,
                                          ind_msg_ptr->contactless_state_changed.pc_contactless_state_changed,
                                          ind_msg_ptr->contactless_state_changed.pc_contactless_state_changed_len,
                                          &command_type);
  }
#endif /* FEATURE_QCRIL_GSTK_CONTACTLESS */
  else if (ind_msg_ptr->run_at_command_valid)
  {
    QCRIL_GSTK_QMI_RETURN_IF_OUT_OF_RANGE(ind_msg_ptr->run_at_command.pc_run_at_command_len,
                                          QCRIL_GSTK_QMI_COMMAND_MIN_SIZE,
                                          QMI_CAT_RAW_PROACTIVE_CMD_MAX_LENGTH_V02);

    QCRIL_LOG_INFO("%s", "Command will be handled by Android: RUN AT CMD TLV");
    send_evt_conf                 = TRUE;
    user_ref                      = ind_msg_ptr->run_at_command.uim_ref_id;
    ril_unsol_resp.ril_unsol_type = RIL_UNSOL_STK_EVENT_NOTIFY;
    qcril_gstk_qmi_prepare_unsol_response(&ril_unsol_resp.ril_unsol_resp,
                                          ind_msg_ptr->run_at_command.pc_run_at_command,
                                          ind_msg_ptr->run_at_command.pc_run_at_command_len,
                                          &command_type);
  }
  else
  {
     ...
     !(qcril_gstk_qmi_info.gstk_command_info[slot_index].flags & QCRIL_GSTK_QMI_RIL_IS_READY_BMSK))
  {
    ...
    if (qcril_gstk_qmi_info.gstk_command_info[slot_index].cmd_ptr != NULL)
    {
      qcril_setup_timed_callback( instance_id, ind_data_ptr->modem_id,
                                  qcril_gstk_qmi_resend_proactive_cmd,
                                  &QCRIL_GSTK_QMI_TIMER_RESEND,
                                  &timer_id );
      qcril_gstk_qmi_info.timer_id = timer_id;
...
)
{
  ...
  if (!qcril_gstk_qmi_convert_cat_slot_id(slot_type, &slot_id) ||
      (qmi_ril_get_sim_slot() != slot_id))
  {
    QCRIL_LOG_ERROR("Invalid input slot_type: %d, slot_id: %d", slot_type, slot_id);
    return;
  }
  ...
  open_ch_err = qcril_scws_open_channel((uint16)open_channel_ind_ptr->open_channel_info.port,
                                        (uint32)open_channel_ind_ptr->open_channel_info.ch_id,
                                        slot_id);
  if (open_ch_err == EADDRINUSE)
  {
    if (qcril_gstk_scws_open_ch_timer[slot_id].timer_id != 0)
    {
    ...
    if (qcril_gstk_scws_open_ch_timer[slot_id].retry_count < QCRIL_GSTK_SCWS_OPEN_CH_MAX_RETRIES)
    {
      if (qcril_gstk_scws_open_ch_timer[slot_id].scws_open_ch_msg_ptr != NULL)
      {
        qcril_free(qcril_gstk_scws_open_ch_timer[slot_id].scws_open_ch_msg_ptr);
      }

      qcril_gstk_scws_open_ch_timer[slot_id].scws_open_ch_msg_ptr =
              (cat_scws_open_channel_ind_msg_v02 *)qcril_malloc(sizeof(cat_scws_open_channel_ind_msg_v02));

      if (qcril_gstk_scws_open_ch_timer[slot_id].scws_open_ch_msg_ptr != NULL)
      {
        memcpy(qcril_gstk_scws_open_ch_timer[slot_id].scws_open_ch_msg_ptr,
               open_channel_ind_ptr,
               sizeof(cat_scws_open_channel_ind_msg_v02));

        qcril_gstk_scws_open_ch_timer[slot_id].retry_count++;
        qcril_gstk_scws_open_ch_timer[slot_id].scws_open_ch_timer.tv_sec  = QCRIL_GSTK_SCWS_OPEN_CH_RETRY_TIME;

        if(qcril_setup_timed_callback( qmi_ril_get_process_instance_id(),
                                       QCRIL_DEFAULT_MODEM_ID,
                                       qcril_gstk_scws_open_ch_timer_cb,
                                       &qcril_gstk_scws_open_ch_timer[slot_id].scws_open_ch_timer,
                                       &qcril_gstk_scws_open_ch_timer[slot_id].timer_id) == 0)
        {
          return;
        }
      }
    ...
    memset(&qcril_gstk_scws_open_ch_timer[slot_id], 0, sizeof(qcril_gstk_scws_open_ch_timer[0]));
  }
...
)
{
  ...
  qcril_instance_id_e_type  instance_id =
                   QCRIL_EXTRACT_INSTANCE_ID_FROM_USER_DATA( timer_id );
  uint8                     slot_id     =
             qcril_gstk_qmi_convert_instance_to_slot_index(instance_id);

  if((qmi_ril_get_sim_slot() == slot_id) &&
     (slot_id < QMI_UIM_MAX_CARD_COUNT) &&
     (qcril_gstk_scws_open_ch_timer[slot_id].timer_id != 0 )&&
     (qcril_gstk_scws_open_ch_timer[slot_id].scws_open_ch_msg_ptr != NULL))
  {
    QCRIL_LOG_INFO("SCWS OPEN Channel timer callback");
    qcril_gstk_scws_open_ch_timer[slot_id].timer_id = 0;
    qcril_gstk_qmi_scws_open_channel(qcril_gstk_scws_open_ch_timer[slot_id].scws_open_ch_msg_ptr);
  }
...
(
  const qcril_gstk_qmi_rsp_params_type    * rsp_params_ptr
)
{
  ...
  cat_send_envelope_cmd_resp_msg_v02 * env_cmd_resp_ptr = NULL;
  RIL_Errno                            ril_err          = QCRIL_UIM_RIL_E_INTERNAL_ERR;
  uint8                                index            = 0;

  QCRIL_LOG_INFO("qcril_gstk_qmi_process_qmi_envelope_response\n");
  ...
  env_cmd_resp_ptr = (cat_send_envelope_cmd_resp_msg_v02 *)rsp_params_ptr->msg_ptr;

  if (env_cmd_resp_ptr == NULL ||
      rsp_params_ptr->orig_req_data == NULL ||
      env_cmd_resp_ptr->indication_token_valid)
  {
    return;
  }
  ...
  qcril_default_request_resp_params(rsp_params_ptr->orig_req_data->instance_id,
                                    rsp_params_ptr->orig_req_data->token_id,
                                    rsp_params_ptr->orig_req_data->request_id,
                                    ril_err,
                                    &resp );

  if (env_cmd_resp_ptr->resp.result == QMI_RESULT_SUCCESS_V01 &&
      rsp_params_ptr->transp_err == QMI_NO_ERR)
  {
    if (env_cmd_resp_ptr->env_resp_data_valid &&
        (env_cmd_resp_ptr->env_resp_data.env_resp_data_len > 0) &&
        (env_cmd_resp_ptr->env_resp_data.env_resp_data_len <= QMI_CAT_RAW_ENV_RSP_DATA_MAX_LENGTH_V02))
    {
      env_response_len = QCRIL_GSTK_QMI_ENVRSP_DATA_SIZE(
                           env_cmd_resp_ptr->env_resp_data.env_resp_data_len) + 1;
      ...
      if (env_response_ptr)
      {
        ...
        QCRIL_GSTK_QMI_ENVRSP_COPY((uint8 *)env_response_ptr,
                                   env_cmd_resp_ptr->env_resp_data.env_resp_data,
                                   env_cmd_resp_ptr->env_resp_data.env_resp_data_len);
      }
      else
      {
        env_response_len = 0;
    ...
    if (rsp_params_ptr->orig_req_data->request_id == RIL_REQUEST_STK_SEND_ENVELOPE_WITH_STATUS)
    {
      memset(&ril_response, 0, sizeof(RIL_SIM_IO_Response));
      if (env_cmd_resp_ptr->env_resp_data_valid)
      {
        /* Update the response upon success */
        ril_response.sw1         = env_cmd_resp_ptr->env_resp_data.sw1;
        ril_response.sw2         = env_cmd_resp_ptr->env_resp_data.sw2;
        ril_response.simResponse = env_response_ptr;
      }
  ...
  resp.ril_err_no = ril_err;

  qcril_send_request_response(&resp);

  /* Response sent to client, search and reset envelope request cache for this token */
  for (index = 0; index < QCRIL_GSTK_MAX_ENVELOPE_COMMAND; index++ )
  {
    if (qcril_gstk_env_rsp_ind[index].in_use == FALSE)
    {
      continue;
    }
    if (qcril_gstk_env_rsp_ind[index].token == (uintptr_t)rsp_params_ptr->orig_req_data->token_id)
    {
      memset(&qcril_gstk_env_rsp_ind[index], 0x00, sizeof(qcril_gstk_response_ind_info_type));
      break;
    }
  }
...
/*=========================================================================
  ...
  FUNCTION:  qcril_gstk_qmi_process_qmi_tr_response
...
)
{
  ...
  QCRIL_LOG_INFO("qcril_gstk_qmi_process_qmi_tr_response\n");
  ...
  send_tr_resp_ptr = (cat_send_tr_resp_msg_v02 *)rsp_params_ptr->msg_ptr;

  if (send_tr_resp_ptr == NULL ||
      rsp_params_ptr->orig_req_data == NULL ||
      send_tr_resp_ptr->indication_token_valid)
  {
    return;
  }
  ...
  if (send_tr_resp_ptr->resp.result == QMI_RESULT_SUCCESS_V01 &&
      rsp_params_ptr->transp_err == QMI_NO_ERR)
  {
    ril_err = RIL_E_SUCCESS;
  }
  ...
  /* Reset cache */
  memset(&qcril_gstk_tr_rsp_ind, 0x00, sizeof(qcril_gstk_response_ind_info_type));
...
)
{
  ...
  do
  {
    ...
    qmi_err_code = qmi_client_init_instance(client_service,
                                            ...
                                            NULL,
                                            &qcril_gstk_qmi_info.qmi_cat_svc_os_params_primary,
                                            time_out,
  ...
  if (qmi_sec_modem_port != QMI_CLIENT_QMUX_MAX_INSTANCE_IDS)
  {
    ...
    do
    {
      ...
      qmi_err_code = qmi_client_init_instance(client_service,
                                              ...
                                              qcril_gstk_qmi_indication_cb,
                                              NULL,
                                              &qcril_gstk_qmi_info.qmi_cat_svc_os_params_secondary,
                                              time_out,
...
)
{
  ...
  qcril_gstk_qmi_rsp_params_type    rsp_params;
  uint8                             index = 0;

  QCRIL_LOG_DEBUG( "%s", "qcril_gstk_qmi_srvc_release_client");

  memset(&rsp_params, 0x00, sizeof(qcril_gstk_qmi_rsp_params_type));
  ...
  /* TR was sent from ATEL, but QCRIL is waiting for the response in an indication
     from the modem, clean-up the pending request and send immediately a fake
     response to ATEL to indicate the error */
  if (qcril_gstk_tr_rsp_ind.in_use)
  {
    cat_send_tr_resp_msg_v02          tr_resp;

    memset(&tr_resp, 0x00, sizeof(cat_send_tr_resp_msg_v02));

    /* Update response parameters */
    rsp_params.msg_id        = QMI_CAT_SEND_TR_RESP_V02;
    rsp_params.msg_ptr       = &tr_resp;
    rsp_params.transp_err    = QMI_SERVICE_ERR;
    rsp_params.orig_req_data = &qcril_gstk_tr_rsp_ind.orig_req;

    /* send response to the ATEL */
    qcril_gstk_qmi_process_qmi_tr_response(&rsp_params);

    /* Reset cache */
    memset(&qcril_gstk_tr_rsp_ind, 0x00, sizeof(qcril_gstk_response_ind_info_type));
  }

  /* Envelope was sent from ATEL, but QCRIL is waiting for the response in an
     indication from the modem, clean-up the pending request and send immediately
     a fake response to ATEL to indicate the error */
  for (index = 0; index < QCRIL_GSTK_MAX_ENVELOPE_COMMAND; index++)
  {
    cat_send_envelope_cmd_resp_msg_v02  env_resp;
    ...
    memset(&env_resp, 0x00, sizeof(cat_send_envelope_cmd_resp_msg_v02));

    /* Update response parameters */
    rsp_params.msg_id        = QMI_CAT_SEND_EVENLOPE_CMD_RESP_V02;
    rsp_params.msg_ptr       = &env_resp;
    rsp_params.transp_err    = QMI_SERVICE_ERR;
    rsp_params.orig_req_data = &qcril_gstk_env_rsp_ind[index].orig_req;

    /* send response to the ATEL */
    qcril_gstk_qmi_process_qmi_envelope_response(&rsp_params);
  }
...
/*===========================================================================
   ...
   FUNCTION:  qcril_gstk_qmi_process_tr_rsp_ind

 ===========================================================================*/
static void qcril_gstk_qmi_process_tr_rsp_ind
(
  cat_send_tr_ind_msg_v02    * tr_rsp_ind_ptr
)
{
  qcril_gstk_qmi_rsp_params_type    rsp_params;
  cat_send_tr_resp_msg_v02          tr_resp;

  QCRIL_LOG_INFO("qcril_gstk_qmi_process_tr_rsp_ind\n");

  if (tr_rsp_ind_ptr == NULL ||
      qcril_gstk_tr_rsp_ind.in_use == FALSE ||
      qcril_gstk_tr_rsp_ind.token != tr_rsp_ind_ptr->indication_token)
  {
    return;
  }
  ...
  /* Copy TR response data */
  tr_resp.resp                   = tr_rsp_ind_ptr->resp;
  tr_resp.tr_response_data_valid = tr_rsp_ind_ptr->tr_response_data_valid;
  memcpy(&tr_resp.tr_response_data,
         &tr_rsp_ind_ptr->tr_response_data,
         sizeof(cat_terminal_resp_type_v02));
  tr_resp.indication_token_valid = FALSE;

  /* Update response parameters */
  rsp_params.msg_id        = QMI_CAT_SEND_TR_RESP_V02;
  rsp_params.msg_ptr       = &tr_resp;
  rsp_params.transp_err    = QMI_NO_ERR;
  rsp_params.orig_req_data = &qcril_gstk_tr_rsp_ind.orig_req;

  /* send response to the ATEL */
  qcril_gstk_qmi_process_qmi_tr_response(&rsp_params);
} /* qcril_gstk_qmi_process_tr_rsp_ind */


/*===========================================================================

   FUNCTION:  qcril_gstk_qmi_process_env_rsp_ind

 ===========================================================================*/
static void qcril_gstk_qmi_process_env_rsp_ind
(
  cat_send_envelope_cmd_ind_msg_v02    * env_rsp_ind_ptr
)
{
  qcril_gstk_qmi_rsp_params_type      rsp_params;
  cat_send_envelope_cmd_resp_msg_v02  env_resp;
  ...
  QCRIL_LOG_INFO("qcril_gstk_qmi_process_env_rsp_ind\n");

  if (env_rsp_ind_ptr == NULL)
  {
    return;
  }
  ...
  for (index = 0; index < QCRIL_GSTK_MAX_ENVELOPE_COMMAND; index++ )
  {
    ...
    if (qcril_gstk_env_rsp_ind[index].token == env_rsp_ind_ptr->indication_token)
    {
      break;
    }
  }

  if (index >= QCRIL_GSTK_MAX_ENVELOPE_COMMAND)
  {
    return;
  }
  ...
  /* Copy TR response data */
  env_resp.resp                = env_rsp_ind_ptr->resp;
  env_resp.env_resp_data_valid = env_rsp_ind_ptr->env_resp_data_valid;
  memcpy(&env_resp.env_resp_data,
         &env_rsp_ind_ptr->env_resp_data,
         sizeof(cat_envelope_resp_type_v02));
  env_resp.indication_token_valid = FALSE;
  ...
  /* Reset cache */
  memset(&qcril_gstk_env_rsp_ind[index], 0x00, sizeof(qcril_gstk_response_ind_info_type));
} /* qcril_gstk_qmi_process_env_rsp_ind */
...
)
{
  ...
  QCRIL_LOG_INFO("Pending TR resp: 0x%x, Msg_id: 0x%x",
                                   qcril_gstk_tr_rsp_ind.in_use,
                                   ind_data_ptr->msg_id);

  /* TR was sent from ATEL, but QCRIL is waiting for the response from the
     modem, but received proactive command so clean-up the pending request
     and send immediately a fake TR response to ATEL to indicate the error */
  if (qcril_gstk_tr_rsp_ind.in_use &&
      (ind_data_ptr->msg_id == QMI_CAT_EVENT_REPORT_IND_V02 ||
       ind_data_ptr->msg_id == QMI_CAT_SCWS_OPEN_CHANNEL_IND_V02 ||
       ind_data_ptr->msg_id == QMI_CAT_SCWS_CLOSE_CHANNEL_IND_V02) )
  {
    cat_send_tr_resp_msg_v02          tr_resp;
    qcril_gstk_qmi_rsp_params_type    rsp_params;
  ...
  switch(ind_data_ptr->msg_id)
  {
     ...
     case QMI_CAT_SEND_TR_IND_V02:
       qcril_gstk_qmi_process_tr_rsp_ind(
          (cat_send_tr_ind_msg_v02*)ind_data_ptr->msg_ptr);
       break;
     case QMI_CAT_SEND_EVENLOPE_CMD_IND_V02:
       qcril_gstk_qmi_process_env_rsp_ind(
          (cat_send_envelope_cmd_ind_msg_v02*)ind_data_ptr->msg_ptr);
       break;
     default:
...
)
{
      ...
      (data_available_resp_ptr->resp.error  != QMI_ERR_NONE_V01))
  {
    ...
    qcril_scws_data_available_error(rsp_params_ptr->orig_req_data->data.bip_id,
...
)
{
  ...
  switch (rsp_params_ptr->msg_id)
  {
    case QMI_CAT_SCWS_DATA_AVAILABLEA_RESP_V02:
      qcril_gstk_qmi_process_qmi_data_available_response(rsp_params_ptr);
      break;
    case QMI_CAT_SEND_EVENLOPE_CMD_RESP_V02:
      qcril_gstk_qmi_process_qmi_envelope_response(rsp_params_ptr);
      break;
    case QMI_CAT_SEND_TR_RESP_V02:
      qcril_gstk_qmi_process_qmi_tr_response(rsp_params_ptr);
      break;
    default:
      QCRIL_LOG_ERROR("%Invalid Msg ID: 0x%x", rsp_params_ptr->msg_id);
      break;
  }
...
)
{
  ...
  QCRIL_GSTK_ADD_ENTRY_TO_REQUEST_LIST(params_ptr);

  QCRIL_LOG_INFO("%s", "qcril_gstk_qmi_request_stk_send_envelope_command\n");

  memset(&resp, 0x00, sizeof(qcril_request_resp_params_type));

  /* Find free envelope response indication cache */
  for (index = 0; index < QCRIL_GSTK_MAX_ENVELOPE_COMMAND; index++)
  {
    if (qcril_gstk_env_rsp_ind[index].in_use)
    {
      continue;
    }
    break;
  ...
  env_cmd_resp_ptr = (cat_send_envelope_cmd_resp_msg_v02*)
                        qcril_malloc(sizeof(cat_send_envelope_cmd_resp_msg_v02));
  if (env_cmd_resp_ptr == NULL)
  {
  ...
  memset(env_cmd_resp_ptr, 0, sizeof(cat_send_envelope_cmd_resp_msg_v02));

  /* Update request parameters */
  memset(orig_ptr, 0, sizeof(qcril_gstk_original_request_type));

  orig_ptr->instance_id  = instance_id;
  orig_ptr->modem_id     = modem_id;
  orig_ptr->token_id     = params_ptr->t;
  orig_ptr->request_id   = params_ptr->event_id;
  ...
  if (index < QCRIL_GSTK_MAX_ENVELOPE_COMMAND)
  {
    ...
    env_cmd_req_ptr->indication_token       = (uintptr_t)params_ptr->t;

    /* Cache the original request */
    qcril_gstk_env_rsp_ind[index].in_use = TRUE;
    qcril_gstk_env_rsp_ind[index].token = env_cmd_req_ptr->indication_token;
    memcpy(&qcril_gstk_env_rsp_ind[index].orig_req,
           orig_ptr,
           sizeof(qcril_gstk_env_rsp_ind[index].orig_req));
  }

  /* Primary will always send envelope commands to card */
  QCRIL_LOG_QMI( modem_id, "qmi_gstk_service", "send_envelope" );
  qmi_err_code = qmi_client_send_msg_async(qcril_gstk_qmi_info.qmi_cat_svc_client_primary,
  ...
  QCRIL_LOG_ERROR("Error for SEND_ENVELOPE_CMD_REQ, client_err: 0x%x, error_code: 0x%x\n",
                  qmi_err_code, env_cmd_resp_ptr->resp.error);
...
send_envelope_response:
  ...
  /* Clean up original request if allocated */
...
)
{
  ...
  if (qcril_gstk_tr_rsp_ind.in_use)
  {
    QCRIL_LOG_ERROR("TR already sent");
    goto send_tr_response;
  }
  ...
  /* Allocate request pointer on the heap */
  send_tr_resp_ptr = (cat_send_tr_resp_msg_v02 *)
                      qcril_malloc(sizeof(cat_send_tr_resp_msg_v02));
  if (send_tr_resp_ptr == NULL)
  {
    QCRIL_LOG_ERROR("%s", "error allocating memory for send_tr_resp_ptr!");
    ril_err = QCRIL_UIM_RIL_E_NO_MEMORY;
    goto send_tr_response;
  }

  memset(send_tr_resp_ptr, 0x00, sizeof(cat_send_tr_resp_msg_v02));
  ...
  if ((send_tr_req_ptr->terminal_response.terminal_response_len > QCRIL_GSTK_QMI_COMMAND_DETAILS_TLV_LENGTH) &&
      ((send_tr_req_ptr->terminal_response.terminal_response[0] & 0x7F) == QCRIL_GSTK_QMI_COMMAND_DETAILS_TAG) &&
      (send_tr_req_ptr->terminal_response.terminal_response[1] == QCRIL_GSTK_QMI_COMMAND_DETAILS_TLV_LENGTH) &&
      (send_tr_req_ptr->terminal_response.terminal_response[2] == QCRIL_GSTK_CACHED_PCMD_CMD_NUM) &&
      ((send_tr_req_ptr->terminal_response.terminal_response[3] == QCRIL_GSTK_CMD_TYPE_SETUP_IDLE_MODE_TEXT) ||
       (send_tr_req_ptr->terminal_response.terminal_response[3] == QCRIL_GSTK_CMD_TYPE_SETUP_EVENT_LIST) ||
       (send_tr_req_ptr->terminal_response.terminal_response[3] == QCRIL_GSTK_CMD_TYPE_SETUP_MENU)))
  {
  ...
  if(qcril_gstk_qmi_info.gstk_command_info[slot_index].cmd_ptr)
  {
    ...
    send_tr_req_ptr->indication_token       = (uintptr_t)params_ptr->t;

    /* Cache the original request and return */
    qcril_gstk_tr_rsp_ind.in_use       = TRUE;
    qcril_gstk_tr_rsp_ind.token        = send_tr_req_ptr->indication_token;
    memcpy(&qcril_gstk_tr_rsp_ind.orig_req,
           orig_ptr,
           sizeof(qcril_gstk_tr_rsp_ind.orig_req));
    ...
    /* TR sent to the modem clear the global proactive command cache */
    QCRIL_LOG_INFO("%s", "Dismiss pending proactive cmd!\n");
    QCRIL_GSTK_QMI_FREE_PTR(qcril_gstk_qmi_info.gstk_command_info[slot_index].cmd_ptr);
    ...
    QCRIL_LOG_ERROR("Error for SEND_TR_REQ, client_err: 0x%x, error_code: 0x%x\n",
                    qmi_err_code, send_tr_resp_ptr->resp.error);
    ril_err = QCRIL_UIM_RIL_E_MODEM_ERR;
  }
  else
  {
...
)
{
         ...
         QCRIL_GSTK_QMI_CMD_STK_OPEN_CHANNEL)))
  {
    ...
    if (qmi_err_code != 0)
    {
      QCRIL_LOG_ERROR("Error for EVENT_CONFIRMATION_REQ, client_err: 0x%x, error_code: 0x%x\n",
                      qmi_err_code, event_conf_resp.resp.error);
      ril_result = QCRIL_UIM_RIL_E_MODEM_ERR;
    }
  ...
  else
  {
    ...
    ril_result = QCRIL_UIM_RIL_E_INTERNAL_ERR;
  }
...
)
{
  qcril_instance_id_e_type             instance_id      = QCRIL_DEFAULT_INSTANCE_ID;
  qcril_modem_id_e_type                modem_id         = QCRIL_DEFAULT_MODEM_ID;
  RIL_Errno                            ril_result       = QCRIL_UIM_RIL_E_INTERNAL_ERR;
  int                                  len              = 0;
  int                                  qmi_err_code     = 0;
  uint8                                slot_index       = QCRIL_GSTK_INVALID_SLOT_INDEX_VALUE;
  cat_send_envelope_cmd_req_msg_v02  * env_cmd_req_ptr  = NULL;;
      ...
      (params_ptr->datalen == 0))
  {
    ...
    ril_result = QCRIL_UIM_RIL_E_INVALID_ARGUMENTS;
    goto send_envelope_response;
  }
  ...
  {
    QCRIL_LOG_ERROR("%s", "error allocating memory for env_cmd_req_ptr!");
    ril_result = QCRIL_UIM_RIL_E_NO_MEMORY;
    goto send_envelope_response;
  }
  ...
  if (env_cmd_resp_ptr == NULL)
  {
    QCRIL_LOG_ERROR("%s", "error allocating memory for env_cmd_resp_ptr!");
    goto send_envelope_response;
  }

  memset(env_cmd_resp_ptr, 0x00, sizeof(cat_send_envelope_cmd_resp_msg_v02));
...
)
{
  ...
  /* Stop SCWS open channel retry timer if running */
  if(qcril_gstk_scws_open_ch_timer[slot_index].timer_id != 0)
  {
    (void)qcril_cancel_timed_callback(&qcril_gstk_scws_open_ch_timer[slot_index].timer_id);
  }

  /* Clean up SCWS open ch timer */
  if(qcril_gstk_scws_open_ch_timer[slot_index].scws_open_ch_msg_ptr != NULL)
  {
    qcril_free(qcril_gstk_scws_open_ch_timer[slot_index].scws_open_ch_msg_ptr);
  }
  memset(&qcril_gstk_scws_open_ch_timer[slot_index], 0, sizeof(qcril_gstk_scws_open_ch_timer[0]));
    ...
    switch (get_cache_req.command_id)
    {
      case CAT_CACHED_COMMAND_ID_SETUP_MENU_V02:
        if (get_cache_resp_ptr->setup_menu_valid)
        {
          ...
          if (get_cache_resp_ptr->setup_menu.pc_setup_menu[1] == 0x81)
          {
            two_byte_len++;
          }

          /* Command details can be 0x01 or 0x81, when comparing ignore the last bit */
          if (get_cache_resp_ptr->setup_menu.pc_setup_menu[0] == QCRIL_GSTK_QMI_PROACTIVE_SIM_COMMAND_TAG &&
              (get_cache_resp_ptr->setup_menu.pc_setup_menu[2 + two_byte_len] & 0x7F) == QCRIL_GSTK_QMI_COMMAND_DETAILS_TAG &&
              get_cache_resp_ptr->setup_menu.pc_setup_menu[3 + two_byte_len] == QCRIL_GSTK_QMI_COMMAND_DETAILS_TLV_LENGTH &&
              get_cache_resp_ptr->setup_menu.pc_setup_menu[5 + two_byte_len] == QCRIL_GSTK_CMD_TYPE_SETUP_MENU)
          {
            /* Set the command number as 0xFE for cached proactive command, so that TR can be ignored */
            get_cache_resp_ptr->setup_menu.pc_setup_menu[4 + two_byte_len] = QCRIL_GSTK_CACHED_PCMD_CMD_NUM;
          ...
          {
            QCRIL_LOG_ERROR("Invalid Set up menu proactive command");
            return;
          }
        }
        break;
      ...
      case CAT_CACHED_COMMAND_ID_SETUP_EVENT_LIST_V02:
        if (get_cache_resp_ptr->setup_event_list_raw_valid)
        {
          ...
          /* Length of the proactive command two bytes, increment command number bit position */
          if (get_cache_resp_ptr->setup_event_list_raw.pc_setup_event_list[1] == 0x81)
          {
            two_byte_len++;
          }

          /* Command details can be 0x01 or 0x81, when comparing ignore the last bit */
          if (get_cache_resp_ptr->setup_event_list_raw.pc_setup_event_list[0] == QCRIL_GSTK_QMI_PROACTIVE_SIM_COMMAND_TAG &&
              (get_cache_resp_ptr->setup_event_list_raw.pc_setup_event_list[2 + two_byte_len] & 0x7F) == QCRIL_GSTK_QMI_COMMAND_DETAILS_TAG &&
              get_cache_resp_ptr->setup_event_list_raw.pc_setup_event_list[3 + two_byte_len] == QCRIL_GSTK_QMI_COMMAND_DETAILS_TLV_LENGTH &&
              get_cache_resp_ptr->setup_event_list_raw.pc_setup_event_list[5 + two_byte_len] == QCRIL_GSTK_CMD_TYPE_SETUP_EVENT_LIST)
          {
            /* Set the command number as 0xFE for cached proactive command, so that TR can be ignored */
            get_cache_resp_ptr->setup_event_list_raw.pc_setup_event_list[4 + two_byte_len] = QCRIL_GSTK_CACHED_PCMD_CMD_NUM;
          ...
          {
            QCRIL_LOG_ERROR("Invalid Set up event list proactive command");
            return;
          }
        }
        break;
      ...
      case CAT_CACHED_COMMAND_ID_SETUP_IDLE_TEXT_V02:
        if (get_cache_resp_ptr->idle_mode_text_valid)
        {
          ...
          /* Length of the proactive command two bytes, increment command number bit position */
          if (get_cache_resp_ptr->idle_mode_text.pc_setup_idle_mode_text[1] == 0x81)
          {
            two_byte_len++;
          }

          if (get_cache_resp_ptr->setup_event_list_raw.pc_setup_event_list[0] == QCRIL_GSTK_QMI_PROACTIVE_SIM_COMMAND_TAG &&
              (get_cache_resp_ptr->idle_mode_text.pc_setup_idle_mode_text[2 + two_byte_len] & 0x7F) == QCRIL_GSTK_QMI_COMMAND_DETAILS_TAG &&
              get_cache_resp_ptr->idle_mode_text.pc_setup_idle_mode_text[3 + two_byte_len] == QCRIL_GSTK_QMI_COMMAND_DETAILS_TLV_LENGTH &&
              get_cache_resp_ptr->idle_mode_text.pc_setup_idle_mode_text[5 + two_byte_len] == QCRIL_GSTK_CMD_TYPE_SETUP_IDLE_MODE_TEXT)
          {
            /* Set the command number as 0xFE for cached proactive command, so that TR can be ignored */
            get_cache_resp_ptr->idle_mode_text.pc_setup_idle_mode_text[4 + two_byte_len] = QCRIL_GSTK_CACHED_PCMD_CMD_NUM;
          ...
          {
            QCRIL_LOG_ERROR("Invalid Set up Idle mode text proactive command");
            return;
          }
        }
        break;
