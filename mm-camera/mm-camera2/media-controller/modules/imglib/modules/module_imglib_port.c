 ...
 *   @events_lock: Lock protecting port events array
 *
...
static boolean module_imglib_reserve_compatible_port(void *data1, void *data2)
{
  ...
  /* If link port is provided check if link is possible */
  if (reserve_data->p_port) {
    if (!mct_port_check_link(reserve_data->p_port, port))
      return FALSE;
  }
 ...
 * Arguments:
 *   @port: mct port pointer
 *   @mirror_port: Used for internal topology linking
 *   @stream_info: Stream info
...
static mct_port_t *module_imglib_get_topo_port(mct_port_t *port,
  mct_port_t *mirror_port, mct_stream_info_t *stream_info, uint32_t topo_index)
{
  ...
  if (!port || !port->port_private || !stream_info) {
    IDBG_ERROR("%s:%d invalid input", __func__, __LINE__);
    goto out;
  }
  ...
  if (mod_ports) {
    ...
    reserve_data.p_port = mirror_port;
  ...
  if (NULL == p_topo_port) {
    ...
    if (module->request_new_port) {
      ...
      IDBG_ERROR("%s:%d Request new port is not supported in %s"
        ,__func__, __LINE__, MCT_MODULE_NAME(module));
    }
  }
  ...
  unsigned int identity)
{
  ...
  session_params = module_imglib_get_session_params(p_mct_mod,
  ...
  for(current = 0; current < CAM_INTF_PARM_MAX; current++) {
    if(!p_table->is_valid[current])
        continue;
    param.type = current;
    param.parm_data  = get_pointer_of(current, p_table);
  ...
  mct_stream_info_t *stream_info, uint32_t port_events_mask)
{
  ...
  for (i = 0; i < MODULE_IMGLIB_PORT_MAX_HOLD_EVENTS; i++) {
    ...
    port_events[i].buf_divert_data.num_planes =
      (uint32_t)stream_info->buf_planes.plane_info.num_planes;
...
static boolean module_imglib_port_handle_events(mct_port_t *port, mct_event_t *event)
{
  mct_port_t *mirror_port;
  imglib_port_data_t *port_data;
  module_imglib_topo_holder_t *topo;
  boolean ret = FALSE;
  ...
  port_data = (imglib_port_data_t *)port->port_private;
  if (!port_data->internal_topo) {
    IDBG_MED("%s:%d No internal topology", __func__, __LINE__);
    return event_processed;
  }
  topo = port_data->internal_topo;
  ...
  case MCT_EVENT_CONTROL_CMD: {
    ...
    case MCT_EVENT_CONTROL_SET_SUPER_PARM:
      MCT_PROF_LOG_BEG(PROF_IMGLIB_SP);
      if ((topo->port_events_mask & MODULE_IMGLIB_PORT_PARSE_SUPER_PARAMS) &&
          p_ctrl_event->control_event_data) {
        mct_event_super_control_parm_t *super_param =
          p_ctrl_event->control_event_data;
        mct_event_t param_event;

        param_event = *event;
        param_event.identity = super_param->identity;
        param_event.u.ctrl_event.type = MCT_EVENT_CONTROL_SET_PARM;

        for (i = 0; i < super_param->num_of_parm_events; i++) {
          param_event.u.ctrl_event.control_event_data =
              &super_param->parm_events[i];

          ret = module_imglib_sent_event_to_mirrors(port, &param_event);
          if (FALSE == ret) {
            IDBG_ERROR("%s:%d] Sent super param fail\n",  __func__, __LINE__);
            break;
          }
        }
        event_processed = ret;
    ...
    case MCT_EVENT_CONTROL_SOF: {
      ATRACE_BEGIN("imglib SOF");
      img_dyn_feat_metadata_t *p_md_dynfeat =
        img_scene_mgr_get_featmeta(get_scene_mgr(),
        p_ctrl_event->current_frame_id);
      if (p_md_dynfeat) {
        IDBG_MED("%s:%d] Dynamic data ready %d mask 0x%llx frame_id %d",
          __func__, __LINE__, p_md_dynfeat->dyn_data_ready,
          p_md_dynfeat->dyn_data.dyn_feature_mask,
          p_ctrl_event->current_frame_id);

        module_imglib_post_dynamic_scene_data(&p_md_dynfeat->dyn_data,
          port, event->identity);
      } else {
        IDBG_MED("%s:%d] Dynamic data skipped frame_id %d",
          __func__, __LINE__, p_ctrl_event->current_frame_id);
      }
      ATRACE_END();
      break;
    }
  ...
  case MCT_EVENT_MODULE_EVENT: {
    ...
    case MCT_EVENT_MODULE_BUF_DIVERT: {
      ...
      pthread_mutex_lock(&port_data->events_lock);
      pending_ack = port_events->pending_ack[MCT_EVENT_MODULE_BUF_DIVERT_ACK];
      pthread_mutex_unlock(&port_data->events_lock);
      ...
      if (port_events->buf_divert_data.use_int_buffers) {
        for (i = 0; i < port_events->buf_divert_data.num_planes; i++) {
          memcpy(port_events->buf_divert_data.addr[i],
            (void *)(uintptr_t)((unsigned int *)p_buf_divert->vaddr)[i],
      ...
      for (i = 0; i < PORT_ATTACHED_TOPO(port); i++) {
        ...
        mirror_port_data =  mirror_port->port_private;

        pthread_mutex_lock(&port_data->events_lock);
        port_events->pending_ack[MCT_EVENT_MODULE_BUF_DIVERT_ACK] |=
          (uint32_t)(1 << mirror_port_data->index);
        pending_ack = port_events->pending_ack[MCT_EVENT_MODULE_BUF_DIVERT_ACK];
        pthread_mutex_unlock(&port_data->events_lock);
      ...
      if (pending_ack) {
        /* If we have internal buffers set ack flag always */
        p_buf_divert->ack_flag = port_events->buf_divert_data.use_int_buffers;
      } else {
        p_buf_divert->ack_flag = TRUE;
      }
    ...
    case MCT_EVENT_MODULE_STATS_AEC_UPDATE: {
      stats_update_t *p_stats_update =
        (stats_update_t *)p_mod_event->module_event_data;
      img_scene_detect_mgr_t *p_scene_mgr = get_scene_mgr();
      if (p_stats_update && p_scene_mgr) {
        rc = img_scene_mgr_handle_aec_update(p_scene_mgr,
          IMGLIB_SESSIONID(event->identity),
          &p_stats_update->aec_update);
      }
      break;
    }
    case MCT_EVENT_MODULE_SET_CHROMATIX_PTR: {
      ...
      rc = img_scene_mgr_handle_chromatix_update(get_scene_mgr(),
        IMGLIB_SESSIONID(event->identity),
        p_chromatix_param);
      if (IMG_ERROR(rc)) {
        IDBG_ERROR("%s:%d] cannot update chromatix %d",  __func__, __LINE__, rc);
        /* fall through - not fatal error */
      }
      break;
    }
    default:
...
/**
 * Function: module_imglib_port_handle_session_params
 *
 * Description: Helper function for handling session parameters
 *
 * Arguments:
 *   @port: Pointer to imglib port
 *   @event: mct event
 *
 * Return values:
 *   TRUE if session params store is successful
 *   FALSE: otherwise
 *
 * Notes: None;
 **/
static boolean module_imglib_port_handle_session_params(mct_port_t *port,
    mct_event_t *event)
{
  boolean ret = FALSE;
  uint32_t i;
  ...
  case MCT_EVENT_CONTROL_CMD: {
    ...
    case MCT_EVENT_CONTROL_SET_SUPER_PARM: {
        ...
        for (i = 0; i < super_param->num_of_parm_events; i++) {
          ret = module_imglib_store_session_params(p_mct_mod,
            &super_param->parm_events[i], IMGLIB_SESSIONID(event->identity));
          if (FALSE == ret) {
            IDBG_ERROR("%s:%d] Store session params fail\n",  __func__, __LINE__);
            break;
          }
  ...
  mct_event_t *event)
{
  ...
  if (!port_data->internal_topo) {
    IDBG_ERROR("%s:%d Fail internal topology is missing", __func__, __LINE__);
    return FALSE;
  }
  topo = port_data->internal_topo;
  ...
  case MCT_EVENT_MODULE_EVENT: {
    ...
    case MCT_EVENT_MODULE_BUF_DIVERT_ACK: {
      ...
      index = (uint32_t)(isp_buf_divert_ack->buf_idx % MODULE_IMGLIB_PORT_MAX_HOLD_EVENTS);
      port_events = &port_data->port_events[index];
    ...
    case MCT_EVENT_MODULE_BUF_DIVERT:
      /* Buffer divert should not be shared between topologies */
      break;
    default:
      if (topo->port_events_mask & MODULE_IMGLIB_PORT_PAR_TOPO_SHARING_EVENT) {
        mct_port_t *mirror_port;
        imglib_mirror_port_data_t *curr_mirr_port_data;
        mct_event_direction orig_event_direction;
        uint32_t i;

        /* Switch event direction when sending to internal topologies */
        orig_event_direction = event->direction;
        if (orig_event_direction == MCT_EVENT_UPSTREAM) {
          event->direction = MCT_EVENT_DOWNSTREAM;
        } else if (orig_event_direction == MCT_EVENT_DOWNSTREAM) {
          event->direction = MCT_EVENT_UPSTREAM;
        } else {
          IDBG_ERROR("%s:%d Invalid port direction %d", __func__, __LINE__,
            orig_event_direction);
          ret = FALSE;
          break;
        }

        for (i = 0; i < PORT_ATTACHED_TOPO(imglib_port); i++) {
          mirror_port = port_data->mirror_port[i];
          curr_mirr_port_data = mirror_port->port_private;

          /* Do not broadcast event to same port */
          if (curr_mirr_port_data->index == mirror_port_data->index) {
            continue;
          }
        ...
        event->direction = orig_event_direction;
      }
      break;
    }
    break;
  ...
  /* Send event upstream to modules connected to imglib */
  if (ret && send_upstream) {
...
out:
  module_imglib_port_handle_session_params(port, event);
  ...
  void *vstream_info)
{
  ...
  if ((0 == stream_info->identity) || (0 != port_data->reserved_identity)) {
    IDBG_MED("%s:%d port already reserved or invalid identity %d",
  ...
  for (i = 0; i < PORT_ATTACHED_TOPO(port); i++) {
    ...
    top_port = module_imglib_get_topo_port(port, port_data->mirror_port[i],
      stream_info, i);
    if (!top_port) {
...
out:
  ...
  if (FALSE == ret) {
    IDBG_ERROR("%s:%d] Can not connect with internal topology, "
               "reserve this port as empty", __func__, __LINE__);
    /* Release mirror port */
    if (NULL != top_port) {
      for (; i > 0; i--) {
  ...
  if (p_port->port_private) {
    ...
    for (i = 0; i < port_data->num_mirror_ports; i++) {
      if (port_data->mirror_port[i]) {
        ...
        /* Remove mirror port parent */
        MCT_OBJECT_PARENT(port_data->mirror_port[i]) =
          mct_list_remove(MCT_OBJECT_PARENT(port_data->mirror_port[i]),
            p_mct_mod);
        MCT_OBJECT_NUM_PARENTS(port_data->mirror_port[i])--;

        mct_port_destroy(port_data->mirror_port[i]);
        port_data->mirror_port[i] = NULL;
      }
    }
    pthread_mutex_destroy(&port_data->events_lock);
  }
  ...
  mct_port_direction_t dir, boolean static_p, uint32_t num_mirror_ports)
{
  ...
  imglib_port_data_t *port_data = NULL;
  ...
  index = (uint32_t)((MCT_PORT_SRC == dir) ?
  ...
  for (index = 0; index < num_mirror_ports; index++) {
    ...
    temp_list = mct_list_append(MCT_OBJECT_PARENT(mirror_port),
      MCT_OBJECT_CAST(p_mct_mod), NULL, NULL);
    if (!temp_list) {
      IDBG_ERROR("%s: Set mirror parent failed", __func__);
      mct_port_destroy(mirror_port);
      mirror_port = NULL;
      free(mirror_port_data);
      mirror_port_data = NULL;
      goto error;
    }
    MCT_OBJECT_PARENT(mirror_port) = temp_list;
    MCT_OBJECT_NUM_PARENTS(mirror_port)++;

    /* Init port data structure */
    port_data->mirror_port[index] = mirror_port;
  }

  pthread_mutex_init(&port_data->events_lock, NULL);
...
error:
  ...
  if (port_data) {
    module_imglib_free_port(p_mct_mod, p_port);
  } else if (p_port) {
    mct_port_destroy(p_port);
  }
  return NULL;
