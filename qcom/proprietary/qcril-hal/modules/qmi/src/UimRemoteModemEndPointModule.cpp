...
static RIL_UIM_RMT_RemoteSIMInfo  remote_sim_info = {0, RIL_UIM_RMT_UICC_REMOTE_TRANSPORT_OTHER, RIL_UIM_RMT_UICC_REMOTE_USAGE_NORMAL, RIL_UIM_RMT_UICC_REMOTE_CONNECTION_UNAVAILABLE};
...
)
{
  ...
  {
    auto ind_ptr = std::make_shared<UimRmtRemoteSimStatusIndMsg>(remote_sim_info);

    /* Send unsolicited indication to framework */
    if (ind_ptr != NULL)
    {
      ind_ptr->broadcast();
    }
...
{
  uint8_t  instance_id = qmi_ril_get_process_instance_id();
  ...
  if(remote_sim_info.isEnable)
  {
    RIL_UIM_RMT_RemoteSIMInfo remote_sim = {};
    auto ind_ptr = std::make_shared<UimRmtRemoteSimStatusIndMsg>(remote_sim);
  ...
  if (req_msg_ptr != nullptr)
  {
       ...
       rsp_data_ptr->result == QMI_UIM_REMOTE_RESULT_SUCCESS)
    {
          ...
          req_data_ptr->req_data.event_req.event == QMI_UIM_REMOTE_CONNECTION_AVAILABLE)
      {
        ...
        remote_sim_info.transportType = (RIL_UIM_RMT_TransportType)req_data_ptr->req_data.event_req.transport;
        remote_sim_info.usage = (RIL_UIM_RMT_Usage)req_data_ptr->req_data.event_req.usage;
        remote_sim_info.remoteEvent = (RIL_UIM_RMT_remoteEvent)req_data_ptr->req_data.event_req.event;
      }
    }
