...
struct nl_msg * alloc_nlmsg(int vendor_cmd, char* iface_name)
{
	struct nl_msg *nlmsg;
	void *msg_head = NULL;
	...
	msg_head = genlmsg_put(nlmsg, /* pid = */ 0, /* seq = */ 0,
		/*family*/gw_update_info.nl80211_fam_id, /* hdrlen*/ 0,
		/* flags*/0, /* cmd*/ NL80211_CMD_VENDOR, /* version = */ 0);
...
static struct nl_msg *nl80211_alloc_msg(int flags, uint8_t cmd)
{
        struct nl_msg *msg;

        msg = nlmsg_alloc();
        if (!msg) {
		wsvc_printf_err("Failed to allocate NL message");
		return NULL;
	}

	if (!genlmsg_put(msg, 0, 0, gw_update_info.nl80211_fam_id,
			 0, flags, cmd, 0)) {
		wsvc_printf_err("Failed to put cmd in nl msg");
		nlmsg_free(msg);
		return NULL;
	}
	return msg;
...
static int clear_nud_stats(struct cnss_gw_data *gw_data,const char* iface_name)
{
	struct nl_msg *nlmsg;
	int ret = -1;
	...
	nlmsg = alloc_nlmsg(QCA_NL80211_VENDOR_SUBCMD_NUD_STATS_SET, iface_name);
	if (!nlmsg)
		return ret;
				 ...
				 const char *iface_name)
{
	...
	return clear_nud_stats(gw_data,iface_name);
}
			     ...
			     const char* iface_name)
{
	...
	nlmsg = alloc_nlmsg(QCA_NL80211_VENDOR_SUBCMD_NUD_STATS_GET, iface_name);
	if (!nlmsg)
		return ret;
...
/* Get interface mode for current interface i.e STA/AP  */
static uint32_t get_interface_activemode(struct cnss_gw_data *gw_data,
				         const char *iface_name)
{
	struct nl_msg *nlmsg;
	struct nl_cb *cb;
	...
	cb = nl_cb_clone(gw_data->g_cb);
	if (!cb)
		return 0;

	info.ifname = iface_name;

	wsvc_printf_dbg("Get current interface mode %s\n",iface_name);
	nlmsg = nl80211_alloc_msg(NLM_F_DUMP, NL80211_CMD_GET_INTERFACE);
	if (!nlmsg)
		goto end;
	nl_cb_set(cb, NL_CB_VALID, NL_CB_CUSTOM,
		  interface_rsp_handler, &info);
	ret = send_recv_nlmsg(nlmsg, cb);
	...
	struct nlmsghdr *nlmsg, int idx)
{
	...
	/*iface_type = get_interface_activemode(gw_data, gw_update_info.new_gw_info.if_name);*/

	/* sometimes kernel ends up sending neighbors which are stale and
	 * unreachable, just skip them
	 */
	/*if (iface_type == NL80211_IFTYPE_STATION) {*/
			...
			case NUD_PERMANENT:
				...
				if (gw_data->nud_sm.state == NUD_PROBE)
					status = handle_nud_reachable(gw_data, attr, len,
						gw_update_info.new_gw_info.if_name);
			break;
			case NUD_PROBE:
	...
	struct route_entry_tag *r_entry, uint8_t rtm_family)
{
	...
	if (rta->rta_type == RTA_GATEWAY) {
		...
		inet_ntop(AF_INET, r_entry->gw_addr, gw_addr,
			addr_str_len);
		temp_gw_info.ipv4_type =
			(rtm_family == AF_INET) ? AF_INET : 0;
		temp_gw_info.ipv6_type =
			(rtm_family == AF_INET6) ? AF_INET6 : 0;
		temp_gw_info.ip_len = addr_str_len;
		...
		memcpy(temp_gw_info.ip_addr, r_entry->gw_addr, data_len);
		gw_update_info.gw_ip_addr_flag = true;
		wsvc_printf_dbg("ip type flag %d\n",gw_update_info.gw_ip_addr_flag);
	}
	...
	if (rta->rta_type == RTA_OIF) {
		...
		/* check if the gateway update is for interface */
			strlcpy(temp_gw_info.if_name, r_entry->ifname,
				sizeof(temp_gw_info.if_name));

		gw_update_info.gw_iface_flag = true;
	}
	...
	if (!strcmp(r_entry->ifname, "wlan0")) {
		if (gw_update_info.gw_ip_addr_flag &&
		    gw_update_info.gw_iface_flag) {
			memcpy(new_gw_info, &temp_gw_info, sizeof(struct cnss_gw_info));
		}
	}
}
...
static int wlan_service_set_neigh_ucast_solicit(const char* str, const char* iface_name)
{
	int ret = 0;
	char ipv4_usolicit[MAX_DEF], ipv6_usolicit[MAX_DEF];

	ret = snprintf(ipv4_usolicit, MAX_DEF,
		       "/proc/sys/net/ipv4/neigh/%s/ucast_solicit", iface_name);
	if(ret < 0 || ret >= MAX_DEF)
	   wsvc_printf_err("failed to copy ipv4 ucast_solicit");
	else
	   ret = wlan_service_update_sys_param (&ipv4_usolicit, str);

	ret = snprintf(ipv6_usolicit, MAX_DEF,
		       "/proc/sys/net/ipv6/neigh/%s/ucast_solicit", iface_name);
	if(ret < 0 || ret >= MAX_DEF)
	   wsvc_printf_err("failed to copy ipv6 ucast_solicit");
	else
	   ret = wlan_service_update_sys_param (&ipv6_usolicit, str);

	return ret;
}

static int wlan_service_set_neigh_retrans_time_ms(const char* str,const char* iface_name)
{
	int ret = 0;
	char ipv4_time_ms[MAX_DEF], ipv6_time_ms[MAX_DEF];

	ret = snprintf(ipv4_time_ms, MAX_DEF,
		       "/proc/sys/net/ipv4/neigh/%s/retrans_time_ms", iface_name);
	if(ret < 0 || ret >= MAX_DEF)
	   wsvc_printf_err("failed to copy ipv4 retrans_time_ms");
	else
	   ret = wlan_service_update_sys_param (&ipv4_time_ms, str);

	ret = snprintf(ipv6_time_ms, MAX_DEF,
		       "/proc/sys/net/ipv6/neigh/%s/retrans_time_ms", iface_name);
	if(ret < 0 || ret >= MAX_DEF)
	   wsvc_printf_err("failed to copy ipv6 retrans_time_ms");
	else
	   ret = wlan_service_update_sys_param (&ipv6_time_ms, str);
...
int cnss_gw_update_loop(int sock)
{
	...
	if (gw_data.new_route && gw_data.neighbor_success &&
		...
		ret = cnss_update_wlan_drv(&gw_data, gw_update_info.new_gw_info.if_name);

		if (ret < 0) {
			wsvc_printf_err("failed to update gateway\n");
			goto cleanup;
		}
...
static int interface_rsp_handler(struct nl_msg *msg, void *arg)
{
	...
	const char *ifname = "";
	uint32_t  iface_type = NL80211_IFTYPE_UNSPECIFIED;
	...
	if (tb_msg[NL80211_ATTR_IFTYPE]){
		iface_type = nla_get_u32(tb_msg[NL80211_ATTR_IFTYPE]);
	}
	if (strncmp(ifname, ifinfo->ifname, IFNAMSIZ-1) == 0) {
		ifinfo->iface_type = iface_type;
	}

	wsvc_printf_dbg("interface response handler invoked\n");
	return 0;
}
...
static int nud_stats_rsp_handler (struct nl_msg *msg, void *arg)
{
	...
	if (result < 0) {
		wsvc_printf_err("Failed to parse nud_stats_rsp\n");
		return -1;
	}
...
static int cnss_gw_init_wifi_socket(struct cnss_gw_data *gw_data)
{
	...
	gw_update_info.nl80211_fam_id =
		genl_ctrl_resolve(gw_update_info.wifi_sockfd, "nl80211");
	if (gw_update_info.nl80211_fam_id < 0) {
...
int cnss_update_wlan_drv(struct cnss_gw_data *gw_data, const char *iface_name)
{
	int ret, err, res;
