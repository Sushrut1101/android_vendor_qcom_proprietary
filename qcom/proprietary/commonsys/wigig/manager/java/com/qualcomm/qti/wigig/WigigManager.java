...
public class WigigManager {
    ...
    /**
     * Broadcast intent action indicating that Sensing has been enabled, disabled,
     * or unknown. One extra provides this state as an int.
     * Another extra provides the previous state, if available.
     *
     * @see #EXTRA_SENSING_STATE
     * @see #EXTRA_PREVIOUS_SENSING_STATE
     */
    @SdkConstant(SdkConstantType.BROADCAST_INTENT_ACTION)
    public static final String SENSING_STATE_CHANGED_ACTION =
            "com.qualcomm.qti.wigig.SENSING_STATE_CHANGED";

    /**
     * The lookup key for an int that indicates whether Sensing is enabled,
     * disabled or unknown.  Retrieve it with
     * {@link android.content.Intent#getIntExtra(String,int)}.
     *
     * @see #SENSING_STATE_DISABLED
     * @see #SENSING_STATE_ENABLED
     * @see #SENSING_STATE_UNKNOWN
     * @see #SENSING_STATE_FAILED
     */
    public static final String EXTRA_SENSING_STATE = "sensing_state";

    /**
     * The previous Sensing state.
     *
     * @see #EXTRA_SENSING_STATE
     */
    public static final String EXTRA_PREVIOUS_SENSING_STATE = "previous_sensing_state";

    /**
     * Sensing is disabled.
     *
     * @see #SENSING_STATE_CHANGED_ACTION
     * @see #getSensingState()
     */
    public static final int SENSING_STATE_DISABLED = 0;
     ...
     * @see #getSensingState()
     */
    public static final int SENSING_STATE_ENABLED = 1;

    /**
     * Sensing is in an unknown state. This state will occur when an error happens while enabling
     * or disabling.
     *
     * @see #SENSING_STATE_CHANGED_ACTION
     * @see #getSensingState()
     */
    public static final int SENSING_STATE_UNKNOWN = 2;

    /**
     * Sensing is in failed state. This state will occur when load driver failed.
     *
     * @hide
     */
    public static final int SENSING_STATE_FAILED = 3;
    ...
    /**
     * Enable or disable Sensing.
     * @param enable {@code true} to enable, {@code false} to disable
     * @return {@code true} if the request to enable/disable sensing was accepted
     */
    public boolean setSensingEnabled(boolean enable) {
        try {
            mService.setSensingEnabled(enable);
            return true;
        } catch (RemoteException e) {
            Log.e(TAG, "setSensingEnabled: " + e);
            return false;
        }
    }

    /**
     * Return whether Sensing stack is enabled
     * @return {@code true} if the Sensing stack is enabled
     */
    public boolean isSensingEnabled() {
        try {
            return mService.isSensingEnabled();
        } catch (RemoteException e) {
            Log.e(TAG, "isSensingEnabled: " + e);
            return false;
        }
    }
     ...
     * {@link WigigManager#registerSoftApCallback(SoftApCallback, Handler)}.
     *
     * @hide
     */
     ...
     * via callback. Note that receiving an immediate WIGIG_AP_STATE_FAILED value for soft AP state
