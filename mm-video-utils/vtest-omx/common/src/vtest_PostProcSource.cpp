...
PostProcSource::PostProcSource(Crypto *pCrypto)
    ...
    snprintf(m_pName, MAX_NAME_LENGTH, "%s", "PostProcSource");
...
OMX_ERRORTYPE PostProcSource::UseBuffers(BufferInfo **pBuffers,
        ...
        OMX_U32 ePortIndex) {
    (void)nWidth, (void)nHeight, (void)ppExtraDataBuffers;
    (void)nExtraDataBufferCount, (void)nExtraDataBufferSize;
    {
        Mutex::Autolock autoLock(m_pLock);
