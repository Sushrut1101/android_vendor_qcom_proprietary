...
typedef struct
{
  ...
  wds_data_system_status_type_v01 data_sys_status[ QCRIL_MAX_INSTANCE_ID ];
...
*/
/*=========================================================================*/
int qcril_arb_query_max_num_of_instances
(
  void
)
{
...
)
{
    ...
    QCRIL_LOG_INFO( "qcril_arb_set_data_sys_status instance:%d", (int)cur_instance);

    QCRIL_LOG_DEBUG("recevd data_sys_status pref_network=0x%x, len=%d",
                    data_sys_status->preferred_network,
    ...
    for(i = 0; i < data_sys_status->network_info_len; i++)
    {
      QCRIL_LOG_ESSENTIAL("recvd data_sys_status nw=0x%x, rat_mask=0x%x, so_mask=0x%x",
                      ...
                      data_sys_status->network_info[i].so_mask );
    }
    ...
    qcril_arb.data_sys_status[ cur_instance ] = *data_sys_status;
    if(data_sys_status->network_info_len > QCRIL_ARB_NETWORK_INFO_LEN )
    {
      qcril_arb.data_sys_status[ cur_instance ].network_info_len = QCRIL_ARB_NETWORK_INFO_LEN;
    }
...
)
{
  ...
  QCRIL_LOG_ESSENTIAL( "qcril_arb_set_pref_data_tech action new tech %d for instance %d", (int) pref_data_tech, (int) cur_instance);
/*
  if ( instance_id < QCRIL_MAX_INSTANCE_ID )
  {
*/
    ...
    if( pref_data )
    {
        ...
        if( TRUE == qcril_arb.is_current[cur_instance] )
        {
            ...
            else
            {
                ...
                {
                    if( qcril_arb.data_sys_status[cur_instance].preferred_network ==
                        qcril_arb.data_sys_status[cur_instance].network_info[iter_i].network )
                    {
                        pref_data->data_sys_status_info.network_type = (qmi_wds_data_network_type)qcril_arb.data_sys_status[ cur_instance ].network_info[iter_i].network;
                        // RIL REFACTOR TODO: This needs to be re worked. JIRA ATEL-72 created
                        pref_data->data_sys_status_info.rat_mask.cdma_rat_mask = (qmi_wds_cdma_rat_mask)qcril_arb.data_sys_status[ cur_instance ].network_info[iter_i].rat_mask;
                        pref_data->data_sys_status_info.db_so_mask.so_mask_value = (int)qcril_arb.data_sys_status[cur_instance].network_info[iter_i].so_mask;
                        break;
                    }
                }
...
void qcril_qmi_arb_reset_dsd_system_status_info()
{
    ...
    memset(&qcril_arb.data_sys_status,  0, sizeof(qcril_arb.data_sys_status));
    QCRIL_MUTEX_UNLOCK( &qcril_arb.mutex, "qcril_arb.mutex" );
}
