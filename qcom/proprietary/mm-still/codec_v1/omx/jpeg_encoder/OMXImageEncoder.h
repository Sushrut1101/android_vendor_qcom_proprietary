...
// stats debug mask for exif
#define STATS_DEBUG_MASK            (1 << 1)

// default number of EXIF markers
#define NUM_EXIF_MARKERS                 (3)
// number of EXIF markers for mobicat
#define NUM_EXIF_MARKERS_MOBICAT         (1)
// number of EXIF markers for MPO
#define NUM_EXIF_MARKERS_MPO             (2)
// number of EXIF markers for stats debug
#define NUM_EXIF_MARKERS_STATS_DEBUG     (3)
...
protected:
 ...
 /** omx_component_get_config:
  ...
  /** setNumExifMarkers
   *
   *  calculates number of EXIF markers based on if
   *  mobicat/3adebug/mpo is enabled
   **/
  void setNumExifMarkers();
...
protected:
   ...
   *  Quantization table
   **/
  OMX_IMAGE_PARAM_QUANTIZATIONTABLETYPE m_quantTable[QIQuantTable::QTABLE_MAX];
  ...
  /** m_sw3aVersion
   *
   *  3a sw ver info of 8byte length as per 3A debug header spec
   **/
  uint16_t m_sw3aVersion[4];
   ...
   * MPO info
   */
  QOMX_JPEG_MULTI_IMAGE_INFO m_MultiJpegSequence;
