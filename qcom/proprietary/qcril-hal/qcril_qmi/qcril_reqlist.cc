...
static qtimutex::QtiSharedMutex qcril_reqlist_mutex;
...
static void qcril_reqlist_clear_reqlist_public_type(qcril_reqlist_public_type* req_pub)
{
  ...
  if( req_pub )
  {
    ...
    if ( req_pub->req_data )
    {
      ...
      QCRIL_LOG_INFO("freed req_data for request: %d, token id: %" PRId32 " (%p)", req_pub->request, req_pub->t_id, req_pub->t);
    }
  }
  else
  {
    ...
    *dest = *src;
    ...
    if ( NULL != src->req_data && 0 != src->req_datalen)
    {
      ...
      if ( NULL != dest->req_data )
      {
        ...
        QCRIL_LOG_INFO("copied req_data for request: %d, token id: %" PRId32 " (%p)", dest->request, dest->t_id, dest->t);
      }
      else
      {
...
static qcril_reqlist_buf_type* qcril_reqlist_find_following_blocked_request_buf(const qcril_reqlist_buf_type* buf_ptr)
{
  ...
  if ( NULL != tmp )
  {
    QCRIL_LOG_INFO("for request %d, find the blocked request %d (token id %" PRId32 " (%p))",
                   buf_ptr->pub.request, tmp->pub.request, tmp->pub.t_id, tmp->pub.t);
  }
  else if ( NULL != buf_ptr )
  {
    QCRIL_LOG_INFO("for request %d, didn't find the blocked request", buf_ptr->pub.request);
  }
  else
  {
...
)
{
  ...
  while ( buf_ptr != NULL )
  {
    if ( buf_ptr->pub.t == t )
    {
      QCRIL_LOG_INFO( "[RID %d] Found ReqList entry : %s(%p), token id %" PRId32 " (%p)",
  ...
  QCRIL_LOG_INFO( "[RID %d] Not found ReqList entry : token %p", instance_id, t );
...
/*===========================================================================
  ...
  FUNCTION:  qcril_reqlist_find_with_possibly_freed_token

===========================================================================*/
/*!
    @brief
    Find an entry in the list based on RIL_Token
    Almost the same as qcril_reqlist_find, but need one more parameter
    "token_id", which is used for logging purpose only

    @return
    A pointer to the entry if found, NULL otherwise.
*/
/*=========================================================================*/
static qcril_reqlist_buf_type *qcril_reqlist_find_with_possibly_freed_token
(
  qcril_instance_id_e_type instance_id,
  RIL_Token t,
  int32_t token_id
)
{
  ...
  while ( buf_ptr != NULL )
  {
    if ( buf_ptr->pub.t == t )
    {
      QCRIL_LOG_INFO( "[RID %d] Found ReqList entry : %s(%p), token id %" PRId32 "",
                      instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                      token_id);
      return buf_ptr;
    }
  ...
  QCRIL_LOG_INFO( "[RID %d] Not found ReqList entry : token id %" PRId32 "", instance_id, token_id);

  return NULL;

} /* qcril_reqlist_find_with_possibly_freed_token() */
...
)
{
  ...
  while ( buf_ptr != NULL )
  {
    for ( i=0; i<num; i++ )
    {
      ...
      {
        QCRIL_LOG_INFO( "[RID %d] Found ReqList entry : %s(%d), token id %" PRId32 " (%p)",
                        instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                        buf_ptr->pub.t_id,
                        buf_ptr->pub.t );
        return buf_ptr;
  ...
  for ( i = 0; i < QCRIL_ARB_MAX_INSTANCES; i++ )
  {
    ...
    while ( buf_ptr != NULL )
    {
      if ( buf_ptr->pub.req_id == req_id )
      {
        ...
        {
          *instance_id_ptr = static_cast<qcril_instance_id_e_type>(i);
        }
...
)
{
  ...
  while ( buf_ptr != NULL )
  {
    ...
    {
      QCRIL_LOG_INFO( "[RID %d] Found ReqList entry : %s(%d), token id %" PRId32 " (%p), pending %s (%lu)",
                      instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                      buf_ptr->pub.t_id,
                      buf_ptr->pub.t,
...
)
{
  ...
  while ( buf_ptr != NULL  )
  {
    ...
    {
      QCRIL_LOG_INFO( "[RID %d] Found ReqList entry : %s(%d), token id %" PRId32 " (%p), pending %s (%lu), sub id %lu",
                      instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                      buf_ptr->pub.t_id,
                      buf_ptr->pub.t,
...
)
{
  ...
  uint32_t t_id = qcril_log_get_token_id( entry_ptr->t );
  buf_ptr = qcril_reqlist_find( instance_id, entry_ptr->t );

  if ( buf_ptr != NULL )
  {
    /* Token is already in the list */
    QCRIL_LOG_ERROR( "[RID %d] Fail to add ReqList entry with token [%p (token id %" PRId32 ")] (already exists)",
                     instance_id, buf_ptr->pub.t, buf_ptr->pub.t_id);
    return E_NOT_ALLOWED;
  }

  QCRIL_LOG_DEBUG("[RID %d] Creating new entry for token %p with token id %" PRId32 "",
          instance_id, entry_ptr->t, t_id );
  ...
  if ( buf_ptr == NULL )
  {
    ...
    QCRIL_LOG_ERROR( "[RID %d] Fail to add ReqList entry for token id %" PRId32 " [%p] (no buffer)",
                     instance_id, t_id, entry_ptr->t );
    return E_NO_MEMORY;
  }
  ...
  QCRIL_LOG_INFO( "[RID %d] Event %s(%d), token id %" PRId32 " [ %p], req id %d : ",
                  instance_id, qcril_log_lookup_event_name( pub_ptr->request ), pub_ptr->request,
                  pub_ptr->t_id, pub_ptr->t, pub_ptr->req_id  );
  for ( i = 0; i < QCRIL_ARB_MAX_MODEMS; i++ )
  {
...
/*===========================================================================
  ...
  FUNCTION:  qcril_reqlist_get_entry_from_request_range

===========================================================================*/
/*!
    @brief
    Find request within a range.

    @return
    None
*/
/*=========================================================================*/
IxErrnoType qcril_reqlist_get_entry_from_group
(
  qcril_instance_id_e_type instance_id,
  qcril_evt_group grp,
...
)
{
  ...
  while ( buf_ptr != NULL  )
  {
    if ( qcril_is_event_in_group( buf_ptr->pub.request, grp ))
    {
      QCRIL_LOG_INFO( "[RID %d] Found ReqList entry : %s(%d), token id %" PRId32 " (%p)",
                      instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ),
                      buf_ptr->pub.request, buf_ptr->pub.t_id, buf_ptr->pub.t );
      *req_info = buf_ptr->pub;
      status    = E_SUCCESS;
      break;
    }
    else
  ...
  if ( status != E_SUCCESS )
  {
    QCRIL_LOG_INFO("[RID %d] No requests in ReqList: group: %p", instance_id, grp);
  }
  return status;
} /* qcril_reqlist_get_entry_from_request_range */
...
)
{
  ...
  while ( buf_ptr != NULL )
  {
    QCRIL_LOG_DEBUG( "    %s (%d), token id %" PRId32 " (%p) followed %p following %p",
                     qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                     buf_ptr->pub.t_id, buf_ptr->pub.t,
...
)
{
  ...
  { // generally we do so for backwards compatibilty with Data RIL and UIM RIL as QMI RIL "new" code would send failure directly anway
    ...
    qcril_default_request_resp_params( instance_id, entry_ptr->t, entry_ptr->request,
                                       ril_err,
                                       &resp );
  ...
  { // generally we do so for backwards compatibilty with Data RIL and UIM RIL as QMI RIL "new" code would send failure directly anway
    ...
    switch(status)
    {
      case E_SUCCESS:
        ...
        break;
      case E_NOT_ALLOWED:
        ril_err = RIL_E_INVALID_STATE; //needs to be changed after internal discussion
        break;
      case E_NO_MEMORY:
        ril_err = RIL_E_NO_MEMORY;
        break;
...
)
{
  ...
  QCRIL_LOG_INFO( "[RID %d] Try to delete ReqList entry : token (%p)", instance_id, t);
...
/*===========================================================================
  ...
  FUNCTION:  qcril_reqlist_free_deferred

===========================================================================*/
/*!
    @brief
    Frees an entry from the list of outstanding RIL_REQUESTs.
    The function is the almost the same as qcril_reqlist_free, except it
    needs one more parameter "token_id" to avoid dereference the token, that
...
*/
/*=========================================================================*/
IxErrnoType qcril_reqlist_free_deferred
(
  qcril_instance_id_e_type instance_id,
  RIL_Token t,
  int32_t token_id
)
{
  ...
  buf_ptr = qcril_reqlist_find_with_possibly_freed_token( instance_id, t, token_id );
  QCRIL_LOG_INFO( "[RID %d] Try to delete ReqList entry : token id %" PRId32 " [%p]", instance_id, token_id, t);
  status = qcril_reqlist_remove_request_pub_from_reqlist(buf_ptr, instance_id);
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    else
    {
      ...
      QCRIL_LOG_INFO( "[RID %d] Update ReqList entry : %s(%d), token id %" PRId32 " (%p), state %s(%d)",
                      instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                      buf_ptr->pub.t_id,
                      buf_ptr->pub.t, qcril_reqlist_lookup_state_name( state ), state );
    }
  }
  else
  {
...
)
{
  ...
  {
    buf_ptr->pub.pending_event_id[ modem_id ] = pending_event_id;
    QCRIL_LOG_INFO( "[RID %d] Update event field of ReqList entry : %s(%d), token id %" PRId32 " (%p), MID %d pending %s (%d)",
                    instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                    buf_ptr->pub.t_id, buf_ptr->pub.t, modem_id,
                    qcril_log_lookup_event_name( pending_event_id ), pending_event_id );
  }
  else
  {
...
)
{
  ...
  if (buf_ptr != NULL)
  {
    ...
    QCRIL_LOG_INFO( "[RID %d] Update sub id field of ReqList entry : %s(%d), token id %" PRId32 " (%p), sub id %lu",
                    instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                    buf_ptr->pub.t_id, buf_ptr->pub.t, sub_id );
  }
  else
  {
    /* Token not in the list */
...
/*===========================================================================
  ...
  FUNCTION:  qcril_reqlist_query_possibly_freed_token
...
/*!
    ...
    Finds the entry in the list for the given RIL_Token and returns it to
    the requesting subsystem.
    It is possible token has been freed, so do not de-reference it. The caller
    needs to provide token id for logging purposes
...
*/
/*=========================================================================*/
IxErrnoType qcril_reqlist_query_possibly_freed_token
(
  qcril_instance_id_e_type instance_id,
  RIL_Token t,
  uint32_t token_id,
...
} /* qcril_reqlist_query_possibly_freed_token() */
...
)
{
  ...
  if ( buf_ptr != NULL && NULL != sub_ptr )
  {
    ...
    QCRIL_LOG_INFO( "[RID %d] Update sub info of ReqList entry : %s(%d), token id %" PRId32 " (%p)",
                    instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                    buf_ptr->pub.t_id, buf_ptr->pub.t );
  }
  else
...
)
{
  ...
  *result = E_SUCCESS;
  ...
  if ( buf_ptr != NULL )
  {
    ...
    for ( i = 0; i < QCRIL_ARB_MAX_MODEMS; i++ )
    {
      QCRIL_LOG_DEBUG( "[RID %d] %s(%d) token id %" PRId32 " (%p), MID %d, State %s(%d)",
                       instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                       buf_ptr->pub.t_id,
                       buf_ptr->pub.t, i, qcril_reqlist_lookup_state_name( buf_ptr->pub.state[ i ] ),
                       buf_ptr->pub.state[ i ] );
           ...
           ( buf_ptr->pub.state[ i ] == QCRIL_REQ_COMPLETED_FAILURE ) )
      {
        if ( NULL != modem_ids_done_list )
        {
          modem_ids_done_list->modem_id[ modem_ids_done_list->num_of_modems++ ] =
              static_cast<qcril_modem_id_e_type>(i);
        }
    ...
    if ( status == E_SUCCESS )
    {
      QCRIL_LOG_DEBUG( "[RID %d] Complete all AMSS events, ReqList entry : %s(%d), token %" PRId32 " (%p), result %d",
                       instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                       buf_ptr->pub.t_id, buf_ptr->pub.t, *result );
    }
  }
  else
  {
...
)
{
  ...
  if ( NULL != data )
  {
    auto requests_requirement =
        static_cast<qcril_reqlist_generic_concurency_requests_requirement_type *>(data);
    ...
    *followed_req = NULL;
    ...
    req_buf = qcril_reqlist_find_by_requests(instance_id, requests_requirement->req_ids, req_ids_num);
...
/*===========================================================================
  ...
  FUNCTION:  qcril_reqlist_get_reqlist_by_instance_id

===========================================================================*/
/*!
    @brief
     Retrieve reqlist of specific instance_id.
     'qcril_reqlist_mutex' Mutex needs to be acquired before using this API.

    @return
    A pointer to the entry if found, NULL otherwise.
*/
/*=========================================================================*/
qcril_reqlist_buf_type *qcril_reqlist_get_reqlist_by_instance_id
(
  qcril_instance_id_e_type instance_id
)
{
  return qcril_reqlist[ instance_id ].head_ptr;

}/* qcril_reqlist_get_reqlist_by_instance_id() */

/*===========================================================================

  FUNCTION:  qcril_reqlist_acquire_mutex

===========================================================================*/
/*!
    @brief
    API to acquire 'qcril_reqlist_mutex' mutex.

    @return
    A pointer to the entry if found, NULL otherwise.
*/
/*=========================================================================*/
void qcril_reqlist_acquire_mutex()
{
  QCRIL_MUTEX_LOCK( &qcril_reqlist_mutex, "qcril_reqlist_mutex" );
}
...
/*===========================================================================
  ...
  FUNCTION:  qcril_reqlist_release_mutex

===========================================================================*/
/*!
    @brief
    API to release 'qcril_reqlist_mutex' mutex.

    @return
    A pointer to the entry if found, NULL otherwise.
*/
/*=========================================================================*/
void qcril_reqlist_release_mutex()
{
  QCRIL_MUTEX_UNLOCK( &qcril_reqlist_mutex, "qcril_reqlist_mutex" );
}
