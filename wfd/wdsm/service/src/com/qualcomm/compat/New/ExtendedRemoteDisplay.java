...
public final class ExtendedRemoteDisplay {
    ...
    private static final int TEARDOWN_TIMEOUT = 12000; // 12 seconds
    /*DEFAULT_TIMEOUT of 5 sec is the time for which we wait for the sink RTSP/1.0 200 OK response.
    REQUEST_TIMEOUT of 6 sec is the time for which we wait for Teardown from sink.
    Hence the TEARDOWN_TIMEOUT should come out to be 6+5+1(ballpark) sec.*/
