/*===========================================================================
  ...
  Copyright (c) 2012-2016, 2020 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
struct ss_algo_t {
	...
	char                    device_units[MAX_UNIT_NAME_SIZE];
};
...
static int settimer(struct sampling_group *sampling_group)
{
	...
	ret_val = timerfd_settime(sampling_group->timer_id, 0, &sampling_group->timer_val, NULL);

	if (ret_val == -1) {
		msg("%s: timerfd_settime failed\n",__func__);
		return ret_val;
	}

	thermalmsg(LOG_LVL_DBG, (LOG_LOGCAT | LOG_TRACE),
		"%s: Start timer %d.%03d(sec)", __func__,
...
static int handle_sampling_adjust(struct ss_algo_t *ss_info, uint32_t algo_idx, uint32_t sampling)
{
	...
	if(s_idx >= sampling_group_count) {
		...
		sampling_groups[s_idx].timer_id = timer_register_event(s_idx, timer_expired);
		if (sampling_groups[s_idx].timer_id == -1) {
...
static void handle_config_set_sig(void)
{
	...
	for (i = 0; i < cfg_set_cnt; i++) {
		...
		if (ret < 0) {
			msg("Error in validating clients\n");
			return;
		}
...
static void increase_mitigation_lvl(struct ss_algo_t *instance)
{
	...
	if (applied_lvl >= 0) {
		thermalmsg(LOG_LVL_DBG, (LOG_LOGCAT | LOG_TRACE),
			"%s: SS Id %s, Device %s, Req. %d%s, Applied  %d%s, TC delay %d",
			__func__, instance->desc, instance->setting->device,
			req.value, instance->device_units, applied_lvl,
			instance->device_units, instance->tc_delay);
		thermalmsg(LOG_LVL_INFO, LOG_TRACE, "%s: %s: %s=%s %s=%d%s",
		...
		for (idx = 0; idx < clnt_cnt; idx++) {
			...
			if (E0 < 0 || (E0 == 0 && algo_clnt[idx].curr_lvl <= 0)) {
				...
				if (algo_clnt[idx].curr_lvl <= 0) {
					...
					pthread_mutex_unlock(&wait_mutex);
					algo_clnt[idx].state = SS_STATE_STOP_SAMPLING;
					handle_thresh_sig(SS_READ_CUR_TEMP);
				}
			}
...
static void handle_thresh_sig(enum temp_read_type type)
{
	...
	for (idx = 0; idx < clnt_cnt; idx++) {
		...
		if (type == SS_READ_CUR_TEMP)
			temp = sensors_manager_read(algo_clnt[idx].sensor_clnt);
		else
			temp = sensors_manager_read_trip_temp(algo_clnt[idx].sensor_clnt);
...
static void *algo_monitor(void *data)
{
	...
	for(i = 0; i < sampling_group_count; i++) {
		...
		sampling_groups[i].timer_id = timer_register_event(i, timer_expired);
	...
	while (1) {
		...
		if (local_ev_mask & THRESH_EV) {
			...
			handle_thresh_sig(SS_READ_TRIP_TEMP);
		}
...
int ss_algo_init(struct thermal_setting_t *setting)
{
	...
	while (cfg) {
		...
		if (dev_info.dev_type != DEVICE_OP_VALUE_TYPE &&
		    dev_info.dev_type != DEVICE_CDEV_TYPE) {
