...
static struct RIL_Env            * qcril_sap_response_api;
...
)
{
  ...
  QCRIL_LOG_DEBUG("handle_sap_request_resp, token: %d, MsgId: 0x%x, qmi_err_code: 0x%x, err_code: %d",
                  qcril_log_get_token_id((RIL_Token)token),
                  message_id,
                  qmi_err_code,
                  err_code);
    ...
    case MsgId_RIL_SIM_SAP_CONNECT:
      {
        ...
        resp.response = (qmi_err_code == QMI_NO_ERR) ?
    ...
    case MsgId_RIL_SIM_SAP_APDU:
      {
        ...
        resp.response     = qcril_uim_sap_convert_err_to_proto_err(message_id, err_code);
        if (data_len == 0 ||
            data_ptr == NULL ||
    ...
    case MsgId_RIL_SIM_SAP_SET_TRANSFER_PROTOCOL:
      {
        ...
        /* Encode the outgoing message */
        stream = pb_ostream_from_buffer(response_ptr, sizeof(RIL_SIM_SAP_SET_TRANSFER_PROTOCOL_RSP));
...
)
{
  ...
  req_ptr = (qcril_uim_sap_atel_request *)params_ptr->data;
  if (req_ptr == NULL)
  {
  ...
  switch (req_ptr->request_id)
  {
    ...
    case MsgId_RIL_SIM_SAP_DISCONNECT:
      ...
      result = qcril_uim_sap_handle_qmi_sap_connection(params_ptr->t,
                                                       req_ptr->request_id,
                                                       req_ptr->data_ptr,
                                                       req_ptr->data_len);
      break;
    ...
    case MsgId_RIL_SIM_SAP_RESET_SIM:
    case MsgId_RIL_SIM_SAP_TRANSFER_CARD_READER_STATUS:
      result = qcril_uim_sap_handle_qmi_sap_request(params_ptr->t,
                                                    req_ptr->request_id,
                                                    req_ptr->data_ptr,
                                                    req_ptr->data_len,
                                                    &err_code);
...
{
  char *version = "Qualcomm Technologies Inc. RIL UIM SAP 1.0";
