...
/** af_port_zero_touch_lag_buf_ack_ctrl_t:
*    @is_touch_buf_done: TRUE if there is no need to previous frame's buffer.
*    @prev_frame_buf_idx: previous frame's buffer index.
*
*/
typedef struct {
  boolean is_touch_buf_done;
  uint8 prev_frame_buf_idx;
} af_port_zero_touch_lag_buf_ack_ctrl_t;
...
typedef struct {
  ...
  af_port_zero_touch_lag_buf_ack_ctrl_t zero_touch_lag;
...
typedef struct {
  ...
  boolean                 need_wakeup_trigger;
}af_dual_cam_info;

/** af_port_pdaf_zero_touch_lag_field_t:
*   indicates which field need to be updated.
**/
typedef enum {
  AF_PD_TOUCH_ROI_TYPE = 0,
  AF_IS_FIRST_FRAME_TOUCH,
  AF_IS_FIRST_TOUCH,
...
/** af_port_pdaf_early_res_field_t:
*   indicates which field need to be updated.
**/
typedef enum {
  AF_PDLIB_PROCESS_RES = 0,
  AF_PDLIB_BOTTOM_MULTI_WIN_PROCESS,
  ALL
} af_port_pdaf_early_res_field_t;

/** pd_zero_touch_lag_info_t:
*   @ is_it_first_frame:  TRUE if this is the first frame on which touch happens.
*                         This flag will be used in IFace thread at the end of frame.
*                         If this flag is TRUE, means touch has been handled so PD stat
*                         won't be sent again to PDlibat at the end of frame.
*   @ is_it_first_touch:  TRUE if this is the first frame on which touch happens.
*                         This flag will be used in AF thread to decide if sends
*                         ROI coordinate to PDLib along with PD stat or not.
*                         If the flag is TRUE, after sending ROI config to PDLib it gets FALSE.
*   @ roi_type: It shows if ROI is touch or general ROI. When ROI is touch,
*               after the first frame handleing touch for ZTL,
*               eventhough ROI from upper layer is still touch but this ROI
*               will be changed to general.
*   @ pdaf_config:  Keep ROI information which needs to be sent to PDLib along with touch buffer
*   @ prev_frame_idivert_buf_idx: keep previous frame buf index to be used in touch case.
**/
typedef struct {
  boolean is_it_first_frame;
  boolean is_it_first_touch;
  af_roi_type roi_type;
  pdaf_window_configure_t pdaf_config;
  int8_t prev_frame_idivert_buf_idx;
} pd_zero_touch_lag_info_t;


/** pd_lib_early_result_t:
*   @ mutex:  to sync accessing and updateing info by IFace, ISP and AF thread
*   @ process_bottom_multi_window:  TRUE if MultiWindow feature is enabled via tuning or setProp
*                         and PDLib is not confident after processing center ROI.
*   @ pdaf_processing_result_early_int:  result of PDLib stat processing. If PDLib is
*                         confident, it returns PDAF_PDLIB_RESULT_DONE otherwise it returns
*                         PDAF_PDLIB_RESULT_PARTIAL
**/
typedef struct {
  pthread_mutex_t mutex;
  boolean process_bottom_multi_window;
  pdaf_lib_processing_result_t pdaf_processing_result_early_int;
} pd_lib_early_result_t;

/** pd_zero_touch_lag_t:
*   @ mutex: to sync accessing and updateing info by IFace, ISP and AF thread
*   @ info: Zero touch lag information
**/
typedef struct {
  pthread_mutex_t mutex;
 ...
 *
 *    @p_af_tuning:   chromatix pointer
 *
 *    @pd_zero_touch_lag: zero touch lag information
 *
...
typedef struct _af_port_private {
  ...
  af_tuning_algo_adapter_t      af_algo_tuning;
  af_algo_tune_parms_adapter_t  *p_af_tuning;
  float                         focus_value;
  boolean                       spot_light_detected;
  boolean                       is_af_hj_enabled;
  pd_lib_early_result_t         pd_lib_early_result;
  pd_zero_touch_lag_t           pd_zero_touch_lag;
  int                           current_dist_far_in_cm;
  int                           current_dist_near_in_cm;
  int                           is_lens_moving_manual;
} af_port_private_t;
