...
uint8_t DataRspRequired[ ] = { 0x90, 0x00};
...
void ProcessReturnedMessage( PNCI_MESSAGE pReadNCIMessage )
{
    ...
    uint8_t payloadLength = pReadNCIMessage->len;
    ...
    case NCIMT_DATA:                                // data?
        if (ese_dwp_test)
        {
            if( memcmp( DataRspRequired, &nciReplyMessage[payloadLength + 1], sizeof( DataRspRequired ) ) == 0 )
            {
    ...
    do
    {
        ...
        {                                                // not a good reply?
            ...
            {                                            // check if NCI version is 1.0
                LOG_ERROR( "%s: bad reply for coreResetRsp, check chipid", __func__ );
                check_property = TRUE;
            }
        }
        ...
        if( check_property )
        {
            ...
            chip_version = nciReplyMessage[ CHIP_ID ];

        switch( chip_version )             // what type of chip is it?
        {
        case 0x48:
            whatNQChip = NQ_210;
        ...
        case 0x51:
            whatNQChip = NQ_330;
            LOG_INFORMATION( "Connected to NQ330\NQ440 \n" );
            break;
        ...
        case 0xa4:
            whatNQChip = SN_100;
            LOG_INFORMATION( "Connected to SN100 \n" );
            break;
