...
static bool sIsImsPdpConnected = FALSE;

bool qcril_qmi_nas_ims_is_pdp_connected()
{
  QCRIL_LOG_FUNC_RETURN_WITH_RET(sIsImsPdpConnected);
  return sIsImsPdpConnected;
}
...
uint8_t qcril_qmi_nas_ims_registered_wlan_status()
{
  ...
  if ((wifi_call != ImsWfcSettingsStatusInd::Status::WFC_ON) && (status == TRUE))
  {
    QCRIL_LOG_DEBUG("registered on WLAN when WFC is off, maybe VT offloading...");
  }

  QCRIL_LOG_DEBUG( "IMS registered over WLAN STATUS %d", status);
  return status;
}
...
bool qcril_qmi_nas_handle_ims_pdp_status_change(bool isPdpConnected)
{
  sIsImsPdpConnected = isPdpConnected;
  QCRIL_LOG_INFO(".. isPdpConnected = %s", isPdpConnected ? "true" : "false");
  if (sIsImsPdpConnected == false)
  {
    qcril_qmi_nas_cancel_ims_shutdown_timer();
