   ...
   jpege_engine_dst_t *p_dest)
{
    ...
    {
       //default to 4 segments
       default_piece_height = luma_height/4;
    }
    ...
    if (p_hybrid->output_buf_received_cnt == p_hybrid->output_buf_cnt)
    {
        ...
        while (i < p_hybrid->output_buf_cnt)
        {
            ...
            while (bytes_remaining)
            {
                ...
                // Also reserve 2 bytes for the EOI marker
                bytes_to_copy = STD_MIN(bytes_remaining,
                  (p_dest_buf->size - p_dest_buf->offset - 2));
                ...
                // Take into consideration the 2 bytes reserved for EOI marker.
                if (i == (p_hybrid->output_buf_cnt-1) ||
                  p_dest_buf->offset == (p_dest_buf->size - 2))
                {
