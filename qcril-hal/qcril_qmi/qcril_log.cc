...
#ifdef QMI_RIL_UTF
char __thread log_buf_raw[ QCRIL_MAX_LOG_MSG_SIZE ];
#endif
qtimutex::QtiSharedMutex log_lock_mutex;
qtimutex::QtiSharedMutex log_timer_mutex;
...
static struct inter_rild_info_type
{
  ...
  qtimutex::QtiSharedMutex send_lock_mutex;
}inter_rild_info;
...
int qcril_ipc_evaluate_rilds_socket_paths(char *rild_socket_name)
{
    ...
    inter_rild_info.other_rilds_addr_info = qcril_malloc2(
        inter_rild_info.other_rilds_addr_info,
        inter_rild_info.other_rilds_addr_info_len);
    if(inter_rild_info.other_rilds_addr_info)
    {
        for(iter_i = 0; iter_i < num_of_rilds; iter_i++)
        {
            ...
            else
            {
                ...
                inter_rild_info.other_rilds_addr_info[iter_other_rilds_addr_info].
                    rild_instance_id = static_cast<qcril_instance_id_e_type>(iter_i);
...
int qcril_multiple_rild_ipc_send_func(ipc_message_id_type message_id, void * payload, int payload_length, int dest_rild_instance_id) //generic send function
{
    ...
    if(inter_rild_info.info_valid)
    {
          inter_rild_info.send_lock_mutex.lock();
          if(inter_rild_info.other_rilds_addr_info)
          {
          ...
          inter_rild_info.send_lock_mutex.unlock();
    }
    return 0;
}
...
*/
/*=========================================================================*/
void qcril_log_print_RIL_SignalStrength (const char* header,
                                         const RIL_SignalStrength* ptr)
{
...
*/
/*=========================================================================*/
void qcril_log_print_RIL_CDMA_SignalInfoRecord(
        const char* header,
        const RIL_CDMA_SignalInfoRecord* ptr)
{
...
*/
/*=========================================================================*/
void qcril_log_print_RIL_IMS_SMS_Message(const char* header,
                                         const RIL_IMS_SMS_Message* ptr)
{
  ...
  if(cellIdentity) {
    ...
    switch(cellIdentity->cellInfoType) {
      case RIL_CELL_INFO_TYPE_GSM:
        qcril_log_print_ril_message_payload("%scellIdentity.mcc  = %s",
                                                header, cellIdentity->cellIdentityGsm.mcc);
        qcril_log_print_ril_message_payload("%scellIdentity.mnc  = %s",
                                  ...
                                  size_t datalen, RIL_Errno error)
{
  ...
  qcril_log_print_ril_message_payload("Message ID: %d, Message Type: %d, Data Length: %u, Error: %d",
                               qcril_event_get_android_request(message_id),
                               message_type,
                               (unsigned int)datalen, error);
