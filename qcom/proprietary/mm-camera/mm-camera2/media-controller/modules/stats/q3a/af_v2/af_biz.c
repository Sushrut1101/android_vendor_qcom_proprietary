...
static void af_biz_util_cur_pos_after_lens_move(af_biz_internal *af) {
  ...
  if(af->af_out.move_lens.reset_lens == TRUE){
    /*When we reset lens position, we fall to far end
    This is an understanding between sensor and AF*/
    af->af_input.cur_lens_pos = info->position_far_end;
  } else {
...
void af_biz_util_update_output_data(af_biz_internal *af,
  ...
  case AF_OUTPUT_MOVE_LENS:
  {
    ...
    Q3A_MEMCPY(&af->af_out.move_lens, (af_move_lens_t *)data,
  ...
  case AF_OUTPUT_STATUS:
    ...
    Q3A_MEMCPY(&af->af_out.focus_status, (af_status_t *)data,
     sizeof(af_status_t));
    break;

  case AF_OUTPUT_ROI_INFO: // not used
    af->af_out.type = (af_output_type)(AF_OUTPUT_ROI_INFO | af->af_out.type);
    Q3A_MEMCPY(&af->af_out.roi_info, (af_roi_info_t *)data,
  ...
  case AF_OUTPUT_STATS_CONFIG:
    ...
    Q3A_MEMCPY(&af->af_out.af_stats_config, (af_config_t *)data,
      sizeof(af_config_t));
    break;
  case AF_OUTPUT_PDAF_CONFIG:
    af->af_out.type = (af_output_type)(AF_OUTPUT_PDAF_CONFIG | af->af_out.type);
    Q3A_MEMCPY(&af->af_out.pdaf_config, (af_core_pdaf_config_data_t *)data,
  ...
  case AF_OUTPUT_DCIAF_CONFIG:
    ...
    Q3A_MEMCPY(&af->af_out.dciaf_config, (af_core_dciaf_config_data_t *)data,
  ...
  case AF_OUTPUT_EZ_METADATA:
    ...
    Q3A_MEMCPY(&af->af_out.eztune, (af_eztune_t *)data,
      sizeof(af_eztune_t));
    break;

  case AF_OUTPUT_SWAF_CONFIG: {
    int k = 0;
    Q3A_MEMCPY(&af->af_out.swaf_config, (swaf_config_data_t *)data, sizeof(swaf_config_data_t));
    af->af_out.type = (af_output_type)(AF_OUTPUT_SWAF_CONFIG | af->af_out.type);
    break;
  }

  case AF_OUTPUT_EXP_COMPENSATE:
    af->af_out.type = (af_output_type)(AF_OUTPUT_EXP_COMPENSATE | af->af_out.type);
    af->af_out.exp_comp_enable = *(boolean*)data;
    break;
...
static void af_biz_util_update_roi_output_data(af_biz_internal *af,
  ...
  af->cur_roi_info = af->af_out.roi_info;
}/*af_biz_util_update_roi_output_data*/
...
void af_biz_util_pack_output(af_biz_internal *af,
  ...
  Q3A_MEMCPY(af_out, &af->af_out, sizeof(af_output_data_t));
} /* af_biz_util_pack_output */
 ...
 **/
float af_biz_get_fovc_factor(af_biz_internal *af, int target_pos)
{
  ...
  property_get("persist.vendor.camera.fovc.fudge", prop, "0"); \
  ...
  lens_shift_mm = (slope * ((target_pos - lens_pos_anchor2)) + lens_shift_anchor2);
  factor = ((lens_shift_mm) / focal_length_mm) * fudge_factor;

  lens_shift_mm_max =
      (slope * ((af->af_mode.near_end - lens_pos_anchor2)) + lens_shift_anchor2);
  max_factor = ((lens_shift_mm_max) / focal_length_mm) * fudge_factor;

  if (factor < 0) {
    final_factor = max_factor;
  } else {
    final_factor = max_factor - factor;
  }

  AF_MSG_LOW("target_pos %d focal_length: %f lens_pos_anchor_1: %d lens_pos_anchor_2: %d "
    "anchor1_distance: %f mm anchor2_distance: %f mm fudge_factor: %f "
    "lens_shift_anchor1 %f lens_shift_anchor2 %f slope %f! final_factor %f max_factor: %f",
    target_pos, focal_length_mm, lens_pos_anchor1, lens_pos_anchor2,
    anchor1_distance_mm, anchor2_distance_mm, fudge_factor,
    lens_shift_anchor1, lens_shift_anchor2, slope, final_factor, max_factor);
...
static int af_biz_reset_lens(af_biz_internal *af) {
  ...
  Q3A_MEMSET(&reset_form, 0, sizeof(af_move_lens_t));
  ...
  if(af->is_fovc_enabled){
    ...
    reset_form.mag_factor = af_biz_get_fovc_factor(af, af->af_mode.far_end);
    AF_MSG_LOW("fov comp is enabled - factor: %f", reset_form.mag_factor);
  } else {
    reset_form.fovc_enabled = FALSE;
    reset_form.mag_factor = 0.0f;
    AF_MSG_LOW("fov comp is disabled");
  }
...
static int af_biz_move_lens_to(af_biz_internal *af, int pos) {
  ...
  if (steps != 0) {
    ...
    Q3A_MEMSET(&move_lens, 0, sizeof(af_move_lens_t));
    ...
    if (af->is_fovc_enabled) {
      move_lens.fovc_enabled = TRUE;
      move_lens.mag_factor = af_biz_get_fovc_factor(af, pos);
    } else {
      move_lens.fovc_enabled = FALSE;
      move_lens.mag_factor = 0.0f;
    }
...
static int af_biz_focus_mode_internal(af_biz_internal *af,
  ...
  if (!rc) {
    ...
    if (af->af_mode_changed == TRUE) {
      ...
      if (!IS_CAF_ENABLED(af_mode)) {
        if (af_mode == AF_MODE_INFINITY || af_mode == AF_MODE_MACRO) {
          ...
          if(af->sof_id != 0) {
            AF_HIGH(" Infinity or Macro mode, move lens to def pos");
  ...
  Q3A_MEMCPY(&set_param.u.set_focus.focus_mode, &af->af_mode,
...
static void af_biz_map_roi(af_biz_internal *af,
  ...
  af_core_roi_type input_type = AF_CORE_ROI_TYPE_FIXED;
  if ( AF_ROI_TYPE_GENERAL == roi_info->type)
  {
      input_type = AF_CORE_ROI_TYPE_FIXED;
  }
  else if ( AF_ROI_TYPE_TOUCH == roi_info->type)
  {
      input_type = AF_CORE_ROI_TYPE_TOUCH;
  }
  else if ( AF_ROI_TYPE_FACE == roi_info->type)
  {
      input_type = AF_CORE_ROI_TYPE_FACE;
  }

  if (af->af_input.roi_info.roi_type == input_type) {

    for (i = 0;
      i < (int)roi_info->num_roi && i < AF_MAX_ROI_COUNT; i++) {
      if ((af->af_input.roi_info.roi_cordinates.roi[i].x != roi_info->roi[i].x) ||
    ...
    if ((i == (int)roi_info->num_roi) || (i == AF_MAX_ROI_COUNT)) {
      AF_LOW("Same ROI received, no need to reconfigure");
      return ;
    }
 }

  af_core_config_roi_type core_roi_info;
  Q3A_MEMSET(&core_roi_info, 0, sizeof(af_core_config_roi_type));
  if (roi_info->type == AF_ROI_TYPE_FACE) {
    core_roi_info.roi_type = AF_CORE_ROI_TYPE_FACE;
  } else if (roi_info->type == AF_ROI_TYPE_TOUCH) {
    core_roi_info.roi_type = AF_CORE_ROI_TYPE_TOUCH;
  } else if (roi_info->type == AF_ROI_TYPE_GENERAL) {
    core_roi_info.roi_type = AF_CORE_ROI_TYPE_FIXED;
  } else {
    core_roi_info.roi_type = AF_CORE_ROI_TYPE_FIXED;
  }
  core_roi_info.roi_cordinates.num_of_roi = roi_info->num_roi;

  AF_LOW(" type: %d num_roi: %d",
      roi_info->type, roi_info->num_roi);
  for (i = 0;
      i < (int)roi_info->num_roi && i < AF_MAX_ROI_COUNT; i++) {
    core_roi_info.roi_cordinates.roi[i].x = roi_info->roi[i].x;
    core_roi_info.roi_cordinates.roi[i].y = roi_info->roi[i].y;
    core_roi_info.roi_cordinates.roi[i].width = roi_info->roi[i].dx;
    core_roi_info.roi_cordinates.roi[i].height = roi_info->roi[i].dy;
    core_roi_info.weight[i] = roi_info->weight[i];
  ...
  if (TRUE == af->af_input.roi_info.roi_updated) {
    /**
     * af_biz_map_roi may be called serval times at one frame.
     * we should make sure the touch focus cmd has a highest priority
     **/
    if (core_roi_info.roi_type != AF_CORE_ROI_TYPE_TOUCH
        && af->af_input.roi_info.roi_type == AF_CORE_ROI_TYPE_TOUCH) {
      AF_LOW("TOUCH roi info should not be reset!");
    } else {
      af->af_input.roi_info = core_roi_info;
...
/** af_biz_map_aec_manual_info: update aec info in input sturcture
 *
 *  @af: internal AF data structure
 *
 ...
 *  Return: 0 - Success <1 - Failure
 **/
static int af_biz_map_aec_manual_info(af_biz_internal *af,
...
static int af_biz_map_aec_info(af_biz_internal *af,
  ...
  af->af_input.aec_info.total_drc_gain = aec_info->total_drc_gain;
...
static int af_biz_map_sensor_info(af_biz_internal *af,
  ...
  Q3A_MEMCPY(&af->sensor_input, sensor_info,
  ...
  af->af_input.img_sensor_info.frame_time =
    sensor_info->frame_time;

  //  some param not passed in core
  return rc;
}

static int af_biz_map_pickup_gyro_info(af_biz_internal *af, stats_af_t *stats) {
  af_roi_info_t* roi_info;
  uint32_t sensor_res_width, sensor_res_height;
  uint64_t baf;
  uint64_t roi_start_exp, roi_end_exp;
  float exposure_time;
  uint32_t frame_time;
  float ratio;
  uint64_t ts;
  ...
  // get information
  sensor_res_width = af->sensor_input.sensor_res_width;
  sensor_res_height = af->sensor_input.sensor_res_height;
  roi_info = &af->cur_roi_info;

  exposure_time = af->af_input.aec_info.cur_exp_time;
  frame_time = af->af_input.img_sensor_info.frame_time;

  baf = ((uint64_t)stats->time_stamp.time_stamp_sec) * 1000000 + ((uint64_t)stats->time_stamp.time_stamp_us);

  // calculate roi exposure duration.
  ratio = ((float)roi_info->roi[0].dy)/(float)sensor_res_height;

  roi_start_exp = baf - (uint64_t)(ratio*frame_time) - (uint64_t)(exposure_time*USEC_PER_SEC);
  roi_end_exp = baf;

  // reset gyro information for af_biz_process
  Q3A_MEMSET(&af->af_input.motion_sensor_info.gyro_info, 0, sizeof(af_core_input_gyro_type));

  // find first gyro sample in exposure duration
  idx = (af->gyro_history_idx - 1 + GYRO_HISTORY_LENGTH) % GYRO_HISTORY_LENGTH;
  ts = af->gyro_history[idx].timestamp;
  while (idx != af->gyro_history_idx && roi_start_exp < ts) {
    idx = (idx - 1 + GYRO_HISTORY_LENGTH) % GYRO_HISTORY_LENGTH;
    ts = af->gyro_history[idx].timestamp;
  }
  idx = (idx + 1) % GYRO_HISTORY_LENGTH;
  ts = af->gyro_history[idx].timestamp;

  // find all gyro samples in exposure duration.
  for (i=0; idx != af->gyro_history_idx && ts < roi_end_exp; i++) {
    af->af_input.motion_sensor_info.gyro_info.angular_velocity_x[i]
        = ((float)af->gyro_history[idx].value[0]) / (1 << 16);
    af->af_input.motion_sensor_info.gyro_info.angular_velocity_y[i]
        = ((float)af->gyro_history[idx].value[1]) / (1 << 16);
    af->af_input.motion_sensor_info.gyro_info.angular_velocity_z[i]
        = ((float)af->gyro_history[idx].value[2]) / (1 << 16);

    idx = (idx + 1) % GYRO_HISTORY_LENGTH;
    ts = af->gyro_history[idx].timestamp;
  }

  // setup other gyro related information.
  if (i>0) {
    af->af_input.motion_sensor_info.gyro_info.enabled = true;
    af->af_input.motion_sensor_info.gyro_info.float_ready = true;
    af->af_input.motion_sensor_info.gyro_info.sample_index = i;
  } else {
    // take latest gyro sample if no gyro sample in exposure duration.
    idx = (af->gyro_history_idx - 1 + GYRO_HISTORY_LENGTH) % GYRO_HISTORY_LENGTH;
    ts = af->gyro_history[idx].timestamp;

    af->af_input.motion_sensor_info.gyro_info.angular_velocity_x[0]
        = ((float)af->gyro_history[idx].value[0]) / (1 << 16);
    af->af_input.motion_sensor_info.gyro_info.angular_velocity_y[0]
        = ((float)af->gyro_history[idx].value[1]) / (1 << 16);
    af->af_input.motion_sensor_info.gyro_info.angular_velocity_z[0]
        = ((float)af->gyro_history[idx].value[2]) / (1 << 16);

    af->af_input.motion_sensor_info.gyro_info.sample_index = 1;
    af->af_input.motion_sensor_info.gyro_info.enabled = true;
    af->af_input.motion_sensor_info.gyro_info.float_ready = true;
  }

  AF_LOW("Samples %d", af->af_input.motion_sensor_info.gyro_info.sample_index);
  for (i=0; i<af->af_input.motion_sensor_info.gyro_info.sample_index; i++) {
    AF_LOW("Sample %d: gyro %f, %f, %f",
        i,
        af->af_input.motion_sensor_info.gyro_info.angular_velocity_x[i],
        af->af_input.motion_sensor_info.gyro_info.angular_velocity_y[i],
        af->af_input.motion_sensor_info.gyro_info.angular_velocity_z[i]);
...
/** af_biz_map_gyro_info: update gyro info in input sturcture
 *
 *  @af: internal AF data structure
 *
 *  @gyro_info: gyro input send by aec
 *
 *  Return: 0 - Success <1 - Failure
 **/
static int af_biz_map_gyro_info(af_biz_internal *af,  af_input_from_gyro_t *gyro_info) {
  int rc = 0;
  int i;

  af->af_input.motion_sensor_info.gyro_info.enabled = gyro_info->float_ready;
  af->af_input.motion_sensor_info.gyro_info.sample_index = 1;
  af->af_input.motion_sensor_info.gyro_info.angular_velocity_x[0] = gyro_info->flt[0];
  af->af_input.motion_sensor_info.gyro_info.angular_velocity_y[0] = gyro_info->flt[1];
  af->af_input.motion_sensor_info.gyro_info.angular_velocity_z[0] = gyro_info->flt[2];

  for(i=0; i<gyro_info->sample_len; i++) {
    af->gyro_history[af->gyro_history_idx] = gyro_info->sample[i];
    af->gyro_history_idx = (af->gyro_history_idx + 1) % GYRO_HISTORY_LENGTH;
  }

  return rc;
}

/** af_biz_reset_gyro_info: reset gyro sample info * *
     @af: internal AF data structure * *
     Return: 0 - Success <1 - Failure
 **/
static int af_biz_reset_gyro_info(af_biz_internal *af) {
  int rc = 0;
  AF_HIGH("E");
  af->af_input.motion_sensor_info.gyro_info.enabled = FALSE;
  af->af_input.motion_sensor_info.gyro_info.sample_index = 0;
  return rc;
}
...
static int af_biz_map_pdaf_info(af_biz_internal *af,
  ...
  if (TRUE == af->af_input.stats_info.depth_stats.pdaf_info.is_peripheral_valid) {
    ...
    memset(af->af_input.stats_info.depth_stats.pdaf_info.peripheral, 0 ,sizeof(af->af_input.stats_info.depth_stats.pdaf_info.peripheral));
  }
  af->af_input.stats_info.depth_stats.pdaf_info.is_single_valid =
    depth_service->input.u.pdaf_info.is_single_valid;
  if (TRUE == af->af_input.stats_info.depth_stats.pdaf_info.is_single_valid) {
    for (i = 0; i < MAX_SINGLE_WINDOWS; i++) {
      af->af_input.stats_info.depth_stats.pdaf_info.single[i].defocus =
        depth_service->input.u.pdaf_info.single[i].defocus;
      af->af_input.stats_info.depth_stats.pdaf_info.single[i].df_conf_level =
        depth_service->input.u.pdaf_info.single[i].df_conf_level;
      af->af_input.stats_info.depth_stats.pdaf_info.single[i].phase_diff =
        depth_service->input.u.pdaf_info.single[i].phase_diff;
    }
  } else {
    memset(af->af_input.stats_info.depth_stats.pdaf_info.single, 0 ,sizeof(af->af_input.stats_info.depth_stats.pdaf_info.single));
  }
...
static int af_biz_map_tof_info(af_biz_internal *af,
  ...
  af->af_input.stats_info.depth_stats.tof_info.max_distance =
   ...
   Q3A_MEMCPY(&(af->af_input.stats_info.depth_stats.tof_info.daaf_info.cal_data),
...
static int af_biz_map_dciaf_info(af_biz_internal *af,
  ...
  Q3A_MEMCPY(&(af->af_input.stats_info.depth_stats.dciaf_info.daaf_info.cal_data),
       &(depth_service->input.cal_data), sizeof(depth_cal_data_t));
  return rc;
}
...
static int af_biz_map_stream_crop_info(af_biz_internal *af,
  ...
  if(memcmp(&af->af_stream_crop, stream_crop, sizeof(af_stream_crop_t))) {
    ...
    Q3A_MEMCPY(&af->af_stream_crop, stream_crop, sizeof(af_stream_crop_t));
    af->af_input.roi_info.roi_updated = TRUE;
  }
  // calculate zoom
...
static int af_biz_map_isp_capability(af_biz_internal *af,
  ...
  af->stats_depth = stats_caps_info->stats_depth;
...
void af_biz_initialize_internal_data(af_biz_internal *af) {
  ...
  af->af_ctrl_mode = 1;
  af->hfr_mode = 0;
  Q3A_MEMSET(&af->af_mode, 0, sizeof(af_mode_info_t));
...
void af_biz_handle_stats_config(af_biz_internal *af,
  ...
  if (core_stats_info->mask & AF_CORE_STATS_MASK_BAYER) {
    Q3A_MEMSET(&af->stats_config, 0 , sizeof(af_config_t));
  ...
  if (core_stats_info->mask & AF_CORE_STATS_MASK_PREVIEW_ASSISTED) {
    Q3A_MEMSET(&sw_stats_config, 0 , sizeof(swaf_config_data_t));
...
void af_biz_handle_core_output(af_biz_internal *af) {
  ...
  if (core_out->mask & AF_CORE_OUTPUT_EXP_COMPENSATE) {
    af_biz_util_update_output_data(af, AF_OUTPUT_EXP_COMPENSATE,
      &core_out->af_exp_compensate);
  }
  ...
  Q3A_MEMSET(&af->af_core_out, 0, sizeof(af_core_output_params_type));
} /* af_biz_handle_core_output */
...
static void af_biz_map_bg_stats(af_biz_internal* af,
  q3a_core_bg_stats_type *core_bg_stats, const q3a_bg_stats_t* q3a_bg_stats)
{
  if (!af || !core_bg_stats || !q3a_bg_stats) {
    AF_ERR("Invalid Parameter");
    return;
  }
  ...
  core_bg_stats->bit_depth = af->stats_depth;
...
boolean af_biz_set_parameters(af_set_parameter_t *param,
  ...
  case AF_SET_PARAM_INIT_CHROMATIX_PTR: {
    ...
    Q3A_MEMCPY(&af->af_input.extended_tuning_data,
    ...
    Q3A_MEMCPY(&set_param.u.tuning_load.extended_tuning_info,
      &param->u.af_init_param.core_tuning_params,
      sizeof(af_core_tuning_params_t));
    af->af_algo_ops.set_parameters(af->af_core, &set_param, &af->af_core_out);
    need_callback = TRUE;
  ...
  case AF_SET_PARAM_RUN_MODE: {
    ...
    /* When we move from camera to camcorder or vice versa,
           ...
           move after switch */
    if(af->is_fovc_enabled){
      Q3A_MEMSET(&fovc_form, 0, sizeof(af_move_lens_t));
  ...
  case AF_SET_PARAM_WAIT_FOR_AEC_EST:
    set_param.param_type = AF_CORE_SET_PARAM_WAIT_FOR_AEC_EST;
    af->wait_for_aec_est = param->u.af_wait_for_aec_est;
    set_param.u.af_led_assist = !(param->u.af_wait_for_aec_est);
    af->af_algo_ops.set_parameters(af->af_core, &set_param, &af->af_core_out);
  ...
  case AF_SET_PARAM_UPDATE_AEC_INFO:
    ...
    break;
  case AF_SET_PARAM_AEC_MANUAL_UPDATE:
    af_biz_map_aec_manual_info(af, &param->u.aec_info);
    break;
  ...
  case AF_SET_PARAM_FD_ENABLED: {
    AF_LOW("AF_SET_PARAM_FD_ENABLED %d", param->u.fd_enabled);
    set_param.u.fd_enabled = param->u.fd_enabled;
    set_param.param_type = AF_CORE_SET_PARAM_FD_ENABLE;
    af->af_algo_ops.set_parameters(af->af_core, &set_param, &af->af_core_out);
    if (FALSE == set_param.u.fd_enabled) {
      af_roi_info_t default_roi;
      Q3A_MEMSET(&default_roi, 0, sizeof(af_roi_info_t));
      default_roi.type = AF_ROI_TYPE_GENERAL;
      default_roi.frm_id = af->frame_id;
      default_roi.num_roi = 1;
      af->af_input.face_detected = FALSE;
      af->isp_reconfig_needed = TRUE;
      af_biz_map_roi(af, &default_roi);
    }
  }
    break;
...
// Dont need it any more.
  ...
  case AF_SET_PARAM_AF_HJ_ENABLE: {
    AF_LOW("af biz AF_SET_PARAM_AF_HJ_ENABLE");
    af->is_af_hj_enabled = param->u.af_hj_enable;
    set_param.param_type = AF_CORE_SET_PARAM_AF_HJ_ENABLE;
    set_param.u.af_hj_enable = af->is_af_hj_enabled;
    af->af_algo_ops.set_parameters(af->af_core, &set_param, &af->af_core_out);
  }
    break;
  case AF_SET_PARAM_SKIP_FINE_SCAN: {
    AF_LOW("af biz AF_SET_PARAM_SKIP_FINE_SCAN");
    af->is_fine_scan_skipped = param->u.skip_fine_scan;
    set_param.param_type = AF_CORE_SET_PARAM_SKIP_FINE_SCAN;
    set_param.u.skip_fine_scan = af->is_fine_scan_skipped;
    af->af_algo_ops.set_parameters(af->af_core, &set_param, &af->af_core_out);
  }
    break;
  case AF_SET_PARAM_BG_STATS: {
    set_param.param_type = AF_CORE_SET_PARAM_BG_STATS;
    af_biz_map_bg_stats(af, &set_param.u.bg_stats, param->u.bg_stats.p_q3a_bg_stats);
    af->af_algo_ops.set_parameters(af->af_core, &set_param, &af->af_core_out);
  }
    break;
  default:
...
boolean af_biz_get_parameters(af_get_parameter_t *param, void *af_internal) {
  ...
  case AF_GET_PARAM_STATS_CONFIG_INFO:
    Q3A_MEMCPY(&param->u.af_stats_config, &af->stats_config,
      sizeof(af_config_t));
    break;
  ...
  uint8_t num_of_outputs, void *af_obj)
{
  ...
  if (af->af_algo_ops.process) {
    is_af_hj_enabled = af->is_af_hj_enabled
        || af->af_input.tuning_info.af_algo.af_haf.haf_extended.caf_hjr_info.caf_HJ_Algorithm_enable
        || af->af_input.tuning_info.af_algo.af_haf.haf_extended.hjr_info.HJ_Algorithm_enable;

    if(is_af_hj_enabled)
      af_biz_map_pickup_gyro_info(af, stats);
