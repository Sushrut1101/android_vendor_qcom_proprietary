 ...
 *
 *  Overview: This file acts as HAL layer and makes ioctls call for fm
 *
 **/
        ...
        if(event_listener_thread == 0) {
           ...
           if(ret == 0) {
              ...
              if(ret == FM_SUCCESS) {
                 ...
                 pthread_mutex_unlock(&mutex_turn_on_cond);
                 ALOGE("Unlocked the FM on cond mutex\n");
                 event_listener_canceled = true;
                 pthread_join(event_listener_thread, NULL);
                 close(fd_driver);
                 fd_driver = -1;
...
)
{
    ...
    if((cur_fm_state != FM_OFF) &&
       ...
       if(ret == FM_SUCCESS) {
          ALOGE("FM get freq is successfull, freq is: %ld\n", freq);
       }else {
...
)
{
    ...
    if((cur_fm_state == FM_ON) &&
        ...
        ret = QcomFmIoctlsInterface::set_freq(fd_driver,
                                             freq);
        if(ret == FM_SUCCESS) {
           ALOGE("FM set frequency command set successfully\n");
           pthread_mutex_lock(&mutex_tune_compl_cond);
           ts = set_time_out(TUNE_EVENT_TIMEOUT);
           pthread_cond_timedwait(&tune_compl_cond, &mutex_tune_compl_cond, &ts);
           pthread_mutex_unlock(&mutex_tune_compl_cond);
        }else {
           if((cur_fm_state != FM_OFF)) {
              set_fm_state(FM_ON);
           }
           ALOGE("FM set freq command failed %d\n", ret);
        }
    }else {
...
)
{
    ...
    if(cur_fm_state == FM_ON) {
        ALOGE("FM seek up started\n");
        set_fm_state(SEEK_IN_PROGRESS);
        tuned_freq = prev_freq;
        ret = QcomFmIoctlsInterface::set_control(fd_driver,
        ...
        pthread_cond_timedwait(&seek_compl_cond, &mutex_seek_compl_cond, &ts);
        pthread_mutex_unlock(&mutex_seek_compl_cond);
        if((cur_fm_state != SEEK_IN_PROGRESS) && !seek_canceled) {
           ALOGE("Seek completed without timeout\n");
           freq = GetChannel();
           if(tuned_freq == freq)
               freq = -1;
        }else {
...
)
{
    ...
    if(cur_fm_state == SEEK_IN_PROGRESS) {
        ...
        if(ret == FM_SUCCESS) {
            ALOGE("FM Seek cancel command set successfully\n");
            seek_canceled = true;
        }else {
...
)
{
    ...
    if(cur_fm_state == FM_ON) {
        ...
        ALOGE("Wait for Scan Timeout or scan complete");
        pthread_cond_timedwait(&scan_compl_cond, &mutex_scan_compl_cond, &ts);
        ALOGE("Scan complete or timedout");
...
)
{
    ...
    if(cur_fm_state == SCAN_IN_PROGRESS) {
       ret = QcomFmIoctlsInterface::set_control(fd_driver,
                                       V4L2_CID_PRV_SRCHON, 0);
       if(ret == FM_SUCCESS) {
          ALOGE("FM Scan cancel command successfull");
       }else {
...
)
{
    ...
    if((cur_fm_state != FM_OFF) &&
       ...
       ret = QcomFmIoctlsInterface::get_control(fd_driver,
                            V4L2_CID_PRV_SOFT_MUTE, mode);
       if(ret == FM_SUCCESS) {
          ALOGE("FM Get soft mute is successful: %ld\n", mode);
       }else {
...
)
{
    ...
    if((obj_p->cur_fm_state == FM_ON) && obj_p->rds_enabled) {
       ...
       if((raw_rds[RT_OR_ERT_IND] == 0) &&
          ...
          ALOGE("Item toggel bit: %d, Item running bit: %d\n",
          ...
          for(i = 0; i < raw_rds[RT_PLUS_TAGS_NUM_IND]; i++) {
              ...
              ALOGE("TagCode: %d, TagStartPos: %d, Taglen: %d\n",
...
{
    int bytesread;
    char event_buff[STD_BUF_SIZE] = {0};
    bool status = true;
    FmRadioController_qcom *obj_p = static_cast<FmRadioController_qcom*>(arg);
...
)
{
     ...
     ALOGE("FM handle Tune event\n");
     freq = GetChannel();
     switch(cur_fm_state) {
         case FM_ON:
            if(af_enabled && (freq != prev_freq)
                && (prev_freq > 0)) {
               ALOGE("AF jump happened\n");
            }
            break;
...
{
     ALOGE("FM handle seek next event\n");
}
...
{
     ALOGE("FM handle seek complete event\n");
     switch(cur_fm_state) {
         case SEEK_IN_PROGRESS:
            break;
         case SCAN_IN_PROGRESS:
...
)
{
     ...
     if(cur_fm_state == FM_OFF_IN_PROGRESS) {
        ALOGE("Expected disabled event\n");
     }else {//Enexpected disabled
        ALOGE("Unexpected disabled event\n");
     }
...
)
{
    ...
    freq = GetChannel();
    ALOGE("FM handle AF Jumped event\n");
    if(af_enabled && (freq != prev_freq)) {
       ALOGE("AF Jump occured, prevfreq is: %ld, af freq is: %ld\n", prev_freq, freq);
    }
    prev_freq = freq;
}
