...
public class WigigActivity extends PreferenceActivity {
    ...
    private SwitchPreference mSensingSwitchPreference;
    ...
    public WigigActivity(int prefid, String[] actions) {
        ...
        if (actions != null && actions.length > 0) {
            for (String a : actions) {
                if (!(a.equals(WigigManager.WIGIG_STATE_CHANGED_ACTION) ||
                        ...
                        a.equals(WigigManager.SENSING_STATE_CHANGED_ACTION))) {
                    mFilter.addAction(a);
                }
            }
        }
        ...
        mFilter.addAction(WigigManager.SENSING_STATE_CHANGED_ACTION);
    ...
    protected void handleEvent(Intent intent) {
        ...
        else if (action.equals(WigigManager.SENSING_STATE_CHANGED_ACTION)) {
            int newState = intent.getIntExtra(WigigManager.EXTRA_SENSING_STATE,
                    WigigManager.SENSING_STATE_UNKNOWN);
            int prevState = intent.getIntExtra(WigigManager.EXTRA_PREVIOUS_SENSING_STATE,
                    WigigManager.SENSING_STATE_UNKNOWN);
            Log.i(TAG, "SENSING_STATE_CHANGED_ACTION: " + prevState + "=>" + newState);

            if (newState == WigigManager.SENSING_STATE_ENABLED) {
                onSensingFrameworkEnabled();
            } else if ((newState == WigigManager.SENSING_STATE_DISABLED) ||
                       (newState == WigigManager.SENSING_STATE_FAILED) ||
                       (newState == WigigManager.SENSING_STATE_UNKNOWN)) {
                onSensingFrameworkDisabled();
            }
        }
    ...
    protected void onWigigSwitchedOn() {
        ...
        mModePreference.setEnabled(!isSensingEnabled());
    }
    ...
    protected void onSensingSwitchedOn() {
        Log.d(TAG, "onSensingSwitchedOn()");
        mModePreference.setEnabled(false);
        setSensingEnabled(true);
    }

    protected void onSensingSwitchedOff() {
        Log.d(TAG, "onSensingSwitchedOff()");
        if (isWigigNetEnabled()) {
    ...
    protected void onSensingFrameworkEnabled() {
        Log.d(TAG, "onSensingFrameworkEnabled()");
        setSensingSwitch(true);
    }

    protected void onSensingFrameworkDisabled() {
        Log.d(TAG, "onSensingFrameworkDisabled()");
        setSensingSwitch(false);
    }
    ...
    protected boolean isSensingEnabled() {
        return mWigigManager != null && mWigigManager.isSensingEnabled();
    }
    ...
    protected boolean isWigigNetEnabled() {
        return mWigigManager != null && (mWigigManager.isWigigEnabled() ||
            mWigigManager.isWigigApEnabled() || mWigigManager.isRateUpgradeEnabled());
    }
    ...
    public boolean setSensingEnabled(boolean enabled) {
        if (mWigigManager != null) {
            boolean ret = mWigigManager.setSensingEnabled(enabled);
            if (!ret) {
                Toast.makeText(this, "Could not" + (enabled ? "enable" : "disable") + "sensing!", Toast.LENGTH_SHORT).show();
                Log.e(TAG, "Could not" + (enabled ? "enable" : "disable") + "sensing!");
            }
            return ret;
        }
        return false;
    ...
    protected void setWigigSwitch(boolean state) {
        boolean modeState = state && !isSensingEnabled();

        Log.d(TAG, "setWigigSwitch() " + state);
        mSwitchPreference.setChecked(state);
        mModePreference.setEnabled(modeState);
    }

    protected void setSensingSwitch(boolean state) {
        boolean modeState = !state && isWigigNetEnabled();

        Log.d(TAG, "setSensingSwitch() " + state);
        if (mSensingSwitchPreference != null) {
           mSensingSwitchPreference.setChecked(state);
        }
    ...
    private Preference.OnPreferenceChangeListener sSensingSwitchListener = new Preference.OnPreferenceChangeListener() {
        @Override
        public boolean onPreferenceChange(Preference preference, Object value) {
            String stringValue = value.toString();

            if (preference instanceof SwitchPreference) {
                Log.d(TAG, "Sensing Switch changed to " + stringValue);
                if (mWigigManager != null) {
                    if (stringValue.equals("true")) {
                        onSensingSwitchedOn();
                    } else {
                        onSensingSwitchedOff();
                    }
                }
            }
            return true;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        */
        ...
        if (mPrefResourceId != NO_RESOURCES_TO_LOAD) {
            ...
            /* Handle sensing switch */
            mSensingSwitchPreference = (SwitchPreference) findPreference("enable_sensing_switch");
            if (mSensingSwitchPreference != null) {
                mSensingSwitchPreference.setOnPreferenceChangeListener(sSensingSwitchListener);
            }
        }
    }
