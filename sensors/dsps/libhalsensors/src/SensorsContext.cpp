...
#define SNS_LOW_LAT_TIMESTAMP_OFFSET_SUID 0xaedf2240d32b403a
#define SNS_LOW_LAT_STOP_ALL_SENSORS_SUID 0x9cca14049af5f2dc
...
//       for the sensor to stream at another nearby rate.
#define SNS_DIRECT_RATE_NORMAL_SAMPLE_PERIOD_US    20000U // 50 Hz
#define SNS_DIRECT_RATE_FAST_SAMPLE_PERIOD_US       5000U // 200 Hz
#define SNS_DIRECT_RATE_VERY_FAST_SAMPLE_PERIOD_US  1250U // 800 Hz
#endif
...
#endif
{
    ...
    pthread_cond_init(&(sensor_info_sensor1_cb->cb_cond), &cond_attr);
...
#ifdef SNS_DIRECT_REPORT_SUPPORT
    /* Initialize the Direct Report related objects */
    pthread_mutex_init(&open_direct_channels_mutex, NULL);
    open_direct_channels = std::list<DirectChannel*>();
#endif
}
...
SensorsContext::~SensorsContext()
{
    ...
    // TODO: CLOSE THE GRALLOC MODULE, IF AVAILABLE
    // TODO: CLEAN UP OTHER ITEMS FROM THE DIRECT REPORT CHANNELS
    // Go through the direct report channels and clean them up
}
...
int SensorsContext::activate(int handle, int en)
{
    ...
    if (en) {
        ...
        active_sensors.set(handle);
    }
    else {
        active_sensors.set(handle, false);
        if(active_sensors.none() && 1 == time_service->getTimeSyncServiceStatus()) {
            HAL_LOG_VERBOSE("All sensors stop, stop the time_service.");
            time_service->timeServiceStop();
        }
    }
    pthread_mutex_unlock(&active_mutex);
    HAL_LOG_VERBOSE("activate sensors is %zu", active_sensors.count());
                             ...
                             int64_t period_ns, int64_t timeout)
{
    int err;
    if (true == mSensors[handle]->getAttribOK()) {
         if(strcmp(ACCEL_SENSOR_NAME,mSensors[handle]->getName())== 0 &&
            period_ns < HZ_TO_NSEC(mSensors[handle]->getMaxFreqBkp())){
            period_ns = HZ_TO_NSEC(mSensors[handle]->getMaxFreqBkp());
         }
...
void SensorsContext::addSensor(int handle)
{
    ...
    switch(handle){
        ...
        case HANDLE_ACCELERATION_WAKE_UP:
            ...
            break;
        case HANDLE_ACCELERATION_UNCALIBRATED:
        case HANDLE_ACCELERATION_UNCALIBRATED_WAKE_UP:
            if (is_accel_available) {
                HAL_LOG_DEBUG("%s: UNCAL ACCEL will be added later with handle:%d",
                               __FUNCTION__, handle);
            } else {
                HAL_LOG_DEBUG("%s: UNCAL ACCEL disabled!", __FUNCTION__);
            }
            break;
        case HANDLE_GYRO:
...
int SensorsContext::updateSensorList()
{
    ...
    HAL_LOG_INFO("%s", __FUNCTION__);
    pthread_mutex_lock(&sensor_info_sensor1_cb->cb_mutex);
    if (Utility::isAccelAvailable()) {
        /* Add & update uncal Accel and it's wake-up version */
        mSensors[HANDLE_ACCELERATION_UNCALIBRATED] = new AccelerometerUncalibrated(HANDLE_ACCELERATION_UNCALIBRATED);
        if(mSensors[HANDLE_ACCELERATION_UNCALIBRATED] != NULL) {
            (static_cast<SMGRSensor*>(mSensors[HANDLE_ACCELERATION_UNCALIBRATED]))->setSensorInfo(sensor_datatype);
        }
        if (!strncmp("true", wu_sensor, 4)) {
            mSensors[HANDLE_ACCELERATION_UNCALIBRATED_WAKE_UP] = new AccelerometerUncalibrated(HANDLE_ACCELERATION_UNCALIBRATED_WAKE_UP);
            if(mSensors[HANDLE_ACCELERATION_UNCALIBRATED_WAKE_UP] != NULL) {
                (static_cast<SMGRSensor*>(mSensors[HANDLE_ACCELERATION_UNCALIBRATED_WAKE_UP]))->setSensorInfo(sensor_datatype);
            }
        }
    }
            ...
            sns_smgr_single_sensor_info_resp_msg_v01* smgr_resp)
{
    ...
    for (i = 0; i < (int)smgr_resp->SensorInfo.data_type_info_len; i++) {
        ...
        // PEND: A lot of this logic is probably better suited for the SMGRSensor class to handle
        if (handle != -1 && error == false) {
            ...
            if (mSensors[handle] != NULL) {
                ...
                if(strcmp(ACCEL_SENSOR_NAME,
                           mSensors[handle]->getName())==0) {
                           mSensors[handle]->setMaxFreqBkp(sensor_datatype->MaxSampleRate);
                           mSensors[handle]->setMaxFreq(ACCEL_MAX_FREQ);
                }
                ...
                /* This is to avoid setting undefined value to non BMA sensor  */
                if (strcmp(ACCEL_SENSOR_NAME,
                           mSensors[handle]->getName()) != 0) {
                           mSensors[handle]->setMaxFreqBkp(mSensors[handle]->getMaxFreq());
                }
        ...
        if (handle_wakeup != -1 && error == false && !strncmp("true", wu_sensor, 4))
        {
            ...
            if (mSensors[handle_wakeup] != NULL) {
                ...
                /* This is to avoid setting undefined value */
                 if (strcmp(ACCEL_SENSOR_NAME,
                           mSensors[handle_wakeup]->getName()) != 0){
                 mSensors[handle_wakeup]->setMaxFreqBkp(mSensors[handle_wakeup]->getMaxFreq());
                }
...
#ifdef SNS_DIRECT_REPORT_SUPPORT
int SensorsContext::registerDirectChannel(const struct sensors_direct_mem_t* mem)
{
    int ret = 0;
    struct native_handle *mem_handle = NULL;

    HAL_LOG_VERBOSE("%s: enter", __FUNCTION__);
    ...
    if ( (mem->handle == NULL) ||
        this->isUsedBufferFd(mem->handle->data[0]))
    {
      HAL_LOG_ERROR("%s: invalid direct_mem", __FUNCTION__);
      return -EINVAL;
    }
    if (this->is_used_memory(mem->handle)) {
      HAL_LOG_ERROR("%s : memory is in use %p", __FUNCTION__, mem->handle);
      return -EINVAL;
    }
    // Construct a new DirectChannel object
    DirectChannel* channel = new DirectChannel(mem->handle, mem->size);
    ret = channel->init();
    if (ret != 0)
    {
        HAL_LOG_ERROR("%s: channel initialization FAILED ret %d", __FUNCTION__, ret);
        return -EINVAL;
    }

    // Obtain the lock on the collection
    ret = pthread_mutex_lock(&open_direct_channels_mutex);
    if (ret == 0)
    {
        // Add the new DirectChannel object to the list of DirectChannels
        ...
        if (ret != 0)
        {
            HAL_LOG_ERROR("%s: error unlocking mutex ret %d", __FUNCTION__, ret);
        }
    }
    else
    {
        HAL_LOG_ERROR("%s: unable to lock mutex ret %d", __FUNCTION__, ret);
    }

    // If all goes well, return the android handle
    if (ret == 0)
    {
        HAL_LOG_VERBOSE("%s: assigned channel handle %d", __FUNCTION__, channel->getClientChannelHandle());
        return channel->getClientChannelHandle();
    }
    else
    {
        return -1;
...
void SensorsContext::unregisterDirectChannel(int channel_handle)
{
    int ret;
    std::list<DirectChannel*>::const_iterator iter;

    HAL_LOG_VERBOSE("%s: channel_handle %d", __FUNCTION__, channel_handle);
    ...
    if (ret == 0)
    {
        ...
        iter = findDirectChannel(channel_handle);
        ...
        if (iter != open_direct_channels.cend())
        {
            ...
            // completeness-sake.
            ret = sns_low_lat_stream_stop((*iter)->getLowLatHandle());
        ...
        {
            HAL_LOG_ERROR("%s: unable to find channel_handle %d", __FUNCTION__, channel_handle);
        }
        // Release the lock
        ret = pthread_mutex_unlock(&open_direct_channels_mutex);
        if (ret != 0)
...
int SensorsContext::configDirectReport(int sensor_handle, int channel_handle,
    const struct sensors_direct_cfg_t *config)
{
  ...
  if (iter != open_direct_channels.cend())
  {
      sns_low_lat_handle = (*iter)->getLowLatHandle();
  }
  else
  {
      HAL_LOG_ERROR("%s: The channel %d is not available!", __FUNCTION__, channel_handle);
      return -EINVAL;
  }
  ...
  if (-1 == sensor_handle)
  {
      ...
      {
          // Send in the special STOP_ALL_SENSORS SUID to have the sensors core
          // stop all of the sensor streams for this channel without tearing down
          // the channel itself.
          uint64_t stop_all_sensors_suid = SNS_LOW_LAT_STOP_ALL_SENSORS_SUID;
          ret = sns_low_lat_stream_start(sns_low_lat_handle, &stop_all_sensors_suid,
              1, 0, 0);
      }
      else
      {
          HAL_LOG_ERROR("%s: unexpected inputs. sensor_handle %d rate_level %d", __FUNCTION__, sensor_handle, config->rate_level);
          ret = -EINVAL;
      }
  }
  // If the sensor is in a good state
  else if (true == mSensors[sensor_handle]->getAttribOK())
  {
      uint64_t suid;
      uint64_t timestamp_offset[2];
      unsigned int flags = 2; //SNS_LOW_LAT_ANDROID_STYLE_OUTPUT
      int type;

      // Obtain the sensor's SUID
      suid = mSensors[sensor_handle]->getSuid();
      if (0 == suid)
      {
          HAL_LOG_ERROR("%s: Handle %d does not have an SUID for direct report", __FUNCTION__, sensor_handle);
          return -EINVAL;
      }

      // Determine if calibration is needed or not
      type = mSensors[sensor_handle]->getType();
      if ((type == SENSOR_TYPE_MAGNETIC_FIELD) ||
          (type == SENSOR_TYPE_GYROSCOPE))
      {
          flags |= 4; //SNS_LOW_LAT_CALIBRATED_STREAM
      }

      if (config->rate_level == SENSOR_DIRECT_RATE_STOP)
      {
          // Stop the individual stream by calling "start" with a sampling period of 0
          ret = sns_low_lat_stream_start(sns_low_lat_handle, &suid, 1, 0, flags);
          if (ret)
          {
              HAL_LOG_ERROR("%s: Deactivate the handle %d is not successful", __FUNCTION__, sensor_handle);
          }
      }
      // Check to make sure the sensor supports the desired rate
      else if ((mSensors[sensor_handle]->getFlags() & SENSOR_FLAG_MASK_DIRECT_REPORT) >> SENSOR_FLAG_SHIFT_DIRECT_REPORT >= config->rate_level)
      {
          // Convert the rate level to an equivalent sampling period (in usec)
          unsigned int sample_period_us = directRateLevelToSamplePeriod(config->rate_level);
          if (0 == sample_period_us)
          {
              HAL_LOG_ERROR("%s: Unexpected direct report rate %d for handle %d!", __FUNCTION__, config->rate_level, sensor_handle);
              return -EINVAL;
          }

          // Send in the timestamp offset
          timestamp_offset[0] = SNS_LOW_LAT_TIMESTAMP_OFFSET_SUID;
          timestamp_offset[1] = TimeSyncService::getElapsedRealtimeNanoOffset();
          ret = sns_low_lat_stream_start(sns_low_lat_handle, timestamp_offset, 2, sample_period_us, flags);
          if (ret)
          {
              HAL_LOG_ERROR("%s: Sending in timestamp offset was not successful", __FUNCTION__);
          }

          // Start the stream
          ret = sns_low_lat_stream_start(sns_low_lat_handle, &suid, 1, sample_period_us, flags);
          if (ret)
          {
              HAL_LOG_ERROR("%s: Activate the handle %d is not successful", __FUNCTION__, sensor_handle);
          }
          else
          {
              // If everything is successful, then return the sensor's handle
              ret = mSensors[sensor_handle]->getHandle();
          }
      }
      else
      {
          HAL_LOG_ERROR("%s: Unsupported direct report rate %d for handle %d!", __FUNCTION__, config->rate_level, sensor_handle);
          ret = -EINVAL;
      }
  }
  else
  {
      HAL_LOG_ERROR("%s: The sensor handle %d is not available!", __FUNCTION__, sensor_handle);
      ret = -EINVAL;
  }
...
std::list<DirectChannel*>::const_iterator SensorsContext::findDirectChannel(int channel_handle)
{
    for (auto iter = open_direct_channels.cbegin();
        iter != open_direct_channels.cend(); ++iter)
    {
        if ((*iter)->getClientChannelHandle() == channel_handle)
        {
            return iter;
        }
    }
...
bool SensorsContext::isUsedBufferFd(int buffer_fd)
{
    for (auto iter = open_direct_channels.cbegin();
        iter != open_direct_channels.cend(); ++iter)
    {
      HAL_LOG_ERROR("%s: iter %d buffer_fd %d", __FUNCTION__, (*iter)->getBufferFd(), buffer_fd);
        if ((*iter)->getBufferFd() == buffer_fd)
        {
            return true;
        }
    }
...
bool SensorsContext::is_used_memory(const struct native_handle *mem_handle)
{
    int ret = -EINVAL;
    bool found = false;
    ...
    ret = pthread_mutex_lock(&open_direct_channels_mutex);
    if (ret != 0) {
        HAL_LOG_ERROR("%s : error locking mutex ret %d", __FUNCTION__, ret);
    ...
    ret = pthread_mutex_unlock(&open_direct_channels_mutex);
    if (ret != 0) {
        HAL_LOG_ERROR("%s : error unlocking mutex %d", __FUNCTION__, ret);
    }
    return found;
}
unsigned int SensorsContext::directRateLevelToSamplePeriod(int rate_level)
{
    ...
    switch (rate_level)
    {
        ...
        {
            sample_period_us = SNS_DIRECT_RATE_NORMAL_SAMPLE_PERIOD_US;
            break;
        }
        case SENSOR_DIRECT_RATE_FAST:
        {
            sample_period_us = SNS_DIRECT_RATE_FAST_SAMPLE_PERIOD_US;
            break;
        }
        case SENSOR_DIRECT_RATE_VERY_FAST:
        {
            sample_period_us = SNS_DIRECT_RATE_VERY_FAST_SAMPLE_PERIOD_US;
            break;
        }
        default:
        {
            HAL_LOG_ERROR("%s: Unexpected direct report rate %d!", __FUNCTION__, rate_level);
            break;
        }
    }
