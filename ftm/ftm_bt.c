...
#define PINTEST_DEVPATH "/dev/btpower"
...
/* HCI Event buffer */
static uint8 event_buf[HC_VS_MAX_ACL + HCI_ACL_HDR_SIZE + EXT_PIN_CON_LEN];
...
/*===========================================================================
FUNCTION   ftm_bt_hci_hal_vs_and_cc_event

DESCRIPTION
  Processes the VS event and CC event buffer and stores the App version and
  HW version
...
===========================================================================*/
void ftm_bt_hci_hal_vs_and_cc_event(const uint8* pEventBuffer, uint8 nLength)
{
  const uint8 poke_reg_addr[] = {0xFF,0x0B,BT_QSOC_EDL_CMD_CODE,0x01,
              0x34,0x00,0x00,0x8C,0x04};
  /* VS Event */
  if (pEventBuffer[0] == BT_HCI_VND_SPECIFIC_EVT) {
    if (nLength > 3) {
      if ((pEventBuffer[1] > 5)   /* VS Length > 5*/
  ...
  } else if (pEventBuffer[0] == BT_HCI_CMD_CMPLT_EVT) {   /* CC Event */
    if (nLength > 6) {
      if ((pEventBuffer[1] > 8)   /* CC Length > 8*/
          && (pEventBuffer[3] == BT_QSOC_EDL_CMD_OPCODE)
          && (pEventBuffer[5] == FTM_USER_CMD_PASS)
          && (pEventBuffer[6] == BT_QSOC_EDL_APPVER_CMD_SUBOPCODE)) {
        if (NULL != bt_soc_app_version_string) {
          free(bt_soc_app_version_string);
          bt_soc_app_version_string = NULL;
        }
        bt_soc_app_version_string = (uint8 *)malloc(nLength-8);
        if (NULL != bt_soc_app_version_string) {
          memmove(bt_soc_app_version_string,
            &pEventBuffer[8],nLength-8);
        }
      }else if ((nLength > 14)  // make sure we have enough event bytes
              && (!memcmp(pEventBuffer,poke_reg_addr,sizeof(poke_reg_addr)))) {
         bt_soc_hw_version[0] = pEventBuffer[11];
         bt_soc_hw_version[1] = pEventBuffer[12];
         bt_soc_hw_version[2] = pEventBuffer[13];
         bt_soc_hw_version[3] = pEventBuffer[14];
      }
    }
  }
} /* ftm_bt_hci_hal_vs_and_cc_event */
...
boolean ftm_bt_hci_hal_read_event (uint8 * event_buf_ptr)
{
  ...
  */
       ...
       is_slim_bus_test == 1)
  {
      ...
      fd_pintest = open(PINTEST_DEVPATH, O_RDONLY, O_NONBLOCK);
      if(fd_pintest < 0)
          printf("\nfailed to open\n");
...
boolean ftm_bt_hci_qcomm_handle_event ()
{
  ...
  switch(global_state)
  {
    case FTM_SOC_READ_APP_VER:
      /* Read out the CC event or VS event, and then determine
         if we need to read again or not*/
      if (ftm_bt_hci_hal_read_event(event_buf) != TRUE) {
        printf("Failed to read VS event or CC event");
        return FALSE;
      }

      if ( (event_buf[1]  ==  BT_HCI_VND_SPECIFIC_EVT)  /* VS Event */
             && (event_buf[2] > UNIFIED_HCI_VS_MIN_LENGTH) )
      {
        ftm_bt_hci_hal_vs_and_cc_event(&event_buf[1], 2 + event_buf[2]);
        /* Read out the CC event*/
        if (ftm_bt_hci_hal_read_event(event_buf) != TRUE) {
          printf("Failed to read HCI event");
          return FALSE;
        }
      } else if ( (event_buf[1]  ==  BT_HCI_CMD_CMPLT_EVT)  /* CC Event */
                  && (event_buf[2] > UNIFIED_HCI_CC_MIN_LENGTH) ){
        unsigned short int opcode;
        opcode = (event_buf[4] | (event_buf[5] << 8));
        if (BT_QSOC_EDL_CMD_OPCODE == opcode) {
          /* CC event read as first event, VS event not expected,
