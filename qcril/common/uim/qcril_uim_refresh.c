...
--------   ---     ----------------------------------------------------------
03/18/15   yt      Asynchronously read EF-PBR during refresh registration
...
const qmi_uim_file_id_type file_id_table_adf_isim[] =
        {
          ...
          { 0x6F07, { 4, (unsigned char*)qcril_uim_refresh_file_list[QCRIL_UIM_INDEX_ADF] } }    /* EF-IST                  */
        };
...
extern void SecureElementhandleCardStateChange(qmi_uim_card_state_type card_state);
...
)
{
  ...
  /* Allocate original request, it is freed in qmi_uim_callback */
  callback_request_ptr = qcril_uim_allocate_orig_request(QCRIL_DEFAULT_INSTANCE_ID,
                                                         modem_id,
                                                         (RIL_Token)QCRIL_TOKEN_ID_INTERNAL,
  ...
  callback_request_ptr->data.refresh_pbr.dyn_mask = dynamic_mask;
  callback_request_ptr->data.refresh_pbr.record_index = record_index;
  callback_request_ptr->data.refresh_pbr.record_size = record_size;
  callback_request_ptr->data.refresh_pbr.num_records = num_records;
...
)
{
  ...
  QCRIL_LOG_DEBUG( "qcril_uim_refresh_read_pbr_record_resp: token=%d qmi_err_code=0x%X \n",
                    qcril_log_get_token_id((RIL_Token)original_request_ptr->token),
                    params_ptr->qmi_rsp_data.qmi_err_code);

  dynamic_mask = original_request_ptr->data.refresh_pbr.dyn_mask;
  session_type = original_request_ptr->session_type;

  if (params_ptr->qmi_rsp_data.qmi_err_code == 0)
  {
    /* Parse the response & update the EF table */
    res = qcril_uim_read_ef_pbr_parse_resp(&params_ptr->qmi_rsp_data, session_type);
    if (res < 0)
    {
  ...
  if (original_request_ptr->data.refresh_pbr.record_index <
      original_request_ptr->data.refresh_pbr.num_records)
  {
    res = qcril_uim_refresh_read_pbr_record(
            session_type,
            original_request_ptr->data.refresh_pbr.record_index + 1,
            original_request_ptr->data.refresh_pbr.record_size,
            original_request_ptr->data.refresh_pbr.num_records,
            dynamic_mask);
  }
      ...
      original_request_ptr->data.refresh_pbr.num_records)
  {
    ...
    if(qcril_uim_convert_session_type_to_mask(session_type, &reg_mask))
    {
      ...
      qcril_uim_refresh_calculate_files_and_register(session_type, reg_mask);

      if ((qcril_uim_extract_index(&app_index, &slot, session_type) == RIL_E_SUCCESS) &&
          (slot < QMI_UIM_MAX_CARD_COUNT) &&
...
)
{
  ...
  res = qcril_uim_queue_send_request(QCRIL_UIM_REQUEST_GET_RESPONSE,
                                     qcril_uim.qmi_handle,
                                     &get_attr_params,
                                     qmi_uim_callback,
...
)
{
  ...
  QCRIL_LOG_DEBUG( "qcril_uim_refresh_get_pbr_attr_resp: token=%d qmi_err_code=0x%X \n",
                    qcril_log_get_token_id((RIL_Token)original_request_ptr->token),
                    params_ptr->qmi_rsp_data.qmi_err_code);
...
)
{
  ...
  switch (refresh_ind_ptr->refresh_event.refresh_mode)
  {
    ...
    case QMI_UIM_REFRESH_MODE_RESET:
      ...
      {
        SecureElementhandleCardStateChange(QMI_UIM_CARD_STATE_ERROR);
      }
      break;
...
)
{
  ...
  if (refresh_instance_id == qcril_instance_id)
  {
    ...
    if (ril_refresh_resp_ptr->result == SIM_FILE_UPDATE)
    {
    ...
    if (ril_refresh_resp_ptr->result != SIM_RESET &&
        index < QMI_UIM_MAX_APP_PER_CARD_COUNT)
    {
      ril_hook_refresh_resp.app_type = (RIL_AppType) qcril_uim.card_status.card[slot].application[index].app_type;
    }
...
)
{
  ...
  else
  {
    ...
    if ((qcril_uim_extract_index(&app_index, &slot, qmi_session_type) == RIL_E_SUCCESS) &&
        (slot < QMI_UIM_MAX_CARD_COUNT) &&
        (app_index < QMI_UIM_MAX_APP_PER_CARD_COUNT))
    {
      if (qcril_uim.refresh_info.pbr_read_in_progress[slot])
...
)
{
  ...
  /* Next find the RILD instance id */
    ...
    {
      QCRIL_LOG_ERROR("qcril_instance_id lookup failed!\n");
    }
  #else
    QCRIL_LOG_INFO("Using QCRIL_DEFAULT_INSTANCE_ID\n");
  #endif
#else
    ...
    case QMI_UIM_REFRESH_STAGE_START:
      switch (refresh_ind_ptr->refresh_event.refresh_mode)
      {
        ...
        case QMI_UIM_REFRESH_MODE_RESET:
          ...
          memset(&qcril_uim.card_iccid_info, 0x00, sizeof(qcril_uim.card_iccid_info));
