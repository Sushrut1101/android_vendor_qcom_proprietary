     ...
     is_report_time_created(false)
{
    if(getAttribOK() == true) {
        ...
        if (bWakeUp == false) {
            ...
            /* Create timer for Thresh. */
            if (initReportTimer()) {
                HAL_LOG_ERROR("%s:Thresh initReportTimer failed", __FUNCTION__);
            } else {
...
Thresh::~Thresh()
{
    if (is_report_time_created) {
        if (delReportTimer()) {
            HAL_LOG_ERROR(" %s:Thresh report timer delete failed", __FUNCTION__);
        }
    }
}
...
===========================================================================*/
void Thresh::reportSingleEvent(hal_sam_sample_t *curr_sample)
{
    ...
    /* Check sample is not duplicate or not reported before timeout elapsed for non-wakeup sensor
     * Drop it for now and report valid sample after timeout elapsed. */
    if (bWakeUp == false && is_lastsample_reset == true) {
        if (!bComplySampleRate(thresh_last_event.timestamp, android::elapsedRealtimeNano())) {
            float time = HZ_TO_NSEC(freq) - la_sample.timestamp + thresh_last_event.timestamp;
            if (is_report_time_created) {
                if (unsetReportTimer()) {
    ...
    if ((is_lastsample_reset == false) ||
        (is_lastsample_reset == true && la_sample.distance != thresh_last_event.distance)) {
        HAL_LOG_VERBOSE( "%s: prox data: %f %f %f %f SAM TS: %u HAL TS:%" PRId64 " elapsedRealtimeNano:%" PRId64 "",
                         __FUNCTION__, curr_sample->data[0], curr_sample->data[1], la_sample.distance,
                         thresh_last_event.distance, curr_sample->timestamp, la_sample.timestamp, android::elapsedRealtimeNano() );
...
===========================================================================*/
void Thresh::timeOutReporterForLastSample(sigval_t val)
{
    Thresh *thresh_handle = (Thresh *)val.sival_ptr;
    hal_sam_sample_t *curr_sample = &thresh_handle->latest_event;
    thresh_handle->reportSingleEvent(curr_sample);
}
/*============================================================================
  enable
============================================================================*/
...
void Thresh::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        memcpy(&latest_event, curr_sample, sizeof(hal_sam_sample_t));
