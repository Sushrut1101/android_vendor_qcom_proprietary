...
void eseSpiTest(int argc, char **argv )
{
    ...
    do
    {
        ...
        {
            LOG_INFORMATION("\nResponse from eSE: ");
            for (i=0; i<=(recv_response[2]+PAYLOAD_INDEX); i++)
            {
...
void eseDwpTest()
{
    ...
    else if (whatNQChip == SN_100)
    {
        cmds = sizeof(SN100_ESE_DWP) / sizeof(SN100_ESE_DWP[0]);
        sendcmds(SN100_ESE_DWP, cmds);
    }
    else
...
void ftm_nfc_dispatch_nq_test( int argc, char **argv )
{
    ...
    do
    {
        ...
        case 'e':
            ...
            {
                LOG_INFORMATION("eSE DWP test to ensure eSE is powered ON\n");
                ese_dwp_test = 1;
                eseDwpTest();
            }
            ese_spi_test = 1;
        ...
        if(type_of_test == 'n' || default_test)
        {
            switch(whatNQChip)
            {
                ...
                case NQ_330:
                    ...
                    break;
                case SN_100:
                    cmds = sizeof(SN100_cmds) / sizeof(SN100_cmds[0]);
                    sendcmds(SN100_cmds, cmds);
                    break;
                default:
