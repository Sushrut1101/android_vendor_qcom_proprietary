...
public class WigigConfigStore extends IpConfigStore {
        ...
        while (!done) {
            ...
            for (int i = 1; i < lines.length; i++) {
                ...
                if (mConfiguredNetworks.getByConfigKey(config.configKey()) != null) {
                    ...
                    if (mWigigNative.removeNetwork(config.networkId)) {
                        mWigigNative.saveConfig();
                    } else {
                        Log.d(TAG, "Failed to remove duplicate network " + config.networkId);
                    }
    ...
    private void writeIpAndProxyConfigurations() {
        ...
        // TODO: writeIpAndProxyConfigurationToFile is deprecated, use writeIpConfigurations instead
        super.writeIpAndProxyConfigurationsToFile(ipConfigFile, networks);
    }
    ...
    private NetworkUpdateResult addOrUpdateNetworkNative(WifiConfiguration config, int uid) {
        ...
        setVariables: {
            ...
            if (config.allowedKeyManagement.get(WifiConfiguration.KeyMgmt.WPA_PSK)) {
                // enable FT-PSK for secured networks
                config.allowedKeyManagement.set(WifiConfiguration.KeyMgmt.FT_PSK);
            }
            ...
            if (!mWigigNative.setNetworkVariable(
                        ...
                        "enable_edmg",
                        "1")) {
                Log.e(TAG, "failed to enable EDMG");
                break setVariables;
            }
    ...
    private void readNetworkVariables(WifiConfiguration config) {
        ...
        // hide FT-PSK from higher layers
        config.allowedKeyManagement.clear(WifiConfiguration.KeyMgmt.FT_PSK);
