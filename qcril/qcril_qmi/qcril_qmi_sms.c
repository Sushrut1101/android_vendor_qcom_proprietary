...
static errno_enum_type qcril_sms_send_sms_memory_status
(
...
)
{
  ...
  {
    qcril_default_request_resp_params(QCRIL_DEFAULT_INSTANCE_ID, params_ptr->t, params_ptr->event_id,
                                      ril_err, &resp);
  ...
  else if (raw_send_resp->resp.result == QMI_RESULT_FAILURE_V01)
  {
             ...
             (raw_send_resp->resp.error == QMI_ERR_INVALID_OPERATION_V01))
    {
      retry_status = TRUE;
            ...
            (params_ptr->event_id == RIL_REQUEST_IMS_SEND_SMS))
    {
...
)
{
  ...
  {
    ril_err = qcril_qmi_util_convert_qmi_response_codes_to_ril_result(QMI_NO_ERR, &(ack_resp->resp));
...
)
{
  ...
  {
    ril_err = qcril_qmi_util_convert_qmi_response_codes_to_ril_result(QMI_NO_ERR, &(delete_resp->resp));
...
)
{
  ...
  char smsc_address[ QCRIL_SMS_MAX_SMSC_ADDRESS_SIZE ] = {'\0'};
  RIL_Errno ril_err;
  ...
  if (get_smsc_addr_resp->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    if ( qcril_sms_convert_smsc_address_to_ril_format( get_smsc_addr_resp, smsc_address ) )
    {
      ...
      resp.resp_len = strlen( smsc_address ) + 1;
      qcril_send_request_response( &resp );
    }
    else
    {
      QCRIL_LOG_INFO("Failed to convert SMSC Address in RIL format.");
      qcril_default_request_resp_params( instance_id, params_ptr->t, params_ptr->event_id, RIL_E_INTERNAL_ERR, &resp );
      qcril_send_request_response( &resp );
    }
  }
  else
  {
    ril_err = qcril_qmi_util_convert_qmi_response_codes_to_ril_result(QMI_NO_ERR, &(get_smsc_addr_resp->resp));
...
)
{
  ...
  {
    ril_err = qcril_qmi_util_convert_qmi_response_codes_to_ril_result(QMI_NO_ERR, &(set_smsc_addr_resp->resp));
...
)
{
  ...
  do
  {
    ...
    if ((ret = qcril_qmi_client_send_msg_async(
                                    QCRIL_QMI_CLIENT_WMS,
...
)
{
  ...
  {
    ril_err = qcril_qmi_util_convert_qmi_response_codes_to_ril_result(QMI_NO_ERR, &(mem_status_resp->resp));
...
)
{
  ...
  do
  {
    ...
    else
    {
      ...
      ril_err = qcril_qmi_util_convert_qmi_response_codes_to_ril_result(QMI_NO_ERR, &(bc_config_resp->resp));
      break;
    }
  }while(0);
...
)
{
  ...
  {
    ril_err = qcril_qmi_util_convert_qmi_response_codes_to_ril_result(QMI_NO_ERR, &(bc_activate_resp->resp));
...
)
{
  ...
  do
  {
    ...
    if ( qcril_qmi_client_send_msg_sync_ex(
                                  ...
                                  QCRIL_QMI_SYNC_REQ_UNRESTRICTED_TIMEOUT ) != RIL_E_SUCCESS )
    {
  ...
  QCRIL_LOG_DEBUG("IMS SMS Registration state = %d, IMS SMS format = %d",
      ims_registration_state[0], ims_registration_state[1]);
  /* Send RIL_E_SUCCESS response, and remove the entry from ReqList. */
  qcril_default_request_resp_params( instance_id, params_ptr->t, params_ptr->event_id, RIL_E_SUCCESS, &resp );
...
)
{
   ...
   do
   {
      ...
      if (TRUE == qcril_qmi_sms_block_retry_3pgg_sms_on_lte_only(ims_sms_msg->tech, ims_sms_msg->retry))
      {
      ...
      {
         if ( ims_sms_msg->retry && (!wms_ready_atleast_limited_3gpp() &&
              !(is_voice_over_lte && lte_sms_status_valid && (NAS_SMS_STATUS_3GPP_V01 == lte_sms_status) ) )
            )
         {
            ril_err = RIL_E_INVALID_STATE;
      ...
      {
         // tech is 3GPP2
         if ( ims_sms_msg->retry && ( !wms_ready_atleast_limited_3gpp2() &&
              !(is_voice_over_lte && lte_sms_status_valid && (NAS_SMS_STATUS_1X_V01 == lte_sms_status)) )
            )
         {
            ril_err = RIL_E_INVALID_STATE;
...
)
{
  ...
  ims_Mwi ims_mwi;
  memset(&ims_mwi, 0, sizeof(ims_mwi));
  ims_MwiMessageSummary *mwi_msg_summary_ptr = NULL;
  ims_MwiMessageSummary **mwi_msg_summary_dptr = NULL;
  ims_MwiMessageDetails *mwi_msg_details_ptr = NULL;
  ims_MwiMessageDetails **mwi_msg_details_dptr = NULL;
  ...
  do
  {
    ...
    if (sum_len > 0)
    {
      mwi_msg_summary_ptr = qcril_malloc(sizeof (ims_MwiMessageSummary) *
                                             sum_len);
      mwi_msg_summary_dptr = qcril_malloc(sizeof (ims_MwiMessageSummary *) *
                                             (sum_len+1));
      if (mwi_msg_summary_ptr == NULL || mwi_msg_summary_dptr == NULL)
      {
      ...
      {
        ims_mwi.mwiMsgSummary.arg = (void*) mwi_msg_summary_dptr;
        for ( i = 0; i < sum_len; i++)
        {
          mwi_msg_summary_dptr[i] = &mwi_msg_summary_ptr[i];

          mwi_msg_summary_ptr[i].has_messageType = TRUE;
          mwi_msg_summary_ptr[i].messageType     =
                  qcril_qmi_sms_map_qmi_mwi_msg_type_to_ims_msg_type(
                  qmi_mwi_msg->message_waiting_summary_info[i].message_type);

          mwi_msg_summary_ptr[i].has_newMessage  = TRUE;
          mwi_msg_summary_ptr[i].newMessage      =
                  qmi_mwi_msg->message_waiting_summary_info[i].new_msg;

          mwi_msg_summary_ptr[i].has_oldMessage  = TRUE;
          mwi_msg_summary_ptr[i].oldMessage      =
                  qmi_mwi_msg->message_waiting_summary_info[i].old_msg;

          mwi_msg_summary_ptr[i].has_newUrgent   = TRUE;
          mwi_msg_summary_ptr[i].newUrgent       =
                  qmi_mwi_msg->message_waiting_summary_info[i].new_urgent;

          mwi_msg_summary_ptr[i].has_oldUrgent   = TRUE;
          mwi_msg_summary_ptr[i].oldUrgent       =
                  qmi_mwi_msg->message_waiting_summary_info[i].old_urgent;

        }
      }
    }
    ims_mwi.ueAddress.arg = qmi_ril_util_str_clone(qmi_mwi_msg->UE_address);
        ...
        qmi_mwi_msg->message_waiting_detail_info_len > 0)
    {
      ...
      mwi_msg_details_ptr  = qcril_malloc(sizeof (ims_MwiMessageDetails)  * detail_len);
      mwi_msg_details_dptr = qcril_malloc(sizeof (ims_MwiMessageDetails*) * (detail_len+1));

      if (mwi_msg_details_ptr == NULL || mwi_msg_details_dptr == NULL)
      {
        QCRIL_LOG_FATAL("malloc failed");
        break;
      }
      ...
      {
        ims_mwi.mwiMsgDetail.arg = (void*) mwi_msg_details_dptr;
        for ( i = 0; i < detail_len; i++)
        {
          mwi_msg_details_dptr[i] = &mwi_msg_details_ptr[i];

          mwi_msg_details_ptr[i].toAddress.arg = qmi_ril_util_str_clone(
                  qmi_mwi_msg->message_waiting_detail_info[i].to_address);
          mwi_msg_details_ptr[i].fromAddress.arg = qmi_ril_util_str_clone(
                  qmi_mwi_msg->message_waiting_detail_info[i].from_address);
          mwi_msg_details_ptr[i].subject.arg = qmi_ril_util_str_clone(
                  qmi_mwi_msg->message_waiting_detail_info[i].subject);
          mwi_msg_details_ptr[i].date.arg = qmi_ril_util_str_clone(
                  qmi_mwi_msg->message_waiting_detail_info[i].date_time);
          mwi_msg_details_ptr[i].has_priority = TRUE;
          mwi_msg_details_ptr[i].priority = qcril_qmi_sms_map_qmi_mwi_priority_to_ims_priority(
                  qmi_mwi_msg->message_waiting_detail_info[i].priority);
          mwi_msg_details_ptr[i].messageId.arg = qmi_ril_util_str_clone(
                  qmi_mwi_msg->message_waiting_detail_info[i].message_id);
          mwi_msg_details_ptr[i].has_messageType = TRUE;
          mwi_msg_details_ptr[i].messageType = qcril_qmi_sms_map_qmi_mwi_msg_type_to_ims_msg_type(
  ...
  if (mwi_msg_summary_ptr)
  {
    qcril_free(mwi_msg_summary_ptr);
  }
  if (mwi_msg_summary_dptr)
  {
    qcril_free(mwi_msg_summary_dptr);
  }
  if (mwi_msg_details_ptr)
  {
    qcril_free(mwi_msg_details_ptr);
  }
  if (mwi_msg_details_dptr)
  {
    qcril_free(mwi_msg_details_dptr);
  }
  if (ims_mwi.ueAddress.arg)
  {
    qcril_free(ims_mwi.ueAddress.arg);
  }
...
void query_wms_ready_status(void)
{
   ...
   qmi_err = qmi_client_send_msg_async_with_shm(
                     qcril_qmi_client_get_user_handle(QCRIL_QMI_CLIENT_WMS),
