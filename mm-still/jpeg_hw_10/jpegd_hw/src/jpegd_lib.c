...
volatile uint32_t g_jdloglevel;
  ...
  jpegd_base_config_t *p_base_cfg)
{
  ...
  rc = ioctl (p_jpg->fd, MSM_JPEG_IOCTL_GET_HW_VERSION, &hw_cmd);
  ...
  rc = jpegd_hw_fe_cfg(p_jpg->fd, &fe_cfg, version);
  if (rc) {
    JDDBG_ERROR("%s:%d] Error %d", __func__, __LINE__, rc);
    return JPEGDERR_EFAILED;
  }
  ...
  rc = jpegd_hw_we_cfg(p_jpg->fd, &we_cfg, &img_info,
    p_base_cfg->scale_factor, version);
  if (rc) {
    JDDBG_ERROR("%s:%d] Error %d", __func__, __LINE__, rc);
    return JPEGDERR_EFAILED;
  }
  ...
  rc = jpegd_hw_decode_cfg(p_jpg->fd, 0, p_base_cfg->height,
    p_base_cfg->width, (jpegd_subsampling_t)p_base_cfg->format,
    p_base_cfg->restart_interval);
  if (rc) {
    JDDBG_ERROR("%s:%d] Error %d", __func__, __LINE__, rc);
    return JPEGDERR_EFAILED;
 ...
 * DESCRIPTION:
 *==========================================================================*/
int jpegd_lib_output_buf_get(void *handle, jpegd_buf *buf __unused)
{
  int rc;
  jpegd_lib_t *p_jpg = (jpegd_lib_t *)handle;
  struct msm_jpeg_buf msm_buf;
...
#ifdef DUMP_OUT_YUV
  char *output_filename = QCAMERA_DUMP_FRM_LOCATION"dec1.yuv";
 ...
 * DESCRIPTION:
 *==========================================================================*/
int jpegd_lib_input_buf_get(void *handle, jpegd_buf *buf __unused)
{
  int rc;
  jpegd_lib_t *p_jpg = (jpegd_lib_t *)handle;
  struct msm_jpeg_buf msm_buf;
...
int jpegd_lib_init(void **handle, void *p_user, jpegd_evt_handler evt_handler)
{
  ...
  property_get("persist.vendor.camera.mmstill.logs", prop, "0");
  g_jdloglevel = (uint32_t)atoi(prop);
  JDDBG_ERROR("%s:%d ###jpeghw_Loglevel %d", __func__, __LINE__, g_jdloglevel);
#endif
