...
bool QImageSWEncoder::IsAvailable(QIEncodeParams& aEncodeParams)
{
  ...
  QIDBG_MED("%s:%d] scale enabled %d", __func__, __LINE__, lScaleEnabled);
  if (lScaleEnabled) {
 ...
 * Notes: none
 *==========================================================================*/
int QImageSWEncoder::Start(QIThread *a_pUseEncThread)
{
  QI_UNUSED(a_pUseEncThread);
  ...
  mpEncoderThread->setThreadName("JPEG_SWEnc");
...
void QImageSWEncoder::ConfigureDimensions()
{
  ...
  QIDBG_HIGH("%s:%d] dim pad %dx%d orig %dx%d scale "
    "input %dx%d crop (%d, %d) output %dx%d",
    __func__, __LINE__,
  ...
  jpege_img_data_t *p_source)
{
  ...
  if (p_config->stride_cfg.enable) {
    ...
    //If stride is disabled, then luma_stride is set to
    //lumawidth and chroma stride is set to twice the chroma size
    ...
    case JPEG_H2V1:
    case JPEG_H2V2: {
      p_engine->stride_cfg.input_chroma_stride = 2 *
        ((p_source->width + 1) >> 1); // odd luma support
      p_engine->stride_cfg.output_chroma_stride = 2 *
        ((p_source->width + 1) >> 1); // odd luma support
      break;
    }
    default:
  ...
  // Scaling parameter to set output chroma width based on sampling type
  ...
  if (p_config->scale_cfg.enable) {
    ...
    p_engine->jpege_scale.vert_luma_row_index = 0;
    p_engine->jpege_scale.vert_luma_output_height =
      p_config->scale_cfg.output_height;
    p_engine->jpege_scale.vert_chroma_row_index = 0;
    p_engine->jpege_scale.vert_chroma_output_height =
      p_config->scale_cfg.output_height >> sampling_v_shift;
    p_engine->jpege_scale.piece_height = p_config->scale_cfg.output_height;
...
static void jpege_engine_sw_insert_restart_marker(jpege_engine_sw_t *p_engine)
{
  ...
  currentMarker = (uint16_t)(BASE_RESTART_MARKER +
    p_engine->restartMarkerModCount);
  p_engine->restartMarkerModCount =
    (uint32_t)(p_engine->restartMarkerModCount + 1) %
    MAX_RESTART_MARKER_COUNT;
 ...
 * Notes: none
 *==========================================================================*/
int QImageSWEncoder::FillDestBuffer(bool aForceFlush __unused)
{
  ...
  jpege_img_cfg_t *p_config)
{
  ...
  case 90: {
    ...
    for (i = 1; i < 8; i+=2) {
      for (j = 0; j < 8; j++) {
          p_engine->lumaQuantTableQ16[i*8+j] = (int16_t)
            (int16_t)(-p_engine->lumaQuantTableQ16[i*8+j]);
          p_engine->chromaQuantTableQ16[i*8+j] =
            (int16_t)(-p_engine->chromaQuantTableQ16[i*8+j]);
      }
    }
    break;
  }
  ...
  for (i = JBLOCK_SIZE_MINUS_1; i != 0; i--) {
    p_engine->zigzagOffsetTable[i] =
      (uint16_t)((zigzagTablePtr[i - 1] -
      zigzagTablePtr[i]) * sizeof(int16_t));
  }
 ...
 * Notes: none
 *==========================================================================*/
void QISWOutputThread::run(QThreadObject *aData __unused)
{
 ...
 * Notes: none
 *==========================================================================*/
void QISWEncoderThread::run(QThreadObject *aData __unused)
{
  mEncoder.Encode();
}
...
void QImageSWEncoder::Encode()
{
  ...
  *(p_bitstream->nextBytePtr++) = 0xD9;
  ...
  //If fd is set clean the cache before complete the image
  if (mOutputImage->Fd() >= 0) {
    QIONBuffer::DoCacheOps(mOutputImage->Fd(), mOutputImage->FilledLen(),
      QIONBuffer::CACHE_CLEAN);
  }
...
void QImageSWEncoder::ReleaseSession()
{
  ...
  if (mpOutputThread) {
    mpOutputThread->StopThread(NULL);
  }

  if (mpEncoderThread) {
    mpEncoderThread->StopThread(NULL);
  }
