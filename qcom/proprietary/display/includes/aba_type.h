...
#define SCALEQ8(x, uf) ((x) * 255 / ((1 << (8 - uf)) - 1))
...
#define PIXEL_Q8Q10(x)   ((((x)<<10) + ((x)<<2) - (x) + 128) >> 8)
#define PIXEL_Q10Q8(x)   ((((x)<<10) + (x) - ((x)<<2) + 2048) >>12)
#define PIXEL_Q10Q8FLOOR(x)   ((((x)<<10) + (x) - ((x)<<2)) >>12)
...
/*
* AbaSwitchPointStateType
* Describes if a switch point is
* a start point or end point
*/

typedef enum
{
  ABA_SWITCH_POINT_STATE_UNKNOWN,
  ABA_SWITCH_POINT_STATE_START,
  ABA_SWITCH_POINT_STATE_END,
  ABA_SWITCH_POINT_STATE_MAX
}AbaSwitchPointStateType;

/*
* AbaSwitchPointType
* Describes the types of switch point supported
*/

typedef enum
{
  ABA_SWITCH_POINT_UNKNOWN,
  ABA_SWITCH_POINT_AL,
  ABA_SWITCH_POINT_BL,
  ABA_SWITCH_POINT_MAX
}AbaSwitchPointType;
...
typedef struct
{
  ...
  uint32_t uHistogramBitDepth;        //Number of bits used to represent the maximum histogram gray level
  uint32_t uLUTSize;                  //Hardware look up table size: e.g 256, 128, etc
  uint32_t uLUTSizeBitDepth;          //Number of bits used to represent uLutSize
  uint32_t uLUTMaxVal;                //Hardware LUT max value: 255, 1023, etc
  uint32_t uLUTBitDepth;              //Number of bits used to represent the maximum LUT gray level
  uint32_t uHalfBin;                  //reserved
...
typedef enum
{
  ABA_STATE_UNKNOWN,                 // ABA is in unknown state
  ABA_STATE_NOTINIT,                 // ABA is not initialized
  ABA_STATE_DEACTIVATING,            // ABA is transitioning to inactive
...
typedef struct
{
  uint32_t        uABAVersion;
  uint32_t        uSVIVersion;
  uint32_t        uCABLVersion;
  uint32_t        uFOSSVersion;
}AbaVersionType;
