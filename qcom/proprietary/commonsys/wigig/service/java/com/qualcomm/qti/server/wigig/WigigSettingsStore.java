...
final class WigigSettingsStore {
    ...
    /* Values stored in SENSING_ON */
    private static final String SENSING_ON = "sensing_on";
    private static final int SENSING_DISABLED                      = 0;
    private static final int SENSING_ENABLED                       = 1;

    /* Persisted state that tracks the sensing interaction from settings */
    private int mPersistSensingState = SENSING_DISABLED;
    ...
    WigigSettingsStore(Context context) {
        ...
        mPersistSensingState = getPersistedSensingState();
    }
    ...
    /**
     * Returns true if sensing is currently on.
     *
     * @return {@code true} if sensing is on.
     */
    synchronized boolean getSensingOn() {
        return mPersistSensingState != SENSING_DISABLED;
    }
    ...
    /**
     * Update the persisted sensing state upon sensing  enable/disable.
     * The new sensing  state depends on current networking mode, as sensing cannot be
     * enabled in AP, P2P or FST modes.
     *
     * Returns false if sensing state didn't change or true if sensing state might have changed
     */
    synchronized boolean handleSensingToggled(boolean sensingEnabled) {
        Log.i(TAG, "handleSensingToggled(" + sensingEnabled + ")");

        if (sensingEnabled) {
            persistSensingState(SENSING_ENABLED);
        } else {
            persistSensingState(SENSING_DISABLED);
        }

        Log.i(TAG, "new Sensing state=" + mPersistSensingState);
        return true;
    }
    ...
    private void persistSensingState(int state) {
        final ContentResolver cr = mContext.getContentResolver();
        mPersistSensingState = state;
        Settings.Global.putInt(cr, SENSING_ON, state);
    }
    ...
    private int getPersistedSensingState() {
        final ContentResolver cr = mContext.getContentResolver();
        try {
            return Settings.Global.getInt(cr, SENSING_ON);
        } catch (Settings.SettingNotFoundException e) {
            Settings.Global.putInt(cr, SENSING_ON, SENSING_DISABLED);
            return SENSING_DISABLED;
        }
    }
}
