...
int netmgr_qmi_reset ( int link )
{
  ...
  return netmgr_platform_vtable.qmi.reset(link);
}
...
{
    VERIFY_VTABLE_FUNC( kif, iface_link_update );
    return netmgr_platform_vtable.kif.iface_link_update( link );
}
...
{
  VERIFY_VTABLE_FUNC( tc, flow_control );
  return netmgr_platform_vtable.tc.flow_rebind(link,
                                               qos_flow);
}
...
{
  VERIFY_VTABLE_FUNC( kif, wlan_open );
  return netmgr_platform_vtable.kif.wlan_open( link );
}
...
{
  VERIFY_VTABLE_FUNC( kif, wlan_close );
  return netmgr_platform_vtable.kif.wlan_close( link );
}
...
{
  VERIFY_VTABLE_FUNC(kif, enable_ipv6);
  return netmgr_platform_vtable.kif.enable_ipv6(link);
}
