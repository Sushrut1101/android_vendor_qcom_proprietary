...
static void* pb_log_throughput()
{
    ...
    while(1) {
        ...
        port_log_high("The number of bytes written (uplink) in %d seconds is %lu",
                THROUGHPUT_INTERVAL,
                labs((long)(count_bytes_to_smd_new - count_bytes_to_smd_prev)));

        port_log_high("The number of bytes written (dwnlink) in %d seconds is %lu",
                THROUGHPUT_INTERVAL,
                labs((long)(count_bytes_to_ext_new - count_bytes_to_ext_prev)));
...
FUNCTION    : pb_clear_modem_bits

DESCRIPTION : This function clears specified bits in modem using TIOCMBIC ioctl
...
============================================================================*/
static void pb_clear_modem_bits(int new_modem_bits, dun_portparams_s *pportparams)
{

    if(new_modem_bits) {
        if(ioctl(pportparams->smdport_fd, TIOCMBIC, &new_modem_bits) < 0) {
            port_log_err("Error while clearing SMD port bits : %s\n",
                    strerror(errno));
            return;
        }
...
dun_internal_error handle_ctrl_request(dun_ipc_ctrl_msg_type ctrl_msg,
        dun_portparams_s *pportparams)
{
    ...
    switch(ctrl_msg) {
        case DUN_CRTL_MSG_DISCONNECT_REQ:
            ...
            /* Clear RTS/CTS bits, without this modem does not know conenction is disconnected */
            if (!pportparams->isTtyPort)
                pb_clear_modem_bits(RMT_MODEM_SIGNAL_RTSCTS, pportparams);
...
dun_internal_error handle_dun_request (dun_portparams_s *pportparams)
{
    dun_internal_error err = DUN_ERR_NONE;
    DUN_ATCMD_E atcmd;
    int num_read;
...
int do_read(int s, dun_portparams_s *pportparams)
{
    ...
    if(ipc_msg_id == DUN_IPC_MSG_CTRL_REQUEST) {
        ...
        retval = handle_ctrl_request(dun_ipc_ctrl_msg, pportparams);
        ...
        retval = handle_dun_request(pportparams);
...
FUNCTION    : check_if_ok_to_send_data_to_remote_dev

DESCRIPTION : This function checks if data can be sent to remote device or not

DEPENDENCIES : None

RETURN VALUE : NO if data can't be sent, else YES

============================================================================*/
int check_if_ok_to_send_data_to_remote_dev(char *buf)
{
    int ret = NO;
    char no_carrier[] = "\r\nNO CARRIER\r\n";
    if (dun_disconnected)
        return ret;
    if (pb_dun_state == DUN_STATE_IDLE && !memcmp(buf, no_carrier, strlen(no_carrier)))
        return ret;
    return YES;
}
    ...
    while (pportparams->dlink_running) {
        ...
        if ((pb_dun_state == DUN_STATE_CONNECTED)||(pb_dun_state == DUN_STATE_IDLE)) {
            ...
            if (num_read > 0) {
                ...
                port_log_err("pb_dataxfr_dlink ok_to_send_data %d\n",
                    check_if_ok_to_send_data_to_remote_dev((char *)ipc_msg.xfer_buf));
                if (pportparams->isTtyPort ||
                    check_if_ok_to_send_data_to_remote_dev((char *)ipc_msg.xfer_buf)) {
                    /* fill all DUN IPC parameters */
                    ipc_msg.msg_type = DUN_IPC_MSG_DUN_RESPONSE;
                    ipc_msg.msg_len  = num_read;
                    if (write(pportparams->conn_sk, (void *) &ipc_msg, num_read +
                        DUN_IPC_HEADER_SIZE) < 0) {
...
int pb_init_ports(dun_portparams_s *pportparams)
{
    ...
    LOGI("pportparams->isTtyPort: %d", pportparams->isTtyPort);
