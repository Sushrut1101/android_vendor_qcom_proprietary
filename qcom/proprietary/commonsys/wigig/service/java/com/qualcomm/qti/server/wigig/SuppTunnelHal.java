...
public class SuppTunnelHal {
    ...
    private boolean initSupplicantTunnelProvider() {
        synchronized (mLock) {
            try {
                ...
                Log.e(TAG, "ISuppTunnelProvider.getService generic exception: " + e);
                return false;
            }
    ...
    private ISuppTunnel initSupplicantTunnelLocked(int type) {
        ...
        try {
            ...
            Log.e(TAG, "getTunnel exception: " + e);
            return null;
        } catch (Throwable e) {
            // any exception from here may cause HwBinder to exit, which
            // will kill the system server, so catch any other exception.
            Log.e(TAG, "getTunnel generic exception: " + e);
            return null;
        }
    ...
    private boolean registerCallbackLocked(ISuppTunnelCallback callback) {
        if (mISuppTunnel == null || mHostapdISuppTunnel == null) {
            Log.e(TAG, "Supplicant/Hostapd tunnel service not available");
            return false;
        }
        try {
            SuppTunnelStatus status = mISuppTunnel.registerCallback(callback);
            if (status.code != SuppTunnelStatusCode.SUCCESS) {
                Log.e(TAG, "mISuppTunnel.registerCallback failed, code: " + status.code + " " + status.debugMessage);
                return false;
            }
            status = mHostapdISuppTunnel.registerCallback(callback);
            if (status.code != SuppTunnelStatusCode.SUCCESS) {
                Log.e(TAG, "mHostapdISuppTunnel.registerCallback failed, code: " + status.code + " " + status.debugMessage);
                return false;
            }
        } catch (RemoteException e) {
