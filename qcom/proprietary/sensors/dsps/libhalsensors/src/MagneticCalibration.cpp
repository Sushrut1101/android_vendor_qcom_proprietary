...
void MagneticCalibration::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    switch (msg_hdr->msg_id) {
        case SNS_SAM_MAG_CAL_REPORT_IND_V01:
            ...
            if(bWakeUp == false) {
                ...
                sensor_data.sensor = HANDLE_MAGNETIC_FIELD_SAM_WAKE_UP;
    ...
    if (error == false) {
        ...
        HAL_LOG_VERBOSE("%s: SAM Mag X: %f Y: %f Z: %f SAM TS: %u HAL TS:%" PRId64 " elapsedRealtimeNano:%" PRId64 "",
                         __FUNCTION__, sensor_data.magnetic.x, sensor_data.magnetic.y,
                         sensor_data.magnetic.z, timestamp, sensor_data.timestamp,
