...
bool init_surface(draw_control_t * draw) {
    ...
    if (dtoken == nullptr) {
        MMI_ALOGE("getInternalDisplayToken() failed");
        return false;
    }
    ...
    draw->pixels_percent_x = (float)draw->dinfo.w / (float)PERCENT_MAX;
    draw->pixels_percent_y = (float)draw->dinfo.h / (float)PERCENT_MAX;
