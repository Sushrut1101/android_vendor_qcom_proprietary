...
bool Socket::recv(string &s, size_t size) const
{
    size_t recv_size = 0;
    int status = 0;
    bool rc = true;
...
bool Socket::connect(const string &host, const int port)
{
    ...
    int status = inet_pton (AF_INET, host.data(), &m_addr.sin_addr);
...
bool ServerSocket::recv(string &s, size_t size, bool &pipe_event, uint32_t pipe_msg_size)
{
    bool rc = false;
