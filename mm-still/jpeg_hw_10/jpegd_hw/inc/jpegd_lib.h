/*============================================================================
   Copyright (c) 2012-2014, 2016, 2019 Qualcomm Technologies, Inc.  All Rights Reserved.
...
typedef struct {
  ...
  uint32_t rotation;
  uint32_t mal_en;
  uint32_t mal_boundary;
  uint32_t plane0_enable;
  uint32_t plane1_enable;
  uint32_t plane2_enable;
  uint32_t read_enable;
  uint32_t swc_fetch_enable;
  uint32_t bottom_vpad_enable;
  uint32_t cbcr_order;
  uint32_t memory_format;
  uint32_t burst_length_max;
  uint32_t byte_ordering;
} jpegd_cmd_fe_cfg;

typedef struct {
  uint32_t flip;
  uint32_t rotation;
  uint32_t pop_buff_on_eos;
  uint32_t mal_enable;
  uint32_t mal_boundary;
  uint32_t pln2_enable;
  uint32_t pln1_enable;
  uint32_t pln0_enable;
  uint32_t cbcr_order;
  uint32_t memory_format;
  uint32_t burst_length_max;
  uint32_t byte_ordering;
