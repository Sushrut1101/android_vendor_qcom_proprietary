...
static void init_user_space_sensors(void)
{
	...
	while ((tdirent = readdir(tdir))) {
		...
		if (sensor_cnt >= MAX_NUM_SENSORS) {
			msg("%s: Max supported sensor reached\n", __func__);
			break;
		}
		...
		if (!tzdir)
			continue;
		if (snprintf(name, MAX_PATH, POLICY_TZ_TYPE, tdirent->d_name) < 0)
		...
		if (snprintf(name, MAX_PATH, SENSOR_TZ_TYPE, tdirent->d_name) < 0)
			msg("%s: truncation error\n", __func__);
		dbgmsg("%s: Opening %s\n", __func__, name);
		ret = read_line_from_file(name, buf, sizeof(buf));
		if (ret <= 0) {
...
#ifdef ENABLE_BW_SENSOR
	/* BW_SENSOR */
	ret_val = get_bw_client_sensors_list(&bw_sensor_list);
	if (ret_val > 0) {
		if (add_tgt_sensors_set(bw_sensor_list, ret_val) != 0) {
			msg("%s: Error adding BANDWIDTH TS\n", __func__);
			ret_val = -(EFAULT);
		} else {
			ret_val = 0;
		}
