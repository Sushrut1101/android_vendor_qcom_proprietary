...
Return<sp<ILocHidlAGnss>> LocHidlGnss::getExtensionLocHidlAGnss_1_1() {
    FROM_HIDL_CLIENT();
    if (mGnss != nullptr && mAGnssVendor == nullptr) {
        sp<IAGnss> agnss = mGnss->getExtensionAGnss();
        mAGnssVendor = new LocHidlAGnss(agnss);
...
Return<sp<ILocHidlDebugReportService>> LocHidlGnss::getExtensionLocHidlDebugReportService_1_1() {
    FROM_HIDL_CLIENT();
    if (mDebugReportService == nullptr) {
        mDebugReportService = new LocHidlDebugReportService();
    }
...
Return<sp<ILocHidlFlpService>> LocHidlGnss::getExtensionLocHidlFlpService_1_1() {
    FROM_HIDL_CLIENT();
    if (mGnssFlpServiceProvider == nullptr) {
        mGnssFlpServiceProvider = new LocHidlFlpService();
    }
...
Return<sp<ILocHidlGeofenceService>> LocHidlGnss::getExtensionLocHidlGeofenceService_1_1() {
    FROM_HIDL_CLIENT();
    if (mGnssGeofenceServiceProvider == nullptr) {
        mGnssGeofenceServiceProvider = new LocHidlGeofenceService();
    }
...
Return<sp<ILocHidlIzatProvider>> LocHidlGnss::getExtensionLocHidlIzatNetworkProvider_1_1() {
    FROM_HIDL_CLIENT();
    if (mIzatNetworkProvider == nullptr) {
        mIzatNetworkProvider = new LocHidlIzatProvider(LocHidlIzatStreamType::NETWORK);
    }
...
Return<sp<ILocHidlIzatProvider>> LocHidlGnss::getExtensionLocHidlIzatFusedProvider_1_1() {
    FROM_HIDL_CLIENT();
    if (mIzatFusedProvider == nullptr) {
        mIzatFusedProvider = new LocHidlIzatProvider(LocHidlIzatStreamType::FUSED);
    }
...
Return<sp<ILocHidlGnssNi>> LocHidlGnss::getExtensionLocHidlGnssNi_1_1() {
    FROM_HIDL_CLIENT();
    if (mGnss != nullptr && mGnssNiVendor == nullptr) {
        sp<IGnssNi> gnssNi = mGnss->getExtensionGnssNi();
        mGnssNiVendor = new LocHidlGnssNi(gnssNi);
...
Return<sp<ILocHidlIzatOsNpGlue>> LocHidlGnss::getExtensionLocHidlIzatOsNpGlue_1_1() {
    FROM_HIDL_CLIENT();
    if (mIzatOsNpGlue == nullptr) {
        mIzatOsNpGlue = new LocHidlIzatOsNpGlue();
    }
...
Return<sp<ILocHidlIzatSubscription>> LocHidlGnss::getExtensionLocHidlIzatSubscription_1_1() {
    FROM_HIDL_CLIENT();
    if (mIzatSubscription == nullptr) {
        mIzatSubscription = new LocHidlIzatSubscription();
    }
...
Return<sp<ILocHidlRilInfoMonitor>> LocHidlGnss::getExtensionLocHidlRilInfoMonitor_1_1() {
    FROM_HIDL_CLIENT();
    if (mGnssRilInfoMonitor == nullptr) {
        mGnssRilInfoMonitor = new LocHidlRilInfoMonitor();
    }
...
Return<sp<ILocHidlXT>> LocHidlGnss::getExtensionLocHidlXT_1_1() {
    FROM_HIDL_CLIENT();
    if (mGnssXT == nullptr) {
        mGnssXT = new LocHidlXT();
    }
...
Return<sp<ILocHidlWiFiDBReceiver>> LocHidlGnss::getExtensionLocHidlWiFiDBReceiver_1_1() {
    FROM_HIDL_CLIENT();
    if (mIzatWiFiDBReceiver == nullptr) {
        mIzatWiFiDBReceiver = new LocHidlWiFiDBReceiver();
    }
...
ILocHidlGnss* HIDL_FETCH_ILocHidlGnss(const char* /* name */) {
    ...
    dlerror();
    const char* gnsslib = GNSS_LIB_V11;
    void *handle = dlopen(gnsslib, RTLD_NOW);
    if (nullptr == handle || (error = dlerror()) != nullptr)  {
        LOC_LOGW("failed to dlopen %s", gnsslib);

        gnsslib = GNSS_LIB_V10;
        dlerror(); // reset dlerror
        handle = dlopen(gnsslib, RTLD_NOW);
        if (nullptr == handle || (error = dlerror()) != nullptr)  {
            LOC_LOGE("dlopen for %s failed, error = %s", gnsslib, error == nullptr ? "" : error);
            return nullptr;
        }
    }
    ...
    if ((error = dlerror()) != nullptr || getter == NULL)  {
        LOC_LOGE("dlsym for %s failed, error = %s", gnsslib, error == nullptr ? "" : error);
        return nullptr;
    }
    sp<IGnss> gnss = (IGnss*)(*getter)(nullptr);
    if (gnss == nullptr) {
        LOC_LOGE("failed to get IGnss from %s", gnsslib);
        return nullptr;
    }
    auto hal = new LocHidlGnss(gnss);
