/******************************************************************************
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2009-2015, 2017-2018 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
#define LINUX_QMI_PLATFORM_CONFIG_FILE "/system/vendor/etc/data/qmi_config.xml"
#else
...
#define WAIT_TIME_BEFORE_NEXT_RETRY   50000
...
#define LINUX_QMI_PERSIST_RADIO_SGLTE_CSFB "persist.vendor.radio.sglte_csfb"

#ifdef FEATURE_QMI_IWLAN
  #define LINUX_QMI_PERSIST_IWLAN_ENABLED    "persist.vendor.data.iwlan.enable"
...
===========================================================================*/
/*===========================================================================
  FUNCTION  linux_qmi_qmux_if_get_use_qmuxd
===========================================================================*/
/*!
@brief
...
/*=========================================================================*/
static int
linux_qmi_qmux_if_get_use_qmuxd( void )
{
  QMI_DEBUG_MSG_2("qmuxd: %s: use_qmuxd [%d]", __func__ , qmi_qmux_dpm_state.use_qmuxd);
  return qmi_qmux_dpm_state.use_qmuxd;
}
/*===========================================================================
...
)
{
  ...
  cptr = strtok_r(dev_name, "/", &saveptr);
...
)
{
  ...
  /* Set use_qmuxd to TRUE for all targets. Targets with Shim
     support should override this in config file.*/
  qmi_qmux_dpm_state.use_qmuxd = TRUE;
...
#ifdef FEATURE_QMI_ANDROID
  /* qmuxd does not use configdb for any target other than 4.5 and for off target testing.
 ...
 /* Off Target - based off of Fusion4.5 PCIe Device above */
  else if (DS_TARGET_OFF_TARGET == target)
  {
    QMI_DEBUG_MSG_0 ("qmuxd: Off Target target configuration\n");
    if (0 != qmi_platform_config_configure_ports_xml(LINUX_QMI_PLATFORM_CONFIG_FILE, "off_target"))
    {
      QMI_ERR_MSG_2("qmuxd: Configuration using %s [%s] failed, reverting to defaults", LINUX_QMI_PLATFORM_CONFIG_FILE, "off_target");
  ...
  else if (DS_TARGET_FUSION_8084_9X45 == target)
  {
    QMI_DEBUG_MSG_0 ("qmuxd: Fusion 8084 + 9x45 (PCIe) target configuration\n");
    if (0 != qmi_platform_config_configure_ports_xml(LINUX_QMI_PLATFORM_CONFIG_FILE, "fusion_8084_9x45_pcie"))
    {
      QMI_ERR_MSG_2("qmuxd: Configuration using %s [%s] failed, reverting to defaults", LINUX_QMI_PLATFORM_CONFIG_FILE, "fusion_8084_9x45_pcie");
      /* Pre-fill with default configuration (in case configdb fails) */
      for (i = 0; i < LINUX_QMI_MAX_CONN_SUPPORTED; i++)
      {
        linux_qmi_conn_id_enablement_array[i].enabled = FALSE;
  ...
  else if (DS_TARGET_MSM8994 == target)
  {
    ...
    qmi_qmux_dpm_state.dpm_enabled =  TRUE;
    ...
    QMI_DEBUG_MSG_2 ("qmuxd:  MSM8994 configuration, Enabling channel %d [%s] "
    ...
    QMI_DEBUG_MSG_2 ("qmuxd:  MSM8994 configuration, \n"
  ...
  /* If DPM is enabled check for modem shim support */
  if ( TRUE == qmi_qmux_dpm_state.dpm_enabled  &&
       FALSE == linux_qmi_qmux_if_get_use_qmuxd())
  {
    int rc = QMI_NO_ERR;
    QMI_DEBUG_MSG_2("%s: dpm enabled on target [%s]",
                    __func__, target_str);

    rc = qmi_qmux_process_modem_capabilities();
    if (QMI_NO_ERR != rc )
    {
      QMI_DEBUG_MSG_2("%s: Modem caps query failed rc[%d]",
                      __func__, rc);
    }
    else
    {
      QMI_DEBUG_MSG_2("%s: modem cap query successful. Shim [%d]",
                      __func__, qmi_qmux_dpm_state.is_shim_supported);
      if (TRUE == qmi_qmux_dpm_state.is_shim_supported)
      {
        /* Disabling all channels*/
        QMI_DEBUG_MSG_1("%s: Disabling all channels",__func__);
        for (i = QMI_CONN_ID_RMNET_0; i < LINUX_QMI_MAX_CONN_SUPPORTED; i++)
        {
          qmi_qmux_disable_port(linux_qmi_conn_id_enablement_array[i].qmi_conn_id,
                        linux_qmi_conn_id_enablement_array[i].data_ctl_port, TRUE);
...
linux_qmi_qmux_if_server_alloc_client_id(void)
{
  ...
  /* Check if we are using shim reserved client id*/
  if (QMI_QMUX_DONT_USE_QMUXD_CLIENT_ID == alloc_client_id)
  {
    alloc_client_id = qmux_client_id++;
  }
...
)
{
  ...
  if ( TRUE == qmi_qmux_dpm_state.is_shim_supported &&
       FALSE == linux_qmi_qmux_if_get_use_qmuxd() )
  {
    qmux_client_id = QMI_QMUX_DONT_USE_QMUXD_CLIENT_ID;
    if (send (client_fd,
             (void *) &qmux_client_id,
    ...
    close(client_fd);
    QMI_ERR_MSG_2 ("qmuxd: Shim layer supported by modem, closing client fd[%d] client_id[%d]\n",
                   client_fd,qmux_client_id);
    return;
  }
      ...
      else if (DS_TARGET_MDM == target)
      {
              ...
              QMI_CONN_ID_RMNET_USB_0)
        {
           wait_for_modem = TRUE;
                ...
                || (DS_TARGET_FUSION_8096_9X55 == target))
      {
        if (linux_qmi_conn_id_enablement_array[i].qmi_conn_id ==
