...
boolean log_to_file = FALSE;
  ...
  #define NETMGR_UTIL_WAKELOCK_TIMEOUT_DEFAULT  2
...
netmgr_util_get_wakelock_timeout( void )
{
  ...
  if(netmgr_main_cfg.wakelock_timeout)
      ret = netmgr_main_cfg.wakelock_timeout;
  else
      ret = NETMGR_UTIL_WAKELOCK_TIMEOUT_DEFAULT;
...
{
  if (netmgr_wakelock_timer)
  {
    timer_delete(netmgr_wakelock_timer);
    netmgr_wakelock_timer = NULL;
  }
}
...
)
{
  ...
  switch (type)
  {
    case NETMGR_IPSEC_ALGO_HASH:
    {
      ...
      else if (WDS_IPSEC_CRYPTO_ALGO_SHA512_V01 == algo)
      {
        algo_str = "hmac(sha512)";
      }
      else if (WDS_IPSEC_CRYPTO_ALGO_SHA384_V01 == algo)
      {
        algo_str = "hmac(sha384)";
      }
    }
      break;
...
/*===========================================================================
  FUNCTION  netmgr_util_wakelock_force_release
===========================================================================*/
/*!
@brief
  Releases the netmgr wakelock forcefully if multiple wl refs are held.
...
*/
/*=========================================================================*/
void netmgr_util_wakelock_force_release()
{
  NETMGR_LOCK_MUTEX(wakelock_mutex);
