...
int ParseInputForfd(const char *name,char *sysfspath) {
    ...
    strlcpy(devname, dirname, strlen(dirname));
    ...
    *filename++ = '/';
    while((de = readdir(dir))) {
        ...
        continue;
        strlcpy(filename, de->d_name, strlen(de->d_name));
...
int main(int argc, char *argv[])
{
    ...
    if(!strcmp(argv[2],MODE_GET_STATUS))
    {
         ...
         int len;
         pChar = getsysfspath(argv[1]);
         if (*pChar != NULL) {
             len = strlen(getsysfspath(argv[1]));
         }
         else {
             return -1;
         }
         strlcpy(sysfspath,getsysfspath(argv[1]),len);
