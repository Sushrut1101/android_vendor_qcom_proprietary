...
extern int qmi_csi_xport_qrtr_supported(void);
extern qmi_csi_xport_ops_type qcsi_qrtr_ops;
...
#ifdef QMI_CSI_OPTIONS_LOG_MSG_CB_VALID
...
void __attribute__ ((constructor)) qmi_csi_init(void)
{
  ...
  use_xport_qrtr = qmi_csi_xport_qrtr_supported();
  if(use_xport_qrtr)
    qmi_csi_xport_start(&qcsi_qrtr_ops, NULL);
  else
...
{
  if(use_xport_qrtr)
    qmi_csi_xport_stop(&qcsi_qrtr_ops, NULL);
  else
