  ...
  depth_service_output_t *output)
{
  ...
  for (i = 0; i < size; i++) {
    ...
    target_lens_pos_grid_dac = (defocus_grid_dac + input->info.af.af_focus_pos_dac) *
     ...
     AF_LOW("i %d defocus %d input->u.pdaf_info.defocus[i] %d phase_diff %f",
       i, input->u.pdaf_info.defocus[i].defocus,
       input->u.pdaf_info.defocus[i].df_conf_level,
       input->u.pdaf_info.defocus[i].phase_diff);
  }
  output->is_ready = TRUE;
  return TRUE;
}
