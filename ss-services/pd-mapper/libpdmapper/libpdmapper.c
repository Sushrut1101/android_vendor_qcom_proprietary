...
#define PD_LOCATER_DEBUG_PROP "persist.vendor.pd_locater_debug"
...
enum SR_Result_Enum servreg_get_domainlist(char *service, SR_DL_Handle *handle)
{
	...
	int qmi_notifier_initialized = 0;
	int qmi_notifier_released = 0;
	...
	rc = qmi_client_notifier_init(serviceObject, &osParams, &notifier);
	if (rc != QMI_NO_ERR) {
		if (debug_mode)
...
error:
	...
	// Only free notifier if we have not released it earlier
	if (qmi_notifier_initialized && (!qmi_notifier_released))
		qmi_client_release(notifier);
