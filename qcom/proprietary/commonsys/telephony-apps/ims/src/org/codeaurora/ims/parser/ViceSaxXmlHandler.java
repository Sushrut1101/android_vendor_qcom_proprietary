...
public class ViceSaxXmlHandler extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName,
        ...
        if (qName.equalsIgnoreCase(Element.VICE_LOCAL_TARGET) &&
                !mDoesRemoteDialogHaveSubElement) {
        ...
        if (qName.equalsIgnoreCase(Element.VICE_DIALOG_REMOTE)) {
            ...
            mDoesDialogHaveSubElement = true;
            mDoesRemoteDialogHaveSubElement = true;
        }
        ...
        if (qName.equalsIgnoreCase(Element.VICE_REMOTE_TARGET) &&
                mDoesRemoteDialogHaveSubElement) {
            mViceDialogRemoteTargetElement = new Element();
            mViceDialogRemoteTargetElement.setTagName(Element.VICE_REMOTE_TARGET);
            mViceDialogRemoteTargetElement.setParentTag(Element.VICE_DIALOG_REMOTE);
            mViceDialogRemoteTargetElement.setMapAttribute();
            mViceDialogRemoteSubList.add(mViceDialogRemoteTargetElement);
            mViceDialogRemoteTargetElement.setAttributValue(
                    Element.VICE_REMOTE_TARGET_URI,
                    attributes.getValue(Element.VICE_REMOTE_TARGET_URI));
            Log.d(LOGTAG, "Root Tag Name:" + mViceDialogRemoteTargetElement.getTagName());
            Log.d(LOGTAG, "attribute vals = " + mViceDialogRemoteTargetElement.getAttributeValue(
                    Element.VICE_REMOTE_TARGET_URI));
        }
    }
    ...
    public void endElement(String uri, String localName, String qName)
        ...
        if (mViceDialogInfoElement != null) {
            ...
            if(mDoesDialogHaveSubElement) {
                if (qName.equalsIgnoreCase(Element.VICE_DIALOG_SA)) {
                    ...
                    Log.d(LOGTAG, "VICE_DIALOG_REMOTE_IDENTITY attribute vals = "
                            + com.qualcomm.ims.utils.Log.pii(
            ...
            if (mDoesRemoteDialogHaveSubElement) {
                if (qName.equalsIgnoreCase(Element.VICE_DIALOG_REMOTE)) {
                    mDoesRemoteDialogHaveSubElement = false;
                }
            }
    ...
    public List<ImsViceParser.DialogInfo> dumpDialogInfo() {
        if (mDialogInfoList != null && !mDialogInfoList.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                final ImsViceParser.DialogInfo info = mDialogInfoList.get(i);
                Log.d(LOGTAG, "dumpDialogInfo[" + i + "] -> index "
                        + ", dialog ID = " + info.dialogId
                        + ", exclusive = " + info.exclusive
                        + ", direction = " +  info.direction
                        + ", state = " + info.state
                        + ", param name = " + info.paramName
                        + ", param val = " + info.paramVal
                        + ", remote identity = " + com.qualcomm.ims.utils.Log.pii(
                                info.remoteIdentity)
                        + ", local identity = " + com.qualcomm.ims.utils.Log.pii(
                                info.localIdentity));
                if (info.mediaAttributes != null &&
                        !info.mediaAttributes.isEmpty()) {
                    int mediaattri = info.mediaAttributes.size();
                    for (int j = 0; j < mediaattri; j++) {
                        Log.d(LOGTAG, "dumpDialogInfo[" + i + "] -> index "
                                + ", dialog media type = "
                                + info.mediaAttributes.get(j).mediaType
                                + ", dialog media direction = "
                                + info.mediaAttributes.get(j).mediaDirection
                                + ", dialog media port = "
                                + info.mediaAttributes.get(j).mediaPort
                        );
                    }
                }
            }
