    ...
    if (wms_arg_ind_msg->transfer_route_mt_message_valid == 1) 
    {
        if(wms_arg_ind_msg->transfer_route_mt_message.format == WMS_MESSAGE_FORMAT_CDMA_V01)/* CDMA */
        {
            ...
            if(wms_arg_ind_msg->mt_message_valid == 1)
            {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "\nMT_MESSAGE IS VALID\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "storage_type:%d, storage_index:%d\n", wms_arg_ind_msg->mt_message.storage_type, wms_arg_ind_msg->mt_message.storage_index);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            }
            else
            {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "MT_MESSAGE IS INVALID\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            }
            if(wms_arg_ind_msg->transfer_route_mt_message_valid == 1)
            {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "TRANSFER_ROUTE_MT_MESSAGE IS VALID\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "ack_indicator: %d\n", wms_arg_ind_msg->transfer_route_mt_message.ack_indicator);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "transaction_id: %d\n", wms_arg_ind_msg->transfer_route_mt_message.transaction_id);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "format:%d\n", wms_arg_ind_msg->transfer_route_mt_message.format);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);

                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "message_data :\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                for (i = 0; i < wms_arg_ind_msg->transfer_route_mt_message.data_len; i++)
                {
                    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "%02x\t", wms_arg_ind_msg->transfer_route_mt_message.data[i]);
                    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
                }
            ...
            {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "TRANSFER_ROUTE_MT_MESSAGE IS INVALID\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            }
            if(wms_arg_ind_msg->message_mode_valid == 1)
            {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "MESSAGE MODE VALID: %d\n", wms_arg_ind_msg->message_mode);
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            }
            else
            {
                snprintf(temp_char_array, TEMP_ARRAY_SIZE, "MESSAGE MODE INVALID\n");
                qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
            }
        }
        ...
        {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "MESSAGE MODE GW, NO PROCESS\n");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
        else if (wms_arg_ind_msg->transfer_route_mt_message.format == WMS_MESSAGE_FORMAT_MWI_V01)
        {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "MESSAGE MODE MWI, NO PROCESS\n");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
        else
        {
            snprintf(temp_char_array, TEMP_ARRAY_SIZE, "UNKNOWN MESSAGE MODE\n");
            qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    }
}
...
int qmi_simple_ril_mo_sms_req_callback (qmi_util_service_message_info* callback_msg_info,qmi_simple_ril_cmd_completion_info* uplink_message)
{
    if (uplink_message->error_code == QMI_NO_ERR || uplink_message->error_code == QMI_IDL_LIB_MISSING_TLV)
        ...
        {
        snprintf(temp_char_array, TEMP_ARRAY_SIZE, "ERROR in sending the MO_SMS Request, ERROR CODE:%d\n",uplink_message->error_code);
        qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
        }
    return 0;
}
...
static void wms_print_mo_sms_resp(wms_raw_send_resp_msg_v01* wms_arg_resp_msg,qmi_simple_ril_cmd_completion_info* uplink_message)
{
    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "MESSAGE ID %d\n", wms_arg_resp_msg->message_id);
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "RESULT CODE %d\n", wms_arg_resp_msg->resp.result);
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "CAUSE CODE VALID %d\n", wms_arg_resp_msg->cause_code_valid);
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "CAUSE_CODE %d\n", wms_arg_resp_msg->cause_code);
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "ERROR_CLASS_VALID %d\n", wms_arg_resp_msg->error_class_valid);
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
    snprintf(temp_char_array, TEMP_ARRAY_SIZE, "ERROR_CLASS %d\n", wms_arg_resp_msg->error_class);
    qmi_simple_ril_util_add_entry_to_info_set(uplink_message->info_set, temp_char_array);
}
