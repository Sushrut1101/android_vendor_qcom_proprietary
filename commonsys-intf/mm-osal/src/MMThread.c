...
{
/*
    In MM-OSAL all the pthreads created (via MM_Thread_CreateEx/MM_Thread_Create)
    are default joinable threads (setting PTHREAD_CREATE_JOINABLE in attributes).

    Currently pthread library enforces additional check on tid validity when
    calling pthread_detach. As per this check, calling pthread_detach on a valid
    (non-zero) thread tid that is not present in the process thread list will
    result in a fatal exception. Earlier, it simply used to return with ESRCH.

    Since threads created via MM-OSAL are joinable, MM_Thread_Join ensures that
    child thread resources are cleaned up and freed. This also removes the thread
    tid from the process thread list. Hence clients calling MM_Thread_Release at
    a later point of time after MM_Thread_Join results in crash due to this check
*/
    // This API should be no-op on Android/Linux
    return 0;
}
...
)
{
    ...
    {
        void* retval = NULL;
        status = pthread_join(phandle, &retval);
        if (pnExitCode)
        {
            *pnExitCode = (int)(long)(retval);
        }
    }
...
)
{
   int retValue = 1;
   pthread_t phandle = (pthread_t)handle;
   int rc = pthread_detach( phandle );
   if( 0 == rc  )
   {
      MM_MSG_PRIO( MM_GENERAL, MM_PRIO_LOW, "pthread_detach(): success");
      retValue = 0;
   }
   else
   {
      MM_MSG_PRIO1( MM_GENERAL, MM_PRIO_ERROR, "pthread_detach(): failed %d", rc);
   }
   return retValue;
}
