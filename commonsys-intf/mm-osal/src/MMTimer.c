...
#define TIME_MICROSEC_IN_SEC 1000000
#define TIME_NSEC_IN_MICROSEC 1000
                ...
                if (nResult == 0)
                {
                   ...
                   *pHandle = pTimer;
                   MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_FATAL,
                      "timer_create: timer_id %d", pTimer->m_timerId);
                }
                else
                {
                   MM_CriticalSection_Release(pTimer->m_timerLock);
                   pTimer->m_timerLock = NULL;
                   MM_Free(pTimer);
                }
            }
            else
...
)
{
    ...
    if ( pTimer )
    {
        ...
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_FATAL,
                      "timer_delete: timer_id %d", pTimer->m_timerId);
...
)
{
  ...
  if ( pTime )
  {
    ...
    microsec = ((unsigned long long)time.tv_sec * TIME_MICROSEC_IN_SEC) + ((unsigned long long)time.tv_nsec / TIME_NSEC_IN_MICROSEC);
