...
typedef enum seccam_if_destination_type_t {
    SECCAM_IF_DESTINATION_QTEE = 1,
    SECCAM_IF_DESTINATION_MLVM,
    SECCAM_IF_DESTINATION_MAX
} seccam_if_destination_type_t;
...
typedef seccam_if_notif_action_t (*secCamDoneSendFrameNotification_t)(
        uint64_t surface_id,
        uint32_t ret, seccam_if_frame_rsp_data_t* rsp,
        void* context,
 ...
 * @param[in]   destination     Secure execution environment for data processing.
 *
 * @return
 * OK(0)                  -- Success.
...
status_t secCamStartTzApp(const char* ta_name, int32_t buf_size,
        uint32_t* if_version_maj, uint32_t* if_version_min,
        seccam_if_destination_type_t destination);
...
/**
 * secCamStartUsecase() and secCamStopUsecase() are for
 * triggering start and stop for 2dfa usecase
 */
status_t secCamStartUsecase();

status_t secCamStopUsecase();
...
/**
 * Query to know if dual stream is required based on supported format type
 *
 * @return
 * true -- Success (Non Linear format is supported and dual stream is required)
 */
bool secCamIsDualStreamSupported();
...
/**
 * Release all pending surfaces on UnBind
 */
void secCamReleaseAllSecureSurface();
