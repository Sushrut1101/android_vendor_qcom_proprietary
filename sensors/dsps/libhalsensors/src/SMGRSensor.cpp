...
hal_sensor1_cb_t* SMGRSensor::getSMGRSensor1Cb()
{
    if (smgr_sensor1_cb == NULL) {
        ...
        pthread_cond_init(&(smgr_sensor1_cb->cb_cond), &cond_attr);
...
int SMGRSensor::batch(int flags, int64_t period_ns, int64_t timeout)
{
    ...
    HAL_LOG_INFO("%s:sensor(%s) handle:%d flags:0x%x period_ns %" PRId64 "",
    ...
    if (sample_rate  == freq && report_rate_Q16 == report_rate &&
        ...
        freq = sample_rate;
        if (getMaxBufferedSamples() == 0) {
            report_rate = FX_FLTTOFIX_Q16(sample_rate);
        } else {
...
int SMGRSensor::flush()
{
    ...
    else if (batching || getMaxBufferedSamples() > 0) {
        HAL_LOG_DEBUG("SMGRSensor::flush for batching");
        pthread_mutex_lock(&smgr_sensor1_cb->cb_mutex);
        err = SMGRReportAdd(freq, report_rate, batching, false);
        pthread_mutex_unlock(&smgr_sensor1_cb->cb_mutex);
        if (err != true) {
            HAL_LOG_ERROR("%s: Failed for handle %d @ samp %f Hz rpt %d Hz batched %d",
                             ...
                             sns_smgr_buffering_req_msg_v01 **buff_req)
{
    ...
    /* Request for accurate time stamps */
    (*buff_req)->Item[0].SampleQuality = SNS_SMGR_SAMPLE_QUALITY_ACCURATE_TIMESTAMP_V01;
    (*buff_req)->Item[1].SampleQuality = SNS_SMGR_SAMPLE_QUALITY_ACCURATE_TIMESTAMP_V01;
...
/*===========================================================================
  FUNCTION:  processRegResp
    Process the Response from registry service
    Parameters
    @msg_hdr : sensor1 message header
    @msg_ptr : sensor1 message data
===========================================================================*/
void SMGRSensor::processRegResp(Sensor** mSensors, sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    bool error = false;

    if( SNS_REG_SINGLE_READ_RESP_V02 == msg_hdr->msg_id ) {
        sns_reg_single_read_resp_msg_v02 *msgPtr =
                    (sns_reg_single_read_resp_msg_v02*) msg_ptr;
        uint8_t handle = msg_hdr->txn_id;
        if( 0 == msgPtr->resp.sns_result_t ) {
               SMGRSensor *pSMGRSnsHandle = static_cast<SMGRSensor *>(mSensors[handle]);
               if(pSMGRSnsHandle->reg_req_len > SNS_MAX_REGISTRY_REQUEST_SIZE) {
                   HAL_LOG_ERROR("%s: copylen %d more than the supported",
                       __FUNCTION__, pSMGRSnsHandle->reg_req_len);
                   error = true;
               } else {
                  memcpy(pSMGRSnsHandle->reg_value,
                            msgPtr->data,
                            pSMGRSnsHandle->reg_req_len);
               }
        } else {
             HAL_LOG_ERROR("%s: Received failed response err: %d",
                    __FUNCTION__, msgPtr->resp.sns_err_t);
             error = true;
        }
    } else {
        error = true;
        HAL_LOG_ERROR( "%s: Unhandled message id received: %i",
                ...
                sensor1_msg_header_s* msg_hdr)
{
    ...
    if (mSensors[HANDLE_MAGNETIC_FIELD_UNCALIBRATED_SAM] != NULL) {
        mSensors[HANDLE_MAGNETIC_FIELD_UNCALIBRATED_SAM]->flushSendCmplt();
    }
    if (mSensors[HANDLE_MAGNETIC_FIELD_UNCALIBRATED_SAM_WAKE_UP] != NULL) {
        mSensors[HANDLE_MAGNETIC_FIELD_UNCALIBRATED_SAM_WAKE_UP]->flushSendCmplt();
    }
    pthread_mutex_unlock(&data_cb->data_mutex );
    return;
}
...
void SMGRSensor::processBufferingInd(Sensor** mSensors, sns_smgr_buffering_ind_msg_v01* smgr_ind)
{
    ...
    * in the buffering request */
    ...
    if (mSensors[smgr_ind->ReportId]->getFlags() & SENSOR_FLAG_WAKE_UP) {
    ...
    HAL_LOG_DEBUG("%s: Samples_len=%d Items=%d max_reports_per_index=%d",
        ...
        (int32_t)max_reports_per_index);

    /* HAL would discard all events but not the latest event, only if the conditions below are met:
     * 1.   It is an on-change sensor (e.g. step counter, light, proximity, etc.)
     * 2.   It is NOT batching
     * 3.   Indication Message
                saying it is Last Sample with SNS_BATCH_LAST_IND_V01
                saying it is batch only indication with  SNS_BATCH_ONLY_IND_V01    */

    if ((0 < smgr_ind->Indices_len ) &&
            (smgr_ind->Indices_len <= SNS_SMGR_BUFFERING_REQUEST_MAX_ITEMS_V01)) {

        int handle = getHandleFromInd(smgr_ind->ReportId,
                     smgr_ind->Indices[smgr_ind->Indices_len-1].DataType,
                     smgr_ind->Indices[smgr_ind->Indices_len-1].SensorId);

        if (handle == -1 ) {
            HAL_LOG_ERROR(" %s: ReportId = %d  DataType = %d SensorId = %d ", __FUNCTION__,
                smgr_ind->ReportId, smgr_ind->Indices[smgr_ind->Indices_len-1].DataType,
                smgr_ind->Indices[smgr_ind->Indices_len-1].SensorId);
            if (mSensors[smgr_ind->ReportId]->getFlags() & SENSOR_FLAG_WAKE_UP) {
        ...
        HAL_LOG_DEBUG("%s: Handle=%d mSensors[handle]->isBatchingEnable()=%d",
            __FUNCTION__, handle, mSensors[handle]->isBatchingEnable());
        HAL_LOG_DEBUG("%s: Handle=%d mSensors[handle]->getFlags()=%d smgr_ind->IndType=%d",
            __FUNCTION__, handle, mSensors[handle]->getFlags(), smgr_ind->IndType);

        if (mSensors[handle] && (mSensors[handle]->isBatchingEnable() == false) &&
            (mSensors[handle]->getFlags() & SENSOR_FLAG_ON_CHANGE_MODE)) {
            ...
            for (j = 0; j < smgr_ind->Indices_len; j++) {
                for (i = 0; i < smgr_ind->Indices[j].SampleCount; i++) {
                    ts_offset[j] += smgr_ind->Samples[smgr_ind->Indices[j].
                        FirstSampleIdx+i].TimeStampOffset;
                }
                i = i - 1;
                report_msg.Item_len++;
                report_msg.Item[j].SensorId = smgr_ind->Indices[j].SensorId;
                report_msg.Item[j].DataType = smgr_ind->Indices[j].DataType;
                report_msg.Item[j].ItemData[0] = smgr_ind->Samples[smgr_ind->Indices[j].
                    FirstSampleIdx+i].Data[0];
                report_msg.Item[j].ItemData[1] = smgr_ind->Samples[smgr_ind->Indices[j].
                    FirstSampleIdx+i].Data[1];
                report_msg.Item[j].ItemData[2] = smgr_ind->Samples[smgr_ind->Indices[j].
                    FirstSampleIdx+i].Data[2];
                report_msg.Item[j].TimeStamp = (smgr_ind->Indices[j].FirstSampleTimestamp
                    + ts_offset[j]);
                report_msg.Item[j].ItemFlags = smgr_ind->Samples[smgr_ind->Indices[j].
                    FirstSampleIdx+i].Flags;
                report_msg.Item[j].ItemQuality = smgr_ind->Samples[smgr_ind->Indices[j].
                    FirstSampleIdx+i].Quality;
                report_msg.Item[j].ItemSensitivity = 0;
            }

            if ((SNS_BATCH_LAST_IND_V01 == smgr_ind->IndType) ||
                (SNS_BATCH_ONLY_IND_V01 == smgr_ind->IndType)) {
                HAL_LOG_DEBUG("%s:report(on_change_sensor) eventtype %d",
                 ...
                 * but the only side effect is an incorrect debug print */
                 ...
                 for (j = 0; j < smgr_ind->Indices_len; j++) {
                     if (i < smgr_ind->Indices[j].SampleCount) {
                         ...
                         report_msg.Item[j].ItemData[2] = smgr_ind->Samples[smgr_ind->Indices[j].
                             FirstSampleIdx+i].Data[2];
                         ts_offset[j] += smgr_ind->Samples[smgr_ind->Indices[j].FirstSampleIdx+i].
                             TimeStampOffset;
                         report_msg.Item[j].TimeStamp = (smgr_ind->Indices[j].FirstSampleTimestamp
                             + ts_offset[j]);
                         report_msg.Item[j].ItemFlags = smgr_ind->Samples[smgr_ind->Indices[j].
...
void SMGRSensor::processReportInd(Sensor** mSensors, sns_smgr_periodic_report_ind_msg_v01* smgr_ind)
{
    ...
    bool bWakeUpFlag = (mSensors[smgr_ind->ReportId]->getFlags() & SENSOR_FLAG_WAKE_UP);
    ...
    HAL_LOG_VERBOSE("%s: Id: %s: Ty: %u Q: %u", __FUNCTION__,
        ((smgr_data->SensorId==SNS_SMGR_ID_ACCEL_V01)                 ? "ACCEL_DATA" :
         ...
         (smgr_data->SensorId==SNS_SMGR_ID_RGB_V01)                   ? "RBG_CCT_DATA" :
         ...
         (smgr_data->SensorId==SNS_SMGR_ID_ULTRA_VIOLET_V01)          ? "UV_DATA" :
    ...
    if (mSensors[handle] != NULL) {
     ...
     if ((handle == HANDLE_MAGNETIC_FIELD_UNCALIBRATED || handle == HANDLE_MAGNETIC_FIELD_UNCALIBRATED_WAKE_UP)
          && smgr_ind->Item_len < 2) {
        HAL_LOG_ERROR("%s: Magnetic sensor events with invalid length", __FUNCTION__);
        goto error;
     }
    ...
    * sensor events. If the events are identical, don't report them */
    ...
    if (Utility::insertQueue(&sensor_data, bWakeUpFlag, data_cb)) {
...
/*===========================================================================
  FUNCTION:  isRawDataMode
    Check Raw Data mode status using registry service
===========================================================================*/
bool SMGRSensor::isRawDataMode()
{
    bool resp = false;
    if( registryReadReq( SNS_REG_ITEM_RAW_DATA_MODE_V02, 1 )
      == false ) {
        HAL_LOG_ERROR("%s: Unable to send rdm read message", __FUNCTION__);
        return false;
    }
    resp = Utility::waitForResponse(TIME_OUT_MS,
            &smgr_sensor1_cb->cb_mutex,
    ...
    HAL_LOG_DEBUG("%s: Received Response: %d", __FUNCTION__, smgr_sensor1_cb->error);
    if(smgr_sensor1_cb->error) {
        HAL_LOG_ERROR("%s: Error in rdm message response", __FUNCTION__);
        return false;
    }
    return ( reg_value[0] == 1 ) ? true : false;
}

/*===========================================================================
  FUNCTION:  registryReadReq
    Send commad to registry service to read any registry item
    Parameters
    @reg_item_id : Registry value
    @len          : length of the item to be read
===========================================================================*/
bool SMGRSensor::registryReadReq(uint16_t reg_item_id, uint8_t len )
{
    sensor1_error_e                 error;
    sensor1_msg_header_s            msgHdr;
    sns_reg_single_read_req_msg_v02 *msg_ptr = NULL;

    if(len > SNS_MAX_REGISTRY_REQUEST_SIZE) {
        HAL_LOG_ERROR("%s: len %d more than the supported",
            __FUNCTION__, len);
        return false;
    }
    ...
    error = sensor1_alloc_msg_buf( smgr_sensor1_cb->sensor1_handle,
         ...
         (void**)&msg_ptr );
    if( SENSOR1_SUCCESS != error ) {
        HAL_LOG_ERROR("%s: sensor1_alloc_msg_buf returned(get) %d", __FUNCTION__, error);
        return false;
    }
    ...
    msgHdr.txn_id = handle;

    /* Create the message */
    msg_ptr->item_id = reg_item_id;

    smgr_sensor1_cb->error = false;
    error = sensor1_write( smgr_sensor1_cb->sensor1_handle, &msgHdr, msg_ptr );
    if( SENSOR1_SUCCESS != error ) {
                     ...
                     void *msg_ptr)
{
    ...
    case SENSOR1_MSG_TYPE_RESP:
        ...
        } else if ( msg_hdr->service_number
                            == SNS_REG2_SVC_ID_V01 ) {
            SMGRSensor::processRegResp(mSensors, msg_hdr, msg_ptr );
        }
        break;
    ...
    case SENSOR1_MSG_TYPE_BROKEN_PIPE:
        ...
        if (smgr_sensor1_cb != NULL) {
            ...
            HAL_LOG_ERROR("%s: smgr_sensor1_cb is NULL!", __FUNCTION__);
            return;
        }
        break;
...
int getHandleFromInd (int ReportId, int DataType, int SensorId) {
    ...
    case SNS_SMGR_ID_ACCEL_V01:
        if (SNS_SMGR_DATA_TYPE_PRIMARY_V01 == DataType) {
            ...
            if (ReportId == HANDLE_ACCELERATION_UNCALIBRATED) {
                handle = HANDLE_ACCELERATION_UNCALIBRATED;
            }  else if (ReportId == HANDLE_ACCELERATION_UNCALIBRATED_WAKE_UP) {
                handle = HANDLE_ACCELERATION_UNCALIBRATED_WAKE_UP;
            }  else if (ReportId == HANDLE_ACCELERATION_WAKE_UP) {
                handle = HANDLE_ACCELERATION_WAKE_UP;
            }
        }
        break;
    case SNS_SMGR_ID_MAG_V01:
        if (ReportId == HANDLE_MAGNETIC_FIELD_UNCALIBRATED_SAM) {
            /* the data is reported by the HANDLE_MAGNETIC_FIELD_SAM */
            handle = HANDLE_MAGNETIC_FIELD_UNCALIBRATED_SAM;
        }
        else if (ReportId == HANDLE_MAGNETIC_FIELD_UNCALIBRATED) {
            handle = HANDLE_MAGNETIC_FIELD_UNCALIBRATED;
        }
        else if (ReportId == HANDLE_MAGNETIC_FIELD_UNCALIBRATED_WAKE_UP) {
            handle = HANDLE_MAGNETIC_FIELD_UNCALIBRATED_WAKE_UP;
        }
        else if (ReportId == HANDLE_MAGNETIC_FIELD) {
            handle = HANDLE_MAGNETIC_FIELD;
        }
        else if (ReportId == HANDLE_MAGNETIC_FIELD_WAKE_UP) {
    ...
    case SNS_SMGR_ID_PROX_LIGHT_V01:
        if(SNS_SMGR_DATA_TYPE_PRIMARY_V01 == DataType) {
            ...
            if(ReportId == HANDLE_PROXIMITY_NON_WAKE_UP)
                handle = HANDLE_PROXIMITY_NON_WAKE_UP;
        } else if (SNS_SMGR_DATA_TYPE_SECONDARY_V01 == DataType) {
            if(ReportId == HANDLE_LIGHT)
                handle = HANDLE_LIGHT;
            if(ReportId == HANDLE_LIGHT_WAKE_UP)
                handle = HANDLE_LIGHT_WAKE_UP;
        }
        break;
    case SNS_SMGR_ID_GYRO_V01:
        if (ReportId == HANDLE_GYRO_UNCALIBRATED) {
            handle = HANDLE_GYRO_UNCALIBRATED;
        }  else if(ReportId == HANDLE_GYRO_UNCALIBRATED_WAKE_UP) {
            handle = HANDLE_GYRO_UNCALIBRATED_WAKE_UP;
        }  else if(ReportId == HANDLE_GYRO) {
            handle = HANDLE_GYRO;
        }  else if(ReportId == HANDLE_GYRO_WAKE_UP) {
            handle = HANDLE_GYRO_WAKE_UP;
        }
        break;
    case SNS_SMGR_ID_PRESSURE_V01:
        if (SNS_SMGR_DATA_TYPE_PRIMARY_V01 == DataType) {
            if(ReportId == HANDLE_PRESSURE)
                handle = HANDLE_PRESSURE;
            if(ReportId == HANDLE_PRESSURE_WAKE_UP)
                handle = HANDLE_PRESSURE_WAKE_UP;
        }
        break;
    case SNS_SMGR_ID_HUMIDITY_V01:
        if (SNS_SMGR_DATA_TYPE_PRIMARY_V01 == DataType) {
            if(ReportId == HANDLE_RELATIVE_HUMIDITY)
                handle = HANDLE_RELATIVE_HUMIDITY;
            if(ReportId == HANDLE_RELATIVE_HUMIDITY_WAKE_UP)
                handle = HANDLE_RELATIVE_HUMIDITY_WAKE_UP;
        }
        else if (SNS_SMGR_DATA_TYPE_SECONDARY_V01 == DataType) {
            if(ReportId == HANDLE_AMBIENT_TEMPERATURE)
                handle = HANDLE_AMBIENT_TEMPERATURE;
            if(ReportId == HANDLE_AMBIENT_TEMPERATURE_WAKE_UP)
                handle = HANDLE_AMBIENT_TEMPERATURE_WAKE_UP;
        }
        break;
    case SNS_SMGR_ID_STEP_COUNT_V01:
        if (SNS_SMGR_DATA_TYPE_PRIMARY_V01 == DataType) {
            if(ReportId == HANDLE_SMGR_STEP_COUNT)
               handle = HANDLE_SMGR_STEP_COUNT;
            if(ReportId == HANDLE_SMGR_STEP_COUNT_WAKE_UP)
    ...
    case SNS_SMGR_ID_STEP_EVENT_V01:
        if (SNS_SMGR_DATA_TYPE_PRIMARY_V01 == DataType) {
            if(ReportId == HANDLE_SMGR_STEP_DETECTOR)
                handle = HANDLE_SMGR_STEP_DETECTOR;
            if(ReportId == HANDLE_SMGR_STEP_DETECTOR_WAKE_UP)
                handle = HANDLE_SMGR_STEP_DETECTOR_WAKE_UP;
        }
        break;
    case SNS_SMGR_ID_GAME_ROTATION_VECTOR_V01:
        if (SNS_SMGR_DATA_TYPE_PRIMARY_V01 == DataType) {
            if(ReportId == HANDLE_SMGR_GAME_RV)
                handle = HANDLE_SMGR_GAME_RV;
            if(ReportId == HANDLE_SMGR_GAME_RV_WAKE_UP)
    ...
    case SNS_SMGR_ID_OBJECT_TEMP_V01:
        ...
        break;
    default:
        HAL_LOG_ERROR("No such ID %d, ", SensorId);
        handle = -1;
    }
