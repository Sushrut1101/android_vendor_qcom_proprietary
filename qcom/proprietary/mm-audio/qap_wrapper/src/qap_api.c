...
#define DDP_LIB_NAME "libdolbyddpdec.so"
#define MAT_LIB_NAME "libdolbymatdec.so"
#define TH_LIB_NAME "libdolbytruehddec.so"
#define DAP_LIB_NAME "libdolbydap.so"
#define OAR_LIB_NAME "libdolbyoar.so"
...
qap_lib_handle_t qap_load_library(const char* lib_name)
{
    ...
    if (strstr(lib_name, MS12_LIB_NAME)|| strstr(lib_name, DDP_LIB_NAME)
        || strstr(lib_name, MAT_LIB_NAME)||strstr(lib_name, TH_LIB_NAME)
        || strstr(lib_name, DAP_LIB_NAME)||strstr(lib_name, OAR_LIB_NAME)) {
                            ...
                            void *reply_data)
{
    ...
    uint32_t *param_data = (uint32_t *) cmd_data;
    uint32_t param_id = 0;
    uint32_t data = 0;
                                      ...
                                      void* priv_data)
{
    ...
    return session->audio_module->audio_session_set_callback(
                                                  session->session_handle, cb, priv_data);
}

qap_status_t qap_lib_set_log_callback(qap_lib_handle_t lib_handle,
                                      qap_log_func_t func)
{
    qap_wrapper_lib_handle_t *qap_wrapper_hdl;
    audio_module_lib_interface_t *audio_module = NULL;
    const char *sym_str = AUDIO_MODULE_LIBRARY_INFO_SYM_AS_STR;
    ...
    audio_module = dlsym(qap_wrapper_hdl->lib_handle, sym_str);
    if (audio_module == NULL) {
        ALOGE("%s:: unable to find the symbol %s", __func__, sym_str);
        return QAP_STATUS_ERR;
    }

    if (audio_module->audio_lib_set_log_callback) {
        audio_module->audio_lib_set_log_callback(func);
    }

    return QAP_STATUS_OK;
}

qap_status_t qap_lib_set_log_level(qap_lib_handle_t lib_handle,
                                       qap_log_level_t level)
{
    qap_wrapper_lib_handle_t *qap_wrapper_hdl;
    audio_module_lib_interface_t *audio_module = NULL;
    const char *sym_str = AUDIO_MODULE_LIBRARY_INFO_SYM_AS_STR;
    ...
    if (audio_module->audio_lib_set_log_level) {
        audio_module->audio_lib_set_log_level(level);
    }
...
int qap_module_process_sync(qap_module_handle_t module_handle,
                               qap_audio_buffer_t *input_buffer, qap_audio_buffer_t *output_buffer)
{
    qap_module_t *qap_handle;
    qap_wrapper_session_t *session = NULL;
    ...
    return session->audio_module->audio_module_process_sync(
                                      qap_handle->module_handle, input_buffer, output_buffer);
}
                                    ...
                                    void* priv_data)
{
    ...
    return session->audio_module->audio_module_set_callback(
                                        qap_mod_handle->module_handle,
                                        cb,
                                        priv_data);
}
