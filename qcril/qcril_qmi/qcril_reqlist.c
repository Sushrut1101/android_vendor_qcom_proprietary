...
static qcril_reqlist_buf_type* qcril_reqlist_find_following_blocked_request_buf(const qcril_reqlist_buf_type* buf_ptr)
{
  ...
  if ( NULL != tmp )
  {
    QCRIL_LOG_INFO("for request %d (token id %" PRId32 " (%p)), find the blocked request %d (token id %" PRId32 " (%p))",
                   buf_ptr->pub.request, buf_ptr->pub.t_id, buf_ptr->pub.t, tmp->pub.request, tmp->pub.t_id, tmp->pub.t);
  }
  else if ( NULL != buf_ptr )
  {
    QCRIL_LOG_INFO("for request %d (token id %" PRId32 " (%p)), didn't find the blocked request", buf_ptr->pub.request, buf_ptr->pub.t_id, buf_ptr->pub.t);
  }
  else
  {
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    QCRIL_LOG_ERROR( "[RID %d] Fail to add ReqList entry with token id %" PRId32 " (%p) (already exists)",
                     instance_id, buf_ptr->pub.t_id, buf_ptr->pub.t );
...
*/
/*=========================================================================*/
IxErrnoType qcril_reqlist_get_entry_from_request_range
(
  qcril_instance_id_e_type instance_id,
  int request_min,
  int request_max,
...
)
{
  ...
  while ( buf_ptr != NULL  )
  {
    if ( ( buf_ptr->pub.request > request_min ) &&
         ( buf_ptr->pub.request < request_max ) )
    {
      QCRIL_LOG_INFO( "[RID %d] Found ReqList entry : %s(%d), token id %" PRId32 " (%p)",
  ...
  if ( status != E_SUCCESS )
  {
    QCRIL_LOG_INFO("[RID %d] No requests in ReqList: range %d - %d", instance_id, request_min, request_max);
  }
  return status;
...
static IxErrnoType qcril_reqlist_remove_request_pub_from_reqlist(qcril_reqlist_buf_type *buf_ptr, qcril_instance_id_e_type instance_id)
{
  ...
  if ( NULL != buf_ptr )
  {
    ...
    QCRIL_LOG_INFO( "[RID %d] Deleted ReqList entry : token id %" PRId32 " [%p]", instance_id, buf_ptr->pub.t_id, buf_ptr->pub.t );
  ...
  if ( buf_ptr != NULL )
  {
    ...
    else
    {
      ...
      QCRIL_LOG_INFO( "[RID %d] Update ReqList entry : %s(%d), token id %" PRId32 " (%p), state %s(%d)",
                      instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                      buf_ptr->pub.t_id, buf_ptr->pub.t, qcril_reqlist_lookup_state_name( state ), state );
    }
  }
  else
  {
  ...
  if ( buf_ptr != NULL )
  {
    ...
    for ( i = 0; i < QCRIL_ARB_MAX_MODEMS; i++ )
    {
      QCRIL_LOG_DEBUG( "[RID %d] %s(%d) token id %" PRId32 " (%p), MID %d, State %s(%d)",
                       instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
                       buf_ptr->pub.t_id, buf_ptr->pub.t, i, qcril_reqlist_lookup_state_name( buf_ptr->pub.state[ i ] ),
                       buf_ptr->pub.state[ i ] );
    ...
    if ( status == E_SUCCESS )
    {
      QCRIL_LOG_DEBUG( "[RID %d] Complete all AMSS events, ReqList entry : %s(%d), token id %" PRId32 " (%p), result %d",
                       instance_id, qcril_log_lookup_event_name( buf_ptr->pub.request ), buf_ptr->pub.request,
