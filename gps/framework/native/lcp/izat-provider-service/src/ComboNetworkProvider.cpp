...
// QNP fix from modem will be informed to GNSS adapter
// and fix with latitude/longitude differs less than threshold
// (defined in MODEM_CPI_BLOCK_LAT_LON_DIFF_THRESHOLD_DEG) will not be
// injected for the interval (defined in MODEM_CPI_BLOCK_INTERVAL_MSEC).
#define MODEM_CPI_BLOCK_INTERVAL_MSEC  500
#define MODEM_CPI_BLOCK_LAT_LON_DIFF_THRESHOLD_DEG 0.00001
...
void ComboNetworkProvider::reportError(const IZatLocationError* /*error*/,
        const ILocationProvider* /*providerSrc*/)
{
}
...
void ComboNetworkProvider::reportBestLocation(const LocationReport & report)
{
    ...
    do {
        ...
        // If this position is a propagated fix (already was reported) or
        // if this position is coming from modem, then we need to report to GNSS
        // HAL layer to block the CPI injection from network provider
        if ((report.mIsPropagatedFix || (true == report.mHasProcessorSource &&
            LocationReport::ProcessorSrc_Modem == report.mProcessorSource)) &&
            (nullptr != getGnssInterface()) &&
            (nullptr != getGnssInterface()->blockCPI)) {

            getGnssInterface()->blockCPI(report.mLatitude,
                                         report.mLongitude,
                                         report.mHorizontalAccuracy,
                                         MODEM_CPI_BLOCK_INTERVAL_MSEC,
                                         MODEM_CPI_BLOCK_LAT_LON_DIFF_THRESHOLD_DEG);
        }
...
typedef const GnssInterface* (HIDL_FETCH_IGnssInterface_t)();
const GnssInterface* ComboNetworkProvider::getGnssInterface() {

    static bool getGnssInterfaceFailed = false;
    static const GnssInterface* gnssInterface = nullptr;

    if (nullptr == gnssInterface && !getGnssInterfaceFailed) {
        LOC_LOGd("Loading libgnss.so::getGnssInterface ...");
        HIDL_FETCH_IGnssInterface_t* getter = nullptr;
        ...
        if (nullptr == handle) {
            ...
            getter = (HIDL_FETCH_IGnssInterface_t*)dlsym(handle, "getGnssInterface");
        ...
        if (nullptr == getter) {
            ...
            gnssInterface = (const GnssInterface*)(*getter)();
        }
    }
