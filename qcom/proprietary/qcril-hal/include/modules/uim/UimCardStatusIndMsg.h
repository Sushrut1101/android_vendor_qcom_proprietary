...
} qcril_uim_card_status_e_type;

/*! @brief Card status for a Slot
*/
typedef struct
{
  uint8_t slot;                              /* Slot where the card resides */
  qcril_uim_card_status_e_type status;       /* Status of the card */
} qcril_uim_card_info_type;

class UimCardStatusIndMsg : public UnSolicitedMessage,
                            public add_message_id<UimCardStatusIndMsg>
{
  private:
    qcril_uim_card_info_type       mCardInfo;
  ...
  public:
    ...
    inline UimCardStatusIndMsg(const qcril_uim_card_info_type  &card_info):
    ...
    inline  const qcril_uim_card_info_type &get_card_status(void)
    {
      return mCardInfo;
    }
};
