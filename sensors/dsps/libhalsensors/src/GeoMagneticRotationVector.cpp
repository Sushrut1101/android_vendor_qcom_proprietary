/*============================================================================
  @file GeoMagneticRotationVector.cpp

  @brief
  GeoMagneticRotationVector class implementation.
...
/*============================================================================
  GeoMagneticRotationVector Constructor
============================================================================*/
GeoMagneticRotationVector::GeoMagneticRotationVector(int handle)
    :SAMSensor(handle)
{
    if(getAttribOK() == true) {
        ...
        setName("GeoMagnetic Rotation Vector");
        setVendor("QTI");
        setType(SENSOR_TYPE_GEOMAGNETIC_ROTATION_VECTOR);

        if(handle == HANDLE_GEOMAGNETIC_ROTATION_VECTOR_WAKE_UP){
            bWakeUp = true;
            setName("GeoMagnetic Rotation Vector -Wakeup");
            setFlags(SENSOR_FLAG_CONTINUOUS_MODE|SENSOR_FLAG_WAKE_UP);
        } else if (handle == HANDLE_GEOMAGNETIC_ROTATION_VECTOR) {
            bWakeUp = false;
...
/*============================================================================
  GeoMagneticRotationVector Destructor
============================================================================*/
...
/*============================================================================
  enable
============================================================================*/
int GeoMagneticRotationVector::enable(int en)
{
    int err;
    ...
    if (enabled == en) {
        HAL_LOG_INFO("GeoMagneticRotationVector is already enabled/disabled %d", enabled);
        return 0;
    }
...
int GeoMagneticRotationVector::sendBatchReq()
{
    sensor1_error_e  error;
    sensor1_msg_header_s req_hdr;
...
===========================================================================*/
void GeoMagneticRotationVector::processResp(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
...
/*===========================================================================
  FUNCTION:  processInd
===========================================================================*/
void GeoMagneticRotationVector::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if(bWakeUp == false) {
            ...
            la_sample.sensor = HANDLE_GEOMAGNETIC_ROTATION_VECTOR_WAKE_UP;
        ...
        la_sample.data[4] =
            ( curr_sample->accuracy > 0 && curr_sample->accuracy < 4 )
              ? SNS_ROT_VEC_HEAD_ERR_EST_RAD / curr_sample->accuracy
              : M_PI;
        ...
        HAL_LOG_VERBOSE( "%s: GEO MAG RV: [0]: %f [1]: %f [2]: %f acc: %d SAM TS: %u HAL TS:% " PRId64 " elapsedRealtimeNano:%" PRId64 "",
