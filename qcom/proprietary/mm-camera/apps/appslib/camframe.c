  ...
  if(enabled == 3 /*both preview & video*/ ||
    ...
    if( skip_cnt % skip_mode == 0) {
      if (frameCnt >= 0 && frameCnt <= frm_num) {
        if(is_video) {
          snprintf(buf, BUFF_SIZE_128, QCAMERA_DUMP_FRM_LOCATION"%d_v.yuv", frameCnt);
        } else {
          snprintf(buf, BUFF_SIZE_128, QCAMERA_DUMP_FRM_LOCATION"%d.yuv", frameCnt);
        }
...
void *cam_frame_v4l2(void *data)
{
  ...
  display_alloc.heap_id_mask = (0x1 << CAMERA_ION_HEAP_ID | 0x1 << ION_IOMMU_HEAP_ID);
...
int cam_get_snapshot_images(void *data, cam_ctrl_dimension_t *dimension)
{
  ...
  t_alloc.len = lengthT;
  t_alloc.flags = 0;
  t_alloc.heap_id_mask = (0x1 << CAMERA_ION_HEAP_ID | 0x1 << ION_IOMMU_HEAP_ID);
  t_alloc.align = 4096;
  ...
  s_alloc.len = lengthS;
  s_alloc.flags = 0;
  s_alloc.heap_id_mask = (0x1 << CAMERA_ION_HEAP_ID | 0x1 << ION_IOMMU_HEAP_ID);
  s_alloc.align = 4096;
