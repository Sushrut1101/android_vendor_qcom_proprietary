...
#define ESOC_SOC_LINK_INFO_NODE "esoc_link_info"
...
#define RET_SUBSYS_UNSUPPORTED 3

/* Modem prefixed with "SDX" are all supported */
#define SDXFUSION_MODEM_PREFIX "SDX"

int is_sdx(char *soc_name)
{
	return (!strncmp(soc_name, SDXFUSION_MODEM_PREFIX, strlen(SDXFUSION_MODEM_PREFIX)));
}
...
static int get_subsys_string(char *path, char *buf, int buf_size, int log_err)
{
	return RET_FAILED;
}
...
/*
 * List of supported legacy fusion modems
 */
#define NUM_LEGACY_MODEMS 6
const char *legacy_modems[NUM_LEGACY_MODEMS] = {
	"QSC",
	"MDM9x15",
	"MDM9x25",
	"MDM9x35",
...
/*
 * Given the name of a node under the root esoc dir, return
 * a pointer to a buffer containing the 'additional info' field
 * of the transport link connecting the ESOC represented by the node
 * to the APQ. This buffer should be freed after use.
 */
char *get_soc_extra_link_info(char *esoc_dev_node)
{
	char soc_link_node_path[MAX_PATH_LEN];
	char *soc_link = (char*)malloc(MAX_SOC_LINK_INFO_LEN * sizeof(char));
	if (!soc_link) {
		ALOGE("Failed to allocate memory for soc link");
		goto error;
	}
	memset((void*)soc_link, '\0', MAX_SOC_LINK_INFO_LEN * sizeof(char));
	snprintf(soc_link_node_path,
			sizeof(soc_link_node_path),
			"%s/%s/%s",
			ESOC_ROOT_DIR,
			esoc_dev_node,
			ESOC_SOC_LINK_INFO_NODE);
	if (get_subsys_string(soc_link_node_path,
				soc_link,
				MAX_SOC_LINK_INFO_LEN * sizeof(char),
				0) !=
			RET_SUCCESS) {
		goto error;
	}
...
int esoc_supported(char *esoc_node)
{
	...
	for (i = 0; i < NUM_LEGACY_MODEMS; i++) {
		if (!strncmp(soc_name, legacy_modems[i],
					MAX_SOC_NAME_LEN)) {
			free(soc_name);
			return RET_SUCCESS;
		}
...
int get_esoc_details(struct mdm_info *dev, char *esoc_dev_name)
{
	...
	free(temp);
	//Extra link info
	temp = get_soc_extra_link_info(esoc_dev_name);
	if (!temp) {
		//Extra link info is a optional field..Just copy N/A into it
		//if it was not accessible,empty, etc
		strlcpy(dev->mdm_link_extra_data, "N/A",
				sizeof(dev->mdm_link_extra_data));
	} else {
		strlcpy(dev->mdm_link_extra_data, temp,
				sizeof(dev->mdm_link_extra_data));
		free(temp);
	}
	//Ramdump path
...
int get_subsystem_info(struct mdm_info *dev, char *dev_name)
{
	int subsys_supported = 0;
	if (!dev || !dev_name) {
		ALOGE("Invalid argument passed to get_subsystem_info");
		return RET_FAILED;
	}
	if (dev_name[0] != '\0' && dev_name[strlen(dev_name)-1] == '\n')
		dev_name[strlen(dev_name)-1] = '\0';
	if (!strncmp(dev_name, SENSOR_DEV_NAME, MAX_NAME_LEN)) {
		dev->type = SUBSYS_TYPE_SENSOR;
		strlcpy(dev->mdm_link, "n/a", sizeof(dev->mdm_link));
		snprintf(dev->ram_dump_path,
				sizeof(dev->ram_dump_path),
				"N/A");
		subsys_supported = 1;
	} else if (!strncmp(dev_name, INTERNAL_MDM_DEV_NAME, MAX_NAME_LEN)) {
		...
		subsys_supported = 1;
	} else if (!strncmp(dev_name, SPSS_DEV_NAME, MAX_NAME_LEN)) {
		strlcpy(dev->mdm_link, LINK_SMD, sizeof(dev->mdm_link));
		snprintf(dev->ram_dump_path,
				sizeof(dev->ram_dump_path),
				"%s/%s",
				RAMDUMP_ROOT_DIR, dev_name);
		dev->type = SUBSYS_TYPE_SPSS;
		subsys_supported = 1;
	}
	if (subsys_supported) {
...
int get_system_info(struct dev_info *dev)
{
	...
	dev->num_modems = 0;
	dev->num_additional_subsystems = 0;
	...
	while((de = readdir(dir_ssr_bus))) {
		...
		//Keeping this check here in order to mantain backword
		//compatibility.Ideally all subsystems should be part of
		//the same array in the dev_info structure but in order
		//to reduce code churn from other modules just keeping
		//modems and other subsystem separate
		if(!strncmp(subsys_name, "modem", 5)) {
			if (dev->num_modems >= MAX_SUPPORTED_MDM) {
				ALOGE("Too many mdm's detected");
				goto error;
			}
			if( get_subsystem_info(&(dev->mdm_list[dev-> \
						num_modems]),
			...
			if (dev->num_additional_subsystems >=
					MAX_SUPPORTED_SUBSYSTEMS) {
				ALOGE("Too many additional subsystems..");
				goto error;
			}
			rcode = get_subsystem_info(&(dev->subsystem_list[dev-> \
						num_additional_subsystems]),
					subsys_name);
			if (rcode == RET_FAILED){
				ALOGE("Failed to get information on : %s",
						subsys_name);
				goto error;
			} else {
				if (rcode != RET_SUBSYS_UNSUPPORTED)
					dev->num_additional_subsystems++;
			}
		}
	}
