...
============================================================================*/
Gestures::Gestures(int handle)
    :SAMSensor(handle)
{
    if(getAttribOK() == true) {
        svc_num = SNS_SAM_BASIC_GESTURES_SVC_ID_V01;
        trigger_mode = SENSOR_MODE_EVENT;
        HAL_LOG_INFO("%s: handle:%d", __FUNCTION__, handle);
        setName("Basic Gestures");
        setVendor("QTI");
        setType(SENSOR_TYPE_BASIC_GESTURES);
        setFlags(SENSOR_FLAG_ON_CHANGE_MODE|SENSOR_FLAG_WAKE_UP);
        setMaxRange(7); /* range of enum values */
        setResolution(1);
...
int Gestures::enable(int en)
{
    ...
    if (enabled == en) {
        HAL_LOG_INFO("GESTURES is already enabled/disabled %d", enabled);
        return 0;
    }
...
===========================================================================*/
void Gestures::processResp(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    if(true != error ) {
        ...
        case SNS_SAM_BASIC_GESTURES_ENABLE_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_BASIC_GESTURES_ENABLE_RESP_V01", __FUNCTION__);
            instance_id = ((sns_sam_basic_gestures_enable_resp_msg_v01 *)msg_ptr)->instance_id;
            break;
        case SNS_SAM_BASIC_GESTURES_CANCEL_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_BASIC_GESTURES_CANCEL_RESP_V01", __FUNCTION__);
            /* Reset instance ID */
            instance_id = 0xFF;
            break;
        case SNS_SAM_BASIC_GESTURES_GET_ATTRIBUTES_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_BASIC_GESTURES_GET_ATTRIBUTES_RESP_V01", __FUNCTION__);
...
/*===========================================================================
  FUNCTION:  processInd
===========================================================================*/
void Gestures::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    switch(msg_hdr->msg_id) {
        case SNS_SAM_BASIC_GESTURES_REPORT_IND_V01:
            HAL_LOG_DEBUG("%s: SNS_SAM_BASIC_GESTURES_REPORT_IND_V01", __FUNCTION__);
        ...
        case SNS_SAM_BASIC_GESTURES_ERROR_IND_V01:
            HAL_LOG_ERROR("%s: SNS_SAM_BASIC_GESTURES_ERROR_IND_V01", __FUNCTION__);
            error = true;
            break;
        default:
    ...
    if (error == false) {
        ...
        HAL_LOG_VERBOSE("%s: GESTURES: %f SAM TS: %u HAL TS:%" PRId64 " elapsedRealtimeNano:%" PRId64 "",
