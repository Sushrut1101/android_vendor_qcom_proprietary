...
hal_ts_offset_is( uint32_t timestamp_ssc, uint64_t timestamp_ap )
{
  ...
  HAL_LOG_DEBUG("%s: Apps: %" PRIu64"; DSPS: %u; Offset : %" PRId64 " (diff %" PRId64 " @%d)",
                __FUNCTION__,  timestamp_ap, timestamp_ssc, g_time_control->timestamp_offset_apps_dsps,
...
hal_timestamp_calc( uint64_t dsps_timestamp, uint32_t activity )
{
  ...
  if( g_time_control->timestamp_last_sent[ activity ] > rv &&
      llabs(g_time_control->timestamp_last_sent[ activity ] - rv) < TS_CORRECT_THRESH )
  {
  ...
  if( NULL == g_time_control->sensor1_hndl )
  {
    ...
    else
    {
      ...
      else
      {
        ...
        g_time_control->is_resp_arrived = false;
