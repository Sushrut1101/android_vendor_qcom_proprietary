...
#define list_for_each_entry(type, entry, pMap, field) \
...
#define list_for_each_entry_safe(type, entry, pTemp, pMap, field) \
        for( entry = list_get_node_data(list_get_head(pMap), type, field),\
        pTemp =list_get_node_data(list_get_next_node(&entry->field), type, field);\
        &entry->field != pMap ; \
        entry = pTemp, \
        pTemp = list_get_node_data(list_get_next_node(&entry->field), type, field))
