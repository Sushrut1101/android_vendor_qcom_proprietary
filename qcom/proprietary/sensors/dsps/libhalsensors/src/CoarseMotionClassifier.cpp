...
void CoarseMotionClassifier::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    switch(msg_hdr->msg_id) {
        case SNS_SAM_CMC_REPORT_IND_V02:
            ...
            if (sam_cmc_rpt_ptr->ms_event == SNS_SAM_CMC_MS_EVENT_ENTER_V02)
            {
    ...
    if (error == false) {
        ...
        HAL_LOG_VERBOSE("%s: CMC data[0]: %f data[1]: %f SAM TS: %u HAL TS:%" PRId64 " elapsedRealtimeNano:%" PRId64 "",
