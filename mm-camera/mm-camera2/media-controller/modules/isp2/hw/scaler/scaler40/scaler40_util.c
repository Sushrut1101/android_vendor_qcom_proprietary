...
/** scaler40_util_compare_identity:
 *
 *  @data1: identity1
 *  @data2: identity2
 *
...
/** scaler40_util_find_port_based_on_identity:
 *
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean scaler40_util_find_port_based_on_identity(void *data1,
  void *data2)
{
...
/** scaler40_util_get_port_from_module:
 *
...
/** scaler40_util_compare_identity_from_stream_param:
 *
 ...
 **/
static boolean scaler40_util_compare_identity_from_stream_info(void *data1,
  void *data2)
{
...
/** scaler40_util_append_stream_info:
 *
 *  @port: port handle
 *
  ...
  mct_stream_info_t *stream_info)
{
  ...
  l_stream_info = mct_list_find_custom(scaler->l_stream_info,
    stream_info, scaler40_util_compare_identity_from_stream_info);
  if (l_stream_info) {
  ...
  ret = scaler40_init(scaler);
  if (ret == FALSE) {
    ISP_ERR("failed: scaler40_open");
    goto ERROR;
  }
  ...
  ISP_HIGH("%s scaler dim %dx%d before_dis %dx%d fmt %d iden 0x%x",
  ...
  uint32_t identity)
{
  ...
  l_stream_info = mct_list_find_custom(scaler->l_stream_info,
    &identity, scaler40_util_compare_identity_from_stream_info);
  if (!l_stream_info) {
    PTHREAD_MUTEX_UNLOCK(&scaler->mutex);
    return TRUE;
  }
  ...
  struct msm_vfe_cfg_cmd_list **hw_update_list)
{
  ...
  ret = scaler40_util_append_cfg(hw_update_list, hw_update);
  if (ret == FALSE) {
