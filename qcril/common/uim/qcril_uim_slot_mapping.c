...
)
{
  ...
  if(slots_status.no_of_slots <= QMI_UIM_MAX_SLOT_COUNT)
  {
    for (slot_id = 0; slot_id < slots_status.no_of_slots; slot_id++)
    {
      ...
      memcpy(slots_status.slot_status[slot_id].iccid, params_ptr->qmi_rsp_data.rsp_data.slots_status_rsp.slot_status[slot_id].iccid,
...
)
{
  ...
  if (params_ptr->qmi_rsp_data.qmi_err_code == QMI_ERR_NO_EFFECT_V01)
  {
...
)
{
  ...
  if (original_req_ptr != NULL)
  {
    if (qcril_uim_queue_send_request(QCRIL_UIM_REQUEST_GET_SLOTS_STATUS,
                                     qcril_uim.qmi_handle,
                                     &dummy,
                                     qmi_uim_callback,
                                     (void*)original_req_ptr) >= 0)
...
)
{
  ...
  RadioConfigProcessSlotStatusInd(slot_status_change_ind_ptr);
  ...
  for (i = 0; i < status.no_of_slots && i < QMI_UIM_MAX_SLOT_COUNT; i++)
  {
    ...
    memcpy(status.slot_status[i].iccid, slot_status_change_ind_ptr->slot_status[i].iccid, QMI_UIM_MAX_ICCID_LEN);
   ...
   qcril_hook_unsol_response(ind_param_ptr->instance_id,
                             (int) QCRIL_EVT_HOOK_UNSOL_SLOT_STATUS_CHANGE_IND,
                             (char *)&status,
                             sizeof(status));
#endif /* QMI_RIL_UTF */
