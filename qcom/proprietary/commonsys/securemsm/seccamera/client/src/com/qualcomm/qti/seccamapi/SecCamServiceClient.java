...
public class SecCamServiceClient extends Handler {
    ...
    public boolean start(Context context, ClientCallback callback) {
        return start(context, callback, null, 0, DESTINATION_QTEE);
    }

    //=========================================================================
    public boolean start(Context context, ClientCallback callback, String taName, int taBufferSize) {
        return start(context, callback, taName, taBufferSize, DESTINATION_QTEE);
    }

    //=========================================================================
    public boolean start(Context context, ClientCallback callback, String taName,
                         int taBufferSize, int destination) {
        accessLock_.lock();
        if (false == serviceConnected_) {
            ...
            if (taName != null) {
                ...
                Log.d(LOG_TAG, "Start service, TA: " + taName + ", DESTINATION: " + destination);
            }
            else {
    ...
    //=========================================================================
    public boolean isDualStreamSupported() {
        accessLock_.lock();
        if(isSeviceConnected()) {
            try {
                Log.d(LOG_TAG, "Send MSG: isDualStreamSupported");
                Message msg = Message.obtain();
                msg.what = MSG_GET_FORMAT_SUPPORT;
                msg.replyTo = activityMessenger_;
                replayReadyLatch_ = new CountDownLatch(1);
                result_ = false;
                bundServiceMessenger_.send(msg);
                try {
                    if(!replayReadyLatch_.await(MSG_REPLAY_TIMEOUT, TimeUnit.SECONDS)) {
                        Log.e(LOG_TAG, "isDualStreamSupported - ERROR: timeout!");
                    }
                    else {
                        accessLock_.unlock();
                        return result_;
                    ...
                    Log.d(LOG_TAG, "isDualStreamSupported - ERROR: " + ex);
                    ex.printStackTrace();
                }
            } catch (RemoteException ex) {
                Log.d(LOG_TAG, "isDualStreamSupported - ERROR: " + ex);
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case MSG_GET_FORMAT_SUPPORT: {
                Bundle in_bundle = msg.getData();
                result_ = in_bundle.getBoolean("result");
                Log.d(LOG_TAG, "handleMessage - " + MSG_GET_FORMAT_SUPPORT + ":" + result_);
                replayReadyLatch_.countDown();
                break;
            }
            default: {
