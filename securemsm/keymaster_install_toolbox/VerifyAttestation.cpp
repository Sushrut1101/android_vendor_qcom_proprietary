...
static const char* StringifyTags(keymaster_tag_t tag) {
    ...
    case KM_HARDWARE_TYPE:
        return "KM_HARDWARE_TYPE";
    ...
    case KM_TRUSTED_USER_PRESENCE_REQUIRED:
        return "KM_TRUSTED_USER_PRESENCE_REQUIRED";
    case KM_TRUSTED_CONFIRMATION_REQUIRED:
        return "KM_TRUSTED_CONFIRMATION_REQUIRED";
    case KM_UNLOCKED_DEVICE_REQUIRED:
        return "KM_UNLOCKED_DEVICE_REQUIRED";
    ...
    case KM_BOOT_PATCHLEVEL:
        return "KM_BOOT_PATCHLEVEL";
    default:
        return "<Unknown>";
    }
    return "<Unknown>";
...
bool VerifyAttestation::verify_attestation_record(const string& challenge,
    ...
    if(expected_attestation_version != att_attestation_version) {
    ...
    keymaster_security_level_t type;
    if (expected_tee_enforced.GetTagValue(HARDWARE_TYPE, &type))
        att_tee_enforced.push_back(HARDWARE_TYPE, type);
...
keymaster_error_t VerifyAttestation::Attestation() {
    ...
    uint32_t expected_keymaster_version = device->get_keymaster_version();
    uint32_t expected_attestation_version;
    if(expected_keymaster_version == 3)
        expected_attestation_version = 2;
    else
        expected_attestation_version = 3;

    if(!verify_attestation_record(
        "challenge", sw_enforced(), hw_enforced(), expected_keymaster_version,
        level, expected_attestation_version,
        cert_chain.entries[0])) {
