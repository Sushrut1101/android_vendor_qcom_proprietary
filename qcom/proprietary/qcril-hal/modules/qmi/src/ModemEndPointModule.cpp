...
static void qmiAsyncCb(qmi_client_type user_handle, unsigned int msg_id,
  ...
  if (resp_cb_data != nullptr) {
    ...
    if (asyncCbData && asyncCbData->pModule != nullptr) {
...
void ModemEndPointModule::onQmiAsyncCb(qmi_client_type user_handle, unsigned int msg_id,
  ...
  (void)msg_id;
  (void)resp_buf;
  Log::getInstance().d("[" + mName + "]: onQmiAsyncCb: msg_id: " + std::to_string(msg_id) +
                       ", transp_err: " + std::to_string(transp_err));
...
void ModemEndPointModule::onQmiUnsolIndCb(qmi_client_type user_handle, unsigned int msg_id,
  ...
  if (((decoded_payload != nullptr) && (qmi_err == QMI_NO_ERR))
    ...
    Log::getInstance().d("[" + mName + "]: failed to decode indication message."
                         + " qmi err: " + std::to_string(qmi_err)
                         + " msg id: " + std::to_string(msg_id)
                         + " src buf size: " + std::to_string(ind_buf_len)
                         + " dest buf size: " + std::to_string(decoded_payload_len));
  }
...
void ModemEndPointModule::handleQmiClientSetup(
  ...
  if (mOwner.getState() == ModemEndPoint::State::OPERATIONAL) {
    Log::getInstance().d("[ModemEndPointModule]: " + mOwner.mName + " is already operational");
