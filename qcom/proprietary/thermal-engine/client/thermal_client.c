...
#ifdef USE_ANDROID_LOG
#include "common_log.h" /* define after log/log.h */
...
static pthread_t thermal_client_bw_thread;
static int thermal_client_shutdown = 0;
static int thermal_bw_client_shutdown = 0;
static int first_client = 1;
static struct thermal_msg_data client_msg;
static struct thermal_msg_data bw_client_msg;
struct thermal_cdata *list_head = NULL;
static pthread_mutex_t clnt_list_mtx;
static pthread_mutex_t clnt_reg_mtx;
static pthread_mutex_t bw_clnt_mtx;
static pthread_mutex_t bw_data_mtx;
static int pipe_fds[2] = {-1, -1};
static int bw_pipe_fds[2] = {-1, -1};
static int bw_thread_triggered;
...
static struct algorithms algos[] = {
	...
	{.name = "step_wise", .type = STEP_WISE_ALGO_TYPE},
...
static int add_set_points_to_field(void *value, void *data);
static int add_thresholds_clr_to_field(void *value, void *data);
static int add_set_points_clr_to_field(void *value, void *data);
static int add_actions_to_field(void *value, void *data);
static int add_cdev_array_to_field(void *value, void *data);
static int add_action_info_to_field(void *value, void *data);
static int add_upper_limits_to_field(void *value, void *data);
static int add_lower_limits_to_field(void *value, void *data);
...
/* field_table for both SS, PID and Token Bucket */
static struct field_table pid_ss_tb_fields[] = {
	{
	 .field_name = "disable",
	 .type = FIELD_INT,
	 .field_mask = DISABLE_FIELD,
...
/* field_table for Stepwise */
static struct field_table step_wise_fields[] = {
	{
	 .field_name = "disable",
	 .type = FIELD_INT,
	 .field_mask = DISABLE_FIELD,
	...
	{
	 .field_name = "polling_delay",
	 .type = FIELD_INT,
	 .field_mask = POLLING_DELAY_FIELD,
	 .offset = offsetof(struct thermal_msg_data, config.s_setting.polling_delay),
	 .sub_offset = 0,
	 .callback = &add_int_to_field
	},
	{
	 .field_name = "passive_delay",
	 .type = FIELD_INT,
	 .field_mask = PASSIVE_DELAY_FIELD,
	 .offset = offsetof(struct thermal_msg_data, config.s_setting.passive_delay),
	 .sub_offset = 0,
	 .callback = &add_int_to_field
	},
	{
	 ...
	 .type = FIELD_INT_ARR,
	 .field_mask = SET_POINT_FIELD,
	 .offset = offsetof(struct thermal_msg_data, config.s_setting),
	 .sub_offset = offsetof(struct trip_pt, set_point),
	 .callback = &add_set_points_to_field
	},
	{
	 .field_name = "clr_temp",
	 .type = FIELD_INT_ARR,
	 .field_mask = SET_POINT_CLR_FIELD,
	 .offset = offsetof(struct thermal_msg_data, config.s_setting),
	 .sub_offset = offsetof(struct trip_pt, set_point_clr),
	 .callback = &add_set_points_clr_to_field
	},
	{
	 .field_name = "device",
	 .type = FIELD_ARR_STR,
	 .field_mask = UNKNOWN_FIELD,
	 .offset = offsetof(struct thermal_msg_data, config.s_setting),
	 .sub_offset = offsetof(struct trip_pt, devs),
	 .callback = &add_cdev_array_to_field
	},
	{
	 .field_name = "upper_limit",
	 .type = FIELD_ARR_INT_ARR,
	 .field_mask = UPPER_LIMIT_FIELD,
	 .offset = offsetof(struct thermal_msg_data, config.s_setting),
	 .sub_offset = offsetof(struct trip_pt, devs),
	 .callback = &add_upper_limits_to_field
	},
	{
	 .field_name = "lower_limit",
	 .type = FIELD_ARR_INT_ARR,
	 .field_mask = LOWER_LIMIT_FIELD,
	 .offset = offsetof(struct thermal_msg_data, config.s_setting),
	 .sub_offset = offsetof(struct trip_pt, devs),
	 .callback = &add_lower_limits_to_field
	}

};

/*==============================================================================
FUNCTION do_bw_request_to_thermal

Bandwidth client request sending thread.
...
RETURN VALUE
	void * - not used.
===============================================================================*/
static void *do_bw_request_to_thermal(void *data)
{
	ssize_t rc = 0;
	struct sockaddr_un client_addr_send;
	char *send_sock_path = (char *)data;
	fd_set readfds, testfds;
	int pipe_fd_notify = 0;
	int bw_clnt_snd_soc_fd = -1;

	FD_ZERO(&readfds);
	while (thermal_bw_client_shutdown != 1) {
		char buf[5] = {0};
		struct thermal_msg_data cur_data;

		if (bw_pipe_fds[0] < 0) {
			if (pipe(bw_pipe_fds)) {
				msg(
				"Thermal-Lib:Pipe error. Re-trying after 5s\n");
				sleep(5);
				continue;
			} else {
				FD_SET(bw_pipe_fds[0], &readfds);
			}
		}

		if (bw_clnt_snd_soc_fd < 0) {
			bw_clnt_snd_soc_fd = socket(AF_LOCAL, SOCK_STREAM, 0);
			if (bw_clnt_snd_soc_fd < 0) {
			...
			FD_SET(bw_clnt_snd_soc_fd, &readfds);
		}

		if (pipe_fd_notify == 0) {
			memset(&client_addr_send, 0,
				sizeof(struct sockaddr_un));
			strlcpy(client_addr_send.sun_path, send_sock_path,
				UNIX_PATH_MAX);
			client_addr_send.sun_family = AF_LOCAL;
			rc = connect(bw_clnt_snd_soc_fd,
				(struct sockaddr *)&client_addr_send,
				(socklen_t)(sizeof(sa_family_t) +
				strlen(send_sock_path)));
			if (rc != 0) {
				msg("Thermal-Lib:%s:connect failure, err:%d\n",
					__func__, (int)rc);
				close(bw_clnt_snd_soc_fd);
				bw_clnt_snd_soc_fd = -1;
				sleep(1);
				continue;
			}
		}
		...
		if (pipe_fd_notify == 1) {
			rc = read(bw_pipe_fds[0], buf, 5);
			if (rc < 0)
				msg("bw pipe read error:%d\n", errno);
			pipe_fd_notify = 0;
		}

		pthread_mutex_lock(&bw_data_mtx);
		memset(&cur_data, 0, sizeof(struct thermal_msg_data));
		strlcpy(cur_data.client_name, bw_client_msg.client_name,
			CLIENT_NAME_MAX);
		cur_data.req_data = bw_client_msg.req_data;
		pthread_mutex_unlock(&bw_data_mtx);

		rc = send_to_fd(bw_clnt_snd_soc_fd,
		...
		info("Thermal-Lib: Client request sent for %s with val:0x%x\n",
			cur_data.client_name, cur_data.req_data);
		...
		while (thermal_bw_client_shutdown != 1) {
			...
			if (rc < 1)
				continue;
                        ...
                        if (FD_ISSET(bw_pipe_fds[0], &testfds)) {
				pipe_fd_notify = 1;
				break;
			}

                        if (FD_ISSET(bw_clnt_snd_soc_fd, &testfds)) {
				info(
				"%s:Server is crashed,reconnect & send again\n",
					__func__);
				FD_CLR(bw_clnt_snd_soc_fd, &readfds);
				close(bw_clnt_snd_soc_fd);
				bw_clnt_snd_soc_fd = -1;
				break;
			}
...
/*================================================================================================
FUNCTION thermal_bandwidth_client_request

Thermal bandwidth client request to thermal function.
The client which will send/notify request to thermal server.
...
int thermal_bandwidth_client_request(char *client_name, int req_data)
{
	...
	if (NULL == client_name) {
		msg("Thermal-Lib:%s: unexpected NULL", __func__);
		return ret;
	}
	/* Check for client is supported or not*/
	for (i = 0; i < ARRAY_SIZE(req_clients); i++) {
		if (0 == strncmp(req_clients[i].name, client_name,
			CLIENT_NAME_MAX))
			break;
	}
	...
	BW_LM_INPUT_VALIDATE(req_data, ret);
	if (ret)
		return ret;

	pthread_mutex_lock(&bw_clnt_mtx);
	if (thermal_bw_client_shutdown != 0)
		thermal_bw_client_shutdown = 0;

	pthread_mutex_lock(&bw_data_mtx);
	memset(&bw_client_msg, 0, sizeof(struct thermal_msg_data));
	strlcpy(bw_client_msg.client_name, client_name, CLIENT_NAME_MAX);
	bw_client_msg.req_data = req_data;
	pthread_mutex_unlock(&bw_data_mtx);

	if (bw_thread_triggered == 0) {
		rc = pthread_create(&thermal_client_bw_thread, NULL,
			do_bw_request_to_thermal,
			(void*) req_clients[i].send_sock_path);
		if (rc != 0) {
			msg("Thermal-Lib: Unable to create pthread to "
			    "send client request from %s", client_name);
			ret = rc;
			goto error_handler;
		}
		bw_thread_triggered = 1;
	} else {
		if (bw_pipe_fds[1] > -1) {
			/* Notify thread to send new bandwidth request */
			err = write(bw_pipe_fds[1], "send", 5);
			if (err < 0)
	...
	ret = 0;
error_handler:
	pthread_mutex_unlock(&bw_clnt_mtx);

	return ret;
}

/*===========================================================================
FUNCTION thermal_bandwidth_client_cancel_request

Function to cancel all bandwidth request and release resources for
bw clients.
...
void thermal_bandwidth_client_cancel_request(char *client_name)
{
	...
	pthread_mutex_lock(&bw_data_mtx);
	bw_client_msg.req_data = 0;
	pthread_mutex_unlock(&bw_data_mtx);

	thermal_bw_client_shutdown = 1;
	if (bw_pipe_fds[1] > -1) {
		ret = write(bw_pipe_fds[1], "exit", 5);
		if (ret < 0)
			msg("pipe write error:%d\n", errno);
	}

	if (bw_thread_triggered)
		pthread_join(thermal_client_bw_thread, NULL);
	bw_thread_triggered = 0;
	info("%s: Removing all bw request for %s\n", __func__, client_name);
	pthread_mutex_unlock(&bw_clnt_mtx);
}
...
static void *do_request_to_thermal(void *data)
{
	...
	while(1) {
		...
		rc = send_to_fd(sockfd_client_send, (unsigned char *)&client_msg,
			sizeof(struct thermal_msg_data));
		if (rc <= 0) {
			msg("Thermal-Lib-Client: "
...
/*==============================================================================
FUNCTION add_common_stepwise_entry_to_field

Common helper function for multiple stepwise settings entry values to
client field.

ARGRUMENTS
	field - field in which value will be stored
	ptr_s - socket structure pointer which has value to be stored
	type - enum which indicating field type

RETURN VALUE
	O on succuess, negative error (-1) on failure
===============================================================================*/
static int add_common_stepwise_entry_to_field(struct field_data *field,
						struct step_settings *ptr_s,
						enum supported_fields type)
{
	uint32_t i = 0;
	uint32_t j = 0;
	int *int_data = NULL;
	...
	field->num_data = ptr_s->trip_pt_num;

	switch(type) {
	case SET_POINT_FIELD:
	case SET_POINT_CLR_FIELD:
		int_data = (int *)calloc(sizeof(int), field->num_data);
		if (int_data == NULL) {
		...
		for (i = 0; i < field->num_data; i++) {
			if (type == SET_POINT_FIELD)
				int_data[i] = ptr_s->trips[i].set_point;
			else
				int_data[i] = ptr_s->trips[i].set_point_clr;
		}
		field->data = int_data;
		break;
	case UPPER_LIMIT_FIELD:
	case LOWER_LIMIT_FIELD:
		action_info =
		(struct action_info_data *)calloc(sizeof(*action_info),
			field->num_data);
		...
		for (i = 0; i < field->num_data; i++) {
			action_info[i].num_actions = ptr_s->trips[i].device_num;
			for (j = 0; j < ptr_s->trips[i].device_num &&
				j < MAX_ACTIONS; j++) {
				if (type == UPPER_LIMIT_FIELD)
					action_info[i].info[j] =
					ptr_s->trips[i].devs[j].upper_limit;
				else
					action_info[i].info[j] =
					ptr_s->trips[i].devs[j].lower_limit;
			}
		}
		field->data = action_info;
		break;
	default:
		msg("%s:Not supported field type:%d\n", __func__, type);
		return -1;
	}
...
/*==============================================================================
FUNCTION add_set_points_to_field

Helper function for multiple integer set point values to a field with stepwise
settings
...
{
	struct field_data *field = (struct field_data *)data;
	struct step_settings *ptr_s = (struct step_settings *)value;

	if (value == NULL || data == NULL)
		return -1;

	return add_common_stepwise_entry_to_field(field, ptr_s, SET_POINT_FIELD);
}

/*=============================================================================
FUNCTION add_set_points_clr_to_field

Internal helper function for adding set point clear integer array data
to field

ARGUEMENTS
	value - pointer to set point clear data array in thermal_msg_data
	struct
	data - pointer where set point clear data array with be stored
	in field_data struct
...
static int add_set_points_clr_to_field (void *value, void *data)
{
	...
	return add_common_stepwise_entry_to_field(field, ptr_s, SET_POINT_CLR_FIELD);
}
...
static int add_actions_to_field(void *value, void *data)
{
	...
	for (i = 0; i < field->num_data; i++) {
		...
		memset(char_data[i], 0, sizeof(char) * (string_len));
		for (j = 0; j < ptr_m->t[i].num_actions; j++) {
			if (j > 0) {
...
/*=============================================================================
FUNCTION add_cdev_array_to_field

Internal helper function for adding the device name string array data to
the field

ARGUMENTS
	value - value (currently pointer to a thermal_msg_data struct)
	data - field for storing the value
...
static int add_cdev_array_to_field(void *value, void *data)
{
	...
	char_data = (char **)calloc(sizeof(char *), field->num_data);
	if (char_data == NULL) {
		msg("%s: calloc failed", __func__);
		return -1;
	}
	...
	for (i = 0; i < field->num_data; i++) {
		string_len = (DEVICES_MAX_NAME_LEN+1) * ptr_s->trips[i].device_num;
		char_data[i] = (char *)calloc(sizeof(char), (string_len));
		if (char_data[i] == NULL) {
			msg("%s: calloc failed", __func__);
			for (j = 0; j < i; j++)
				free(char_data[j]);
		...
		for (j = 0; j < ptr_s->trips[i].device_num; j++) {
			if (j > 0) {
				if (strlcat(char_data[i], "+", string_len) >= string_len)
					break;
			}
			if (strlcat(char_data[i], ptr_s->trips[i].devs[j].name, string_len) >= string_len)
				break;
		}
	}
	field->data = char_data;
...
/*==============================================================================
FUNCTION add_upper_limits_to_field

Internal function for upper limit data to field
...
static int add_upper_limits_to_field(void *value, void *data)
{
	...
	return add_common_stepwise_entry_to_field(field, ptr_s,
							UPPER_LIMIT_FIELD);
}

/*==============================================================================
FUNCTION add_lower_limits_to_field

Internal function for lower limit data to field
				   ...
				   uint32_t *num_configs)
{
	...
	rc = recv_from_fd(sockfd, (unsigned char *)thermal_msg,
		sizeof(struct thermal_msg_data));
	if (rc <= 0) {
				...
				struct config_instance **configs)
{
	...
	rc = send_to_fd(sockfd_client, (unsigned char *)&query_msg,
		sizeof(struct thermal_msg_data));
	if (rc <= 0) {
		msg("Thermal-Lib-Client: "
	...
	while (1) {
		...
		switch(thermal_msg.msg_type) {
			...
			case PID_ALGO_TYPE:
				...
				idx++;
				break;
			case STEP_WISE_ALGO_TYPE:
				if (add_settings_to_client_config(&config_local[idx],
						&thermal_msg, step_wise_fields,
						ARRAY_SIZE(step_wise_fields))) {
					msg("Thermal-Lib-Client:%s: config data is invalid\n",
						__func__);
					goto error_handler;
				}
...
/*==============================================================================
FUNCTION add_upper_lower_limits_to_sock_data

Internal function for adding lower_limit data to socket data.
...
RETURN VALUE
	0 on success, negative error(-1) on failure.
==============================================================================*/
static int add_upper_lower_limits_to_sock_data(void *data, void *field,
					int sub_offset, enum supported_fields type)
{
	uint32_t i = 0;
	uint32_t j = 0;
	struct action_info_data *action_info = NULL;
	struct field_data *ptr_field = (struct field_data *)field;
	struct step_settings *ptr_s = (struct step_settings *)data;
	...
	action_info = (struct action_info_data *)ptr_field->data;
	ptr_s->trip_pt_num = ptr_field->num_data;

	for (i = 0; i < ptr_s->trip_pt_num; i++) {
		struct step_device_info *ptr_device = NULL;
		ptr_s->trips[i].device_num = action_info[i].num_actions;
		ptr_device = (struct step_device_info *)((char *)&ptr_s->trips[i] + \
								sub_offset);
		for (j = 0; j < action_info[i].num_actions; j++)
			if (type == UPPER_LIMIT_FIELD)
				ptr_device[j].upper_limit = action_info[i].info[j];
			else
				ptr_device[j].lower_limit = action_info[i].info[j];
	}
...
/*==============================================================================
FUNCTION add_set_points_to_sock_data

Internal function for adding set_temp/clr_temp data to
socket data.
...
RETURN VALUE
	0 on success, negative error(-1) on failure.
==============================================================================*/
static int add_set_points_to_sock_data(void *data, void *field, int sub_offset)
{
	uint32_t i = 0;
	int *value = NULL;
	struct field_data *ptr_field = (struct field_data *)field;
	...
	for (i = 0; i < ptr_s->trip_pt_num && i < THRESHOLDS_MAX; i++) {
		int *int_data = NULL;
		int_data = (int *)((char *)&ptr_s->trips[i] + sub_offset);
		*int_data = value[i];
	}
	...
	for (i = 0; i < config->num_fields; i++) {
		...
		switch(fields_local[i].data_type) {
			...
			case FIELD_INT_ARR:
				...
				if ((strcmp(fields_local[i].field_name, "set_temp") == 0) ||
						(strcmp(fields_local[i].field_name, "clr_temp") == 0)) {
					add_set_points_to_sock_data((void *)((char *)soc_data + \
							    fields[j].offset),
							    &fields_local[i],
							    fields[j].sub_offset);
				} else {
			...
			case FIELD_ARR_INT_ARR:
				...
				if (strcmp(fields_local[i].field_name, "upper_limit") == 0) {
					add_upper_lower_limits_to_sock_data((void *)((char *)soc_data + \
								fields[j].offset),
								&fields_local[i],
								fields[j].sub_offset,
								UPPER_LIMIT_FIELD);
				} else if (strcmp(fields_local[i].field_name, "lower_limit") == 0) {
					add_upper_lower_limits_to_sock_data((void *)((char *)soc_data + \
								fields[j].offset),
								&fields_local[i],
								fields[j].sub_offset,
					...
					struct thermal_msg_data *soc_data)
{
	...
	switch(soc_data->msg_type) {
		...
		case PID_ALGO_TYPE:
			if (add_algo_specific_data_to_sock_data(config, soc_data,
				  pid_ss_tb_fields, ARRAY_SIZE(pid_ss_tb_fields)))
				return -1;
			break;
		case STEP_WISE_ALGO_TYPE:
			if (add_algo_specific_data_to_sock_data(config, soc_data,
				step_wise_fields, ARRAY_SIZE(step_wise_fields)))
				return -1;
			break;
		default:
			return -1;
				       ...
				       struct config_instance *config)
{
	...
	switch(socket_data->msg_type) {
		...
		case PID_ALGO_TYPE:
			...
			break;
		case STEP_WISE_ALGO_TYPE:
			s_setting =  &socket_data->config.s_setting;
			s_setting->trip_pt_num = 0;
			s_setting->polling_delay = 0;
			s_setting->passive_delay = 0;
			for (i = 0; i < THRESHOLDS_MAX; i++) {
				uint32_t j = 0;
				s_setting->trips[i].set_point = INT_MIN;
				s_setting->trips[i].set_point_clr = INT_MIN;
				s_setting->trips[i].device_num = 0;
				for (j = 0; j < ACTIONS_MAX; j++) {
					s_setting->trips[i].devs[j].name[0] = '\0';
					s_setting->trips[i].devs[j].upper_limit = 0;
					s_setting->trips[i].devs[j].lower_limit = 0;
				}
			}
		default:
			break;
			       ...
			       unsigned int config_size)
{
	...
	rc = send_to_fd(sockfd_client, (unsigned char *)&config_set_data,
		sizeof(struct thermal_msg_data));
	if (rc <= 0) {
		msg("Thermal-Lib-Client: "
		    "Unable to send request data to fd %d",
                                   ...
                                   unsigned int config_size)
{
	...
	for (idx = 0; idx < config_size; idx++) {
		...
		for (i = 0; i < configs[idx].num_fields; i++) {
			char **field_str = configs[idx].fields[i].data;
			if (configs[idx].fields[i].field_name != NULL)
				free(configs[idx].fields[i].field_name);

			if (!configs[idx].fields[i].data)
				continue;

			switch(configs[idx].fields[i].data_type) {
			case FIELD_ARR_STR:
				for (j = 0; j < configs[idx].fields[i].num_data;
					j++)
					free(field_str[j]);
				free(field_str);
				break;
...
static void *do_listen(void *data)
{
	...
	int (*rpt_callback)(char *, void *, void *);
	struct thermal_cdata *callback_node;
	static struct sockaddr_un client_addr;
	struct thermal_msg_data thermal_msg;
	int sockfd_client_recv = -1;
	fd_set readfds, testfds;
	char buf[REPORT_MSG_MAX] = {0};
	char *socket_info = (char *)data;

	if (!data) {
		msg("Thermal-Client-Lib: Invalid input\n");
		return NULL;
	}

	while (thermal_client_shutdown != 1) {

		if (pipe_fds[0] < 0 && pipe(pipe_fds)) {
		...
		memset(&client_addr, 0, sizeof(struct sockaddr_un));
		snprintf(client_addr.sun_path, UNIX_PATH_MAX, "%s", socket_info);
		client_addr.sun_family = AF_LOCAL;

		rc = connect(sockfd_client_recv, (struct sockaddr *)&client_addr,
				     (socklen_t)(sizeof(sa_family_t) + strlen(socket_info)));
		if (rc != 0) {
		...
		while (thermal_client_shutdown != 1) {
			...
			/* Handle report action monitor case */
			if (!strcmp(socket_info, THERMAL_SEND_RULE_SOCKET)) {
				memset(buf, 0, REPORT_MSG_MAX);
				rc = recv(sockfd_client_recv, (unsigned char *)buf,
					REPORT_MSG_MAX, 0);
				if (rc <= 0) {
				...
				for (callback_node = list_head; callback_node != NULL;
					...
					callback_node = get_callback_node_from_list(
								callback_node, "report");
					if (callback_node) {
						rpt_callback = callback_node->callback;
						if (rpt_callback)
							rpt_callback(buf,
						...
						msg(
						"No callback for report, stop listening\n");
				...
				continue;
			} /* End of report action handle */

			memset(&thermal_msg, 0, sizeof(struct thermal_msg_data));
			rc = recv_from_fd(sockfd_client_recv, (unsigned char *)&thermal_msg,
				sizeof(struct thermal_msg_data));
			if (rc <= 0) {
				msg("Thermal-Lib-Client:%s: recv failed", __func__);
				break;
...
int thermal_client_register_callback(char *client_name, int (*callback)(int, void *, void *), void *data)
{
	...
	info("Thermal-Lib-Client: Registration successful "
...
/*================================================================================================
FUNCTION thermal_client_register_report_callback

Thermal client registration function for thermal report event notification.
The client registers a callback function and connects to thermal local socket.
It will monitor /dev/socket/thermal-send-rule socket where report_rule events are
notified. On socket data recv, it sends data to client via this callback function.

ARGUMENTS
	callback    - callback function pointer with string buffer pointer, user_data
	              pointer and reserved data as arguments
	data        - user data
...
int thermal_client_register_report_callback(int (*callback)(char *, void *, void*), void *data)
{
	...
	if (NULL == callback) {
		msg("Thermal-Lib-Client:%s: unexpected NULL client registration "
		    "failed ", __func__);
		return rc;
	}

	pthread_mutex_lock(&clnt_list_mtx);
	client_cb_handle = add_to_list("report", callback, data);
	if (client_cb_handle == 0) {
		msg("Thermal-Lib-Client: %s: Client Registration failed", __func__);
		pthread_mutex_unlock(&clnt_list_mtx);
		goto error_handler;
	...
	rc = pthread_create(&thermal_client_recv_thread, NULL, do_listen,
		(void *)THERMAL_SEND_RULE_SOCKET);
	if (rc != 0) {
		msg("Thermal-Lib-Client: Unable to create pthread to "
		    "listen thermal events for report events");
		rc = -1;
		goto error_handler;
	}

	info("Thermal-Lib-Client: Registration successful "
	     "for thermal report events:%d", client_cb_handle);
	rc = (int)client_cb_handle;
error_handler:
	return rc;
}
