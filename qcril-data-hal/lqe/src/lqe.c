...
#define LQE_LOCK_CLIENT() QCRIL_DATA_MUTEX_LOCK(LQE_CLIENT_MUTEX)
#define LQE_UNLOCK_CLIENT() QCRIL_DATA_MUTEX_UNLOCK(LQE_CLIENT_MUTEX)
...
typedef int (*lqe_qmi_toggle_reporting_fptr)(int lqe_client_handle, lqe_data_direction_e direction, lqe_indications_switch_e ind_switch);
typedef int (*lqe_qmi_get_uplink_capability_fptr)(int lqe_client_handle);
typedef int (*lqe_qmi_get_downlink_capability_fptr)(int lqe_client_handle);
typedef int (*lqe_qmi_set_reporting_criteria_fptr)(int lqe_client_handle, lqe_data_direction_e direction, qcril_data_lce_rpt_criteria* criteria);
...
typedef struct lqe_qmi_vtable_s
{
  ...
  lqe_qmi_toggle_reporting_fptr                       lqe_qmi_toggle_reporting;
  lqe_qmi_get_uplink_capability_fptr                  lqe_qmi_get_uplink_capability;
  lqe_qmi_get_downlink_capability_fptr                lqe_qmi_get_downlink_capability;
  lqe_qmi_set_reporting_criteria_fptr                 lqe_qmi_set_reporting_criteria;
} lqe_qmi_vtable_t;
...
lqe_qmi_vtable_t lqe_qmi_vtbl =
{
  ...
  .lqe_qmi_toggle_reporting           = lqe_ott_toggle_reporting,
  .lqe_qmi_get_uplink_capability      = lqe_ott_get_uplink_capability,
  .lqe_qmi_get_downlink_capability    = lqe_ott_get_downlink_capability,
  .lqe_qmi_set_reporting_criteria     = lqe_ott_configure_throughput_report_criteria,
};
...
)
{
  ...
  if(lqe_config.lqe_qmi_service != LQE_QMI_SERVICE_OTT)
  {
    LQE_LOG_ERROR("%s(): LQE is using [0x%x] QMI service. Dropping ind",
                  __func__, lqe_config.lqe_qmi_service);
    goto bail;
  }
...
)
{
  ...
  if(lqe_config.lqe_qmi_service != LQE_QMI_SERVICE_WDS)
  {
    LQE_LOG_ERROR("%s(): LQE is using [0x%x] QMI service. Dropping ind",
                  __func__, lqe_config.lqe_qmi_service);
    goto bail;
  ...
  for( index=0; index < LQE_CLIENT_MAX; index++ )
  {
    if( !lqe_config.clients_info[index].is_valid ) {
      ...
      *client_pptr = &lqe_config.clients_info[index];
      lqe_config.lqe_qmi_service = LQE_QMI_SERVICE_NONE;
      break;
    }
  }
...
)
{
  ...
  if(lqe_config.lqe_qmi_service == LQE_QMI_SERVICE_NONE)
  {
    ...
    {
      LQE_LOG_ERROR("%s(): OTT service available on modem! \n", __func__);
      if(LQE_SUCCESS == (ret = lqe_int_ott_client(index)))
      {
        lqe_config.lqe_qmi_service = LQE_QMI_SERVICE_OTT;
      }
    }
    else
    {
      ...
      {
        lqe_config.lqe_qmi_service = LQE_QMI_SERVICE_WDS;
  ...
  else if(lqe_config.lqe_qmi_service == LQE_QMI_SERVICE_OTT)
  {
...
)
{
  if(LQE_IS_INDEX_VALID(index))
  {
    ...
    if(client_ptr->lqe_params.is_valid)
    {
      ...
      // set known criteria for each rat and start reporting if previously enabled
      for (int i = 0; i < LQE_MAX_OTT_RAT; i++) {
        qcril_data_lce_rpt_criteria *c = &client_ptr->lqe_params.criteria_list[i].criteria;
        if (LQE_SUCCESS !=
              lqei_set_throughput_indication_criteria( LQE_MAKE_HNDL(index),
                                                       LQE_DATA_UPLINK_DIRECTION,
                                                       c )) {
          LQE_LOG_ERROR("Failed to set uplink criteria for rat %d", i);
        }
      }
      for (int i = 0; i < LQE_MAX_OTT_RAT; i++) {
        qcril_data_lce_rpt_criteria *c = &client_ptr->lqe_params.criteria_list[i].criteria;
        if (LQE_SUCCESS !=
              lqei_set_throughput_indication_criteria( LQE_MAKE_HNDL(index),
                                                       LQE_DATA_DOWNLINK_DIRECTION,
                                                       c )) {
          LQE_LOG_ERROR("Failed to set downlink criteria for rat %d", i);
        }
      }
      if(client_ptr->lqe_params.ul_report_enabled > 0)
      {
        if (lqei_toggle_throughput_reporting( LQE_MAKE_HNDL(index),
                                               LQE_DATA_UPLINK_DIRECTION,
                                               LQE_TURN_ON_INDICATIONS) != LQE_SUCCESS) {
          LQE_LOG_ERROR("Failed to start uplink reporting");
        }
      }
      if(client_ptr->lqe_params.dl_report_enabled > 0)
      {
        if (lqei_toggle_throughput_reporting( LQE_MAKE_HNDL(index),
                                               LQE_DATA_DOWNLINK_DIRECTION,
                                               LQE_TURN_ON_INDICATIONS) != LQE_SUCCESS) {
          LQE_LOG_ERROR("Failed to start downlink reporting");
        }
      }
    }
  }
...
)
{
  ...
  switch(event_id)
  {
    case LQE_QMI_SYS_EVENT_MODEM_OUT_OF_SERVICE_IND:
      ...
      {
        LQE_LOG_INFO("%s(): SSR OOS in progress... ignoring duplication call", __func__);
        return;
      }
...
)
{
  ...
  if (LQE_SUCCESS == (ret = lqe_qmi_vtbl.lqe_qmi_toggle_reporting(index,
                                                                  direction, indications)))
  {
    if (indications == LQE_TURN_ON_INDICATIONS)
    {
      LQE_LOG_DEBUG("%s: turning on downlink throughput indications on "
                 "client handle:%d", __func__, index);
      if (direction == LQE_DATA_UPLINK_DIRECTION) {
        client_ptr->lqe_params.ul_report_enabled = 1;
      } else {
        client_ptr->lqe_params.dl_report_enabled = 1;
      }
    }
    else
    {
      LQE_LOG_DEBUG("%s: turning off downlink throughput indications on "
                 "client handle:%d", __func__, index);
      if (direction == LQE_DATA_UPLINK_DIRECTION) {
        client_ptr->lqe_params.ul_report_enabled = 0;
      } else {
        client_ptr->lqe_params.dl_report_enabled = 0;
      }
    }
    ret = LQE_SUCCESS;
  }
...
)
{
  ...
  if ((LQE_SUCCESS == (ret =
         lqe_qmi_vtbl.lqe_qmi_get_uplink_capability(index))) &&
      (LQE_SUCCESS == (ret =
          lqe_qmi_vtbl.lqe_qmi_get_downlink_capability(index)))
      ) { /* UL and DL reporting supported. */
    if ((LQE_SUCCESS == (ret =
          lqe_toggle_registration_downlink_throughput_indications
          ( client_handle, LQE_TURN_ON_INDICATIONS ))) &&
        (LQE_SUCCESS == (ret =
          lqe_toggle_registration_uplink_throughput_indications
          ( client_handle, LQE_TURN_ON_INDICATIONS )))) {
      /* Subscribed to UL and DL indications. See NOTE above about
       * START/STOP action expected by QMI OTT */
            ret = LQE_SUCCESS;
    }
  }
  return ret;
...
{
  unsigned int index = 0;
  if (!LQE_IS_HANDLE_VALID(client_handle) || (criteria == NULL))
  {
    LQE_LOG_ERROR("%s,Invalid handle passed", __func__);
    return LQE_FAILURE;
  }
  ...
  int ret = lqe_qmi_vtbl.lqe_qmi_set_reporting_criteria(index, direction, criteria);
  ott_rat_enum_type_v01 rat = criteria->ran;

  if ((rat >= LQE_MAX_OTT_RAT) || (rat < 0)) {
    LQE_LOG_ERROR("%s,Invalid ran passed, cannot update cache", __func__);
    return ret;
  }

  if (ret == LQE_SUCCESS) {
    // update cache if successful
    client_ptr->lqe_params.criteria_list[rat].criteria.hysteresisMs =
      criteria->hysteresisMs;
    client_ptr->lqe_params.criteria_list[rat].criteria.hysteresisDlKbps =
      criteria->hysteresisDlKbps;
    client_ptr->lqe_params.criteria_list[rat].criteria.hysteresisUlKbps =
      criteria->hysteresisUlKbps;
    client_ptr->lqe_params.criteria_list[rat].criteria.thresholdsDownlinkKbps_size =
      MIN(QMI_OTT_DL_THRESHOLD_LIST_LEN_V01, criteria->thresholdsDownlinkKbps_size);
    client_ptr->lqe_params.criteria_list[rat].criteria.thresholdsUplinkKbps_size =
      MIN(QMI_OTT_UL_THRESHOLD_LIST_LEN_V01, criteria->thresholdsUplinkKbps_size);
    for (int i = 0;
          i < client_ptr->lqe_params.criteria_list[rat].criteria.thresholdsDownlinkKbps_size;
          i++) {
      client_ptr->lqe_params.criteria_list[rat].criteria.thresholdsDownlinkKbps[i] =
        criteria->thresholdsDownlinkKbps[i];
    }
    for (int i = 0;
          i < client_ptr->lqe_params.criteria_list[rat].criteria.thresholdsUplinkKbps_size;
          i++) {
      client_ptr->lqe_params.criteria_list[rat].criteria.thresholdsUplinkKbps[i] =
