...
#define TAG "QCRIL_MBN_HW_UPDATE"
...
#include "modules/mbn/HwMbnUpdateStatusInd.h"
#include "qmi.h"

#define QCRIL_MBN_HW_FEATURE_ENABLED "persist.vendor.radio.hw_mbn_update"
#define QCRIL_MBN_SW_OTA_FOR_MODEM_CENTRIC "persist.vendor.radio.start_ota_daemon"
...
qtimutex::QtiSharedMutex qcril_mbn_hw_config_mutex;

#define MBN_HW_CONFIG_LOCK()    qcril_mbn_hw_config_mutex.lock()
#define MBN_HW_CONFIG_UNLOCK()  qcril_mbn_hw_config_mutex.unlock();
...
const char *qcril_mbn_hw_get_hw_name_to_look_for
(
...
void qcril_qmi_hw_mbn_publish_update_status();
/*===========================================================================
...
)
{
    ...
    /*check hw_mbn_update property.if it is disabled there no need to check
      hw_mbn current state and return TRUE to process PDC OEM requests.
    */
    int is_hw_enabled = 1;
    ...
    do
    {
        ...
        else
        {
                                        ...
                                        config_id_len))
            {
                ...
                qcril_qmi_hw_mbn_publish_update_status();
...
)
{
    ...
    //store the sub count
    qcril_mbn_max_apss_instance_id = apss_max_subscriptions;
...
#ifdef QMI_RIL_UTF
#define HW_CFG_PATH "data/modem_config/mcfg_hw/"
#else
#define HW_CFG_PATH DATA_PARTITION_ROOT "modem_config/mcfg_hw/"
#endif
/*===========================================================================
...
{
    const char *hw_cfg_path     = HW_CFG_PATH;
    ...
    {
        QCRIL_LOG_ESSENTIAL("QCRIL_ERROR:IO: No hw mbn config directory");
    }
    ...
    else
    {
        ...
        for (index = file_names.begin(); index != file_names.end(); index++)
        {
            ...
            ret = qcril_mbn_db_add_hw_mbn_file(file_name);
            if (ret != E_SUCCESS)
            {
            ...
            if (ret == E_SUCCESS)
            {
                ret = qcril_mbn_db_add_hw_mbn_qc_version(file_name,
                                                   int_value);
                if (ret != E_SUCCESS)
                {
            ...
            if (ret == E_SUCCESS)
            {
                ret = qcril_mbn_db_add_hw_mbn_oem_version(file_name,
                                                   int_value);
                if (ret != E_SUCCESS)
                {
            ...
            if (ret == E_SUCCESS)
            {
                if(str_value)
                {
                    ret = qcril_mbn_db_add_hw_mbn_hw_name(file_name,
                                                       str_value);
                    if (ret != E_SUCCESS)
                    {
...
void qcril_qmi_hw_mbn_publish_update_status() {
    std::shared_ptr<HwMbnUpdateStatusInd> ind_msg_ptr =
        std::make_shared<HwMbnUpdateStatusInd>(true);
