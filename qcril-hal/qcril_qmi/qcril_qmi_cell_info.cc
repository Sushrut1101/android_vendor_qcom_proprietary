...
struct qcril_cell_info_chain
{
  const string name;
  unsigned prio;
  std::function<int(qcril_chain_data *fd)> process;
...
static struct qcril_cell_info_chain *chain_start = &v6;
#else
  ...
  if (ril_response && resp_len && ncells)
  {
    ...
    for (c = chain_start; c != NULL; c = c->next)
    {
      if(c->process)
      {
        if (c->process(&cd))
        {
          QCRIL_LOG_DEBUG(">>%s<< Successful", c->name.c_str());
          success = TRUE;
        }
      }
    }
...
)
{
  ...
  if (cell)
  {
    memset(cell->mcc, 0, sizeof(cell->mcc));
    memset(cell->mnc, 0, sizeof(cell->mnc));
    cell->lac = INT_MAX;
    cell->cid = INT_MAX;
    ...
    if (cid_valid && cid != 0xffffffff)
    {
        ...
        if (plmn != NULL)
        {
            ...
            snprintf( cell->mcc, NAS_MCC_MNC_MAX_SIZE, "%03d", (int)mcc);
            if ( (mnc > 99) ||  QCRIL_MNC_INCLUDES_PCS(plmn) )
            {
                snprintf (cell->mnc, NAS_MCC_MNC_MAX_SIZE, "%03d", (int)mnc);
            }
            else
            {
                snprintf (cell->mnc, NAS_MCC_MNC_MAX_SIZE, "%02d", (int)mnc);
            }
        }
    }
  }
...
)
{
  if (cell)
  {
    ...
    if (ss_valid)
    {
        ...
        convertWcdmaRscpFromQmi(cell->rscp,ss);
    }

    if (ecio_valid)
    {
        convertWcdmaEcioFromQmi(cell->ecio,ecio);
    }
...
int qcril_cell_location_to_ril_cell_info_v6(struct qcril_chain_data *data)
{
  ...
  if ( QCRIL_RIL_VERSION > 6 && data && data->input && data->parm1 && !data->output)
  {
    ...
    qcril_qmi_nas_convert_cell_location_to_ril_cellinfo(
      ...
      data->ret1.v.ui = (ncells * sizeof(RIL_CELL_INFO_LIST_TYPE));
      data->ret2.v.ui = ncells;
      data->output = output;
    ret = TRUE;
  }
  return ret;
