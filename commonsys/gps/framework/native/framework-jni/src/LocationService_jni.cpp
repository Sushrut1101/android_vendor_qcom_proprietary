...
#include <vendor/qti/gnss/2.1/ILocHidlAGnssCallback.h>
...
using ILocHidlAGnssV2_1 = ::vendor::qti::gnss::V2_1::ILocHidlAGnss;
...
Return<void> AGnssCallback::agnssStatusIpV4Cb(
    ...
    env->CallVoidMethod(mCallbacksObj,
                        ...
                        (int)agps_status.type,
                        (int)ILocHidlAGnssCallback::LocHidlApnTypeMask::APN_TYPE_MASK_SUPL,
...
struct LocHidlAGnssCallback : public ILocHidlAGnssCallback {
...
Return<void> LocHidlAGnssCallback::agnssStatusIpV6Cb(
    const ILocHidlAGnssCallback::AGnssStatusIpV6&) {
...
Return<void> LocHidlAGnssCallback::agnssStatusIpV4Cb(
...
Return<void> LocHidlAGnssCallback::locHidlAgnssStatusIpV6Cb(
    const ILocHidlAGnssCallback::LocHidlAGnssStatusIpV6&) {
...
Return<void> LocHidlAGnssCallback::locHidlAgnssStatusIpV4Cb(
...
static sp<ILocHidlAGnssV2_1>  agnssVendorIface2_1 = nullptr;
static sp<IAGnssCallback> agnssCb = nullptr;
static sp<ILocHidlAGnssCallback> agnssCb2_1 = nullptr;

static jboolean LocationService_init(JNIEnv* env, jobject obj);

class LocHidlLocationDeathRecipient : virtual public hidl_death_recipient
{
    static Mutex sGetInstanceLock;
    static sp<LocHidlLocationDeathRecipient> sLocHidlDeathRecipient;
    Mutex mLock;
    uint64_t mDeathCount;
    unordered_map<std::string, const std::function<void ()>> mRunnables;

    LocHidlLocationDeathRecipient() :
            hidl_death_recipient(), mLock(), mDeathCount(0), mRunnables() {
        registerRunnable("LocationService", []() {
                gnssVendorHal = nullptr;
                gnssNiVendorIface = nullptr;
                agnssVendorIface = nullptr;
                ALOGE("ILocHidlGnss died");
                LocationService_init(nullptr, nullptr);
        });
    }
public:
    static sp<LocHidlLocationDeathRecipient>& get() {
        Mutex::Autolock autoLock(sGetInstanceLock);
        if (sLocHidlDeathRecipient == nullptr) {
            sLocHidlDeathRecipient = new LocHidlLocationDeathRecipient();
        }
        return sLocHidlDeathRecipient;
    }

    // hidl_death_recipient interface
    virtual void serviceDied(uint64_t cookie, const wp<IBase>& /*who*/) override {
        ALOGE("%s]: died with cookie: %" PRIu64, __func__, cookie);
        Mutex::Autolock autoLock(mLock);
        for (auto item : mRunnables) {
            item.second();
            ALOGD("%s]: mRunnables[] - %s", __func__, item.first.c_str());
        }
    }

    uint64_t getDeathCount() { return mDeathCount; }

    void registerRunnable(std::string& who, const std::function<void ()>& runnable) {
        registerRunnable(std::move(who), runnable);
    }
    void registerRunnable(std::string&& who, const std::function<void ()>& runnable) {
        Mutex::Autolock autoLock(mLock);
        mRunnables.emplace(who, runnable);
        ALOGD("%s]: mRunnables[%zu] - %s", __func__, mRunnables.size(), who.c_str());
    }

    void deregisterRunnable(const std::string& who) {
        Mutex::Autolock autoLock(mLock);
        mRunnables.erase(who);
    }
};

Mutex LocHidlLocationDeathRecipient::sGetInstanceLock;
sp<LocHidlLocationDeathRecipient> LocHidlLocationDeathRecipient::sLocHidlDeathRecipient = nullptr;
...
static sp<ILocHidlGnss>& getVendorHal() {
    if (gnssVendorHal == nullptr) {
        ...
        if (vendorGnss == nullptr) {
            ...
            auto deathRecipient = LocHidlLocationDeathRecipient::get();
            auto r = gnssVendorHal->linkToDeath(deathRecipient, deathRecipient->getDeathCount());
            if (!r.isOk() || r == false) {
...
static sp<ILocHidlAGnss>& getAGnssVendorIface() {
    ...
    if (gnssVendorHalV2_1 != nullptr) {
        auto agnssVendor = gnssVendorHalV2_1->getExtensionLocHidlAGnss_2_1();
        if (!agnssVendor.isOk()) {
            ALOGE("Unable to get a handle to AGnssVendor extension !");
        }
        else {
            agnssVendorIface2_1 = agnssVendor;
            agnssVendorIface = agnssVendorIface2_1;
        }
    }
    else {
        auto vendorGnss = getVendorHal();
...
static jboolean LocationService_init(JNIEnv* env, jobject obj)
{
    ...
    auto agnssIface = getAGnssVendorIface();
    ...
    if (agnssVendorIface2_1 != nullptr) {
        if (agnssCb2_1 == nullptr) {
            agnssCb2_1 = new LocHidlAGnssCallback();
        }
        TO_HIDL_SERVICE();
        r = agnssVendorIface2_1->setCallbackExt(agnssCb2_1);
        if (!r.isOk()) {
            ALOGE("Error invoking HIDL API [%s]", r.description().c_str());
        }
    }
        ...
        jint agpsType, jstring apn, jint bearerType)
{
    ...
    auto iface = getAGnssVendorIface();
    if (iface == nullptr) {
...
extern int register_GnssConfigService(JNIEnv *env);
extern int register_WiFiDBReceiver(JNIEnv *env);
extern int register_WWANDBReceiver(JNIEnv *env);
extern int register_WWANDBProvider(JNIEnv *env);
extern int register_WiFiDBProvider(JNIEnv *env);
...
extern "C" jint JNI_OnLoad(JavaVM* vm, void* /*reserved*/)
{
    ...
    register_GnssConfigService(env);
    register_WiFiDBReceiver(env);
    register_WiFiDBProvider(env);
    register_WWANDBReceiver(env);
    register_WWANDBProvider(env);
    return JNI_VERSION_1_4;
}

void registerDeathRecipient(std::string who, const std::function<void ()>& runnable) {
    LocHidlLocationDeathRecipient::get()->registerRunnable(who, runnable);
}
...
void deregisterDeathRecipient(const std::string who) {
    LocHidlLocationDeathRecipient::get()->deregisterRunnable(who);
}
