...
public class UiccTerminalO extends Terminal {
    ...
    public UiccTerminalO(Context context, int slot) {
    ...
    protected int internalOpenLogicalChannel(byte[] aid, byte p2) throws CardException,
        ...
        for (int i = 1; i < channelId.length; i++) {
            if (channelId[i] == 0) {
                try {
                        response = manager.iccOpenLogicalChannel(subId[0], ByteArrayConverter.byteArrayToHexString(aid), p2int);
