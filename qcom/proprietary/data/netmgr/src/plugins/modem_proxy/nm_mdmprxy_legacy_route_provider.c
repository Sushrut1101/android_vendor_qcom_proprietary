...
{
  { "%s" NM_MDMPRXY_FLAGS " -t mangle -A nm_mdmprxy_pkt_marker -m mark --mark %u"
                          " -j nm_mdmprxy_pkt_forwarder",
    "%s" NM_MDMPRXY_FLAGS " -t mangle -D nm_mdmprxy_pkt_marker -m mark --mark %u"
                          " -j nm_mdmprxy_pkt_forwarder",
    nm_mdmprxy_markjump_formatter },

  { "%s" NM_MDMPRXY_FLAGS " -t mangle -A nm_mdmprxy_pkt_forwarder -j ACCEPT",
    "%s" NM_MDMPRXY_FLAGS " -t mangle -D nm_mdmprxy_pkt_forwarder -j ACCEPT",
...
static const struct nm_mdmprxy_cmd per_oiface_rule_list[] = {{0,0,0}};
...
static const struct nm_mdmprxy_cmd local_breakout_rule_list[] =
{
  ...
  { "%s" NM_MDMPRXY_FLAGS " -t mangle -I nm_mdmprxy_mngl_pre_tee -j TEE --gateway %s --oif %s",
...
{
  nm_mdmprxy_iptables_init(NM_MDMPRXY_MARK);
