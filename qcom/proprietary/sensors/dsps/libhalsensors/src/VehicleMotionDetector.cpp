/*============================================================================
  @file VehicleMotionDetector.cpp

  @brief
  VehicleMotionDetector class implementation.
...
/*============================================================================
  VehicleMotionDetector Constructor
============================================================================*/
VehicleMotionDetector::VehicleMotionDetector(int handle)
    :SAMSensor(handle)
{
    if(getAttribOK() == true) {
        svc_num = SNS_SAM_VMD_SVC_ID_V01;
        trigger_mode = SENSOR_MODE_EVENT;
        HAL_LOG_INFO("%s: handle:%d", __FUNCTION__, handle);
        setName("VMD");
        setVendor("QTI");
        setType(SENSOR_TYPE_MOTION_VEHICLE);
...
/*============================================================================
  VehicleMotionDetector Destructor
============================================================================*/
...
/*============================================================================
  enable
============================================================================*/
int VehicleMotionDetector::enable(int en)
{
    int err;
    ...
    if (enabled == en) {
        HAL_LOG_INFO("VMD is already enabled/disabled %d", enabled);
        return 0;
    }
...
===========================================================================*/
void VehicleMotionDetector::processResp(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    if (crsp_ptr->sns_result_t != 0 &&
        msg_hdr->msg_id != SNS_SAM_VMD_CANCEL_RESP_V01) {
    ...
    if(true != error ) {
        ...
        case SNS_SAM_VMD_ENABLE_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_VMD_ENABLE_RESP_V01", __FUNCTION__);
            instance_id = ((sns_sam_qmd_enable_resp_msg_v01 *)msg_ptr)->instance_id;
            break;
        case SNS_SAM_VMD_CANCEL_RESP_V01:
        case SNS_SAM_VMD_DISABLE_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_VMD_CANCEL/DISABLE_RESP_V01", __FUNCTION__);
            /* Reset instance ID */
            instance_id = 0xFF;
            break;
        case SNS_SAM_VMD_GET_ATTRIBUTES_RESP_V01:
            HAL_LOG_DEBUG("%s: Received SNS_SAM_VMD_GET_ATTRIBUTES_RESP_V01", __FUNCTION__);
...
/*===========================================================================
  FUNCTION:  processInd
===========================================================================*/
void VehicleMotionDetector::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    sns_sam_qmd_report_ind_msg_v01*  sam_vmd_rpt_ptr =
    ...
    switch( msg_hdr->msg_id )
    {
        case SNS_SAM_VMD_REPORT_IND_V01:
            HAL_LOG_DEBUG("%s: SNS_SAM_VMD_REPORT_IND_V01", __FUNCTION__);
            sensor_data.type = SENSOR_TYPE_MOTION_VEHICLE;
            sensor_data.sensor = HANDLE_MOTION_VEHICLE;
            sensor_data.data[0] = sam_vmd_rpt_ptr->state;
            timestamp = sam_vmd_rpt_ptr->timestamp;
            break;
        case SNS_SAM_VMD_ERROR_IND_V01:
            HAL_LOG_ERROR("%s: SNS_SAM_VMD_ERROR_IND_V01", __FUNCTION__);
            error = true;
            break;
        default:
    ...
    if ( error == false ) {
        ...
        HAL_LOG_VERBOSE( "%s: VMD: motion %f SAM TS: %u HAL TS:%" PRId64 " elapsedRealtimeNano:%" PRId64 "",
