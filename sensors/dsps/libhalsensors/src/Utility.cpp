...
const char * Utility::SensorTypeToSensorString(int SensorType)
{
    switch(SensorType){
        ...
        case SENSOR_TYPE_WRIST_TILT_GESTURE:
            return SENSOR_STRING_TYPE_WRIST_TILT_GESTURE;
#endif
...
/*============================================================================
  FUNCTION setExpireTime
    Get time from CLOCK_MONOTONIC and set expire time for response callback.
    Parameters:
    @timeout : the pthread wait time out time, 0 means wait forever
============================================================================*/
struct timespec Utility::setExpireTime(int timeout) {
    struct timespec expire_time;

    /* Calculate absolute expire time */
    clock_gettime(CLOCK_MONOTONIC, &expire_time);
    expire_time.tv_sec += timeout / 1000;
    expire_time.tv_nsec += ((timeout % 1000) * 1000000);
    /* calculate carry over */
    if (expire_time.tv_nsec >= 1000000000) {
                        ...
                        bool *cond_var)
{
    ...
    if (*cond_var == true) {
        ...
        if (0 != timeout) {
            expire_time = setExpireTime(timeout);
        }
...
/*============================================================================
  FUNCTION isAccelAvailable
    Check the accel sensor is available or not
============================================================================*/
bool Utility::isAccelAvailable()
{
    SensorsContext *context = SensorsContext::getInstance();
    return((context->getSensor(HANDLE_ACCELERATION) != NULL) ? true : false);
}
                     ...
                     hal_data_cb_t* data_cb, Sensor **mSensors)
{
    ...
    if ((rv == true) && (SENSOR_TYPE_META_DATA == data_ptr->type)
         && mSensors[data_ptr->meta_data.sensor] != NULL) {
    ...
    if ((rv == true) && mSensors[data_ptr->sensor] != NULL) {
        if (mSensors[data_ptr->sensor]->getFlags()&SENSOR_FLAG_WAKE_UP) {
           decrement_wake_events_in_queue_count(data_cb);
        }
    }
    return rv;
...
bool Utility::isDirectChannelTypeSupported()
{
    char prop_val[PROPERTY_VALUE_MAX] = {0};

    property_get(HAL_PROP_DIRECT_CHANNEL, prop_val, "false");
