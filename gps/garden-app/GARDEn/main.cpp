...
#if defined(USE_GLIB) && defined(FEATURE_AUTOMOTIVE)
...
void readConfigFile(char* path) {
    ...
    char networkInitResp[LOC_MAX_PARAM_STRING];
    char* tokenPtr;
    char* argPtr;
    ...
    strlcpy(sOptions.gpsConfPath, path, sizeof(sOptions.gpsConfPath));
...
static void printHelp(char **arg) {
     ...
     gardenPrint("    -D:  Delete All or not usage -D 0/1");
     gardenPrint("    -E:  EngineTypeMask SPE:Bit 0, DRE: Bit 1, PPE: Bit 2 eg -E engineTypeMask which is a bitmask of SPE|DRE|PPE. Use only with -D option \n"
                  "Usage: -E <engineMask> -D 0/1");
     gardenPrint("    -S:  (LE only) LocationClient API test. "
                 "-S should be the last in the argument line for auto start test case.\n"
                 "Usage: -i <interval> -l <numloops> -S <option>, which is bitmasks of"
                 " data|nmea|sv|loc. 15 for all.");
     gardenPrint("    -U:  (LE only) Redirect NMEA Strings to NMEA port, "
                 "-U can be used only in conjunction with the -S option.."
                 " Usage: -i <interval> -l <numloops> -U 1 -S <option>");
...
bool setRequiredPermToRunAsLocClient()
{
    ...
    if (0 == getuid()) {
        // For LE, we need to also add "diag" to existing groups for garden app
        // to run in loc client mode for diag logging.
        // We need to add diag grpid at end, so MAX+1
        gid_t gardenAppGrpsIds[LOC_PROCESS_MAX_NUM_GROUPS+1] = {};
        int i = 0, numGrpIds = 0;

        // Get current set of supplementary groups.
        memset(gardenAppGrpsIds, 0, sizeof(gardenAppGrpsIds));
        numGrpIds = getgroups(LOC_PROCESS_MAX_NUM_GROUPS, gardenAppGrpsIds);
        if(numGrpIds == -1) {
            LOC_LOGE("Could not find groups. ngroups:%d\n", numGrpIds);
        }
        else {
            gardenPrint("Curr num_groups = %d, Current GIDs: ", numGrpIds);
            for(i=0; i<numGrpIds; i++) {
                gardenPrint("%d ", gardenAppGrpsIds[i]);
            }
        }
        ...
        if (grp) {
            ...
            gardenPrint("Diag Group id = %d", gardenAppGrpsIds[numGrpIds]);
            numGrpIds++;
...
static GARDEN_RESULT parse_args(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv,
                    "r:l:t:u:hR:s:d:m:i:a:x:P:N:g:D:w:c:nye:o:z:A:B:T::Z:F:L:G:f:C:MKS:U:D:E:")) != -1) {
        switch (opt) {
        case 'r':
        ...
        case 'K':
            ...
            break;
        case 'L':
            strlcpy(sOptions.gpsConfPath, optarg, sizeof(sOptions.gpsConfPath));
        ...
        case 'C':
            ...
            break;
        case 'U':
            sOptions.routeToNmeaPort = atoi(optarg);
            gardenPrint("Redirect NMEA strings to NMEA Port:%d", sOptions.routeToNmeaPort);
            break;
        case 'D': {
            sOptions.deleteAll = atoi(optarg);
#ifdef USE_GLIB
            if ((sOptions.deleteAll != 0) && setRequiredPermToRunAsLocClient()) {
                LocationClientApiCase* clientApiCase = new LocationClientApiCase;
                clientApiCase->testDeleteAll(sOptions.engineTypeMask);
                sleep(2);
            }
            return GARDEN_RESULT_ABORT;
#else
            gardenPrint("-D option is for LE only. Ignored.");
            break;
#endif
        }
        case 'E':
            sOptions.engineTypeMask = atoi(optarg);
            break;
        case 'S': {
#ifdef USE_GLIB
                ...
                if (setRequiredPermToRunAsLocClient()) {
                    ...
                    clientApiCase->autoStartCases(
                            sOptions.interval, sOptions.l,
                            sOptions.useLocationClientApi,
                            sOptions.routeToNmeaPort);
                }
                return GARDEN_RESULT_ABORT;
#else
...
static GARDEN_RESULT menuTest() {
    ...
    // select api
    menuPrint("\n\n"
            "1: Use remote API mode\n"
            "2: Not remote API mode\n"
...
//Correction data framwork is only meant for LE targets with distro=auto
#ifdef FEATURE_AUTOMOTIVE
    if (p[0] == '3') {
        GARDEN_ADD(CDFWApiCase, "DGNSS Correction Data Menu",
            {
                CDFWApiCase* cdfwApiCase = (CDFWApiCase*)icase;
                cdfwApiCase->menuCase();
            });
        ret = GARDEN_RUN();
        GARDEN_DESTROY();
        return ret;
