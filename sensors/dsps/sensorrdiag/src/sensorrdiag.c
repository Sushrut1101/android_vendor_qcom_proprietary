                     ...
                     void *msg)
{
  ...
  {
    sns_rdiag_sensor1_delayed_rsp_msg *sensor1_delay_rsp = NULL;
    uint8_t encoded_arr[SNS_DIAG_RESP_MSG_MAX_LEN] = {0};
    ...
    uint8_t  rsp_cnt)
{
  ...
  else
  {
    ...
    if(1 == rsp_cnt)
    {
      // 1 denotes a final response to the SRT
      diagpkt_subsys_set_rsp_cnt(sns_diag_res, 1);

      // PEND: The code below is a workaround for a potential diag misbehavior
      // If the error message is seen, then we need to engage the diag team so
      // that they may fix it on their end.

      // Check the diag's delayed_resp_id to make sure it's also 0, if it isn't
      // then print out an error message and then force it to 0.
      diagpkt_subsys_delayed_rsp_id_type diag_dly_rsp_id =
          diagpkt_subsys_get_delayed_rsp_id(sns_diag_res);
      if (diag_dly_rsp_id != 0)
      {
        LOGE("%s: Unexpected diag delayed_rsp_id %u! Forcing to 0.", __func__,
            (unsigned int) diag_dly_rsp_id);
        diagpkt_subsys_reset_delayed_rsp_id(sns_diag_res);
      }
    }
