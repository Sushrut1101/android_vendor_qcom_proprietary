...
/*============================================================================
  MACRO DEFINES
  ==============================================================================*/
#define SNS_DIAG_TX_MODE_PROP "debug.vendor.sns.diag.tx.mode"
                                           ...
                                           )
{
  ...
  char sns_debug_config_file_name[255] = "";
  char *sns_debug_config_file_names[] = {SNS_DBG_CONFIG_FILE_NAME,
                                         SNS_DBG_CONFIG_FILE_NAME_LEGACY};
  ...
  for (int index = 0; index < 2; index++) {
    if (SNS_SUCCESS == sns_fsa_realpath(sns_debug_config_file_names[index])) {
      strlcpy(sns_debug_config_file_name, sns_debug_config_file_names[index], strlen(sns_debug_config_file_names[index]) + 1);
      break;
    }
  }

  /* Open and read the mask from file */
  if ((dbg_config_file_ptr = fopen(sns_debug_config_file_name,"r")) == NULL)
  {
    SNS_PRINTF_STRING_ERROR_0(DBG_MOD_INIT,
