...
typedef enum {
    ...
    /* Channel present, assignment unknown (for object audio) */
    QAP_AUDIO_PCM_CHANNEL_UNKNOWN = 35
} qap_pcm_chmap;
...
typedef enum {
    ...
    QAP_AUDIO_FORMAT_MAT,
    QAP_AUDIO_FORMAT_THD,
    QAP_AUDIO_FORMAT_PCM_32_BIT_FLOAT
} qap_audio_format_t;
...
/* Session Input module config */
typedef struct qap_input_config {
    qap_audio_format_t format;
...
typedef struct qap_input_buff_params {
    ...
    qap_input_config_t input_config;
    uint32_t bytes_consumed;  /* number of bytes consumed */
...
typedef enum {
    QAP_SESSION_PROCESS_SYNC,
    QAP_SESSION_PROCESS_ASYNC,
} qap_session_processing_type_t;
