...
/**_af_range_info:
*    @near_end: nearest end of the search range
*    @far_end: farthest end of the search range
**/
typedef struct _af_range_info_t {
  int near_end;
  int far_end;
} af_range_info_t;
...
/** _af_input_from_aec:
*    @frame_period:           frame period in ms. Inverse will give fps.
...
typedef enum _pdaf_mw_peripheral_window_t {
    PDAF_MW_TOP_LEFT_WINDOW = 0,
    PDAF_MW_TOP_CENTER_WINDOW,
    PDAF_MW_TOP_RIGHT_WINDOW,
    PDAF_MW_LEFT_WINDOW,
    PDAF_MW_RIGHT_WINDOW,
    PDAF_MW_BOTTOM_LEFT_WINDOW,
    PDAF_MW_BOTTOM_CENTER_WINDOW,
    PDAF_MW_BOTTOM_RIGHT_WINDOW,
} pdaf_mw_peripheral_window_t;
...
typedef enum {
    ...
    PDAF_MW_CENTRAL,
    PDAF_MW_PERIPHERAL
} pdaf_mw_windows_type_t;

/** pdaf_mw_selected_roi_t:
*
*    @type : central or peripheral
*    @roi_index : list of selected windows with similar depth
*    @selected_win_count: number of selected windows
*
* ROI configuration information for multiwindow PDAF
**/

typedef struct {
    pdaf_mw_windows_type_t type;
    int roi_index[MAX_PDAF_WINDOW];
    int selected_win_count;
} af_mw_selected_roi_t;
...
typedef struct {
   ...
   /* sensor output */
    ...
    pdaf_defocus_t peripheral[MAX_PERIPHERAL_WINDOWS];
    boolean is_single_valid;
    pdaf_defocus_t single[MAX_SINGLE_WINDOWS];
    boolean status;
} pdaf_update_t;
