...
static QPE_VIDEO_ERROR sem_timedwait()
{
 ...
 do
  ...
  if (result) {
    ERROR5("Failure in [%s:%d] result = %d errno =%d ", __func__, __LINE__, result,errno);
...
{
  mNativeWindowSurfaceFar = nullptr;
...
int VideoReceiver::DropFrameCheck(long long int av_ts, uint8* cvo_info,
                                  int8 *retreive_index)
{
  uint64_t av_tstamp = (uint64_t) av_ts;

  for (int i = 0; i < FRAME_STATE_SET_SIZE; i++) {
    DEBUG5_STR(
      "DropFrameCheck belongs_to_drop_set %d  frameStateIndex %d source_time_video_frame %lu, av_tstamp_in_us %llu",
      frameStateSet[i].belongs_to_drop_set, i,
      (unsigned long)frameStateSet[i].source_time_video_frame,
      (unsigned long long)frameStateSet[i].av_tstamp_in_us);

    if ((frameStateSet[i].av_tstamp_in_us == av_tstamp) ||
        ((frameStateSet[i].av_tstamp_in_us / 1000) == (av_tstamp / 1000))) {
      ...
      *retreive_index = i;
      ...
      if (frameStateSet[i].belongs_to_drop_set) {
        ...
        playoutInfo.system_time_render_video_frame =
          (frameStateSet[i].av_tstamp_in_us / 1000 +
           (getCurrentSystemTime(NULL) -
            frameStateSet[i].app_tstamp) );
      }
...
int VideoReceiver::AddToFrameStateMap(QP_MULTIMEDIA_FRAME* pFrameData,
                                      int belongs_to_drop_set, long long int ullCurSysTime)
{
  ...
  frameStateSet[nIdx].system_time_decode_video_frame = system_time_decode_video_frame;
  frameStateSet[nIdx].av_tstamp_in_us = pFrameData->ullFrameTimeUs;
  frameStateSet[nIdx].app_tstamp = pFrameData->ullSysTime;
  ...
  DEBUG6_STR("AddToFrameStateMap dropset %d seqno %d frameStateIndex %d source_time_video_frame %u, av_tstamp_in_us %llu",
             ...
             (unsigned long long)pFrameData->ullFrameTimeUs);
...
void VideoReceiver::SetNativeWindow(ANativeWindow* surfaceFar)
{
  PlayerMsg info;
  mNativeWindowSurfaceFar = surfaceFar;
  CRITICAL3_STR("mNativeWindowSurfaceFar = %p, m_ePlayerState %d",
                mNativeWindowSurfaceFar.get(), m_ePlayerState);
  surfaceChanged = true;
  ...
  if (VIDEO_INIT == m_ePlayerState && mNativeWindowSurfaceFar.get() ) {
    ...
    if ((mNativeWindowSurfaceFar.get() == NULL) &&
      ...
      m_ePlayerState = VIDEO_CODEC_PENDING;
      CRITICAL2("Move to configured state, m_ePlayerState %d", m_ePlayerState);
      memset(&info, 0, sizeof(PlayerMsg));
...
void VideoReceiver::RenderFrame(void* data)
{
  ...
  if (iPlayerState == eVideoStateActive && !m_codecConfig.iErrorState) {
    ullCurTime = getCurrentSystemTime(NULL);
    DEBUG4_STR("TIMEDELAY::VideoDecFillBuffer call frame queuebuffer = %d, time = %llu, render frame = %d",
               tempfillbuffercallbackcount + 1, (unsigned long long)ullCurTime,
               m_RenderFrameCount);
    ...
    if (tCodecConfig.eCodec == VIDEO_CODEC_H263) {
      ...
      if (err != OK) {
        ERROR2("VideoReceiver::Configure:: Error getting the decoder component handle for H263 codec Err Nor %d\n",
               err);
        iPlayerState = eVideoStateNotReady;
        ERROR2("PlayerStateTransition:Player:VIDEO_MSG_ERROR:%d",
               iPlayerState);
...
void VideoReceiver::ReleaseMediaBuffersAfterReconfigure()
{
  ...
  ullCurTime = getCurrentSystemTime(NULL);
  CRITICAL2_STR("TIMEDELAY::eVideoStateReConfiguring end time = %llu",
                (unsigned long long)ullCurTime);
    ...
    IOMX::buffer_id buffer)
{
  ...
  uint64_t ullCurTime = getCurrentSystemTime(NULL);
  CRITICAL4_STR("Profiling Decoder->IMS EmptyBufferDone frame = %d, appstime = %lld buffer=%d",
                tempemptybuffercallbackcount, (long long)ullCurTime, buffer);
    ...
    /*, OMX_PTR platform_private,OMX_PTR data_ptr, OMX_U32 pmem_offset*/)
{
  ...
  uint64_t ullCurTime = getCurrentSystemTime(NULL);
  CRITICAL5_STR("Profiling Decoder->IMS FillBufferDone frame = %d, appstime = %lld buffer=%d FBD Time - %llu",
                tempfillbuffercallbackcount,
                (long long)ullCurTime, buffer, timestamp);
    ...
    for (; i < m_iMinInputBufCount; i++) {
      if (m_pDecInpBufHeader_iomx[i]) {
        if (m_pDecInpBufHeader_iomx[i]->filllen == 0) {
          uint64_t ullCurTime = getCurrentSystemTime(NULL);
          DEBUG2_STR("TIMEDELAY::emptyBuffer start time = %lld",
                     (long long)ullCurTime);
...
QPE_VIDEO_ERROR VideoReceiver::EnablePort(uint8_t iPort)
{
  ...
  for (i = 0; i < portFmt.nBufferCountMin; i++) {
    ...
    CRITICAL4("Profiling IMS->Decoder EnablePort FillBuffer call frame = %d, appstime = %lu buffer=%d ",
              tempfillbuffercallbackcount + 1, (unsigned long)ullCurTime,
              m_pDecOutpBufHeader_iomx[i]->buffer_id);
...
QPE_VIDEO_ERROR VideoReceiver::TearDown()
{
  ...
  if (mNativeWindowSurfaceFar.get()) {
    m_ePlayerState = VIDEO_SURFACE_RECEIVED;
  } else {
    m_ePlayerState = VIDEO_INIT;
  }
  ...
  if (iVolLen > 0 && iVolLen < VIDEOOMX_MAX_VOL_LENGTH  && pVolFrame != NULL) {
    for (uint8_t i = 0; i < m_iMinInputBufCount; i++) {
      if (m_pDecInpBufHeader_iomx[i]->filllen == 0) {
        ...
        uint64_t ullCurTime = getCurrentSystemTime(NULL);
        CRITICAL3_STR(
          "TIMEDELAY::IOMXVideoDecEmptyBuffer call frame = %d, time = %lld",
          tempemptybuffercallbackcount + 1,
          (long long)ullCurTime);
        ...
        int error = m_NodeId->emptyBuffer(m_pDecInpBufHeader_iomx[i]->buffer_id,
                                          InputBuff, 0, ullCurTime, -1);
