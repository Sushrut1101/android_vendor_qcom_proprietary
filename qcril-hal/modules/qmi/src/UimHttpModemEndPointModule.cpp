...
)
{
  ...
  me->onQmiUnsolIndCb(user_handle, msg_id, (unsigned char *)ind_buf, ind_buf_len);
}
...
)
{
  ...
  Log::getInstance().d("qmi_uim_http_request_cb msg_id: " + std::to_string(msg_id));
  /* Wrapper for member method*/
...
)
{
  ...
  if (qmi_http_ind_ptr->url_valid)
  {
    ...
    QCRIL_LOG_INFO("QMI-> UimHttpEnd URL: %s", http_trasaction_ind_ptr->url);
  }
  /* Copying headers */
  if (qmi_http_ind_ptr->headers_valid)
  {
        ...
        index++)
    {
      ...
      QCRIL_LOG_INFO("index: %d custom_header name: %s value: %s ", index,
                      http_trasaction_ind_ptr->headers.custom_header[index].name,
                      http_trasaction_ind_ptr->headers.custom_header[index].value);
    }
    /*Assign the incremented value of index to indicate headers count being actually copied*/
    http_trasaction_ind_ptr->headers.custom_header_len = index;
  }
...
)
{
  ...
  if(http_req_ptr->headers_valid)
  {
                          ...
                          header_index < UIM_HTTP_CUST_HEADER_MAX_COUNT; header_index++)
    {
      ...
      QCRIL_LOG_INFO("header_index: %d header name: %s value: %s ", header_index,
                      qmi_http_req_ptr->headers[header_index].name,
                      qmi_http_req_ptr->headers[header_index].value);
    }
  }
