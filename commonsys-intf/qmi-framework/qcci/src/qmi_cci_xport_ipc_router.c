...
void qmi_cci_xport_ipcr_deinit(void)
{
...
static int open_lookup_sock_fd(void)
{
  ...
  if (lookup_sock_fd < 0)
  {
    lookup_sock_fd = socket(AF_MSM_IPC, SOCK_DGRAM | SOCK_CLOEXEC, 0);
    if (lookup_sock_fd < 0)
    {
...
static int qmi_cci_xport_ctrl_port_init(void)
{
  ...
  ctrl_port->ctl_fd = socket(AF_MSM_IPC, SOCK_DGRAM | SOCK_CLOEXEC, 0);
 ...
 )
{
  ...
  xp->fd = socket(AF_MSM_IPC, SOCK_DGRAM | SOCK_CLOEXEC, 0);
  if(xp->fd < 0)
  {
